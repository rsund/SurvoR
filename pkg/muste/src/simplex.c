...
static int simplx(double *A, int m, int n, int mp, int np, int m1, int m2, int m3, int *izrov, int *iposv);
static int simp1(double *A, int mp, int np, int mm, int *ll, int nll, int iabf, int *pkp, double *pbmax);
static int simp2(double *A, int m, int n, int mp, int np, int *l2, int nl2, int *pip, int *pkp, double *pq1);
...
static int mat_del_row(int k, double *A, int m, int n, char *rlab, char *clab, int lr, int lc);
static int mat_del_col(int k, double *A, int m, int n, char *rlab, char *clab, int lr, int lc);
...
void muste_simplex(char *argv) {
    ...
    m3 = atoi(word[4]);
    if (mb - 1 != m1 + m2 + m3) {
        muste_sprintf(sbuf, "\n# of constraints %d+%d+%d not equal to row%s-1=%d", m1, m2, m3, word[1], mb - 1);
        sur_print(sbuf);
        WAIT;
        return;
    }
    ...
    */
    ...
    case 0:
        ...
        break;
    case -1:
        eoutput("No feasible solution");
        WAIT;
        break;
    case -2:
        eoutput("Unbounded objective function");
        WAIT;
        break;
    case -3:
        not_enough_memory();
        return;
    case -4:
        eoutput("Bad input constraints count!");
        WAIT;
        return;
    case -5:
        eoutput("Bad input tableau: Negative values in the first column!");
        WAIT;
        break;
    }
    output_close(eout);
...
static int simplx(double *A, int m, int n, int mp, int np, int m1, int m2, int m3, int *izrov, int *iposv) {
    ...
    if (m != m1 + m2 + m3) {
        return (-4); /* bad input constraints counts */
    }
    nl1 = n;
    for (k = 1; k <= n; ++k) {
        l1[k - 1] = k;
...
L10:
    ...
    */
    ...
    if (ip == 0) {
        return (-2); /* unbounded objective function */
    }
L1:
...
static int simp1(double *A, int mp, int np, int mm, int *ll, int nll, int iabf, int *pkp, double *pbmax) {
    ...
    *pbmax = A[mm + mp * (*pkp)];
    ...
    for (k = 2; k <= nll; ++k) {
        if (iabf == 0) {
            test = A[mm + mp * ll[k - 1]] - *pbmax;
        } else {
            test = fabs(A[mm + mp * ll[k - 1]]) - fabs(*pbmax);
        }
        if (test > 0.0) {
            *pbmax = A[mm + mp * ll[k - 1]];
            *pkp = ll[k - 1];
        }
    }
    return (1);
...
static int printout() {
    ...
    muste_sprintf(x, "Simplex solution for Linear Programming problem %s:", word[1]);
    eoutput(x);
    muste_sprintf(x, "Number of variables %d", m);
    eoutput(x);
    muste_sprintf(x, "Number of '<' constraints %d", m1);
    eoutput(x);
    muste_sprintf(x, "Number of '>' constraints %d", m2);
    eoutput(x);
    muste_sprintf(x, "Number of '=' constraints %d", m3);
    eoutput(x);
    fnconv(A[0], accuracy, s);
    muste_fieldcopy(t, rlab, lr);
    p = t + lr - 1;
    while (p > t && *p == ' ') {
        *p = EOS;
        --p;
    }
    muste_sprintf(x, "Max.value=%s of %s obtained for: (SIMPLEX.M)", spois(s), spois(t));
    eoutput(x);
    for (i = 0; i < n; ++i) {
        for (k = 0; k < m; ++k) {
            if (iposv[k] == i + 1) {
        ...
        fnconv(y, accuracy, s);
        xx[i] = y;
        muste_sprintf(x, " %.*s %s", lc, clab + (i + 1) * lc, s);
        eoutput(x);
    }
    matrix_save("SIMPLEX.M", xx, n, 1, clab + lc, "simplex ", lc, 8, -1, "Simplex_solution", 0, 0);

    strcpy(x, "Values of slack variables: (SLACK.M)");
    eoutput(x);
    ...
    if (m1 + m2) {
        for (k = m; k < m + m1 + m2; ++k) {
            ...
            fnconv(y, accuracy, s);
            muste_sprintf(x, " %.*s %s", lr, rlab + (k + 1 - m) * lr, s);
            eoutput(x);
        }
        matrix_save("SLACK.M", yy, m1 + m2, 1, rlab + lr, "slack   ", lr, 8, -1, "Slack_values", 0, 0);
        //        muste_free(yy);
    }
    //     muste_free(xx);
    save_tableau();
...
static int save_tableau() {
    ...
    for (i = 0; i < m; ++i) {
        k = iposv[i];
        if (k > n) {
            cop(rlab2, i + 1, rlab, k - n);
        } else {
            cop(rlab2, i + 1, clab, k);
        }
    }
    ...
    for (j = 0; j < n; ++j) {
        k = izrov[j];
        if (k <= n) {
            cop(clab2, j + 1, clab, k);
        } else {
            if (k > n + m1 + m2) {
                cop(clab2, j + 1, lab0, 0);
            } else {
                cop(clab2, j + 1, rlab, k - n);
            }
        }
    }
    ...
    matrix_save("TSIMPLEX.M", A, mp - 1, n0, rlab2, clab2, 8, 8, -1, "Simplex_Output_Tableau", 0, 0);
    return (1);
}
