...
static char *specs0[] = {"MAXFIELDS",   "PRIND", "FIRST",  "LAST",     "NAMES",     "FILTER",
                         "MISSING",     "SKIP",  "MODE",   "FORMAT",   "DELIMITER", "LIMIT",
                         "SKIP_ERRORS", "MATCH", "VARLEN", "NEWSPACE", "NOFIELDS",  "REMOVE_QUOTES",
...
static void sanatilaylitys(int maxtila) {
    muste_sprintf(sbuf, "\nToo long record in text file (max length =%d  8*ep4)", maxtila);
    sur_print(sbuf);
    WAIT;
}
...
static int sa_missing(char *p) {
    ...
    len = strlen(p);
    if (len < 1) {
        return (1); // RS ADD 11.3.2013
    }
    if (*cmissing) /* MISSING=<string> */
    {
        if (strstr(p, cmissing) != NULL) {
            return (1);
        }
        return (0);
    }
...
static int sasplit2(FILE *text, char *sana[], int max, char *sanatila, int maxtila, char *erotin[], int pituus[],
                    unsigned char code[]) {
    char *p;
    int i, h;
    char merkki;
        ...
        *p = EOS;
        ...
        if (pituus[i] > 0) {
            for (h = 0; h < pituus[i]; ++h) {
                ...
                if (p - sanatila > maxtila - 1) {
                    sanatilaylitys(maxtila);
                    return (-3);
                }
                *p++ = merkki;
            }
        ...
        while (1) {
            ...
            if ((*erotin[i] == EOS && merkki == ' ') || strchr(erotin[i], merkki) != NULL || merkki == '\t') {
                if (*sana[i] == EOS) {
                    continue;
                }
                break;
...
static int sasplit(FILE *text, char *sana[], int max, char *sanatila, int maxtila, char *erotin[], int pituus[],
                   unsigned char code[]) {
    char *p;
    int i, h;
    char merkki;
    ...
    for (i = 0; i < max; ++i) {
        ...
        *p = EOS;
        ...
        while (1) {
            ...
            if (merkki == ' ' || strchr(erotin[i], merkki) != NULL || merkki == '\t') {
                if (*sana[i] == EOS) {
                    continue;
                }
                break;
...
static void format_error() {
    ...
    muste_sprintf(sbuf, "\nFormat error in text file %s:", word[2]);
    sur_print(sbuf);
        ...
        if (*names == EOS && j == l3) {
            for (; j <= l4; ++j) {
                ...
                if (strlen(names) + strlen(jakso) > NL * LLENGTH) {
                    muste_sprintf(sbuf, "\nMore than %d bytes on NAMES lines %ld,%ld !", NL * LLENGTH, l3, l4);
                    sur_print(sbuf);
                    WAIT;
                    sulje();
                    return (-1);
...
static int match_copy() {
    ...
    prind_count = 0;
    muste_sprintf(sbuf, "\n%d active fields to be copied", m_act);
    sur_print(sbuf);
    muste_sprintf(sbuf, "\nCopying records from %s to %s:", word[2], word[3]);
    sur_print(sbuf);
    ...
    for (j = l1; j <= l2; j += (long)ii) {
        if (!muoto) {
            ...
            i = strlen(jakso);
            while (jakso[i - 1] == '\n' || jakso[i - 1] == '\r') {
                jakso[--i] = EOS; // RS ADD \r
            }
            if (koodi) {
                conv((unsigned char *)jakso, code); // RS ADD
            } else if (*encoding) {
                muste_iconv(jakso, "CP850", encoding);
                muste_iconv(skip, "CP850", encoding);
            } // RS 2.5.2013
            if (nskip) {
                skip_char(jakso, skip); // RS ADD
            }
            if (muste_quotes) // RS ADD
            {
                if (fixed_delimiter) {
                    k = split_by_char_quotes(jakso, tsana, m, limit_char);
                } else {
                    k = split_quotes(jakso, tsana, m);
                }
            } else {
                /* 16.3.1996 */ if (fixed_delimiter) {
                    k = split_by_char(jakso, tsana, m, limit_char);
                } else {
            ...
            /*      k=split(jakso,tsana,m);  */
            if (k < m) {
                muste_sprintf(sbuf, "\nNot enough fields on line %ld in text file %s", j, word[2]);
                sur_print(sbuf);
                WAIT;
                sulje();
                return (-1);
        ...
        while (1) {
            ++j2;
            if (j2 > d2.n) {
                muste_sprintf(sbuf, "\nRecord #%ld of %s not found in %s", j, word[2], word[3]);
                sur_print(sbuf);
                if (match_var >= 0) {
                    sur_print("\nMATCH value is ");
                    muste_sprintf(sbuf, "%s", tsana[match_var]);
                    sur_print(sbuf);
                }
                WAIT;
                sulje();
...
/* j vain virheilm.varten */
{
    ...
    if (koodi) {
        conv((unsigned char *)jakso, code);
    } else if (*encoding) {
        muste_iconv(jakso, "CP850", encoding);
        muste_iconv(skip, "CP850", encoding);
    } // RS 2.5.2013
    if (nskip) {
        skip_char(jakso, skip);
    }
    ...
    if (k < m && !skip_errors) {
        muste_sprintf(sbuf, "\nNot enough fields on line %ld in text file %s (%d<%d)", j, word[2], k, m);
        sur_print(sbuf);
        WAIT;
        sulje();
        return (-1);
...
static int tutki_perusmuoto() {
    ...
    i = lue_seuraava_rivi(1L, jakso, tsana);
    if (i < 0) {
        return (-1);
    }
    for (i = 0; i < m; ++i) {
        if (muste_isnumber_dec(tsana[i], muste_dec)) {
            return (1);
        }
    }
    i = lue_seuraava_rivi(2L, jakso, tsana);
    if (i < 0) {
        return (-1);
    }
    for (i = 0; i < m; ++i) {
    ...
    if (i == m) {
        return (1); /* 2.rivissÑ ei lukuja */
    }
    l1 = 2L;
    l3 = 1L;
    rewind(text);
    i = lue_seuraava_rivi(1L, names, varname);
    if (i < 0) {
        return (-1);
    }
    return (1);
...
static int edit_varnames(char **varname, int m) {
    ...
    nimimax2 = NIMIMAX2;
    if (max_varlen + 32 > nimimax2) {
        nimimax2 = max_varlen + 32;
    }
...
static int tutki_textdata() {
    ...
    */
    ...
    if (i >= 0) {
        check_varnames = atoi(spb[i]);
    }

    if (check_varnames) {
        i = edit_varnames(varname, m_act);
        if (i < 0) {
            return (-1);
        }
    }
    ...
    for (j = l1; j <= l2; j += (long)ii) {
        if (!muoto) {
            ...
            if (n_muste_numsep) {
                skip_char(jakso, muste_numsep); // RS 11.3.2013
            }
        ...
        for (i = 0; i < m_act; ++i) {
            ...
            strcpy(jakso, jakso2);                       // RS 2018-12-14
                                                         /*       if (k<0) return(-1);     */
            if (strlen(jakso) < 1 || sa_missing(jakso)) {
                continue;
            }
            if (tyyppi[i] == 2) {
                len = strlen(jakso);
        ...
        if (check_varnames) // 10/2009
        {
            ...
            */
            ...
            if (strlen(varname[i]) <= 8) { // RS ADD
                ...
                k = snprintf(p, NIMIMAX, "%s %s", varname[i], x);
            }
...
static int luo_uusi() {
    ...
    */
    muste_sprintf(sbuf, "\nSince Survo data file %s does not exist,", word[3]);
    sur_print(sbuf);
    sur_print("\ncreating a new one...");
    i = varaa_tilat2();
    if (i < 0) {
    ...
    fim = m_act;
    for (i = 0; i < fim; ++i) {
        muste_fieldcopy(vartype + i * 9, space, 8);
        vartype[i * 9 + 8] = EOS;
        vartype[i * 9 + 1] = 'A';
        if (tyyppi[i] == 2) {
            vartype[i * 9 + 0] = 'S';
            ...
            if (varlen[i] > max_varlen) {
                muste_sprintf(sbuf, "\nThe length of the field %s (%d) is more than %d.", varname[i], varlen[i],
                              max_varlen);
                sur_print(sbuf);
                muste_sprintf(sbuf, "\nYou may increase the limit by specification VARLEN=%d", varlen[i]);
                sur_print(sbuf);
    ...
    fitextn = 1;
    fitextlen = c2;
    strcpy(jakso, " Copied from text file ");
    strcat(jakso, word[2]);
    privi[0] = jakso;
    fitext = privi;
    i = fi_create(word[3], filen, fim1, fim, 0L, fil, fiextra, fitextn, fitextlen, fitext, varname, varlen, pvartype);
    if (i < 0) {
        return (-1);
    }
    data_open(word[3], &d2);
    ...
    *p = EOS;
    ...
    if (i >= 0) {
        ...
        while (1) {
            ...
            if (strncmp(sana[0], "FIELDS", 6) == 0 && strcmp(sana[1], spb[i]) == 0) {
                ...
                edread(x, rivi);
                k = split(x + 1, sana, 4);
                if (k == 4 && muste_strcmpi(sana[3], "LF") != 0) {
                    continue;
                }
    ...
    if (rivi <= r2) {
        while (1) {
            ...
            i = atoi(sana[0]);
            if (i != m + 1) {
                muste_sprintf(sbuf, "\nError on edit line %d:", rivi);
                sur_print(sbuf);
                muste_sprintf(sbuf, "\nField # %d expected!", m + 1);
                sur_print(sbuf);
                WAIT;
                return (-1);
            }
            ...
            if (*sana[1] != '-' || sana[1][1] != EOS) {
                ...
                varname[m_act - 1] = p;
                q = sana[1];
                if (p - nimitila > 8 * ep4 - strlen(q) - 1) {
                    nimiylitys();
                    return (-1);
                }
                while (*q) {
                ...
                *p++ = EOS;
                ...
                for (i = 0; i < m_act - 1; ++i) /* 28.4.1997 */
                {
                    if (strcmp(varname[i], varname[m_act - 1]) == 0) {
                        muste_sprintf(sbuf, "\nField name `%s' at least twice in the list!", varname[i]);
                        sur_print(sbuf);
                        WAIT;
                        return (-1);
                    }
            ...
            if (k > 2) {
                ...
                if (i == 0) {
                    pituus[m] = 0;
                    q = y;
                    if (p - nimitila > 8 * ep4 - strlen(y) - 1) {
                        nimiylitys();
                        return (-1);
                    }
                    while (*q) {
    ...
    if (m == 0) {
        i = etsi_rivi(modelline);
        if (i < 0) {
            return (-1); // RS 12.8.2013 l1 -> modelline
        }
        p = muste_fgets(jakso, NL * LLENGTH, text);
        if (p == NULL) {
            return (-1);
        ...
        i = strlen(jakso);
        while (jakso[i - 1] == '\n' || jakso[i - 1] == '\r') {
            jakso[--i] = EOS; // RS ADD CHA 11.3.2013 =='\n' -> <32 || jakso[i-1]==limit_char
        }
        if (koodi) {
            conv((unsigned char *)jakso, code); // RS ADD
        } else if (*encoding) {
        ...
        {
            if (fixed_delimiter) {
                m = split_by_char_quotes(jakso, tsana, ep4 + 1, limit_char);
            } else {
                m = split_quotes(jakso, tsana, ep4 + 1);
            }
        } else {
            /* 16.3.96*/ if (fixed_delimiter) {
                m = split_by_char(jakso, tsana, ep4 + 1, limit_char);
            } else {
                m = split(jakso, tsana, ep4 + 1); /* +1: 12.1.92 */
            }
        }
        ...
        if (*names) {
            if (koodi) {
                conv((unsigned char *)names, code); // RS ADD
            } else if (*encoding) {
                muste_iconv(names, "CP850", encoding);
                muste_iconv(skip, "CP850", encoding);
            } // RS 2.5.2013
            if (nskip) {
                skip_char(names, skip); // RS ADD
            }

            if (muste_quotes) // RS ADD
            {
                if (fixed_delimiter) {
                    i = split_by_char_quotes(names, varname, ep4, limit_char);
                } else {
                    i = split_quotes(names, varname, ep4);
                }
            } else {
                /* 16.3.96*/ if (fixed_delimiter) {
                    i = split_by_char(names, varname, ep4, limit_char);
                } else {
                    i = split(names, varname, ep4);
                }
            }
    ...
    *p = EOS;
    ...
    if (i >= 0) {
        ...
        while (1) {
            ...
            if (strncmp(sana[0], "FIELDS", 6) == 0 && !muste_nofields) {
                break; // RS ADD muste_nofields
            }
            ++rivi;
        }
    }
    ...
    while (rivi <= r2) {
        ...
        *p++ = EOS;
        ...
        erotin[m] = p2;
        q = sana[2]; /* erotin=prefix_text */
        if (p2 - sanatila > 8 * ep4 - strlen(q) - 1) {
            nimiylitys();
            return (-1);
        }
        while (*q) {
    ...
    for (i = 0; i < m; ++i) {
        ...
        v[i] = k;
        if (muste_strcmpi(erotin[i], "#START") == 0) {
            name_field = k;
        }
    }
    return (1);
        ...
        for (i = 0; i < m; ++i) {
            ...
            if (v[i] == name_field) {
                ...
                if (q == NULL) {
                    ...
                    muste_fieldcopy(x, jakso, q - jakso);
                    x[q - jakso] = EOS;
                    p0 = q;
                }
            } else {
...
static int format_save(char *format) {
    ...
    i = split(x, osa, 3);
    if (i < 2) {
        sur_print("\nError in FORMAT - check specification!");
        WAIT;
        return (-1);
    }
    if (i > 1 && strncmp(osa[0], "WORD", 4) != 0 && strncmp(osa[0], "CHAR", 4) != 0) {
        sur_print("\nUnknown FORMAT - check specification!");
        WAIT;
        return (-1);
    }
...
void muste_file_save(int argc, char *argv[]) {
    ...
    *names = EOS;
    ...
    i = spfind("NAMES");
    if (i >= 0) {
        if (muste_strcmpi(spb[i], "DEFAULT") == 0) {
            suora_siirto = 1;
        } else {
            strcpy(x, spb[i]);
            i = split(x, sana, 2);
            ...
            if (l4 >= l1 || l4 < 0L) {
                sur_print("\nNAMES line(s) not before FIRST line!");
                WAIT;
                return;
            }
            perusmuoto = 0;
    ...
    code[256] = EOS;
    *encoding = EOS; // RS 2.5.2013
    i = spfind("ENCODING");
    if (i >= 0) {
        strcpy(encoding, spb[i]);
    }

    i = spfind("MISSING");
    if (i >= 0) {
        strcpy(cmissing, spb[i]);
    } else {
        *cmissing = EOS;
    }
    ...
    i = spfind("DEC");
    if (i >= 0) {
        muste_dec = *spb[i]; // RS 11.3.2013
    }

    muste_nofields = 0; // RS ADD
    i = spfind("NOFIELDS");
    if (i >= 0) {
        muste_nofields = atoi(spb[i]);
    }

    muste_quotes = 0; // RS ADD
    i = spfind("REMOVE_QUOTES");
    if (i >= 0) {
        muste_quotes = atoi(spb[i]);
    }
    ...
    */
    ...
    for (j = l1; j <= l2; j += (long)ii) {
        ...
        */
        ...
        ++j2;
        if (j2 > d2.n) {
            d2.n = j2; /* fi_save vaatii j2<=d2.n  */
        }
        if (d2.m > m_act) {
            fi_miss_obs(&d2.d2, j2);
        }
        ...
        for (i = 0; i < m_act; ++i) {
            ...
            if (d2.vartype[v2[i]][0] == 'S') {
                strcpy(jakso2, tsana[vi]); /* ennen 2.4.91 jakso */
                                           // Rprintf("\nv[i]: %d, v2[i]: %d, jakso2: %s, strlen: %d varlen:
                                           // %d",v[i],v2[i],jakso2,strlen(jakso2),d2.varlen[v2[i]]);
                ...
                if (sa_missing(tsana[vi])) {
                    ...
                    if (n_muste_numsep) {
                        skip_char(tsana[vi], muste_numsep); // RS 11.3.2013
                    }
                    xx = atof(tsana[vi]);
