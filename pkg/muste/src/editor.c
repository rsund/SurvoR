...
static int init_shadow_codes()
        ...
        if (shad_nr>1) { muste_sprintf(shad_name,"shadows%d",shad_nr);
}
        ...
        if (edfield!=NULL)
                ...
                if (muste_strcmpi(edfile,eopen)!=0 && etu!=2 && save_warning)
                    ...
                    muste_sprintf(sbuf,"\nEdit file %s already exists! Overwrite it (Y/N)?",
                                                                       edfile);
                                sur_print(sbuf);
                    ch=(char)nextch_editor(); muste_sprintf(sbuf,"%c",ch); sur_print(sbuf);
...
//  col=atoi(parm[4])+1;   - 13.12.2011
   ...
   for (j=j1; j<=j2; ++j)
       ...
       for (i=0; i<col2-col1+1; ++i) { sbuf[i]=ch; 
}sbuf[i]=EOS;
...
static int shadow_set()
   ...
   if (!hard)
       {
       len=strlen(xs)-1; while (xs[len]==' ') { --len; 
}++len;
       for (j=j1; j<=j2; ++j)
           {
           if (zs[j]==0)
...
static int display_colors()
        ...
        if (muste_strcmpi(parm[1],"#ALL")==0 || muste_strcmpi(parm[1],"ALL")==0)
            ...
            for (j=0; j<8; ++j)
                ...
                for (i=0; i<16; ++i)
                    {
                    color=16*j+i; muste_sprintf(x,"%3d",color);
...
/*      CURSOR_OFF;             */
        if (rsh!=j) { muste_sprintf(x,"%6u ",j); } else { muste_sprintf(x,"Shadow:");
}
...
static int disp_rbuf() // RS 25.3.2013
    ...
    while (1)
        ...
        muste_sprintf(sbuf,".muste.highlight('%s',%d,%d,%d,%d,%d,%d)",out,firstvisible,lastvisible,
...
void pyyhi_alarivi()
        {
        LOCATE(r3+2,1); ERASE; /* muste_sprintf(sbuf,"%.*s",c3+6,space); sur_print(sbuf); */
        }
...
void line_merge()
        ...
        if (empty(x+c1+c-1,c2-c1-c+2))
            ...
            if (len>c2-c1-c+2) { if (shad) { shadt(j,j+1); 
}return; }
...
// RS REM        extern int ref_r1,ref_r;
        ...
        soft_disp(0);
        cursor(r,c);
        PR_EBLK; muste_sprintf(sbuf,"%c",(char)PREFIX); sur_print(sbuf);
...
static char *muste_get_savename() // RS 1.5.2014
    ...
    if (plukum<2) { return(muste_nullname);
}
    muste_strupr(para[0]);    
    if (strcmp(para[0],"SAVE")!=0) { return(muste_nullname);
}
    return(para[1]);    
    }
...
int goto_control_char(int k) /* 5.4.1999 */
        ...
        muste_sprintf(rivi1,"%d",j0); parm[1]=rivi1;
        muste_sprintf(rivi2,"%d",j); parm[2]=rivi2;
        op_goto2(g,parm);
        c=1; disp();
        return(1);
        }
...
void b_line_merge(int j,int col)    /* norm. col=c1+c-1 */
        ...
        if (empty_line(x+col,c2-col+1))
            ...
            if (len>c2-col+1) { if (shad) { b_shadt(j,j+1); 
}return; }
...
void autotrim(int rivi1,int rivi2)  // RS pois?
        ...
        if (i>1) { k+=muste_sprintf(y+k," %s",sana[1]);
}
        if (i>2) { k+=muste_sprintf(y+k," %s",sana[2]);
}
        b_insertl(rivi1-1);
        edwrite(y,rivi1,1);
        activate();
...
// RS ALT            muste_sprintf(prompt_line,"Mark %s 1 by %s. DEL=Cancel%s",sana1,sana4,sbuf); // RS
            muste_sprintf(sbuf,"%s",prompt_line); sur_print(sbuf);
            move_ind=1;
            break;
          case 1:
...
int m_copy_word1() // poimii talteen hiiren oikeanpuoleisella napilla
   ...
   edread(x,rr);
   p=x+cc-1; while (p>x && *p!=' ') { --p; 
}++p;
   q=x+cc+1; while (q<x+c2-1 && *q!=' ') { ++q; 
}--q;
...
//   sur_get_mouse_position(&c0,&r0);
   ...
   if (x[c0]!=' ')
       {
       p=x+c0-1; while (p>x && *p!=' ') { --p; 
}++p;
       q=x+c0+1; while (q<x+c2-1 && *q!=' ') { ++q; 
}--q;
       if (*ww==EOS && c0>1) { --p;
}
...
int op_jump_end(int i)
        ...
        muste_sprintf(x,"%d",j); px[1]=x;
        px[2]=x+30; muste_sprintf(px[2],"%d",i);
        op_goto2(3,px);
        return(1);
        }
...
// RS REM    char s;
    ...
    while (1)
        ...
        *kieli=(char)getck();
        muste_sprintf(sbuf,"%c",*kieli); sur_print(sbuf);
...
int op_nextfile() // NEXTFILE XYZ,PS
    ...
    j=r1+r-1;
    edwrite(space,j,1);
    muste_sprintf(sbuf,"NEXTFILE %s,%s / %s%d.%s",
...
//        int i;
        ...
        if (g<3)
            ...
            if (g==2) { insdel_k=atoi(parm[1]); if (insdel_k==0) { return; 
}} else { insdel_k=1;
}
            }                          // 21.4.2003
        else
...
// RS REM    int win_type;
    ...
    if (muste_strcmpi(parm[1],"SCREEN")==0)
        ...
        muste_sprintf(sbuf,"Current screen: %d %d",
                 par[0],par[1]);
        edwrite(sbuf,r1+r,1);
        return(1);
        }
...
// RS REM            strcpy(wname,os_win_name);
        ...
        edwrite(space,r1+r,1);
        muste_sprintf(sbuf,"Current window: %d %d %d %d",
...
*/
            ...
            edwrite(space,r1+r,1);
            muste_sprintf(sbuf,"Current font: %d %d",
                     par[0],par[1]);
            edwrite(sbuf,r1+r,1);
            return(1);
            }
...
*/
		...
		i=0; while (pxx[i]==' ' && i<strlen(pxx)) { i++; 
}pxx=pxx+i;        
		cumpit=strlen(xx);
        while (1)
            {
...
/*					
					if (strchr(parm[1],' ')!=NULL)
					  ...
					  if (strchr(parm[2],' ')!=NULL) 
					    muste_sprintf(sbuf,"file.copy(\"'%s'\",\"'%s'\",overwrite=TRUE) # %s",parm[1],parm[2],parm[0]); 
    				  else muste_sprintf(sbuf,"file.copy(\"'%s'\",\"%s\",overwrite=TRUE) # %s",parm[1],parm[2],parm[0]); 
    				  }
    				else
    				  {
    				  if (strchr(parm[2],' ')!=NULL)
...
//  *p=EOS; ++q;
    ...
    if (strchr(q,'.')==NULL) // 18.4.2003
        ...
        while (1)
            ...
            if (x[i]=='\r') { x[i]=EOS; 
}if (i>0) { if (x[i-1]=='\r') { x[i-1]=EOS; // RS ADD
}
}
            command=x+2;
    ...
    if (ok)
        ...
        if (strncmp(q,"www.",4)==0) { 
        	if(strchr(q,'&')!=NULL) { muste_sprintf(sbuf,"*%s 'http://%s'",command,q);
        	} else { muste_sprintf(sbuf,"*%s http://%s",command,q);
}
        } else if(strchr(q,'&')!=NULL) { muste_sprintf(sbuf,"*%s '%s'",command,q);
        } else { muste_sprintf(sbuf,"*%s %s",command,q);
}
...
muste_restore_stack_count();   // RS 9.3.2014              
	    ...
	    if (strcmp(OO,"GOTO")==0)    { i=op_goto(); goto_load_ind=1; muste_restore_stack_count(); return(i); }  
else    if (strcmp(OO,"REDIM")==0)   { sur_dump(); op_redim(1); muste_restore_stack_count(); return(1); }
else    if (strcmp(OO,"FONT")==0 || strcmp(OO,"WINDOW")==0)
            { sur_dump(); op_font(); muste_restore_stack_count(); return(1); }
else    if (strcmp(OO,"TKFONT")==0)   { muste_choosefont(); muste_restore_stack_count();  return(1); }
else    if (strcmp(OO,"SAVE")==0)    { op_save();
                                       if (etu==0) { sur_wait(100L,nop,1);
}
                                       muste_restore_stack_count(); return(1);
                                     }

else    if (strcmp(OO,"LOAD")==0)    { sur_dump(); op_load(); goto_load_ind=1; muste_restore_stack_count(); return(1); }
else    if (strcmp(OO,"SCRATCH")==0) { sur_dump(); op_scratch(); muste_restore_stack_count(); return(1); }
else    if (strcmp(OO,"PATHS")==0) { muste_showpaths(); muste_restore_stack_count(); return(1); }
else    if (strcmp(OO,"CD")==0 || 
            strcmp(OO,"PATH")==0 || 
            strcmp(OO,"DISK")==0)  { op_cd(); muste_restore_stack_count(); return(1); } // RS CHA   { i=op_path(); return(i); }
else    if (strcmp(OO,"MD")==0 || 
            strcmp(OO,"MKDIR")==0)  { sur_make_dir(parm[1]); muste_restore_stack_count(); return(1); } // RS NEW
else    if (strcmp(OO,"RD")==0 ||
            strcmp(OO,"RMDIR")==0) { sur_remove_dir(parm[1]); muste_restore_stack_count(); return(1); } // RS NEW 
else    if (strcmp(OO,"WIN")==0)  {  i=op_win(); muste_restore_stack_count(); return(i); }           
else    if (strcmp(OO,"RESIZE")==0)  { sur_dump(); i=op_resize(); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"SET")==0)     { op_set(); muste_restore_stack_count(); return(1); }
else    if (strcmp(OO,"TIME")==0)    { sur_dump(); i=op_time(); muste_restore_stack_count(); return(i); }
//else    if (strcmp(OO,"TUTOR")==0)   { sur_dump(); i=op_tutor(); return(i); }
else    if (strcmp(OO,"-")==0)       { i=op_jump(1); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"INIT")==0 && g>=3 ) {sur_dump(); muste_restore_stack_count(); return(op_init()); } // g 8.8.03
else    if (strcmp(OO,"WAIT")==0)    { i=op_wait(); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"WAIT_TUT")==0) { i=op_wait_tut(); muste_restore_stack_count(); return(i); } // 30.12.2000
else    if (strcmp(OO,"QPATH")==0)   { i=op_qpath(); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"COUNT")==0)   { sur_dump(); i=op_count(); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"CLEAR")==0)   { sur_dump(); i=op_clear(); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"INSERT")==0 || strcmp(OO,"I")==0) { sur_dump(); insdel(); muste_restore_stack_count(); return(1); }
else    if (strcmp(OO,"DELETE")==0 || strcmp(OO,"D")==0) { sur_dump(); insdel(); muste_restore_stack_count(); return(1); }
else    if (strcmp(OO,"CHECK")==0) { i=op_check(1); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"CHECK0")==0) { i=op_check(0); muste_restore_stack_count();  return(i); }
else    if (strcmp(OO,"NEXTFILE")==0) { i=op_nextfile(); muste_restore_stack_count(); return(i); } // 13.9.2000
else    if (strcmp(OO,"SOFTKEYS")==0) { i=op_softkeys(); muste_restore_stack_count(); return(i); } // 15.3.2000
else    if (strcmp(OO,"TEMPDISK")==0) { i=op_tempdisk(); muste_restore_stack_count(); return(i); } // 27.4.2004
else    if (strcmp(OO,"COLX")==0)  { i=op_colx(); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"SYS")==0 || strcmp(OO,"SYSDEL")==0)
            { muuta_apu_tiedostoa(3); muste_restore_stack_count(); return(1); } // 14.7.2006

else    if (strcmp(OO,"SYSTEM")==0)  { i=survoapu1(0,NULL); muste_restore_stack_count(); return(i); }  

else    if (strcmp(OO,"COPY")==0)    { sur_dump(); op_copy(); muste_restore_stack_count(); return(1); }
else    if (strcmp(OO,"PASTE")==0)   { sur_dump(); op_paste(1); muste_restore_stack_count(); return(1); } // 6.1.2002

else    if (strcmp(OO,"STATUS")==0)  { op_status(); muste_restore_stack_count(); return(1); }

else    if (strcmp(OO,"INSERTL")==0) { sur_dump(); op_insertl(); muste_restore_stack_count(); return(1); } // 10.6.2006
else    if (strcmp(OO,"DELETEL")==0) { sur_dump(); op_deletel(); muste_restore_stack_count(); return(1); } // 18.6.2006
else    if (strcmp(OO,"LINEINS")==0) { sur_dump(); op_lineins(); muste_restore_stack_count(); return(1); } // 10.6.2006

else    if (strcmp(OO,"COPYBLO")==0) { sur_dump(); op_copyblock(); muste_restore_stack_count(); return(1); } // RS

else    if (strcmp(OO,"OUTPUT")==0)  { i=op_output(); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"ROUT")==0)  { i=op_rout(); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"SETUP")==0)   { i=op_setup(); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"SHADOW")==0)   { sur_dump(); i=op_shadow(); muste_restore_stack_count(); return(i); }
else    if (strcmp(OO,"SESSION")==0) { i=op_session(); muste_restore_stack_count(); return(i); } // 7.4.2000
else    if (strcmp(OO,"PUTENV")==0) { putenv(parm[1]); muste_restore_stack_count(); return(1); } // 24.3.2005
else    if (strcmp(OO,"LOWLINE")==0) { op_lowline(); muste_restore_stack_count(); return(1); } // 25.6.2006
else    if (strcmp(OO,"FILETIME")==0) { op_filetime(); muste_restore_stack_count(); return(1); } // 30.7.2008
else    if (strcmp(OO,"COLOR")==0) { i=op_color(); muste_restore_stack_count(); return(i); } // 30.12.2000
else    if (strcmp(OO,"TEXTCOLS")==0)  { sur_dump(); i=op_textcols(); muste_restore_stack_count(); return(i); } // 17.3.2001
else    if (strcmp(OO,"FILES")==0) { op_files(); muste_restore_stack_count(); return(1); } // 9.6.2005
else    if (strcmp(OO,"NET")==0) { op_net(); muste_restore_stack_count(); return(1); } // 13.2.2006
else    if (strcmp(OO,"EXT_FUNC")==0)   { sur_dump(); op_ext_func(); muste_restore_stack_count(); return(1); }
...
// RS REM                if (strlen(OO)==1) return(1);
                i=op_dos(); muste_restore_stack_count(); return(i);
                }


else    if (strcmp(OO,"FIND")==0 || strcmp(OO,"REPLACE")==0 ||
            strcmp(OO,"-FIND")==0 || strcmp(OO,"-REPLACE")==0) 
            { sur_dump(); first_word_on_line_search=0; 
              i=op_find(); muste_restore_stack_count(); return(i); }
            
else    if (strcmp(OO,"MASK")==0 || strncmp(OO,"MASK=",5)==0 || 
            strcmp(OO,"VARS")==0 || strncmp(OO,"VARS=",5)==0)
             { sur_dump();  file_act("MASK"); muste_restore_stack_count(); return(1); }    
             
else    if (strcmp(OO,"HELP")==0)    { i=help("HELP"); muste_restore_stack_count(); return(1); }
else    if (strcmp(OO,"F")==0)       { i=help("F"); muste_restore_stack_count(); return(1); }   
            ...
            help2(); muste_restore_stack_count(); return(1);
            }
...
void prefix()
        ...
        if (special)
            ...
            switch(m)
              ...
              case CODE_TAB:
                i=sur_getch(); if (i=='\\') { strcpy(id_jatko,"  "); 
}break;  // RS getch -> sur_getch
              case CODE_DOWN:
                if (r<r3) { r=r3; break; }
                i=lastline2();
                if (i<r3) { r1=1; r=i+1; if (i<1) { r=1; 
}disp(); break; }
...
*/
              ...
              case 'R': case 'r': muste_sprintf(x,"%d@%d",r1+r-1,c1+c-1);
                                  tutcat(x); break;
...
// Rprintf("case L: %c",*x);                                  
                   ...
                   /* 18.8.00  */ else if (*x=='K')
                                         { 
                                         muste_sprintf(x,"%d",end_empty_lines()); tutcat(x);
                                         break; }
...
// RS REM                char x[LLENGTH];
                ...
                switch (m)
                  ...
                  case CODE_BACKSP:
                    ...
                    if (muste_selection) { sur_dump(); muste_erase(); muste_selection=FALSE; break; } // RS ADD 26.7.2012
...
// muste_fixme("FIXME: CODE_ACTIV not yet implemented!\n"); // RS FIXME 
                  ...
                  case 153: sur_dump(); op_paste(2); disp(); break;  // 24.4.2006
        ...
        if (numtab)
            ...
            if ( (m>='0' && m<='9') || (m=='+') || (m=='-') )
                ...
                for (; i<c1+c-2; ++i) { x[i]=x[i+1];
                           muste_sprintf(sbuf,"%c",x[i]); sur_print(sbuf); }
                x[c1+c-2]=m; muste_sprintf(sbuf,"%c",m); sur_print(sbuf);
                edwrite(x,r1+r-1,0);
                return(1);
                }
            else { numtab=0;
...
static void muste_set_sysname() // RS ADD 22.9.2011
  ...
  survoapu1(1,sbuf);
  if (strcmp(sysname,"darwin")==0) { muste_mac=1;
  } else { muste_mac=0;
}
  ...
  if (!i)
      ...
      p=strchr(sysname,' ');
      if (p==NULL) { muste_sprintf(sbuf,"SYSTEM R_command=%s",sysname);
      } else { muste_sprintf(sbuf,"SYSTEM R_command=\"%s\"",sysname);
}
      survoapu1(1,sbuf);   
      }  
  }
...
// RS REM turha nykyään?    strcpy(last_disk,"Y:");
        *esysd=EOS;  // RS CHA nyt levytynnus vain jos survo_path:in toinen merkki on : 
        if (survo_path[1]==':') { esysd[0]=*survo_path; 
}esysd[1]=':'; esysd[2]=EOS;  // 9.10.91
...
// Rprintf("\ninfo_s=%s|",info_s);
        ...
        *language='0'; // RS ADD
        ...
        if (!tmp_by_session)  { i=sys_save_restore(1); if (i<0) { return(-1); 
}} // 26.2.2001
...
//		valitse_kieli("2"); // RS English
        ...
        if (i) { muste_restored_session=atoi(x);     
}
        ...
        if (muste_restored_session)
            ...
            strcpy(x,etmpd); strcat(x,"SESSIONS.SYS");
            *xses=EOS;
            i=sur_find_file(x);
            if (i!=0)  // RS 13.2.2014
                {
	    ...
	    if (strcmp(x1,"<empty>")!=0) { alkututor=2; // RS 30.1.2013
			...
			if (sur_find_file(sbuf)) { alkututor=1; // RS ADD
			    ...
			    {
			    if (hae_apu("start_sucro",x1)) { alkututor=2;
}
			    }
			}
...
int seek_word0(int mode)
        ...
        if (mode==1) { if (p-x<=c1+c3-1) { c=p-x-c1+1; 
}return(1); }
        c=p-x-c1+1;
        col_display();
        return(1);
...
static void disp_nappihaku(char *haku)
        {
        cursor(r3+1,17); muste_sprintf(sbuf,"%s",haku); sur_print(sbuf);
        }
...
static void putsaa()
        {
        cursor(r3+1,0);
        muste_sprintf(sbuf,"%.*s",c3,space); sur_print(sbuf);
        }
...
/* Rprintf("\nsh_haku=%s",sh_haku); getch(); */
            ...
            strcpy(sbuf,haku); chrconv(sbuf,haku); // RS 22.3.2013
            strcpy(sbuf,korvaus); chrconv(sbuf,korvaus); // RS 22.3.2013
...
/* Rprintf("\nsh_haku=%s sh_korvaus=%s sh=%d\n",sh_haku,sh_korvaus,sh); getch(); */
        ...
        if (move_ind) { pyyhi_alarivi(); }
                cursor(r3+1,0);
                PR_EBLD;
                muste_sprintf(sbuf,"Search for word: %s",haku); sur_print(sbuf);
                cursor(r,c); SAVE_CURSOR;
  ...
  {  muste_sprintf(sbuf,"n=%u: Next case=N Continuous search=C Interrupt by ENTER",n);
     sur_print(sbuf);
        ...
        if (!jatkuva2)
            {
            cursor(r3+1,0);
            muste_sprintf(sbuf,"%.*s",c3,space); sur_print(sbuf); cursor(r3+1,0);
            PR_EBLK;
            if (n>0)
                {
                ...
                {
                if (n_act==0)
                    { muste_sprintf(sbuf,"%.32s not found!   Press ENTER!",haku); sur_print(sbuf); m=0; }
                else
                    {
