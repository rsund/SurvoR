...
static int init_shadow_codes() {
    ...
    shad_nr = 1;
    strcpy(shad_name, "shadows");
    i = hae_apu("shadows#", x);
    if (i) {
        shad_nr = atoi(x);
    }
    if (shad_nr > 1) {
        muste_sprintf(shad_name, "shadows%d", shad_nr);
    }
    ...
    i = split(x, osa, 10);
    for (i = 0; i < 10; ++i) {
        shadow_int[i] = atoi(osa[i]);
    }
    for (shadow = 0; shadow < 256; ++shadow) {
        if (shadow == 32) {
            shadow_code[shadow] = shadow_int[0];
            ...
            shadow_code[shadow] = shadow;
        } else {
            shadow_code[shadow] = shadow - 16;
        }
    }
    return (1);
}
...
static int subst_survo_path_in_editor_core(char *s, int muunto) // RS 16.11.2012
{
    ...
    subi = 1;
    while (1) {
        muste_sprintf(subx, "subst%d", subi);
        i = hae_apu(subx, x);
        if (!i) {
            break;
        }
    ...
    i = 0;
    if (!i) {
        i = muste_subst_abrev(s, "<Temp>", etmpd, muunto, 1, '<');
    }
    if (!i) {
        strcpy(x, survo_path);
        strcat(x, "SYS/");
        i = muste_subst_abrev(s, "<Sys>", x, muunto, 1, '<');
    }
    if (!i) {
        i = muste_subst_abrev(s, "<Survo>", survo_path, muunto, 1, '<');
    }
    if (!i) {
        i = muste_subst_abrev(s, "<R>", muste_Rpath, muunto, 0, '<');
    }
    if (!i) {
        i = muste_subst_abrev(s, "<Home>", muste_homedir, muunto, 1, '<');
    }
    if (!i) {
        i = muste_subst_abrev(s, "~", muste_homedir, muunto, 0, '~');
    }
    if (!i) {
        i = muste_subst_abrev(s, "<Start>", muste_startpath, muunto, 1, '<');
    }
...
int unsubst_survo_path_in_editor(char *s) // RS 16.11.2012
{
    ...
    i = 0;
    if (!i) {
        i = muste_unsubst_abrev(s, "<Temp>", etmpd);
    }
    if (!i) {
        strcpy(x, survo_path);
        strcat(x, "SYS/");
        i = muste_unsubst_abrev(s, "<Sys>", x);
    }
    if (!i) {
        i = muste_unsubst_abrev(s, "<Survo>", survo_path);
    }
    if (!i) {
        i = muste_unsubst_abrev(s, "<R>", muste_Rpath);
    }
    if (!i) {
        i = muste_unsubst_abrev(s, "~", muste_homedir);
    }
...
static int medit_restore() {
    ...
    muste_sprintf(sbuf, "%sSURVOMDT", etmpd);
    edload(sbuf, 1);
...
int op_file(char *op) {
    ...
    if (strncmp(s, "ACT", 3) == 0) {
        s[3] = EOS;
    }
    if (strcmp(s, "UPDATE") == 0 || strcmp(s, "STATUS") == 0 || strcmp(s, "INIT") == 0 || strcmp(s, "REDUCE") == 0 ||
        strcmp(s, "ACT") == 0 || strcmp(s, "MASK") == 0 || strcmp(s, "MAKE") == 0 || strcmp(s, "COND") == 0 ||
        strcmp(s, "MASKLOAD") == 0 || strcmp(s, "CONVERT") == 0 || strcmp(s, "CREATE") == 0) {
    ...
    if (strcmp(s, "MEDIT") == 0) // 30.4.2003
    {
        ...
        if (par3 == 1) {
            ...
            *p = EOS;
            muste_sprintf(info, "%s %s %s", parm[2], parm[3], p + 1);
        }
        muste_sprintf(sbuf, "%sSURVOMDT", etmpd);
        edt_talletus(sbuf);
        strcpy(data_name, parm[2]);

        muste_sprintf(sbuf, "%sSURVOMSP", etmpd); // 11.6.2003
        ...
        if (par3 == 1) {
            ...
            muste_sprintf(info, "%s NULL NULL", parm[2]);
            medit = 1;
            medit_r1 = r1;
            strcpy(op, s);
            muste_dump();
...
static int ed_malloc(unsigned int ed1, unsigned int ed2, unsigned int edshad) {
    ...
    */
    ...
    if (ed1 > 253 || ed1 * (ed2 + edshad) > 65500) {
        large_field = 1;
    } else {
        large_field = 0;
    }
...
static int edload32(char *edfile) {
    ...
    lisays = ed1 * (ed2 + edshad) + (ed2 + 1) * sizeof(int) - ved1 * (ved2 + vedshad) - (ved2 + 1) * sizeof(int);
    if (lisays > 0) {
    ...
    r2 = ed2;
    strcpy(eopen, edfile);
    for (i = 0; i < ed1 * (ed2 + edshad); ++i) {
        z[i] = ' ';
    }
    for (i = 0; i < ed1 * ed2; i += ed1) {
        z[i] = '*';
    }
    shadinit();
    if (ei_onnistunut) {
...
int check_start_field_language(char *edfile) // 1.2.2006
{
    ...
    muste_sprintf(x, "%sSTART.EDT", survo_path);
    if (muste_strcmpi(edfile, x) != 0) {
        return (1);
    }
    /* RS REM No need to use different languages
        ch='S'; if (*language=='2') ch='E';
        muste_sprintf(edfile,"%s%cSTART.EDT",survo_path,ch);
    */
    return (1);
}
...
static int edload(char *field, int shad) {
    ...
    filename(edfile, field);
    file_name_ext(edfile, ".EDT");
    i = strlen(edfile) - 1;
    if (edfile[i] == '.') {
        edfile[i] = EOS; // RS ADD Drop last .
    }
    ...
    if (edfield == NULL) {
        edit_file_not_found(edfile);
        return (-2);
    } // RS 20.11.2014 return -2 instead of -1
    time(&aika_save);
    for (i = 0; i < ELE; ++i) {
        rivi[i] = (char)getc(edfield);
    }
    rivi[ELE - 1] = EOS;
    if (strncmp(rivi, "SURVO 98", 8) == 0) {
        return (edload32(edfile));
    }

    g = split(rivi, sana, 5);
    if (strcmp(sana[0], "SURVO84ED") != 0) {
        sur_print("\nNot a Survo edit file!");
        WAIT;
        muste_fclose(edfield);
        return (-1);
    }
    ...
    ed2 = atoi(sana[2]);
    if (g > 4 && *sana[4] == 'S') {
        edshad = atoi(sana[4] + 1);
    }

    muste_fclose(edfield); /* suljetaan malloc-varausten tiivistämiseksi */
    k = ed_malloc(ed1, ed2, edshad);
    if (k < 0) {
        return (-1);
    }
    edfield = muste_fopen2(edfile, "rb");
    ...
    shadinit();
    for (i = 0; i < ed1; ++i) {
        x[i] = (char)getc(edfield);
    }
    if (strncmp(x, "Sha", 3) != 0) {
        muste_fclose(edfield);
        return (1);
    }
    j = 1;
...
static int edsave32(char *edfile, int shad) {
    ...
    fprintf(edfield, "SURVO 98 edit field:    %d       %d       %d (32 bit version)%s", d1, d2, d3, rivin_loppu);
    form = 3;
    if (ed2 >= 1000) {
        form = 4;
    }
...
int edsave(char *field, int shad, int check) {
    ...
    if (check) {
        ...
        if (edfield != NULL) {
            if (muste_strcmpi(edfile, eopen) != 0 && etu != 2 && save_warning) {
                PR_EBLD;
                muste_sprintf(sbuf, "\nEdit file %s already exists! Overwrite it (Y/N)?", edfile);
                sur_print(sbuf);
                ch = (char)nextch_editor();
    ...
    strcat(header, muste_itoa(ed1, number, 10));
    strcat(header, " ");
    strcat(header, muste_itoa(ed2, number, 10));
    strcat(header, " ");
    strcat(header, "                    ");
    header[20] = EOS;
    strcat(header, muste_itoa(ed1, number, 10));
    strcat(header, " S");
    strcat(header, muste_itoa(edshad, number, 10));
    strcat(header, " ");
    ...
    if (check) {
        strcpy(eopen, edfile); /* if 17.2.89 */
    }
    muste_fclose(edfield);
    return (1);
}
...
int op_redim(int komento /* 1=REDIM command 0=internal call(sh.c) */
) {
    int i, j, k;
    char x[LLENGTH];
    ...
    j = ued1 * (ued2 + uedshad) + (ued2 + 1) * sizeof(int) - ed1 * (ed2 + edshad) - (ed2 + 1) * sizeof(int);
    if (j > 0) {
        i = testaa_lisays(j);
        if (i < 0) {
            sur_print("\nNot enough memory!");
...
static int shadow_erase() {
    ...
    j1 = edline2(parm[2], 1, 1);
    if (j1 == 0) {
        return (-1);
    }
    j2 = edline2(parm[3], j1, 1);
    if (j2 == 0) {
        return (-1);
    }
    for (j = j1; j <= j2; ++j) {
...
static int shadow_replace() {
    ...
    if (muste_strnicmp(parm[2], "char(", 5) == 0) {
        ch1 = atoi(parm[2] + 5);
    } else {
        ch1 = *parm[2];
    }
    if (muste_strnicmp(parm[3], "char(", 5) == 0) {
        ch2 = atoi(parm[3] + 5);
    } else {
        ch2 = *parm[3];
    }
    j1 = edline2(parm[4], 1, 1);
    if (j1 == 0) {
        return (-1);
    }
    j2 = edline2(parm[5], j1, 1);
    if (j2 == 0) {
        return (-1);
    }
    for (j = j1; j <= j2; ++j) {
...
static int shadow_setc() // set -> setc
{
    ...
    if (!muste_is_path(nimi)) {
        muste_sprintf(nimi, "%sSYS/", survo_path); // RS \\ -> /
        strcat(nimi, parm[5]);
    }
...
static int op_shadow() {
    if (muste_strcmpi(parm[1], "ERASE") == 0) {
        shadow_erase();
        return (1);
    }
    if (muste_strcmpi(parm[1], "BLOCK") == 0) {
        shadow_block();
        return (1);
    }
    if (muste_strcmpi(parm[1], "REPLACE") == 0) {
        shadow_replace();
        return (1);
    }
    if (muste_strcmpi(parm[1], "SETC") == 0) {
        shadow_setc();
        return (1);
    }
    // 14.12.2011
...
static int display_colors() {
    ...
    if (muste_strcmpi(parm[1], "#ALL") == 0 || muste_strcmpi(parm[1], "ALL") == 0) {
        ...
        for (j = 0; j < 8; ++j) {
            ...
            for (i = 0; i < 16; ++i) {
                color = 16 * j + i;
                muste_sprintf(x, "%3d", color);
                vara = shadow_code[color];
                shadow_code[color] = color;
    ...
    if (color2 > color1 + r3 - r - 1) {
        color2 = color1 + r3 - r - 1;
    }
    if (color2 > 255) {
        color2 = 255;
    }
    ...
    for (color = color1; color <= color2; ++color) {
        muste_sprintf(x, " %3d ABCDEFGHIJKLMNOPQRSTUVWXYZ 0123456789", color);
        vara = shadow_code[color];
        shadow_code[color] = color;
...
static int op_color() {
    ...
    if (g > 1) {
        ...
        if (n == 0) {
            ...
            for (i = 0; i < 256; ++i) {
                shadow_code[i] = (unsigned char)getc(codes);
            }
            muste_fclose(codes);
            return (2);
        }
    ...
    muste_srand(*pi);
    for (i = 0; i < 256; ++i) {
        shadow_code[i] = (unsigned char)((int)(n * RND));
    }
    return (2);
}
...
static int move_disp(unsigned int j) {
    ...
    switch (move_ind) {
    case 3:
        if (j < move_r1 || j > move_r2) {
            return (1);
        }
        cc1 = mc1 - c1 + 1;
        if (!move_words && cc1 > (int)c3) {
            return (1);
        }
        if (cc1 < 0) {
            cc1 = 0; // RS ADD
...
int headline_editor() {
    ...
    muste_sprintf(x, "%c%c%4u", pref, dispm2, lev); // RS CHA c1+c-1 -> lev
    write_string(x, 6, hshadow, 1, 1);
    muste_sprintf(x, "%3u", c1);
    write_string(x, 3, ')', 1, 7);
    ...
    strcpy(sbuf, edisk);
    unsubst_survo_path_in_editor(sbuf);
...
void displine(unsigned int j, unsigned int lev) {
    ...
    /*      CURSOR_OFF;             */
    if (rsh != j) {
        muste_sprintf(x, "%6u ", j);
    } else {
        muste_sprintf(x, "Shadow:");
    }
    x[7] = z[(j - 1) * ed1];
    x[8] = EOS;
    if (x[7] == '?') {
        write_string(space, c3 + 8, ' ', j - r1 + 2, 1);
        return;
    }
    if (!line_labels_off) {
        write_string(x, 8, 2, j - r1 + 2, 1);
    } // RS CHA 23.9.2012 shadow '2' -> 2
    else {
        write_string(space, 8, 0, j - r1 + 2, 1);
    } // RS CHA 23.9.2012 shadow ' ' -> 0
    if (c3 > c2) {
        write_string(space, c3 - c2, '7', j - r1 + 2, c2 + 9);
    }
    if (lev > 0) {
        px = z + (j - 1) * ed1 + c1;
        ...
        if (*px == '.') {
            ...
            if (i == lev) {
                write_string(stripe, lev, ' ', j - r1 + 2, 9);
                return;
            }
        }
        if (*px == '#') // 19.4.2010
...
int dispoint() {
    ...
    int k;
    char *px;
    if (muste_help_running) {
        return (1);
    }
    if (survopoint_on) {
        ++survopoint_disp_n; // 15.3.2012
    }
    headline_editor();
...
static int muste_search_rbufline(int curline, int check) {
    ...
    for (i = curline; i <= l; i++) {
        ...
        if (p == NULL) {
            ...
            if (p != NULL) {
                j = p - rivi;
                if (!(rivi[j - 1] == (char)PREFIX && rivi[j - 2] != (char)PREFIX && p[1] != (char)'.' &&
                      p[1] != (char)')' && p[1] != (char)',')) {
                    p = rivi;
                }
            }
...
static int disp_rbuf() // RS 25.3.2013
{
    ...
    while (1) {
        ...
        if (muste_rbuf1 > (r1 + r3) || muste_rbuf2 < r1) {
            continue;
        }

        if (r1 > muste_rbuf1) {
            firstvisible = r1 - muste_rbuf1 + 1;
            screenmin = 1;
        } else {
            firstvisible = 1;
            screenmin = muste_rbuf1 - r1 + 1;
        }

        if (muste_rbuf2 < (r1 + r3)) {
            lastvisible = muste_rbuf2 - muste_rbuf1 + 1;
            screenmax = r3 - ((r1 + r3) - muste_rbuf2) + 1;
        } else {
            lastvisible = muste_rbuf2 - muste_rbuf1 - (muste_rbuf2 - (r1 + r3) + 1) + 1;
            screenmax = r3;
        }
        ...
        muste_sprintf(sbuf, ".muste.highlight('%s',%d,%d,%d,%d,%d,%d)", out, firstvisible, lastvisible, firstxvisible,
                      screenmin, screenmax, screenwidth);
...
int disp() {
    ...
    {
        for (; i < r3; ++i) {
            write_string(space, c3 + 8, '7', i + 2, 1);
        }
    }
...
void dispch(int m) {
    ...
    if (dispm > 0) {
        disp_mode(dispm + '0');
        if (m == (int)' ') {
            m = (int)z[(r1 + r - 2) * ed1 + c1 + c - 1];
        }
    }
    muste_sprintf(merkki, "%c", (unsigned char)m);               // RS ADD 10.10.2012
...
int disp_shadow() {
    ...
    edread(x, rsh);
    if (zs[rsh - 1] == 0) {
        creatshad(rsh - 1); /* 13.3.86 */
    }
    edwrite(x, zs[rsh - 1], 0);
    edwrite(sh_vara, rsh, 0);
    zs[rsh] = zsrsh;
    testshad(rsh - 1);
    j = rsh;
    rsh = 0;
...
void pyyhi_alarivi() {
    LOCATE(r3 + 2, 1);
    ERASE; /* muste_sprintf(sbuf,"%.*s",c3+6,space); sur_print(sbuf); */
}
...
void prompt_editor(char *kysymys, char *vastaus, int pituus) {
    ...
    for (i = 0; i < strlen(vastaus); ++i) {
        tila[i] = vastaus[i];
    }
    muste_sprintf(sbuf, "%s%.*s*", kysymys, pituus, tila);
    sur_print(sbuf);
    ...
    while (1) {
        ...
        case CODE_INSERT:
            ...
            for (i = pituus - 1; i >= pos; --i) {
                tila[i] = tila[i - 1];
            }
            tila[pos - 1] = ' ';
            for (i = pos; i <= pituus; ++i) {
                muste_sprintf(sbuf, "%c", tila[i - 1]);
                sur_print(sbuf);
            }
            for (i = pos; i <= pituus; ++i) {
                PR_LEFT;
...
void line_merge() {
    ...
    x1[1] = EOS;
    strncat(x1, x + c1 + c - 1, c2 - c1 - c + 2);
    edwrite(space, j, c1 + c - 1);
    edwrite(x1, j + 1, 0);
    if (shad) {
        sx1[1] = EOS;
        strncat(sx1, sx + c1 + c - 1, c2 - c1 - c + 2);
...
int lopetuskysely() {
    ...
    {
        edrun = 0;
        muste_sprintf(sbuf, "quit(save=\"no\",status=1)");
        muste_evalr(sbuf);
        return (1);
    }
...
static int line_copy_do(int i, int j) {
    ...
    if (shad) {
        ...
        edread(sx1, zs[i]);
        len2 = c2 + 1;
        while (len2 > 1 && sx1[len2 - 1] == ' ') {
            --len2;
        }
    }
    len = c2 + 1;
    while (len > 1 && x1[len - 1] == ' ') {
        --len;
    }
    if (len2 > len) {
        len = len2;
...
int line_copy() {
    ...
    cursor(r, c);
    PR_EBLK;
    muste_sprintf(sbuf, "%c", (char)PREFIX);
    sur_print(sbuf);
    cursor(r3 + 1, 1);
    PR_EBLD;
    *x = EOS;
    prompt_editor("Line to be copied ? ", x, 13); // RS CHA (s) & 6->13
    PR_ENRM;
    j = r1 + r - 1;
    ...
    /*
                {
                *xl=EOS; xl++;
                muste_sprintf(sx,"COPY");
                muste_sprintf(sx1,"%d",j);
...
int op_goto2(int g, char *parm[]) {
    ...
    vc1 = c1; /* 21.2.1991 */
    c1 = atoi(parm[3]);
    col = atoi(parm[4]) - c1 + 1;
    if (c1 < 1 || c1 > c2 - c3 + 1 || col < 1 || col > c3) {
        if (c2 - c3 + 1 < 0) {
            c1 = 1; // RS ADD 15.11.2012
        } else {
            c1 = vc1;
            return (1);
        }
...
int op_goto() { return (op_goto2(g, parm)); }
...
static char muste_nullname[] = "(NO NAME)";
static char *para[MAXPARM];

static char *muste_get_savename() // RS 1.5.2014
{
    int plukum;
    edread(sbuf, 1);
    plukum = splitq(sbuf + 1, para, MAXPARM);
    if (plukum < 2) {
        return (muste_nullname);
    }
    muste_strupr(para[0]);
    if (strcmp(para[0], "SAVE") != 0) {
        return (muste_nullname);
    }
    return (para[1]);
}
...
int get_console_name(char *x) {
    ...
    buffer = muste_get_savename(); // RS 1.5.2014

    if (strcmp(buffer, "(NO NAME)") == 0) // RS 1.5.2014
    {
        i = muste_sprintf(x, "%s", system_name);
        if (*sur_session != 'A' && *sur_session != EOS) {
            i = muste_sprintf(x, "%s: %s", sur_session, system_name);
        }
    } else {
        i = muste_sprintf(x, "[%s] - %s", buffer, system_name);
        if (*sur_session != 'A' && *sur_session != EOS) {
            i = muste_sprintf(x, "%s [%s]: %s", sur_session, buffer, system_name);
        }
    }
    ...
    muste_sprintf(x + i, " - %s - ver. %s", licensed, version);
    ...
    */
    ...
    muste_sprintf(window_name2, "%sSURVO", sur_session); // 24.1.2001
    return (1);
}
...
int op_resize() {
    ...
    if (rr + rs + 2 < 25 && cc < 72) {
        cc = 72; // RS cc>72 sallittu;  pienillä r3, c3 oltava 72
    }
...
int goto_control_char(int k) /* 5.4.1999 */
{
    ...
    muste_sprintf(rivi1, "%d", j0);
    parm[1] = rivi1;
    muste_sprintf(rivi2, "%d", j);
    parm[2] = rivi2;
    op_goto2(g, parm);
    c = 1;
    disp();
...
void b_insertl(int rivi /* norm. r1+r-1 */
) {
    unsigned int j;
    char x[LLENGTH]; // RS REM , x1[LLENGTH];
...
void b_line_merge(int j, int col) /* norm. col=c1+c-1 */
{
    ...
    if (!empty_line(x1 + 1, c2)) {
        return;
    }
    x1[1] = EOS;
    strncat(x1, x + col, c2 - col + 1);
    edwrite(space, j, col);
    edwrite(x1, j + 1, 0);
    if (shad) {
        sx1[1] = EOS;
        strncat(sx1, sx + col, c2 - col + 1);
...
void autotrim(int rivi1, int rivi2) // RS pois?
{
    ...
    i = split(x, sana, 3);
    k = muste_sprintf(y, "%s %d %d", sana[0], rivi1 + 1, rivi2 + 1);
    if (i > 1) {
        k += muste_sprintf(y + k, " %s", sana[1]);
    }
    if (i > 2) {
        k += muste_sprintf(y + k, " %s", sana[2]);
    }
    b_insertl(rivi1 - 1);
    edwrite(y, rivi1, 1);
    activate();
...
void store_move() {
    ...
    if (hae_apu("autotrim", trim_command)) {
        autotrim(r_alku, r_loppu); // RS pois?
    }
}
...
int m_move_text() {
    ...
    LOCATE(r3 + 2, 1);
    PR_EBLK;
    muste_sprintf(prompt_line, "Defining a block by the rightmost mouse button (See MOUSE6?)     Cancel by DEL!");
...
int script_save(int mr1, int mr2, int mc1, int mc2) {
    ...
    if (hae_apu("script_file", sname) == 0) {
        return (1);
    }
...
int op_block(int rr, int cc) {
    ...
    mr1 = move_r1;
    mr2 = move_r2;
    strcpy(survoblo, etmpd);
    strcat(survoblo, "SURVO.BLO");
    strcpy(survowrd, etmpd);
    strcat(survowrd, "SURVO.WRD");
...
int move_block(int k /* 0=move_block 1=move_words */
) {
    int i;
    ...
    case 0:
        ...
        if (move_words) {
            ...
            *sbuf = EOS;
            ...
            strcpy(sana1, "corner");
            strcpy(sana2, "copy");
            strcpy(sana3, "Erase block");
            strcpy(sana4, "alt-F4");
            //                nappi=CODE_MOVE;
            muste_sprintf(sbuf, "  (Exit from %s by F8!)", system_name);
        }

        //          muste_sprintf(prompt_line,"Mark %s 1 by %s. %s=Cancel%s",
        //           sana1,key_label[nappi],key_label[CODE_DELETE],sbuf);
        muste_sprintf(prompt_line, "Mark %s 1 by %s. %s=Cancel%s", sana1, sana4, key_label[CODE_DELETE], sbuf);
        // RS ALT            muste_sprintf(prompt_line,"Mark %s 1 by %s. DEL=Cancel%s",sana1,sana4,sbuf); // RS
        muste_sprintf(sbuf, "%s", prompt_line);
        sur_print(sbuf);
        move_ind = 1;
        break;
    case 1:
        move_r1 = r1 + r - 1;
        mc1 = c1 + c - 1;
        //          muste_sprintf(prompt_line,"Mark %s 2 by %s. %s=Cancel",
        //           sana1,key_label[nappi],key_label[CODE_DELETE]);

        muste_sprintf(prompt_line, "Mark %s 2 by %s. %s=Cancel", sana1, sana4, key_label[CODE_DELETE]);
        // RS ALT            muste_sprintf(prompt_line,"Mark %s 2 by %s. DEL=Cancel",sana1,sana4); // RS
        ...
        //          cursor(r,c); PR_EBLK; muste_sprintf(sbuf,"%c",ch); sur_print(sbuf);
        //          PR_ENRM;
        break;
    case 2:
        move_r2 = r1 + r - 1;
        ...
        if (!m_move_ind) // 21.3.2004
        {
            ...
            muste_sprintf(prompt_line, "Select place and %s by %s. %s=%s, %s=Cancel", sana2, sana4,
                          key_label[CODE_ERASE], sana3, key_label[CODE_DELETE]);
            /* RS ALT
...
static int test_empty_space(int r0, int c0) {
    ...
    kork = move_r2 - move_r1 + 1;
    lev = mc2 - mc1 + 1;

    for (j = r0; j < r0 + kork; ++j) {
        edread(x, j);
        if (strncmp(x + c0, space, lev) != 0) {
            return (-1);
        }
    }
    return (1);
                   ...
                   // valitun sanan tai sanaparin. 13.11.2012
{
    ...
    *w = EOS;
    ...
    if (zs[rr] == 0) {
        varjo = 0;
        *ws = EOS;
        muste_fieldcopy(ws, space, len1);
    } else {
        varjo = 1;
        edread(xs, zs[rr]);
        *ws = EOS;
        strncat(ws, xs + (p - x), len1);
    }
    return (1);
}
...
int m_copy_word2() // kirjoittaa em. sanan tai sanaparin 13.11.2012
{
    ...
    *p = EOS;
    ...
    PR_EBLK;
    if (len1 < 19) {
        muste_snprintf(sana, sizeof(sana), "'%.*s'", len1, ww);
    } // strcpy(sana,"'"); strncat(sana,ww,len1); strcat(sana,"'"); }
    else {
        strcpy(sana, "the same word");
    }

    muste_sprintf(prompt_line, "Copy %s again by the leftmost mouse button. Cancel by DEL!", sana);
    len1 = strlen(prompt_line);
    if (len1 < 79) {
        strncat(prompt_line, space, 79 - len1);
    }
    sur_print(prompt_line);
    PR_ENRM;
...
int mouse_define_block() {
    ...
    case 0:
        ...
        ******************************/
        m_move_r1 = r1 + r - 1;
        m_mc1 = c1 + c - 1;
        ++m_move_ind; // 8.10.2012
        if (one_click_copy) {
            m_copy_word1(); // 8.10.2012
        }
        break;
...
void code_code() {
    ...
    if (special_code == 0) {
        ...
        PR_EBLD;
        *luku = EOS;
        prompt_editor("Enter key code: ", luku, 5);
        if (*luku == EOS) {
            special_code = (int)(*(z + ed1 * (r1 + r - 2) + c1 + c - 1));
        } else {
            special_code = atoi(luku);
        }
...
int open_time_file(char *s) {
    if (muste_strcmpi(s, "CLOSE") == 0) {
        muste_fclose(time_file);
        time_file = NULL;
        time_file_on = 0;
        return (1);
    }
...
int file_time_start() { return (file_time_end("---")); }
...
int op_time() {
    ...
    if (g > 2 && muste_strcmpi(parm[1], "COUNT") == 0) {
        ...
        if (muste_strcmpi(parm[2], "START") == 0) {
            ...
            timestart = 0.0;
            if (g > 3) {
                timestart = atof(parm[3]);
            }
        } else {
            edwrite(space, r1 + r - 1, 1);
            muste_sprintf(sbuf, "TIME COUNT END   %.3f", timestart + timecount2 - timecount1);
    ...
    strcpy(x, "TIME ");
    strcat(x, aika);
    strncat(x, space, c2 - strlen(x));
    edwrite(x, r1 + r - 1, 1);
    return (2);
}

int sur_wait(long aika,            /* millisek. */
...
int sur_wait2(long aika,           /* millisek. */
              int (*display)(void) /* näyttötoimitus sekuntien välillä */
...
int save_wait(int m) {
    ...
    a1 = a0 >> 7;
    a2 = a1 >> 2; /* "jako 100:lla" 1/128+1/512=0.0097.. */
...
int op_count() {
    ...
    k = edline2(parm[3], 1, 1);
    if (k == 0) {
        return (-1);
    }
    if (g > 4) {
        alkuarvo = atof(parm[4]);
    }
    if (g > 5) {
        askel = atof(parm[5]);
    }
...
int valitse_kieli(char *s) {
    ...
    *crt_exit = *s;
    ...
    strcpy(sbuf, survo_path);
    strcat(sbuf, "SURVO.APU");
    *lang = *crt_exit;
    lang[1] = EOS;
...
int kielenvalinta() // 7.9.2003
{
    ...
    while (1) {
        LOCATE(5, 14);
        PR_ENRM;
        sur_print("English=E Suomi=S ?  ");
        LOCATE(5, 34);
        *kieli = (char)getck();
        muste_sprintf(sbuf, "%c", *kieli);
        sur_print(sbuf);
        if (strchr("EeSs", *kieli) != NULL) {
            break;
        }
        sur_sleep(300L);
    }
...
int survo_apu() {
    ...
    while (1) {
        ...
        *p = EOS;
        ...
        j = r1 + r - 1;
        edwrite(space, j, 1);
        muste_sprintf(sbuf, "CLEAR SYSTEM %s %s=%s", parm[2], parm[3], p);
        edwrite(sbuf, j, 1);
    }
    muste_fclose(tied);
    return (1);
...
int op_clear() {
    ...
    if (strcmp(parm[1], "*") == 0) {
        ...
        len = p - survo_id;
        muste_fieldcopy(x, survo_id, len);
        x[len] = EOS;
        if (strlen(tut_info) + strlen(x) > LLENGTH - 2) {
            return (1);
        }
        strcat(tut_info, x);
        strcat(tut_info, "@");
    ...
    muste_fieldcopy(clear, space, ed1);
    clear[ed1] = EOS;
    if (g == 3) {
        for (i = c1 + c - 1; i < ed1; ++i) {
            clear[i] = 'X';
        }
    } else {
        k = edline2(parm[3], 1, 1);
...
int op_nextfile() // NEXTFILE XYZ,PS
{
    ...
    while (1) {
        ...
        muste_sprintf(name, "%s%d.%s", sbuf, i, parm[2]);
    ...
    j = r1 + r - 1;
    edwrite(space, j, 1);
    muste_sprintf(sbuf, "NEXTFILE %s,%s / %s%d.%s", parm[1], parm[2], parm[1], i, parm[2]);
    edwrite(sbuf, j, 1);
    return (1);
}
...
int op_delete(void) {
    ...
    for (j = insdel_l1; j <= insdel_l2; ++j) {
        ...
        if (len > cut + insdel_k) {
            memmove(x + cut, x + cut + insdel_k, strlen(x + cut + insdel_k) + 1);
        } else {
            x[cut] = EOS;
        }
...
void insdel() {
    ...
    if (g < 3) {
        ...
        edread(x, insdel_l2);
        while (!ins_empty(x + 1) && insdel_l2 < r2) {
            edread(x, ++insdel_l2);
        }
        if (insdel_l2 < r2) {
            --insdel_l2;
        ...
        insdel_l1 = edline2(parm[1], 1, 1);
        if (insdel_l1 == 0) {
            return;
        }
        insdel_l2 = edline2(parm[2], insdel_l1, 1);
        if (insdel_l2 == 0) {
            return;
        }
        if (g >= 4) {
            insdel_k = atoi(parm[3]);
        } else {
            insdel_k = 1;
        }
    }
    ...
    if (strchr("Ii", *parm[0]) != NULL) {
        op_insert();
        return;
    }
    if (strchr("Dd", *parm[0]) != NULL) {
        op_delete();
        return;
    }
}
...
static int op_win() {
    ...
    if (muste_strcmpi(parm[1], "MIN") == 0) {
        sur_main_window_show(wname, 0);
        return (1);
    }
    if (muste_strcmpi(parm[1], "NORMAL") == 0) {
        sur_main_window_show(wname, 1);
        return (1);
    }
    if (muste_strcmpi(parm[1], "MAX") == 0) {
        sur_main_window_show(wname, 2);
        return (1);
    }
    ...
    if (muste_strcmpi(parm[1], "SCREEN") == 0) {
        ...
        muste_sprintf(sbuf, "Current screen: %d %d", par[0], par[1]);
        edwrite(sbuf, r1 + r, 1);
        return (1);
    }

    if (muste_strcmpi(parm[1], "MOVE") == 0) {
        sur_move_window(wname, atoi(parm[2]), atoi(parm[3]), atoi(parm[4]), atoi(parm[5]));
        return (1);
    }
    ...
    if (muste_strcmpi(parm[1], "WINDOW") == 0 || muste_strcmpi(parm[1], "SURVO") == 0) // 6.7.2006
    {
        ...
        // parametrit esim. Lucinda_Console,54,20,400
            ...
            muste_fixme("FIXME: Set fonts with parameters not implemented!\n"); // RS FIXME
        }
    ...
    if (muste_strcmpi(parm[1], "GET") == 0) {
        ...
        if (g > 2 && muste_strcmpi(parm[2], "HELP") == 0) // 19.9.2001
        {
            ...
            // RS REM            muste_sprintf(wname,"%s - Help Window",system_name);
        } else {
        ...
        edwrite(space, r1 + r, 1);
        muste_sprintf(sbuf, "Current window: %d %d %d %d", par[0], par[1], par[2], par[3]);
        edwrite(sbuf, r1 + r, 1);
        return (1);
    }
        ...
        if (g > 2 && muste_strcmpi(parm[2], "GET") == 0) {
            if (g > 3 && muste_strcmpi(parm[3], "HELP") == 0) // 19.9.2001
            {
                muste_fixme("FIXME: WIN FONT GET HELP not implemented; returning main window font!\n"); /* RS FIXME
                                muste_sprintf(wname,"%s - Help Window",system_name);
            ...
            } else if (g > 3 && muste_strcmpi(parm[3], "OS") == 0)                                      // 4.10.2001
            {
                ...
                */
                ...
                sur_get_font(wname, par);
            }
            edwrite(space, r1 + r, 1);
            muste_sprintf(sbuf, "Current font: %d %d", par[0], par[1]);
            edwrite(sbuf, r1 + r, 1);
            return (1);
        } else {
            if (g > 3) {
...
static int op_tempdisk() {
    ...
    if (muste_strcmpi(parm[1], "GET") == 0) {
        edwrite(space, j, 1);
        muste_sprintf(sbuf, "TEMPDISK GET \"%s\"", etmpd); // RS 25.11.2012 ADD "
                                                           //        muste_simplify_path(etmpd); // RS not useful here
        edwrite(sbuf, j, 1);
        return (1);
    }
    if (muste_strcmpi(parm[1], "SET") == 0) { // RS CHA 25.11.2012
        if (g < 3) {
            return (-1);
        }
        strcpy(sbuf, parm[2]);
        muste_expand_path(sbuf);
        if (muste_is_write_access(sbuf)) {
            strcpy(etmpd, sbuf); // muste_is_directory(sbuf) &&
        } else {
            sur_print("\nERROR! \"");
            sur_print(sbuf);
...
// 1=PASTE command 2=ctrl+shift
{
    ...
    */
    ...
    while (1) {
        ...
        if (mode == 3) {
            edread(x, j); // RS ADD 6.10.2012
        }
        muste_fieldcopy(pasteline, p, len);
        pasteline[len] = EOS; // RS CHA
        aputab[0] = EOS;
        for (i = 0; i < len; i++) { // RS 3.12.2012
            if ((unsigned char)pasteline[i] < 32) {
                ...
                kirjain[0] = pasteline[i];
                strcat(aputab, kirjain);
            }
        }
        if (mode == 3) {
...
static int copytofile(unsigned int j1, unsigned int j2, int alku) {
    ...
    if (muste_strcmpi(parm[3 + k], "TO") == 0) {
        ...
        *out = EOS;
        if (!muste_is_path(x)) {
            strcpy(out, edisk); // RS 17.5.2013 path
        }
        strcat(out, x);
    } else {
        strcpy(out, eout);
    ...
    i = output_open(out);
    if (i < 0) {
        return (-1); // RS 17.5.2013 error_check
    }
    for (j = j1; j <= j2; ++j) {
        edread(x, j);
        output_line(x + alku, out, 0);
    }
    output_close(out);
    return (1);
}
...
static int op_copy() {
    ...
    k = 1 - alku;
    j1 = edline2(parm[1 + k], 1, 1);
    if (j1 == 0) {
        return (-1);
    }
    j2 = edline2(parm[2 + k], j1, 1);
    if (j2 == 0) {
        return (-1);
    }
    if (g == 3 + k) {
        copytofile(j1, j2, alku);
        return (1);
    }
    if (muste_strcmpi(parm[3 + k], "TO") == 0) {
        copytofile(j1, j2, alku);
...
static int op_copyblock() // RS
{
    ...
    if (resout) {
        ...
        if (j2 >= 0) {
            insert_mode = atoi(sbuf);
            if (insert_mode == 1 && mr > 1) {
                mr--;
            }
        }
        block_from_store();
...
static int op_status() {
    ...
    if (muste_strcmpi(parm[1], "CAPSLOCK") == 0) {
        edwrite(space, j, 1);
        muste_sprintf(sbuf, "STATUS CAPSLOCK %d", s_caps_on());
        edwrite(sbuf, j, 1);
        return (1);
    }
    return (1);
...
int ractivate(int select) // RS NEW
{
    ...
    */
    ...
    /*
            muste_sprintf(sbuf,"%.*s",c2,copy+1);
...
static int op_output() {
    ...
    if (g > 1) {
        ...
        if (*x == '-') {
            *eout = EOS;
        } else if (*x == '.' || strchr(x, '\\') != NULL || strchr(x, '/') != NULL || strchr(x, '<') != NULL ||
                   strchr(x, '~') != NULL || strchr(x, ':') != NULL) { // RS ADD FIXME path
            strcpy(eout, x);
        } else {
            strcpy(eout, edisk);
            strcat(eout, x);
    ...
    edwrite(space, j, 1);
    strcpy(sbuf, eout);
    unsubst_survo_path_in_editor(sbuf); // 24.11.2001
    strcpy(x, "OUTPUT ");
    strcat(x, sbuf);
    edwrite(x, j, 1);
...
static int op_filetime() // 30.7.2008
{
    ...
    if (i == 1) {
        j = r1 + r;
        edwrite(space, j, 1);
        muste_sprintf(sbuf, "%s %s", pvm, klo);
        edwrite(sbuf, j, 1);
    }
    return (1);
}
...
static int op_textcols() {
    ...
    jj1 = edline2(parm[1], 1, 1);
    if (!jj1) {
        return (1);
    }
    jj2 = edline2(parm[2], jj1, 1);
    if (!jj2) {
        return (1);
    }
    lev = atoi(parm[3]);
...
static int help2() {
    ...
    */
    ...
    help_window = 0;
    help_window_open = 0; // RS FIXME TEMP
...
static int help(char *helpword) {
    ...
    move_clear();
    if (*help_sana == EOS || *helpword == '?') {
        strcpy(info, helpword);
    } else if (*help_sana == '>') {
        strcpy(info, "DOS");
    } else {
        strcpy(info, help_sana);
    }
    strcat(info, ">");
    if (qpath[0] != '"') {
        strcat(info, "\""); // RS ADD quotes if missing
    }
    strcat(info, qpath);
    if (qpath[0] != '"') {
        strcat(info, "\""); // RS ADD
...
static int op_files() {
    ...
    muste_sprintf(path, "%s/*.*", parm[1]); // RS CHA \\ -> /
...
static int op_dos() {
    ...
    if (q2 != NULL) {
        ...
        if (q2 > p) {
            if (*(q2 - 1) != ' ') {
                i = FALSE; // RS 20.2.2013
            }
        }
        if (q2[2] != PREFIX && q2[2] != '.' && q2[2] != ')' && q2[2] != ',' && i) {
            p = q2 + 1; // RS ADD
        }
    }
    strcpy(xx, p);
    ...
    *x = EOS;
    ...
    if (strcmp(parm[0], ">START") == 0) {
        set_win = 2; // RS 5.12.2012
    }
    if (strcmp(parm[0], ">SET_WIN") == 0 || strcmp(parm[0], ">SET_EWIN") == 0) {
        set_win = 1;
    }
    ...
    /* RS NYI
            if (i==3 && strcmp(parm[2],"/NEW_SURVO")==0)
                {
                set_win=1;
                muste_sprintf(x,"%sS.EXE",survo_path);
    ...
    */
    ...
    if (i > 1 && set_win == 0 &&
        (strcmp(parm[0], ">COPY") == 0 || strcmp(parm[0], ">copy") == 0)) // RS ADD set_win REM etu &&
    {
        if (i == 2) {
            parm[2] = nullpath;
        }
        ...
        if (strchr(parm[1], '*') == NULL && strchr(parm[1], '+') == NULL
            ...
            if (etu) {
                ...
                muste_sprintf(sbuf, "file.copy(\"%s\",\"%s\",overwrite=TRUE) # %s", parm[1], parm[2], parm[0]);
                /*
                                    if (strchr(parm[1],' ')!=NULL)
                                      {
                                      if (strchr(parm[2],' ')!=NULL)
                                        muste_sprintf(sbuf,"file.copy(\"'%s'\",\"'%s'\",overwrite=TRUE) #
                   %s",parm[1],parm[2],parm[0]); else muste_sprintf(sbuf,"file.copy(\"'%s'\",\"%s\",overwrite=TRUE) #
                   %s",parm[1],parm[2],parm[0]);
                                      }
                                    else
                                      {
                                      if (strchr(parm[2],' ')!=NULL)
    ...
    if (i > 1 && set_win == 0 &&
        ((strcmp(parm[0], ">DEL") == 0 || strcmp(parm[0], ">del") == 0)    // RS 30.1.2013 set_win
         || (strcmp(parm[0], ">RD") == 0 || strcmp(parm[0], ">rd") == 0))) // RS REM etu &&
    {
        muste_expand_path(parm[1]);
        if (etu) {
            i = sur_delete(parm[1]);
            ...
            //                	if (strchr(parm[1],' ')!=NULL && strchr(parm[1],'/')!=NULL)
            //                	  muste_sprintf(sbuf,"unlink(\"'%s'\") # %s",parm[1],parm[0]);
            //    				else
            muste_sprintf(sbuf, "unlink(\"%s\") # %s", parm[1], parm[0]);
            muste_copytofile(sbuf, muste_command);
            i = muste_evalsource(muste_command);
    ...
    if (i > 1 && set_win == 0 && (strcmp(parm[0], ">MD") == 0 || strcmp(parm[0], ">md") == 0)) // RS 30.1.2013 set_win
    {
        muste_expand_path(parm[1]);
        if (etu) {
            i = sur_make_dir(parm[1]);
        } else {
            //                	if (strchr(parm[1],' ')!=NULL && strchr(parm[1],'/')!=NULL)
            //                	  muste_sprintf(sbuf,"dir.create(\"'%s'\") # %s",parm[1],parm[0]);
            //                	else
            muste_sprintf(sbuf, "dir.create(\"%s\") # %s", parm[1], parm[0]);
            muste_copytofile(sbuf, muste_command);
            i = muste_evalsource(muste_command);
   ...
   */
    ...
    // Rprintf("\nx: %s\nsbuf: %s\nxx:%s",x,sbuf,xx);
    if (set_win == 2) {
        i = muste_system(sbuf + 6, 2); // RS 5.12.2012
    } else if (set_win == 1) {
        i = muste_system(sbuf, 0);
    } else {
        i = muste_system(sbuf, 1);
    }
...
int make_fence_stop_list(char *list) // 30.4.2010
{
    ...
    for (i = 0; i < n_fence_stop; ++i) {
        ...
        if (p == NULL) {
            ...
            *p = EOS;
            strcpy(fence_stop_list[i][1], p + 1);
        }
...
int fence_restore(int varatalletus, int rj) {
    ...
    if (varatalletus) {
        strcpy(survoblo, etmpd);
        strcat(survoblo, "SURVO1.BLO");
        mr = r1 + r;
        mc = 0;
        block_from_store();
    }
    strcpy(survoblo, etmpd);
    strcat(survoblo, "SURVO2.BLO");
    mr = rj;
    mc = 0;
    block_from_store();
    return (1);
...
int fence_activate() {
    ...
    while (j < r2) {
        ...
        if (fence_warning && strncmp(x, "*#", 2) == 0) {
            PR_EINV;
            muste_sprintf(sbuf, "\nFence line obviously missing above line %d!", j);
            sur_print(sbuf);
            WAIT;
            return (1);
        }
...
static int open_appl() {
    ...
    *(p + 1) = EOS;
    ...
    if (strchr(q, '.') == NULL) // 18.4.2003
    {
        ...
        strcat(nimi, "SYS/OPEN.SYS");
        open_sys = muste_fopen2(nimi, "rt");
        if (open_sys == NULL) {
            return (-1);
        }
        while (1) {
            p = muste_fgets(x, 100, open_sys);
            if (p == NULL) {
                muste_fclose(open_sys);
                return (-1);
            }
            ...
            *command = EOS;
            ...
            if (x[2] == '*') {
                ...
                if (x[1] == '0') {
                    if (strncmp(q, x + 2, strlen(x + 2)) == 0) {
                        ok = 1;
                        break;
                    }
                } else {
                    if (muste_strnicmp(q, x + 2, strlen(x + 2)) == 0) {
                        ok = 1;
                        break;
                    }
                }
    ...
    if (ok) {
        ...
        if (strncmp(q, "www.", 4) == 0) {
            if (strchr(q, '&') != NULL) {
                muste_sprintf(sbuf, "*%s 'http://%s'", command, q);
            } else {
                muste_sprintf(sbuf, "*%s http://%s", command, q);
            }
        } else if (strchr(q, '&') != NULL) {
            muste_sprintf(sbuf, "*%s '%s'", command, q);
        } else {
            muste_sprintf(sbuf, "*%s %s", command, q);
        }
...
int emptyline(int curline) {
    ...
    j = lastline2();
    while (i <= j) {
        if (strncmp(space, z + (i - 1) * ed1 + 1, (unsigned int)(ed1 - 1)) == 0) {
            break;
        }
        ++i;
    }
...
int activate() {
    ...
    {
        //            muste_sprintf(copy,"%sSND\\%s.WAV",survo_path,act_sound[0]);
    ...
    *info_2 = EOS;
    ...
    if (soft_act) {
        ...
        muste_snprintf(actline, LLENGTH, "?%s",
                       soft_actline); // strcpy(actline,"?"); strncat(actline,soft_actline,LLENGTH);
        soft_act = 0;
        soft_act2 = 1;
        // Rprintf("\nsoftact: %s",actline);
    } else {
    ...
    if (p == NULL) {
        ...
        if (p != NULL) {
            ...
            if (i == 0)
                ...
                p = NULL; // RS 20.2.2013
            else if (strncmp(actline + 1, "MAT LOAD ", 9) == 0 || strncmp(actline + 1, "LOADM ", 6) == 0)
                p = NULL; // RS 1.8.2013 skip MAT LOAD ## format if given without commas
            else if (!(actline[i - 1] == (char)PREFIX && actline[i - 2] != (char)PREFIX && p[1] != (char)'.' &&
                       p[1] != (char)')' && p[1] != (char)','))
                p = NULL;
            // RS double PREFIX needed for activation, but no triple or other special case
            //              else { p=NULL; }
    ...
    if (p == NULL && actline[1] == '#' && actline[2] != '#') // RS CHA 22.11.2012
    {
        ...
        {                                                    // Do editorial arithmetics 21.10.2011/SM
            ...
            g = split(p + 1, parm, MAX_FENCE_STOP);
            if (g == 1) {
                parm[1] = "";
            }
            ...
            *********************************/
            ...
            for (i = 0; i < n_fence_stop; ++i) {
                if (muste_strcmpi(parm[0], fence_stop_list[i][0]) == 0) {
                    if (*fence_stop_list[i][1] == EOS ||
                        (*fence_stop_list[i][1] != EOS && muste_strcmpi(parm[1], fence_stop_list[i][1]) == 0)) {
                        muste_sprintf(sbuf, "\nCannot activate %s %s (Remove # and activate again!)", parm[0], parm[1]);
                        PR_EINV;
                        sur_print(sbuf);
                        WAIT;
                        return (1);
            ...
            {
                muste_sprintf(copy, "%s#SURVO.EDT", edisk);
                edt_talletus(copy);
            }
    ...
    for (i = 0; i < g; ++i) {
        if (parm[i][0] == '/' && parm[i][1] == EOS) {
            break;
        }
    }
    g = i;
    if (g) {
        copy[parm[g - 1] - p + strlen(parm[g - 1])] = EOS; /* 9.12.1999 */
    }
    cursor(r, 1);
    PR_EINV;
    if (*actline != '?') {
        muste_sprintf(sbuf, "%.*s", c3, copy + 1);
        sur_print(sbuf);
    }

    muste_strncpy(op_tila, parm[0], OPLEN);
    op_tila[OPLEN - 1] = EOS;
    op = op_tila;
    child_call0 = 0;
    if (*actline == '\'') {
    ...
    if (muste_strcmpi(op, "S") == 0) {
        strcpy(op_tila, "SHOW"); // 26.4.2006
    }

    strcpy(OO, op);
    muste_strupr(OO);

    muste_fieldcopy(op_sana, OO, 8);
    op_sana[8] = EOS;
    ...
    // if (strlen(OO)>16)
    //   { Rprintf("OO=%s\n",OO); getck(); }
    if ((erun != 0 || etu != 0) && *actline == '/' && *(actline + 1) == '/' && *(actline + 2) == '/') {
        return (1); // RS Null-activate for comment lines in Sucros
    }

    if (strcmp(OO, "NOP") == 0) {
        return (1);                    // RS No operation activation
    ...
    muste_save_stack_count(1); // RS 9.3.2014
    if (strcmp(OO, "RBUF") == 0) {
        muste_rbuf_show = 1 - muste_rbuf_show;
        disp();
        return (1);
    }
    if (muste_rbuf_show) // RS 25.3.2013
    {
        ...
        if (i > 0) {
            ...
            muste_restore_stack_count(); // RS 9.3.2014
            return (1);
        }
    }
    ...
    if (strcmp(OO, "GOTO") == 0) {
        ...
        muste_restore_stack_count();
        return (1);
    } else if (strcmp(OO, "TKFONT") == 0) {
        muste_choosefont();
        muste_restore_stack_count();
        return (1);
    } else if (strcmp(OO, "SAVE") == 0) {
        op_save();
        if (etu == 0) {
            sur_wait(100L, nop, 1);
        }
    ...
    else if (strcmp(OO, "LOAD") == 0) {
        sur_dump();
        op_load();
        goto_load_ind = 1;
        muste_restore_stack_count();
        return (1);
    } else if (strcmp(OO, "SCRATCH") == 0) {
        sur_dump();
        op_scratch();
        muste_restore_stack_count();
        return (1);
    } else if (strcmp(OO, "PATHS") == 0) {
        muste_showpaths();
        muste_restore_stack_count();
        return (1);
    } else if (strcmp(OO, "CD") == 0 || strcmp(OO, "PATH") == 0 || strcmp(OO, "DISK") == 0) {
        op_cd();
        muste_restore_stack_count();
        return (1);
    } // RS CHA   { i=op_path(); return(i); }
    else if (strcmp(OO, "MD") == 0 || strcmp(OO, "MKDIR") == 0) {
        sur_make_dir(parm[1]);
        muste_restore_stack_count();
        return (1);
    } // RS NEW
    else if (strcmp(OO, "RD") == 0 || strcmp(OO, "RMDIR") == 0) {
        sur_remove_dir(parm[1]);
        muste_restore_stack_count();
        return (1);
    } // RS NEW
    else if (strcmp(OO, "WIN") == 0) {
        i = op_win();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "RESIZE") == 0) {
        sur_dump();
        i = op_resize();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "SET") == 0) {
        op_set();
        muste_restore_stack_count();
        return (1);
    } else if (strcmp(OO, "TIME") == 0) {
        sur_dump();
        i = op_time();
        muste_restore_stack_count();
        return (i);
    }
    // else    if (strcmp(OO,"TUTOR")==0)   { sur_dump(); i=op_tutor(); return(i); }
    else if (strcmp(OO, "-") == 0) {
        i = op_jump(1);
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "INIT") == 0 && g >= 3) {
        sur_dump();
        muste_restore_stack_count();
        return (op_init());
    } // g 8.8.03
    else if (strcmp(OO, "WAIT") == 0) {
        i = op_wait();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "WAIT_TUT") == 0) {
        i = op_wait_tut();
        muste_restore_stack_count();
        return (i);
    } // 30.12.2000
    else if (strcmp(OO, "QPATH") == 0) {
        i = op_qpath();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "COUNT") == 0) {
        sur_dump();
        i = op_count();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "CLEAR") == 0) {
        sur_dump();
        i = op_clear();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "INSERT") == 0 || strcmp(OO, "I") == 0) {
        sur_dump();
        insdel();
        muste_restore_stack_count();
        return (1);
    } else if (strcmp(OO, "DELETE") == 0 || strcmp(OO, "D") == 0) {
        sur_dump();
        insdel();
        muste_restore_stack_count();
        return (1);
    } else if (strcmp(OO, "CHECK") == 0) {
        i = op_check(1);
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "CHECK0") == 0) {
        i = op_check(0);
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "NEXTFILE") == 0) {
        i = op_nextfile();
        muste_restore_stack_count();
        return (i);
    } // 13.9.2000
    else if (strcmp(OO, "SOFTKEYS") == 0) {
        i = op_softkeys();
        muste_restore_stack_count();
        return (i);
    } // 15.3.2000
    else if (strcmp(OO, "TEMPDISK") == 0) {
        i = op_tempdisk();
        muste_restore_stack_count();
        return (i);
    } // 27.4.2004
    else if (strcmp(OO, "COLX") == 0) {
        i = op_colx();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "SYS") == 0 || strcmp(OO, "SYSDEL") == 0) {
        muuta_apu_tiedostoa(3);
        muste_restore_stack_count();
        return (1);
    } // 14.7.2006

    else if (strcmp(OO, "SYSTEM") == 0) {
        i = survoapu1(0, NULL);
        muste_restore_stack_count();
        return (i);
    }

    else if (strcmp(OO, "COPY") == 0) {
        sur_dump();
        op_copy();
        muste_restore_stack_count();
        return (1);
    } else if (strcmp(OO, "PASTE") == 0) {
        sur_dump();
        op_paste(1);
        muste_restore_stack_count();
        return (1);
    ...
    else if (strcmp(OO, "STATUS") == 0) {
        op_status();
        muste_restore_stack_count();
        return (1);
    }

    else if (strcmp(OO, "INSERTL") == 0) {
        sur_dump();
        op_insertl();
        muste_restore_stack_count();
        return (1);
    } // 10.6.2006
    else if (strcmp(OO, "DELETEL") == 0) {
        sur_dump();
        op_deletel();
        muste_restore_stack_count();
        return (1);
    } // 18.6.2006
    else if (strcmp(OO, "LINEINS") == 0) {
        sur_dump();
        op_lineins();
        muste_restore_stack_count();
        return (1);
    ...
    else if (strcmp(OO, "OUTPUT") == 0) {
        i = op_output();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "ROUT") == 0) {
        i = op_rout();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "SETUP") == 0) {
        i = op_setup();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "SHADOW") == 0) {
        sur_dump();
        i = op_shadow();
        muste_restore_stack_count();
        return (i);
    } else if (strcmp(OO, "SESSION") == 0) {
        i = op_session();
        muste_restore_stack_count();
        return (i);
    } // 7.4.2000
    else if (strcmp(OO, "PUTENV") == 0) {
        putenv(parm[1]);
        muste_restore_stack_count();
        return (1);
    } // 24.3.2005
    else if (strcmp(OO, "LOWLINE") == 0) {
        op_lowline();
        muste_restore_stack_count();
        return (1);
    } // 25.6.2006
    else if (strcmp(OO, "FILETIME") == 0) {
        op_filetime();
        muste_restore_stack_count();
        return (1);
    } // 30.7.2008
    else if (strcmp(OO, "COLOR") == 0) {
        i = op_color();
        muste_restore_stack_count();
        return (i);
    } // 30.12.2000
    else if (strcmp(OO, "TEXTCOLS") == 0) {
        sur_dump();
        i = op_textcols();
        muste_restore_stack_count();
        return (i);
    } // 17.3.2001
    else if (strcmp(OO, "FILES") == 0) {
        op_files();
        muste_restore_stack_count();
        return (1);
    } // 9.6.2005
    else if (strcmp(OO, "NET") == 0) {
        op_net();
        muste_restore_stack_count();
        return (1);
    } // 13.2.2006
    else if (strcmp(OO, "EXT_FUNC") == 0) {
        sur_dump();
        op_ext_func();
        muste_restore_stack_count();
        return (1);
    ...
    else if (strcmp(OO, "FIND") == 0 || strcmp(OO, "REPLACE") == 0 || strcmp(OO, "-FIND") == 0 ||
             strcmp(OO, "-REPLACE") == 0) {
        sur_dump();
        first_word_on_line_search = 0;
        i = op_find();
        muste_restore_stack_count();
        return (i);
    }

    else if (strcmp(OO, "MASK") == 0 || strncmp(OO, "MASK=", 5) == 0 || strcmp(OO, "VARS") == 0 ||
             strncmp(OO, "VARS=", 5) == 0) {
        sur_dump();
        file_act("MASK");
        muste_restore_stack_count();
    ...
    else if (strcmp(OO, "HELP") == 0) {
        i = help("HELP");
        muste_restore_stack_count();
        return (1);
    } else if (strcmp(OO, "F") == 0) {
        i = help("F");
        muste_restore_stack_count();
        return (1);
    ...
    else if ((*OO == 'C' || *OO == 'L') && strchr(OO, '?') == NULL && strchr("+-*/%", OO[1]) != NULL &&
             strchr(OO, '=') == NULL) {
        strcpy(op, "EDI2");
        strcpy(pref, "&");
    }
    ...
    */
    muste_restore_stack_count();
    // muste_show_resource_usage(); WAIT;
    muste_dump();
    ...
    */
    ...
    if (strcmp(OO, "INIT") == 0) {
        muste_sprintf(info, "%s %d", ser_number, ver); // 10.8.2003
    }
...
int write_wordk() {
    ...
    */
    ...
    while (*p != EOS && *p != '@') {
        ...
        /*
        z[ed1*(r1+r-2)+c1+vc-1]=*p;
        if (vc<c3) { muste_sprintf(sbuf,"%c",*p); sur_print(sbuf); }
        ++vc; ++p;
        */
    }
    /*    if (vc<=c3) c=vc;    */
...
void prefix() {
    ...
    if (special) {
        ...
        case CODE_EXEC: /* RUN-aktivointi */
            ...
            while (erun) {
                ...
                if (time_file_on) {
                    file_time_start(); // 12.11.2003
                }
                i = activate();
                if (time_file_on) {
                    file_time_end(parm[0]);
                }
    ...
    } else /* not special */
    {
        ...
        case 'R':
        case 'r':
            muste_sprintf(x, "%d@%d", r1 + r - 1, c1 + c - 1);
            tutcat(x);
            break;
        ...
        case 'L':
            *x = (char)nextch_editor();
            ...
            /* 18.8.00  */ else if (*x == 'K') {
                muste_sprintf(x, "%d", end_empty_lines());
                tutcat(x);
                break;
            }
            ...
            /* 23.2.2003 */ else if (*x == 's') {
                muste_sprintf(x, "%.2f", 0.01 * (double)ver);
                tutcat(x);
                break;
            } else if (*x == 'S') {
                *x = (char)nextch_editor();
            ...
            {
                muste_sprintf(x, "%d", r3);
                tutcat(x);
                break;
            } else if (*x == 'z') // 5.5.2004
            {
                muste_sprintf(x, "%d", c3);
                tutcat(x);
                break;
            }
        ...
        case 'w':
            ...
            *p = EOS;
            if (paint_on) {
                sur_paint(x, q - x + 1, p - x - 1, '7'); // 17.11.1996
            }
            tutcat(q + 1);
        ...
        case 148:
            sur_user_name();
            break; // 21.2.93      // RS pikku ö

        case 'F':
            type_without_limit = 1;
            break;
        case 'f':
        ...
        case 'U':
            ...
            break;
        case ',':
            if (sucro_pause) // muutettu 21.8.2004
            {
                sucro_pause = 0;
                r = r_pause;
                r1 = r1_pause;
                ...
                etu = 2;
                tut_avaa();
                //  muste_sprintf(sbuf,"%sSND\\SIB23A.WAV",survo_path);
                //  sur_play_sound(sbuf);
                tut_sound("3");
...
static int load_codes2(char *codefile, unsigned char *code, int col) {
    ...
    if (col > 1) {
        muste_fseek(codes, (long)(col - 1) * 256L, SEEK_SET);
    }
    for (i = 0; i < 256; ++i) {
        code[i] = (unsigned char)getc(codes);
    }
    muste_fclose2(codes);
    return (1);
}
...
static int w_codes_load(int k, unsigned char code[]) {
    ...
    strcpy(codefile, survo_path);
    strcat(codefile, "SYS/WIN.BIN");
...
static int www_remove_spec_chars(char *s) {
    ...
    while (1) {
        if (*p != EOS && strchr(merkit, *p) != NULL) {
            ++p;
        } // RS 5.9.2013 ADD *p!=EOS
        else {
            break;
        }
    }
...
static int get_www(char *site, char *s, char sep, char *quotes) // 10.4.2008
{
    ...
    if (mc1 && mc2) {
        ...
        *clip = EOS;
        ...
        ++i;
        j = 0;
        while (j < c2 - 10 && sbuf[i] != ' ') {
            clip[j++] = sbuf[i++]; // SM ADD j<c2-10 && 22.7.2012
        }
        clip[j] = EOS;
        p = clip;
    ...
    *open_copy = EOS;
    ...
    {
        if ((unsigned char)clip[i] < 32 ||
            (unsigned char)clip[i] >
                127) // || clip[i]==91 || clip[i]==92 || clip[i]==93 || clip[i]==94 || clip[i]==95 || clip[i]==96)
        {
            muste_sprintf(sbuf, "%s%%%2x", open_copy, (unsigned char)clip[i]);
        } else {
            muste_sprintf(sbuf, "%s%c", open_copy, (unsigned char)clip[i]);
        }
    ...
    edread(open_copy, r1 + r - 1);
    edwrite(space, r1 + r - 1, 1);
    muste_sprintf(sbuf, "*%s%s / ", site, clip);
    edwrite(sbuf, r1 + r - 1, 0);
    cc = c;
    c = 1;
    activate();
...
static int open_any_file() {
    ...
    edread(open_copy, r1 + r - 1);
    edwrite(space, r1 + r - 1, 1);
    // RS CHA    muste_sprintf(sbuf,">>START %s",clip);
    muste_sprintf(sbuf, "*/OPEN %s", clip);
    edwrite(sbuf, r1 + r - 1, 0);
    cc = c;
    c = 1;
    erunn = erun;
...
static int show_line_labels() {
    ...
    for (j = 0; j < n; ++j) {
        ch = (char)255;
        for (i = 0; i < n; ++i) {
            if ((unsigned char)x[i] < (unsigned char)ch) {
                h = i;
                ch = x[i];
            }
        }
...
static int display_name_of_variable() {
    ...
    *p = EOS;
    muste_sprintf(info, "GET_VAR_NAME %s", q);
    op = "CREATE";
    //    childp("FI\\");
    muste_dump();
    muste_file_create(arguc, arguv);
...
int muste_save_firstline() // RS ADD 26.9.2012
{
    ...
    LOCATE(r3 + 2, 1);
    PR_EBLK;
    muste_sprintf(sbuf, "Edit field %s saved!", parm[1]);
    sur_print(sbuf);
    PR_ENRM;
    sur_wait(1000L, nop, 1);
    disp();
...
void muste_save_firstline_name(char *name) // RS ADD 26.9.2012
{
    ...
    edread(actline, 1);
    g = splitq(actline + 1, parm, MAXPARM);
    muste_sprintf(sbuf, "*SAVE %s", name);
    if (g > 0) {
        strcpy(OO, parm[0]);
        muste_strupr(OO);
        if (strcmp(OO, "SAVE") == 0) {
            ...
            muste_sprintf(sbuf, "*SAVE %s%s", name, loppu);
        } else {
            oc = c;
            oc1 = c1;
            or = r;
...
static int muste_clearselection(void) {
    ...
    if (insert_type && insert_mode) {
        ...
        for (j = move_r1; j <= move_r2; j++) {
            ...
            if (len > mc2) {
                memmove(x + mc1, x + mc2 + 1, strlen(x + mc2 + 1) + 1);
            } else {
                x[mc1] = EOS;
            }
...
static int get_longest_line() {
    ...
    for (j = 1; j <= r2; ++j) {
        ...
        if (ch == '.' && strncmp(x + 1, piste, len) == 0) {
            continue;
        }
        if (ch == '#' && strncmp(x + 1, risu, len) == 0) {
            continue;
        }
        if (len > max_len) {
            max_len = len;
...
int prefix2() {
    ...
    if ((char)m >= '0' && (char)m <= '9') {
        ...
        *quotes = EOS;
        ...
        // getck();
        get_www(sbuf2, term, separator, quotes);
        return (1);
    }
    // Rprintf("\nm=%c",(char)m); getck();
    // get_www(program, additional term, separator, quotes)
    ...
    case 'v':
    case 'V':
        display_name_of_variable();
        return (1);
    case '#':
        modify_command_and_set_fence_line();
        return (1);
    case 'A': // 15.3.2012
        start_survopoint_disp();
        return (1);
    case 'a':
        stop_survopoint_disp();
        return (1);
    ...
    case 'E': // RS ADD
        ...
        m = nextch_editor();
        muste_emacs = FALSE;
        sur_dump(); // RS 9.10.2012 required for undo
        switch (m) {
        ...
        case 'X': // cut
            if (insert_mode == 0) {
                muste_cutselection(1);
            } else {
                muste_cutselection(2);
            }
            return (1);
        case 'x': // shift-cut
...
int key_special(int m) {
    ...
    case CODE_EXEC:
        ...
        *op_sana = EOS; // 10.3.1997
        if (etu == 1) {
            lue_hetki(&wait_hetki); // 26.5.1995
        }
        space_break = 0;
        PR_ENRM;
        if (k <= 1) {
    ...
    case CODE_REF:
        ...
        if (((ref_c1 + ref_c == c1 + c) && (ref_r1 + ref_r == r1 + r)) || (ref_r1 + ref_r - 1 > r2) ||
            (ref_c1 + ref_c - 1 > c2)) {
            ref_c1 = 0;
            break;
        }
    ...
    default:
        ...
        if (c <= c3 && c <= c2) {
            ...
            if (type_without_limit && c == c3 && c <= c2) /* 2.4.1997 */
            {
                ...
                ++c; // RS ALT only with if(c1+c-1<c2) not working as otherwise no automatic next line
            }
            break;
        }
...
static int init_sapu(char *apufile, int offset) // RS 1.11.2012 offset
{
    ...
    *p = EOS;
    ...
    if (apu0 == NULL) {
        //            muste_sprintf(sbuf,"\nFile %s missing!",afile); // RS Rprintf -> muste_sprintf
        //            sur_print(sbuf);
        //            WAIT;
...
static void edscratch(unsigned lin /* ens.tyhjennettävä rivi */
) {
    unsigned int i, j, l;
    l = (lin - 1) * ed1;
    for (j = lin; j <= ed2; ++j) {
...
static int load_sessions(char *nimi) {
    ...
    if (sessions == NULL) {
        return (-1); // RS CHA exit(0); // 19.3.2004
    }
    for (i = 0; i < N_SESS; ++i) {
...
static int remove_current_session() {
    ...
    strcpy(nimi, sbuf);
    strcat(nimi, "SESSIONS.SYS");
    load_sessions(nimi);
    i = sur_session[0] - 'A' + 1;
    edwrite(space, i, 1);
    muste_sprintf(x, "%c 0", sur_session[0]);
    edwrite(x, i, 1);
    save_sessions(nimi);
    muste_sprintf(x, "curses ");
    muuta_apu_tiedostoa_core(2, x); // RS 13.2.2014
    return (1);
}
...
static int by_session() {
    ...
    muste_sprintf(muste_clipfile, "%sMUSTE.CLP", sur_session); // RS ADD
    muste_sprintf(muste_command, "%sMUSTE.CMD", sur_session);  // RS ADD
    return (1);
}
...
static int set_sur_session() {
    ...
    muste_sprintf(sbuf, "%c %d", (char)('A' + vapaa), viim + 1);
    ...
    for (i = 0; i < N_SESS; ++i) { // tyhjentaa alkurivit!
        edwrite(space, i + 1, 1);
    }
    by_session();

    muste_sprintf(x, "curses=%c,%d,%s", sur_session[0], muste_dumpcount, etmpd);
    muuta_apu_tiedostoa_core(1, x); // RS 13.2.2014
...
static void muste_set_sysname() // RS ADD 22.9.2011
{
    ...
    muste_get_R_string(sysname, ".muste$sysname", LLENGTH);
    muste_strlwr(sysname);
    muste_sprintf(sbuf, "SYSTEM sysname=%s", sysname);
    survoapu1(1, sbuf);
    ...
    muste_sprintf(sbuf, "SYSTEM R_path=%s", sysname);
    survoapu1(1, sbuf);
    ...
    if (!i) {
        ...
        p = strchr(sysname, ' ');
        if (p == NULL) {
            muste_sprintf(sbuf, "SYSTEM R_command=%s", sysname);
        } else {
            muste_sprintf(sbuf, "SYSTEM R_command=\"%s\"", sysname);
        }
        survoapu1(1, sbuf);
...
static int muste_editor_init(char *apufile, int tunnus) {
    ...
    *op_sana = EOS;
    ...
    strcpy(sana, survo_path);
    strcat(sana, "SURVO.APU"); // RS 1.11.2012
    ...
    */
    ...
    if (i) {
        ...
        muste_sprintf(sbuf, ".muste.findfontsize(x=%d,y=%d)", atoi(osa[0]), atoi(osa[1]));
        muste_evalr(sbuf);
    ...
    prind = 0;
    i = hae_apu("prind", sana);
    if (i) {
        prind = atoi(sana); // RS ADD 13.11.2012
    }
    ...
    *muste_rout = EOS;
    strcpy(sbuf, survo_path);
    i = strlen(sbuf); // 6.3.1999
                      // RS REM sbuf[i-2]=EOS; // formally <Survo>   // RS i-2 poistaa ihan liikaa
    strcpy(qpath, sbuf);
    strcat(qpath, "Q/EDQ"); // RS KORJAA filesep
    hae_edisk(edisk);       // 25.3.1991, 6.1.93

    //        strcpy(etmpd,sbuf); strcat(etmpd,"TMP/"); // 26.10.1996   RS CHA \\ -> /
    i = muste_get_R_int(".muste$writeaccess");
    if (i) {
        strcpy(etmpd, survo_path);
        strcat(etmpd, "TMP/");
    } else {
        muste_get_R_string(etmpd, ".muste$Rtempdir", LLENGTH);
    }
    hae_apu("tempdisk", etmpd);
    subst_survo_path_in_editor(etmpd);
    ch = etmpd[strlen(etmpd) - 1];
    if (ch != '/' && ch != '\\') {
        strcat(etmpd, "/");
    }
    strcpy(eout, etmpd);
    strcat(eout, "RESULTS"); // RS CHA \\ -> /
    ...
    if (ch != '/' && ch != '\\') {
        strcat(muste_homedir, "/");
    }

    hae_apu("eout", eout);
    subst_survo_path_in_editor(eout);
    hae_apu("rout", muste_rout);
    subst_survo_path_in_editor(muste_rout);
    hae_apu("last_disk", last_disk);
    hae_apu("qpath", qpath);
    subst_survo_path_in_editor(qpath);
    speclist = SPECLIST;
    i = hae_apu("speclist", sana);
    if (i) {
        speclist = atoi(sana);
    }

    specmax = SPECMAX;
    i = hae_apu("specmax", sana);
    if (i) {
        specmax = atoi(sana);
    }
    scale_check = 0; /* no scale type checks */
    i = hae_apu("scale_check", sana);
    if (i) {
        scale_check = atoi(sana);
    }
    accuracy = 7; /* # of significant digits in free format results etc. */
    i = hae_apu("accuracy", sana);
    if (i) {
        accuracy = atoi(sana);
    }
    if (accuracy < 4) {
    ...
    i = hae_apu("results", sana);
    if (i) {
        results = atoi(sana);
    }
    if (results < 0) {
        results = 0; // RS accuracy -> results
    }
    if (results > 100) {
        results = 100; // RS accuracy -> results
    }
    ...
    i = hae_apu("insert_type", sana);
    if (i) {
        insert_type = atoi(sana);
    }
    ins_lines_on = 0; /* 1=new_lines_automatically_in_insert */
    i = hae_apu("insert_lines", sana);
    if (i) {
        ins_lines_on = atoi(sana);
    }

    one_click_copy = 1;
    i = hae_apu("one_click_copy", sana);
    if (i) {
        one_click_copy = atoi(sana); // 13.11.2012
    }
    ...
    */
    ...
    child_wait = 0;
    i = hae_apu("child_wait", sana);
    if (i) {
        child_wait = atoi(sana);
    }
    autosave = 0;
    i = hae_apu("autosave", sana);
    if (i) {
        autosave = atoi(sana);
    }
    ...
    *tut_info = EOS;
    shad_off = 0;
    for (i = 0; i < 256; ++i) {
        shad_active[i] = (unsigned char)i;
    }
    space_break2 = 1;
    i = hae_apu("space_break", sana);
    if (i) {
        space_break2 = atoi(sana);
    }
    *sucropath = EOS;
    i = hae_apu("sucropath", sucropath);
    if (i) // 1.12.2007
    {
        i = strlen(sucropath) - 1;
        if (sucropath[i] != '\\') {
            strcat(sucropath, "\\"); // 26.7.2005
        }
    }
    *break_sucro = EOS;
    i = hae_apu("break_sucro", break_sucro);
    i = hae_apu("pre_ctnue", sana);
    if (i) {
        pre_ctnue = atoi(sana);
    }
    i = hae_apu("loudness", sana);
    if (i) {
        loudness = atoi(sana);
    }
    i = hae_apu("s84_warning", sana);
    if (i) {
        s84_warning = atoi(sana);
    }
    //      spawn_max=0; i=hae_apu("spawn_max",sana);
    //      if (i) spawn_max=atoi(sana);
    child_call = 0;
    i = hae_apu("child_call", sana);
    if (i) {
        child_call = atoi(sana);
    }
    i = hae_apu("output_level", sana);
    if (i) {
        output_level = atoi(sana);
    }

    mat_parser = 0;
    i = hae_apu("mat_parser", sana);
    if (i) {
        mat_parser = atoi(sana);
    }
    i = hae_apu("help_window", sana);
    if (i) {
        help_window = atoi(sana);
    }
    ...
    if (i) {
        ...
        if (apu == NULL) {
            muste_sprintf(sbuf, "<Survo>/SYS/%s", gplot_layout);
            apu = muste_fopen2(sbuf, "rt");
            if (apu == NULL) {
                *gplot_layout = EOS;
            } else {
                strcpy(gplot_layout, sbuf);
            }
        }
        muste_fclose(apu);
    }
    ...
    key_sleep = 0;
    i = hae_apu("key_sleep", sana);
    if (i) {
        key_sleep = atoi(sana);
    }

    i = hae_apu("display_keys", sana);
    if (i) {
        display_keys = atoi(sana);
    }
    i = hae_apu("exit_warning", sana);
    if (i) {
        exit_warning = atoi(sana);
    }
    i = hae_apu("save_warning", sana);
    if (i) {
        save_warning = atoi(sana);
    }
    tut_wait_c = 10;
    i = hae_apu("sucro_speed", sana);
    if (i) {
        tut_wait_c = atoi(sana);
    }

    i = hae_apu("search_caps", sana);
    if (i) {
        search_caps = atoi(sana);
    }
    // 20.4.2002
    tmp_by_session = 0;
    i = hae_apu("session_tmp", sana);
    if (i) {
        tmp_by_session = atoi(sana);
    }
    del_tmp = 1;
    i = hae_apu("del_tmp", sana);
    if (i) {
        del_tmp = atoi(sana);
    }
    spec_check = 0; // 1.6.2004
    i = hae_apu("spec_check", sana);
    if (i) {
        spec_check = atoi(sana);
    }
    show_lines = 0; // 13.4.2006
    i = hae_apu("show_lines", sana);
    if (i) {
        show_lines = atoi(sana);
    }

    minus_paths = 0; // 30.9.2010
    i = hae_apu("minus_paths", sana);
    if (i) {
        minus_paths = atoi(sana);
    }

    survopoint = '~'; // RS CHA EOS; // 15.3.2012
    i = hae_apu("Survopoint", sana);
    if (i) {
        survopoint = *sana;
    }
    survopoint_on = 0;
    i = hae_apu("Survopoint_on", sana);
    if (i) {
        survopoint_on = atoi(sana);
    }
    survopoint_disp = 30; // RS CHA 0 -> 30
    i = hae_apu("Survopoint_disp", sana);
    if (i) {
        survopoint_disp = atoi(sana);
    }
    ...
    i = hae_apu("fence_save", sana);
    if (i) {
        fence_save = atoi(sana);
    }

    fence_warning = 1; // 21.4.2010
    i = hae_apu("fence_warning", sana);
    if (i) {
        fence_warning = atoi(sana);
    }

    n_fence_stop = 0; // RS ADD
    i = hae_apu("fence_stop", sbuf);
    if (i) {
        make_fence_stop_list(sbuf);
    }
    ...
    muste_snprintf(language, 32, "%.31s", space);
    language[30] = 'X'; //*language=EOS; strncat(language,space,31); language[30]='X';
    *language = '0';    // RS ADD
    p = strchr(info_s, '~');
    if (p != NULL) {
        if (*(p + 1) == 'E') {
    ...
    muste_alkutheme = 1;
    strcpy(inittheme, "WHITE"); // RS 14.11.2012
    i = hae_apu("theme", sana);
    if (i) {
        muste_strncpy(inittheme, sana, 64);
        if (strcmp(sana, "CLASSIC") == 0) {
            muste_alkutheme = 0;
        }
    }
    ...
    if (i) {
        if (sur_file_exists(sana)) {
            i = muste_evalsource(sana);
        }
    }

    check_stack = 1000000L;
    i = hae_apu("check_stack", sana);
    if (i) {
        check_stack = atol(sana);
    }
    muste_sprintf(sbuf, "%s*.TMP", etmpd);
    sur_delete(sbuf);

    if (!tmp_by_session) {
        i = sys_save_restore(1);
        if (i < 0) {
            return (-1);
        }
    } // 26.2.2001
...
{
    unsigned int i;
    char x[LLENGTH], x1[LLENGTH], x2[LLENGTH], xses[LLENGTH];
    int realdumpcount;              // RS 13.2.2014
    int muste_restored_session = 0; // RS 13.2.2014
                                    // RS REM        int m=0;
    int k;
    int oldc, oldr; // RS 13.2.2014
                    // RS REM        char *p;
    ...
    *orig_setup = EOS; // RS Alustetaan varmuuden vuoksi
    ...
    if (!i) {
        muste_snprintf(
            orig_setup, LNAME, "%sSURVO.APU",
            survo_path); // muste_strncpy(orig_setup,survo_path,LNAME); strncat(orig_setup,"SURVO.APU",LNAME);
    }
    ...
    */
    ...
    k = muste_editor_init(orig_setup, 1);
    if (k < 0) {
        return (-1);
    }
    edrun = 1;
    ...
    */
    ...
    muste_restored_session = 0; // RS 5.5.2014

    i = hae_apu("force_restore", x);
    if (i) {
        muste_restored_session = atoi(x);
    }

    if (muste_restored_session) {

        muste_restored_session = 0; // RS 13.2.2014

        strcpy(x, etmpd);
        strcat(x, "SESSIONS.SYS");
        *xses = EOS;
        i = sur_find_file(x);
        if (i != 0) // RS 13.2.2014
        {
            i = hae_apu("curses", x);
            if (i) {
                realdumpcount = muste_dumpcount;
                strcpy(x1, etmpd);
                strcpy(x2, edisk);
                i = split(x, parm, 3);
                strcpy(sur_session, parm[0]);
                muste_dumpcount = atoi(parm[1]);
                strcpy(etmpd, parm[2]);

                restore_dump();
                muste_restored_session = 1;
                muste_dumpcount = realdumpcount;
                strcpy(etmpd, x1);

                i = sur_file_exists(edisk);
                if (!i) {
                    strcpy(edisk, x2);
                }
    ...
    i = 0;
    if (muste_restored_session) // RS 13.2.2014
    {
        muste_sprintf(x, "%d,%d", oldr, oldc);
        i = 1;
    } else {
    ...
    if (muste_restored_session) // RS 13.2.2014
    {
        muste_sprintf(sbuf, "Restoring previous session! %s", xses);
    ...
    if (strcmp(x1, "<empty>") != 0) {
        alkututor = 2; // RS 30.1.2013
    } else {
        muste_sprintf(sbuf, "%sMUSTE.STA", muste_startpath);
        if (sur_find_file(sbuf)) {
            alkututor = 1; // RS ADD
        } else {
            if (hae_apu("start_sucro", x1)) {
                alkututor = 2;
            }
        }
    }
    ...
    if (alkututor) {
        ...
        if (alkututor == 2) {
            muste_snprintf(start_etufile, LNAME, "%s %s", "alkutut", x1); // RS 25.11.2012 parm[1]=x1;  //  XXXX 14.7.92
        } else {
            FILE *apu; // RS ADD
            muste_sprintf(sbuf, "%s/MUSTE.STA", muste_startpath);
            apu = muste_fopen2(sbuf, "rt");
            yys(sbuf);
            muste_fclose(apu);
        ...
        strcpy(sbuf, parm[1]);
        muste_append_path(sbuf, ".TUT");

        if (sur_find_file(sbuf)) {
            muste_sprintf(sbuf, "Start sucro found: %s", parm[1]);
            write_string(sbuf, strlen(sbuf), '1', 4, 3);
            LOCATE(4, 3);
            PR_EINV;
            muste_sleep(1000);
...
int fconv(double luku, char muoto[], char sana[]) {
    ...
    if (*muoto == EOS) {
        if (fabs(luku) >= 1 || fabs(luku) < 1e-14) {
            muste_sprintf(sana, "%.14g", luku);
        } else {
            if (luku == 0) {
                strcpy(sana, "0");
                return (1);
            }
            muste_sprintf(sana, "%.14f", luku);
            len = strlen(sana) - 1;
    ...
    muste_sprintf(sana, "%*.*f", len, dec, luku);
    if (strlen(sana) > len) {
        return (-1);
    }
    return (1);
...
int linerr2(char sana[]) {
    /*
              PR_EINV;
              muste_sprintf(sbuf,"Line '%s' not found!",sana);
              if (dsp) return(1);
              sur_print("\n"); sur_print(sbuf);
              WAIT;
    */
...
int lastline2() {
    ...
    j = ed2;
    while (j > 0) {
        if (strncmp(space, z + (j - 1) * ed1 + 1, (unsigned int)(ed1 - 1)) != 0) {
            break;
        }
        --j;
    }
...
/* virheilm 1=virheilmoitus 0=ei virheilmoitusta */
{
    ...
    } else if (SANA[0] == '-') // && strlen(SANA)==1) // RS 8.5.2014
    {
        ...
        for (j = lin1; j <= ed2; ++j) {
            if (*(z + (j - 1) * ed1) == *p) {
                break; // RS 9.1.2013 sana -> p (also all p below)
            }
        }
        if (j > ed2) {
            if (virheilm) {
...
int hae_apu(char *s, char *t) {
    ...
    {
        muste_get_R_string(t, ".muste$sysname", 64);
        muste_strlwr(t);
        //		muste_sprintf(t,"windows");
        return (1);
    }
...
int survoapu1(int h, char *s) {
    ...
    *(q + 1) = EOS;
    ...
    if (strcmp(p, "speclist=") == 0) {
        speclist = atoi(par);
    }
    if (strcmp(p, "specmax=") == 0) {
        specmax = atoi(par);
    }
    if (strcmp(p, "scale_check=") == 0) {
        scale_check = atoi(par);
    }
    if (strcmp(p, "accuracy=") == 0) {
        accuracy = atoi(par);
    }
    if (strcmp(p, "results=") == 0) {
        results = atoi(par);
    }
    if (strcmp(p, "output_level=") == 0) {
        output_level = atoi(par);
    }
    if (strcmp(p, "mat_parser=") == 0) {
        mat_parser = atoi(par);
    }
    if (strcmp(p, "help_window=") == 0) {
        help_window = atoi(par);
    }
    if (strcmp(p, "videomode=") == 0) {
        strcpy(videomode, par);
    }
    if (strcmp(p, "search_caps=") == 0) {
        search_caps = atoi(par);
    }
    if (strcmp(p, "spec_check=") == 0) {
        spec_check = atoi(par);
    }
    if (strcmp(p, "show_lines=") == 0) {
        show_lines = atoi(par);
    }
    ...
    if (strcmp(p, "cursor=") == 0) {
        i = split(par, sana, 2);
        if (i > 0) {
            cur_par[0] = atoi(sana[0]);
        }
        if (i > 1) {
            cur_par[1] = atoi(sana[1]);
        }
    }
...
int sys_save_restore(int k) // 1=SAVE 2=RESTORE
{
    ...
    if (k == 1) {
        ...
        for (i = 0; i < strlen(gplot_layout); i++) {
            if (gplot_layout[i] == '*') {
                gplot_layout[i] = ' '; // RS ADD
            }
        }
        muste_fscanf(temp_apu, "%s\n", sbuf);
        if (strcmp(sbuf, "-") == 0) {
...
int edwrite(char *x, unsigned int lin, unsigned int col) {
    ...
    if (lin < 1 || lin > (unsigned int)(ed2 + edshad)) {
        muste_sprintf(sbuf, "Line number error! (%u)\n", lin);
        /*            if (dsp) return(1);    */
        sur_print(sbuf);
        WAIT;
...
*/
{
    ...
    for (p = 0; p < max; p++) {
        sana[p] = muste_nullstring; // RS ADD
    }
...
static void childp_restore() {
    ...
    */
    ...
    if (etu == 1) {
        lue_hetki(&wait_hetki); /* 27.5.1995 */
    }

    if (medit) {
        medit_restore(); // 30.4.2003
    }
    if (child_wait) {
        sur_wait((long)(100L * child_wait), nop, 1);
    }
...
int sur_dump() {
    ...
    muste_sprintf(sbuf, "MUSTE%.2d.EDT", muste_dumpcount % muste_dumpcountmax); // RS CHA 9.10.2012
    strcat(x, sbuf);
    edt_talletus(x);
    autosave32 = 0;
    ...
    muste_sprintf(sbuf, "MUSTE%.2d.DMP", muste_dumpcount % muste_dumpcountmax); // RS CHA 9.10.2012
    strcat(x, sbuf);
...
void muste_dump() {
    ...
    sur_dump();
    muste_last_stack_count = muste_save_stack_count(2);
}
...
int s_edt(char *siirtop) {
    restore_dump(); // RS CHA siirtop -> sur_session
...
int s_end(char *siirtop) {
    ...
    ued2 = ed2;
    uedshad = edshad; // RS
    sur_dump();       // RS CHA siirtop -> sur_session
    redim_save = 0;   // RS
    muste_remarks_return = FALSE;
    return (1);
}
...
int s_init(char *siirtop) {
    ...
    */
    ...
    q2 = strstr(p, "##");
    if (q2 != NULL) {
        if (strncmp(comline + 1, "MAT LOAD ", 9) == 0 || strncmp(comline + 1, "LOADM ", 6) == 0) {
            p = comline; // RS 1.8.2013 skip MAT LOAD ## format if given without commas
        } else {
            i = TRUE;
            if (q2 > p) {
                if (*(q2 - 1) != ' ') {
...
int s_init_extrasplit() {
    ...
    q2 = strstr(p, "##");
    if (q2 != NULL) {
        if (strncmp(comline_org + 1, "MAT LOAD ", 9) == 0 || strncmp(comline_org + 1, "LOADM ", 6) == 0) {
            p = comline_org; // RS 1.8.2013 skip MAT LOAD ## format if given without commas
        } else {
            i = TRUE;
            if (q2 > p) {
                if (*(q2 - 1) != ' ') {
    ...
    while (i < g_org && strcmp(word_org[i], "/") != 0) {
        ++i;
    }
    g_org = i;
    return (1);
...
int sp_check() {
    ...
    if (*z == ' ') /* 22.6.1998 */ /* No check if empty control char at first line */
    {
        ...
        if (hae_apu("specmax", x)) {
            specmax = atoi(x);
        } else {
            specmax = 10000;
        }
        if (hae_apu("speclist", x)) {
            speclist = atoi(x);
        } else {
            speclist = 1000000;
        }
    ...
    for (j = 1; j <= r2; ++j) /* Loop through the edit field lines          */
    {
        ...
        *x = EOS;
        ...
        while (1) {
            p = strchr(p, '=');
            if (p == NULL) {
                break; /* Next line if no more  "="-marks            */
            }
            if (*(p + 1) == EOS) {
                break; /* Next line if at the end of line            */
            }
            if (*(p + 1) == '=') {
            ...
            if (j >= own_spec_line1 && j <= own_spec_line2) {
                ++i; // RS ADD
            }
            ...
            while (*(q - 1) == '&') /* Expression splitted to several lines        */
            {
                ...
                ++j;
                if (j > r2) {
                    break; /* Go to next line and check if last           */
                }
                edread(x, (unsigned int)j);
                q = x + 1;
                while (*q && *q == ' ') {
                    ++q; /* Skip spaces in the beginning of line        */
                }
                if (*q == EOS) {
                    break; /* Next line if at the end of line             */
                }
            ...
            if (varjo) {
                tila += ((tila - tila0) * i); /* Double space needed if shadows in use       */
            }
            ++p;
        }
    }
...
int jatkorivit(int j) {
    ...
    while (1) {
        ...
        *(spl - 2) = EOS;
        --spl;
        if (spl - splist + strlen(p) + 2 > speclist) {
            return (-1);
        }
        strcat(spl - 1, p);
        spl += strlen(p);
...
int spread3(char *x, int j) {
    ...
    while (pos < ed1) {
        ...
        if (p == NULL) {
            break; /* Ready if no more found                       */
        }
        if (*(p + 1) == EOS) {
            break; /* Ready if at the end of line                  */
        }
        if (*(p + 1) == '=') {
            pos += 2; /* Jump over "="                                */
            continue;
        ...
        if (spn >= specmax) {
            return (-spn); /* Return if too many specifications           */
        }
        ...
        if (spl - splist + pos - i + 1 > speclist) {
            return (-spn);
        }
        ...
        *(spl - 1) = EOS; /* Update pointers, spa=left side              */
        ...
        if (spl - splist + i - pos + 1 > speclist) {
            return (-spn);
        }
...
int spread2(int lin, int *raja1) {
    ...
    *raja1 = j;
    for (j = *raja1 + 1; j <= ed2; ++j) /* Read the specifications */
    {
        ...
        if (j == r1 + r - 1) {
            continue; /* Skip the activated line */
        }
        spn = spread3(x, j);
        if (spn < 0) {
            return (spn);
...
int sp_init_extra(int lin, int extra_bytes, int extra_specs) {
    ...
    if ((p = strstr(x, "SPECS=")) != NULL) // 20.12.2010
    {
        ...
        if (i == 1) {
            if (k < lin) {
                own_spec_line1 = k;
                own_spec_line2 = lin - 1;
            } else if (k > lin) {
                own_spec_line1 = lin + 1;
                own_spec_line2 = k;
            }
        } else if (i == 2) {
            own_spec_line1 = k;
    ...
    while (pos < ed1) {
        ...
        *spa = EOS;
        ...
        if (strcmp(spa, s) == 0) {
            ...
            *t = EOS;
            strncat(t, x + pos + 1, len1);
            if (*(t + strlen(t) - 1) == '&') {
                spec_jatkorivit(t, j + 1, len);
            }
            return (1);
        }
        ++pos;
...
static int find_global() {
    ...
    pch = z + r2 * (c2 + 1) - 1;
    ch = *pch;
    *pch = EOS; /* last element in edit field temporarily EOS */
    p = strstr(z, "*GLOBAL*");
    if (p == NULL) {
        *pch = ch;
        return (-1);
...
int spec_find(char *s, /* spec to be found */
...
int seek_word0(int mode) {
    ...
    if (p - x > c2) {
        return (1); // RS 20.4.2013 moved here from below next line
    }
    if (mode == 1) {
        if (p - x <= c1 + c3 - 1) {
            c = p - x - c1 + 1;
...
int miau_koodit() {
    ...
    m = nextch_editor();
    if (m == '!') {
        strcpy(tut_info2, tut_info);
        return (1);
    }
    if (m == '&') {
        if (strlen(tut_info) + strlen(tut_info2) >= LLENGTH) {
            return (1);
        }
        strcat(tut_info, tut_info2);
        return (1);
    }
    if (m == '~') /* 16.5.92 */
    {
...
static int etsi(char *haku, unsigned int paikka) {
    ...
    while (1) {
        if (reverse_search) {
            ...
            if (caps_on) {
                while (upf(z[u]) != *haku && u < ed1 * ed2) {
                    ++u;
                }
            } else {
                while (z[u] != *haku && u < ed1 * ed2) {
                    ++u;
                }
            }
            if (u == ed1 * ed2) {
        ...
        *vert_sana = EOS;
        strncat(vert_sana, z + u, strlen(haku));
        // Rprintf("\nhaku=%s vert=%s|",haku,struprf(vert_sana)); getck();
        if ((!caps_on && strcmp(haku, vert_sana) == 0) || (caps_on && strcmp(haku, struprf(vert_sana)) == 0)) {
            if (u % ed1 != 0) {
                if (!sh) {
                    return (u);
                }
                ...
                if (zs[j] > 0) {
                    edread(xs, zs[j]);
                    *sh_kohde = EOS;
                    strncat(sh_kohde, xs + i, strlen(haku));
                    if (strncmp(sh_kohde, space, strlen(haku)) == 0) {
                        *sh_kohde = EOS;
                    }
                    if (strcmp(sh_haku, sh_kohde) == 0) {
                        return (u);
                    }
                } else /* zs[j]=0 */
                {
...
static int op_find() {
    ...
    edread(x, r1 + r - 1);
    if (*x == '#') {
        caps_on = 1; // myˆs # kontr.sar. -> case insensitive
    }
    ...
    if (*info == 'F') {
        ...
        *haku = EOS;
        ...
        if (i > 1 && haku[i - 1] == '~') // 12.6.2004 23.6.2004
        {
            muste_fixme("\nFIXME: ~ FIND not implemented!"); // RS FIXME
                                                             /* RS FIXME
                                                                             fr=r; fr1=r1; fc=c; fc1=c1; muste_sprintf(rivi," %d",r1+r);
                                                                             ...
                                                                             while (1)
                                                                                 ...
                                                                                 cursor(r3+1,0);
                                                                                 PR_EBLD;
                                                                muste_sprintf(sbuf," d=%d   Next case by N!   Interrupt by ENTER!",i);
                                                                                 sur_print(sbuf);
        ...
        */
        ...
        if (i > 0) {
            edread(xs, i);
            *sh_haku = EOS;
            strncat(sh_haku, xs + (haku - pline), strlen(haku));
            if (strncmp(sh_haku, space, strlen(haku)) != 0) {
                sh = 1;
            }
    ...
    else /* REPLACE */
    {
        ...
        korvaus = parm[2];
        strcpy(sbuf, haku);
        chrconv(sbuf, haku); // RS 22.3.2013
        strcpy(sbuf, korvaus);
        chrconv(sbuf, korvaus); // RS 22.3.2013
        if (caps_on) {
            struprf(haku); // 20.4.2002
        }
        lain = 0;
        if (*haku == '"') {
            pline = y;
        ...
        if (i > 0 && !muste_noshadow) {
            ...
            *sh_haku = EOS;
            strncat(sh_haku, xs + (haku - pline), strlen(haku));
            *sh_korvaus = EOS;
            strncat(sh_korvaus, xs + (korvaus - pline), strlen(korvaus));
            if (strncmp(sh_haku, space, strlen(haku)) == 0) {
                *sh_haku = EOS;
            }
            if (strncmp(sh_korvaus, space, strlen(korvaus)) == 0) {
                *sh_korvaus = EOS;
            }
            if (*sh_haku || *sh_korvaus) {
                sh = 1;
            }
    ...
    while (kesken) {
        ...
        if (jatkuva && jatkuva2 == 1 && korvauksia > 100) // RS 6.9.2013
        {
            cursor(r3 + 1, 0);
            PR_EBLD;
            muste_sprintf(sbuf, "n=%u replaces. Interrupt by ENTER", n);
            sur_print(sbuf);
            cursor(r, c);
            if (sur_kbhit()) {
                i = sur_getch();
        ...
        if (nhaku) {
            ...
            cursor(r3 + 1, 0);
            PR_EBLD;
            muste_sprintf(sbuf, "Search for word: %s", haku);
            sur_print(sbuf);
            cursor(r, c);
            SAVE_CURSOR;

            if (etu && etuu == 1) {
                m = read_nextkey_editor(); // 22.9.2009 RS 22.11.2012 read_
            } else {                       // 22.9.2009
                m = nextch_editor();
            }
            RESTORE_CURSOR;
            ...
            if (m == CODE_EXEC || m == CODE_DELETE) {
                if (first_word_on_line_search) {
                    paikka += ed1; // 25.9.2009
                } else {
                    ++n_act;
                    if (reverse_search) {
                        --paikka;
            ...
            } else if (m == CODE_DISP) /* F5 */
            {
                ...
                *ch = (char)m;
                ...
                strcat(haku, ch);
                if (caps_on) {
                    muste_strupr(haku); // koe 15.1.2009
                }
                disp_nappihaku(haku);
            }
            /*        LOCATE(rr,cc);    */
        ...
        while (!jatkuva) {
            ...
            if (replace) {
                muste_sprintf(sbuf, "n=%u: Replace=R Continuous=C Next=N Interrupt=ENTER", n);
                sur_print(sbuf);
            } else {
                muste_sprintf(sbuf, "n=%u: Next case=N Continuous search=C Interrupt by ENTER", n);
                sur_print(sbuf);
            }
    ...
    if (!jatkuva2) {
        ...
        PR_EBLK;
        if (n > 0) {
            muste_sprintf(sbuf, "%u cases of %.32s found!   Press ENTER!", n, haku);
            sur_print(sbuf);
        } else {
            if (n_act == 0) {
                muste_sprintf(sbuf, "%.32s not found!   Press ENTER!", haku);
                sur_print(sbuf);
                m = 0;
            } else {
                muste_sprintf(sbuf, "%u cases of %.32s found!   Press ENTER!", n_act, haku);
                sur_print(sbuf);
            }
        }
    }
    ...
    } else if (m != CODE_RETURN && jatkuva2 == 0) {
        while (1) {
            m = nextch_editor();
            if (m == CODE_RETURN) {
                break;
...
static int delete_fenced_output(int j1, int j2) // 20.4.2010
{
    ...
    while (1) {
        ...
        i2 = i1;
        while (i2 <= j2) {
            ch = (char)z[(i2 - 1) * ed1 + 1];
            // Rprintf("\ni2: %d, ch: %c",i2,ch);
            //        *sbuf=ch; // RS 17.2.2014 dummy use of ch here FIXME
            if (ch != '#') {
                ++i2;
                continue;
            }
...
// cd = control char (tyyli=2)
{
    ...
    muste_sprintf(sbuf, "%sSURVO.TMP", etmpd);
    ...
    for (j = k1; j <= k2; ++j) {
        edread(x, j);
        if (tyyli == 1) {
            if ((p = strstr(x + 1, t)) == NULL) {
                continue;
            }
            if (shad) {
                if (zs[j] > 0) {
                    edread(xs, zs[j]);
                    if (strncmp(ts, xs + (p - x), len) != 0) {
                        continue;
                    }
                }
            }
...
static int op_lineins() {
    ...
    k1 = edline2(parm[1], 1, 1);
    if (!k1) {
        return (1);
    }
    k2 = edline2(parm[2], k1, 1);
    if (!k2) {
        return (1);
    }
    ...
    case 0:
        ...
        break;
    case 1:
        ins_by_text(tyyli, j, nj, txt + 1, stxt + 1, k, 'X');
        break;
    case 2:
        ins_by_text(tyyli, j, nj, txt + 1, stxt + 1, k, *parm[4]);
        break;
    }
...
static int op_session() {
    if (g == 1) {
        edwrite(space, r1 + r - 1, 1);
        muste_sprintf(sbuf, "SESSION %s", sur_session);
        edwrite(sbuf, r1 + r - 1, 1);
        return (1);
    }
    sur_session[0] = *parm[1];
    sur_session[1] = EOS;
    return (1);
}
...
int muste_editor_eventhandler() {
...

    {
        ...
        if (m == -5 || m == -1) {
            return (edrun); // RS REM continue; /* 13.4.1996 */
        }
        ...
        scroll_line = r + 2;
        if (scroll_line > r3) {
            scroll_line = r3; // RS Needs to be known!
        }
