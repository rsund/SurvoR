...
static int uusi_aloitus(void);
static int kokoa_tiedot(void);
static int tutki_sana(void);
static int tutki_sana2(void);
static int lopeta_lause(void);
static int tutki_lyhenteet(void);
static int tutki_kieli(void);
static int iso_kirjain(char ch);
static int wiio(void);
static int tulostus(void);
static int tulosta_rivi(char *rivi);
static int aloita_aika(void);
static int nollaa_laskurit(void);
static int mittaa_aika(char *sana, char mv, int lause_loppu);
static int mittaa_lukuaika(void);
static int init_regressio(void);
static int regressio(void);
static int lue_lukija(char *nimi, char *tiedot);
static int talleta_lukija(void);
static int talleta_lukijarivi(void);
static int aseta_vokaalit(void);
...
void muste_lue(char *argv) {
    ...
    strcpy(luki_tiedosto, survo_path);
    strcat(luki_tiedosto, "SYS/LUKIJAT.TXT"); // RS \\ -> /
    ...
    *lukijan2_nimi = EOS;
    ...
    if (i < 0) {
        ...
        mittaus = 1;
        strcpy(lukijan2_nimi, spb[i]);
    }

    sur_print("\n");
    PR_EUDL;
    sarake = 0;
    if (scroll_line > r3 - 15) {
        scroll_line = 5;
    }

    for (i = 0; i < r3 - scroll_line + 3; ++i) {
        sur_print("                                                                               \n");
    }
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (*p == '-') {
                ...
                if (*(p - 1) == ' ' || *(p - 1) == EOS) { /* "rivin" edess\204 aina EOS */
                    sana[ns++] = *p;
                    sana[ns] = EOS;
                    continue;
                }
...
static int tutki_sana2() {
    ...
    for (i = 0; i < ns; ++i) {
        ++nmerkki[(int)(unsigned char)sana[i]];
    }
    ...
    if (mv) {
        muste_sprintf(sbuf, " %s%c", sana, mv);
        sur_print(sbuf);
        sarake += ns + 2;
    } else {
...
static char *lyhenne[] = {"mm",   "muun-muassa", "esim", "esimerkiksi",  "kpl", "kappaletta",
                          "n",    "noin",        "ns",   "niin-sanottu", "jne", "ja-niin-edesp\204in",
                          "v",    "vuonna",      "yht",  "yhteens\204",  "ko",  "kysess\204 oleva",
                          "jatk", "jatkuu"};
...
static int tutki_lyhenteet() /* ja luvut */
{
    ...
    n_lyhenne = 10;
    if (strchr("+-*/.0123456789", *sana) != NULL) {
        return (2); /* luku */
    }
    sana[ns] = EOS;
    if (strchr(sana, '.') != NULL) {
...
static int tulostus() {
    ...
    if (luettavuus == -9999) {
        ...
        if ((double)nruotsi > 10.0 + (double)nsanat / 50.0) {
            ...
            if (luettavuus > 100) {
                strcat(y, " (Teksti eritt\204in vaikeata)");
            } else if (luettavuus >= 68) {
                strcat(y, " (Hyvin vaikeata = tiedelehtien ylin nelj\204nnes)");
            } else if (luettavuus >= 60) {
                strcat(y, " (Teksti muistuttaa tiedelehti\204)");
            } else if (luettavuus >= 44) {
                strcat(y, " (Teksti sanomalehtikielen rajoissa)");
            } else if (luettavuus >= 35) {
                strcat(y, " (Teksti muistuttaa lastenlehti\204)");
            } else {
                strcat(y, " (Eritt\204in helppoa = lastenlehtien alin nelj\204nnes)");
            }
        }
    }
    ...
    if (edrivi) {
        ...
        muste_sprintf(x2, "tekstitiedosto %s", nimi);
    }
    ...
    muste_sprintf(x, " Tekstiss\204 on %d virkett\204, %d sanaa ja %d kirjainta.", nlauseet, nsanat, nkirjaimet);
    tulosta_rivi(x);
    ...
    if (nlauseet) {
        ...
        if (*lukijan_nimi) {
            muste_sprintf(x, " lukijan ollessa %s parametrein %.2f,%.2f,%.2f .", lukijan_nimi, pisteaika, sanaaika,
                          tavkerroin);
        } else {
            muste_sprintf(x, " nopeusparametreilla %.2f,%.2f,%.2f", pisteaika, sanaaika, tavkerroin);
        }
        tulosta_rivi(x);
    }
        ...
        *mittadata = EOS;
        ...
        if (i >= 0) {
            ...
            if (!muste_is_path(mittadata)) {
                strcpy(mittadata, edisk);
                strcat(mittadata, spb[i]);
            }
            ...
            fprintf(mittatiedot, "Tavukertoimet:               1.1 1.3 1.2 1.3 1.2 1 1.6  1.6  1.5 1.5\n");
            fprintf(mittatiedot, "Aika Lauseet Sanat Tavut Kirj KO KOK OK  KOO OO  O KOOK KOKK OOK OKK\n");
        }
    }
...
static int mittaa_aika(char *sana, char mv, int lause_loppu) {
    ...
    while (*p) {
        ...
        for (i = 0; i < ntavutyypit; ++i) {
            if (strcmp(p, tavutyyppi[i]) == 0) {
                break;
            }
        }
        if (i < ntavutyypit) {
            aika += tavkerroin * tavuaika[i];
            ++nttyyppi[i];
        } else {
            aika += MUU_TAVUAIKA * tavkerroin;
            ++nmuutavu;
        }
        p = q + 1;
    }
...
static int mittaa_lukuaika() {
    ...
    muste_sprintf(sbuf, "    Odotettu lukemisaika on %.2f sek.", odotettu_aika);
    sur_print(sbuf);
    i = nextch("");
    PR_EUDL;
    if (i == CODE_EXEC) {
    ...
    sur_print("\n");
    loppu = clock();
    lukuaika = (double)(loppu - alku) / CLOCKS_PER_SEC;
    ++kokeita; // RS CHA CLK_TCK -> CLOCKS_PER_SEC
    muste_sprintf(sbuf, "  Todellinen lukemisaika on %.2f sek.  Erotus=%.2f   N=%d", lukuaika, odotettu_aika - lukuaika,
                  kokeita);
    sur_print(sbuf);
    regressio();
    PR_EINV;
...
static int regressio() {
    ...
    muste_sprintf(sbuf, "\n  Lukuparametrit: Lausev\204lit=%.2f  Sanav\204lit=%.2f Tavut=%.2f", A[0], A[1], A[2]);
    sur_print(sbuf);
    pisteaika = A[0];
    pilkkuaika = PILKKUKERROIN * pisteaika;
    sanaaika = A[1];
...
static int lue_lukija(char *nimi, char *tiedot) {
    ...
    while (1) {
        ...
        if (!oletuslukija && strcmp(osa[0], nimi) != 0) {
            continue;
        }
...
static int talleta_lukija() {
    ...
    ei_talletettu = 1;
    lukijat = fopen(luki_tiedosto, "rt");
    strcpy(luki2, survo_path);
    strcat(luki2, "SYS/SURVO.TXT"); // RS CHA \\ -> /
    ...
    strcat(sbuf, "SYS/LUKIJAT.TXT"); // RS \\ -> /
...
static int tavuta(char *sana, char *tavut, char *tavut2) {
    ...
    *tavut2 = EOS;
    ...
    if (tavuja < 2 && strlen(sana) > 5) {
        return (-1);
    }
    if ((double)strlen(sana) / (double)tavuja > 5) {
        return (-1);
    }
    return (1);
}
...
static int tavuta2(char *sana, char *tavut, char *tavut2) {
    ...
    len = strlen(sana);
    for (i = 0; i < len; ++i) {
        if (vokaali[(int)(unsigned char)sana[i]]) {
            x[i] = 'O';
        } else {
            x[i] = 'K';
        }
...
static char *numero[] = {"nolla ", "yksi ",  "kaksi ",        "kolme ",     "nelj\204 ",
                         "viisi ", "kuusi ", "seitsem\204n ", "kahdeksan ", "yhdeks\204n "};
