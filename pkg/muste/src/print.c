...
static int testaa_sivu(void);
static int uusi_sivu(void);
static void testaa_plus(void);
static int uusi_rivi(void);
static int dev_file(char *x);
static int load_dev(void);
static int pr_empty(char *s);
static void vdc(int arvo, char *luku);
static void binopen(void);
static int alkukoodit(void);
...
static int header_print(void);
static void set_page_number(char *x);
static int loppusulku(int j, int j2);
static int tyhjat_rivit(int j);
static int tekstityyppi(void);
static int p_special(char *s);
static int pitch_load(void);
static int strpitch(char *s);
static void p_charsize(void);
static int ptulosta(char *x, char *xs);
...
static int ps_ptulosta(char *x, char *xs);
static int ps_print(char *xx, char *xxs, int tosi);
static int ps_tabprint(char *xx, char *xxs);
static void set_tabwidth(void);
...
static void init_gchars(void);
static int gchars(char *x, char **sana, int n, char *rivi);
static void gchar_erotus(char *x);
static void gchar_tulosta(void);
static int ps_autocad(void);
static int ps_file(void);
static int ps_epsfile(void);
...
static int putcenc(unsigned char merkki, FILE *tiedosto) // RS 15.10.2014
{
    ...
    muume[0] = (unsigned char)merkki;
    muume[1] = EOS;
    if (*encoding) {
        muste_iconv((char *)muume, encoding, "CP850");
    }
    i = strlen((char *)muume);
    j = 0;
    if (i > 1) {
        for (j = 0; j < i - 1; j++) {
            putc((int)muume[j], tiedosto);
        }
    }
...
static int initencoding(char *x, char **sana, int n) // RS 15.10.2014
{
    ...
    for (i = 0; i < 256; ++i) {
        code[i] = (unsigned char)i; // Clear CODES
    }
    return 1;
}
...
static int tulosta_rivit(int j1, int j2) {
    ...
    for (j = j1; j <= j2; ++j) {
        ...
        if (*p == 'r' && page_number != ((page_number >> 1) << 1)) {
            erivi = 2;
        }
        /* -8.12.1992 oli == */
        ...
        if (gchar_on) {
            strcpy(tabrivi0, tabrivi);
            gchar_erotus(x);
        }
        if (pr_type == 1) {
            i = read_footnotes(x, xs, j, j2);
            if (i == -1) {
                return (-1); /* 5.11.88 */
            }
            if (i == -2) {
                i = read_footnotes(x, xs, j, j2); /* uusi sivu */
            }
            if (i > 0) {
                j = i; /* ohita alaviitteet */
            }
        }
        if (printing_on) // 21.8.2008
        {
            if (ascii_text_saving && !ind_page_number) {
                ascii_save(x + 1);
            }
...
static int tulosta(char x[], char xs[]) {
    ...
    if (*xs == EOS) {
        for (i = 1; i < len; ++i) {
            putcenc((int)code[(unsigned char)x[i]], kirjoitin);
        }
        uusi_rivi();
        return (1);
    }
    ...
    while ((unsigned char)xs[i]) {
        ...
        *******************************************************/
        ...
        if (p != NULL) {
            ...
            p = yy;
            while (*p) {
                putcenc((unsigned char)(*p), kirjoitin);
                ++p;
            }
        }
        k = 0;
        while ((unsigned char)xs[i] == varjo) {
            putcenc((unsigned char)code[(unsigned char)x[i]], kirjoitin);
...
static int testaa_sivu() {
    if (line_count + line_spacing > page_length) {
        return (uusi_sivu());
    }
    return (1);
}
...
static int lue_koodit(char *x) {
    ...
    if (n) {
        if (printing_on == 0) // 12.1.2011
        {
            ...
            if (muste_strcmpi(sana[0], "[PRINT_ON]") == 0) {
                printing_on = 1;
            }
            return (1);
        }
        ...
        if (muste_strcmpi(sana[0], "DEF_SPACE") == 0) {
            i = def_space(x1, sana, n, x);
            return (i);
        }
        ...
        if (muste_strcmpi(sana[0], "ENCODING") == 0) {
            i = initencoding(x1, sana, n);
            return (i);
        } // RS 15.10.2014
        ...
        if (muste_strcmpi(sana[0], "HEADER_LINES") == 0) {
            i = hlines(x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "CHAPTER") == 0) {
            i = chapter(x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "PICTURE") == 0) {
            i = picture(x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "TEXT") == 0) {
            i = textfile(x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "DOS") == 0) {
            i = dos(x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "CONTROL") == 0) {
            i = controls(x1, sana, n, x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "NULL") == 0) {
            i = null_ch(n, sana[1]);
            return (i);
        }
        if (muste_strcmpi(sana[0], "REPLACE") == 0) {
            i = replace(x);
            return (i);
        }
        /*****************
        ...
        ******************/
        if (muste_strcmpi(sana[0], "BINMODE") == 0) {
            binopen();
            return (1);
        }
        ...
        if (muste_strcmpi(sana[0], "PS_CODE") == 0) {
            i = ps_code(x1, sana, n, x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "INDEX") == 0) {
            i = print_index(x1, sana, n, x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "GCHAR") == 0) {
            i = gchars(x1, sana, n, x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "AUTOCAD") == 0) {
            i = picture(x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "FILE") == 0) {
            i = picture(x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "EPSFILE") == 0) {
            i = picture(x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "ASCII_TEXT") == 0) {
            i = ascii_text(x);
            return (i);
        }
        if (muste_strcmpi(sana[0], "CONTROL_OFF") == 0) {
            if (n < 2) {
                *control_off = EOS;
                ...
                strcpy(control_off, sana[1]);
            }
            return (1);
        }
        if (muste_strcmpi(sana[0], "LIST") == 0) {
            i = pr_list(x);
            return (i);
        }
        // 22.1.2007
        if (muste_strcmpi(sana[0], "SILENT") == 0) {
            silent = 1;
            return (1);
        }
    }
...
static int shadows(char *x, char **sana, int n, char *rivi) /* shadow <koodi> <alkukoodisana> <loppukoodisana> */
{
    ...
    if (n == 2 && muste_strcmpi(sana[1], "list") == 0) {
        strcpy(y, etmpd);
        strcat(y, "SHADOWS.TXT");
        shadows_in_use = muste_fopen(y, "wt");
        if (shadows_in_use == NULL) {
            return (-1);
        }
    ...
    while (*s) {
        if (*s == '[') {
            ...
            *****************************************************/
            ...
            sur_print(sbuf);
            WAIT;
            return (-1); // 6.2.2001 ennen return(-1); RS CHA exit(0)
            s = p + 1;
            continue;
        }
        *y = *s;
...
static int pr_test(int k) {
    if ((int)(pr_osoitin - pr_tila) + k + 2 < kooditila) {
        return (1);
    }
...
*/
{
    ...
    for (p = 0; p < max; p++) {
        sana[p] = muste_nullstring_print; // RS ADD
    }
    ...
    for (p = 0; p < len; ++p) {
        if (rivi[p] == ' ') {
            ...
            /* 16.10.1996 */ if (defspace && g > 0 && rivi[p] == defspace) {
                rivi[p] = ' ';
            }
            if (edell == 0) {
                sana[g] = rivi + p;
...
static int hlines(char *x) /* header_lines */
{
    ...
    i = split(x, sana, 5);
    hline1 = edline2(sana[1], 1, 1);
    if (hline1 == 0) {
        return (-1);
    }
    hline2 = edline2(sana[2], hline1, 1);
    if (hline2 == 0) {
        return (-1);
    }
    hline3 = 0;
    if (i == 5) {
        hline3 = edline2(sana[3], 1, 1);
        if (hline3 == 0) {
            return (-1);
        }
        hline4 = edline2(sana[4], hline3, 1);
        if (hline4 == 0) {
            return (-1);
        }
    }
    ...
    if (line_count <= 0.0) {
        header_print(); /* ensimm?ist? sivua varten */
    }
    return (1);
}
...
static int tyhjat_rivit(int j) {
    ...
    dn = atof(sana[0]);
    if (dn <= 0) {
        muste_sprintf(sbuf, "\nIncorrect & or %% line %d for empty lines", j);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int p_special(char *s) /* tulkkaa laitetiedoston %-sanat */
{
    ...
    *p = EOS;
    ...
    if (strcmp(x, "pitch_ind") == 0) {
        ...
        if (pitch_unit) {
            pitch_unit = pitch[pitch_ind]; /* 28.11.88 */
        }
        return (1);
    }
    ...
    if (strcmp(x, "line_extra") == 0) {
        if (!count_off && prind) {
            line_count += atof(p) * line_spacing;
        }
        return (1);
    }
    ...
    if (strcmp(x, "line_continues") == 0) {
        line_continues = atoi(p);
        return (1);
    }
    if (strcmp(x, "footnote_number") == 0) {
        set_footnote_number(p);
        return (1);
    }
    ...
    if (strcmp(x, "pstatus") == 0) {
        ...
        if (i == 1) {
            ...
            uusi_vstrike(parm[10]);
            uusi_hstrike(parm[11]);
...
static int pitch_load() {
    ...
    strcpy(x, survo_path);
    strcat(x, "SYS/PITCH.BIN"); /* 30.12.91 */
...
static int strpitch(char *s) {
    ...
    len = 0;
    while (*s) {
        len += pitch[(unsigned char)(*s + pitch_ind)];
        ++s;
    }
...
static int ptulosta(char *x, char *xs) {
    ...
    */
    ...
    if (*xs == EOS) {
        for (i = 1; i < len; ++i) {
            if (gap[i]) {
                p_gap(code[(unsigned char)x[i]], gap[i]);
            } else {
                putcenc((unsigned char)code[(unsigned char)x[i]], kirjoitin);
            }
        }
...
static void p_gap(char merkki, int gap) {
    ...
    *luku = (char)'\233';
    ...
    i = 0;
    while (koodi[i]) {
        putc((unsigned int)koodi[i], kirjoitin);
        ++i;
    }
}
...
static int chapter(char *x) {
    ...
    if (i == 2) {
        i = 4;
        sana[3] = current_field;
    }
    if (i < 4) {
        muste_sprintf(sbuf, "\nError in %s", x);
        sur_print(sbuf);
        WAIT;
        return (-1); // RS 19.7.2016 y -> x
    }
    strcpy(kpl, sana[1]);
    i = edt_avaus(sana[3]);
    if (i < 0) {
    ...
    if (strcmp(kpl, "*") == 0) {
        ...
        j2 = uedline(def[k - 1], j1);
        if (j2 == 0) {
            uvirhe(def[k - 1], sana[3]);
            return (-1);
        }
    }
...
static int edt_avaus(char *edfile) {
    int i;
    char nimi[LNAME];
    char x[LLENGTH]; // RS REM , y[LLENGTH];
    char *sana[5];
    int j, k, h;
    char rivi[ELE];
    int g;
...
static int edt32to16(char *name32, char *name16) {
    ...
    if (edt32_file == NULL) {
        error_file_32_16(name32);
        return (-1);
    }
    edt16_file = muste_fopen(name16, "wb");
    if (edt16_file == NULL) {
        error_file_32_16(name16);
        return (-1);
    }
    ...
    while (!feof(edt32_file)) {
        ...
        if (luettu) {
            if (luettu == -1) /* Shadow line */
            {
                ...
                p = (char *)&js;
                *sbuf = *p;
                *(sbuf + 1) = *(p + 1); // RS 4.2.2013
                for (i = 0; i < nc; ++i) {
                    putc((int)sbuf[i], shadow_file);
                }
                luettu = 0;
                continue;
            }
...
static int lst_file_find(char *lista) {
    ...
    *nimi = EOS;
    if (strchr(lista, ':') != NULL || strchr(lista, '\\') != NULL ||
        strchr(lista, '/') != NULL) // RS ADD / unix path FIXME
    {
        strcpy(nimi, lista);
                            ...
                            /*                                *=automatic      */
{
    ...
    if (*sana[3] != '*') {
        ...
        y_home = y_origin - line_count / 720.0 * 254.0;
    }
...
static int merkki() { return (getc(canon_file)); }

static void pict_file_error(char *s) {
    muste_sprintf(sbuf, "\nCannot find  %s  in picture file!", s);
    sur_print(sbuf);
    WAIT;
}
...
/* int gap[];  venytysarvot positioittain */
{
    ...
    while ((p = strchr(tabrivi + sar + 1, 'T')) != NULL && sar + 1 < len) {
        sar2 = p - tabrivi;
        muste_fieldcopy(jakso, x + sar + 1, sar2 - sar);
        jakso[sar2 - sar] = EOS;
...
static int ps_tulosta(char *x, char *xs) {
    ...
    if (*xs == EOS) {
        ps_kirjoita("(");
        for (i = 1; i < len; ++i) {
            x[i] = code[(unsigned char)x[i]];
        }
        x[len] = EOS;
        len = ps_replace(x + 1);
        for (i = 1; i <= len; ++i) {
            putcenc((unsigned char)x[i], kirjoitin);
        }
        ps_kirjoita(") s_show\n");
        uusi_rivi();
        return (1);
    ...
    while ((unsigned char)xs[i]) {
        ...
        **********************************************/
        ...
        h = ps_replace(y);
        for (j = 0; j < h; ++j) {
            putcenc((unsigned char)y[j], kirjoitin);
        }
        ps_kirjoita(") prnshow\n");
...
static int ps_kirjoita(char *s) {
    ...
    for (i = 0; i < strlen(s); ++i) {
        putcenc((unsigned char)s[i], kirjoitin);
    }
    return (1);
}

static int ps_ptulosta(char *x, char *xs) {
    int i, len, slen; // RS REM ,h,j,k;
                      // RS REM        unsigned char varjo;
    char *p;
    // RS REM        char y[LLENGTH], yy[LLENGTH];
    ...
    x[len] = EOS;
    xs[len] = EOS;
    if (strchr(".:", x[len - 1]) != NULL) {
        end_tol = end_spec_tol;
    } else {
        end_tol = end_norm_tol;
    }
    if (pakkovenytys) {
        end_tol = 2000.0;
    }
...
static int ps_tabprint(char *xx, char *xxs) {
    ...
    while (i1 < len) {
        ...
        i2 = i1 + 1;
        while (i2 < len) {
            if (tabrivi[i2] == 'T' || tabrivi[i2] == '(' || tabrivi[i2] == ')') {
                break;
            }
            ++i2;
        }
        ...
        k = i2 - i1 + 1;
        muste_fieldcopy(x, xx + i1, k);
        x[k] = EOS;
        muste_fieldcopy(xs, xxs + i1, k);
        xs[k] = EOS;
...
/* int tosi;  0=testaus 1=painatus */
{
    ...
    char y[LLENGTH], yy[LLENGTH];
    char x[LLENGTH], xs[LLENGTH];
    // RS REM        char s[LLENGTH];
    int splen;
    // RS REM        static int nspace;
...
static int fn_talleta(int j, int j2) {
    ...
    while (j <= j2) {
        if (tila == 0) {
            edread(x, j);
            if (zs[j] == 0) {
                muste_fieldcopy(xs, space, strlen(x));
                xs[strlen(x)] = EOS;
            } else {
                edread(xs, zs[j]);
            }
...
static int fn_tulosta() {
    ...
    vline_spacing = line_spacing;
    line_spacing = note_spacing;
    strcpy(x, "[NOTE_START]");
    i = lue_koodit(x);
    if (i < 0) {
        return (1);
    ...
    while (!feof(fn_file)) {
        ...
        fn_load(xs);
        if (strncmp(xs, space, strlen(x)) == 0) {
            *xs = EOS;
        }
        p = x;
        if (*p == '-') {
...
static void set_footnote_number(char *s) { n_footnote = atoi(s) - 1; }
...
static void pane_hakemistoon(char *x, char *xs) {
    ...
    while (p != NULL && *p != EOS) {
        ...
        ++i1;
        muste_fieldcopy(sana, x + i1, i2 - i1 + 1);
        sana[i2 - i1 + 1] = EOS;
        if (pr_empty(x + i2 + 1) && x[i2] == '-') {
            sana[i2 - i1] = EOS;
            strcpy(rivin_loppu, sana);
            loppu_merkitty = 1;
            break;
        }
        index_save(sana, page_number - 1);
                                                           ...
                                                           /* gchar T<merkki> <koodisana> */
{
    ...
    if (gchar[varjo] != NULL && len_gchar[varjo] >= strlen(sana[2])) {
        ...
        gchar[varjo] = pr_osoitin;
        strcpy(pr_osoitin, sana[2]);
        pr_osoitin += len_gchar[varjo] + 1;
    }
    if (!t_gchar[varjo]) {
        gmerkit[n_gchar] = varjo;
        ++n_gchar;
    }
...
static void gchar_erotus(char *x) {
    ...
    for (i = 0; i < len; ++i) {
        gcharx[i] = ' ';
        if (strchr((char *)gmerkit, x[i]) == NULL) {
            continue;
        }
        gcharx[i] = x[i];
        x[i] = ' ';
        ...
        if (t_gchar[(unsigned char)gcharx[i]] == 'T') {
            tabrivi[i] = 'T';
        }
        ++gchar_ind;
    }
