...
static int tulosta_rivit(int j1,int j2)
        ...
        for (j=j1; j<=j2; ++j)
            ...
            switch (tila)
              ...
              case 2: i=textread(x,j); if (i<0) { return(1); 
}break;
                }
            ...
            if (*p=='T') { i=tabs(j); if (i<0) { return(-1); 
}continue; }
            if (*p=='C') { erivi=1;
}
            if (*p=='R') { erivi=2;
            ...
            if (printing_on) // 21.8.2008
              ...
              if ( pitch_unit && (line_length || *tabrivi) )
                  ...
                  else         { if (gchar_on) { gchar_tulosta(); 
}i=ptulosta(x,xs); }
                  if (i<0) { return(-1);
}
                  }
...
static int shadows(char *x,char **sana,int n,char *rivi)   /* shadow <koodi> <alkukoodisana> <loppukoodisana> */
        ...
        } else { i=muunna(sana[1],y); if (i<0) { return(-1); 
}varjo=*y; }
...
static int p_special(char *s) /* tulkkaa laitetiedoston %-sanat */
        ...
        *p=EOS;
        ...
        if (strcmp(x,"line_length")==0)
            { line_length=atof(p); if (line_length>0.0) { pitch_load(); 
}return(1); }
        ...
        if (strcmp(x,"line_extra")==0)
            { if (!count_off && prind) { line_count+=atof(p)*line_spacing; 
}return(1); }
...
static int strpitch(char *s)
        ...
        len=0; while (*s) { len+=pitch[(unsigned char)(*s+pitch_ind)]; ++s; }
...
// RS REM        int skaala=1; /* tilap. */
   ...
   /*   muste_itoa(skaala*gap,luku,10); strcat(koodi,luku); kun skaala!=1  */
        ...
        *luku=(char)'\233'; strcat(koodi,luku);
        strcat(koodi,"w");

        i=0; while (koodi[i]) { putc((unsigned int)koodi[i],kirjoitin); ++i; }
...
// Rprintf("nr=%d nc=%d nshad=%d\n",nr,nc,nshad); getch();
        ...
        for (i=strlen(sbuf); i<nc; ++i) { sbuf[i]=' '; 
}sbuf[i]=EOS;
...
static int fn_tulosta()
        ...
        while (!feof(fn_file))
            {
            fn_load(x); if (feof(fn_file)) { break; 
}fn_load(xs);
...
static void pane_hakemistoon(char *x,char *xs)
        ...
        while (p!=NULL && *p!=EOS)
            ...
            i2=p-xs;
            while (i2>0) { if (x[i2]!=' ') { break; 
}--i2; }
            if (i2==0) { ++p; continue; }
            while (x[i2]!=EOS) { if (x[i2]==' ') { break; 
}++i2; }
            --i2;
            i1=i2;
            while (i1>0) { if (x[i1]==' ') { break; 
}--i1; }
            ++i1;
...
static int gchars(char *x,char **sana,int n,char *rivi)   /* gchar <merkki> <koodisana> */
        ...
        i=strlen(p);
        if (i==1) { varjo=*p;
        } else { i=muunna(p,y); if (i<0) { return(-1); 
}varjo=*y; }
...
static void gchar_erotus(char *x)
        ...
        for (i=0; i<len; ++i)
            {
            gcharx[i]=' ';
            if (strchr((char *)gmerkit,x[i])==NULL) { continue;
}
            gcharx[i]=x[i]; x[i]=' ';
            vpitch_unit=0;
            if (*tabrivi==EOS)
                {
                for (h=0; h<len; ++h) { tabrivi[h]=' '; 
}tabrivi[len]=EOS;
                if (ps_charwidth==0.0) { ps_charwidth=0.6*font_size;
}
                if (pitch_unit==0) { vpitch_unit=1; pitch_unit=1; }
                }
            if (t_gchar[(unsigned char)gcharx[i]]=='T') { tabrivi[i]='T';
}
            ++gchar_ind;
            }
        gcharx[len]=EOS;
