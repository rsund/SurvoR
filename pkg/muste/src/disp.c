...
void muste_flushscreen() {
    muste_sprintf(komento, "update idletasks");
    Muste_EvalTcl(komento, FALSE);
}

void muste_fixme(char *kommentti) { Rprintf("%s", kommentti); }
...
int sur_cursor_position(int *prow, int *pcol) {
    ...
    muste_sprintf(komento, ".muste.getcursor()");
    muste_evalr(komento);
...
int sur_set_cursor(int dwSize, int bVisible) {
    ...
    if (i) {
        i = splitq(plotkomento, ss, 2);
    }
    if (i < 2) {
        ss[0] = muste_default_cursor_color;
        ss[1] = muste_default_insert_color;
    }
    ...
    if (dwSize > 100) {
        dwSize -= 100;
        muste_sprintf(komento, "configure -insertwidth %d -insertbackground \"%s\"", dwSize, ss[1]);
        Muste_EvalTcl(komento, TRUE);
    } else {
        muste_sprintf(komento, "configure -insertwidth %d -insertbackground \"%s\"", dwSize, ss[0]);
        Muste_EvalTcl(komento, TRUE);
    }
...
int sur_screen_dim(int *sizex, int *sizey) {
    ...
    muste_sprintf(komento, ".muste.getscreendim()");
    muste_evalr(komento);
...
void sur_get_window_rect(char *wname, int par[]) {
    ...
    muste_sprintf(komento, ".muste.getwindowdim()");
    muste_evalr(komento);
...
void muste_init_plotwindows() {
    ...
    muste_sprintf(komento, ".muste$plotwin <- list()");
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$plotwin[[%d]] <- 0.0", MAXPLOTWINDOWS);
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$plotwinsize <- list()");
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$plotwinsize[[%d]] <- 0.0", MAXPLOTWINDOWS);
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$canvas <- list()");
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$canvas[[%d]] <- 0.0", MAXPLOTWINDOWS);
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$canvasfonts <- list()");
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$canvasfonts[[%d]] <- 0.0", MAXPLOTWINDOWS);
    muste_evalr(komento);

    for (i = 0; i < MAXPLOTWINDOWS; i++) {
        muste_canvasfonts[i] = 0;
    }
}

void sur_pos_window(char *wname, int x, int y) {
    muste_sprintf(komento, "tcl(\"wm\",\"geometry\",%s,\"+%d+%d\")", wname, x, y);
    muste_evalr(komento);
}
...
int muste_focus_from_plotwin_to_editor(int id) {
    ...
    muste_sprintf(komento, ".muste.focus.editor()");
    muste_evalr(komento);

    /*
        muste_sprintf(komento,"tkfocus(.muste$txt)");
        muste_evalr(komento);

    // RS Needed to get focus back to Muste editor in Mac
        sur_get_window_rect(muste_window_name,opt);
        muste_sprintf(komento,"tkwm.withdraw(.muste$ikkuna)");
        muste_evalr(komento);
        muste_sprintf(komento,"tkwm.deiconify(.muste$ikkuna)");
        muste_evalr(komento);
    ...
    */
    ...
    /*	RS This causes problems in WIN7
        ...
        muste_sprintf(komento,"tklower(.muste$plotwin[[%d]],.muste$ikkuna)",id);
        muste_evalr(komento);
    */
...
int muste_plottcl(int id, char *komento, int win) {
    ...
    if (id != muste_old_plotid) {
        muste_sprintf(plotkomento, ".muste$plotwinid<-.Tk.ID(.muste$plotwin[[%d]])", id);
        muste_evalr(plotkomento);

        muste_sprintf(plotkomento, ".muste$canvasid<-.Tk.ID(.muste$canvas[[%d]])", id);
        muste_evalr(plotkomento);
    ...
    if (win) {
        muste_sprintf(plotkomento, "%s %s", muste_plotwindow, komento);
    } else {
        muste_sprintf(plotkomento, "%s %s", muste_plotcanvas, komento);
    }
    // Rprintf("\n%s",plotkomento);
...
int muste_window_style(int id, int style) {
    ...
    muste_sprintf(komento, "tkwm.withdraw(.muste$plotwin[[%d]])", id);
    muste_evalr(komento);

    muste_sprintf(komento, "tkwm.deiconify(.muste$plotwin[[%d]])", id);
    muste_evalr(komento);
...
int muste_canvas_background(int id, char *color) {
    ...
    muste_sprintf(komento, "tkconfigure(.muste$canvas[[%d]],background=\"%s\")", id, color);
    muste_evalr(komento);
...
int muste_line_plot(int id, double x1, double y1, double x2, double y2) {
    ...
    //    muste_sprintf(komento,"tkcreate(.muste$canvas[[%d]],\"line\",%g,%g,%g,%g)",id,x1,y1,x2,y2);
    //    muste_evalr(komento);
    ...
    muste_sprintf(komento, "create line %g %g %g %g -tags lw%d -fill %s -width %g", x1, y1, x2, y2, line_width,
                  muste_pencolor, leveys);
    switch (line_type) {
    case 0:
        strcat(komento, " -capstyle round");
        break;
    case 1:
        strcat(komento, " -dash --");
        break;
    ...
    case 7:
        ...
        break;
    case 8:
        strcat(komento, " -capstyle projecting");
        break;
    }
...
int muste_curve_plot(int id, double x1, double y1, double x2, double y2, double cx1, double cy1, double cx2,
                     double cy2) {
    // RS 27.12.2012
    ...
    muste_sprintf(komento, "create line %g %g %g %g %g %g %g %g -smooth raw -tags lw%d -fill %s -width %g", x1, y1, cx1,
                  cy1, cx2, cy2, x2, y2, line_width, muste_pencolor, leveys);
    switch (line_type) {
    case 0:
        strcat(komento, " -capstyle round");
        break;
...
int muste_rectangle_plot(int id, double x1, double y1, double x2, double y2) {
    ...
    //    muste_sprintf(komento,"tkcreate(.muste$canvas[[%d]],\"rectangle\",%g,%g,%g,%g)",id,x1,y1,x2,y2);
    //    muste_evalr(komento);

    muste_sprintf(komento, "create rectangle %g %g %g %g -fill %s -outline %s", x1, y1, x2, y2, muste_pencolor2,
                  muste_pencolor);
    muste_plottcl(id, komento, FALSE);
...
int muste_polygon_plot(int id, char *chain) {
    ...
    *muste_polychain = EOS;
    for (k = 0; k < n; k++) {
        muste_sprintf(buffer, "%g %g ", pol_point_x[k], pol_point_y[k]);
        strcat(muste_polychain, buffer);
    }

    muste_sprintf(komento, "create polygon %s -fill %s -outline %s", muste_polychain, muste_pencolor2,
                  muste_linecolor); // RS 15.1.2013 pencolor->linecolor
    muste_plottcl(id, komento, FALSE);
...
int muste_ellipse_plot(int id, double x1, double y1, double x2, double y2) {
    ...
    //    muste_sprintf(komento,"tkcreate(.muste$canvas[[%d]],\"oval\",%g,%g,%g,%g)",id,x1,y1,x2,y2);
    //    muste_evalr(komento);

    muste_sprintf(komento, "create oval %g %g %g %g -fill %s -outline %s", x1, y1, x2, y2, muste_pencolor2,
                  muste_pencolor);
    muste_plottcl(id, komento, FALSE);
...
int muste_arc_plot(int id, double x1, double y1, double x2, double y2, double a1, double a2) {
    ...
    muste_sprintf(komento, "create arc %g %g %g %g -extent %g -start %g -fill %s", x1, y1, x2, y2, a2, a1,
                  muste_pencolor);
    muste_plottcl(id, komento, FALSE);
...
int muste_text_plot(int id, double x1, double y1, char *x) {
    ...
    //	muste_sprintf(komento,"tcl("create text %g %g -text \"%s\" -anchor \"nw\" -fill %s",x1,y1,y,muste_charcolor);
    muste_sprintf(komento,
                  "tkcreate(.muste$canvas[[%d]],\"text\",%g,%g,text=\"%s\",anchor=\"nw\",fill=\"%s\",font=.muste$"
                  "canvasfonts[[%d]][[%d]][[1]])",
                  id, x1, y1, y, muste_charcolor, id, muste_canvasfonts[id]);
    muste_evalr(komento);

    //    muste_sprintf(komento,"create text %g %g -text \"%s\" -anchor \"nw\" -fill %s",x1,y1,y,muste_charcolor);
    //    muste_plottcl(id, komento, FALSE);
...
int muste_create_plotwindow(int id, char *title) {
    ...
    muste_sprintf(komento, "if (is.tkwin(.muste$plotwin[[%d]])) tkdestroy(.muste$plotwin[[%d]])", id, id);
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$plotwin[[%d]] <- tktoplevel(.muste$ikkuna)", id);
    muste_evalr(komento);

    muste_sprintf(komento, "tkwm.geometry(.muste$plotwin[[%d]],\"+%d+%d\")", id, x_whome, y_whome);
    muste_evalr(komento);

    muste_flushscreen();

    muste_sprintf(komento,
                  ".muste$canvas[[%d]] <- tkcanvas(.muste$plotwin[[%d]],width=%d,height=%d,background=\"white\")", id,
                  id, x_wsize, y_wsize);
    muste_evalr(komento);

    muste_sprintf(komento, "tkwm.title(.muste$plotwin[[%d]], \"%s\")", id, title);
    muste_evalr(komento);

    muste_sprintf(komento, "tkgrid(.muste$canvas[[%d]])", id);
    //    muste_sprintf(komento,"tkpack(.muste$canvas[[%d]],\"-expand\",TRUE,\"-fill\",\"both\")",id);
    muste_evalr(komento);

    muste_flushscreen();

    muste_sprintf(komento, ".muste$plotwinsize[[%d]] <- list()", id);
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$plotwinsize[[%d]][[%d]] <- 0.0", id, 2);
    muste_evalr(komento);

    muste_sprintf(komento, ".muste.canvas.windif(%d)", id);
    muste_evalr(komento);

    muste_sprintf(komento, "tkbind(.muste$plotwin[[%d]],\"<Configure>\",muste:::.muste.canvas.scale)", id);
    // Rprintf("\nkomento: %s",komento);
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$canvasfonts[[%d]] <- list()", id);
    muste_evalr(komento);

    muste_sprintf(komento, ".muste$canvasfonts[[%d]][[%d]] <- 0.0", id, MAXFONTS);
    muste_evalr(komento);

    muste_sprintf(komento,
                  ".muste$canvasfonts[[%d]][[1]] <- "
                  "list(tkfont.create(family=\"Courier\",size=%d,weight=\"bold\",slant=\"roman\"),14)",
                  id, (int)((double)x_wsize / 1000 * 14));
    muste_evalr(komento);
...
void muste_createcanvasfont(int id) {
    ...
    muste_sprintf(
        komento,
        ".muste$canvasfonts[[%d]][[%d]] <- list(tkfont.create(family=\"%s\",size=%d,weight=\"%s\",slant=\"%s\"),%g)",
        id, muste_canvasfonts[id], muste_fontfamily, (int)((double)x_wsize / 1000 * muste_fontsize), muste_fontweight,
        muste_fontslant, muste_fontsize);
    muste_evalr(komento);
}
...
void muste_delete_plotwindow(int id) {
    ...
    muste_sprintf(komento, "if (is.tkwin(.muste$canvas[[%d]])) tkdestroy(.muste$canvas[[%d]])", id, id);
    muste_evalr(komento);
...
void sur_get_font(char *wname, int par[]) {
    ...
    muste_sprintf(komento, ".muste.getfontdim()");
    muste_evalr(komento);
...
void sur_get_textwidth(char *teksti, int par[], int id) {
    ...
    muste_sprintf(komento, ".muste.getfontdim(\"%s\",.muste$canvasfonts[[%d]][[%d]][[1]])", teksti, id,
                  muste_canvasfonts[id]);
    muste_evalr(komento);
...
int sur_set_focus(char *wname) {
    muste_sprintf(komento, "tkfocus(\"-force\",%s)", wname);
    muste_evalr(komento);
    muste_sprintf(komento, "tkfocus(%s)", wname);
    muste_evalr(komento);
    return 1;
}
...
int sur_main_window_show(char *wname, int status) {
    ...
    // RS   if (status==0) muste_sprintf(komento,"tcl(\"wm\",\"iconify\",%s)",wname);
    //   else muste_sprintf(komento,"tcl(\"wm\",\"deiconify\",%s)",wname);
    //   sur_set_focus(wname);

    if (status == 0) {
        //      muste_sprintf(komento,"tcl(\"wm\",\"focusmodel\",%s,\"active\")",wname);
        //      muste_evalr(komento);

        muste_sprintf(komento, "tkwm.iconify(%s)", wname);
        muste_evalr(komento);

        muste_window_minimized = TRUE;
    } else {
        //      muste_sprintf(komento,"tcl(\"wm\",\"focusmodel\",%s,\"passive\")",wname);
        //      muste_evalr(komento);

        //      sur_set_focus(wname);
        //      muste_sprintf(komento,"tkwm.deiconify(%s)",wname);

        if (muste_window_minimized) {

            Rprintf("FIXME: sur_main_show_window KLUDGE (problems in Windows VISTA)\n"); // RS FIXME
            muste_sprintf(komento, ".muste.end()");
            muste_evalr(komento);

            muste_sprintf(komento, ".muste.init()");
            muste_evalr(komento);
...
void muste_resize(int conx, int cony) {
    muste_sprintf(komento, ".muste.resize(%d,%d)", conx, cony);
    muste_evalr(komento);
    muste_flushscreen();
}

void muste_font(int size) {
    muste_sprintf(komento, "tkfont.configure(.muste$font,size=%d)", size);
    muste_evalr(komento);
}

void muste_choosefont() {
    muste_sprintf(komento, ".muste.choosefont()");
    muste_evalr(komento);
}
...
int read_string(char *s, char *s2, int len, int r, int c) /* suoraan n?yt?lt? */
{
    ...
    muste_sprintf(komento, ".muste$readbuffer<-tclvalue(tkget(.muste$txt,\"%d.%d\",\"%d.%d\"))", r, c, r, len);
    // Rprintf("\n%s",komento);
    muste_evalr(komento);
...
void survo_ajax_screenbuffer() // RS 1.12.2015
{
    ...
    for (i = 1; i <= r3 + 2; i++) {
        ...
        apubuf[j + 1] = '\0';
        strcat(str, apubuf);
        muste_sprintf(apubuf, "%d\t%d\t", 0, i - 1);
        strcat(str, apubuf);
    }
    muste_sprintf(apubuf, "%d", r3 + 2);
    strcat(str, apubuf);
...
void survo_close_ajaxbuffer() {
    ...
    muste_sprintf(komento, "muste:::survo.sendajax()");
    muste_evalr(komento);
...
int write_string(char *x, int len, int shadow, int row, int col) {
    ...
    while (i < len) {
        ...
        {
            /* RS Handle Tcl-special characters: 34="  36=$  91=[  92=\   */
            // muste_sprintf(apubuf,"%c",x[i]);
            if ((unsigned char)x[i] == 34 || (unsigned char)x[i] == 36 || (unsigned char)x[i] == 91 ||
                (unsigned char)x[i] == 92) {
                y[j++] = 92;
            }
            y[j++] = x[i];
            pit++;
            ...
            if ((unsigned char)x[i] > 127 || i == len - 1) {
                if ((unsigned char)x[i] == 213) {
                    ...
                    muste_sprintf(komento, "delete %d.%d %d.%d", row, k, row, k + pit);
                    Muste_EvalTcl(komento, TRUE);

                    muste_sprintf(komento, "insert %d.%d \"%s\\u20AC\" shadow%d", row, k, y, (unsigned char)shadow);
                    Muste_EvalTcl(komento, TRUE);

                    // muste_sprintf(komento,"tkinsert(.muste$txt,\"%d.%d\",\"%s\\u20AC\",\"shadow%d\")",row,k,y,(unsigned
                    // char) shadow); 	tkinsert(.muste$txt,"1.0","koe\u20AC") muste_evalr(komento);
                    ...
                    if (muste_mac && shadow == 32 && shadow != 32) // RS 20.3.2013 /  29.1.2022 disabled
                    {
                        ...
                        while (yind < (y + ylen)) {
                            ...
                            if (transhadow == 9999 && ypit < 2 && *yind != EOS) {
                                *(yind - 1) = ' ';
                                continue;
                            }
                            muste_fieldcopy(plotkomento, yoldind, ypit);
                            plotkomento[ypit] = EOS;
                            //                            if (transhadow==9999) plotkomento[ypit-1]=' ';
                            muste_sprintf(komento, "insert %d.%d \"%s\" shadow%d", row, k + (int)(yoldind - y - yexto),
                                          plotkomento, transhadow);
                            Muste_EvalTcl(komento, TRUE);
                            yoldind = yind;
                            yexto += yext;
                            yext = 0;
    ...
    // RS REM T?m? n?ytt?isi olevan turha:    sur_locate(row,col);
        ...
        muste_sprintf(komento,"delete %d.%d %d.%d",row,col-1,row,col-1+len);
        Muste_EvalTcl(komento,TRUE);

        muste_sprintf(komento,"insert %d.%d \"%s\" shadow%d",row,col-1,y,(unsigned char) shadow);
        Muste_EvalTcl(komento,TRUE);
    */
...
int sur_scroll_up(int lines, int row1, int col1, int row2, int col2, int attr) {
    muste_sprintf(komento, "delete %d.0 %d.0", row1 - 1, row1);
    Muste_EvalTcl(komento, TRUE);

    muste_sprintf(komento, "insert %d.0 \" \n\"", row2);
    Muste_EvalTcl(komento, TRUE);
...
int sur_scroll_down(int lines, int row1, int col1, int row2, int col2, int attr) {
    muste_sprintf(komento, "delete %d.0 %d.0", row2 + 1, row2 + 2);
    Muste_EvalTcl(komento, TRUE);
...
int sur_scroll(int r1, int r2, int n, int suunta) {
    ...
    if (suunta == 7) {
        n = -n;
        sur_scroll_down(n, r1 + 1, 0, r2, c3 + 8, 119);
    } else {
        sur_scroll_up(n, r1 + 2, 0, r2 + 1, c3 + 8, 119);
    }
    return (1);
}
