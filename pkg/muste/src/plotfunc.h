...
static int p_inquiry() {
    ...
    print_rivi(x, j++);
    muste_sprintf(x, "XDIV=%d,%d,%d YDIV=%d,%d,%d", (int)xdiv1, (int)xdiv2, (int)xdiv3, (int)ydiv1, (int)ydiv2,
                  (int)ydiv3);
    print_rivi(x, j++);
...
/* int laji; 0=text_mode 1=vector_mode 2=no immediate output*/
{
    ...
    *(*pp - 1 - sulkuind) = EOS;
    ...
    if (laji == 0) {
        k = p_textcontrol(x + sulkuind);
        if (k < 0) {
            return (-1);
        }
    } else if (laji == 1) {
        k = p_linecontrol(x + sulkuind);
        if (k < 0) {
            return (-1);
        }
    }
    ...
    *pp = x;
    if (**pp == '[') {
        while (1) {
            q = strchr(*pp, ']');
            if (q == NULL) {
                muste_sprintf(sbuf, "] missing in %s!", x);
                p_error(sbuf);
                return (-1); // RS ADD
                             //                  Rprintf("\n] missing in %s!\n",x); WAIT; return(-1);
            }
...
static int frame(int frtype) /* FRAME: 0=ei 1=vain sisÑ 2=myîs ulko 3=vain ulko */
{
    ...
    if (i >= 0) {
        ...
        x_home = arit_atoi(osa[0]);
        y_home = arit_atoi(osa[1]);
    }
    i = spfind("SIZE");
    if (i >= 0) {
        strcpy(x, spb[i]);
        ...
        x_size = arit_atoi(osa[0]);
        y_size = arit_atoi(osa[1]);
    }
    ...
    */
    ...
    if (pr_type != 1) {
        if (frtype >= 2 && frtype < 6) {
            plot_box(x_home, y_home, x_size, y_size);
        }
        if (frtype == 5 || frtype == 6) {
            plot_halfbox(xx, yy, x_kuva, y_kuva); // 27.11.02
        }
        if (frtype > 0 && frtype < 3) {
            plot_box(xx, yy, x_kuva, y_kuva);
        }
    }
...
static int header(char *otsikko) {
    ...
    p_text2((unsigned char *)p, (unsigned char *)ps, x_home + (int)kirjainlev, y_home + y_size - (int)(a * kirjainkork),
            1);
    return (1);
}
...
static int frames() {
    ...
    *pkoodi = EOS;
    ...
    for (h = 0; h < ntext; ++h) {
        ...
        //                    }
        if (nt < 4) {
            muste_sprintf(sbuf, "Coords missing in frame %s | Syntax: name=x,y,width,height[,shading]", spb[i]);
            p_error(sbuf);
            return (-1); // RS ADD
        }
        xk = arit_atoi(sana[0]) + x_home;
        yk = arit_atoi(sana[1]) + y_home;
        ...
        /* 14.2.94 */
        {
            ...
            p_fill_bar(xk, yk, arit_atoi(sana[2]) + xk, arit_atoi(sana[3]) + yk, fill);
        } else {
            plot_box(xk, yk, arit_atoi(sana[2]), arit_atoi(sana[3]));
            if (nt == 5) {
                p_fill(xk + tikki / 2, yk + tikki / 2, atoi(sana[4]));
            }
        }
    }
    return (1);
...
static int fills() {
    ...
    for (h = 0; h < ntext; ++h) {
        ...
        //                    }
        if (nt < 3) {
            muste_sprintf(sbuf, "Incomplete fill point %s | Syntax: name=x,y,shading", spb[i]);
            p_error2(sbuf);
            return (-1); // RS ADD
        }
        xk = arit_atoi(sana[0]);
...
static void plot_xscale(int n,          /* arvojen lkm */
                        double value[], /* skaala-arvot */
                        char *label[],  /* skaalanimet */
                        int x0, int y0, /* alkupiste */
    ...
    */
    if ((frametype == 0 || frametype == 3) && !scalemove_y) {
        return;
    }
    find_tickturn();
    for (i = 0; i < n; ++i) {
        ...
        if (*q == '?') {
            ...
            p_line2(x1, y0, x1, y0 - 2 * tickturn * tikki, 1);
        }
        // 25.4.2003
        if (pyramid && *q == '-') {
            ++q; // 18.10.2005
...
static void plot_yscale(int n,          /* arvojen lkm */
                        double value[], /* skaala-arvot */
                        char *label[],  /* skaalanimet */
                        int x0, int y0, /* alkupiste */
                        int pituus      /* asteikon pituus */
    ...
    /*  Rprintf("\nyscale: n=%d\n",n);
        for (i=0; i<n; ++i) Rprintf("%g ",value[i]); getch();
    */
    if ((frametype == 0 || frametype == 3) && !scalemove_x) {
        return;
    }
    find_tickturn();
    ...
    if (ycharwidth > 0.0) {
        klev = ycharwidth / 0.28346;
    }
    i = spfind("YSCALEPOS");
    if (i >= 0) {
        strcpy(x, spb[i]);
        i = split(x, osa, 2);
        yscalepos1 = atof(osa[0]);
        if (i > 1) {
            yscalepos2 = atof(osa[1]);
        }
    }
    for (i = 0; i < n; ++i) {
        int x1, k;
        ...
        if (*q == '?') {
            ...
            p_line2(x0, y1, x0 - 2 * tickturn * tikki, y1, 1);
        }
        // 25.4.2003
        p = strchr(q, '.');
        if (p != NULL) {
...
static int xdiv() {
    ...
    xdiv1 = arit_atof(osa[0]);
    xdiv2 = arit_atof(osa[1]);
    xdiv3 = arit_atof(osa[2]);
    return (1);
}
...
static int ydiv() {
    ...
    ydiv1 = arit_atof(osa[0]);
    ydiv2 = arit_atof(osa[1]);
    ydiv3 = arit_atof(osa[2]);
    return (1);
}
...
static int legend(int koko) {
    ...
    *s = EOS;
    ...
    if (i >= 0) {
        ...
        k = split(s2, osa, 2);
        if (k > 1) {
            return (legend2(koko, p)); /* spb[i] -22.1.92 */
        }
...
static int legend2(int koko, char *s1) /* LEGEND=x_leg,y_leg,n_col */
{
    ...
    x_leg = x_home + arit_atoi(osa[0]);
    y_leg = y_home + arit_atoi(osa[1]);
    if (i > 2) {
        n_col = arit_atoi(osa[2]);
        if (n_col < 1) {
            n_col = 1;
        }
    }
    ...
    if (i >= 0) {
        ...
        x_box = arit_atoi(osa[0]);
        y_box = arit_atoi(osa[1]);
        if (i > 2) {
            koko = arit_atoi(osa[2]);
            ykoko = y_ratio * koko;
        }
        if (i > 3) {
            ykoko = arit_atoi(osa[3]);
        }
    }

    x_text = x_box + 1.5 * koko;
    y_text = y_box;
    ...
    ********************************************************************/
    ...
    for (l = dat.l1; l <= dat.l2; ++l) {
        ...
        if (grouping_var >= 0) {
            ...
            if (j == 0 || strcmp(x, prev_group) == 0 || *x == EOS || strcmp(x, "-") == 0) {
        ...
        for (i = 0; i < em - 1; ++i) {
            ...
            if (y == MISSING8) {
                ...
                muste_sprintf(sbuf, "Value of %.8s missing in obs.#%ld!", dat.varname[dat.v[i]], l);
                p_error(sbuf);

                return (-1);
            }
            xmat[j * em + i] = y;
            if (pyramid && xtype[i] == 'A') {
                xmat[j * em + i] = -y; // 18.10.2005
            }
            if (devvar1 >= 0) // 1.5.2004
            {
                k = data_load(&dat, l, devvar1, &y);
                if (k < 0) {
                    return (-1); // RS 17.1.2013
                }
                devmat[2 * j] = y;
    ...
    if (pyramid) // reverse order
    {
        ...
        for (j = 0; j <= (en - 1) / 2; ++j) {
            strcpy(sbuf, xnimi[j]);
            strcpy(xnimi[j], xnimi[k]);
            strcpy(xnimi[k], sbuf);
            for (i = 0; i < em - 1; ++i) {
                y = xmat[j * em + i];
                xmat[j * em + i] = xmat[k * em + i];
                xmat[k * em + i] = y;
            }
            --k;
        }
...
static int dataopen(char data[]) {
    ...
    if (i >= 0) {
        ...
        j = 0;
        k = 0; // Poista GROUPING-muuttuja aktiivisista!
        for (i = 0; i < dat.m_act; ++i) {
            if (dat.v[i] == grouping_var) {
                k = 1;
                continue;
            }
            dat.v[j] = dat.v[i];
    ...
    i = 0;
    if (namevar < 0) {
        namevar = dat.v[i]; // oli 0
    }

    if (dat.vartype[namevar][0] != 'S') {
        namevar = -1;
    }
...
static int skaala_arvot(char *s, char *list, char *osa[], int *n, int scalespace) {
    ...
    *n = 0;
    ...
    for (h = 0; h < k; ++h) {
        ...
        {
            osa[(*n)++] = p_list;
            if (*n >= MAXSCALELIST || p_list - list > scalespace - strlen(p) - 1) {
                scale_err(s);
                return (-1);
            }
            while (*p) {
...
static void scale_err(char *s) {
    ...
    muste_sprintf(sbuf, "Error in scale notation %s: More than %d values", s, MAXSCALELIST);
    p_error(sbuf);
...
static double paras_arvo(double x, double y) {
    ...
    muste_sprintf(a, "%21.10f", x);
    a[21] = '\0';
    muste_sprintf(b, "%21.10f", y);
    b[21] = '\0';
    i = 0;
    while (a[i] == ' ') {
        a[i++] = '0';
...
static void plot_xscale2(int n,          /* arvojen lkm */
                         double value[], /* skaala-arvot */
                         char *label[],  /* skaalanimet */
                         int x0, int y0, /* alkupiste */
                         int pituus      /* asteikon pituus */
...
static void plot_yscale2(int n,          /* arvojen lkm */
                         double value[], /* skaala-arvot */
                         char *label[],  /* skaalanimet */
                         int x0, int y0, /* alkupiste */
                         int pituus      /* asteikon pituus */
...
static int define(char *x, char **sana, int n, char *rivi) {
    ...
    i = 0;
    while (i < n_sana) {
        if (muste_strcmpi(sana[1], pr_sana[i]) == 0) {
            break;
        }
        ++i;
    }
    ...
    pr_sana[i] = pr_osoitin;
    strcpy(pr_osoitin, sana[1]);
    pr_osoitin += strlen(sana[1]) + 1;
    pr_koodi[i] = pr_osoitin;
    strcpy(pr_osoitin, sana[2]);
...
static int shadows(char *x, char **sana, int n, char *rivi) /* shadow <koodi> <alkukoodisana> <loppukoodisana> */
{
    ...
    shadow[varjo] = pr_osoitin;
    strcpy(pr_osoitin, sana[2]);
    pr_osoitin += strlen(sana[2]) + 1;
    if (n < 4) {
        shadow2[varjo] = NULL;
    } else {
        shadow2[varjo] = pr_osoitin;
        strcpy(pr_osoitin, sana[3]);
        ...
        if (*s == '[') {
            ...
            *p = EOS;
            ...
            while (i < n_sana) {
                ...
                if (muste_strcmpi(s + 1, pr_sana[i]) == 0) {
                    break;
                }
                ++i;
            }
...
static int makro(char *sana, char *muunnos) {
    ...
    *s = EOS;
    ...
    i = 0;
    while (i < n_sana) {
        if (muste_strnicmp(prsana, pr_sana[i], len) == 0) {
            break;
        }
        ++i;
    }
...
static void muste_pcur(int argc, char *argv[]) {
    ...
    i = spfind("DEVICE");
    if (i < 0) {
        strcpy(laite, "MUSTE_PR.PS"); // RS CHA PRN -> MUSTE_PR.PS
    } else {
        strcpy(laite, spb[i]);
...
static void plot_tscale(int n,          /* arvojen lkm */
                        double value[], /* skaala-arvot */
                        char *label[],  /* skaalanimet */
                        int x0, int y0, /* alkupiste */
                        int pituus,     /* asteikon pituus */
...
static int xyscale(char *suunta) /* "X" tai "Y" */
{
    ...
    if (i >= 0) {
        ...
        i = split(ax, axx, 2);
        if (i > 0) {
            scalemove_y = arit_atoi(axx[0]);
        }
        if (i > 1) {
            scalemove_x = arit_atoi(axx[1]);
        }
    }
    ...
    if (*suunta == 'X') {
        ...
        if (scalemove_y) {
            p_line2(xx, yy + scalemove_y, xx + x_kuva, yy + scalemove_y, 1);
        }
        ...
        if (scalemove_x) {
            p_line2(xx + scalemove_x, yy, xx + scalemove_x, yy + y_kuva, 1);
        }
...
static int plot_curves() {
    ...
    if (i < 0) {
        ...
        for (k = 0; k < n_mess; ++k) {
            ...
            if (i > 1) {
                c_step[k] = arit_atoi(osa2[1]);
            }
            if (i > 2) {
                c_x[k] = arit_atoi(osa2[2]);
            }
            if (i > 3) {
                c_y[k] = arit_atoi(osa2[3]);
            }
            c_i[k] = 0;
            *c_text[k] = EOS;
        }
        ...
        while (i < nloop) {
            ...
            if (k == -1) /* 26.5.92 */
            {
                ...
                if (k < 0) {
                    ++i;
                    continue;
                } else if (capability[0] && prind) {
                    muste_sprintf(sbuf, " obs#=%ld", obs);
                    sur_print(sbuf);
                }
            } else {
                if (arvo[k] == loop_end[i]) {
                    ++i;
                    continue;
                }
                arvo[k] += loop_step[i];
...
static int coord(double t, int *px, int *py) {
    ...
    if (integral_ind) {
        y *= integral_const / integral_value;
    }
    if (x < xmin || x > xmax || y < ymin || y > ymax) {
        double xq, yq;

        if (!out) {
            outline(xv, yv, x, y, px, py);
            out = 1;
            xv = x;
            yv = y;
            return (1);
...
/* int *px,*py;   rajakoordinaatit */
{
    ...
    if (xsmu == xumu) {
        ...
        t1 = (xmumin - xsmu) / (xumu - xsmu);
        if (t1 < 0 || t1 > 1) {
            t1 = 1;
        }
        t2 = (xmumax - xsmu) / (xumu - xsmu);
        if (t2 < 0 || t2 > 1) {
            t2 = 1;
        }
        if (t2 < t1) {
...
static int plotting_range() {
    ...
    if (i >= 0) {
        ...
        if (k < 2) {
            ...
            muste_sprintf(sbuf, "Plotting range in form: %s=<lower_limit>,<upper_limit>,<step>", muuttujanimi);
            p_error(sbuf);
...
static int find_datapar(int i) {
    ...
    if (!data) {
        ...
        obs = curd.l1;
        while (obs <= curd.l2 && unsuitable(&curd, obs)) {
            ++obs;
        }
        data = 1;
        loopar[nloop] = -1;
    ...
    if (p != NULL) {
        *p = EOS;
        lag_datapar[nvar] = atoi(p + 1);
    }
    k = varfind(&curd, sana[1]);
    if (k < 0) {
        return (-1); // RS CHA FIXME exit(1);
    }
    curd_var[nvar] = k;
...
static void change_color() {
    ...
    if (color_max == 1) {
        ...
        line_color = (int)color % color_max; // 7.7.2010
    }
    p_lineattr();
}
...
static int fill() {
    ...
    if (k < 2) {
        ...
        muste_sprintf(sbuf, "Fill parameters in form: %s=<step>,<lower_limit>,<upper_limit>", spa[i]);
        p_error(sbuf);
    ...
    fill_end = t_end;
    if (k > 1) {
        fill_start = arit_atof(sana[1]);
    }
    if (k > 2) {
        fill_end = arit_atof(sana[2]);
    }
    return (1);
}
...
static int x_coord(double x) { return (xx + x_kuva * (xmu(x) - xmumin) / (xmumax - xmumin)); }

static int y_coord(double y) { return (yy + y_kuva * (ymu(y) - ymumin) / (ymumax - ymumin)); }
...
static int lab_find(char *x, char *lab, int m, int len) {
    ...
    for (i = 0; i < m; ++i) {
        if (strncmp(s, lab + i * len, (unsigned int)len) == 0) {
            break;
        }
    }
    if (i == m) {
...
static double funktio(char *s, double x) {
    ...
    if (strcmp(S, "RAND") == 0) {
        return (uniform(x)); // RS CHA
    }
    if (strcmp(S, "PROBIT") == 0) {
        return (probit(x));
    }
    if (strcmp(S, "ROUND") == 0) {
        return (sur_round(x));
    ...
    if (strcmp(S, "SGAMMA") == 0) {
        return (sur_gamma(x)); /* 2.11.1999 */
    }
...
static int f_edit(char *s, double *x, int n, double *py) {
    ...
    s[len++] = '(';
    i = 0;
    while (i < spn && (spp[i] != ':' || strncmp(s, spa[i], len) != 0)) {
        ++i;
    }
    if (i == spn) {
        s[len - 1] = EOS;
...
static double sur_gamma(double z) { return (exp(lg_gamma(z))); }
...
static void muste_pbar(int argc, char *argv[]) {
    ...
    if (muste_strcmpi(word[1], "/FRAME") == 0) {
        pframe();
        edisp = 1;
        s_end(argv[1]);
        return;
    ...
    v = 0;
    if (muste_strcmpi(gtype, "HBAR") == 0) {
        v = 1;
    }
    if (muste_strcmpi(gtype, "%HBAR") == 0) {
        v = 2;
    }
    if (muste_strcmpi(gtype, "MHBAR") == 0) {
        v = 3;
    }
    if (muste_strcmpi(gtype, "%MHBAR") == 0) {
        v = 4;
    }
    if (muste_strcmpi(gtype, "%AHBAR") == 0) {
        v = 5;
    }
    if (muste_strcmpi(gtype, "NHBAR") == 0) {
        v = 6; /* 2.2.1993 */
    }
    if (muste_strcmpi(gtype, "PYRAMID") == 0) {
        v = 6;
        pyramid = 1;
    } // 18.10.2005
    if (v) {
    ...
    if (muste_strcmpi(gtype, "VBAR") == 0) {
        v = 1;
    }
    if (muste_strcmpi(gtype, "%VBAR") == 0) {
        v = 2;
    }
    if (muste_strcmpi(gtype, "MVBAR") == 0) {
        v = 3;
    }
    if (muste_strcmpi(gtype, "%MVBAR") == 0) {
        v = 4;
    }
    if (muste_strcmpi(gtype, "%AVBAR") == 0) {
        v = 5;
    }
    if (muste_strcmpi(gtype, "NVBAR") == 0) {
        v = 6; /* 2.2.1993 */
    }
    if (v) {
        vbar(v, gtype, word[1]);
...
static int prosentit() {
    ...
    for (j = 0; j < n; ++j) {
        ...
        for (i = 0; i < em2 - 1; ++i) {
            b = xmat[j * em + i];
            if (b < 0.0) {
                muste_sprintf(sbuf, "Negative values not allowed!");
                p_error(sbuf);
                return (-1);
            }
            a += b;
        ...
        tot += xsumma[j];
        if (a == 0.0 && grouping_var >= 0) {
            a = 100.0; // 1.11.2002
        }
        if (a == 0.0) {
            muste_sprintf(sbuf, "All values =0 in an observation");
            p_error(sbuf);
            return (-1);
        }
        for (i = 0; i < em2 - 1; ++i) {
...
static int plot_hbar(int gtype) {
    ...
    */
    ...
    i = spfind("YSCALEPOS");
    if (i >= 0) {
        yscalepos = atoi(spb[i]);
    }

    if (gtype == 5) {
        ytaso = yy + y_kuva - vali2 + vali;
    }
    next_new_group = 0;
    ...
    for (j = 0; j < n; ++j) {
        ...
        if (j == 0 && gap2 == 0.0 && !mhbar) {
            lev2 = yy + y_kuva - ytaso;
        }
        ...
        if (name_ind) {
            ...
            n_patkat = patki2(xnimi[j], lab_delimiter); // 15.4.2011
                                                        ...
                                                        ********************************/
            if (n_patkat == 0) {
                p_text((unsigned char *)(xnimi[j]), x_apu, y_apu, 1);
            } else // pÑtkitty nimi
            {
        ...
        for (i = 0; i < m; ++i) {
            ...
            if (mhbar) {
                k = lev;
                ytaso = (int)(yy + (n - 1 - j) * m * lev + (n - j - 1) * vali + vali1) + (m - i - 1) * k;
            }
            ...
            if (devvar1 >= 0) {
                ...
                p_line2(x_apu2, (int)(ytaso + lev2 / 2), x_apu3, (int)(ytaso + lev2 / 2), 0);
                p_line2(x_apu3, (int)(ytaso + lev2 / 2 - lev2 / 6), x_apu3, (int)(ytaso + lev2 / 2 + lev2 / 6), 0);
                a = devmat[2 * j + 1];
                x_apu3 = xx + (a - xmin) / (xmax - xmin) * x_kuva;
                p_line2(x_apu2, (int)(ytaso + lev2 / 2), x_apu3, (int)(ytaso + lev2 / 2), 0);
                p_line2(x_apu3, (int)(ytaso + lev2 / 2 - lev2 / 6), x_apu3, (int)(ytaso + lev2 / 2 + lev2 / 6), 0);
            ...
            if (*valform != EOS) {
                ...
                if (arvo >= valuemin) {
                    valtext(x_apu, y_apu, x_apu2 - x_apu, arvo, i); // 16.9.2010
                }
                if (*valcode != EOS) {
                    k = p_pen();
                    if (k < 0) {
...
/* char *suunta;  "X" tai "Y" */
{
    ...
    if (i >= 0) {
        ...
        if (*suunta == 'X') {
            ...
            k = 2 * y_kuva / kirjainkork;
        }
        i = autom_scale(x, min, max, k);
        if (i < 0) {
            return (-1);
        }
    }
    ...
    if (*suunta == 'X') {
        ...
        ymin = ymumin = xscaleval[0];
        ymax = ymumax = xscaleval[xscalen - 1];
...
static int valtext(int x1, int y1, int leveys, double arvo, int color) {
    ...
    if (valpaikka * valind >= 0) {
        if (leveys >= 0) {
            paikka = x1 + kirjainlev * valpaikka;
        } else {
            paikka = x1 - kirjainlev * (valpaikka + len);
        }
    } else {
        if (leveys >= 0) {
            paikka = x1 + leveys - kirjainlev * (-valpaikka + len);
        } else {
            paikka = x1 + leveys - kirjainlev * valpaikka;
        }
    }

    if (colors_2010) {
        p_textcolors(shadecolor[color]); // 16.9.2010
    }
...
static void valtext2(int x1, int y1, int korkeus, double arvo, int color) {
    ...
    if (valpaikka * valind >= 0) {
        if (korkeus >= 0) {
            paikka = y1 + kirjainkork * valpaikka;
        } else {
            paikka = y1 - kirjainkork * (valpaikka + 0.5);
        }
    } else {
        if (korkeus >= 0) {
            paikka = y1 + korkeus + kirjainkork * (valpaikka - 0.5);
        } else {
            paikka = y1 + korkeus - kirjainkork * valpaikka;
        }
    }
    if (colors_2010) {
        p_textcolors(shadecolor[color]); // 16.9.2010
...
static void labtext(int x1, int y1, int leveys, char *teksti, int color) {
    ...
    if (labpaikka >= 0) {
        if (leveys >= 0) {
            paikka = x1 + kirjainlev * labpaikka;
        } else {
            paikka = x1 - kirjainlev * (labpaikka + len);
        }
    } else {
        if (leveys >= 0) {
            paikka = x1 + leveys - kirjainlev * (-labpaikka + len);
        } else {
            paikka = x1 + leveys - kirjainlev * labpaikka;
        }
    }
    if (colors_2010) {
        p_textcolors(shadecolor[color]); // 16.9.2010
...
static void labtext2(int x1, int y1, int korkeus, char *teksti, int color) {
    ...
    if (labpaikka >= 0) {
        if (korkeus >= 0) {
            paikka = y1 + kirjainkork * labpaikka;
        } else {
            paikka = y1 - kirjainkork * (labpaikka + 0.5);
        }
    } else {
        if (korkeus >= 0) {
            paikka = y1 + korkeus + kirjainkork * (labpaikka - 0.5);
        } else {
            paikka = y1 + korkeus - kirjainkork * labpaikka;
        }
    }
    if (colors_2010) {
        p_textcolors(shadecolor[color]); // 16.9.2010
...
static int names() {
    ...
    *namecode = EOS;
    ...
    if (p != x) {
        ...
        *(p - sulkuind) = EOS;
        strcpy(namecode, x + sulkuind);
        ++p;
    }
    i = split(p, s, 2);
    if (i > 0) {
        name_ind = atoi(s[0]); // # of levels 3.5.2004-
    }
    if (i > 1) {
        name_gap = atoi(s[1]);
    }
...
static int plot_vbar(int gtype) {
    ...
    for (j = 0; j < n; ++j) {
        ...
        for (i = 0; i < m; ++i) {
            ...
            if (mvbar) {
                k = lev;
                xtaso = (int)(xx + j * m * lev + j * vali + vali1) + i * k;
            }
            ...
            if (devvar1 >= 0) {
                ...
                p_line2((int)(xtaso + lev2 / 2), y_apu2, (int)(xtaso + lev2 / 2), y_apu3, 0);
                p_line2((int)(xtaso + lev2 / 2 - lev2 / 6), y_apu3, (int)(xtaso + lev2 / 2 + lev2 / 6), y_apu3, 0);
                a = devmat[2 * j + 1];
                y_apu3 = yy + (a - xmin) / (xmax - xmin) * y_kuva;
                p_line2((int)(xtaso + lev2 / 2), y_apu2, (int)(xtaso + lev2 / 2), y_apu3, 0);
                p_line2((int)(xtaso + lev2 / 2 - lev2 / 6), y_apu3, (int)(xtaso + lev2 / 2 + lev2 / 6), y_apu3, 0);
            ...
            if (*valform != EOS) {
                ...
                if (arvo >= valuemin) {
                    valtext2(xtaso, y_apu, y_apu2 - y_apu, arvo, i); // 16.9.2010
                }
                if (*valcode != EOS) {
                    k = p_pen();
                    if (k < 0) {
    ...
    for (j = 0; j < n; ++j) {
        ...
        for (i = 0; i < m; ++i) {
            ...
            if (*valform != EOS) {
                ...
                if (arvo >= valuemin) {
                    valtextpie(xp, yp, r, kulma1, kulma2, arvo, i);
                }
                /* ry? */
                if (*valcode != EOS) {
                    k = p_pen();
...
static int contours() {
    ...
    if (i < 0) {
        p_end();
        return (-1);
    } // RS AXES allowed (no own xyscale-funktion for contour) FIXME?
    i = xlabel(xmuunnos);
    if (i < 0) {
        p_end();
        return (-1);
...
static int plot_contours() {
    ...
    */
    ...
    if (capability[0]) {
        sur_print("\nContour plotting: ");
        pros = 1;
        pros_step = (long)((double)ny / 100.0);
    }
    ...
    for (y = y_end - y_step / 2; y > y_start; y -= y_step) {
        ...
        if (capability[0] && iy >= pros * pros_step) {
            muste_sprintf(sbuf, " %d%%", (int)pros);
            sur_print(sbuf); // RS ADD (int)
            ++pros;
        }
...
static int plotting_range_contour(char *mnimi, double *t_start, double *t_end, double *t_step) {
    ...
    if (i < 0) {
        muste_sprintf(sbuf, "Specification %s=<lower_limit>,<upper_limit>,<step> not found!", mnimi);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
    ...
    *rowlabel_code = EOS;
    ...
    if (i >= 0) {
        ...
        if (p != x) {
            ...
            *(p - i) = EOS;
            strcpy(rowlabel_code, x + i);
            ++p;
        }
        i = split(p, osa, 3);
        rowlabels = atoi(osa[0]);
        if (i > 1) {
            nrivitasot = atoi(osa[1]);
            if (nrivitasot < 1) {
                nrivitasot = 1;
            }
        }
    ...
    for (j = d.l1; j <= d.l2; ++j) {
        ...
        case 1:
            ...
            for (i = 0; i < nx; ++i) {
                ...
                if (zarvo == MISSING8) {
                    pxl_value[ix] = (int)(255 * miss_zarvo);
                } else {
        ...
        */
        ...
        p_contour_plot(ny, iy, nx, pxl_value);
        if (capability[0] && prind) {
            muste_sprintf(sbuf, "%d/%d ", iy + 1, ny);
            sur_print(sbuf);
        }
        if (sur_kbhit()) {
            i = sur_getch();
...
static int tutki_data() {
    ...
    */
    ...
    if (norm == 3) {
        for (i = 1; i < nx; ++i) {
            if (min_arvo[i] < min_arvo[0]) {
                min_arvo[0] = min_arvo[i];
            }
            if (max_arvo[i] > max_arvo[0]) {
                max_arvo[0] = max_arvo[i];
            }
        }
    }
    return (1);
...
static void muste_faces(int argc, char *argv[]) {
    ...
    if (strcmp(info, "FACES") == 0) {
        ...
        xdiv1 = xdiv3 = 3 * kirjainlev;
        xdiv2 = x_size - 6 * kirjainlev;
        ydiv1 = ydiv3 = 2 * kirjainkork;
        ydiv2 = y_size - 4 * kirjainkork;
        if (andrews_polar) {
            xdiv1 = 0.0;
            xdiv2 = 1.0;
            xdiv3 = 0.0;
...
static int xyscale_faces(char *suunta) /* "X" tai "Y" */
{
    ...
    if (i >= 0) {
        ...
        if (*suunta == 'X') {
            strcpy(x, "-3.1416:-pi,0,3.1416:pi");
        } else {
            strcpy(x, "-2,-1,0,1,2");
        }
...
static int fheader(char *otsikko) {
    ...
    if (ots >= 0) {
        strcpy(spb[ots], otsikko2);
    }
    header(otsikko2);
    if (ots >= 0) {
        strcpy(spb[ots], otsikko);
    }
    return (1);
}
...
static int plot_faces() {
    ...
    if (i >= 0) {
        strcpy(x, spb[i]);
        i = split(x, osa, 2);
        flev = arit_atoi(osa[0]);
        fkork = 1.1 * y_ratio * flev;
        if (i > 1) {
            fkork = arit_atoi(osa[1]);
        }
    }
    nx = x_kuva / flev;
    ny = y_kuva / fkork;
    ...
    for (j = d.l1; j <= d.l2; ++j) {
        ...
        for (i = 0; i < NVARFACES; ++i) {
            if (v[i] == -1) {
                y[i] = (fmin_faces[i] + fmax_faces[i]) / 2;
            } else {
...
static int tutki_lista() {
    ...
    while (j <= r2) {
        edread(x, j);
        i = split(x + 1, osa, 1);
        if (i == 1 && strncmp(osa[0], "VARIABLES", 9) == 0) {
            break;
        }
        ++j;
    }
    ...
    for (i = 0; i < NVARFACES; ++i) {
        ...
        fmin_faces[i] = atof(osa[4]);
        fmax_faces[i] = atof(osa[5]);

        if (strcmp(osa[0], "-") != 0) {
            k = varfind2(&d, osa[0], 0);
            if (k < 0) {
                muste_sprintf(sbuf, "Error on line %d: Variable %s not in data %s!", j, osa[0], word[1]);
                p_error2(sbuf);
                return (-1); // RS ADD
            }
            v[i] = k;
...
static int tutki_varit(int j) {
    ...
    edread(x, j);
    split(x + 1, osa, 1);
    if (muste_strnicmp(osa[0], "COLORS", 6) != 0) {
        return (1);
    }
    while (j < r2) {
        ++j;
        ...
        if (muste_strnicmp(osa[0], "END", 3) == 0) {
            return (1);
        }
        for (i = 0; i < NV; ++i) {
            if (muste_strnicmp(osa[0], piirre[i], lpiirre[i]) == 0) {
                break;
            }
        }
        if (i == NV || k < 2) {
        ...
        if (strcmp(osa[1], "-") == 0) {
            ...
            vv[i] = -2;
            vcolor[i][0] = arit_atoi(osa[2]); /* vakiovÑri */
            continue;
        }
...
static void plot_face(double *y, int x1, int y1, int fkork, int flev, long j, int (*scolor)(long, int, int *)) {
    double x0, y0;
    int vari;
    int line_color2;
    int fill;
    ...
    x0 = y[0] * muste_cos(-y[1]);
    y0 = y[0] * muste_sin(-y[1]);
    q1 = (y[3] * x0 * y[3] * x0 + (y0 - y[2]) * (y0 - y[2])) / (2 * (y[2] - y0) * y[3]);
    q2 = y[3] * q1;
    q3 = y[2] - q2;
    t0 = atan((y0 - q3) / (y[3] * x0));
    u = PII - t0;
    ts = (u - t0) / 20;
    vari = scolor(j, 0, &fill);
    ...
    q2 = y[4] * q1;
    q3 = q2 - y[2];
    t0 = PII - atan((y0 - q3) / (y[4] * x0));
    u = 3 * PII - t0;
    ts = (u - t0) / 20;
    curve_plot(1, x1, y1, flev, fkork);
    if (fill > -999) {
        p_floodfill((int)(x1 + flev / 2), (int)(y1 + fkork / 2), fill);
    }
    if (vari >= 0) {
...
static void laske_f(int g, double t) {
    ...
    case 3:
        xco = t;
        yco = sgn_faces(qr) * (fabs(qr) - sqrt(fabs(qr * qr - xco * xco))) - q6;
        break;
    case 4:
        xco = q1 + q2 * muste_cos(t);
        yco = q3 + q4 * muste_sin(t + q5);
        break;
    }
}
    ...
    *t_code = EOS;
    ...
    if (i >= 0) {
        ...
        *label_code = EOS;
        ...
        if (i > 1) {
            lab_step = arit_atoi(osa[1]);
        }
        if (i > 2) {
            lab_move = arit_atoi(osa[2]);
        }
        first_label = 4;
    }
    ...
    for (j = d.l1; j <= d.l2; ++j) {
        ...
        first_label += 4;
        if (first_label > lab_step) {
            first_label = lab_move;
        }
    }
...
static void plot_andrews_curve(double *yf, int na, double tt[], char *t_code, char *label, char *label_code,
                               int lab_step, int first_label) {
    // RS REM        int i;
    double t, f;
...
static void plot_apolar_curve(double *yf, int na, double tt[], int x1, int y1, int flev, int fkork) {
    double t;
...
static int plot_drafts() {
    ...
    if (!point_size_varying && (point_var >= 0 || *point_text)) {
        xpp = (int)(-kirjainlev / 2.0);
        ypp = (int)(-kirjainkork / 2.0);
    } else {
        xpp = ypp = 0;
    }
...
static void plot_dboxes() {
    ...
    if (i >= 0) {
        ...
        i = split(s, osa, 2);
        if (i > 1) {
            if (muste_strcmpi(osa[1], "UPPER") == 0) {
                laajuus = 2;
            }
            if (muste_strcmpi(osa[1], "LOWER") == 0) {
                laajuus = 1;
            }
        }
    }
    ...
    dxgap = dxsize / 20;
    dxsize -= dxgap;
    dysize_muste = y_kuva / m;
    dygap = dysize_muste / 20;
    dysize_muste -= dygap;
    ...
    p_pen();
    for (i = 0; i < m; ++i) {
        muste_strncpy(s, d.varname[d.v[i]], 8);
        s[8] = EOS;
...
static int tutki_data3() {
    ...
    insc = inscale(dmin, dmax, jitter_step, nval);
    if (insc < 0) {
        return (-1);
    }
    ...
    if (!insc && jitter) {
        jitter_steps();
        for (i = 0; i < m; ++i) {
            dmin[i] -= jitter_step[i];
            dmax[i] += jitter_step[i];
        }
    }
    return (1);
}
...
static double uniform_faces(double x) {
    ...
    if (x == 0.0) {
        ...
        muste_srand(*pi + muste_rand());
        muste_rand(); /* 1. luku aina pieni!!! */
    } else {
        if (next) {
            return (RND);
        }
    ...
    if (i >= 0) {
        ...
        *point_code = EOS;
        ...
        if (*osa[0] != '0' && atoi(osa[0]) == 0) {
            strncat(point_text, osa[0], 15);
            point_given = 1;
            return (1);
        }
        marker_type = 1; /* marker_size=10; 8.8.88 */
        if (k > 0) {
            marker_type = atoi(osa[0]);
        }
        if (k > 1) {
            marker_size = atoi(osa[1]);
        }
        if (k > 3) {
            point_var = varfind(&d, osa[2]);
            if (point_var < 0) {
                return (-1);
            }
...
static int draft_merkitse(long j, int x, int y) {
    ...
    if (point_var >= 0) {
        ...
        p_text((unsigned char *)point_text, x, y, 1);
        x_pos = x;
        y_pos = y;
    } else {
        p_marker(x, y);
...
static int plot_stars() {
    ...
    if (star_plot == 1) {
        t0 = 0.0;
        u = 2 * PII * (double)(m - 1) / m;
        ts = 2 * PII / m;
        for (i = 0; i < m; ++i) {
            if (minx[d.v[i]] == maxx[d.v[i]]) {
                ++maxx[d.v[i]];
            }
        }
    } else {
...
static void plot_star(double *y, int x1, int y1, int fkork, int flev, long j, int (*scolor)(long, int, int *)) {
    int i;
    // RS REM        int vari;
...
static void plot_profile(double *y, int x1, int y1, int fkork, int flev, long j, int (*scolor)(long, int, int *)) {
    int i;
    // RS REM        int vari;
    // RS REM        int line_color2;
    // RS REM        int fill;
...
static void muste_pdia(int argc, char *argv[]) {
    ...
    i = data_open3(aineisto, &d, 0, 1, 0, 0);
    if (i < 0) {
        return;
    }
    i = mask(&d);
...
static int var_error(char *s) {
    ...
    if (etu == 2) {
        muste_sprintf(tut_info, "˛˛˛@1@PDIA@%s@", sbuf);
        s_end(argv1);
        return (1);
    }
    sur_print("\n");
...
static int diagrams() {
    ...
    if (!aika) {
        tee_label(otsikko, xvar, xmuunnos);
    } else {
        tee_label(otsikko, tvar, xmuunnos);
    }
    i = xlabel(otsikko);
    if (i < 0) {
        p_end();
    ...
    i = plot_conf_band(conf_band[1]);
    if (i < 0) {
        p_end();
        return (-1);
    }
    i = plot_conf_band(conf_band[2]);
    if (i < 0) {
        p_end();
        return (-1);
    }
    i = plot_conf_band(conf_band[3]);
    if (i < 0) {
        p_end();
        return (-1);
    }
...
static int xyscale_dia(char *suunta) /* "X" tai "Y" */
{
    ...
    if (i >= 0) {
        ...
        if (aika && *suunta == 'X') {
            ...
            p = strchr(x, '{');
            if (p != NULL) {
                q = strchr(p + 1, '}'); // RS 4.2.2013 q=NULL
            }
            if (p == NULL || q == NULL) {
                if (!rajat_etsitty) {
                    i = etsi_rajat(suunta);
    ...
    if (*suunta == 'X') {
        ...
        if (aika) {
            plot_tscale_dia(xscalen, xscaleval, xscal, xx, yy + scalemove_y, x_kuva, 1);
        } else {
            plot_xscale(xscalen, xscaleval, xscal, xx, yy + scalemove_y, x_kuva);
        }
    } else {
...
static void plot_tscale_dia(int n,          /* arvojen lkm */
                            double value[], /* skaala-arvot */
                            char *label[],  /* skaalanimet */
                            int x0, int y0, /* alkupiste */
                            int pituus,     /* asteikon pituus */
...
static int plot_diagram() {
    ...
    for (iy = 0; iy < nyvar; ++iy) {
        ...
        if (!point_size_varying && (point_var >= 0 || *point_text)) {
            xp = (int)(-kirjainlev / 2.0);
            yp = (int)(-kirjainkork / 2.0);
        } else {
            xp = yp = 0;
        }
        for (i = 0; i < 13; ++i) {
        ...
        for (i_thick = 0; i_thick < thickness; ++i_thick) {
            ...
            for (j = d.l1; j <= d.l2; j += step) // 22.11.2001
            {
                ...
                if (unsuitable(&d, j)) {
                    missing = 1;
                    continue;
                } // RS 4.3.2014 REM line!=10
                i = coord_dia(j, &x, &y);
                if (!obs_found) {
                    x_pos = x;
                    y_pos = y;
                }
                if (trend || contour || conf_band[1] || conf_band[2] || conf_band[3]) {
...
static int coord_dia(long j, int *px, int *py) {
    ...
    *py = yy + y_kuva * (ymu(y) - ymumin) / (ymumax - ymumin) + y_thick[i_thick];
    xxx = x;
    yyy = y; /* for TREND and/or CONTOUR */
    return (1);
}
...
static int xy_arvot_dia(long j, double *px, double *py) {
    ...
    if (normal) {
        ++i_normal;
        *py = ((double)i_normal - 0.5) / n_normal;
    } else {
...
static int normal_check() {
    ...
    for (j = d.l1; j <= d.l2; ++j) {
        ...
        if (x < x_edell) {
            muste_sprintf(sbuf, "Please, sort data in ascending order by %.8s!", d.varname[xvar]);
            p_error(sbuf);
            return (-1);
        }
        ++n_normal;
...
static int sp_line(int var) /* LINE=1,thickness*thickgap,line_label */
{
    ...
    *line_label = EOS;
    ...
    if (muste_strnicmp(osa[0], "POLY", 4) == 0 || atoi(osa[0]) == 10) // 31.8.2001
    {
        ...
        line_polygon_fill = 0;
        if (k > 1) {
            line_polygon_fill = atoi(osa[1]);
        }
        return (1);
    }
    line = atoi(osa[0]);
    if (muste_strnicmp(osa[0], "CURVE", 5) == 0) {
        line = 9; // RS 3.2.2013
    } else if (muste_strnicmp(osa[0], "CURVE2", 6) == 0) {
        line = 8; // RS 3.2.2013
    }
    if (line == 6 || line == 7) {
        i = lines2();
...
static int sp_point(int var) {
    ...
    *point_text = EOS;
    ...
    if (i >= 0) {
        ...
        if (k > 1) {
            marker_size = (int)arit_atof(osa[1]); // RS CHA 3.8.2012 atoi(osa[1]);
        }
        if (k > 3) {
            point_var = varfind(&d, osa[2]);
            if (point_var < 0) {
...
static int points2() {
    ...
    for (k = 2; k < nline2 + 2; ++k) {
        ...
        i = split(p, osa, 2);
        if (i > 0) {
            marker2[k - 2] = atoi(osa[0]);
        }
        if (i > 1) {
            markersize2[k - 2] = atoi(osa[1]);
        }
    }
    return (1);
}
...
static int diafill() {
    ...
    */
    ...
    for (j = fill_start2; j <= fill_end2; ++j) {
        ...
        i1 = coord2(j, &ax1, &ay1, 1);
        if (missing) {
            missing1 = 1;
            continue;
        }
        i2 = coord2(j, &ax2, &ay2, 2);
        if (missing) {
            missing1 = 1;
            continue;
        }
        ...
        if (i1 > 0 && i2 > 0) {
            ...
            while (x <= x1) {
                ...
                if (x1 == x_pos1) {
                    ...
                    yy1 = (int)(y_pos1 + (x - x_pos1) * (double)(y1 - y_pos1) / (x1 - x_pos1));
                }
                if (x1 == x_pos1) {
                    yy2 = y_pos2;
                } else {
                    yy2 = (int)(y_pos2 + (x - x_pos1) * (double)(y2 - y_pos2) / (x1 - x_pos1));
                }
...
static int fill_find(char *v) {
    ...
    *v = EOS;
    ...
    fill_start2 = (long)arit_atof(osa[1]);
    if (k < 3) {
        return (1);
    }
    fill_end2 = (long)arit_atof(osa[2]);
    if (k < 4) {
        return (1);
    }
    fill_var = varfind2(&d, osa[3], 0);
    if (fill_var < 0) {
        fill_const = arit_atof(osa[3]);
    }
    if (k > 4) {
        fill_line = atoi(osa[4]);
    }
    return (1);
}
...
static int plot_contour() {
    ...
    for (i = 0; i < ntaso; ++i) {
        ...
        if (eps == 0.0) {
            if (sx != sy) {
                t = 0.5 * atan(2 * sx * sy * r / (sx * sx - sy * sy));
                //              else t=0.0;
            } else {
                t = PII / 4; // 18.1.2003
            }
...
static void ellipse(double mx, double my, double sx, double sy, double r, double t, double eps, double *px,
                    double *py) {
    double a;
...
static int find_binorm(double *pmx, double *pmy, double *psx, double *psy, double *pr) {
    ...
    i = split(x, osa, 5);
    if (i < 5) {
        p_error("Error in BINORM: Invalid # of parameters!");
        return (-1);
    }
    *pmx = arit_atof(osa[0]);
    *pmy = arit_atof(osa[1]);
...
static void muste_histo(int argc, char *argv[]) {
    ...
    if (muste_strcmpi("CHISTO", word[0]) == 0) {
        cfunktio = 1;
    } else {
        cfunktio = 0;
    }
    ...
    if (freq_file == NULL) {
        ...
        i = control_code(his_attributes, &p, 2);
        if (i < 0) {
            return;
        }
        strcpy(x, p);
    ...
    if (p != NULL && strcmp(p + 1, "NULL") == 0) {
        return; // FREQ.F only! 14.6.2005
    }
        ...
        if (prind) {
            ...
            */
            ...
            if (fabs(y - aa) < eps || fabs(y - x_step - aa) < eps) {
                ++border_cases;
            } else if (fabs(y - x_step / 2 - aa) < eps) {
                ++middle_cases;
            }
        }
        if (sur_kbhit()) {
    ...
    if (!etu && (double)(border_cases) / (double)n_freq > 0.3) {
        ...
        if (i == 1) {
            ...
            if ((double)(middle_cases) / (double)n_freq < 0.1) {
                muste_sprintf(sbuf, "\nEhdotus: Muuta luokituksen alaraja %g arvoksi %g .", x_lower, aa);
                sur_print(sbuf);
            }
            WAIT;
        } else {
            ...
            if ((double)(middle_cases) / (double)n_freq < 0.1) {
                muste_sprintf(sbuf, "\nSuggestion: Change lower limit from %g to %g .", x_lower, aa);
                sur_print(sbuf);
            }
            WAIT;
        }
...
static int plot_histogram() {
    ...
    */
    ...
    if (*his_attributes) {
        ...
        i = p_linecontrol(his_attributes + i);
        if (i < 0) {
            return (-1);
        }
    }
    ...
    for (i = 0; i < n_class; ++i) {
        ...
        if (y2 > y1 + 1 && !no_fill) {
            if (capability[1]) {
                p_fill(x1 + 1, y1 + 1, fill);
            } else {
                p_fill_bar(x1, y1, x2, y2, fill);
                plot_box(x1, y1, x2 - x1, y2 - y1);
            }
        }
...
static int plot_distribution() {
    ...
    if (*fit_attributes) {
        ...
        i = p_linecontrol(fit_attributes + i);
        if (i < 0) {
            return (-1);
        }
    }
...
static int sp_fit() {
    ...
    *distr = EOS;
    ...
    k = control_code(fit_attributes, &p, 2);
    if (k < 0) {
        return (-1);
    }
    strcpy(distr, p);
...
static int fit_binomial() {
    ...
    prob[0] = pr;
    for (i = 1; i <= n; ++i) {
        prob[i] = pr *= (double)(n - i + 1) / i * p / (1 - p);
    }
    /*
     pr=0.0;
...
static double density(int dnro, double x, double *a) {
    ...
    switch (dnro) {
    case OWN_DISTR:
        return (f_own_distr(x, a));
    case CFUNCTION:
        return (f(x, a));
    case NORMAL:
        return (f_normal(x, a));
    case BINOMIAL:
        return (pr_binomial(x, a));
    case POISSON:
        return (pr_poisson(x, a));
    case LOGNORMAL:
        return (f_lognormal(x, a));
    case UNIFORM:
        return (f_uniform(x, a));
    case MATRIX:
        return (pr_matrix(x, a));
    }
    return (0.0);
}
...
/* char *tyyppi;  DENSITY tai PROBABILITY */
{
    ...
    while (j < r2) {
        ...
        if (muste_strcmpi(tyyppi, osa[0]) != 0) {
            continue;
        }
        p = strchr(osa[1], '(');
        if (p != NULL) {
        ...
        if (muste_strcmpi(distr, osa[1]) != 0) {
            continue;
        }
        nparn = 0;
        if (p == NULL) {
...
static int estimate() {
    ...
    *************************************/
    ...
    if (i >= 0) {
        ...
        k = split(x, osa, npar);
        if (k == 1) {
            step[0] = arit_atof(osa[0]);
            for (i = 1; i < npar; ++i) {
                step[i] = step[0];
            }
        } else if (k > 0) {
            for (i = 0; i < k; ++i) {
                step[i] = arit_atof(osa[i]);
            }
        }
    }
...
static int nelder(double *x, double *py, int n, double (*f)(double[]), double *step, double alpha, double beta,
                  double gamma, char **varname, char *fname, int maxnf) {
    int i, j;
    int nf;
    int jh, js, jl;
    double xx[N + 1][N];
    ...
    while (1) {
        ...
        **************************************************/
        ...
        for (i = 0; i < n; ++i) {
            x0[i] = (1 + alpha) * xc[i] - alpha * xx[jh][i];
        }
        y0 = (*f)(x0);
        ++nf;
        ...
        if (y0 < y[jh]) {
            for (i = 0; i < n; ++i) {
                x00[i] = beta * x0[i] + (1 - beta) * xc[i];
            }
        } else {
            for (i = 0; i < n; ++i) {
                x00[i] = beta * xx[jh][i] + (1 - beta) * xc[i];
            }
        }
        y00 = (*f)(x00);
        ++nf;
...
static int estim_results() {
    ...
    for (i = 0; i < npar; ++i) {
        ...
        if (inv == 1) {
            fnconv(sqrt(H[i * npar + i]), accuracy, sd);
        } else {
            strcpy(sd, "?");
        }
...
static void printout() {
    ...
    kok_osa = 1 + log((double)n_freq) / log(10.0) + 1e-7;
    if (kok_osa < 4) {
        kok_osa = 4;
    }
    i = kok_osa - 4;
    muste_strncpy(kok_lis, space, i);
    kok_lis[i] = EOS;
    /* Rprintf("\nn_freq=%ld kok_osa=%d",n_freq,kok_osa); getch(); */
    output_open(eout);
    muste_sprintf(x, "Frequency distribution of %s in %s: N=%ld", varname, aineisto, n_freq);
    eoutput(x);
    *x = EOS;
    eoutput(x);
    k = muste_sprintf(x, "Class midpoint  %sf     %%   %sSum     %%", kok_lis, kok_lis);
    if (*distr) {
        k += muste_sprintf(x + k, "     %se      %se    %sf     X^2", kok_lis, kok_lis, kok_lis);
    }
    eoutput(x);
    ...
    for (i = imin; i <= imax; ++i) {
        ...
        k = muste_sprintf(x, "%12.12s %*ld %s %*ld %s", label, kok_osa, freq[i], pros, kok_osa + 1, sum, sumpros);
    ...
    if (*distr) {
        ...
        if (rn_class - 1 - npar_est - mat_est > 0) {
            ...
            strcpy(x, "Degrees of freedom negative in Chi-square!");
        }
        eoutput(x);
    }
