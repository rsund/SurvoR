...
static int read_ftable(char *name, FREQ **f, int *pdim, int *pncvar, int *nc, char **varname, char **cname, char *type,
                       int *pndec);
...
static int glm_fit(double *X, int nx, int mx, double *Y, double *W, double *V, char *lab, double *b, double *sb,
                   double *my, double *pdev, int *pdf);
static int lin_fit(double *X, int nx, int mx, double *Y, double *W, double *V, char *lab, double *b, double *sb,
                   double *my, double *pdev, int *pdf);
...
static int print_ftable(char *name, int line, char *eout, int dim, int *nc, int *cumnc, int *ctype, FREQ *f, FREQ total,
                        FREQ missing, char **varname, char **cname, int colvar, int minwidth, int isum, int ipros,
                        char *form, char *type);
...
void muste_tabs(char *argv) {
    ...
    p = word[0];
    if (muste_strcmpi(p, "TABS") == 0) {
        tabs();
        s_end(argv);
        return;
    }
    if (muste_strcmpi(p, "TABM") == 0) {
        tabm();
        s_end(argv);
        return;
    }
    if (muste_strcmpi(p, "TABI") == 0) {
        tabi();
        s_end(argv);
        return;
    }
    if (muste_strcmpi(p, "TABD") == 0) {
        tabd();
        s_end(argv);
        return;
    }
    if (muste_strcmpi(p, "TABJ") == 0) {
        tabj();
        s_end(argv);
        return;
    }
    if (muste_strcmpi(p, "TABFIT") == 0) {
        tabfit();
        s_end(argv);
        return;
    }
    ...
    if (muste_strcmpi(p, "TABDIM") == 0) {
        tabdim();
        s_end(argv);
        return;
    }

    muste_sprintf(sbuf, "\nUnknown operation %s!", word[0]);
    sur_print(sbuf);
    WAIT;
}
...
static int tabs() {
    ...
    *s = EOS;
    ...
    print_ftable(nimi, results_line, eout, dim, nc, cumnc, ctype, f, ZERO, ZERO, varname, cname, dim - nrvar2, digits,
                 0, 0, s, type);
    return (1);
}
...
static int read_ftable(char *name, FREQ **f, int *pdim, int *pncvar, int *nc, char **varname, char **cname, char *type,
    ...
    j2 = edline2(px[3], j1, 1);
    if (j2 == 0) {
        return (-1);
    }
    muste_fieldcopy(type, px[4], 15);
    type[15] = EOS;
    ncell = 0;
    for (j = j1; j <= j2; ++j) {
        edread(x, j);
    ...
    if (j > j2) {
        muste_sprintf(sbuf, "\nLine of row classifiers ending with *'s missing in table %s!", name);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
    ...
    for (j = j0 + 1; j <= j2; ++j) {
        ...
        if (i != k) {
            muste_sprintf(sbuf, "\nNumber of elements on line %d conflicts previous lines!", j);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
    ...
    for (i = 0; i < nrow; ++i) {
        ...
        for (j = j0 + 1; j <= j2; ++j) {
            ...
            h2 = h;
            while (x[h2] != ' ' && h2 < posr[i + 1]) {
                ++h2;
            }
            x[h2] = EOS;
            for (h2 = 0; h2 < nc[ivar]; ++h2) {
                if (strcmp(x + h, cname[nlab + h2]) == 0) {
                    break;
                }
            }
            if (h2 < nc[ivar]) {
    ...
    */
    if (ncell2 != ncell) {
        ...
        for (ivar = 0; ivar < ncol + nrow; ++ivar) {
            ...
            sur_print(sbuf);
            for (h = 0; h < nc[ivar]; ++h) {
                muste_sprintf(sbuf, " %s", cname[i + h]);
                sur_print(sbuf);
            }
            i += nc[ivar];
        }
    ...
    *pndec = 0;
    for (j = j0 + 1; j <= j2; ++j) {
        ...
        for (i = 0; i < k; ++i) {
            len = strlen(px[i]) - 1;
            if (px[i][len] == '-') {
                cvalue = MISSING_VALUE;
                ++missing_values;
            } else if (strcmp(px[i], "*0") == 0) {
                cvalue = STRUCTURAL_ZERO;
            } else {
                cvalue = atof(px[i]); /* depends on FREQ */
            }
            (*f)[cell] = cvalue;
            cell += step;
            p = strchr(px[i], '.');
...
static int check_varname_initials(int dim, char **varname) {
    ...
    for (i = 1; i < dim; ++i) {
        for (k = 0; k < i; ++k) {
            if (*varname[k] != *varname[i]) {
                continue;
            }
            muste_sprintf(sbuf, "\nSame initials in classifier names %s (#%d) and  %s (#%d)", varname[k], k + 1,
                          varname[i], i + 1);
            sur_print(sbuf);
            WAIT;
            return (-1);
...
static int tabm() {
    ...
    k = strlen(word[2]);
    for (i = 0; i < dim; ++i) {
        if (strncmp(word[2], varname[i], k) == 0) {
            break;
        }
    }
    if (i == dim) {
    ...
    k = strlen(word[3]);
    for (i = 0; i < dim; ++i) {
        if (strncmp(word[3], varname[i], k) == 0) {
            break;
        }
    }
    if (i == dim) {
    ...
    *s = EOS;
    ...
    print_ftable(nimi, results_line, eout, dim, nc2, cumnc, ctype, f2, ZERO, ZERO, varname, cname2, ncvar, digits, 0, 0,
                 s, type);
    return (1);
}
...
static int tabi() {
    ...
    k = strlen(word[3]);
    for (i = 0; i < nc[i1]; ++i) {
        if (strncmp(word[3], cname[cumnc[i1] + i], k) == 0) {
            break;
        }
    }
    if (i == nc[i1]) {
    ...
    k = strlen(word[4]);
    for (i = 0; i < nc[i1]; ++i) {
        if (strncmp(word[4], cname[cumnc[i1] + i], k) == 0) {
            break;
        }
    }
    if (i == nc[i1]) {
    ...
    *s = EOS;
    ...
    print_ftable(nimi, results_line, eout, dim, nc, cumnc, ctype, f2, ZERO, ZERO, varname, cname, ncvar, digits, 0, 0,
                 s, type);
    return (1);
}
...
static int tabd() {
    ...
    while (1) {
        ...
        if (f2[ind2] == MISSING_VALUE || f[ind1] == MISSING_VALUE) {
            ...
            if (f[ind1] != STRUCTURAL_ZERO) {
                f2[ind2] = f[ind1];
            }
        } else if (f[ind1] != STRUCTURAL_ZERO) {
            f2[ind2] += f[ind1];
        }
        i = 0;
        while (1) {
    ...
    for (i = cumnc[i1]; i < nclass; ++i) {
        cname[i] = cname[i + nc[i1]];
    }
    cumnc[0] = 0;
    for (i = 1; i < dim - 1; ++i) {
        cumnc[i] = cumnc[i - 1] + nc2[i - 1];
    ...
    *s = EOS;
    ...
    print_ftable(nimi, results_line, eout, dim - 1, nc2, cumnc, ctype, f2, ZERO, ZERO, varname, cname, ncvar, digits, 0,
                 0, s, type);
    return (1);
}
...
static int tabdx(int ncvar) {
    ...
    if (i == dim) {
        ...
        *s = EOS;
        ...
        print_ftable(nimi, results_line, eout, dim, nc, cumnc, ctype, f, ZERO, ZERO, varname, cname, ncvar, digits, 0,
                     0, s, type);
        return (1);
    }
...
static int tabj() {
    ...
    for (i = cumnc[i1] + ic2; i < nclass - 1; ++i) {
        cname[i] = cname[i + 1];
    }
    cumnc[0] = 0;
    for (i = 1; i < dim; ++i) {
    ...
    *s = EOS;
    ...
    print_ftable(nimi, results_line, eout, dim, nc2, cumnc, ctype, f2, ZERO, ZERO, varname, cname, ncvar, digits, 0, 0,
                 s, type);
    return (1);
}
...
static int tabplus() {
    ...
    *s = EOS;
    ...
    print_ftable(nimi, results_line, eout, dim, nc, cumnc, ctype, f, ZERO, ZERO, varname, cname, ncvar, digits, 0, 0, s,
                 type);
    return (1);
}
...
static int fit(int ncvar) {
    ...
    if (*type == 'F') {
        ...
        muste_sprintf(s, "Table %s: Deviance=%s df=%d P=%6.4f", word[1], spois(s1), df, da);
    } else {
        fnconv(deviance, accuracy + 2, s1); /* 27.4.92 */
        muste_sprintf(s, "Table %s: Deviance=%s df=%d", word[1], spois(s1), df);
    }
    print_line(s);
    ...
    if (results > 0) {
        ...
        muste_sprintf(s, "    estimate%.*ss.e.%.*sparameter", accuracy, space, accuracy - 2, space);
        print_line(s);
        ...
        for (i = 0; i < mx; ++i) {
            if (sb[i] < 0) {
                ...
                muste_sprintf(s1, "%*.*f", accuracy + 3, accuracy - 2, b[i]);
    ...
    if (results > 70) {
        ...
        *s = EOS;
        ...
        print_ftable(nimi, results_line, eout, dim, nc, cumnc, ctype, my, ZERO, ZERO, varname, cname, ncvar, digits, 0,
                     0, s, type);
    }
    return (1);
}
...
// int *pdf;         /* degrees of freedom */
{
    ...
    *pdf = -mx;
    ...
    while (1) {
        ...
        muste_sprintf(sbuf, "\nCol. %.8s linearly dependent on previous ones!", lab_fit + 8 * (-i));
        sur_print(sbuf);
...
// int *pdf;         /* degrees of freedom */
{
    ...
    *pdf = -mx;
    ...
    while (1) {
        ...
        muste_sprintf(sbuf, "\nCol. %.8s linearly dependent on previous ones!", lab_fit + 8 * (-i + 1));
        sur_print(sbuf);
        reduce(X, lab_fit, nx, mx, -i);
        --mx;
        ++*pdf;
...
static int glm_fit_space(int mx, int nx) {
    ...
    i = mspace(&u_fit, nx, mx);
    if (i < 0) {
        return (-1);
    }
    i = mspace(&r_fit, mx, mx);
    if (i < 0) {
        return (-1);
    }
    i = mspace(&rinv_fit, mx, mx);
    if (i < 0) {
        return (-1);
    }
    i = mspace(&a_fit, mx, 1);
    ...
    if (idel_fit != NULL) {
        idel_fit = (int *)muste_realloc(idel_fit, mx * sizeof(int));
    } else {
        idel_fit = (int *)muste_malloc(mx * sizeof(int));
    }
    if (idel_fit == NULL) {
        not_enough_memory();
        return (-1);
    ...
    if (lab_fit != NULL) {
        lab_fit = (char *)muste_realloc(lab_fit, 8 * mx);
    } else {
        lab_fit = (char *)muste_malloc(8 * mx);
    }
    if (lab_fit == NULL) {
        not_enough_memory();
        return (-1);
...
static int reduce(double *X, char *lab, int nx, int mx, int i1) {
    ...
    for (i = nx * i1; i < nx * (mx - 1); ++i) {
        X[i] = X[i + nx];
    }
    for (i = 8 * i1; i < 8 * (mx - 1); ++i) {
        lab[i] = lab[i + 8];
    }
    return (1);
}
...
// char *type;              /* F,X etc. */
{
    ...
    if (strlen(form) > minwidth) {
        minwidth = strlen(form);
    }
    sum = NULL;
    nlines = 1;
    ...
    if (total) {
        strcat(x, "   N=");
        muste_sprintf(value, "%d", (int)total);
        strcat(x, value);
        if (missing) {
            strcat(x, " N(missing)=");
            muste_sprintf(value, "%d", (int)missing);
            strcat(x, value);
        }
    }
    output_line(x, eout, line);
    if (line) {
        ++line;
    }
    ...
    ftotal = 0;
    for (k = 0; k < m * nlines; ++k) {
        ftotal += f[k]; /* varmistus */
    }
    if (ipros) {
        if (ipros == 1) {
            ...
            if (ipros) {
                ...
                case 3:
                    if (ftotal == 0) {
                        ...
                        pros = (double)f[z] / ftotal;
                    }
                }
                fconv(100.0 * pros, "###.#", value);
            } else {
                if (f[z] == MISSING_VALUE) {
                    strcpy(value, "-");
                } else if (f[z] == STRUCTURAL_ZERO) {
                    strcpy(value, "*0");
                } else {
                    fconv(pdf[z], form, value);
                }
            }
