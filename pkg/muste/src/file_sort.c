...
static int lomita(int file1, int file2, int kierros, int nro) {
    ...
    if (prind) {
        muste_sprintf(sbuf, "\nMerging files SORT%d%d - %d%d.TMP to SORT%d%d.TMP: ", kierros - 1, file1, kierros - 1,
                      file2, kierros, nro);
        sur_print(sbuf);
    }
    nfi = file2 - file1 + 1;
    ...
    if (prind) {
        muste_sprintf(sbuf, "n=%d", (int)unf);
        sur_print(sbuf); // RS CHA %ld -> %d
    }
    muste_sprintf(nimi, "%sSORT%d%d.TMP", etmpd, kierros, nro);
...
/* kierros;  SORT*0.TMP  *=kierros */
{
    ...
    if (d1.m != d1.m_act || k >= 0) // RS 6.9.2013
    {
        ...
        if (fma > 0) // RS 21.1.2014
        {
            ...
            if (fma) {
                muste_snprintf(sbuf, LLENGTH, " ## FILE COPY %s,%s / IND=ORDER,0 %s", word[2], pathname, x + 22);
            }
            //				else snprintf(sbuf,LLENGTH," ## FILE COPY %s,%s / VARS=ALL IND=ORDER,0 /
            //%s",word[2],pathname,x+22); // RS 4.11.2013
            ...
            i = data_open3(pathname, &apudata, 1, 1, 1, 0);
            if (i < 0) {
                return (-1);
            }
    ...
    if (n_osat == 1) {
        ...
        if (sortf == NULL) {
            muste_sprintf(sbuf, "\nCannot read temporary file %s", nimi);
            sur_print(sbuf);
            WAIT;
            muste_fclose2(uusi);
            return (-1);
    ...
    *s_keyvar = EOS;
    ...
    if (i >= 0 && sort_key_is_string) {
        ...
        if (s_keyvar_nr >= 0) {
            ...
            if (i < slen - 4) {
                muste_sprintf(sbuf, "Length of %s is too small (%d). %d bytes required!", s_keyvar, i, slen - 4);
                sur_print(sbuf);
                WAIT;
    ...
    muste_sprintf(sbuf, "\n\nSaving %ld sorted records to file %s ...", nhav, pathname);
    sur_print(sbuf);
    //      pros=1;
    pros = 1;
    pros_step = (long)((double)nhav / 100.0);
    ...
    case '2':
        while (h > 1) {
            ...
            while (1) {
                ind = '1';
                for (i = j1; i <= j2 - h; ++i) {
                    if ((neg[t] && *(short *)(key + ikey[i] + sp[t]) < *(short *)(key + ikey[i + h] + sp[t])) ||
                        (!neg[t] && *(short *)(key + ikey[i] + sp[t]) > *(short *)(key + ikey[i + h] + sp[t]))) {
                        k = ikey[i];
                        ikey[i] = ikey[i + h];
                        ikey[i + h] = k;
                        ind = '0';
...
static int avaimet() {
    ...
    while (i < g && muste_strcmpi(word[i], "TO") != 0) {
        ...
        */
        sp[nsk] = slen;
        slen += su[nsk] - sl[nsk] + 1;
        stype[nsk] = d1.vartype[sk[nsk]][0];
        if (stype[nsk] != 'S') {
            sort_key_is_string = 0;
        }
        ++i;
        ++nsk;
    } /* while */
...
void muste_file_sort(int argc, char *argv[]) {
    ...
    i = talletus(word[4 + nsk], i);
    if (i < 0) {
        return;
    }
    data_close(&d1);
    i = data_open2(word[4 + nsk], &d1, 0, 1, 1);
    if (i < 0) {
        return;
    }
