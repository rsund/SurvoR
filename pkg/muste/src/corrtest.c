...
static int corrtest_disp(void);
static int printout(void);
static int print_line(char *line);
static int vertailu_annetuilla_arvoilla(void);
static int r_error(double r);
static int n_error(int n);
static int one_sample_test(void);
static int simulation1(void);
static int varaa_tilat1(void);
static int disp01(void);
static int disp1(void);
static int printout1(void);
...
void muste_corrtest(char *argv) {
    ...
    n[0] = talleta(word[1], word[2], 0);
    if (n[0] < 0) {
        return;
    }
    n[1] = talleta(word[3], word[4], 1);
    if (n[1] < 0) {
        return;
    }
    nn = n[0] + n[1];
    ...
    for (i = 0; i < n[0]; ++i) {
        ...
        xx[i] = (ff[0] - sumx[0]) / sumx2[0];
        yy[i] = (ff[1] - sumy[0]) / sumy2[0];
    }
    for (i = n[0]; i < nn; ++i) {
        muste_fread(ff, sizeof(double), 2, ftemp);
        xx[i] = (ff[0] - sumx[1]) / sumx2[1];
        yy[i] = (ff[1] - sumy[1]) / sumy2[1];
    }
    fclose(ftemp);
    /*
            for (i=0; i<n; ++i)
...
static int disp0() {
    ...
    (*g_print)(sbuf);
    LOCATE(r + 3, 9);
    muste_sprintf(sbuf, "Sample 1: Data %s, Variables %s,%s  N1=%d R1=%g", nimi[0], v1[0], v2[0], n[0], rr[0]);
    (*g_print)(sbuf);
    LOCATE(r + 4, 9);
    muste_sprintf(sbuf, "Sample 2: Data %s, Variables %s,%s  N2=%d R2=%g", nimi[1], v1[1], v2[1], n[1], rr[1]);
    (*g_print)(sbuf);
    LOCATE(r + 5, 9);
    test_val = fisher_z(rr, n);
    prob = 1.0 - muste_cdf_std(test_val);
    muste_sprintf(sbuf, "Test based on Fisher's z %g Normal approximation P=%g", test_val, prob);
    (*g_print)(sbuf);
...
static int corrtest_disp() {
    ...
    muste_sprintf(sbuf, "%10d %.8f %.8f lower limit", u, p1, lower);
    (*g_print)(sbuf);
    LOCATE(r + 8, 9);
...
static int vertailu_annetuilla_arvoilla() {
    ...
    one = 0;
    if (muste_strnicmp(word[1], "ONE", 3) == 0) {
        one = 1;
    }
    ...
    if (one) {
        ...
        if (r2 == 0.0) {
            ...
            muste_sprintf(sbuf, "Standard t test value %g  P=%g (2-tailed P=%g)", test_val, prob, prob2);
            print_line(sbuf);
            output_close(eout);
            return (1);
        }
    ...
    muste_sprintf(sbuf, "Test based on Fisher's z %g Normal approximation P=%g", u, prob);
    print_line(sbuf);
    output_close(eout);
...
static int disp01() {
    ...
    (*g_print)(sbuf);
    LOCATE(r + 3, 9);
    muste_sprintf(sbuf, "Sample: Data %s, Variables %s,%s  N=%d R=%g", nimi[0], v1[0], v2[0], n[0], rr[0]);
    (*g_print)(sbuf);
    LOCATE(r + 4, 9);
...
static int disp1() {
    ...
    lower2 = p2 - conf_coeff * se2;
    if (lower2 < 0.0) {
        lower2 = 0.0;
    }
    upper2 = p2 + conf_coeff * se2;
    if (upper2 > 1.0) {
        upper2 = 1.0;
    }
    muste_sprintf(sbuf, "%10d %.8f %.8f %.8f %.8f lower limit", u, p1, lower, p2, lower2);
    (*g_print)(sbuf);
    LOCATE(r + 8, 9);
