...
static int kirjoita(char *rivi) {
    ...
    if (*filter1) {
        koodimuunto(rivi);
        koodimuunto(skip);
    } /* 13.11.92 */ // RS ADD koodimuunto(skip);
    else if (*encoding) {
        muste_iconv(rivi, encoding, "CP850");
        muste_iconv(skip, encoding, "CP850");
    } // RS ADD
    else if (!tulosrivi) {
        muste_iconv(rivi, "", "CP850"); // RS ADD
    }
...
static int kirjoita2(char *rivi) /* 22.1.1996  also control characters in formatted list */
{
    if (*filter1) {
        ...
        muste_iconv(rivi + 1, encoding, "CP850");
        muste_iconv(skip, encoding, "CP850");
    } // RS ADD
      //        if (*filter1) koodimuunto(rivi+1);
      //        else if (*encoding) muste_iconv(rivi+1,encoding,"CP850"); // RS ADD
    else if (!tulosrivi) {
        muste_iconv(rivi + 1, "", "CP850"); // RS ADD
    }

    if (space_char != ' ') {
        space_muunto(rivi + 1); // RS ADD
    }
    if (nskip) {
        skip_char(rivi + 1, skip); // RS ADD
    }
...
static int format_vec(char *nimi, char *txt) {
    ...
    muste_sprintf(sbuf, "\nLoading observations from file %s: ", nimi);
    sur_print(sbuf);
    m = d.m_act;
    for (i = 0; i < m; ++i) {
        vi = d.v[i];
...
static int etsi_muodot() {
    ...
    *formtila = EOS;
    ...
    for (i = 0; i < m; ++i) {
        ...
        p = muoto;
        form[i] = pf;
        if (pf - formtila > maxtila - strlen(p) - 1) {
            ltilavirhe();
            return (-1);
        }
        while (*p) {
...
static void format_load(char *nimi) {
    ...
    for (j = d.l1; j <= d.l2; ++j) {
        ...
        for (k = 0; k < nf; ++k) {
            ...
            while (i < m && frivi[i] == k) {
                ...
                if (d.vartype[vi][0] == 'S') {
                    ...
                    if (a == MISSING8) {
                        ...
                        for (g = 0; g < flen[i]; ++g) {
                            muoto[g] = privi[k][fpos[i] + g];
                        }
                        muoto[flen[i]] = EOS;
                        fconv(a, muoto, sn);
                    }
                }
                for (g = 0; g < strlen(sn); ++g) {
                    y[fpos[i] + g] = sn[g];
                }
                ++i;
            } /* while i */
            ...
            if (*y == '*') {
                g = kirjoita(y + 1); /* 6.6.1996 */
            } else {
                g = kirjoita2(y);
            }
            if (g < 0) {
                return; /* 22.1.1996 kirjoita->kirjoita2 */
            }
        }
    }
}
...
static int find_limits(double min, double max) {
    ...
    for (i = 0; i < m; ++i) {
        ...
        maxx[i] = max;
        *name = EOS;
        strncat(name, d.varname[d.v[i]], 8);
        p = name + 7;
        while (*p == ' ') {
            *p-- = EOS;
        }
...
static int list_names_of_variables(int ii) {
    ...
    min = atof(s[1]);
    max = atof(s[2]);
    i = find_limits(min, max);
    if (i < 0) {
        return (1); // RS CHA exit(0); -> return(1);
    }
...
static void load_names_to_textfile() {
    ...
    if (i >= 0) {
        ...
        if (*sbuf == EOS) {
            ...
            i = split(sbuf, ww, 2);
            if (muste_strcmpi(ww[0], "TAB") == 0) {
                limit_char = '\t';
            } else if (muste_strnicmp(ww[0], "SP", 3) == 0) {
                limit_char = ' ';
            } else if (muste_strnicmp(ww[0], "char(", 5) == 0) {
                limit_char = atoi(ww[0] + 5);
            } else {
                limit_char = *ww[0];
            }
            if (i > 1) {
                no_last_limit = 0; /* 0=myîs viimeisen perÑÑn */
            }
...
void muste_file_load(int argc, char *argv[]) {
    ...
    strcpy(str_comma, "");
    strcpy(str_space, ""); // RS 11.3.2013
    i = spfind("STR_COMMA");
    if (i >= 0) {
        strcpy(str_comma, spb[i]);
    }
    i = spfind("STR_SPACE");
    if (i >= 0) {
        strcpy(str_space, spb[i]);
    }

    *filter1 = EOS; /* 13.11.1992 */
    i = spfind("FILTER");
    if (i >= 0) {
        strcpy(filter1, spb[i]);
    }
    if (*filter1) {
        i = load_codes(filter1, code);
        if (i < 0) {
            ste();
            data_close(&d);
            return;
    ...
    *encoding = EOS; // RS ADD
    ...
    if (*encoding) {
        strcpy(rivi, "Survo R");
        i = muste_iconv(rivi, encoding, "CP850");
        if (i <= 0) {
            return;
        }
    } // RS 2.5.2013
    ...
    if (i >= 0) {
        ...
        if (*sbuf == EOS) {
            ...
            limit_char = ','; // 26.11.2006
        } else {
            i = split(sbuf, ww, 2);
            if (muste_strcmpi(ww[0], "TAB") == 0) {
                limit_char = '\t';
            } else if (muste_strnicmp(ww[0], "SP", 2) == 0) {
                limit_char = ' ';
            } else if (muste_strnicmp(ww[0], "char(", 5) == 0) {
                limit_char = atoi(ww[0] + 5);
            } else {
    ...
    if (k - 1 > leveys) {
        if (tulosrivi) {
            ...
            muste_sprintf(sbuf, "\nThe %d active fields of %s require at least %d positions.", m, word[2], k - 1);
            sur_print(sbuf);
            WAIT;
            ste();
            data_close(&d);
    ...
    if (!jatko || printnames) // RS ADD printnames
    {
        ...
        for (i = 0; i < m; ++i) {
            ...
            if (i == (m - 1)) {
                rivi[limit_pos[i] + (1 - no_last_limit)] = EOS; // RS ADD
            }
        }
        h = kirjoita(rivi);
        if (h < 0) {
        ...
        if (printnames) {
            ...
            if (tulosrivi > 1) {
                *(z + (tulosrivi - 2) * ed1) = label[2];
            }
            if (tulosrivi > 0 && tulosrivi <= r2) {
                *(z + (tulosrivi - 1) * ed1) = label[0];
            }
        }
    }
    ...
    for (j = d.l1; j <= d.l2; ++j) {
        ...
        for (i = 0; i < m; ++i) {
            ...
            if (d.vartype[vi][0] == 'S') {
                ...
                /* 9.11.2002 */ if (*str_comma) {
                    str_korvaus(sana, ',', *str_comma);
                }
                if (*str_space) {
                    str_korvaus(sana, ' ', *str_space);
                }
            } else {
                fi_load(&d.d2, j, vi, &x);
                if (x == MISSING8) {
                ...
                if (strlen(sana) > len[i]) {
                    muste_sprintf(sbuf, "\nFormat %s not wide enough for %g in variable %.8s!", form[i], x,
                                  d.varname[vi]);
                    sur_print(sbuf);
    ...
    if (!jatko && tulosrivi > 0) {
        *(z + (tulosrivi - 2) * ed1) = label[1];
    }
    ste(); // RS CHA if (!tulosrivi) muste_fclose(tekstit);
    data_close(&d);
    s_end(argv[1]);
