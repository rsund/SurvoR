...
static void format_list(char *nimi)
        ...
        if (strlen(y)+3+1>lev) { k=kirjoita(y); if (k<0) { return; 
}*y=EOS; }
        strcat(y,"END");
        kirjoita(y);
        }
...
}if (g<0) { return;  /* 22.1.1996 kirjoita->kirjoita2 */
}
                }
...
tekstit=NULL;
        ...
        *encoding=EOS; // RS ADD
        ...
        if (*encoding) { strcpy(rivi,"Survo R"); i=muste_iconv(rivi,encoding,"CP850"); if (i<=0) { return; 
}} // RS 2.5.2013
