...
//
static char *soft_key[] = {"NEXT P", "PREV P", "NEXT C", "PREV C", "NEXT D", "PREV D",
                           "LAST",   "HOME",   "SEARCH", "SOUND",  "SAVE",   " EXIT "};
static int soft_start[] = {1, 8, 15, 22, 29, 36, 43, 48, 53, 60, 66, 71};
static int soft_len[] = {6, 6, 6, 6, 6, 6, 4, 4, 6, 5, 4, 6};
static int soft_code[] = {CODE_NEXT, CODE_PREV, 145,       146,  149,         150,
                          -3,        CODE_HOME, CODE_SRCH, 1001, CODE_SUCRO1, CODE_EXIT};
...
static char *soft_message[2][N_SOFT] = {
    ...
    {// RS ADD
     ...
     "Next page (PgDn)", "Previous page (PgUp)", "Next case (ctrl-PgDn)", "Previous case (ctrl-PgUp)",
     "Next case with a different value (alt-PgDn)", "Previous case with a different value (alt-PgUp)",
     "To the last case! (F2 down)", "Stepwise to the beginning! (HOME)", "Searching for the cases (alt-F5)",
     "Sound signals on/off! (F2 F7)", "Save this page in an edit file! (F11)", "Stop using FILE MEDIT! (F8)"} // RS ADD
};
...
static int soft_bottom_line_erase_medit(void);
static int tilanvaraus(void);
static int not_enough_space(void);
static int read_medit_spec(void);
static int etsi_alkuhav(long *pjnro);
static int check_set(char *x);
static int field_update1(void);
static int field_update(void);
static int check_compare(int field, char *new);
static int find_pages(void);
static int show_page(void);
static int show_page2(void);
...
static int lopeta_toinen_survo(void);
...
static int find_field_by_mouse(void);
static int replace_value_by_mouse_word(void);
static int goto_next_field(void);
static int goto_previous_field(void);
static int select_next_page(void);
static int next_ehto(char *s);
static int new_jnro(int k);
static int goto_next_field2(void);
static int create_new_case(void);
static int goto_last_case(void);
static int start_editing(void);
static int delete_char(void);
static int erase_chars(void);
static int field_down(void);
static int field_up(void);
static int find_classifications(void);
...
static int update_time_var(void);
...
static int alusta_stat_tilat(void);
...
static int save_open(void);
static int save_page(void);
static int save_pages(void);
static int medit_save_end(void);
static int key_common(int m);
static int ei_saa_editoida(void);
static int prefix(void);
static int n_update(SURVO_DATA_FILE *s, long n);
static int medit_cls(void);
static void sur_set_medit_focus(void);
static int varinfo(void);
...
static void var2(void);
static void spec_error(void);
static void poista_var(void);
static int update_specs(void);
static int var_error(char *s);
static int varfind2_medit(SURVO_DATA *d, char *nimi, int virheilm);
int headline_medit(void);
static int nextch_medit(void);
static int is_special(int m);
static int nextkey(void);
static int soft_keys_init_medit(void);
...
int muste_file_medit(int argc, char *argv[]) {
    ...
    i = data_open2(data_name, &d, 1, 1, 0);
    if (i < 0) {
        return (0);
    }
    ...
    if (i < 0) {
        strcpy(default_format, "###.##");
    } else {
        strcpy(default_format, spb[i]);
    }
...
static int check_set(char *x) {
    ...
    for (i = 0; i < k; ++i) {
        ...
        if (var < 0) {
            if (kieli == 1) {
                muste_sprintf(sbuf, "\nCHECK-muuttujaa %s ei loydy datatiedostosta!", s[i]);
            } else {
                muste_sprintf(sbuf, "\nCHECK field %s not available in the data file!", s[i]);
            }
            cursor_medit(1, 0);
            PR_EBLK;
            sur_print(sbuf);
...
static int field_update() {
    ...
    if (d.vartype[var0][0] == 'S') {
        ...
        for (i = 0; i < len; ++i) {
            if (field_value[old_field][i] != ' ') {
                break;
            }
        }
        if (i == len) {
            fi_miss_save(&d.d2, jnro, var0);
        } else {
            a = atof(field_value[old_field]);
...
static int check_compare(int field, char *new) {
    ...
    switch (type) {
    case 'S':
        if (strcmp(old_value, new) != 0) {
            ok = 0;
        }
        break;
    case '1':
    case '2':
        if (atoi(old_value) != atoi(new)) {
            ok = 0;
        }
        break;
    case '4':
    case '8':
        ...
        if (fabs(u - v) / (fabs(u) + fabs(v)) > 1e-7) {
            ok = 0;
        }
        break;
    }
    ...
    editing = 0;
    *var_name = EOS;
    strncat(var_name, d.varname[var], 8);
    i = 7;
    while (var_name[i] == ' ') {
        var_name[i--] = EOS;
    }
...
static int find_pages() {
    ...
    *p = EOS;
    ...
    for (k = 0; k < n_pages; ++k) {
        strcpy(name, page_name[k]);
        strcat(name, ":");
        for (j = 1; j < ed2; ++j) {
            edread(x, j);
            i = split(x + 1, s, 2);
            if (strcmp(s[0], "PAGE") != 0) {
                continue;
            }
            if (i > 1) {
                if (strcmp(s[1], name) == 0) {
                    break; // RS 28.1.2013 if i>1
                }
            }
        }
        if (j == ed2) {
    ...
    while (1) {
        ...
        *medit_sucro = EOS;
        ...
        if (*medit_condition == EOS && *medit_sucro == EOS) // 10.7.2003
        {
            i = split(p, s, 1);
            if (i == 1) {
                strcpy(medit_condition, s[0]);
            }
        }
        if (*medit_condition == EOS) {
            break;
        }
...
static int show_page2() {
    ...
    while (1) {
        ...
        strcpy(xss, xs); // 26.7.2003
                         // Rprintf("\n%.50s",x+1);
        i = split(x + 1, s, 1);
        if (i == 1 && strcmp(s[0], "END") == 0) {
            break; // tÑydennÑ tyhjÑt (puuttuu!)
        }
        strcpy(x, x2);
        ...
        if (i == 2) {
            ++riv; // luettu %1-rivi, jonka loppu seuraavalla
        }
        ...
        {
            write_string(x + 1, c3 - 1, rivivarjo, riv2 + 1, 1);
            rivivarjo = ' ';
        } else {
            varjo = xs[1];
            v = 1;
        ...
        if (*sound_name) {
            play_sound_by_name(sound_name, riv2, 0);
        }
    }
    ...
    if (*medit_sucro != EOS) {
        ...
        if (sucro2 == 0) {
            muste_sprintf(sbuf, "\nData lines <START> not given for sucro %s!", medit_sucro);
            PR_EIN2;
            sur_print(sbuf);
            WAIT;
            return (-1);
...
static int fill_fields(char *x, char *xs, int riv) {
    ...
    q = strstr(x, " & ");
    if (p != NULL && q != NULL) { // TehdÑÑn jatko seuraavasta rivistÑ!
        *(q + 1) = '/';
        *(q + 3) = EOS;
        ...
        if (var < 0) {
            *class_string = EOS;
            ...
            //          else fconv(value,"",sbuf2);
                ...
                fconv(value, "", sbuf2); // 7.3.2005
            }
            if (*class_string != EOS) {
                strcpy(sbuf2, class_string);
                *class_string = EOS;
            }
        } else {
...
static int read_var_free(int var, char *s, double *pvalue) {
    ...
    if (d.vartype[var][0] == 'S') {
        data_alpha_load(&d, jnro + (long)lag, var, s);
        lag = 0;
        i = strlen(s) - 1;
        while (s[i] == ' ') {
            s[i--] = EOS;
        ...
        data_load(&d, jnro + (long)lag, var, &a);
        lag = 0;
        *pvalue = a; // 17.5.2003
        if (a == MISSING8) {
            strcpy(s, "?");
...
static int find_field_by_mouse() {
    ...
    for (i = 0; i < n_fields; ++i) {
        ...
        if (cc >= field_column[i] && cc < field_column[i] + field_len[i]) {
            break;
        }
    }
    if (i == n_fields) {
        if (n_fields2 && right_mouse_click) {
            for (i = 0; i < n_fields2; ++i) {
                ...
                if (cc >= field_column2[i] && cc < field_column2[i] + field_len2[i]) {
                    break;
                }
            }
            if (i < n_fields2) {
...
static int select_next_page() {
    ...
    while (1) {
        ...
        if (muste_strcmpi(s[0], "END") == 0) {
            break;
        }

        i = next_ehto(s[0]);
        if (i < 0) { // SELECT-tyyppinen
            strcpy(cond, s[0]);
            // RS REM            sel_free();
...
static int erase_chars() {
    ...
    i = c - field_column[current_field];
    *sbuf = EOS;
    strncat(sbuf, space, field_len[current_field] - i);
...
static int field_down() {
    ...
    for (i = current_field + 1; i < n_fields; ++i) {
        if (field_line[i] == field_line[current_field]) {
            continue;
        }
        ...
        d2 = c - field_column[i] - field_len[i] + 1;
        if (d2 < 0) {
            d2 = -d2;
        }
        if (d2 < d1) {
...
static int find_classifications() {
    ...
    for (j = 1; j <= r2; ++j) {
        edread(x, j);
        i = split(x + 1, s, 1);
        if (strcmp(s[0], "CLASSIFICATION") != 0) {
            continue;
        }
        edread(x, j);
        i = split(x + 1, s, 2);
...
static int class_function(char *lauseke) {
    ...
    for (i = 0; i < n_classif; ++i) {
        if (strcmp(s[0], classif_name[i]) == 0) {
            break;
        }
    }
    if (i == n_classif) {
    ...
    i = strlen(class_string) - 1;
    while (i > 0 && class_string[i] == ' ') {
        class_string[i--] = EOS;
    }
...
static int update_time_var() {
    ...
    if (d.d2.vartype[time_var][2] == 'P') {
        return (1);
    }

    pvm();
    muste_sprintf(s, "%.4d-%.2d-%.2d %.2d:%.2d:%.2d", aika->tm_year + 1900, aika->tm_mon + 1, aika->tm_mday,
...
static int stddev_function(char *name, double *py) {
    ...
    if (n_mean[i] < 2L) {
        ...
        *py = sqrt((sum2[i] - sum[i] * sum[i] / (double)n_mean[i]) / (n_mean[i] - 1.0));
    }
    return (1);
}
...
static int key_special_medit(int m) {
    ...
    case CODE_REF:
        if (ref_jnro == 0L) {
            ...
            if (ref_jnro == jnro && ref_page == page_nro && ref_field == current_field) {
                ref_jnro = 0L;
                break;
            } else {
                field_update();
...
static int save_open() {
    ...
    if (*save_edt == EOS) {
        ...
        if (savefile != NULL) {
            ...
            fprintf(savefile, "SURVO 98 edit field:     %d         %d           10 %s", ed1, n_lines, rivin_loppu2);
...
static void sur_set_medit_focus() { muste_fixme("\nFIXME: sur_set_medit_focus() STUB!"); }
/* RS REM
    HWND hwnd;
...
static int hae_muoto(SURVO_DATA_FILE *d, int i, char *muoto) {
    ...
    while (1) {
        ...
        if (p == NULL) {
            ...
            muste_fieldcopy(muoto, p + 1, q - p - 1);
            muoto[q - p - 1] = EOS;
...
static int check_limits(int muuttuja, double x) {
    ...
    if (x < min[muuttuja] || x > max[muuttuja]) {
        ...
        PR_EBLD;
        if (x < min[muuttuja]) {
            muste_sprintf(sbuf, "%.8s < %g (smallest permitted value)", d.varname[muuttuja], min[muuttuja]);
        } else {
            muste_sprintf(sbuf, "%.8s > %g (greatest permitted value)", d.varname[muuttuja], max[muuttuja]);
        }
...
static int check_strarvot(int muuttuja, char *edsana) {
    ...
    *p = EOS;
    ...
    if (m) {
        for (i = 0; i < m; ++i) {
            if (strncmp(edsana, sana[i], len) == 0) {
                break;
            }
        }
        if (i == m) {
            ...
            sur_locate(r3, 1);
            PR_EBLD;
            muste_sprintf(sbuf, "%s is not a permitted value for %.8s", edsana, d.varname[muuttuja]);
            strcat(sbuf, "  Press any key!");
            play_sound(3);
            for (i = 0; i < 5; ++i) {
                soft_bottom_line_erase_medit();
...
static int play_sound(int i) {
    ...
    if (strncmp(medit_sound[i], "NULL", 4) == 0) {
        return (1);
    }
    ...
    *sbuf = '*';
    ...
    for (k = 0; k < n_pages; ++k) {
        ...
        for (i = 0; i < m_page; ++i) {
            ...
            if (type != 'S') {
                chei = get_format(type, x, d.v[var], format);
                if (chei < 0) {
                    return (-1);
                }
            } else {
...
static int etsi() {
    ...
    if (field2_haku) {
        ...
        var = field_var[current_field];
    }

    osahaku = 0;
    if (*hakuavain == '*') {
        osahaku = 1;
        strcpy(arvo, hakuavain + 1);
    }
    hav = atol(hakuavain);
    if (hav > 0L && hav <= n) {
        jnro = jnro0 = hav;
    ...
    if (*hakuavain == EOS) {
        rel = 'M'; /* searching for missing value */
    } else {
    ...
    **************************/
    ...
    while (1) {
        ...
        if (rel == ' ') {
            ...
            if ((!osahaku && strncmp(hakusana, arvo, len) == 0) || (osahaku && strstr(hakusana, arvo) != NULL)) {
                ++n_haku;
                jnro = jnro0 = hav;
                // Rprintf("\n%d|",jnro); getck();
                if (jatkuva_haku) {
            ...
            cursor_medit(r3 - 1, 69);
            PR_EBLD;
            muste_sprintf(sbuf, "%ld", hav);
            sur_print(sbuf);
            if (sur_kbhit()) {
                i = sur_getch();
                if (i == '.') {
...
static int poimi(long j, int i, char *sana) {
    ...
    pit = d.varlen[i];
    if (j == n + 1) {
        muste_fieldcopy(sana, space, pit);
        sana[pit] = EOS;
        return (1);
    }
    if (type == 'S') {
        ...
        case '2':
            ...
            break;
        case '4':
            if (not_float((unsigned char *)sana)) {
                miss = 1;
                break;
            }
            luku4 = *(float *)sana;
            if (luku4 > MISSING4 / 10.0) {
                miss = 1;
                break;
            }
            ...
            break;
        case '8':
            if (not_double((unsigned char *)sana)) {
                miss = 1;
                break;
            }
            luku = *(double *)sana;
            if (luku > MISSING8 / 10.0) {
                miss = 1;
                break;
            }
            fconv(luku, "", sana);
...
static int laske2(char *muuttuja, double *y) {
    ...
    // muutettu    :                   :
    {
        i = sup_arvo_double(muuttuja, y);
        return (i);
    }
    if (lag) {
        i = lag_arvo(muuttuja, y);
        lag = 0;
        if (i < 0) {
            l_virhe = 1;
        }
...
static int f_edit(char *s, double *x, int n, double *py) {
    ...
    ***************************************************/
    ...
    while (i < spn &&
           (spa[i] == NULL || *spa[i] == EOS || spa[i][strlen(spa[i]) - 1] != ':' || strncmp(s, spa[i], len) != 0)) {
        ++i;
    }
...
static int sp2_init() {
    ...
    i = sp_write("MISSING", MISSING8);
    if (i < 0) {
        return (-1);
    }
    i = sp_write("ORDER", MISSING8);
    if (i < 0) {
        return (-1);
    }
    spn_order = spn - 1;
    i = sp_write("N", (double)d.n);
    if (i < 0) {
        return (-1);
    }
    return (1);
...
static int pos_funktio(char *s, double *y) /* pos(var,char) tai pos(var,start_pos,char) */
{
    ...
    strcpy(x2, osa[i - 1]);
    if (muste_strcmpi(x2, "sp") == 0 || muste_strcmpi(x2, "space") == 0) {
        *x2 = ' ';
        x2[1] = EOS;
    }
    if (muste_strcmpi(x2, "comma") == 0) {
        *x2 = ',';
        x2[1] = EOS;
    }
    ...
    while (*p) {
        if (*p == '"') {
            ...
            *q = EOS;
            ...
            muste_fieldcopy(sana, sana2 + start - 1, len);
            sana[len] = EOS;
...
static int str_arvo(char *a, char *s) {
    ...
    if (*a == '"') {
        ...
        muste_fieldcopy(s, x + str_var_start - 1, str_var_len);
        s[str_var_len] = EOS;
    }
    if (code_ind == 2) {
        conv((unsigned char *)s); // RS ADD (unsigned char *)
    }
    return (1);
}
...
static void conv(unsigned char *sana) {
    ...
    for (i = 0; i < strlen((char *)sana); ++i) {
        sana[i] = code[sana[i]]; // RS ADD (char *)
    }
}
...
static int sup_arvo(char *muuttuja, char *s) {
    ...
    p = strchr(muuttuja, ':');
    if (p == NULL) {
        muste_sprintf(sbuf, "Error in %s", muuttuja);
        PR_EBLK;
        sur_print(sbuf);
        WAIT;
        return (-1);
...
static void spec_error() {
    PR_EBLK;
    muste_sprintf(sbuf, "\nToo many active variables + specifications (specmax=%d)", specmax);
    sur_print(sbuf);
...
int mouse_medit_functions(int c_mouse, int r_mouse, int m_click, int m_double_click) {
    ...
    if (m_double_click) {
        ...
        if (r == r3 - 2) {
            if (c > c3 - 7) {
                return (-1); // RS FIXME ??? CHA exit(0); -> return(-1); // tilapainen STOP
            }
        }
        special = 1;
        return (CODE_EXEC);
    ...
    if (r_mouse == r3 - 2) {
        ...
        special = 1;
        for (i = 0; i < n_soft_keys; ++i) {
            if (cc >= soft_start[i] && cc < soft_start[i] + soft_len[i]) {
                break;
            }
        }

        if (i == current_soft_key) {
            return (-1);
        }
        if (i == n_soft_keys) {
            soft_bottom_line_erase_medit();
...
static int spread2_medit(int lin, int *raja1) {
    ...
    *raja1 = j;
    for (j = *raja1 + 1; j <= ed2; ++j) {
        ...
        if (j == r1 + r - 1) {
            continue; /* aktivoidun rivin ohittaminen */
        }
        spn = spread3_medit(x, j);
        if (spn < 0) {
            return (spn);
        }
    }
...
static int varnimet_medit() {
    ...
    for (i = 0; i < d.m; ++i) {
        //          *nimi=EOS; strncat(nimi,d.varname[d.v[i]],8); // 22.5.2003
        *nimi = EOS;
        strncat(nimi, d.varname[i], 8); // 22.5.2003
        k = strlen(nimi);
        while (nimi[k - 1] == ' ') {
            nimi[--k] = EOS;
        }
