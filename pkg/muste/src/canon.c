...
void muste_canon(char *argv) {
    ...
    muste_free(A);
    muste_free(rlab);
    muste_free(clab); /* ei saa k?ytt?? en?? */
    i = mat_oper();
    if (i < 0) {
        return;
    }
...
static int tee_vxyz(int *v, char *s) {
    ...
    n = 0;
    for (i = 0; i < dat.m; ++i) {
        if (strchr(s, dat.vartype[i][1]) != NULL) {
            v[n++] = i;
        }
    }
    return (n);
...
static int mat_oper() {
    ...
    i = matrix_space(&T, nX, 1, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    ...
    i = matrix_space(&T, nX, nX, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    i = mat_mtm(T, X, mX, nX);
    ...
    i = matrix_space(&Y, mX, nX, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    i = mat_gram_schmidt(Y, T, X, mX, nX, 1e-15);
    if (i <= 0) {
        muste_sprintf(sbuf, "\nX variable %.8s linearly dependent on the previous ones!", rlabX - i * lrX);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
    ...
    i = mat_gram_schmidt(Y, T, X, mX, nX, 1e-15);
    if (i <= 0) {
        muste_sprintf(sbuf, "\nY variable %.8s linearly dependent on the previous ones!", rlabX - i * lrX);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
    ...
    i = matrix_space(&T, m1, m2, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    ...
    i = matrix_space(&X, m1, m2, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    mat_copy(X, T, m1, m2);
    i = matrix_space(&Y, m2, m2, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    i = matrix_space(&T, m2, 1, &rlabT, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    ...
    i = matrix_space(&S, m1 + 1, m2, &rlabS, &clabS, 8, 8);
    if (i < 0) {
        return (-1);
    }
    ...
    *mnimi = cx;
    ...
    load_Y("&UY");
    load_X("&V");
    i = matrix_space(&T, m2, m2, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    i = solve_upper(T, Y, X, m2, m2, 1e-15);
    i = matrix_space(&S, m2 + 1, m2, &rlabS, &clabS, 8, 8);
    if (i < 0) {
        return (-1);
    }
    muste_fieldcopy(rlabS, "Constant", 8);
...
static int zregress() {
    ...
    if (i <= 0) {
        muste_sprintf(sbuf, "\nZ variable %.8s linearly dependent on the previous ones!", rlabX - i * lrX);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
    ...
    i = matrix_space(&T, m1, 1, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    i = mat_center(T, X, n, m1);
    i = matrix_space(&T, m3, m1, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    ...
    load_Y("&X");
    i = matrix_space(&T, n, m1, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    mat_sub(T, Y, X, n, m1);
    ...
    i = matrix_space(&T, m2, 1, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    i = mat_center(T, X, n, m2);
    i = matrix_space(&T, m3, m2, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    ...
    load_Y("&Y");
    i = matrix_space(&T, n, m2, NULL, NULL, 8, 8);
    if (i < 0) {
        return (-1);
    }
    mat_sub(T, Y, X, n, m2);
...
static int tulostus() {
    ...
    if (m3) {
        ...
        for (i = 0; i < m3; ++i) {
            ...
            muste_fieldcopy(ots, clab0 + i * lc, lc);
            ots[lc] = EOS;
            h = strlen(ots);
            while (ots[h - 1] == ' ' && h > 0) {
                ots[--h] = EOS;
    ...
    results_line = matrix_print(A0, m, nn, rlab0, clab0, lr, lc, m, nn, NULL, NULL, "12.123", c3, results_line, eout,
