...
int output_open(char *file) {
    ...
    if (output_file == NULL) {
        muste_sprintf(sbuf, "\nCannot open output file/device %s", file);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
int output_line(char *string, char *file, unsigned int editline) {
    ...
    if (output_level == 2) {
        ...
        muste_sprintf(sbuf, "\n%s", s);
        if (prind) {
            sur_print(sbuf); // RS 17.5.2013 prind
        }
    }
    if (file == NULL || *file == EOS) {
        return (1);
...
int wait_remarks(int k) {
    ...
    if (muste_remarks_return) {
        return (-1);
    }
    sur_print("\n");
    PR_EINV;
    if (k == 1) {
        sur_print("Next page by 'space'  |  Load lines by '+'  |  Interrupt by ENTER!");
    } else {
        sur_print("Load lines by '+'  |  Interrupt by ENTER!");
    }
    /*  m=nextch("");    */
    m = sur_getch();
    switch (m) {
