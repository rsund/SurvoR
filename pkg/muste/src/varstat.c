...
char **specs;
*********************/
static char *taskname[] = {"SUM",          "SUM1",   "SUM2",   "SUM3",   "SUM4",     "MEAN",     "STDDEV",
                           "MAX",          "MIN",    "RANGE",  "MEDIAN", "#MISS",    "#N",       "#VAL",
                           "#POS_SIDE",    "FIRST",  "LAST",   "CYCLEN", "#MODEVAL", "#SEQ",     "#RUN",
                           "#RUN_UP_DOWN", "DIVSUM", "DIVMAX", "NORM",   "CENTER",   "STANDARD", "MLT",
                           "DIV",          "ADD",    "SUB",    "SORT",   "CUM",      ""};
...
void muste_varstat(char *argv) {
    ...
    *****************/
    ...
    if (i < 0) {
        ...
        data_close(&d);
        return;
    } // RS 26.3.2013 data_reduce+close
    prind = 0;
...
static int tasks() {
    ...
    if (g < 3) {
        ...
        for (k = 0; k < ntask; ++k) {
            ...
            if (i < 0) {
                muste_sprintf(sbuf, "Task of variable %s is not given as %s=<task> !", osa[k], osa[k]);
                sur_print(sbuf);
                WAIT;
                return (-1);
            }
    ...
    if (!collective) {
        ...
        strcpy(aritvar, word[4]); /* VARSTAT <data>,*,/,<aritvar>  */
    }
    ...
    if (strcmp(str_tp1, "STR_VAL_LIST") == 0 || strcmp(str_tp1, "SUBSTR_VAL_LIST") == 0) {
...
static int create_str_val_list(char type) // SM ADD 22.5.2012
{
    ...
    if (type == 'T') {
        i = spfind("STR_VAL_LIST");
    } else if (type == 'U') {
        i = spfind("SUBSTR_VAL_LIST");
    }
    if (i < 0) {
...
static int taskfind(char *s) {
    ...
    i = 0;
    while (*taskname[i] != EOS && muste_strcmpi(taskname[i], s) != 0) {
        ++i;
    }
    if (*taskname[i] == EOS) {
        muste_sprintf(sbuf, "\nUnknown task name %s !", s);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static double xmean(int it) {
    ...
    if (pot == 2.0) {
        return (sqrt(b / (double)m1));
    }
    if (pot == 0.0) {
        return (exp(b / (double)m1));
    }
    ...
    if (str_val_type) {
        ...
        for (j = 0; j < m; ++j) {
            for (i = 0; i < n_list; ++i) {
                ...
                if (str_val_type == 1) {
                    if (strcmp(str_x[j], str_val_list[i]) == 0) {
                        ++n;
                    }
                } else {
                    if (strstr(str_x[j], str_val_list[i]) != NULL) {
                        ++n;
                    }
                    // Rprintf("\nn=%d",n);
                }
