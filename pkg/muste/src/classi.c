...
static int select_output_vars(void);
static int matrix_names(void);
...
static int select_var_post(void);
static int read_matrices(void);
static int select_active_vars(void);
static int priors(void);
...
void muste_classi(char *argv) {
    ...
    s_init(argv);
    /*Rprintf("info=%s\n",info); getch(); */
    if (muste_strcmpi(word[0], "MAHAL") == 0) {
        op_mahal();
        s_end(argv);
        return;
    }
...
static int matrix_names() {
    ...
    if (i != gg) {
        muste_sprintf(sbuf, "\n# of MSN matrices (%d) does not equal to # (%d) of CORR matrices!", i, gg);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int select_var_post() {
    ...
    if (np != gg) {
        muste_sprintf(sbuf, "\n# (%d) for posterior probability (distance) variables", np);
        sur_print(sbuf);
...
static int read_matrices() {
    ...
    for (k = 0; k < gg; ++k) {
        ...
        if (m != p || n != 3) {
            muste_sprintf(sbuf, "\nIncompatible dimensions (%d,%d) in matrix file %s", m, n, msn_name[k]);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
        ...
        if (m != p || n != p) {
            muste_sprintf(sbuf, "\nIncompatible dimensions (%d,%d) in matrix file %s", m, n, corr_name[k]);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
...
static int select_active_vars() {
    ...
    if (i >= 0) {
        ...
        if (n != p) {
            muste_sprintf(sbuf, "\n# (%d) of columns in COEFF file not equal to %d!", n, p);
            sur_print(sbuf);
            WAIT;
        }
        muste_sprintf(given_in, " (given in COEFF file %s)", x);
    } else {
        coeff_given = 0;
        p1 = p;
        *given_in = EOS;
    ...
    for (i = 0; i < p1; ++i) {
        ...
        if (k < 0) {
            ...
            */
            muste_sprintf(sbuf, "\nVariable %s%s not found in data!", nimi, given_in);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
...
static int priors() {
    ...
    i = split(x, pg, gg);
    if (i != gg) {
        muste_sprintf(sbuf, "\n# of PRIORS (%d) not equal to # of groups (%d)", i, gg);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int classify() {
    ...
    for (k = 0; k < gg; ++k) {
        ...
        if (i < 0) {
            muste_sprintf(sbuf, "\nCovariance matrix of group %d is singular!", k);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
    ...
    for (l = d.l1; l <= d.l2; ++l) {
        ...
        if (var_mahal1 >= 0) {
            if (!miss) {
                i = select_group(d21, 1);
                arvo = (double)(i + 1);
            }
            data_save(&d, l, var_mahal1, arvo);
            if (post_type == 1) {
                save_post(d21, l, 0, miss);
            }
        }
        if (var_mahal2 >= 0) {
            if (!miss) {
                i = select_group(d22, 1);
                arvo = (double)(i + 1);
            }
            data_save(&d, l, var_mahal2, arvo);
            if (post_type == 2) {
                save_post(d22, l, 0, miss);
            }
        }
        if (var_bayes1 >= 0) {
            if (!miss) {
                for (k = 0; k < gg; ++k) {
                    xg[k] = prior[k] * exp(-d21[k] / 2);
                }
                i = select_group(xg, 2);
                arvo = (double)(i + 1);
            }
            ...
            if (post_type == 3) {
                save_post(xg, l, 1, miss);
            }
        }
        if (var_bayes2 >= 0) {
            if (!miss) {
                for (k = 0; k < gg; ++k) {
                    xg[k] = prior[k] / sqrt(det[k]) * exp(-d22[k] / 2);
                }
                i = select_group(xg, 2);
                arvo = (double)(i + 1);
            }
...
static int compute_wtot() {
    ...
    for (k = 0; k < gg; ++k) {
        for (i = 0; i < p * p; ++i) {
            wtot[i] += (nk[k] - 1L) * w[k][i];
        }
    }
    for (i = 0; i < p * p; ++i) {
        wtot[i] /= (double)(ntot - (int)gg);
    }
    return (1);
}
...
static void op_mahal() {
    ...
    laske_mahal();
    tulostus();
    muste_sprintf(sbuf, "Mahalanobis distance in %d variables", m_act);
    update_varname(&d, var_mahal, sbuf);
    muste_sprintf(sbuf, "P value for Mahalanobis distance in %d variables", m_act); // RS 9.4.2013
...
static int laske_mahal() {
    ...
    maxmahal = 0.0;
    jmaxmahal = 0; // RS 9.4.2013 jmaxmahal=0
    for (j = d.l1; j <= d.l2; ++j) {
        if (unsuitable(&d, j)) {
            continue;
        }
...
static int tulostus() {
    ...
    if (jmaxmahal > 0) // RS 9.4.2013
    {
        ...
        if (var_mahal >= 0) {
            i += muste_sprintf(x + i, "Mahalanobis D^2 in %s  ", d.varname[var_mahal]);
        }
        if (var_c2mahal >= 0) {
            i += muste_sprintf(x + i, "P values of D^2 in %s", d.varname[var_c2mahal]);
        }
        print_line(x);
        ...
        *u = EOS;
        ...
        if (d.vartype[0][0] == 'S') {
            ...
            muste_sprintf(u, " (%s)", spois(x));
        }
        muste_sprintf(x, "Max.distance=%s in obs. # %d%s", spois(y), jmaxmahal, u);
    } else {
        muste_sprintf(x, "Missing values in all selected observations of data: %s", word[1]); // RS 9.4.2013
    }
    print_line(x);
    output_close(eout);
    return (1);
