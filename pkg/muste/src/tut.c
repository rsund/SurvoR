...
char s_functions[] = "Sucro functions: E=End of definition, C=Control codes, R=Repeat ? ";
...
int s_tut_init() {
    ...
    if (etu == 2 || sucro_pause) {
        tutor = muste_fopen2(etufile, "rb");
    }
    if (tutor == NULL) {
        sur_print("Sucro error!");
        WAIT;
        return -1;
    } // RS ADD
    muste_fseek(tutor, (long)tutpos, 0);
    return (1);
}
    ...
    if (etu == 0) {
        ...
        /*
                    ...
                    if (apu!=muste_tut_stack_count)
                        {
                        muste_sprintf(sbuf,"\nFIXME: Stack count unbalance with sucro %d vs. %d
           !!!",apu,muste_tut_stack_count); sur_print(sbuf); WAIT; muste_fixme(sbuf);
...
void tutsave(int m) { putc(m, tutor); }
...
int sukrohaku() {
    ...
    for (i = 0; i < n; ++i) {
        ...
        /*
                    k=1;
                    while (*(x+k)==' ' || *(x+k)=='\n' || *(x+k)=='\r' || *(x+k)=='\t') ++k; // RS FIXME tässä ohi myös
        \r:stä, \t:stä ja \n:stä while(*(x+k)!=EOS) { Rprintf("%d",*(x+k)); k++; }
        ...
        */
        ...
        p = x + 1;
        while (*(x + k) == ' ' || *(x + k) == '\n' || *(x + k) == '\r' || *(x + k) == '\t') {
            ++k; // RS FIXME tässä ohi myös \r:stä, \t:stä ja \n:stä
        }
        while (*p != ' ') {
            ++p;
        }
        ...
        *p = EOS;
        ...
        if (muste_strcmpi(etusukro, (x + k)) == 0) {
            break; // RS 1 -> k
        }
    }
    if (i == n) {
...
int tutopen(char *name, char *mode) {
    ...
    *etusukro = EOS;
    ...
    if (p != NULL) {
        *p = EOS;
        if (*name == '*') {
            ...
            i = 1; /* i=0: previous etufile 23.1.93 */
        }
        strcpy(etusukro, p + 1);
        strcat(etusukro, " ");
    }
    if (i) {
        filename(etufile, name);
        file_name_ext(etufile, ".TUT");
    }
    ...
    if (*sucropath && i != 1) {
        i = tutopen2(name, mode, sucropath); /* 10.2.90 */
    }

    if (i != 1 && alkututor) {
        strcpy(x, muste_startpath);
...
int op_tutor() {
    ...
    if (i >= 0) {
        muste_debug = atoi(sbuf);
    }

    if (muste_debug) {
        muste_debug_print("op_tutor"); // RS 26.1.2013
    }
    ...
    if (ntut > 0) {
        ...
        if (muste_debug) // RS 26.1.2013
        {
            muste_sprintf(sbuf, "tut: %d, otutpos: %ld, otutmemd: %s, otutnimi: %s otutalku: %ld, otila=%d, otila2: %d",
                          ntut, tutpos, etusukro, etufile, tutalku, etu, etu2);
            muste_debug_print(sbuf);
        }

    } else {
        strcpy(error_handler, "SURVOERR"); /* 30.4.91 */
    }

    if (parm[0] != NULL && *parm[0] == '/') {
        if (*(parm[0] + 1) == ' ' || *(parm[0] + 1) == EOS) {
            return (1); // RS ADD
        }
        for (i = g; i > 0; --i) {
            parm[i] = parm[i - 1];
    ...
    // Rprintf("\nparm[0]: %s, parm[1]: %s",parm[1],parm[2]);
    del_ref = 1;
    if (g > 2) {
        if (strcmp(parm[2], "@") != 0) {
            *tut_info = EOS;
    ...
    if (i) {
        ...
        ntut = 0; /* ntut=0; 8.11.88 */ // RS 25.11.2012 erun=0
    }
    if (del_ref) {
        ref_c1 = 0; /* mahd. refer.piste unohdetaan */
    }
...
int stack_save_load(int k, char *nimi) //  k:  1=save 2=load
{
    ...
    p = tut_info;
    n = 0;
    while (!feof(edfield) && n < LLENGTH - 1) {
        ++n;
        *p = (char)getc(edfield);
        ++p;
    }
...
static void press_key(int m) {
    ...
    if (k == 2) {
        ch = CODE_EXEC; // mouse_click

    } else if (k) {
        ch = read_nextkey_editor();
    } else {
        ch = m; // RS 22.11.2012 read_ ; sur_wait2:ssa ei saa lukea näppäintä
    }
    ...
    while (ch != m && ch != CODE_LEFT) {
        ...
        if (ch == CODE_HOME) /* 26.4.90 */
        {
            ...
            cursor(r3 + 1, 1);
            PR_EUDL;
            muste_sprintf(sbuf, "%s-%s  ", etufile, etusukro);
            sur_print(sbuf);
            PR_ENRM;
            cursor(r, c);
            CURSOR_ON;
...
int mouse_key_select(int rr, int cc) {
    ...
    read_string(x, NULL, c3 + 8, rr + 1, 1);
    x[c3 + 8] = EOS; // RS Rivin luku suoraan näytöltä disp.c:ssä
                     //  Rprintf("\nx=%s|",x+cc); getck();
                     // Rprintf("\nx=%s|",x+cc);
...
int tutcat(char *s) {
    ...
    if (tut_index >= 1) {
        i = tut_index;
        tut_index = 0; /* tut_set() calls sometimes tutcat() */
...
int tut_set(char *sana, int i) {
    ...
    while (*p && *p != '@') {
        p++; // RS 4.2.2013 REM ch=*p++;
    }
    while (*p) {
        *q++ = *p++;
    }
    *q = EOS;
    if (strlen(x) > LLENGTH - 1) {
        sur_print("\nTutstack overflow!");
        WAIT;
        return (-1);
    }
...
void prefix_y() {
    ...
    if (ntut < 2 || etuu == 1) {
        ...
        if (tuttila[ntut - 2] == 1) {
            ...
            tut_sulje();
            tutpos2 = tutpos;
            tutor = muste_fopen2(tutnimi[ntut - 2], "r+b");
            if (tutor == NULL) {
                sur_print("Sucro error!");
                WAIT;
                return;
            ...
            tut_sulje();
            tutpos2 = tutpos;
            tutor = muste_fopen2(tutnimi[ntut - 2], "rb");
            if (tutor == NULL) {
                sur_print("Sucro error!");
                WAIT;
                return;
    ...
    muste_eventpeek = old_eventpeek; // RS 28.1.2013
...
A:
    ...
    m = getc(tutor);
    {
        while (m == TUT_COMMENT_CODE) {
            m = getc(tutor); /* 12.10.88 */
        }
        if (m == 10) {
            m = 13;
    ...
    if (!feof(tutor) && !tut_loppu) /* 13.10.88 */
    {
        ...
        if (sur_kbhit()) // RS Tämä ei saa päivittää tapahtumaa, jotta nextkey saa luettua sen
        {
            ...
            if (!tut_not_break2) {
                ...
                case ',':
                    sucro_pause = 1; // uusittu 21.8.2004
                    break;
                ...
                default:
                    ...
                    read_nextkey_editor();
                    break; /* 23.10.89 */ // RS 22.11.2012 read_  //RS Mitä tämä tekee???
                }
            }
        }
        special = 0;
        if (etu1 > 1 && !tut_special_code) {
            sur_wait((long)tut_wait_c * etu1, nop, 0);
        }
        ...
        if (etu2 > 0) {
            ...
            if (etu2 == 1) {
                if (etu1 > 1) {
                    sur_wait((long)(4 * tut_wait_c) * etu1, Wdisp_editor, 0);
                }
            } else {
                press_key(m);
            }
    ...
    *sana = EOS; /* 11.12.1998 */
    ...
    case 1:
        ...
        if (ch == 'E' || ch == 'e') {
            ...
            disp();
            if (*sana == 'X') {
                *sana = (char)TUT_EFFECTS_OFF;
                sana[1] = EOS;
                muste_fseek(tutor, (long)-2L, 1);
            }
    ...
    case 2:
        ...
        case 'W':
            ...
            if (etu1 > 1 && etu2 != 2) {
                ...
                } else if (survopoint_on) // 25.3.2012
                {
                    ...
                    sur_wait((long)(5 * tut_wait_c) * atoi(sana) * etu1, Wdisp_editor, 1);
                }
            } else {
                sur_sleep(10);
                Wdisp_editor();
        ...
        case 'V': /*  V<aika>@123@<G|L>jatko1@<G|L>jatko2@<G|L>jatko3@ */
            soft_disp(0);
            read_cond(sana);
            l = atol(sana) * (long)(10 * tut_wait_c);
            if (l == 0L) {
                l = (long)(6000L * tut_wait_c);
            }
            read_tutword(sana);
            if (ei_odotusta) {
                k = 0;
            } else {
                k = sur_wait2(l, Wdisp_editor);
            }
            if (k) {
                sucro_menu = 1;
                ch = 0;
        ...
        case 'O':
            i = strlen(survo_id);
            strcpy(sana, survo_id + i + 1);
            p = sana;
            while (*p) {
                *p -= 19;
                ++p;
            }
            strcat(tut_info, sana);
            strcat(tut_info, "@");
            break;
        ...
        case 'q': /* q<tut_word_index>@ */ /* 30.4.91 */
            ...
            sana[EC3 + 8] = EOS;
            strcpy(prompt_space, sana); /* 23.2.93 */
            if (*prompt_space) {
        ...
        case 'g': /* g<first_row>@<row>@<column>@ */
            ...
            parm[1] = sana;
            parm[2] = jatko;
            parm[3] = jatko2; // RS 22.1.2014 jatko+32 -> jatko2
            if (*parm[3] != '!') {
                op_goto2(4, parm);
            } else /* g<first_row>@<row>@C!@<first_column>@<column>@ */
            {
                ...
                read_cond(jatko3);
                parm[4] = jatko3; // RS 22.1.2014 jatko+64 -> jatko2
                op_goto2(5, parm);
            }
            disp();
            break;
