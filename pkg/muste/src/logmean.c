...
static int load_data() {
    ...
    if (min != max) {
        scale_factor = 2 / (max - min);
    } else {
        scale_factor = max;
    }
    if (method <= 0 || term_comp) {
        scale_factor = 1.0;
    }
...
static int comp2() {
    ...
    while (1) {
        ...
        muste_sprintf(sbuf, "\n%d: lmean=%16.16g", m, lmean);
        sur_print(sbuf);
        if (fabs(lmean - lmean1) == 0.0 || m >= powmax) {
            break;
        }
        lmean1 = lmean;
        ++m;
...
static int other_means() {
    ...
    mean /= (double)n;
    geom_mean = exp(geom_mean / (double)n);
