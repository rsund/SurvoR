...
static int too_many_var(char ch, int n) {
    muste_sprintf(sbuf, "\n# of '%c' variables (now %d) must be less than # of 'A' variables (%d)!", ch, n, m);
    sur_print(sbuf);
    WAIT;
    return (1);
}
...
static int sum_is_0(int var) {
    muste_sprintf(sbuf, "\nSum of values of variable '%.8s' is 0.", d.varname[var]);
    sur_print(sbuf);
    sur_print("\nCannot continue!");
    WAIT;
    return (1);
...
static int corresp() {
    ...
    i = mat_tred2(dd, ee, A, nn, tol);
    if (i < 0) {
        return (-1);
    }
    i = mat_tql2(dd, ee, A, nn, 1e-16, 30);
    if (i < 0) {
        return (-1);
    }
    /*
...
static int y_talletus() {
    ...
    tulosrivi = matrix_print(yy, m, nc, lab1, lab2, 8, 8, m, nc, NULL, NULL, acc, c3, tulosrivi, eout,
                             "Column coordinates (CR_COORD.M)");
    if (nu > 0) {
        tee_otsikot(lab, uv, nu);
        matrix_save("CR_V.M", A, m, nu, lab1, lab, 8, 8, -1, "CR_V.M", 0, 0);
        tulosrivi =
            matrix_print(A, m, nu, lab1, lab, 8, 8, m, nu, NULL, NULL, acc, c3, tulosrivi, eout, "V in SVD (CR_V.M)");
    }
    if (na > 0) {
        for (i = 0; i < nn; ++i) {
            for (j = 0; j < nn; ++j) {
        ...
        tulosrivi = matrix_print(ann, m, na, lab1, lab, 8, 8, m, na, NULL, NULL, acc, c3, tulosrivi, eout,
                                 "Absolute contributions % (CR_CONTR.M)");
    }
    if (ns > 0) {
        for (i = 0; i < nn; ++i) {
            a1 = 0.0;
            for (j = 0; j < nn; ++j) {
                a1 += yy[i + nn * j] * yy[i + nn * j];
            }
            for (j = 0; j < ns; ++j) {
                ann[i + nn * j] = yy[i + nn * j] * yy[i + nn * j] / a1;
            }
        }
        tee_otsikot(lab, sv, ns);
        matrix_save("CR_CORR2.M", ann, m, ns, lab1, lab, 8, 8, -1, "CR_CORR2.M", 0, 0);
        tulosrivi = matrix_print(ann, m, ns, lab1, lab, 8, 8, m, ns, NULL, NULL, acc, c3, tulosrivi, eout,
                                 "Squared correlations (CR_CORR2.M)");
    }
    if (mvar >= 0 || dvar >= 0) {
        for (i = 0; i < nn; ++i) {
            a1 = 0.0;
        ...
        tulosrivi = matrix_print(ann, m, 2, lab1, lab, 8, 8, m, 2, NULL, NULL, acc, c3, tulosrivi, eout,
                                 "Masses and distances from 0 (CR_MDIST.M)");
    }
...
static int x_talletus() {
    ...
    if (nss) {
        ...
        tulosrivi = matrix_print(ss, nss, nc, lab, lab2, 8, 8, nss, nc, NULL, NULL, acc, c3, tulosrivi, eout,
                                 "Column coordinates of supplementary variables (CR_SUPPC.M)");
    }
    ...
    for (l = d.l1; l <= d.l2; ++l) {
        if (cvar == -1) {
            if ((double)l < a1 || (double)l > a2) {
                continue;
            }
        } else {
            data_load(&d, l, cvar, &a3);
            if (a3 == MISSING8 || a3 < a1 || a3 > a2) {
                continue;
            }
        }
        x_tall(l);
...
static int tulostus1() {
    ...
    muste_sprintf(rivi, "Correspondence analysis on data %s: Rows=%d Columns=%d", word[1], n, m);
    eoutput(rivi);
    eoutput(" ");

    muste_sprintf(rivi, "    Canonical   %.*sEigen-    %.*sChi^2 %.*sCumulative", accuracy - 4, space, accuracy - 0,
                  space, accuracy - 4, space);
    eoutput(rivi);
    muste_sprintf(rivi, "    correlation %.*svalue     %.*s      %.*spercentage", accuracy - 4, space, accuracy - 0,
                  space, accuracy - 4, space);
    eoutput(rivi);
    sum_eig = 0.0;
    for (i = 0; i < nn; ++i) {
    ...
    for (i = 0; i < ni; ++i) {
        ...
        k = muste_sprintf(rivi, "%2d %.*s        %.*s", i + 1, accuracy, sana1, accuracy, sana2);
    ...
    k = muste_sprintf(rivi, "    %.*s       %.*s     %.*s", accuracy, space, accuracy, sana1, accuracy + 4, sana2);
    df = (int)(nn - 1L) * (mm - 1L);
