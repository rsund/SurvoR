...
void muste_transform(char *argv) {
    ...
    if (muste_strnicmp(lauseke, "CENTER", 6) == 0) {
        cent_std = 1;
    } else if (muste_strnicmp(lauseke, "STD", 3) == 0 || muste_strnicmp(lauseke, "STAND", 5) == 0) {
        cent_std = 2;
    }
    ...
    if (strchr(lauseke, 'X') != NULL) {
        muuttuja_mukana = 1;
    } else {
        muuttuja_mukana = 0;
    }
    if (strstr(lauseke, "MISSING") != NULL) {
        muuttuja_mukana = 0;
    }
    i = data_open2(word[1], &d, 1, 1, 0);
    if (i < 0) {
        return;
    }
    i = spec_init(r1 + r - 1);
    ...
    for (jj = d.l1; jj <= d.l2; ++jj) // j->jj globaaliksi! 17.3.2010
    {
        ...
        for (i = 0; i < d.m_act; ++i) {
            ...
            if (xarvo == MISSING8 && muuttuja_mukana) {
                yarvo = MISSING8;
            } else {
                l_virhe = 0;
...
static int centstd() {
    ...
    for (i = 0; i < m; ++i) {
        if (n[i] == 0L && (cent_std == 2 && n[i] < 2)) {
            muste_sprintf(sbuf, "\nToo few observations in variable %s!", d.varname[d.v[i]]);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
        ...
        if (cent_std == 2) {
            ...
            if (stddev[i] < 1e-10) {
                muste_sprintf(sbuf, "\nVariable %s is constant %g!", d.varname[d.v[i]], mean[i]);
                sur_print(sbuf);
                WAIT;
                return (-1);
            }
...
static int init_rnd() {
    ...
    if (muste_strnicmp(x, "rand(", 5) == 0) {
        ...
        rand_type = 2;
        i = 6;
    } else if (muste_strnicmp(x, "mrand(", 6) == 0) {
        rand_type = 4;
        i = 6;
    } else {
        rand_type = 1;
    ...
    for (j = d.l1; j <= d.l2; ++j) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            if (d.vartype[v[i]][0] == 'S') {
                ...
                k = data_save(&d, j, v[i], (double)(k + 1));
            }
            if (k < 0) {
                return (-1);
            }
        ...
        if (ind_var) {
            if (type_ind_var == 'S') {
                ...
                k = data_save(&d, j, var, (double)(prev_state + 1));
            }
...
static int tr_diff() {
    ...
    for (j = d.l1 + 1L; j <= d.l2; ++j) {
        ...
        for (i = 0; i < d.m_act; ++i) {
            data_load(&d, j, v[i], &a);
            k = data_save(&d, j, v[i], a - obs_lag[i]);
            if (k < 0) {
                return (-1);
            }
            obs_lag[i] = a;
