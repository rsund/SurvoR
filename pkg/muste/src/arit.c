...
static int laske2(char *muuttuja, double *y) {
    ...
    */
    ...
    if (i < 0) {
        for (i = 0; i < strlen(muuttuja); i++) {
            if (muuttuja[i] == ',') {
                muuttuja[i] = EOS; // RS ADD
            }
        }
    }
...
static double luku(char *sana, int len) {
    ...
    if (sana[len - 1] == '%') {
        return (atof(sana) / 100.0); /* 28.2.1992 */
    }
    return (atof(sana));
}
...
static int laske_integral(char *lauseke, double *y) {
    ...
    */
    ...
    while (n < n0) {
        ...
        h /= 2;
        /*      Rprintf("\ns=%.16g",s);   */
        if (muste_fabs(t - m) / (muste_fabs(s) + eps * eps) < eps) {
            break;
        }
        ++n;
    }
...
static int root_of(char *lauseke, double *y) /* root_of(f(x),x1,x2,accuracy) */
{
    ...
    *q = EOS;
    ...
    if (f * fmid > 0.0) {
        muste_sprintf(sbuf, "\nRoot not bracketed by %g and %g!", x1, x2);
        sur_print(sbuf);
        WAIT;
        l_virhe = 1;
        return (-1);
...
static int f_edit(char *s, double *x, int n, double *py) {
    ...
    s[len++] = '(';
    i = 0;
    while (i < spn && ((spp[i] != ':' && spp[i] != '|') || strncmp(s, spa[i], len) != 0)) {
        ++i;
    }
    if (i == spn) {
        s[len - 1] = EOS;
    ...
    if (spp[i] == '|') {
        ...
        if (ir >= 0) {
            ...
            if (n_remf >= MAX_RF) {
                muste_sprintf(sbuf, "\nToo many recursive | functions (>%d)", MAX_RF);
                sur_print(sbuf);
                l_virhe = 1;
                WAIT;
                return (-1);
...
static char f_tiedosto_read(double *y) {
    ...
    *y = MISSING8;
    ...
    if (str_opnd[0] == NULL) { /* sur_print("\nERROR (f_tiedosto_read)"); WAIT */
        return ('-');
    } // RS ADD REM print 20.3.2013
    ...
    if (j == 0L) {
        ...
        if (p != NULL) {
            *p = EOS;
            strcpy(nimimuuttuja, str_opnd[1]);
        ...
        if (dat.vartype[nimi_var][0] == 'S') {
            ...
            for (j = (unsigned int)dat.l1; j <= (unsigned int)dat.l2; ++j) // RS 64-bit?!?
            {
                ...
                i = strlen(hav_nimi) - 1;
                while (i > 0 && hav_nimi[i] == ' ') {
                    hav_nimi[i--] = EOS;
                }
                if (strcmp(str_opnd[1], hav_nimi) == 0) {
                    break;
                }
                if (*haku_sana2 != EOS && strcmp(haku_sana2, hav_nimi) == 0) {
                    break;
                }
            }
...
int op_arit() {
    ...
    extern char OO[];
    if (strcmp(OO, "GPLOT") == 0) {
        return (2);
    }
    if (strcmp(OO, "PLOT") == 0) {
        return (2);
    }
    if (strcmp(OO, "HISTO") == 0) {
        return (2);
    }
    if (strcmp(OO, "GHISTO") == 0) {
        return (2);
    }
    if (strcmp(OO, "TAB") == 0) {
        return (2);
    }
    if (strcmp(OO, "MASK") == 0 || strncmp(OO, "MASK=", 5) == 0) {
        return (2);
    }
    if (strcmp(OO, "VARS") == 0 || strncmp(OO, "VARS=", 5) == 0) {
        return (2);
    }
    ...
    for (i = 0; i < MAX_RF; i++)
        remember_space[i] = NULL; // RS 25.3.2013
    for (i = 0; i < MAXARG + 4; i++)
        str_opnd[i] = NULL; // RS 25.3.2013
    for (i = 0; i < EP4; i++)
        s[i] = NULL; // RS 25.3.2013
    ...
    */
    ...
    if (i == 2222) {
        strcpy(help_sana, "CONV");
        return (1);
    } /* conversions by CONV1 */
    /*
   Rprintf("i=%d l_virhe=%d errno=%d tulos=%g\n",i,l_virhe,errno,tulos); getch();
    */
    if (i < 0 || l_virhe) // RS REM || errno)
    {
        ...
        if (muste_strcmpi(osa[0], "VAR") == 0 || muste_strcmpi(osa[0], "MAT") == 0 ||
            strcmp(tuntematon_muuttuja, "VARS") == 0 || strcmp(tuntematon_muuttuja, "IND") == 0 ||
            strcmp(tuntematon_muuttuja, "CASES") == 0 || strcmp(tuntematon_muuttuja, "SELECT") == 0)
