...
static int sum_of_distances_from_centers(void);
static int read_centers(void);
static int comp_sum(void);
static int print_results(void);
static int eoutput(char *rivi);
static int centers_error(void);
...
static int read_spec() {
    ...
    if (i >= 0) {
        *x = EOS;
        strncat(x, spb[i], 3);
        muste_strupr(x);
        strcpy(measure_name, spb[i]);
        if (strcmp(x, "EUC") == 0) {
            measure = EUCLID;
            cov_ind = 0;
        } else if (strcmp(x, "MAH") == 0) {
            measure = MAHAL;
            cov_ind = 1;
            scaling = 1;
        } else if (strcmp(x, "CIT") == 0) {
            measure = CITY;
            cov_ind = 0;
        ...
        } else if (strcmp(x, "CAN") == 0) {
            measure = CANBERRA;
            cov_ind = 0;
        } else if (strcmp(x, "BRA") == 0) {
            measure = BRAY_CURTIS;
            cov_ind = 0;
        } else if (strcmp(x, "BHA") == 0) {
            measure = BHATTA;
            cov_ind = 0;
        } else if (strcmp(x, "ANG") == 0) {
            measure = ANGULAR;
            cov_ind = 0;
        } else if (strcmp(x, "COR") == 0) {
            measure = CORREL;
            cov_ind = 0;
...
static int read_weights() {
    ...
    if (i >= 0) {
        ...
        if ((wm == 1 && wn == m) || (wm == m && wn == 1)) {
            return (1);
        }
        muste_sprintf(sbuf, "\nThe weights (%s) should be given as a vector of %d elements!", wname, m);
        sur_print(sbuf);
        WAIT;
        return (-1);
    } else {
...
static int load_cases() {
    ...
    */
    ...
    strcpy(tmpdata, etmpd);
    strcat(tmpdata, "SURVO.TMP");
    ...
    for (j = d.l1; j <= d.l2; ++j) {
        ...
        if (label_var >= 0) {
            ...
            for (i = strlen(sbuf); i < 8; ++i) {
                sbuf[i] = ' ';
            }
        }
        for (i = 0; i < 8; ++i) {
            putc((int)sbuf[i], data);
        }
...
static int binlimits() {
    ...
    for (i = 0; i < m; ++i) {
        *nimi = EOS;
        strncat(nimi, d.varname[d.v[i]], 8);
        k = strlen(nimi) - 1;
        while (k >= 0 && nimi[k] == ' ') {
            nimi[k--] = EOS;
        }
...
static int moments() {
    ...
    for (i = 0; i < m; ++i) {
        ...
        if (scaling) {
            if (const_ind) {
                muste_sprintf(sbuf, "\nVariable %.8s is a constant %g", d.varname[d.v[i]], mean[i]);
                sur_print(sbuf);
                WAIT;
                return (-1);
            }
...
static int compute() {
    ...
    if (measure == MAHAL) {
        ...
        for (i = 0; i < m; ++i) {
            for (k = 0; k < m; ++k) {
                distm[i + m * k] = cov[i + m * k];
            }
        }
        i = mat_inv(cov, distm, m, &a);
        if (i != 1) {
            muste_sprintf(sbuf, "Singular covariance matrix (variable %.8s)!", d.varname[d.v[-i]]);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
    ...
    for (j = 0; j < n; ++j) {
        if (prind) {
            muste_sprintf(sbuf, "%d ", j + 1);
            sur_print(sbuf);
        }
        tmp_read(j, label, xx);

        for (h = 0; h <= j; ++h) {
            if (h == j && measure != BINARY) {
                continue;
            }
            /* 1.4.1995     */
...
static int correl(double *pd, double *x, double *y) {
    ...
    *pd = 0.0;
    ...
    if (fabs(sx) < 1e-20 || fabs(sy) < 1e-20) {
        *pd = 1.0;
    } else {
        *pd = 1 - (*pd) / sqrt(sx * sy);
    }
...
static int binary(double *pd, double *x, double *y) {
    ...
    laske(coeff, pd);
    if (l_virhe) {
        muste_sprintf(sbuf, "\nCannot compute expression %s for a=%g b=%g c=%g d=%g !", coeff, arvo[0], arvo[1],
                      arvo[2], arvo[3]);
        sur_print(sbuf);
        WAIT;
        return (-1);
...
static int print_results() {
    ...
    muste_sprintf(sbuf, "Sum of distances from %d centers is D_total=%g ( N=%d )", n_center, total_sum, n);
    eoutput(sbuf);
    if (results > 0) {
        eoutput("Group    Obs.#   Center     n");
        for (i = 0; i < n_center; ++i) {
            muste_sprintf(sbuf, "%2d    %8d %8.8s%6d", i + 1, obs_nr[i] + 1, lab2 + 8 * i, gfreq[i]);
            eoutput(sbuf);
        }
    }
    eoutput(" ");
