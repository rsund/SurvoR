...
void muste_plot(int argc, char *argv[]) {
    ...
    */
    ...
    muste_gplot_init = 0;
    if (i < 0) {
        sur_print("\nNot enough space for specifications!");
        WAIT;
        return;
    }
    ...
    if (i > 0) {
        if (strcmp(x, "PS") == 0 || muste_strcmpi(x, "PostScript") == 0) {
            strcpy(path, "PS");
        } else if (muste_strcmpi(x, "CRT") == 0 || muste_strcmpi(x, "G") == 0) {
            strcpy(path, "G");
            /*
    ...
    if (i >= 0) {
        ...
        if (muste_strcmpi(x, "CRT") == 0 || muste_strcmpi(x, "G") == 0) {
            strcpy(path, "G");
        } else if (*x == 'P' || *x == 'p') {
            strcpy(path, "PS");
    ...
    if (muste_strcmpi(word[1], "FUNCTION") == 0) {
        suorita("PFUNC.EXE", argv[1]);
        return;
    }
    if (muste_strcmpi(word[1], "FILE") == 0) {
        suorita("FILE.EXE", argv[1]);
        return;
    }
...
static int p_init(char *laite) /* for PS printers */
{
    ...
    if (i >= 0) {
        ...
        if (i > 1) {
            ...
            strcpy(nimi, edisk);
            strcat(nimi, "MUSTE_PR.PS"); // RS CHA
            strcpy(psnimi, nimi);
    ...
    marker_type = 3;
    marker_size = 5 * ps_coeff;
    x_pos = 0;
    y_pos = 0;
    x_ps = y_ps = 0;
    x_home = 250 * ps_coeff;
    y_home = 100 * ps_coeff;
    x_size = 1500 * ps_coeff;
    y_size = 1500 * ps_coeff;
    xdiv1 = 300.0;
    xdiv2 = 1000.0;
    xdiv3 = 200.0;
    ydiv1 = 300.0;
    ...
    if (i >= 0) {
        ...
        i = split(x, sana, 2);
        if (i > 0) {
            width_coeff = atof(sana[0]);
        }
        if (i > 1) {
            height_coeff = atof(sana[1]);
        }
    }
    font_size = 12.0;
...
static void p_end() {
    ...
    p_eps();
    if (ps_printer_direct) {
        win_tulostus(); // 23.9.2001
    }
}
...
static void p_newpage() { send("gsave showpage grestore\n"); }
...
/* int i;       attribute index */
{
    ...
    p = (unsigned char *)y;
    while (*p) {
        *p = code[(unsigned int)(*p)];
        ++p;
    }
    ps_replace((unsigned char *)y);
    muste_sprintf(s, "%d m %d m moveto ", x1, y1);
    send(s); // RS CHA ,x1,y1,y -> x1,y1
    text_move_rot(1);
...
static void text_move_rot(int k) {
    ...
    if (k == 1) {
        ...
        if (y_psmove != 0.0) {
            muste_sprintf(s, "%g m add ", y_psmove);
            send(s);
        }
        if (x_psmove != 0.0) {
            muste_sprintf(s, "exch %g m add exch ", x_psmove);
            send(s);
        }
...

{
    ...
    muste_sprintf(y, "%d m %d m moveto ", x1, y1);
    send(y);
    text_move_rot(1);
    ...
    while ((unsigned char)xs[i]) {
        ...
        if (p != NULL) {
            ...
            p = yy;
            while (*p) {
                putc((int)(*p), kirjoitin);
                ++p;
            }
        }
        k = 0;
        while ((unsigned char)xs[i] == varjo) {
            y[k] = code[(unsigned int)x[i]];
            ++k;
            ++i;
        }
        y[k] = EOS;
...
static int p_fill_polygon(int kerroin, char *s) {
    ...
    if (i % 2 != 0) {
        ...
        if (fill >= 0 && !colors_2010) {
            ...
            p_fillattr(fill);
            muste_sprintf(sbuf,
                          "closepath gsave f_cyan f_mage f_yell f_black setcmykcolor fill grestore stroke grestore\n");
        } else {
            current_fill = 1.0;
            muste_sprintf(sbuf, "closepath stroke grestore\n");
        }
...
static void p_frame(int frtype) {
    ...
    if (frtype >= 2 && frtype < 6) {
        p_fill_bar(x_home, y_home, x_home + x_size, y_home + y_size, -1000);
    }
    if (frtype == 5 || frtype == 6) {
        p_halfframe(xx, yy, xx + x_kuva, yy + y_kuva);
    }
    if (frtype > 0 && frtype < 3) {
        p_fill_bar(xx, yy, xx + x_kuva, yy + y_kuva, -1000);
    }
...
static int p_marker(int x2, int y2) {
    ...
    */
    ...
    case 0: /* filled circle */
        i = size;
        if (i < 1) {
            i = 1; /* if (i>60) i=60; poistettu 28.3.1995 */
        }
        if (marker_color0 == 9999) {
...
static int ps_code(char *x, char **sana, int n, char *rivi) /* ps_code <koodi> <8#xxx> <ps-nimi> */
{
    ...
    *y = '\\';
    y[1] = EOS;
    strcat(y, sana[2] + 2);
    ps_str[merkki] = (unsigned char *)pr_osoitin;
    strcpy(pr_osoitin, y);
...
static int p_path(int nt, char **sana) {
    ...
    for (i = 2; i < nt - 1; i += 2) {
        xk = arit_atoi(sana[i]);
        yk = arit_atoi(sana[i + 1]);
        muste_sprintf(s, "%d %d rl\n", xk, yk);
        send(s); /* m pois 22.11.92 */
    }
...
static int pl_triangle(int x1, int y1, int x2, int y2, int x3, int y3, int t) {
    ...
    muste_sprintf(s, "\nnewpath %d m %d m moveto ", x1, y1);
    send(s);
    muste_sprintf(s, "%d m %d m lineto %d m %d m lineto\n", x2, y2, x3, y3);
    send(s);
    if (t < 0) {
        strcpy(s, "closepath fill\n");
        send(s);
    } else {
        strcpy(s, "stroke\n");
        send(s);
