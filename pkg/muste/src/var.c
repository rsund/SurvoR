    ...
    if (p != NULL) {
        *p = EOS;
        ...
        if (tyyppi == EOS || strchr("1248S", tyyppi) == NULL) {
            ...
            muste_sprintf(sbuf, "\nUnknown type %c for %s!", tyyppi, sana);
            sur_print(sbuf);
...
static int f_edit_var(char *s, double *x, int n, double *py) {
    ...
    ***************************************************/
    ...
    while (i < spn && (spa[i] == NULL || *spa[i] == EOS || spa[i][strlen(spa[i]) - 1] != ':' ||
                       strncmp(s, spa[i], (unsigned int)len) != 0)) {
        ++i;
    }
    ...
    */
    ...
    if (strcmp(S, "RAND") == 0) {
        ...
        if (strncmp(S, "SQR", 3) == 0) /* 29.9.1996 x<0.0 etc. */
        {
            ...
            return (muste_tan(x));
        } else if (strcmp(S, "ATN") == 0 || strcmp(S, "ARCTAN") == 0) {
            return (muste_atan(x));
            ...
            return (fact(x));
        } else if (strcmp(S, "LFACT") == 0 || strcmp(S, "FACT.L") == 0) {
...
static int sup_arvo(char *muuttuja, double *y) {
    ...
    *p = EOS;
    ...
    if (sdata < 1 || sdata > ndata) {
        ...
        WAIT;
        l_virhe = 1;
        return (-1); // RS CHA exit(0) -> lvirhe=1; return(-1)
    }
    k = sdata - 1;
    j = jnro + (long)lag;
    lag = 0; /* 6.10.1989 */
...
static int str_arvo(char *a, char *s) {
    ...
    if (*a == '"') {
        ...
        muste_fieldcopy(s, x + str_var_start - 1, (unsigned int)str_var_len);
        s[str_var_len] = EOS;
    }
    if (code_ind == 2) {
        conv_var((unsigned char *)s);
    }
    return (1);
}
...
static int str_laske(char *lauseke) {
    ...
    *tulos = EOS;
    ...
    while (*p) {
        if (*p == '"') {
            ...
            *q = EOS;
            ...
            muste_fieldcopy(sana, sana2 + start - 1, (unsigned int)len);
            sana[len] = EOS;
    ...
    if (d.type == 2) {
        fi_alpha_save(&d.d2, jnro + (long)str_lag, str_var, sana);
    } else if (d.type == 1) {
        ma_save(&d.d1, (int)(jnro + str_lag), str_var, sana);
    } else {
...
static int muunto0() {
    ...
    mean = s1 / nx2;
    stddev = sqrt((s2 - s1 * s1 / nx2) / (nx2 - 1.0));
    sum = s1; /* 11.10.1996 */
...
static int muunto2(int muunnos) {
    ...
    if (muunnos == 1 || muunnos == 4) {
        return (1);
    }
    if (muunnos == 2 || muunnos == 3) {
        if (stddev < 1e-15) {
            muste_sprintf(sbuf, "\nVariance=0. Cannot continue!");
            sur_print(sbuf);
            return (-1);
        }
    }
    ...
    if (muunnos == 2) {
        for (jxx = 0L; jxx < nxx; ++jxx) {
            ...
            */
            if (xx[jxx] != MISSING8) {
                xx[jxx] = mean + stddev * qnorm(((xx[jxx] - 0.5) / nx2), 0, 1, 1, 0);
            }
        }
        return (1);
    }
    ...
    if (muunnos == 5) {
        for (jxx = 0L; jxx < l; ++jxx) {
            xx[jxx] = xx[nx2 - 1 - jxx] = MISSING8;
        }
        return (1);
    }
    if (muunnos == 6) {
    ...
    if (muunnos == 7) {
        for (jxx = 0L; jxx < nxx; ++jxx) {
            if (xx[jxx] == MISSING8 || fabs(xx[jxx] - mean) > level * stddev) {
                xx[jxx] = MISSING8;
            }
        }
        return (1);
...
static int talletus() {
    ...
    for (jxx = 0L; jxx < nxx; ++jxx) {
        if (prind) {
            muste_sprintf(sbuf, " %ld", oxx[jxx]);
            sur_print(sbuf);
        }
        i = data_save(&d, oxx[jxx], var[0], xx[jxx]);
        if (i < 0) {
            return (-1);
        }
        if (sur_kbhit()) {
...
static void uusi_nimi(int i, char *s) {
    ...
    k = 8;
    while ((x[k] == ' ' || x[k] == '(' || x[k] == '#' || x[k] == '.' || x[k] == ')') && k < len) {
        ++k; // RS 7.10.2014 ADD (#)
    }
    if (k == len || x[k] == EQ) {
        if (x[k] == EQ) {
...
static void op_var2() {
    ...
    if (muste_strcmpi(word[g - 2], "TO") == 0) {
        strcpy(nimi, word[g - 1]);
    } else {
        strcpy(nimi, active_data);
    }

    i = data_open2(nimi, &d, 1, 0, 0);
    if (i < 0) {
        return;
    }
    ...
    *q = EOS;
    if (muste_strcmpi(p, "#RANK") == 0) {
        muunnos = 1;
    } else if (muste_strcmpi(p, "#NORMAL") == 0) {
        muunnos = 2;
    } else if (muste_strcmpi(p, "#STD") == 0) {
        muunnos = 3;
    } else if (muste_strcmpi(p, "#NRANK") == 0) {
        muunnos = 4;
    } else if (muste_strcmpi(p, "#TRUNCP") == 0) {
        muunnos = 5;
    } else if (muste_strcmpi(p, "#WINSP") == 0) {
        muunnos = 6;
    } else if (muste_strcmpi(p, "#TRUNCL") == 0) {
        muunnos = 7;
    } else if (muste_strcmpi(p, "#WINSL") == 0) {
        muunnos = 8;
    } else if (muste_strcmpi(p, "#PROPORTION") == 0) {
        muunnos = 9; /* 11.10.1996 */
    } else if (muste_strcmpi(p, "#PERCENT") == 0) {
        muunnos = 10;
    } else if (muste_strcmpi(p, "#CENTER") == 0) {
        muunnos = 11;
    } else {
        muste_sprintf(sbuf, "\nUnknown transformation %s", p);
        sur_print(sbuf);
        WAIT;
        return;
    }
