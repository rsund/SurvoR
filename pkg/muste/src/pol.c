...
static struct complex *c_mult(struct complex *z, struct complex *z1, struct complex *z2) {
    ...
    z->x = u.x;
    z->y = u.y; /* jotta my?s z=z1 mahdollinen */
    return (z);
}
...
static struct polynom *pol_mult(struct polynom *p, struct polynom *p1, struct polynom *p2) {
    ...
    for (i = 0; i <= p1->n; ++i) {
        for (j = 0; j <= p2->n; ++j) {
            c_add(&(p->a[i + j]), &(p->a[i + j]), c_mult(&tulo, &(p1->a[i]), &(p2->a[j])));
        }
    }
...
static struct polynom *pol_div(struct polynom *p, struct polynom *q, struct polynom *p1, struct polynom *p2) {
    ...
    for (i = p1->n; i >= p2->n; --i) {
        ...
        for (j = i - 1; j >= i - p2->n; --j) {
            c_sub(&(q->a[j]), &(q->a[j]), c_mult(&z1, &z, &(p2->a[p2->n - i + j])));
        }
    }
    i = p2->n - 1;
    while (c_zero(&(q->a[i])) && i > 0) {
        --i;
    }
    q->n = i;
    return (p);
...
static struct complex *pol_value(struct polynom *p, struct complex *z, /* pointer to argument */
...
static void op_roots() {
    ...
    if (i >= 0) {
        ...
        if (i > 1) {
            roots_eps = atof(osa[1]);
        }
    }

    i = muste_pol_load(q, &pol);
    if (i < 0) {
        return;
    }
...
static void op_mult() {
    ...
    *q++ = EOS;
    i1 = muste_pol_load(p, &pol1);
    if (i1 < 0) {
        return;
    }
    i2 = muste_pol_load(q, &pol2);
    if (i2 < 0) {
        return;
    }
    if (i1 == 1 && i2 == 1) {
        pol_mult_real(&pol, &pol1, &pol2);
    } else {
        pol_mult(&pol, &pol1, &pol2);
    }
...
static void op_div() {
    ...
    *q++ = EOS;
    i = muste_pol_load(p, &pol1);
    if (i < 0) {
        return;
    }
    i = muste_pol_load(q, &pol2);
    if (i < 0) {
        return;
    }
...
static void op_value() /* POL V=P(X) */
{
    ...
    i = muste_pol_load(p, &pol);
    if (i < 0) {
        return;
    }
    i = muste_pol_load(q, &arg);
    if (i < 0) {
        return;
    }
