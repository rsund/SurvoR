...
static int m_printout() {
    ...
    muste_sprintf(line, " in %s N=%d", word[1], n);
    if (weight_variable >= 0) {
        strcat(line, " Weight=");
    ...
    for (i = 0; i < d.m_act; ++i) {
        ...
        if (w[i] == 0.0) {
            muste_sprintf(line, " %-8.8s         -          -  %6d", d.varname[d.v[i]], (int)(n - f[i]));
        } else {
            fnconv(sum[i] / w[i], accuracy + 2, mean);
            if (w[i] > 1.0) {
                fnconv(sqrt((sum2[i] - sum[i] * sum[i] / w[i]) / (w[i] - 1)), accuracy + 2, stddev);
            } else {
                muste_fieldcopy(stddev, space, accuracy + 2);
                stddev[accuracy + 2] = EOS;
                stddev[accuracy + 1] = '-';
            }

            h = muste_sprintf(line, " %-8.8s %s  %s  %6d    ", d.varname[d.v[i]], mean, stddev, (int)(n - f[i]));
            for (k = 0; k < n_class; ++k) {
