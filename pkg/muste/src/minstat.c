...
static int printout() {
    ...
    output_open(eout);
    muste_sprintf(line, "Basic statistics of data %s N=%d", word[1], n);
    print_line(line);
    ...
    *rlab = EOS;
    for (i = 0; i < d.m_act; ++i) {
        ...
        if (f[i] == 0L) {
            muste_sprintf(line, "%-8.8s            -  %6d", d.varname[d.v[i]], n - f[i]);
        } else {
            a0 = sum[i] / (double)f[i];
            fnconv(a0 + x0[i], accuracy + 2, stat);
            aa[i + mm * 0] = a0 + x0[i];
            k = muste_sprintf(line, " %-8.8s %s", d.varname[d.v[i]], stat);
            b0 = f[i] - 1;
            if (b0 == 0.0) {
                b0 = 1.0;
            }
