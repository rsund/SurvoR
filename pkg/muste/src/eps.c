...
void muste_eps2(int argc, char *argv[]) {
    ...
    */
    ...
    if (i >= 0) {
        ...
        muste_sprintf(bounding_box, "%%%%BoundingBox: %d %d %d %d", (int)(a * atof(sana[0])), (int)(a * atof(sana[1])),
    ...
    while (1) {
        ...
        if (strncmp(rivi, "/survodict ", 11) == 0) {
            break;
        }
    }
    n = 5;
    while (1) {
        ...
        i = lue(rivi);
        if (i < 0) {
            eps_virhe("%SURVO 84C Graphics END");
            return;
        }
        if (muste_strnicmp(rivi, "%SURVO ", 6) == 0) {
            if (strncmp(rivi, "%SURVO 84C Graphics END", 23) == 0) {
                break;
            }
            if (strncmp(rivi, "%Survo Graphics END", 19) == 0) {
                break;
            }
        }
    }
...
static void specifications() {
    ...
    if (i >= 0) {
        strcpy(version_comment, ala_pois(spb[i]));
    }
    strcpy(title, "%%Title: Survo PostScript file");
    i = spfind("TITLE");
    if (i >= 0) {
        strcpy(title + 9, ala_pois(spb[i]));
    }
    *creator = EOS;
    i = spfind("CREATOR");
    if (i >= 0) {
        strcpy(creator, "%%Creator: ");
        strcat(creator, ala_pois(spb[i]));
    }
    time(&aika);
    strcpy(x, ctime(&aika));
    x[24] = EOS;
    strcpy(creation_date, "%%CreationDate: ");
    strcat(creation_date, x);
    i = spfind("CREATION_DATE");
    if (i >= 0) {
        strcpy(creation_date + 16, ala_pois(spb[i]));
    }
    strcpy(end_comments, "%%EndComments");
    *document_fonts = EOS;
    i = spfind("DOCUMENT_FONTS");
    if (i >= 0) {
        strcpy(document_fonts, "%%DocumentFonts: ");
...
static int join() {
    ...
    for (k = 3; k < g; ++k) {
        ...
        if (muste_strcmpi(psnimi, psnimi2) == 0) {
            muste_sprintf(sbuf, "\nTarget file name %s cannot be equal to any of the components!", psnimi);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
        ...
        while (1) {
            ...
            if (muste_strnicmp(rivi, "%SURVO ", 7) == 0) {
                break;
            }
        }
        if (k == 3) {
...
static int mahtuu() {
    ...
    sur_print("\nCannot save in the target file!");
    WAIT;
    return (-1);
}
