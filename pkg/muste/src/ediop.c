...
static int read_char(char *s, unsigned char *pch) {
    if (muste_strnicmp(s, "char(", 5) == 0) {
        *pch = (unsigned char)atoi(s + 5);
    } else {
        *pch = *s;
    }
    return (1);
...
static int load_codes(char *codefile, unsigned char *code) {
    ...
    strcat(nimi, "SYS/"); // RS CHA \\ /
    strcat(nimi, codefile);
    strcat(nimi, ".BIN");
    i = spfind("FILTER");
    if (i >= 0) {
        strcpy(nimi, survo_path);
        strcat(nimi, "SYS/"); // RS CHA \\ /
        if (!muste_is_path(spb[i])) {
            strcat(nimi, spb[i]);
        } else {
            strcpy(nimi, spb[i]);
        }
...
static void load_codes2(char *code) // RS REM unsigned
{
    ...
    strcpy(x, survo_path);
    strcat(x, "SYS/SORTCODE.BIN"); // RS CHA \\ -> /
...
static int vokaali(unsigned char kirjain) { return ((int)(vokaalit[(int)code[kirjain]] - '0')); }
...
static int tavuta(char *sana) /* return(viimeisen tavun alkuindeksi) */
{
    ...
    while (!vokaali(sana[i - 1]) && i > 0) {
        --i;
    }
    if (i == 0) {
        return (0);
    }
    while (vokaali(sana[i - 1]) && i > 2) {
        --i;
    }
    //      if (i<3) return(0);
    if (i < trim_tolerance + 1) {
        return (0);
    }
    return (i - 1);
}
...
static int tulosta() {
    ...
    ++outj;
    muste_fieldcopy(outx, space, reuna - 1);
    outx[reuna - 1] = EOS;
    muste_fieldcopy(soutx, space, reuna - 1);
    soutx[reuna - 1] = EOS;
    nwords = 0;
    return (1);
}
...
static int trim_kpl(int tav) {
    ...
    *inx = EOS;
    ...
    j = inj;
    sp = sinx + reuna1 - 1;
    muste_fieldcopy(outx, space, reuna1 - 1);
    outx[reuna1 - 1] = EOS;
    muste_fieldcopy(soutx, space, reuna1 - 1);
    soutx[reuna1 - 1] = EOS;
    while (1) {
        while (*p == ' ' && *sp == ' ') {
            ++p;
        ...
        *ssana = EOS;
        ...
        if (*(q - 1) == '-' && empty2(p)) { /* sana jatkuu seuraavalle riville */
            --q;
            --sq;
            *inx = EOS;
            *sinx = EOS;
        ...
        *sq = EOS;
        ...
        if (trim_words) {
            if (nwords == trim_words) {
                i = tulosta();
                if (i < 0) {
                    return (-1);
                }
            }
        } else if ((pitch_unit && (strpitch(outx) + strpitch(sana) > pitch_unit * lev)) ||
            ...
            if (!tav) {
                ...
                strcpy(jakosana, sana);
                strcpy(sjakosana, ssana);
                do {
                    k = tavuta(jakosana);
                    if (k == 0) {
                        break;
                    ...
                    jakosana[k] = EOS;
                    sjakosana[k] = EOS;
                } while ((pitch_unit && (strpitch(outx) + strpitch(jakosana) + pitch['-'] > pitch_unit * lev)) ||
                         (!pitch_unit && (strlen(outx) + k + 1 > lev)));
                if (k > 0) {
                    strcat(outx, jakosana);
                    strcat(outx, "-");
                    strcat(soutx, sjakosana);
                    strcat(soutx, " ");
                    i = tulosta();
                    if (i < 0) {
                        return (-1);
...
static int lastline0(char *z, unsigned int ed1, unsigned int ed2) {
    ...
    j = ed2;
    while (j > 0) {
        if (strncmp(space, z + (j - 1) * ed1 + 1, ed1 - 1) != 0) {
            break;
        }
        ...
        if (zs[j]) {
            if (strncmp(space, z + (zs[j] - 1) * ed1 + 1, ed1 - 1) != 0) {
                break;
            }
        }
    ...
    if (tav) {
        ...
        if (tavuohje) {
            ...
            if (trimfile == NULL) {
                ...
                WAIT;
                tavuohje = 0;
                return (-2); // RS CHA exit(0); // RS FIXME RS 3.2.2014 tavuohje=0
            }
            i = load_codes("LOWCASE", (unsigned char *)code0);
            if (i < 0) {
                return (-1);
            }
        }
...
static int lisays(char merkki) {
    ...
    while ((q = strchr(p, merkki)) != NULL) {
        sq = sp + (q - p);
        strncat(rivi, p, q - p + 1);
        strncat(srivi, sp, sq - sp + 1);
        if (*(q + 1) == ' ' || *(q + 1) == EOS) {
            strcat(rivi, " ");
            strcat(srivi, " ");
            --vajaus;
...
static void sedread(char *srivi, int j) {
    ...
    if (zs[j] == 0) {
        muste_fieldcopy(srivi, space, ed1);
        srivi[ed1] = EOS;
    } else {
        edread(srivi, zs[j]);
    }
...
static void op_trim() {
    ...
    if (g < 3) {
        ...
        jj1 = edline2(word[1], 1, 1);
        if (jj1 == 0) {
            return;
        }
        jj2 = edline2(word[2], jj1, 1);
        if (jj2 == 0) {
            return;
        }
        if (g > 3) {
...
static int num_mask(char **cplus_sana, int nsar) {
    ...
    for (i = 0; i < nsar; ++i) {
        if ((k = muste_isnumber(cplus_sana[i]))) {
            return (i);
        }
    }
...
static int function(char *s) {
    muste_strupr(s);
    if (strcmp(s, "LOG") == 0 || strcmp(s, "LN") == 0) {
        return (1);
    }
    if (strcmp(s, "EXP") == 0) {
        return (2);
    }
    if (strcmp(s, "SQRT") == 0 || strcmp(s, "SQR") == 0) {
        return (3);
    }
    if (strcmp(s, "SIN") == 0) {
        return (4);
...
static void lplus() {
    ...
    if (g < 5) {
        muste_sprintf(sbuf, "\nUsage: %s L1,L2,K,L", word[0]);
        sur_print(sbuf);
        WAIT;
        return;
    }
    oper = word[0][1];
    l1 = edline2(word[1], 1, 1);
    if (l1 == 0) {
        return;
    }
    i = l1;
    ...
    l2 = edline2(word[2], i, 1);
    if (l2 == 0) {
        return;
    }
    k = edline2(word[3], 1, 1);
    if (k == 0) {
        return;
    }
    lr = edline2(word[4], 1, 1);
    if (lr == 0) {
        return;
    }
    edread(cplus_mask, k);
...
static void hae(char m, int *pr, int *pc) {
    ...
    for (j = 1; j <= r2; ++j) {
        ...
        if (lkm || strchr(p + 1, m) != NULL) {
            muste_sprintf(sbuf, "\nCharacter %c appears several times in the edit field!", m);
            sur_print(sbuf);
            WAIT;
            *pr = -1;
            return;
...
static void textmove() {
    ...
    for (j = mr1; j <= mr2; ++j) {
        ...
        edread(x, mr + j - mr1);
        if (edit == 3) {
            memcpy(x + mc, x2 + mc1, lev);
        } else {
            memcpy(x + mc, x2 + mc1 + edit - 1, lev);
        }
...
static void op_move() {
    ...
    if (g == 2) {
        hae(word[1][0], &mr1, &mc1);
        if (mr1 < 0) {
            return;
        }
        hae(word[1][1], &mr2, &mc2);
        if (mr2 < 0) {
            return;
        }
        hae(word[1][2], &mr, &mc);
...
static int del_by_words() {
    ...
    while (1) {
        ...
        if (linedel_reverse && valinta < 2) {
            dlab += lab - lab0 - 1; // RS ADD
        }
...
static int op_linedel() {
    ...
    if (g == 1) {
        ...
        while (1) {
            ...
            edread(sbuf, j);
            if (*sbuf == '*' && strncmp(sbuf + 1, space, ed1 - 1) == 0 && zs[j] == 0) {
                continue;
            }
            break;
        }
    ...
    if (g > 3) {
        ...
        if (muste_strcmpi(word[3], "EMPTY") == 0) {
            ...
            step = atoi(word[4]);
            if (step <= 0) {
                return (-2); // RS CHA exit(0);  // RS FIXME
            }
        } else if (*p != '"' || p[i] != '"') {
            tyyli = 2;
            strcpy(sana, p);
    ...
    if (tyyli == 5) {
        ...
        k1 = edline2(word[1], 1, 1);
        if (!k1) {
            return (1);
        }
        k2 = edline2(word[2], k1, 1);
        if (!k2) {
            return (1);
        }
    }
    ...
    if (i > 0) {
        ...
        // rivinron painoasu!!!
        muste_sprintf(sbuf, "00001|*SAVE %s / Deleted lines%s%s", x, x2, rivin_loppu);
        fputs(sbuf, edt3);
    }
...
static void op_form() {
    ...
    fill = 0;
    if (g > 4) {
        if (muste_strnicmp(word[4], "L", 1) == 0) {
            fill = 1;
        }
        if (muste_strnicmp(word[4], "R", 1) == 0) {
            fill = 2;
        }
    }
    edread(maskline, k);
        ...
        for (i = 0; i < n; ++i) {
            if (type[i] == 'A') {
                ...
                if (strlen(luku) > len[i]) {
                    muste_sprintf(sbuf, "\nFormat %s not wide enough for %s on line %d", fmask[i], form_sana[i], j);
                    sur_print(sbuf);
                    WAIT;
                    return;
                }
...
static void op_putend() {
    ...
    *q = EOS;
    strcpy(sbuf, p);
    chrconv(sbuf, p); // RS ADD 22.3.2013
    for (j = j1; j <= j2; ++j) {
        edread(x, j);
        i = c2;
        while (x[i] == ' ' && i > 0) {
...
static int op_sort() {
    ...
    if (*mask[0] >= '0' && *mask[0] <= '9') {
        ...
        for (j = j1; j <= j2; ++j) {
            ...
            for (i = 0; i < ed1 - 1; ++i) {
                z[(j - 1) * ed1 + i + 1] = zz[(k - j1) * (ed1 - 1) + i];
            }
        }
        if (shadow) {
            shadow_move(j1, j2, sizeof(double));
        }
        return (1);
    }
    ...
    else /* Aakkoslajittelu */
    {
        ...
        i = load_codes("SORTCODE", (unsigned char *)code);
        if (i < 0) {
            return (-1);
        }
        ...
        for (j = j1, k = 0, h = 0; j <= j2; ++j, k += lenkey, h += ed1 - 1) {
            ...
            *luku = EOS;
            for (i = 0; i < an; ++i) {
                strncat(luku, x + apos[i], alen[i]);
            }
            sort_conv((unsigned char *)luku, (unsigned char *)code);
            for (i = 0; i < sort_len; ++i) {
                *(sortlist + k + i) = (unsigned char)luku[i];
            }
...
static int load(int tietue) {
    ...
    if (codes == NULL) {
        muste_sprintf(sbuf, "\nFile %s not found!", word[2]);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
    if (tietue > 1) {
        muste_fseek(codes, (long)((long)n_bytes * (long)(tietue - 1)), 0);
    }
...
static int wsave(int tiet1, int tiet2) {
    ...
    if (tiet1 > 0) {
        muste_fseek(codes, (long)((long)sizeof(int) * (long)(tiet1 - 1)), 0);
    }
    if (tiet2 == 0) {
        tiet2 = 1000000;
    }
...
static int op_codes() {
    ...
    // 9.6.2008
    if (muste_strcmpi(word[1], "REMOVE") == 0) {
        i = char_remove();
        return (i);
    }
    if (muste_strcmpi(word[1], "COPY") == 0) {
        i = char_copy();
        return (i);
    }
    if (muste_strcmpi(word[1], "SAVE") == 0) {
        i = save(tietue);
        return (i);
    }
    if (muste_strcmpi(word[1], "LOAD") == 0) {
        i = load(tietue);
        return (i);
    }
    ...
    if (muste_strcmpi(word[1], "WLOAD") == 0) {
        i = wload(tietue, tietue2);
        return (i);
    }
    if (muste_strcmpi(word[1], "WSAVE") == 0) {
        i = wsave(tietue, tietue2);
        return (i);
    }
...
static int w_codes_load(int k) {
    ...
    strcat(codefile, "SYS/WIN.BIN");       // RS CHA \\ -> /
        ...
        while (!feof(text)) {
            ...
            if (codeconv) {
                if (codeconv == 999) // RS 14.3.2013
                {
                    ...
                    for (i = 0; i < len; ++i) {
                        rivi[i] = (unsigned char)
                            code[(unsigned char)rivi[i]]; // RS CHA char)rivi[i]=code[(unsigned char)rivi[i]];
                    }
                }
            }
            if (riv > r2) {
    ...
    while (muste_fgets(rivi, rpit, text) != NULL) {
        ...
        if (codeconv) {
            if (codeconv == 999) // RS 14.3.2013
            {
                ...
                for (i = 0; i < len; ++i) {
                    rivi[i] = (unsigned char)
                        code[(unsigned char)rivi[i]]; // RS CHA (unsigned char)rivi[i]=code[(unsigned char)rivi[i]];
                }
            }
        }
        /*
...
static void op_update() {
    ...
    if (strchr(name + strlen(name) - 4, '.') == NULL) {
        strcat(name, ".EDT");
    }
    i = update_avaa(name);
    if (i < 0) {
    ...
    for (l = l1; l <= l2; ++l) {
        ...
        for (k = 1; k <= qed2; ++k) {
            ...
            muste_sprintf(sbuf, "%.*s ", keylen, x + 1);
            sur_print(sbuf);
            break;
        }
        if (k > qed2) {
...
static int textlimit_missing(unsigned int j) {
    muste_sprintf(sbuf, "\nText limit character `%c' missing on edit line %d!", textlimit, j);
    sur_print(sbuf);
    WAIT;
    return (1);
}
...
static int muunnos() {
    ...
    while (1) {
        ...
        for (i = 0; i < nc; ++i) {
            if (type[i] == 'T') {
                ok = 0;
                if (ch1 != (unsigned char)*text1[i]) {
                    continue;
                }
                k = 0;
                muste_ftell(txt1); // pos=muste_ftell(txt1);
                while (1) {
                    ++k;
                ...
                if (!ok) {
                    muste_fseek(txt1, (long)-k, SEEK_CUR);
                    continue;
                }
                ok2 = 1;
            }
...
static int g_conversion(char infile[], char outfile[]) // RS 24.1.2013
{
    ...
    strcpy(line1, infile);
    muste_expand_path(line1);
    strcpy(line2, outfile);
    muste_expand_path(line2);
...
static int conv_list() {
    ...
    while (j < r2) {
        ...
        if (strncmp(w[0], "CONVERSIONS", 11) == 0) {
            break;
        }
    }
    if (j == r2) {
...
static int tr_avaa(char *nimi, FILE **ptxt, char *moodi, char *polkunimi) {
    ...
    if (!muste_is_path(nimi)) {
        ...
        muste_strncpy(name, nimi, LLENGTH); // RS 13.3.2013
    }
    if (muste_strcmpi(name, polkunimi) == 0) {
        ei_samaan();
        return (-1);
    }
...
static int op_txtedtout() {
    ...
    i = tr_avaa2(word[1], ".EDT", &txt1, "rb");
    if (i < 0) {
        return (1);
    }
    i = tr_avaa2(word[2], "", &txt2, "wt");
    if (i < 0) {
        return (1);
    }
    ...
    if (strncmp(x, "SURVO84", 7) != 0) {
        ...
        if (strncmp(x, "SURVO 98", 8) == 0) {
            muste_sprintf(sbuf, "\n%s as a SURVO 98 edit file is a standard text file.", word[1]);
            sur_print(sbuf);
        }
        WAIT;
        return (-2); // RS CHA exit(1); // RS FIXME
    }
    i = split(x, osa, 3);
    lev = atoi(osa[1]);
    n = atoi(osa[2]);
    ...
    while (1) {
        ...
        muste_sprintf(sbuf, "%d",
                      (int)((unsigned char)*x) + (int)(256 * (unsigned char)*(x + 1))); // RS 4.2.2013 CHA *(int *)x);
        i = lev - 1;
        while (i >= sizeof(int) && x[i] == ' ') {
            x[i--] = EOS;
        }
        fprintf(txt2, "%s:%s\n", sbuf, x + 2);
    }
...
static int op_txtedtin() {
    ...
    i = tr_avaa2(word[2], ".EDT", &txt1, "wb");
    if (i < 0) {
        return (1);
    }
    i = tr_avaa2(word[1], "", &txt2, "rt");
    if (i < 0) {
        return (1);
    }
    ...
    while (1) {
        ...
        *p = EOS;
        ...
        for (i = strlen(p) - 1; i < lev - sizeof(int); ++i) {
            y[i + sizeof(int)] = ' ';
        }
        fwrite(y, 1, lev, txt1);
    }
...
static int op_txtshort() {
    ...
    if (i >= 0) {
        strcpy(x, spb[i]);
        if (muste_strcmpi(x, "TAB") == 0) {
            limit_char = '\t';
        } else if (muste_strnicmp(x, "SP", 2) == 0) {
            limit_char = ' ';
        } else if (muste_strnicmp(x, "char(", 5) == 0) {
            limit_char = atoi(x + 5);
        } else {
            limit_char = *x;
        }
    ...
    strcpy(nimi, edisk);
    strcat(nimi, word[1]);
    tx1 = muste_fopen(nimi, "rt");
    if (tx1 == NULL) {
        return (1);
    }
    strcpy(nimi, edisk);
    strcat(nimi, word[2]);
    tx2 = muste_fopen(nimi, "wt");
    if (tx2 == NULL) {
        return (1);
    }
    if (g < 4) {
...
static void op_txtrim() {
    ...
    *nimi = EOS;
    i = tr_avaa(word[1], &txt1, "rt", nimi);
    if (i < 0) {
        return;
    }
    i = tr_avaa(word[2], &txt2, "wt", nimi);
    if (i < 0) {
        return;
    }
...
static int count() {
    ...
    while (!ferror(txt2)) {
        ...
        if (ch == (int)'\n') {
            for (i = 0; i < 256; ++i) {
                if (i == 0) {
                    fprintf(txt2, "%ld", freq[0]);
                } else {
                    fprintf(txt2, " %ld", freq[i]);
                }
                freq[i] = 0L;
            }
            fprintf(txt2, "\n");
...
static int op_txtdel() {
    ...
    muste_fclose(txt1);
    muste_fclose(txt2); // RS 5.11.2014
    return (1);
}
...
static int siirto(int j1, int j2) {
    ...
    for (j = j1; j <= j2; ++j) {
        ...
        if (j == j1) {
            ...
            if (i != transp_n) {
                muste_sprintf(sbuf, "\n# of words on line %d not equal to %d!", j, transp_n);
                sur_print(sbuf);
                WAIT;
                return (-1);
            }
...
static void op_transp() {
    ...
    for (j = 0; j < transp_n; ++j) {
        edwrite(space, j + j3, 1);
    }
    transp_pos[0] = 1;
    for (i = 1; i < nr; ++i) {
        transp_pos[i] = transp_pos[i - 1] + transp_lev[i - 1] + 1;
    }
    transp(j3);
}
    ...
    for (j = l3; j <= l4; ++j) {
        if (mx0 < 3) {
            if (mx0 == 1) {
                ...
                edread(s, j);
                muste_fieldcopy(t, s + xcol[1], xlev[1]);
                t[xlev[1]] = EOS;
                XP[1] = atof(t);
            }
            a = b = XP[1];
        ...
        for (i = 0; i < my; ++i) {
            if (mx0 > 2) {
                edread(s, j);
                for (k = 1; k < mx; ++k) {
                    muste_fieldcopy(t, s + xcol[k], xlev[k]);
                    t[xlev[k]] = EOS;
                    XP[k] = atof(t);
                }
            }
            ...
            if (strlen(t) > strlen(ymask[i])) {
                muste_sprintf(sbuf, "\nImage %s in columns %d-%d on the mask line %s", ymask[i], (int)ycol[i],
                              (int)(ycol[i] + strlen(ymask[i]) - 1), word[5]);
                sur_print(sbuf);
...
static void lue_datat() {
    ...
    for (j = l1; j <= l2; ++j) {
        ...
        if (mx > 2) {
            for (i = 1; i < mx; ++i) {
                muste_fieldcopy(t, s + xcol[i], xlev[i]);
                t[xlev[i]] = EOS;
                X[i * n + j - l1] = atof(t);
            }
        }
        for (i = 0; i < my; ++i) {
            muste_fieldcopy(t, s + ycol[i], ylev[i]);
            t[ylev[i]] = EOS;
            Y[i * n + j - l1] = atof(t);
        }
    }
...
static void op_interp() {
    ...
    l3 = edline2(word[3], 1, 1);
    if (l3 == 0) {
        return;
    }
    l4 = edline2(word[4], l3, 1);
    if (l4 == 0) {
        return;
    }
    k = edline2(word[5], 1, 1);
    if (k == 0) {
        return;
    }
    ...
    for (i = 0; i < nf; ++i) {
        if (strchr(msana[i], 'X') != NULL) {
            xcol[mx] = msana[i] - msk;
            xlev[mx] = strlen(msana[i]);
            ++mx;
            continue;
        }
        if (strchr(msana[i], 'Y') != NULL) {
            ycol[my] = msana[i] - msk;
            ylev[my] = strlen(msana[i]);
            ymask[my] = msana[i];
            ++my;
            continue;
        }
...
static int op_reverse() {
    ...
    if (g > 3) {
        ...
        if (reverse_laji == 1) {
            reverse_by_lines(j1, j2);
            return (1);
        }
        if (reverse_laji == 2) {
            reverse_by_bytes(j1, j2);
            return (1);
        }
    }
    // reverse by words
...
static int op_transpose() {
    ...
    if (i >= 0) {
        ...
        i = split(x, tt, 2);
        if (i) {
            if (muste_strcmpi(tt[0], "TAB") == 0) {
                delim_in = '\t';
            } else if (muste_strnicmp(tt[0], "SP", 2) == 0) {
                delim_in = ' ';
            } else if (muste_strnicmp(tt[0], "char(", 5) == 0) {
                delim_in = atoi(tt[0] + 5);
            } else {
                delim_in = *tt[0];
            }
            if (i > 1) {
                if (muste_strcmpi(tt[1], "TAB") == 0) {
                    delim_out = '\t';
                } else if (muste_strnicmp(tt[1], "SP", 2) == 0) {
                    delim_in = ' ';
                } else if (muste_strnicmp(tt[1], "char(", 5) == 0) {
                    delim_out = atoi(tt[1] + 5);
                } else {
                    delim_out = *tt[1];
                }
            }
        ...
        while (1) {
            ...
            if (ch == (int)'\n') {
                ...
                if (n == 0) {
                    ...
                    muste_sprintf(sbuf, "\n%d fields instead of %d on line %d!", i, m, n + 1);
                    sur_print(sbuf);
                    WAIT;
                    return (1);
                }
...
// mode: 1=replace 2=del  3=APU tai APUDEL
{
    ...
    if (s == NULL) // RS 13.2.2014
    {
        ...
        {
            strcpy(apu, "SYS");
            if (muste_strcmpi(parm[0], "SYSDEL") == 0) {
                mode = 2;
            } else {
                mode = 1;
            }
    ...
    if (mode == 1 && strchr(p, '=') == NULL) {
        return (1); // RS 1.11.2012
    }
    i = strlen(p) - 1;
    while (p[i] == ' ') {
        p[i--] = EOS;
...
static void op_tones() { muste_fixme("FIXME: TONES not implemented!\n"); }

static void op_pcopy() { muste_fixme("FIXME: PCOPY not implemented!\n"); }

static void op_delf() { muste_fixme("FIXME: DELF not implemented!\n"); }
...
int op_runr() // RS NEW
{
    ...
    strcpy(out, etmpd);
    strcat(out, "RUNR.CLP");
    strcpy(routtmp, etmpd);
    strcat(routtmp, "ROUT.TMP");
    ofile = muste_fopen(out, "wt");
    if (ofile == NULL) {
        sur_print("\nError opening RUNR.CLP!");
        WAIT;
        return (-1);
    }
    extern int move_ind;
    if ((muste_selection || move_ind) && g == 99) // RS ADD move_ind 20.9.2012
    {
        ...
        */
        ...
        if (g >= 3) {
            ...
            if (strcmp(word[3], "TO") == 0 && g > 4) {
                outfile = word[4];
            } else if (*word[3] != '/') {
                jo = edline2(word[3], 1, 1); // RS ADD if 25.2.2013
            }
        }
    }
    ...
    if (g == 100) {
        return (1); // RS 25.3.2013
    }

    if (outfile != NULL) {
        muste_evalsource_output("RUNR.CLP", outfile);
    } else if (jo > 0) {
        muste_evalsource_output("RUNR.CLP", routtmp);
    } else {
        muste_evalsource_output("RUNR.CLP", muste_rout);
    }
...
static void op_sbar() {
    ...
    if (g < 2) {
        if (muste_sbar) {
            muste_sbar = 0;
            muste_sprintf(str1, ".muste.scrollbar(FALSE)");
        } else {
            muste_sbar = 1;
        }
    } else {
...
static void op_menu() {
    ...
    if (g < 2) {
        muste_sprintf(str1, ".muste.menu(\"ONF\")");
    } else {
        muste_sprintf(str1, ".muste.menu(\"%s\")", parm[1]);
    }
...
static int op_words() // 23-24.8.2010
{
    ...
    if (g == 3) // #words
    {
        ...
        *word_chars = EOS;
        ...
        i = w_load_codes("CHARTYPE.BIN", code00);
        if (i < 0) {
            return (-1);
        }
                ...
                for (i = 0; i < k; ++i) {
                    ...
                    if (*word_chars != EOS) {
                        for (ii = 0; ii < strlen(word_chars); ++ii) {
                            f = 0;
                            p = s[i];
                            while ((p = strchr(p, word_chars[ii])) != NULL) {
                                ++p;
                                ++f;
                            }
        ...
        muste_sprintf((char *)x, "WORDS %s,%s / #words=%d #chars=%d (%d,%d,%d,%d)", word[1], word[2], n, k, n_letters,
                      n_digits, n_punct, n_special); // RS 4.2.2013 ADD (char *)
...
int muste_ediop(char *argv) {
    ...
    if (strcmp(OP, "ARIT") == 0) {
        muste_fixme("FIXME: ARIT (multiprecision artihmetics) not implemented!");
        return (1);
    }
    ...
    if (strcmp(OP, "LOADW") == 0) {
        codeconv = 2; // codeconv 8.4.2001
    }
    if (strcmp(OP, "SAVEW") == 0) {
        codeconv = 1;
    }

    muste_unix = 0; // 17.9.2008
    if (strcmp(OP, "LOADU") == 0) {
        muste_unix = 1;
        codeconv = 2;
    }
    if (strcmp(OP, "SAVEU") == 0) {
        muste_unix = 1;
        codeconv = 1;
    }
