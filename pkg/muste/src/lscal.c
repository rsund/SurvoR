...
static int polytope(double *x, double *py, int nn, double (*f)(double *), double *step, double alpha, double beta,
                    double gamma, int maxnf, double eps, int *pstop);
static int disp_notice(void);
static int poly_malloc(int n);
static int not_mem_for_polytope(void);
static int powell(double *p, double *xi, int n, double ftol, double *fret, double (*func)(double *));
...
static int frprmn(double *p, int n, double ftol, double *fret, double (*func)(double *),
...
void muste_lscal(char *argv) {
    ...
    if (n2 != n) {
        muste_sprintf(sbuf, "\n# of rows in %s (%d) not equal to %d!", word[2], n2, n);
        sur_print(sbuf);
        WAIT;
        return;
    }
    ...
    if (weight && n3 != n) {
        muste_sprintf(sbuf, "Dimension %d of weight matrix not equal to that of distance matrix %d", n3, n);
        WAIT;
        return;
    }
    ...
    output_open(eout);
    if (power_not_2 == 0) {
        muste_sprintf(sbuf, "Least-squares scaling for %d*%d dissimilarity (distance) matrix %s:", n, n, word[1]);
        print_line(sbuf);
    } else {
        muste_sprintf(sbuf, "Multidimensional scaling for %d*%d dissimilarity (distance) matrix %s:", n, n, word[1]);
        print_line(sbuf);
    ...
    if (m > 1) {
        ...
        muste_fieldcopy(x, clab, lc);
        x[lc] = EOS;
        poista_sp(x);
        strcat(sbuf, x);
        strcat(sbuf, ",");
        muste_fieldcopy(x, clab + lc, lc);
        x[lc] = EOS;
        poista_sp(x);
        strcat(sbuf, x);
...
static int test_symmetry(double *dd, int n) {
    ...
    for (i = 0; i < n; ++i) {
        for (j = 1; j < i; ++j) {
            if (fabs(dd[i + n * j] - dd[j + n * i]) / (1.0 + fabs(dd[i + n * j])) > 1e-5) {
                return (0);
            }
        }
    }
...
static double powersum(double *xx) {
    ...
    if (is_symmetric && fitpower != 1000.0) {
        s *= 2;
    }
    return (s);
}
...
static double mitta(char *x) {
    ...
    if (muste_strcmpi(x, "MAD") == 0 || muste_strcmpi(x, "ABS") == 0) {
        return (1.0);
    }
    if (muste_strcmpi(x, "MAX") == 0) {
        return (1000.0);
    }
    power = atof(x + 1);
    if (*x != 'L' || power <= 0.0) {
        muste_sprintf(sbuf, "\nUnknown or improper METRICS or CRITERION (%s)!", x);
        sur_print(sbuf);
        WAIT;
        return (2.0);
    }
    ...
    while (1) {
        ...
        for (j = 2; j < n + 1; ++j) {
            if (y11[j] > y11[jh]) {
                js = jh;
                jh = j;
            } else if (y11[j] > y11[js]) {
                js = j;
            } else if (y11[j] < y11[jl]) {
                jl = j;
            }
        }
        ...
        if (disp && ndisp > dispgap) {
            ...
            muste_sprintf(ss, "\nnf=%d f=%g (f0-f)/f0=%g", nf, y11[jl], dy);
            sur_print(ss);
            ++inote;
            if (inote >= 10) {
                disp_notice();
        ...
        for (i = 0; i < n; ++i) {
            x0[i] = (1 + alpha) * xc[i] - alpha * xx[jh * n + i];
        }
        y0 = (*f)(x0);
        ++nf;
        ...
        if (y0 < y11[jh]) {
            ...
            for (i = 0; i < n; ++i) {
                x00[i] = beta * xx[jh * n + i] + (1 - beta) * xc[i];
            }
        }
        y00 = (*f)(x00);
        ++nf;
...
static void mnbrak(double *ax, double *bx, double *cx, double *fa, double *fb, double *fc, double (*func)(double)) {
    ...
    *fc = (*func)(*cx);
    ...
    while (*fb > *fc) {
        ...
        r = (*bx - *ax) * (*fb - *fc);
        q = (*bx - *cx) * (*fb - *fa);
        u = (*bx) - ((*bx - *cx) * q - (*bx - *ax) * r) / (2 * SIGN(DMAX(fabs(q - r), TINY), q - r));
