...
static int Yvar, Pvar, Rvar, R2var, Vvar, x_vars, *Xvars, b_vars, *Bvars, s_vars, *Svars, t_vars, *Tvars;
static int prediction, residuals, squaredcorr, resvariance, coeffs, stddevs, tvalues;
...
void muste_movreg(char *argv) {
    ...
    Yvar = Pvar = Rvar = R2var = Vvar = -1;
    x_vars = b_vars = s_vars = t_vars = outvars = 0;
    ...
    if (Yvar < 0) {
        muste_kv_s_err("No regressand (Y)!");
        return;
    }
    if (x_vars == 0) {
        muste_kv_s_err("No regressors (X)!");
        return;
    }
...
static void mov_reg(void) {
    ...
    for (j = dat.l1, n = nn = 0L; j <= dat.l2; j++) {
        ...
        mat_mmt(xtxi, xx, p, p);
        for (i = 0, SSE = 0.0; i < span; i++) {
            x = res[i];
            SSE += x * x;
        }
        sigma2 = SSE / (span - p);
        for (i = 0; i < p; i++) {
            varb[i] = sqrt(sigma2 * xtxi[i + i * p]);
        }
        mat_sum(&ysum, Qy->A, span, 1);
        ysum /= span;
        for (i = 0, SST = 0.0; i < span; i++) {
            x = Qy->A[i] - ysum;
            SST += x * x;
        }
        Rsquare = 1.0 - SSE / SST;
...
static void save_missvals(int obs) {
    ...
    if (prediction) {
        data_save(&dat, obs, Pvar, MISSING8);
    }
    if (residuals) {
        data_save(&dat, obs, Rvar, MISSING8);
    }
    if (squaredcorr) {
        data_save(&dat, obs, R2var, MISSING8);
    }
    if (resvariance) {
        data_save(&dat, obs, Vvar, MISSING8);
    }

    if (coeffs) {
        for (i = 0; i < b_vars; i++) {
            data_save(&dat, obs, Bvars[i], MISSING8);
        }
    }
    if (stddevs) {
        for (i = 0; i < s_vars; i++) {
            data_save(&dat, obs, Svars[i], MISSING8);
        }
    }
    if (tvalues) {
        for (i = 0; i < t_vars; i++) {
            data_save(&dat, obs, Tvars[i], MISSING8);
        }
    }
}
...
static void update_varnames(void) {
    ...
    if (prediction) {
        put_varname(Pvar, "~Predicted values from MOVREG");
    }
    if (residuals) {
        put_varname(Rvar, "~Residuals from MOVREG");
    }
    if (squaredcorr) {
        put_varname(R2var, "~R^2 values from MOVREG");
    }
    if (resvariance) {
        put_varname(Vvar, "~Residual variances from MOVREG");
    }
    ...
    if (coeffs) {
        ...
        for (i = 1; i < b_vars; i++) {
            ...
            sbuf[9] = '\0';
            trim2(sbuf);
            muste_sprintf(comment, "~Regression coefficients of %s from MOVREG", sbuf);
    ...
    if (tvalues) {
        for (i = 0; i < t_vars; i++) {
            ...
            sbuf[9] = '\0';
            trim2(sbuf);
            muste_sprintf(comment, "~t-values of %s from MOVREG", sbuf);
