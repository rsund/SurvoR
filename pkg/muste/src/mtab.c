...
static int rc_pros(int iyseq, int gnam[], int i_colseqs, int i_rowseqs, int required[], long *nobsdiv, double *divsum,
                   int indcol);
static int ijth_cellstat(long *nobs, double *sumy, double *ssumy, double *ymin, double *ymax, long *n_grandtotal,
                         double *s_grandtotal, long *n_coltotal, long *n_rowtotal, double *s_coltotal,
                         double *s_rowtotal, long *n_tabletotal, double *s_tabletotal, int ithrow, int jthcol,
                         int ind_oldrow, int *ynam);
static int cellstat(long *nobs, double *sumy, double *ssumy, int ynam, int gnam[], int gseqs[], double *ymin,
                    double *ymax, long *n_grandtotal, double *s_grandtotal, long *n_coltotal, long *n_rowtotal,
                    double *s_coltotal, double *s_rowtotal, long *n_tabletotal, double *s_tabletotal, int i_colseqs,
                    int i_rowseqs);
...
void muste_mtab(int argc, char *argv[]) {
    ...
    if (html_start > 0) {
        ...
        if (l > 1) {
            strcat(xx, "BGCOLOR=");
            strcat(xx, bodybgcolor);
        }
        l = strlen(bodytextcolor);
        if (l > 1) {
            strcat(xx, "TEXT=");
            strcat(xx, bodytextcolor);
        }
        l = strlen(bodylinkcolor);
        if (l > 1) {
            strcat(xx, "LINK=");
            strcat(xx, bodylinkcolor);
        }
        l = strlen(bodyvlinkcolor);
        if (l > 1) {
            strcat(xx, "VLINK=");
            strcat(xx, bodyvlinkcolor);
        }
        l = strlen(bodyalinkcolor);
        if (l > 1) {
            strcat(xx, "ALINK=");
            strcat(xx, bodyalinkcolor);
        }
        strcat(xx, "><A NAME=");
        strcat(xx, htmlname);
        strcat(xx, "></A>");
...
notenough:; /* i=mtb_message(1,10,1);  */
    ...
    write_string("  Press any key!  ", 18, mess_char, 12, 1);
    sur_getch();
errclose:
    goto dclose;
}
...
static int meanscmp(char *argv[]) {
    ...
    if (g < 2) {
        write_string(" * Err009: Correct form: MTAB <data>,L ", 39, mess_char, 11, 1);
        write_string("   Press any key! ", 18, mess_char, 12, 1);
        sur_getch();
        return (-1);
    }
    ...
    if (n_specs < 0) {
        write_string("* Err002: Not enough space for specifications! ", 47, mess_char, 11, 1);
        sur_getch();
        return (-1);
    }
    /*                                                              */
    ...
    if (i >= 0) {
        strcpy(bodybgcolor, spb[i]);
        html_on = 1;
    }
    strcpy(bodytextcolor, " ");
    i = a_spfind("BODYTEXTCOLOR");
    if (i >= 0) {
        strcpy(bodytextcolor, spb[i]);
        html_on = 1;
    }
    ...
    if (i >= 0) {
        strcpy(bodylinkcolor, spb[i]);
        html_on = 1;
    }
    strcpy(bodyvlinkcolor, " ");
    i = a_spfind("BODYVLINKCOLOR");
    if (i >= 0) {
        strcpy(bodyvlinkcolor, spb[i]);
        html_on = 1;
    }
    ...
    i = a_spfind("TBGCOLOR");
    if (i >= 0) {
        strcpy(tablbgcolor, spb[i]);
        html_on = 1;
    }
    strcpy(tablhdcolor, " ");
    i = a_spfind("THDCOLOR");
    if (i >= 0) {
        strcpy(tablhdcolor, spb[i]);
        html_on = 1;
    }
    strcpy(tablvcolor, " ");
    i = a_spfind("TBVCOLOR");
    if (i >= 0) {
        strcpy(tablvcolor, spb[i]);
        html_on = 1;
    }
    strcpy(htmlremarks, " ");
    i = a_spfind("HTMLREMARK");
    if (i >= 0) {
        strcpy(htmlremarks, spb[i]);
        html_on = 1;
    ...
    if (i >= 0) {
        ...
        strcat(html_file, xx);
    avaa:
        html_stream = muste_fopen(html_file, "w");
        if (html_stream == NULL) {
            ijk = muste_sprintf(sbuf, "* Failed to open the file %.32s ", html_file);
            write_string(sbuf, ijk, mess_char, 22, 1);
            sur_getch();
            htmlfile_open = 0;
        } else {
            htmlfile_open = 1;
    ...
    i = a_spfind("HREFVBLE");
    if (i >= 0) {
        hrefvble = varfind2(&d, spb[i], 1);
    }
    i = a_spfind("HREFBASE");
    if (i >= 0) {
        strcpy(hrefbase, spb[i]);
    }
    j = strlen(hrefbase);
    for (i = j - 1; i >= 0; i--) {
        if (hrefbase[i] != ' ') {
    ...
    for (i = 0; i < ngv; i++) {
        k = comp_format(grp_fmt, i, &grp_width[i], &grp_des[i], &format_given);
        if (k < 0) {
            return (-1);
        }
    ...
    if (n_of_nests > 0) {
        for (i = 0; i <= n_of_nests; i++) {
            k = comp_format(gnest_fmt, i, &nest_width[i], &nest_des[i], &format_given);
        }
    }
...
nogrping:
    ...
    if (i < 0) {
        ...
        if (j < 0) {
            j = anovbl_spfind_yfmt(&ny, "VARIABLE", iy, y_fmt, 1);
        }
        if (j >= 0) {
            i = j;
        }
    ...
    for (i = 0; i < ny; i++) {
        k = comp_format(y_fmt, i, &y_width[i], &y_des[i], &format_given);
        if (k < 0) {
            return (-1);
        }
        strcpy(y_title + i * 41, " ");
        ...
        if (k >= 0) {
            ...
            if (html_on > 0) {
                ijk = replace_aakkos(y_title + i * 41);
            }
        } else {
...
lab708:
    ...
    if (i >= 0) {
        ...
        if (k == 1 && ny > 0) {
            if (y_used == 0 && col_given == 1) {
                write_string(" *Err005: Object variables not mentioned in COL or ROW specifications! ", 71, mess_char,
                             11, 1);
                sur_getch();
                return (-1);
            }
    ...
    return (1);
notenough:;
    write_string(" *Err001: Not enough memory!  Press any key! ", 45, mess_char, 11, 1);
    sur_getch();
    return (-1);
}
/*                                          */
...
static int anovbl_spfind_fmt(int *npar, char *parname, int *parlist, char *flist, int indik) {
    ...
    *npar = 0;
    ...
    if (i >= 0) {
        ...
        *npar = j;
        if (j > 8) {
            write_string(" *Err: At most 8 grouping variables", 35, mess_char, 11, 1);
            sur_getch();
            return (-2);
        }
        for (i = 0; i < *npar; i++) {
        ...
        errlab:;
            write_string("*Err008: Left or right parenthesis is missing! ", 47, mess_char, 11, 1);
            sur_getch();
            return (-2);
        lab2:
            n_f = k2 - k1;
        ...
        nestlab:
            ...
            goto errlab;
        lab3:
            muste_fieldcopy(vble, x2 + k1, k2 - k1);
            vble[k2 - k1] = '\0';
            ivble = varfind2(&d, vble, 1);
            if (ivble < 0) {
                return (-3);
...
static int mnsdefs(void) {
    ...
    for (j = 0; j < ngv; j++) {
        ...
        if (i >= 0) {
            ...
            if (html_on > 0) {
                kk = replace_aakkos(y_title + i * 41);
            }
        } else {
        ...
        i = (int)spfind(yy);
        if (i < 0) {
            kk = muste_sprintf(sbuf, "Checking the grouping structure of the variable %.8s ...", yy);
            ...
            for (iii = d.l1; iii <= d.l2; iii++) {
                ...
                if (nested[j] > 0) {
                    ...
                    for (i2 = 0; i2 < pway_nested; i2++) {
                        ...
                        i2long = i2;
                        i2long = i2long * 33;
                        jfnd = read_obs(iii, ij2, igv_vartype_nested[nestaddr], &nested_xval[i2], &nested_area[i2long]);
                        if (jfnd == 0) {
                            goto nexti;
                        }
                    }
                    ...
                    for (i2 = 0; i2 < pway_nested; i2++) {
                        ...
                        if (igv_vartype_nested[nestaddr] == 0) {
                            ...
                            for (i_str = 0; i_str < label_length; i_str++) {
                                i2long = i2;
                                i2long = i2long * 33;
                                if (nested_area[i2long + label_length - i_str - 1] != ' ') {
                                    goto lstbl;
                                }
                            }
                        lstbl:
                            if (label_length - i_str > str_maxlen_nest[i2]) {
                ...
                if (nested[j] == 0) {
                    ...
                    notblank:
                        for (i_str = 0; i_str < label_length; i_str++) {
                            if (xx[label_length - i_str - 1] != ' ') {
                                goto lstblnk;
                            }
                        }
                    lstblnk:
                        if (label_length - i_str > str_maxlen) {
                            str_maxlen = label_length - i_str;
                        }
            ...
            if (nested[j] == 0) {
                ...
                iij = nested_maxn * j2;
                jjj = iij * label_length1;
                charsort(&labels_nested[jjj], &values_nested[iij], &igv_vartype_nested[j2], m, kk_nest, nested_maxn,
        ...
        if (nested[j] == 0) {
            if (igv_vartype[j] <= 0) {
                ...
                for (j2 = 1; j2 < kk; j2++) {
                    if (values[j2 + jmaxn] > max_gvals[j]) {
                        max_gvals[j] = values[j2 + jmaxn];
                    }
                }
            }
        } /* if nested=0    */
    ...
    return (1);
errmaxn:
    i = muste_sprintf(sbuf, "Variable %.8s has too many groups", d.varname[ij]);
            ...
            for (j = 0; j < pway; j++) {
                ...
                if (nested[j] > 0) {
                    ...
                    for (i2 = 0; i2 < pway_nested; i2++) {
                        ...
                        jfnd = (int)read_obs(iii, ij2, igv_vartype_nested[nestaddr], &nested_xval[i2],
                                             &nested_area[i2 * 33]);
                        if (jfnd == 0) {
                            goto rjctd;
                        }
            ...
            for (m = 0; m < ny; m++) {
                ...
                if (ivariables == 0) {
                    ...
                    if (ivbles_type[m] == 0) {
                        ...
                        if (len_alphaval > label_length) {
                            len_alphaval = label_length;
                        }
                        l2 = strlen(xx);
                        xx[l2] = ' ';
                        xx[label_length] = '\0';
                ...
                if (min_yvals[i1] > yval) {
                    min_yvals[i1] = yval;
                }
                if (max_yvals[i1] < yval) {
                    max_yvals[i1] = yval;
                }
                ...
                if (total_maxy[m] < yval) {
                    total_maxy[m] = yval;
                }
                if (total_miny[m] > yval) {
                    total_miny[m] = yval;
                }
...
static void charsort(char *chrarr, double *xarr, int *types, int m, int n, int lngth_array, int *ndtop,
                     int *change_point) {
    ...
    for (i = 0; i < n; i++) {
        if (types[k] <= 0) {
            ...
            if (icomp != 0) {
                muste_fieldcopy(ccc, chrarr + i2, label_length);
                j++;
                if (j > 1) {
                    change_point[j - 2] = i;
                }
    ...
    for (j = 0; j < ny; j++) {
        ...
        if (i < 0) {
            if (ivbles_type[j] == 0) {
                ...
                write_string("*Err011: String indicator variable definition missing ", 54, mess_char, 11, 1);
                sur_getch();
                return (-1);
            }
        }
        if (i >= 0) {
            ...
            if (real_val == 0) {
                ...
                muste_fieldcopy(ccapu, nambuff[0], l);
                ccapu[l] = ' ';
...
/*                                                    */
nextw:
    i = get_next_word(wrd, source, fchar, &lchar, l_source);
    if (i <= 0) {
        goto nomore;
    }
    ioper = 0;
...
next:
    if (ioper == 0) {
        *vble_type = 1;
        ...
        if (i1 == 0) {
            muste_sprintf(sbuf, "\nVariable %.8s should be defined either in VARIABLES or GROUPING/CLASS", wrd);
        }
        goto errwait;
    laby:
        *vble_type = 2;
...
nomore:
    l_oper++;
    i = perform_operlist(result_address, &oper_list[0], &vble_seqs[0], l_oper);
    if (i >= 0) {
        return (1);
    }
errwait:
...
int put_into_buffer(int *address, int vbltype, int vblseqno) {
    ...
    *address = next_buffer;
    ...
    if (vbltype == 1) {
        ...
        kpoint = 0;
        if (nested[igvno] > 0) {
            nestaddr = address_nested[igvno * pway_nested];
        }
        for (i = 0; i < length; i++) {
            gnames[i] = vblseqno;
            gsymbols[i] = i;
...
static int meanspr(void) {
    ...
    for (i = 0; i < 24; i++) {
        ...
        if (k == 3) {
            strcpy(yy, "Deviations");
            ltext = 10;
        }
        if (k == 4) {
            strcpy(yy, "Frequencies");
            ltext = 11;
        }
        if (k == 5) {
            strcpy(yy, "Minimums");
        ...
        if (k == 7) {
            strcpy(yy, "Std. err. of means");
            ltext = 18;
        }
        if (k == 8) {
            strcpy(yy, "Percentages of freq.");
            ltext = 20;
        }
        if (k == 9) {
            strcpy(yy, "Percentages of sums");
            ltext = 19;
        }
        if (k == 10) {
            strcpy(yy, "Column percentages of freq.");
            ltext = 27;
        }
        if (k == 11) {
            strcpy(yy, "Column percentages of sums");
            ltext = 26;
        }
        if (k == 12) {
            strcpy(yy, "Row percentages of freq.");
            ltext = 24;
        }
        if (k == 13) {
            strcpy(yy, "Row percentages of sums");
            ltext = 23;
        }
        if (k == 14) {
            strcpy(yy, "Table percentages of freq.");
            ltext = 26;
        }
        if (k == 15) {
            strcpy(yy, "Table percentages of sums");
            ltext = 25;
        }
        if (k == 16) {
            strcpy(yy, "Columnwise cumulative frequencies");
            ltext = 32;
        }
        if (k == 17) {
            strcpy(yy, "Rowwise cumulative frequencies");
            ltext = 30;
        }
        if (k == 18) {
            strcpy(yy, "Columnwise cumulative sums");
            ltext = 25;
        }
        if (k == 19) {
            strcpy(yy, "Rowwise cumulative sums");
            ltext = 23;
        }
        if (k == 20) {
            strcpy(yy, "Cumulative column percentages of frequencies");
            ltext = 45;
        }
        if (k == 21) {
            strcpy(yy, "Cumulative row percentages of frequencies");
            ltext = 42;
        }
        if (k == 22) {
            strcpy(yy, "Cumulative column perecntages of sums");
            ltext = 38;
        }
        if (k == 23) {
            strcpy(yy, "Cumulative row percentages of sums");
            ltext = 35;
        }
        if (j == 0) {
            strcpy(xx, yy);
            ...
            if (lt + ltext > 72) {
                if (html_on == 0 || strlen(tabltitle) < 2) {
                    foutput(0);
                }
                lt = ltext;
                strcpy(xx, yy);
                        ...
                        for (ik3 = 0; ik3 < nwnd[2]; ik3++) {
                            ...
                            for (ik2 = 0; ik2 < nwnd[1]; ik2++) {
                                ...
                                for (ik1 = 0; ik1 < nwnd[0]; ik1++) {
                                    ...
                                    if (nbackvbles > 0) {
                                        i = (int)mns_prbackv(nbackvbles, first_back, pway);
                                    }
                                    i = (int)mnsprmdv();
                                    if (i < 0) {
                                        return (-1);
...
static int vbl_frmt(int i_grp, int index, int *width, int *des) {
    ...
    indsums = 0;
    if (ny > 0) {
        if (prntopt[1] > 0 || prntopt[17] > 0 || prntopt[18] > 0) {
            indsums = 1;
        }
    }
    if (i_grp == 2) {
        pfrmt = (char *)gnest_fmt;
    }
    if (i_grp == 1) {
        pfrmt = (char *)grp_fmt;
    }
    ...
    /*     width/des = -1 means unknown or default         */
    if (i != 0 && l > 0) {
        return (0); /* format given         */
    }
    if (i_grp == 2) {
        if (igv_vartype_nested[index] > 0) {
            *width = igv_vartype_nested[index];
            *des = 0;
            if (*width < 8) {
                *width = 8;
            }
...
static int mnsprmdv(void) {
    ...
    strcpy(zzname, "M   S   D   F   MIN MAX SEM %F  %S  C%F C%S R%F R%S T%F T%S CCF CRF CCS CRS");
    strcat(zzname, " CR%FCC%FCR%SCC%S");
    i1 = 0;
    if (*col_address >= 0) {
        p_mnspr = (char *)buffer[*col_address];
        col_ways = *(int *)p_mnspr;
        p_mnspr += sizeof(int);
        col_lngth = *(int *)p_mnspr;
        p_mnspr += sizeof(int);
        col_vbltype = *(int *)p_mnspr;
        p_mnspr += sizeof(int);
        col_objvbles = (int *)p_mnspr;
        p_mnspr += col_lngth * sizeof(int);
        if (col_vbltype != 2) {
            col_gvbles = (int *)p_mnspr;
            p_mnspr += col_ways * col_lngth * sizeof(int);
    ...
    if (*row_address >= 0) {
        ...
        row_ways = *(int *)p_mnspr;
        p_mnspr += sizeof(int);
        row_lngth = *(int *)p_mnspr;
        p_mnspr += sizeof(int);
        row_vbltype = *(int *)p_mnspr;
        p_mnspr += sizeof(int);
        row_objvbles = (int *)p_mnspr;
        p_mnspr += row_lngth * sizeof(int);
        if (row_vbltype != 2) {
            row_gvbles = (int *)p_mnspr;
            p_mnspr += row_lngth * row_ways * sizeof(int);
        ...
        rowcont: // p_mnspr=(char *)muste_malloc(3*row_lngth*sizeof(int)); // RS CHA
            ...
            for (j = 0; j < row_lngth; j++) {
                ...
                row_gvbles[j] = igv[i];
                row_objvbles[j] = 0;
                if (ny > 0 && y_used == 0 && col_vbltype == 1) {
                    row_objvbles[j] = iy[0];
                }
            }
            row_vbltype = 1;
            if (y_used == 0 && col_vbltype == 1 && ny > 0) {
                row_vbltype = 3;
            }
        }
    }
...
jatkatabl:;
    if (html_on > 0) {
        for (i = 0; i < row_ways; i++) {
            ...
            if (col_ways < 1) {
                xx[12] = '1'; /* jos ei col-mtjaa niin rowspan=1 */
            }
            for (k = 0; k < ngv; k++) {
    ...
    for (i = 0; i < col_ways; i++) {
        ...
        for (j = firsti; j < lasti; j++) {
            if (col_gvbles[j + i * col_lngth] < 0) {
                continue;
            }
            for (i1 = 0; i1 < pway; i1++) {
                if (igv[i1] == col_gvbles[j + i * col_lngth]) {
                    break;
                }
            }
            k = 0;
    ...
    for (i = 0; i < row_lngth; i++) {
        ...
        if (i > 0) {
            for (j = 0; j < row_ways; j++) {
                ij = i + j * row_lngth;
                if (row_gvbles[ij] != row_gvbles[ij - 1]) {
                    same_row = 0;
                    break;
                }
            }
                ...
                if (nested[k] > 0 && row_gvbles[ijk] >= 0) {
                    for (i1 = pway_nested - 1; i1 >= 0; i1--) {
                        ...
                        if (l > nest_width[nestaddr]) {
                            l = nest_width[nestaddr];
                            yy[l] = '\0';
                        }
                        if (html_on > 0) {
                            strcat(xx, "<TD ALIGN=LEFT>");
                            strcat(xx, gtitle + k * 128);
                            strcat(xx, "</TD>");
                        } else {
                            muste_fieldcopy(xx + ix1, yy, l);
                            if (i1 > 0 || j < row_ways - 1) {
                                xx[ix1 + l] = ' ';
                            }
        ...
        for (rowstat = 0; rowstat < col_stat; rowstat++) {
            ...
            j = 1;
            if (k == 4 || k == 8 || k == 10 || k == 12 || k == 14 || k == 16 || k == 17) {
                j = 0;
            }
            if (k == 20 || k == 21) {
                j = 0;
            ...
            if (j == 1) {
                if (col_vbltype < 2 && row_vbltype < 2) {
                    continue; /* only frequencies possible */
                }
                if (ivariables == 1 && row_vbltype < 2) {
                    continue;
                }
            }
            indij++;
            ...
            if (iprst == 1) {
                for (j = 0; j < row_ways; j++) {
                    ...
                    if (row_symbols[ijk - 1] != row_symbols[ijk]) {
                        same_subrow = 0;
                    }
                    if (j == row_ways - 1 && row_symbols[ijk] >= 9999) {
                        total_row = 1;
                    }
                    if (same_subrow == 0) {
                        if (row_gvbles[ijk] >= 0) {
                            ...
                            if (nested[k] > 0) {
                                ...
                                for (i1 = pway_nested - 1; i1 >= 0; i1--) {
                                    ...
                                    if (outermost == 1) {
                                        strcpy(former_outlab, zz);
                                    }
                                    strcpy(yy + iix1, zz);
                                    yy[iix1 + l] = ' ';
                                    iix1 += nest_width[nestaddr];
                                    if (i1 > 0) {
                                        iix1++;
                                    }
                                }
            ...
            for (j = firsti; j < lasti; j++) {
                ...
                ijth_cellstat(&nobs, &a, &b, &ymin, &ymax, &n_grandtotal, &s_grandtotal, &n_coltotal, &n_rowtotal,
                              &s_coltotal, &s_rowtotal, &n_tabletotal, &s_tabletotal, i, j, indij, &ynam);
                ...
                for (k = 0; k < row_stat; k++) {
                    ...
                    if (ij == 16 || ij == 17) {
                        icode = 2;
                        if (ij == 17) {
                            l = dep_frmt(i, j, mijk, icode, xdev, cum_cnobs);
                        }
                        if (ij == 16) {
                            l = dep_frmt(i, j, mijk, icode, xdev, cum_rnobs);
                        }
                    }
                    /*         cumulative sums                            */
                    if (ij == 18 || ij == 19) {
                        ...
                        if (ij == 19) {
                            l = dep_frmt(i, j, mijk, icode, cum_csumy, cum_cnobs);
                        }
                        if (ij == 18) {
                            l = dep_frmt(i, j, mijk, icode, cum_rsumy, cum_rnobs);
                        }
                    }
                    if (ij == 5) {
                    ...
                    if (ij >= 10 && ij < 16) {
                        ...
                        ndv = nobs;
                        if (ynam < 0) {
                            if (ij == 11 || ij == 13 || ij == 15) {
                                icode = 0;
                            }
                        }
                        if (ij == 10 && n_coltotal > 0L) {
                            prcnt = 100.0 * nobs / n_coltotal;
                        }
                        if (ij == 11 && s_coltotal != 0.0) {
                            prcnt = 100.0 * a / s_coltotal;
                        }
                        if (ij == 12 && n_rowtotal > 0L) {
                            prcnt = 100.0 * nobs / n_rowtotal;
                        }
                        if (ij == 13 && s_rowtotal != 0.0) {
                            prcnt = 100.0 * a / s_rowtotal;
                        }
                        if (ij == 14 && n_tabletotal > 0L) {
                            prcnt = 100.0 * nobs / n_tabletotal;
                        }
                        if (ij == 15 && s_tabletotal != 0.0) {
                            prcnt = 100.0 * a / s_tabletotal;
                        }
                        l = dep_frmt(i, j, mijk, icode, prcnt, nobs);
                    }
...
static int mns_labcpy(int k, int j, int width, int i_col) {
    ...
    if (los == 9999) {
        los9 = strlen(ltotal);
        if (los9 > label_length) {
            los9 = label_length;
        }
        muste_fieldcopy(ccapu, ltotal, los9);
        ccapu[los9] = '\0';
    } else {
        if (j > 0) {
            los += j * maxn;
        ...
        muste_fieldcopy(ccapu, labls + los9, label_length);
        ccapu[label_length] = '\0';
    }
    l = strlen(ccapu);
    ii = strncmp(ccapu, "                                      ", l);
...
static int mns_nest_labcpy(char zz[], int k, int j, int width, int i_col, int outermost) {
    ...
    los = k; // i1=0;
    linspace(ccapu, 35);
    if (strlen(ltotal) > label_length) {
        ltotal[label_length] = '\0';
    }
    if (los == 9999) {
        strcpy(ccapu, ltotal);
    }
    ...
    *ynam = -1;
    ...
    if (ind_oldrow == 1) {
        ...
        *s_tabletotal = dstarea[jthcol + statspacesize * 7];
        ...
        i = cellstat(nobs, sumy, ssumy, *ynam, gnam, gseqs, ymin, ymax, n_grandtotal, s_grandtotal, n_coltotal,
                     n_rowtotal, s_coltotal, s_rowtotal, n_tabletotal, s_tabletotal, i_colseqs, i_rowseqs);
        lstarea[jthcol] = *nobs;
...
*/
{
    ...
    *s_grandtotal = 0.0;
    ...
    if (cumulative_results == 1) {
        ...
        for (k = 0; k < ngv; k++) {
            if (gnam[i_colseqs] == igv[k]) {
                break;
            }
        }
        rc_cols[k] = 1;
        for (k = 0; k < ngv; k++) {
            if (gnam[i_rowseqs] == igv[k]) {
                break;
            }
        }
        rc_cols[k] += 2;
    ...
    if (percents_results == 0) {
        goto overprcnts;
    }
    /*    percents                                                       */
    if (prntopt[9] > 0 || prntopt[10] > 0 || prntopt[20] > 0 || prntopt[22] > 0) {
        i = rc_pros(iyseq, gnam, i_colseqs, i_rowseqs, required, n_coltotal, s_coltotal, 1);
    }
    if (prntopt[11] > 0 || prntopt[12] > 0 || prntopt[19] > 0 || prntopt[21] > 0) {
        i = rc_pros(iyseq, gnam, i_colseqs, i_rowseqs, required, n_rowtotal, s_rowtotal, 0);
    }
    if (prntopt[13] > 0 || prntopt[14] > 0) {
        i = rc_pros(iyseq, gnam, i_colseqs, i_rowseqs, required, n_tabletotal, s_tabletotal, 2);
    }
overprcnts:
    for (j = 0; j < ntlo; j++) {
        jj = j;
        ...
        for (k = 0; k < pway; k++) {
            ...
            if (required[k] > 9999) {
                ...
                if (i_nest > 0) {
                    if (ind[k] >= change_points[nestaddr + i_nest - 1] && ind[k] < change_points[nestaddr + i_nest]) {
                        continue;
                    } else {
                        goto rejected;
                    }
        ...
        if (notsame == 0) {
            ...
            *n_grandtotal = total_lkm[i_gtot];
            if (ny > 0 && ivariables == 0 && ynam >= 0) {
                *sumy += sz[ijk];
                *s_grandtotal = total_sums[i_gtot];
                *ssumy += szy[ijk];
                if (min_yvals[ijk] <= max_yvals[ijk]) {
                    ...
                    if (*ymin > min_yvals[ijk]) {
                        *ymin = min_yvals[ijk];
                    }
                    if (*ymax < max_yvals[ijk]) {
                        *ymax = max_yvals[ijk];
                    }
                }
            }
        ...
        if (cumulative_results > 0) {
            ...
            if (ny > 0 && ivariables == 0 && ynam >= 0) {
                ...
                if (i_col == 1 && min_yvals[ijk] <= max_yvals[ijk]) {
                    ...
                    if (cum_cymin > min_yvals[ijk]) {
                        cum_cymin = min_yvals[ijk];
                    }
                    if (cum_cymax < max_yvals[ijk]) {
                        cum_cymax = max_yvals[ijk];
                    }
                }
                if (i_row == 1 && min_yvals[ijk] <= max_yvals[ijk]) {
                    ...
                    if (cum_rymin > min_yvals[ijk]) {
                        cum_rymin = min_yvals[ijk];
                    }
                    if (cum_rymax < max_yvals[ijk]) {
                        cum_rymax = max_yvals[ijk];
                    }
                }
            }
...
static int rc_pros(int iyseq, int gnam[], int i_colseqs, int i_rowseqs, int required[], long *nobsdiv, double *divsum,
    ...
    *divsum = 0.0;
    ...
    if (ivariables == 1) {
        indprs = 0;
        if (row_vbltype > 1 && indcol != 1) {
            indprs = 1;
        }
        if (col_vbltype > 1 && indcol != 0) {
            indprs = 1;
        }
    }
    if (indprs == 1) {
...
static int get_column_width(int findx, int lindx, int index2, int i_col, int i_y) {
    ...
    for (index = findx; index <= lindx; index++) {
        if (i_y == 1) {
            if (i_col == 1) {
                k = col_objvbles[index];
            }
            if (i_col == 0) {
                k = row_objvbles[index];
            }
            if (k < 0) {
                wdth = -1;
                goto nextj;
        ...
        if (i_y == 0) {
            if (i_col == 1) {
                k = col_gvbles[index + index2 * col_lngth];
            }
            if (i_col == 0) {
                k = row_gvbles[index + index2 * row_lngth];
            }
            if (k < 0) {
                wdth = -1;
                goto nextj;
        ...
        for (mm = 0; mm < pway_nested; mm++) {
            ...
            if (igv_vartype_nested[iaddr] <= 0) {
                ...
                if (cutp_nested[iaddr] == 1) {
                    if (xreal[mm] <= values_nested[valos] + xvert[iaddr]) {
                        goto labfnd;
                    }
                } else {
                    valx = values_nested[valos] - xreal[mm];
                    valx = fabs(valx);
                    if (valx < xvert[iaddr]) {
                        goto labfnd;
                    }
...
static int comp_xval(int j, double xval, int ndj, int jmaxn) {
    ...
    if (igv_vartype[j] <= 0) {
        for (imk = 0; imk < ndj; imk++) {
            valos = imk + jmaxn;
            if (cutp[j] == 1) {
                if (xval <= values[valos] + gxvert[j]) {
                    goto labfnd;
                }
            } else {
                valx = values[valos] - xval;
                valx = fabs(valx);
                if (valx < gxvert[j]) {
                    goto labfnd;
                }
...
int dep_frmt(int ithrow, int jthcol, int width, int i_code, double xval, long longval) {
    ...
    if (ifmt == 0 && (i_code == 1 || i_code == 3)) {
        ...
        if (i_code == 1) {
            ...
            if (l1 >= 6) {
                if (pctdec == 0) {
                    fconv(xval, "123456", ccapu);
                }
                if (pctdec == 1) {
                    fconv(xval, "1234.1", ccapu);
                }
                if (pctdec == 2 || (pctdec > 2 && l1 < 7)) {
                    fconv(xval, "123.12", ccapu);
                }
                if (pctdec == 3 || (pctdec > 3 && l1 == 7)) {
                    fconv(xval, "123.123", ccapu);
                }
                if (pctdec == 4 || (pctdec > 4 && l1 == 8)) {
                    fconv(xval, "123.1234", ccapu);
                }
                if (pctdec == 5 || (pctdec > 5 && l1 == 9)) {
                    fconv(xval, "123.12345", ccapu);
                }
            }
        }
        if (l1 > 8) {
