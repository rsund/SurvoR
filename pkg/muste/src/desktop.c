...
static int WHEREmain(void) {
    muste_fixme("\nFIXME: WHERE not yet implemented");
    return 1;
}
static int DMmain(void) {
    muste_fixme("\nFIXME: DM not yet implemented");
    return 1;
}
...
static Types1 *ty1, types[] = {{"DIR", "CD", 0, 1},        {"EDT", "SHOW", 1, 1},     {"SVO", "FILE_SHOW", 1, 1},
                               {"MAT", "/MATSHOW", 1, 1},  {"M", "/MATSHOW", 1, 0},   {"TUT", "/TUTSHOW", 0, 1},
                               {"BIN", "/CODESHOW", 0, 1}, {"TCH", "/TCHSHOW", 0, 1}, {"EMF", "GPLOT_FILE", 1, 1},
                               {"PDF", "/OPEN", 0, 0},     {"DOC", "/OPEN", 0, 0},    {"XLS", "/OPEN", 0, 0},
                               {"PPT", "/OPEN", 0, 0},     {"JPG", "/OPEN", 0, 0},    {"JPEG", "/OPEN", 0, 0},
                               {"GIF", "/OPEN", 0, 0},     {"HTM", "/OPEN", 0, 0},    {"HTML", "/OPEN", 0, 0},
                               {"PNG", "/OPEN", 0, 0},     {"RTF", "/OPEN", 0, 0},    {"SAV", "/OPEN", 0, 0},
                               {"ZIP", "/OPEN", 0, 0},     {"BMP", "/OPEN", 0, 0},    {"TIF", "/OPEN", 0, 0},
                               {"TIFF", "/OPEN", 0, 0}};
...
static Types2 *ty2, ftypes[] = {{".TUT", TUTSHOW}, {".EDT", SHOW}, {".SVO", FSHOW}, {".MAT", MATSHOW}, {".M", MATSHOW}};
...
static unsigned int recursive, continuous, show_command, exact_search, columns_given, field_ok, files_found, ignorecase,
    search_first, search_file, search_DD, found, search_comment, search_shadows;
static unsigned int matches, first_matches, files_total, lines_total;
static char bigbuffer[LLENGTH], shadow_buffer[LLENGTH], ahead_buffer[LLENGTH], orig_buffer[LLENGTH], check[LNAME / 2],
...
//  DLast=NULL;
    ...
    if (!muste_strcmpi(word[0], "INDEX")) {
        INDEXmain();
    } else if (!muste_strcmpi(word[0], "DIR")) {
        op_dir();
    } else {
        edread(userline, r1 + r - 1); /* command line saved */
        disable_softkeys();
        if (!muste_strcmpi(word[0], "DD")) {
            DDmain();
        } else if (!muste_strcmpi(word[0], "DM")) {
            DMmain();
        } else if (!muste_strcmpi(word[0], "WHERE")) {
            WHEREmain();
        } else if (!muste_strcmpi(word[0], "TREE")) {
            tree();
        } else if (!muste_strcmpi(word[0], "SEARCH")) {
            SEARCHmain();
        }
        enable_softkeys();
...
static void tut_error(int code) /* 28.8.2000 (for SM, softkeys) */
{
    switch (code) {
    case 1:
        strcpy(tut_info, "___@1@INDEX@No files found!"); // 6.9.2012/KV "\254"s corrected to "_"
        break;
    case 11:
        strcpy(tut_info, "___@1@DD@No files found!");
        break;
    default:
        break;
    }
...
static int get_date(char *format) {
    ...
    if (!strcmp(format, "DD.MM.YY")) {
        return 1;
    }
    if (!strcmp(format, "DD/MM/YY")) {
        return 2;
    }
    if (!strcmp(format, "DDMMYY")) {
        return 3;
    }
    if (!strcmp(format, "YYMMDD")) {
        return 4;
    }
    if (!strcmp(format, "MM/DD/YY")) {
        return 5;
    }
...
static int count_date_time_length(void) {
    ...
    GV.print_time = 2;
    if (indexdir) {
        GV.print_date = 6; // DD.MM.YYYY default for DIR (31.1.2001)
    }

    j = spfind("DATE");
    if (j >= 0) {
        GV.print_date = get_date(spb[j]);
    }
...
static void no_mem(void) { disp_err("Not enough memory! (%s)", word[0]); }
...
static Julian _juldnj(struct tm *bdt, Julian Transition) {
    ...
    ay = y + 4716;
    jd_julian = ((1461 * (Work)ay) >> 2) + (153 * (m + 1) / 5) + (Work)bdt->tm_mday - 1524;
    jd_gregorian = jd_julian + 2 - y / 100 + y / 400 - y / 4000;
    if (jd_gregorian >= Transition) {
        jd = jd_gregorian;
    } else {
        jd = jd_julian;
    }
...
static Julian juldn(struct tm *bdt) { return juldnj(bdt, jul_transition); }
...
static int restrictions(void) // Muste: fil muutettu fi (INDEX) - ks. my?s muut!!!
{
    ...
    if (bytmax_i == INT_MIN) {
        ...
        if (bytmax_i >= 0) {
            bytmax = atoi(spb[bytmax_i]);
        }
    }

    if (since_i < 0 && before_i < 0 && bytmin_i < 0 && bytmax_i < 0) {
        return 0;
    }
    ...
    if (size < bytmin || size > bytmax) {
        return 1;
    }

    if (since_i < 0 && before_i < 0) {
        return 0;
    }
    ...
    if (since_i >= 0) {
        ...
        while (1) { /* (not a real while...) */
            if (year < since_date.yy) {
                return 1;
            }
            if (year > since_date.yy) {
                break;
            }
            if (month < since_date.mm) {
                return 1;
            }
            if (month > since_date.mm) {
                break;
            }
            if (day < since_date.dd) {
                return 1;
            }
            break;
        }
    ...
    if (before_i >= 0) {
        ...
        while (1) { /* (not a real while...) */
            if (year > before_date.yy) {
                return 1;
            }
            if (year < before_date.yy) {
                break;
            }
            if (month > before_date.mm) {
                return 1;
            }
            if (month < before_date.mm) {
                break;
            }
            if (day >= before_date.dd) {
                return 1;
            }
            break;
        }
...
static int valid_date(int idx) /* is the date valid at all */
{
    ...
    if (!muste_strnicmp(spb[idx], "TODAY", 5)) {
        i = 0; // (see below)
    }
    if (i) {
        i = spfind("DATE");
        disp_err(" Invalid date: %s=%s (DATE=%s).", spa[idx], spb[idx], i >= 0 ? spb[i] : "DD.MM.YY");
        return -1;
    }
    return 1;
}
...
static void show_yesno(char *str) { write_string("(Yes/No)", 8, Advice, BOTTOMROW, strlen(str) + 4); }
...
static int init_globals(void) {
    ...
    for (j = 0; j < GRPOPTLEN; j++) {
        GV.grouping[j] = '\0';
    }
    for (j = 0; j < SORTOPTLEN; j++) {
        GV.sorting[j] = '\0';
    }

    j = spfind("GROUPING");
    if (j >= 0) {
        if (strlen(spb[j]) >= GRPOPTLEN) {
            spb[j][GRPOPTLEN - 1] = '\0';
        }
...
static void f_sort(void) {
    init_prompt();
    WhiteBottomRow;
    write_string("(One of: DATE,TYPE,SIZE,NAME,TIME,OS. Descending: e.g. -DATE.)", 62, Advice, BOTTOMROW, 14);
    strcpy(answer, GV.sorting);
    LOCATE(BOTTOMROW, 1);
    prompt(" SORT=", answer, SORTOPTLEN - 1);
    muste_strupr(answer);
    if (!strcmp(answer, GV.sorting)) {
        return; /* no changes */
    }
...
static int comp2(const void *val1, const void *val2) /* date & time */
{
    const Files *nr1 = (const Files *)val1;
    const Files *nr2 = (const Files *)val2;
    if (nr1->year > nr2->year) {
        return 1;
    }
    if (nr1->year < nr2->year) {
        return -1;
    }
    if (nr1->month > nr2->month) {
        return 1;
    }
    if (nr1->month < nr2->month) {
        return -1;
    ...
    if (nr1->hour > nr2->hour) {
        return 1;
    }
    if (nr1->hour < nr2->hour) {
        return -1;
    }
    if (nr1->minute > nr2->minute) {
        return 1;
    }
    if (nr1->minute < nr2->minute) {
        return -1;
    }
    if (nr1->second > nr2->second) {
        return 1;
    }
    if (nr1->second < nr2->second) {
        return -1;
...
static int comp6(const void *val1, const void *val2) /* group order (final order) */
{
    const Files *nr1 = (const Files *)val1;
    const Files *nr2 = (const Files *)val2;
    if (nr1->grouporder < nr2->grouporder) {
        return -1;
    }
    if (nr1->grouporder == nr2->grouporder) {
        return 0;
...
static void get_edt_comments(char *str, int len) {
    ...
    if (!strncmp(sbuf, "SURVO84ED", 9)) {
        survo84 = 1;
    }
    if (!strncmp(sbuf, "SURVO 98 edit", 13)) {
        survo98 = 1;
    }
    ...
    if (survo98) { // new way: ordinary text file
        ...
        if (cols != 1) {
            return; /* should be 1st line */
        }
        p = strchr(sbuf, '|');
        if (p != NULL) {
            p += 2; /* skip | and control char */
            while (*p == ' ' && *p != '\0') {
                p++; /* skip possible white space */
            }
...
static void get_svo_comments(char *str, int len) {
    ...
    if (k && strncmp(sbuf, "SURVO 84C DATA", 14)) {
        k = 0;
    }
    if (!k) {
        muste_fclose(fh);
    ...
    for (i = 0, j = 0; i < textn && j < len; i++) {
        ...
        for (k = 0; k < strlen(sbuf) && j < len; j++, k++) {
            str[j] = sbuf[k];
        }
        if (j > 0) {
            str[j] = ';';
        ...
        if (etu == 2) {
            ...
            if (GV.only_grouptypes) {
                ...
                for (i = 0; i < GV.groups; i++) {
                    if (!i) {
                        muste_kv_s_disp("*.%s", GV.group_types[i]);
                    } else {
                        muste_kv_s_disp(",*.%s", GV.group_types[i]);
                    }
                }
    ...
    if (g > 1) { // at least one parameter given
        ...
        if (g > 2) { // two parameters, e.g. INDEX *.EDT CUR+2
            ...
            if (!results_line) {               // not a line - maybe a path?
                ...
                muste_sprintf(GV.filespec, "%s%s", edisk, "*");
            }
        }
...
static int INDEXget_fileinfo_from_R(void) {
    ...
    muste_sprintf(Rcmd, ".muste.desktop.fileinfo.INDEX(\"%s\")", GV.filespec);
    muste_evalr(Rcmd);
    ...
    for (i = 0, fi = &files[0], order_nr = 1; i < GV.filecount; i++, fi++) {
        ...
        if (GV.only_grouptypes) {
            ...
            for (j = 0; j < GV.groups; j++) {
                if (!strcmp(fi->type, GV.group_types[j])) {
                    break;
                }
            }
        ...
        if (strlen(fi->type)) {
            j = spfind(fi->type);
            if (j >= 0) {
                strcpy(fi->cmd, spb[j]); // cmd given by usr, e.g. EDT=LOAD
            }
            ...
            for (k = 0, ty1 = &types[0]; k < TypeCount1; k++, ty1++) {
                if (!muste_strcmpi(fi->type, ty1->type)) { // types match
                    if (j < 0) {
                        strcpy(fi->cmd, ty1->cmd); // default (no cmd was given)
                    }
        ...
        k++; /* 1st space also! */
        if (k > GV.bigglen) {
            GV.bigglen = k; /* update the longest counter */
        }
        if (fi->size > GV.bytes) {
            GV.bytes = fi->size; /* biggest file? */
        }
    }
...
static void INDEXsort_files(void) {
    ...
    if (i >= 0) {
        if (!strcmp(spb[i], "OS")) {
            ...
            muste_kv_s_err("Sorting option %s is unknown!", spb[i]);
        }
    }
    return;
}
...
static void INDEXprintout(void) {
    ...
    GV.list_to_field = 1;
    muste_sprintf(outfile, "%sINDEX.TMP", etmpd);
    j = spfind("OUTFILE");
    if (j >= 0) {
        GV.list_to_field = 0;
    ...
    if (GV.print_size) {
        GV.required += GV.biggest + 1; /* biggest size + " " */
    }
    ...
    if (j >= 0) {
        ...
        if (!strcmp(tmp3[0], "LEFT")) {
            GV.comments_to_left = 1;
            ...
            GV.commlen = atoi(tmp3[0]); /* desired length was given */
        }
        if (i > 1) {                    /* i==2 */
    ...
    bare_format = 0;
    if (!GV.print_size && !GV.print_date && !GV.print_time && !GV.commlen) {
        bare_format = 1;
    }
    stats_format = 0;
    full_format = 0;
    ...
    if (GV.required > MAXWIDTH) {
        GV.commlen -= (GV.required - MAXWIDTH);
    }
    ...
    for (i = -1; i <= GV.groups; i++) { /* -1 for printing only directories! */
        if (i == -1) {
            ...
            if (GV.only_grouptypes) {
                continue; // skip dirs when ONLY=1 (Muste)
            }
            if (GV.selected) {
                continue; // skip dirs if selected files (Muste)
            }
        } else {
            if (i < GV.groups) {
                ...
                the_rest = 1;
                if (GV.only_grouptypes) {
                    break; /* 14.7.95/kv (SM), kept 8.3.96 */
                }
            }
        }
        found_some = 0;
        for (j = 0, fi = &files[0]; j < GV.filecount; j++, fi++) {
            if (SubDirectory(fi) && GV.selected) {
                continue; // skip dirs if selected files (Muste)
            }
            if (fi->printed) {
                continue;
            ...
            if (FileDeleted(fi)) {
                continue; // lazy deleted are not printed (Muste)
            }
...
static int INDEXprint_line(void) {
    ...
    if (SubDirectory(fi)) {
        if (indexdir) {
            return 0; /* dirs are not printed at all! (SM&KV 30.1.2001) */
        }
        ...
        if (full_format) {
            muste_sprintf(wholefilename, "%s%s", edisk, fi->name);
        } else {
            muste_sprintf(wholefilename, "%s", fi->name);
        }

        p = strchr(wholefilename, ' '); // space in the file name?
        if (p != NULL) {
            muste_snprintf(commfile_tmp, LNAME, "%s%s%s", "\"", wholefilename, "\"");
            strcpy(wholefilename, commfile_tmp);
        }
        muste_snprintf(commfile_tmp, LNAME, "%s%s%s", fi->cmd, empty, wholefilename);

        //      if (full_format) {
        //          muste_sprintf(commfile_tmp, "%s%s%s%s", fi->cmd, empty, edisk, fi->name);
        //      } else {
        //          muste_sprintf(commfile_tmp, "%s%s%s", fi->cmd, empty, fi->name);
        //      }
        //
        //      if (!GV.print_filetype) {
    ...
    if (!SubDirectory(fi)) {
        if (!strncmp(fi->cmd, ">", 1) && !indexdir) {
            muste_sprintf(line, "%-*s", GV.bigglen, commfile_tmp);
        }

        if (bare_format) {
            strcpy(commfile_str, commfile_tmp);
            if (stats_format || full_format) {
                strcat(commfile_str, " / ");
            }
        } else {
            if (indexdir) {
                ...
                muste_snprintf(commfile_str, LNAME, "%-*s / ", GV.bigglen, commfile_tmp);
            }
        }
        ...
        if (GV.print_size) {
            ...
            muste_snprintf(size_str, STRMAXL, "%*s ", GV.biggest, bytes);
        }

        INDEXmake_date_and_time(); /* into GV.datetime_str */

        if (indexdir) {
            muste_snprintf(line, LLENGTH, "%s%s%s", commfile_str, size_str, GV.datetime_str);
        } else {
            if (stats_format) {
                muste_snprintf(line, LLENGTH, "%s%s", commfile_str, GV.comment_str);
            } else {
                if (GV.comments_to_left) {
                    muste_snprintf(line, LLENGTH, "%s%-*s %s%s", commfile_str, GV.commlen, GV.comment_str, size_str,
                                   GV.datetime_str);
                } else {
                    muste_snprintf(line, LLENGTH, "%s%s%s%s", commfile_str, size_str, GV.datetime_str, GV.comment_str);
                }
            }
        }
    }
...
static void INDEXmake_date_and_time(void) {
    ...
    case 0:
        break;
    case 1:
        muste_sprintf(GV.datetime_str, "%.2d.%.2d.%.2d ", fi->day, fi->month, year);
        break;
    case 2:
        muste_sprintf(GV.datetime_str, "%.2d/%.2d/%.2d ", fi->day, fi->month, year);
        break;
    case 3:
        ...
        break;
    case 4:
        muste_sprintf(GV.datetime_str, "%.2d%.2d%.2d ", year, fi->month, fi->day);
        break;
    case 5:
        muste_sprintf(GV.datetime_str, "%.2d/%.2d/%.2d ", fi->month, fi->day, year);
        break;
    case 6:
        muste_sprintf(GV.datetime_str, "%.2d.%.2d.%.2d ", fi->day, fi->month, YYYY);
        break;
    case 7:
        muste_sprintf(GV.datetime_str, "%.2d/%.2d/%.2d ", fi->day, fi->month, YYYY);
        break;
    case 8:
        muste_sprintf(GV.datetime_str, "%.2d/%.2d/%.2d ", fi->month, fi->day, YYYY);
        break;
    case 9:
        muste_sprintf(GV.datetime_str, "%.2d%.2d%.2d ", fi->day, fi->month, YYYY);
        break;
    case 10:
        muste_sprintf(GV.datetime_str, "%.2d%.2d%.2d ", YYYY, fi->month, fi->day);
        break;
    default:
        break;
    }
    ...
    case 0:
        break;
    case 1:
        muste_sprintf(time_str, "%.2d:%.2d:%.2d ", fi->hour, fi->minute, fi->second);
        break;
    case 2:
        muste_sprintf(time_str, "%.2d:%.2d ", fi->hour, fi->minute);
        break;
    case 3:
        muste_sprintf(time_str, "%.2d.%.2d.%.2d ", fi->hour, fi->minute, fi->second);
        break;
    ...
    if (GV.print_time) {
        strcat(GV.datetime_str, time_str);
    }
}
...
#define BUFLEN LLENGTH // (buffer defined earlier with LLENGTH...)
    ...
    if (stats_format) {
        ...
        if ((fh = muste_fopen2(tmp, "r")) == NULL) {
            return;
        }
        if (muste_fseek(fh, 0L, SEEK_SET)) {
            return;
        }
        ...
        edt98 = edt84 = 0;
        if (!(strncmp(buffer, "SURVO84ED", 9))) {
            edt84 = 1;
        }
        if (!(strncmp(buffer, "SURVO 98 edit", 13))) {
            edt98 = 1;
        }
        if (edt98 == 0 && edt84 == 0) {
            ...
            ch = strchr(buffer, '\n');
            if (ch == NULL) {
                return; /* no line feed at all -> no text file */
            }
            len = strlen(buffer);
            if (len == BUFLEN - 2) {
                return; /* line feed but not text file */
            }
            /* check some codes used in sucro files... 7.3.2001 */ // Muste: octal
            ch = strchr(buffer, '\373');
            if (ch != NULL) {
                return; // ?(ascii:8)=373
            }
            ch = strchr(buffer, '\374');
            if (ch != NULL) {
                return; // ?(ascii:8)=374
            }
            ch = strchr(buffer, '\375');
            if (ch != NULL) {
                return; // ?(ascii:8)=375
            }
            ch = strchr(buffer, '\376');
            if (ch != NULL) {
                return; // _(ascii:8) 376
            }
        }
        fh = muste_fopen2(tmp, "r");
        ll = 0;
        ...
        if (edt98) {                            /* SURVO 98 edit files */
            ...
            muste_kv_space_split(sbuf, word, 7);
            ll = atoi(word[5]);
            while (!feof(fh)) {
            ...
            if (sscanf(buffer, "%s %u %u", tmp, &cols, &rows) == EOF) {
                muste_fclose(fh);
                return;
            }
        ...
        muste_fclose(fh);
        muste_sprintf(GV.comment_str, "#lines=%u #words=%u", ll, ww);
        return;
    }

    if (fi->get_comments && GV.commlen) {
        if (!muste_strcmpi(fi->type, "EDT")) {
            get_edt_comments(GV.comment_str, LLENGTH - 1);
        } else if (!muste_strcmpi(fi->type, "SVO")) {
            get_svo_comments(GV.comment_str, LLENGTH - 1);
        } else if (!muste_strcmpi(fi->type, "MAT")) {
            get_mat_comments(GV.comment_str, LLENGTH - 1);
        } else if (!muste_strcmpi(fi->type, "M")) {
            get_mat_comments(GV.comment_str, LLENGTH - 1);
        }
...
static void quoted_usage(void) { muste_kv_s_err("Usage: SEARCH \"character string\" (SEARCH=QUOTED)"); }
...
static int search_files(void) {
    ...
    if (j >= 0) {
        ...
        if (strstr(spb[j], "FIRST") != NULL) {
            search_first = 1;
        }
        /* for connecting SEARCH to DD 22.7.1998/kv ! */
        if (strstr(spb[j], "FILE") != NULL) {
            search_file = 1;
        }
        if (strstr(spb[j], "DD") != NULL) {
            search_DD = 1;
        }
        if (strstr(spb[j], "COMMENT") != NULL) {
            search_comment = 1;
        }
        if (strstr(spb[j], "SHADOWS") != NULL) {
            search_shadows = 1;
        }
    }
    if (search_DD && search_first) {
        search_DD = 0; /* for sure 13.2.2002 */
    }

    if (search_shadows) {
        if (show_command == 1 || show_command == 3) {
            show_command = 5;
        } else if (show_command == 2 || show_command == 4) {
            show_command = 6;
        }
    ...
    j = spfind("RUN");
    if (j >= 0) {
        continuous = atoi(spb[j]);
    }
    if (search_DD) {
        continuous = 1; /* 22.7.1998 */
    }
    ...
    for (j = 0; j < n_paths; j++) {
        ...
        muste_sprintf(bigbuffer, "Searching from %s...", GV.filespec);
        ...
        for (i = 0, fi = &files[0]; i < GV.filecount; i++, fi++) {
            ...
            if (k) {
                continue; // some restriction was found
            }
            if (!recursive) { // i.e. search only THIS directory
                ...
                if (p != NULL) {
                    ...
                    sbuf[k] = '\0';
                    // Rprintf("\nsbuf    =|%s|",sbuf);
                    if (strcmp(fi->path, sbuf)) {
                        continue;
                    }
                }
            }
            muste_snprintf(filespec, LNAME, "%s/%s", fi->path, fi->name);
            if (!strcmp(filespec, outfile)) {
                continue;
            }

            fh = muste_fopen2(filespec, "r");
            if (fh == NULL) {
                continue; // esim. SKANDIT tiedostonimiss?!! (4.8.2011)
            }
            files_total++;
...
static int SEARCHget_fileinfo_from_R(void) {
    ...
    if (recursive) { // 11.11.11
        muste_sprintf(Rcmd, ".muste.desktop.fileinfo.SEARCH2(\"%s\")", GV.filespec);
    } else {
        muste_sprintf(Rcmd, ".muste.desktop.fileinfo.SEARCH1(\"%s\")", GV.filespec);
    }
    muste_evalr(Rcmd);
...
static int read_edt_file(char *filename) {
    ...
    if (!edt98) {
        if (sscanf(check, "%s %u %u", tmp, &cols, &rows) == EOF) {
            return retval; /* was -1 */
        }
        muste_fseek(fh, (unsigned int)cols, SEEK_SET); /* first row! */
    } else {
    ...
    for (l = 1; l <= rows; l++) {
        ...
        while (ptr != NULL) {
            ...
            matches++;
            found = 1;
            muste_sprintf(match_msg, "%7u", matches);
            ...
            if (!continuous) {                                                      /* moved down 7 lines 30.10.1998 */
                muste_sprintf(bigbuffer, "Found in Survo edit file %s:", filespec); /* 7.3.2001 */
            ...
            if SKIPPED
                return retval;
            if (search_first && matches) {
                return retval; /* 7.4.97 */
            }
            if (search_file) {
                return retval; /* 22.7.1998 */
            }
        ...
        if CANCELED
            return retval;
        if (!edt98) {
            muste_fseek(fh, cols, (int)muste_ftell(fh));
        }
    }
    return retval;
}
...
static void edt_search_msg(void) {
    write_string(space, ScreenWidth, SearchColor, MessageLine, 1);
    muste_sprintf(bigbuffer, "Searching in Survo edit file %s...", filespec);
    write_string(bigbuffer, strlen(bigbuffer), SearchColor, MessageLine, 1);
}

static void any_search_msg(void) {
    write_string(space, ScreenWidth, SearchColor, MessageLine, 1);
    muste_sprintf(bigbuffer, "Searching in text file %s...", filespec);
    write_string(bigbuffer, strlen(bigbuffer), SearchColor, MessageLine, 1);
}
...
static int read_any_file(char *filename) {
    ...
    l = 0;
    muste_sprintf(bigbuffer, "Checking %s...", filespec);
    write_string(space, ScreenWidth, SearchColor, MessageLine, 1);
    write_string(bigbuffer, strlen(bigbuffer), SearchColor, MessageLine, 1);
    ...
    ch = strchr(buffer, '\n');
    if (ch == NULL) {
        return retval; /* was -1 */ /* no line feed at all -> no text file */
    }

    length = strlen(buffer);
    if (length == LLENGTH - 2) {
        return retval; /* was -1 */ /* line feed but not text file */
    }

    /* check some codes used in sucro files... 7.3.2001 */
    ch = strchr(buffer, '\373');
    if (ch != NULL) {
        return retval; // ?(ascii:8)=373
    }
    ch = strchr(buffer, '\374');
    if (ch != NULL) {
        return retval; // ?(ascii:8)=374
    }
    ch = strchr(buffer, '\375');
    if (ch != NULL) {
        return retval; // ?(ascii:8)=375
    }
    ch = strchr(buffer, '\376');
    if (ch != NULL) {
        return retval; // _(ascii:8) 376
    }
    ...
    while (!feof(fh)) {
        ...
        while (ptr != NULL) {
            ...
            write_string(match_msg, 7, CountColor, CommandLine, 1);
            if (!continuous) {
                muste_sprintf(bigbuffer, "Found in text file %s on line %u:", filespec, l); /* 7.3.2001 */
                write_string(space, ScreenWidth, FoundColor, MessageLine, 1);
                write_string(bigbuffer, strlen(bigbuffer), FoundColor, MessageLine, 1);
                //        LOCATE(ShowLine,1);
...
static void update_field(unsigned int counter, char *str, char *filename) {
    ...
    case 0:
        if (search_shadows) { /* 25.5.2001 */
            muste_sprintf(bigbuffer, "%s\n%s", str, shadow_buffer);
        } else {
            strcpy(bigbuffer, str);
        }
        break;
    case 1:
        muste_sprintf(bigbuffer, "SHOW %s,%u / %s", filename, counter, str);
        break;
    case 2:
        muste_sprintf(bigbuffer, "SHOW %s,%u / %s", filespec, counter, str);
        break;
    case 3:
        muste_sprintf(bigbuffer, "SHOW %s,%u\n%s", filename, counter, str);
        break;
    case 4:
        muste_sprintf(bigbuffer, "SHOW %s,%u\n%s", filespec, counter, str);
        break;
    case 5:
        muste_sprintf(bigbuffer, "SHOW %s,%u / %s\n%s", filename, counter, str, shadow_buffer);
        break;
    case 6:
...
static void write_results(void) {
    ...
    muste_sprintf(bigbuffer, " %u occurence%s found (%u file%s scanned, %u lines read).", matches,
                  (matches > 1) ? "s" : "", files_total, (files_total > 1) ? "s" : "", lines_total);
    ...
    while (1) { /* 22.7.1998 */
        ...
        if (!empty_line(bigbuffer + 1, c2)) {
            if (exact_search) {
                needed = first_matches - written;
            } else {
                needed = matches - written;
            }
            ...
            muste_sprintf(sbuf, NotEn1, needed);
...
static int DDmain(void) {
    whstart = 0;
    if (g > 1) {
        whstart = WhereStart; /* 14.4.96 */
    }
    if (whstart == 0) {
        where_first = 0; /* 13.2.2002 (set to 1 by WHERE if FIRST) */
    }
    if (g > 1) {
        sestart = SEARCHStart; /* 22.7.1998 */
    }
    if (g > 1) {
        dmstart = DMStart; /* 17.1.1999 */
    }
...
//  strcpy(D->files,GV.filespec);       /* includes the */
    ...
    r_was = r;
    r = 1;
    s_end(siirtop); /* move window */
    min_required = GV.required;

    any = 0;
    rv = 0; /* 1.1.98 to return to origdir in err.situations */
            //  handle_dirlist(READ);
            // Rprintf("\nentering while(1)...\n");
    while (1) {
        ...
        if (rv == CODE_REF) {
            break; /* exit and stay in this dir */
        }
...
static void handle_dirlist(const int code) /* either READ or WRITE */
{
    ...
    if (!hae_apu("edisk",DefaultDataPath)) {
        muste_sprintf(DefaultDataPath,"%sD/",survo_path); /* C:\E\D\ 26.10.96 */
    }
    muste_sprintf(sbuf,"%s%s",DefaultDataPath,DirListFile);
    switch(code) {
      case READ:
...
static int DDget_fileinfo_from_R(void) {
    ...
    muste_sprintf(Rcmd, ".muste.desktop.fileinfo.DD(\"%s\")", GV.filespec);
    muste_evalr(Rcmd);
    ...
    for (i = 0, fi = &files0[0], order_nr = 1; i < GV.filecount; i++, fi++) {
        ...
        if (where_outfile) {
            muste_sprintf(tmp, "%s%s", fi->path, fi->name);
            fprintf(output_file, "%s\n", tmp);
        }
...
static void DDsort_files(void) {
    ...
    comp = comp1;
    desc = (GV.sorting[0] == '-' || GV.sorting[1] == '-');

    if (strstr(GV.sorting, "OS") != NULL) {
        comp = (desc) ? comp1b : comp1;
    } else if (strstr(GV.sorting, "DATE") != NULL) {
        comp = (desc) ? comp2b : comp2;
    } else if (strstr(GV.sorting, "TIME") != NULL) {
        comp = (desc) ? comp2b : comp2;
    } else if (strstr(GV.sorting, "SIZE") != NULL) {
        comp = (desc) ? comp3b : comp3;
    } else if (strstr(GV.sorting, "NAME") != NULL) {
        comp = (desc) ? comp4b : comp4;
    } else if (strstr(GV.sorting, "TYPE") != NULL) {
        comp = (desc) ? comp5b : comp5;
    } else if (strstr(GV.sorting, "MARK") != NULL) {
        comp = (desc) ? comp9b : comp9;
    } else if (strstr(GV.sorting, "COMM") != NULL) {
        comp = (desc) ? comp10b : comp10;
    } else {
        if (strlen(GV.sorting)) { /* errorneous option */
            disp_err(" Sorting option %s is unknown!", GV.sorting);
            for (i = 0; i < SORTOPTLEN; i++) {
                GV.sorting[i] = '\0';
            }
        }
    }
...
static void DDgroup_order_files(void) {
    ...
    for (i = -1; i <= groups; i++) {
        ...
        for (j = 0, f = &files[0]; j < GV.filecount; j++, f++) {
            ...
            if (f->hidden_comment) {
                strcpy(sbuf, f->comment); // 5.4.2001
            } else {
                strcpy(sbuf, f->name);
            }
            muste_strupr(sbuf);
            ...
            if ((x != NULL && !strncmp(x, typ, 3)) || the_rest || SubDirectory(f)) {
                f->grouporder = k++;
                f->status = (f->status | FSORTED);
            }
        }
...
static void DDget_comments(char *str, int len) {
    ...
    if (len > 0) {
        ...
        if (strstr(fi->name, ".EDT") != NULL) {
            get_edt_comments(str, len);
        } else if (strstr(fi->name, ".SVO") != NULL) {
            get_svo_comments(str, len);
        } else if (strstr(fi->name, ".MAT") != NULL) {
            get_mat_comments(str, len);
...
static int WHEREget_fileinfo_from_R(void) {
    ...
    muste_sprintf(Rcmd, ".muste.desktop.fileinfo.WHERE(\"%s\")", GV.filespec);
    muste_evalr(Rcmd);
    ...
    //  muste_sprintf(path, "%s/%s", CHAR(STRING_ELT(Robj1,0)), CHAR(STRING_ELT(Robj2,0)) );
    //
...
static int DDf_where(void) {
    ...
    if (GV.filecount == 0) {
        if (where_first) {
            where_first = -1; /* 13.2.2002 (indicated no matches) */
        } else {
            disp_err(" No files found (%s)!", answer);
        }
    }
    return WHERE;
}
...
static int DDdisplay_files(void) {
    ...
    i = 0;
    totalbytes = markbytes = 0L;
    msgnr = 1;
    clear_screen();

    if (whstart) {
        GV.filecount = 0; /* 14.4.96 */
    }
    ...
    while (1) {
        if (GV.filecount) {
            ...
            if (revisit) {
                ...
                while (1) {
                    ...
                    if (DDf_key(CODE_DOWN)) {
                        current = 0; /* incr. current */
                    }
                    if (row == ENDROW) {
                        row = ddSTARTROW;
                        dfi = 0;
                ...
                row = ddSTARTROW;
                if (current > GV.filecount - 1) {
                    current = GV.filecount - 1;
                }
                dfi -= current;
                fi -= current;
                if (dfi < 0) {
            ...
            f = fi;
            DDget_line(dfi);
            DDdisplay_line(dfi, row, strlen(line));
        }
        if (EndOfPageCondition) { /* includes GV.filecount==0 !! */
            ...
            case WHERE:
                ...
                return m;
            case -1:
                return -1; /* returns -1 to DD.C -> exit */
            case TREE:
...
static int DDhandle_key(unsigned int m) {
    ...
    case OPEN:
        filename_to_sbuf();
        // muste_sprintf(answer,"START %s",sbuf);
        // muste_sprintf(answer," %s",sbuf); // 24.11.2012 (no START!)
        muste_sprintf(answer, "%s", sbuf); // 28.11.2012 (no START!!)
                                           // Rprintf("\nDD/OPEN: |%s|",answer);
...
static void DDdrawline(char color, int row, int length) {
    ...
    if (FileMarked(f)) {
        ...
        case FileColor:
        case AttribColor:
            write_string(MarkStr, 1, MarkColor1, row, 1);
            break;
        case Cursor:
        case AttribCursor:
            write_string(MarkStr, 1, MarkColor2, row, 1);
        }
    }
}
...
static void DDinfoline(void) {
    ...
    if (WhereMode) {
        if (GV.dircount == 0) {
            muste_sprintf(line, " %d file%s, %u byte%s.", GV.filecount, (GV.filecount != 1) ? "s" : "", totalbytes,
                          (totalbytes != 1L) ? "s" : "");
        } else {
            muste_sprintf(line, " %d file%s from %d director%s, %u byte%s.", GV.filecount,
                          (GV.filecount != 1) ? "s" : "", GV.dircount, (GV.dircount > 1) ? "ies" : "y", totalbytes,
                          (totalbytes != 1L) ? "s" : "");
        }
    } else { /* normal mode */
        if (GV.dircount == 0) {
            muste_sprintf(line, " %d file%s, %u byte%s.", GV.filecount - GV.dircount,
                          ((GV.filecount - GV.dircount) != 1) ? "s" : "", totalbytes, (totalbytes != 1L) ? "s" : "");
        } else {
            muste_sprintf(line, " %d file%s + %d director%s, %u byte%s.", GV.filecount - GV.dircount,
                          ((GV.filecount - GV.dircount) != 1) ? "s" : "", GV.dircount, (GV.dircount > 1) ? "ies" : "y",
                          totalbytes, (totalbytes != 1L) ? "s" : "");
        }
    }

    if (markcount > 0) {
        muste_sprintf(answer, " Marked: %d file%s, %u byte%s.", markcount, (markcount != 1) ? "s" : "", markbytes,
                      (markbytes != 1L) ? "s" : "");
        strcat(line, answer);
    }
    ReversedWorkRow;
...
static void DDget_line(int dfi) {
    ...
    if ((strcmp(f->name, f->comment) != 0)) { // 1.6.2012
        //      muste_sprintf(line," %s %s%-*s ", namesize,datetime,GV.commlen,sbuf);
        muste_sprintf(line, " %s%s%-*s ", datetime, namesize, GV.commlen, sbuf);
    } else {
        //      muste_sprintf(line," %s %s ", namesize, datetime);
        muste_sprintf(line, " %s%s ", datetime, namesize);
    }
}
...
static void DDmake_file_name_and_size(char *str) {
    ...
    muste_sprintf(file_str, "%-12s ", file_tmp);
    strcpy(size_str, "");

    if (SubDirectory(f)) {
        // Muste: < > around the dir name (2.6.2012)
        muste_sprintf(file_str, "<%s> ", file_tmp);
    } else {
        if (GV.print_size) {                /* 21.7.1998 */
            muste_itoa(f->size, bytes, 10); /* size to a string */
            muste_sprintf(size_str, "%*s", GV.bigglen, bytes);
        }
    }
    // Muste: reversed order: (1.6.2012)
    if (GV.print_size) {
        muste_sprintf(str, "%s %s", size_str, file_str);
...
static int mark_saved_files_from_DM(void) {
    ...
    muste_sprintf(sbuf, "%s%s", etmpd, "DM_DD.TMP");
...
static void DDf_rename(void) {
    ...
    muste_sprintf(sbuf, " Rename %s to ? ", (SubDirectory(f)) ? "directory" : "file");
    prompt(sbuf, answer, 12); // pituus?!
                              //  if (!muste_strcmpi(answer, f->name)) return;
    if (!strcmp(answer, f->name)) {
        return;
    }
    if (!strlen(answer)) {
        return;
...
static int DDf_matshow(void) {
    ...
    filename_to_sbuf();
    strcpy(buf, sbuf);
    muste_snprintf(answer, LNAME, "Matrix file %s", buf);
    WorkRowText(7);

    s_was = scroll_line;
    scroll_line = ddSTARTROW;
    s_end(siirtop); /* change scroll line */
    muste_sprintf(tempfil, "%s%s", etmpd, "DD.TMP");
    sur_delete(tempfil);
    ...
    matrix_print(A, m, n, rlab, clab, lr, lc, m, n, NULL, NULL, format, c3, 0, tempfil, expr);
    output_level = ol_was; /* 2.1.1999 */
    scroll_line = s_was;
    s_end(siirtop);                                /* restore scroll line */
    muste_sprintf(answer, "SHOW \"%s\"", tempfil); // RS ADD " 4.12.2011
    write_cmd_line();
...
static int DDf_act(void) {
    ...
    if (f->command == FSHOW) {
        filename_to_sbuf();
        muste_sprintf(answer, " loading FILE ACTIVATE for %s...", sbuf);
        WorkRowText(7);
        // Rprintf(answer);
        //
        ...
        muste_sprintf(answer, "FILE ACTIVATE \"%s\"", sbuf); // RS ADD " 4.12.2011
        write_cmd_line();
        enable_softkeys();
        muste_dump();
...
static int DDf_load(void) {
    ...
    init_prompt();
    if (WhereMode) {
        muste_sprintf(answer, "%s%s", edisk, "*");
    }
...
static int DDf_show(unsigned int m) /* was void before 4.1.98 */
{
    ...
    switch (f->command) {
    case DIRMOVE:
        muste_sprintf(answer, "%s%s/*", edisk, f->name); // added '/*' 11.11.11
    ...
    case FSHOW:
        WhiteWorkRow;
        filename_to_sbuf();
        muste_sprintf(answer, " loading FILE SHOW for %s...", sbuf);
        WorkRowText(7);
        // muste_sprintf(answer,"FILE SHOW \"%s\"",sbuf); // RS ADD " 4.12.2011
        muste_sprintf(answer, "FILE SHOW %s", sbuf); // 24.11.2012 ("" added in filename_to_sbuf()!)
    ...
    case SHOW:
        ...
        WorkRowText(7);
        filename_to_sbuf();
        // muste_sprintf(answer,"SHOW %s %d",sbuf,f->match); /* 30.10.1998 */
        // muste_sprintf(answer,"SHOW \"%s\" \"%d\"",sbuf,f->match); // RS ADD " 4.12.2011 /* 30.10.1998 */
        // muste_sprintf(answer,"SHOW \"%s\" %d",sbuf,f->match); // (%d is a line number)
        muste_sprintf(answer, "SHOW %s %d", sbuf, f->match);
            /* 30.10.1998 */ // 24.11.2012 ("" added in filename_to_sbuf()!)
        write_cmd_line();
        enable_softkeys();        // RS ADD
        muste_dump();             // RS ADD
...
static int DDf_tutshow(void) {
    ...
    if ((j >= 22) && (strstr(line, "SURVO 84C SUCROS@") != NULL)) {
        ...
        muste_restore_dump();
        disable_softkeys();
        if ((out = muste_fopen2(tempfil, "a")) == NULL) {
            return -1;
        }
        for (m = 0; m < k; m++) {
            muste_sprintf(answer, " Sucro family %s (loading members...%d)", buf, m + 1);
            WorkRowText(7);
            ...
            fputs(answer, out);
            if ((in = muste_fopen2(path, "r")) == NULL) {
                return -1;
            }
            while (muste_fgets(line, LLENGTH, in) != NULL) {
                fputs(line, out);
            }
            muste_fclose(in);
        }
        WhiteWorkRow;
        muste_sprintf(answer, " Sucro family %s", buf);
        WorkRowText(7);
        muste_fclose(out);
        sur_delete(path);
    } else {
        muste_sprintf(answer, " Sucro file %s", buf);
        WorkRowText(7);
        muste_sprintf(answer, "TUTLOAD \"%s\",\"%s\"", buf, tempfil); // RS ADD " 4.12.2011
                                                                      // Rprintf("\n%s", answer);
        write_cmd_line();
...
static void filename_to_sbuf(void) {
    char *p;
    muste_sprintf(sbuf, "%s%s", edisk, f->name);
...
static int DDf_delete(void) {
    ...
    if (markcount > 0) {
        ...
        if (LastFile(f)) {
            while (FileMarked(f) && !FirstFile(f)) {
                f--;
            }
        } else {
            while (FileMarked(f) && !LastFile(f)) {
                f++;
            }
        }
        strcpy(revisitfile, f->name);
        ...
        muste_sprintf(answer, " Delete %d marked file%s ? ", markcount, (markcount > 1) ? "s" : "");
        WhiteBottomRow;
        LOCATE(BOTTOMROW, 1);
        show_yesno(answer);
        j = ask_yesno(answer, "Y");
        if (j > 0) {
            for (i = 0, j = 0, f = &files[0]; i < wasfilecount && j < wasmarkcount; f++) {
                if (FileMarked(f)) {
                    LOCATE(BOTTOMROW, 1);
                    WhiteBottomRow;
        ...
        f = fcurrent;
        if (WhereMode) {
            muste_sprintf(path, "%s%s", f->path, f->name);
        } else {
            muste_sprintf(path, "%s%s", edisk, f->name);
        }
        if (SubDirectory(f)) {
...
static void DDmake_date_and_time(char *dtstr) {
    ...
    switch (GV.print_date) {
    case 1:
        muste_sprintf(dstr, "%.2d.%.2d.%.2d ", fi->day, fi->month, year);
        break;
    case 2:
        muste_sprintf(dstr, "%.2d/%.2d/%.2d ", fi->day, fi->month, year);
        break;
    case 3:
        ...
        break;
    case 4:
        muste_sprintf(dstr, "%.2d%.2d%.2d ", year, fi->month, fi->day);
        break;
    case 5:
        muste_sprintf(dstr, "%.2d/%.2d/%.2d ", fi->month, fi->day, year);
        break;
        /* 21.7.1998 */ /* _SHOW D:\W\DESKTOP\GEN1 69 / */
    case 6:
        muste_sprintf(dstr, "%.2d.%.2d.%.2d ", fi->day, fi->month, YYYY);
        break;
    case 7:
        muste_sprintf(dstr, "%.2d/%.2d/%.2d ", fi->day, fi->month, YYYY);
        break;
    case 8:
        muste_sprintf(dstr, "%.2d/%.2d/%.2d ", fi->month, fi->day, YYYY);
        break;
    case 9:
        muste_sprintf(dstr, "%.2d%.2d%.2d ", fi->day, fi->month, YYYY);
        break;
    case 10:
        muste_sprintf(dstr, "%.2d%.2d%.2d ", YYYY, fi->month, fi->day);
        break;
    default:
        break;
    }
    switch (GV.print_time) {
    case 1:
        muste_sprintf(tstr, "%.2d:%.2d:%.2d ", fi->hour, fi->minute, fi->second);
        break;
    case 2:
        muste_sprintf(tstr, "%.2d:%.2d ", fi->hour, fi->minute);
        break;
    case 3:
        muste_sprintf(tstr, "%.2d.%.2d.%.2d ", fi->hour, fi->minute, fi->second);
        break;
