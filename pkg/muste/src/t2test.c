...
static char *method_text[2] = {"Assuming equal cov.matrices", "Assuming unequal cov.matrices"};
...
void muste_t2test(char *argv) {
    ...
    orig_samples = 1;
    spec_rnd();
    strcpy(tempname, etmpd);
    strcat(tempname, "SURVOHOT.TMP");
    ...
    t2_0 = T2();
    p_hot = 1.0 - muste_cdf_f((double)(n[0] - m - 1) / (n[0] - 2) / (double)m * t2_0, (double)m,
                              (double)(n[1] + n[2] - m - 1), 1e-15);
    ...
    if (method == 1) {
        ...
        muste_sprintf(sbuf, "T2=%g p=%d n1=%d n2=%d P1=%g", t2_0, m, n[1], n[2], p_hot);
    } else {
        eoutput("Yao's two-sample test for equality of mean vectors:");
        muste_sprintf(sbuf, "T2=%g P1=%g (assuming nonequal cov.matrices)", t2_BF0, p_yao);
    }
    eoutput(sbuf);
    if (simumax > 0L) {
        eoutput("Randomization test:");
        p_sim = (double)nn1 / (double)nn;
        muste_sprintf(sbuf, "N=%d P=%g (s.e. %g) %s", nn, p_sim, sqrt(p_sim * (1 - p_sim) / nn),
...
static int data_error() {
    muste_sprintf(sbuf, "\nData sets %s and %s must have same structures!", word[1], word[2]);
    sur_print(sbuf);
    WAIT;
    return (1);
}
...
static double T2() {
    ...
    if (orig_samples) {
        ...
        for (i = 0; i < m; ++i) {
            ss[1][i] = s[i] - ss[0][i];
            for (j = 0; j <= i; ++j) {
                ss2[1][i + m * j] = s2[i + m * j] - ss2[0][i + m * j];
            }
        }
    }
    ...
    if (orig_samples) {
        for (i = 0; i < m; ++i) {
            for (j = 0; j <= i; ++j) {
                ss2[0][i + m * j] += -n[1] * ss[0][i] * ss[0][j] + ss2[1][i + m * j] - n[2] * ss[1][i] * ss[1][j];
...
static double T2_BF() {
    ...
    if (orig_samples) {
        ...
        for (i = 0; i < m; ++i) {
            for (j = 0; j <= i; ++j) {
                ss_apu[i + m * j] = (ss2[0][i + m * j] - ss[0][i] * ss[0][j] / n[1]) / (n[1] * (n[1] - 1)) +
...
static int yao_test() // Srivastava s.119
{
    ...
    ********************/
    ...
    dm = (double)m;
    p_yao = 1.0 - muste_cdf_f((f - dm + 1.0) / (f * m) * t2_BF0, dm, f - dm + 1.0, 1e-15);
