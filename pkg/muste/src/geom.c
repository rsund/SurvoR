...
static char *object_name[] = {"point",    "point_l",  "point_c",  "line",   "circle", "circle_p", "edge",
                              "cross",    "cross_cl", "cross_cc", "save",   "sample", "symbol",   "perpendicular",
                              "midpoint", "parallel", "bisect_a", "stddev", "ortho"};
#define N_OBJ 19
...
void muste_geom(char *argv) {
    ...
    i = spfind("SAMPLES");
    if (i >= 0) {
        samples = n_samples = atoi(spb[i]);
    }
    if (samples) {
        stddev = 0.001;
        i = spfind("STDDEV");
        if (i >= 0) {
            stddev = muste_arit_atof(spb[i]);
        }
    ...
    if (g > 3 && strcmp("0", word[3]) != 0) {
        res_line = edline2(word[3], 1, 1);
        if (res_line == 0) {
            return;
        }
        // Rprintf("\nres_line=%d j1=%d J2=%d|",res_line,j1,j2); getch();
        if (res_line < j2 + 1) {
            muste_sprintf(sbuf, "\nLine %s not accepted for the summary of objects!", word[3]);
            sur_print(sbuf);
            WAIT;
            return;
        }
    ...
    for (j = j1 + 1; j <= j2; ++j) {
        ...
        for (i = 2; i < n_param; ++i) {
            strcpy(param[n_objects][i - 1], s[i]);
        }
        for (i = n_param; i < MAXPAR; ++i) {
            *param[n_objects][i - 1] = EOS;
        }
    ...
    for (sample_i = 0; sample_i < n_samples; ++sample_i) {
        *data_line = EOS;
        ...
        if (samples && disp_count == n_disp) {
            muste_sprintf(sbuf, " %d", sample_i + 1);
            sur_print(sbuf);
            disp_count = 0;
        }
        ...
        for (j = 0; j < n_objects; ++j) {
            if (!samples) {
                ...
                for (i = 0; i < MAXPAR - 1; ++i) {
                    k += muste_sprintf(sbuf + k, "%s ", param[j][i]);
                }
                sur_print(sbuf);
            }
            switch (object[j]) {
            ...
            case POINT_L:
                ...
                if (!samples) {
                    ...
                    // projection:
                    //            Rprintf("\nx1=%g y1=%g x2=%g y2=%g x0=%g y0=%g xv=%g yv=%g",x1,y2,x2,y2,x0,y0,xv,yv);
                    //            getch();
            ...
            case LINE:
                ...
                if (!samples || (samples && exact)) {
                    ...
                    if (ex) {
                        ...
                        randomize(&line_x1[n_lines], &line_y1[n_lines], x, y, point_k1[i], point_k2[i], stddev);
                    }
                    ...
                    if (ex) {
                        ...
                        randomize(&line_x2[n_lines], &line_y2[n_lines], x, y, point_k1[i], point_k2[i], stddev);
                    }
                }

                //              Rprintf("\nLine: %g %g %g
                //              %g",line_x1[n_lines],line_y1[n_lines],line_x2[n_lines],line_y2[n_lines]); getch();
                ++n_lines;
                break;

            case CIRCLE:
                strcpy(circle_name[n_circles], param[j][0]);
                exact = 0;
                ...
                if (!samples || (samples && exact)) {
                    ...
                    if (ex) {
                        circle_x[n_circles] = x;
                        circle_y[n_circles] = y;
                    } else {
                        randomize(&circle_x[n_circles], &circle_y[n_circles], x, y, point_k1[i], point_k2[i], stddev);
                    }
                }
            ...
            case CIRCLE_P:
                ...
                if (*sbuf == '*') {
                    strcpy(sbuf, point_name[current_center]);
                }
                ...
                if (!samples || (samples && exact)) {
                    ...
                    if (ex) {
                        ...
                        randomize(&x1, &y1, x, y, point_k1[ic], point_k2[ic], stddev);
                    }
                    i = get_point(&ex, &x, &y, param[j][2]);
                    if (i < 0) {
                        return;
                ...
                circle_x[n_circles] = x1;
                circle_y[n_circles] = y1;
            ...
            case EDGE:
                strcpy(edge_name[n_edges], param[j][0]);
                if (!samples) {
                    ...
                    if (ex) {
                        ...
                        randomize(&x1, &y1, x, y, point_k1[i], point_k2[i], stddev);
                    }
                    i = get_point(&ex, &x, &y, param[j][2]);
                    if (i < 0) {
                        return;
            ...
            case CROSS:
                ...
                {
                    muste_sprintf(sbuf, "\nLine %s does not intersect line %s!", line_name[il], line_name[ic]);
                    sur_print(sbuf);
                    WAIT;
                    return;
                }
                ...
                point_x[n_points] =
                    (x2 * x4 * (y3 - y1) + x2 * x3 * (y1 - y4) + x1 * x4 * (y2 - y3) + x1 * x3 * (y4 - y2)) / b;
                point_y[n_points] =
                    (y1 * y4 * (x3 - x2) + y1 * y3 * (x2 - x4) + y2 * y4 * (x1 - x3) + y2 * y3 * (x4 - x1)) / b;
            ...
            case CROSS_CL:
                ...
                xv = muste_arit_atof(param[j][3]);
                yv = muste_arit_atof(param[j][4]);

                if (fabs(x1 - x2) > fabs(y1 - y2)) {
                    b = (y2 - y1) / (x2 - x1);
                    aa = 1 + b * b;
                    bb = -2 * x0 + 2 * b * (y1 - y0) - 2 * b * b * x1;
                    cc = x0 * x0 + (y1 - y0) * (y1 - y0) - 2 * b * x1 * (y1 - y0) + (b * x1) * (b * x1) -
                         radius * radius;
                    dd = bb * bb - 4 * aa * cc;
                    if (dd < 0.0) {
                        muste_sprintf(sbuf, "\nLine %s does not intersect circle %s!", line_name[il], circle_name[ic]);
                        sur_print(sbuf);
                        WAIT;
                        return;
                    }
                    ...
                    cc = y0 * y0 + (x1 - x0) * (x1 - x0) - 2 * b * y1 * (x1 - x0) + (b * y1) * (b * y1) -
                         radius * radius;
                    dd = bb * bb - 4 * aa * cc;
                    if (dd < 0.0) {
                        muste_sprintf(sbuf, "\nLine %s does not intersect circle %s!", line_name[il], circle_name[ic]);
                ...
                if ((xx1 - xv) * (xx1 - xv) + (yy1 - yv) * (yy1 - yv) <
                    (xx2 - xv) * (xx2 - xv) + (yy2 - yv) * (yy2 - yv)) {
                    point_x[n_points] = xx1;
                    point_y[n_points] = yy1;
                    point_k1[n_points] = sl11;
                    point_k2[n_points] = sl12;
                } else {
                    point_x[n_points] = xx2;
                    point_y[n_points] = yy2;
                    point_k1[n_points] = sl21;
                    point_k2[n_points] = sl22;
                }
                if (!samples) {
                    minmax_x(point_x[n_points]);
                    minmax_y(point_y[n_points]);
            ...
            case CROSS_CC:
                ...
                if (fabs(x1 - x2) > fabs(y1 - y2)) {
                    ...
                    aa = 1 + c * c;
                    bb = 2 * c * (d - x1) - 2 * y1;
                    cc = (d - x1) * (d - x1) + y1 * y1 - radius * radius;

                    dd = bb * bb - 4 * aa * cc;
                    // Rprintf("\ndd1=%g",dd); getch();
                    if (dd < 0.0) {
                        muste_sprintf(sbuf, "\nCircle %s does not intersect circle %s!", circle_name[ic],
                                      circle_name[il]);
                        sur_print(sbuf);
                        WAIT;
                        return;
                    ...
                    aa = 1 + c * c;
                    bb = 2 * c * (d - y1) - 2 * x1;
                    cc = (d - y1) * (d - y1) + x1 * x1 - radius * radius;
            ...
            case SAMPLE:
                ...
                point_k1[n_points] = point_k1[i];
                point_k2[n_points] = point_k2[i];
                ++n_points;
            ...
            case PERPENDICULAR:
                ...
                line_x1[n_lines] = x;
                line_y1[n_lines] = y;
                line_x2[n_lines] = x + muste_cos(a);
                line_y2[n_lines] = y + muste_sin(a);
            ...
            case PARALLEL:
                ...
                line_x1[n_lines] = x;
                line_y1[n_lines] = y;
                line_x2[n_lines] = x2 + x - x1;
                line_y2[n_lines] = y2 + y - y1;
    ...
    if (res_line && !samples) {
        ...
        {
            muste_sprintf(sbuf, "%3d objects (total)",
                          n_objects - n_object[SAVE] - n_object[SYMBOL] - n_object[SAMPLE]);
            write_line(j++);

            muste_sprintf(sbuf, "%3d points (%d+%d+%d+%d+%d+%d)",
                          n_object[POINT] + n_object[POINT_L] + n_object[POINT_C] + +n_object[CROSS] +
                              n_object[CROSS_CL] + n_object[CROSS_CC],
                          n_object[POINT], n_object[POINT_L], n_object[POINT_C], n_object[CROSS], n_object[CROSS_CL],
                          n_object[CROSS_CC]);
            write_line(j++);
            i = n_object[LINE];
            if (i) {
            ...
            if (i) {
                ...
                muste_sprintf(sbuf, "%3d circle%c (%d+%d)", n_object[CIRCLE] + n_object[CIRCLE_P], ch, n_object[CIRCLE],
                              n_object[CIRCLE_P]);
                write_line(j++);
            }
            i = n_object[EDGE];
        ...
        muste_sprintf(sbuf, "Geometrographic symbol = %dS1+%dS2+%dC1+%dC2+%dC3=%d", gs1, gs2, gc1, gc2, gc3,
                      gs1 + gs2 + gc1 + gc2 + gc3);
        write_line(j++);
        muste_sprintf(sbuf, "Simplicity = %d  Exactitude = %d", gs1 + gs2 + gc1 + gc2 + gc3, gs1 + gc1 + gc2);
        write_line(j++);
...
static int get_point(int *exact, double *px, double *py, char *pname) {
    ...
    *exact = 0;
    ...
    for (i = 0; i < n_points; ++i) {
        if (muste_strcmpi(pname, point_name[i]) == 0) {
            break;
        }
    }
    if (i == n_points) {
        muste_sprintf(sbuf, "\nPoint %s not found!", pname);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int get_line(double *px1, double *py1, double *px2, double *py2, char *pname) {
    ...
    for (i = 0; i < n_lines; ++i) {
        if (muste_strcmpi(pname, line_name[i]) == 0) {
            break;
        }
    }
    if (i == n_lines) {
        muste_sprintf(sbuf, "\nLine %s not found!", pname);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int get_edge(double *px, char *pname, int error) {
    ...
    for (i = 0; i < n_edges; ++i) {
        if (muste_strcmpi(pname, edge_name[i]) == 0) {
            break;
        }
    }
    if (i == n_edges) {
        if (error) {
            muste_sprintf(sbuf, "\nEdge %s not found!", pname);
            sur_print(sbuf);
            WAIT;
            return (-1);
        } else {
...
static int get_circle(double *px, double *py, double *pr, char *pname) {
    ...
    for (i = 0; i < n_circles; ++i) {
        if (muste_strcmpi(pname, circle_name[i]) == 0) {
            break;
        }
    }
    if (i == n_circles) {
        muste_sprintf(sbuf, "\nCircle %s not found!", pname);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
