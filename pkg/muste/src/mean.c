...
static void printout() {
    ...
    output_open(eout);
    muste_sprintf(line, " Means of variables in %s N=%ld%c", word[1], n, EOS);
    ...
    for (i = 0; i < d.m_act; ++i) {
        ...
        if (w[i] == 0.0) {
            muste_sprintf(line, " %-8.8s            -  %6ld", d.varname[d.v[i]], n - f[i]);
        } else {
            fnconv(sum[i] / w[i], accuracy + 2, mean);
            muste_sprintf(line, " %-8.8s %s  %6ld", d.varname[d.v[i]], mean, n - f[i]);
        }
        print_line(line);
    }
    output_close(eout);
