...
void muste_covtest(char *argv) {
    ...
    ********************************************************/
    ...
    muste_sprintf(sbuf, "Randomization test: N=%d P=%g (s.e. %g)", nn, p_sim, sqrt(p_sim * (1 - p_sim) / nn));
    eoutput(sbuf);
    output_close(eout);
...
static int data_error(int k) {
    muste_sprintf(sbuf, "\nData sets %s and %s must have same structures!", otos[0], otos[k]);
    sur_print(sbuf);
    WAIT;
    return (1);
}
