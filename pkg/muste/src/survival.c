...
static int n = 0, strata = 0, predictors = 0, solmu = 0, nodes = 0, numero = 0, terminal = 0, memnum, nro = 1, suitable,
           eikay, parasmuut, puumalli = -1, lifetable = -1, solmuja = 0, MinObs, treshold = 1, sub, isopuu = 0;
...
static char CLASS[LLENGTH], INTERVAL[LLENGTH], NODES[LLENGTH], PENALTY[LLENGTH], variable[20], TESTI[LLENGTH],
    nimi[LLENGTH], line[LLENGTH], OBSLIMIT[LLENGTH]; // ,METHOD[LLENGTH];
...
static int *predind, *c_failed, *c_censored, *c_n_enter, *sensuroitu, *c_indeksi, *Pc_failed, *Pc_left, *Pnimittaja,
    *TerNod, *Codes;
static double *c_lower, *c_upper, *c_eff_size, *c_cpf, *c_cps, *c_cpf_error, *c_survival, *c_failure, *c_surv_error,
    *c_surv_lcl, *c_surv_ucl, *c_pdf, *c_pdf_error, *c_hazard, *c_haz_error, *c_median, *c_med_error, *c_duration,
    *c_aika, *c_class, *x, *Stat, *Tres, *Pc_survival, *Pc_failure, *Pc_st_error, *Pc_lower_cl, *Pc_upper_cl;
...
static int Laske_aikavaleja(void);
static void space_allocation(void);
static void vapauta(void);
static int Lue_Data(void);
static int Pilko(void);
static int Laske(void);
static int Tulosta(void);
static int Luo_uusi(void);
static int Talletus(void);
static int Plaske(void);
static int Ptulosta(void);
static int Pluo_uusi(void);
static int Ptalletus(void);
static int Muodosta(void);
static long double Testi(int tunnus);
static int Tulos(void);
...
static double eff_size(int n_enter, int censored) { return (n_enter - (double)censored / 2); }

static double cpf(int failed, double eff_size) { return (failed / eff_size); }

static double cps(double cpf) { return (1.0 - cpf); }

static double cpf_error(double cpf, double eff_size) { return (double)sqrt(cpf * (1 - cpf) / eff_size); }

static double survival(double ed_cps, double ed_survival) { return (ed_survival * ed_cps); }

static double failure(double survival) { return (1 - survival); }
...
static double surv_lcl(double survival, double surv_error) { return survival - 1.96 * surv_error; }

static double surv_ucl(double survival, double surv_error) { return survival + 1.96 * surv_error; }

static double pdf(double survival, double cpf) { return (survival * cpf / interval); }

static double apu_pdf_error(double cps, double eff_size) { return (1 - cps) / (eff_size * cps); }

static double pdf_error(double pdf, double cps, double eff_size, double summa3) {
    return pdf * sqrt(summa3 + cps / (eff_size * (1 - cps)));
}

static double hazard(double cpf) { return 2 * cpf / (interval * (1 + (1 - cpf))); }

static double haz_error(double hazard, double eff_size, double cpf) {
    return hazard * sqrt((1 - (interval * hazard / 2) * (interval * hazard / 2)) / (eff_size * cpf));
}

static double median(double lower, double lower2, double survival, double survival2, double survival3) {
    return lower2 - lower + interval * (survival2 - survival / 2) / (survival2 - survival3);
}

static double med_error(double survival, double pdf, double eff_size) { return survival / (2 * pdf * sqrt(eff_size)); }
...
static double Pfailure(double survival) { return (1 - survival); }
...
static double lower_cl(double survival, double st_error) { return survival - 1.96 * st_error; }

static double upper_cl(double survival, double st_error) { return survival + 1.96 * st_error; }
...
static void qsort_mtaulukko(int left, int right, tnode *node) { // J‰rjest‰‰ taulukon havainnot
    ...
    for (i = left + 1; i <= right; i++) {
        if ((c_class[i] < c_class[left]) || ((c_class[i] == c_class[left]) && (c_duration[i] < c_duration[left])) ||
            (c_class[i] == c_class[left] && c_duration[i] == c_duration[left] && sensuroitu[i] == 1)) {
...
static void qsort_aika(int left, int right, tnode *node) { // J‰rjest‰‰ taulukon havainnot
    ...
    last = left;
    for (i = left + 1; i <= right; i++) {
        if ((c_duration[i] < c_duration[left]) || (c_duration[i] == c_duration[left] && sensuroitu[i] == 1)) {
            swap_mtaulukko(++last, i, node);
        }
    }
    swap_mtaulukko(left, last, node);
...
static int Pilko() { // Pilkkoo datan luokittelumuuttujan arvojen mukaan									// ja tekee
                     // estimaattorin kullekin luokalle erikseen.
...
static int Laske() { // laskee elinajantaulun sarakkeet
        ...
        if (i == f - 1) {
            ...
            for (j = 0; j < f; j++) {
                if ((c_survival[i] / 2) >= c_survival[j]) {
                    c_median[i] = median(c_lower[i], c_lower[j - 1], c_survival[i], c_survival[j - 1], c_survival[j]);
                    break;
                } else {
                    c_median[i] = -1.0;
                }
    ...
    for (i = 0; i < f; i++) {
        if (c_median[i] <= 0.0) {
            ...
            for (j = 0; j < f; j++) {
                if ((c_survival[i] / 2) >= c_survival[j]) {
                    c_med_error[i] = med_error(c_survival[i], c_pdf[j - 1], c_eff_size[i]);
                    break;
                }
            }
        }
...
static int Tulosta() { // Tulostaa kuvaruudulle elinajantaulun
    ...
    Talletus();
    muste_sprintf(line, "Class=%.0f  N=%d  Events=%d  Censored=%d  MaxDur=%.1f", c_class[b], koko - n,
                  koko - n - censor, censor, MaxAika);
    print_line(line);
    muste_sprintf(line, "\n");
    print_line(line);
    i = spfind("LIMIT");
    ...
    if (aikavaleja <= liikaa) {
        ...
        for (i = 0; i < f; i++) {
            muste_sprintf(line, "%4.0f %5.0f %4d %4d %5d %6.1f %6.3f %7.3f %7.3f %6.3f %7.2f\n", c_lower[i], c_upper[i],
                          c_failed[i], c_censored[i], c_n_enter[i], c_eff_size[i], c_cpf[i], c_survival[i], c_pdf[i],
                          c_hazard[i], c_median[i]);
            print_line(line);
        }
    } else {
        if (strata == 0) {
    ...
    print_line(line);
    muste_sprintf(line, "LINE=3 YSCALE=0(0.2)1 XSCALE=0(%d)%d YLABEL=Survival XLABEL=Duration", (int)step,
                  (int)bestval);
    print_line(line);
    muste_sprintf(line, "......................................................................");
    print_line(line);
    ...
    print_line(line);
    muste_sprintf(line, "LINE=[YELLOW],3 YSCALE=0(0.2)1 XSCALE=0(%d)%d YLABEL=Survival XLABEL=Duration", (int)step,
                  (int)bestval);
    print_line(line);
    muste_sprintf(line, "......................................................................");
    print_line(line);
...
static int Luo_uusi() { // Luo uuden Survo-tiedoston elinajantaululle
    ...
    i = fi_create(nimi, Plen, Pm1, Pm, f, Pf, Pextra, Ptextn, Ptextlen, Ptext, varname, varlen, pvartype);
    if (i < 0) {
        return (-1);
    }
    muste_free(vartype);
...
static int Plaske() { // Laskee tulorajaestimaattorin sarakkeet
    ...
    int i, laskuri = 0, mem_left = koko - n, mem_left2 = koko - n, mem_failed = 0, counter = 0;
...
static int Ptulosta() { // Tulostaa kuvaruudulle tulorajaestimaattorin
    ...
    Ptalletus();
    muste_sprintf(line, "Class=%.0f  N=%d  Events=%d  Censored=%d  MaxDur=%.1f  Mean=%.2f", c_class[b], koko - n,
                  koko - n - censor, censor, c_duration[koko], keski);
    print_line(line);
    muste_sprintf(line, "\n");
    print_line(line);
    muste_sprintf(line, "......................................................................");
    ...
    print_line(line);
    muste_sprintf(line, "LINE=3,1,%.0f YSCALE=0(0.2)1 XSCALE=0(%d)%d YLABEL=Survival XLABEL=Duration", c_class[b],
                  (int)step, (int)bestval);
    print_line(line);
    muste_sprintf(line, "......................................................................");
    print_line(line);
    muste_sprintf(line, "\n");
...
static int Pluo_uusi() { // Luo uuden Survo-tiedoston tulorajaestimaattorille
    ...
    i = fi_create(nimi, Plen, Pm1, Pm, koko - n + 1, Pf, Pextra, Ptextn, Ptextlen, Ptext, varname, varlen, pvartype);
    if (i < 0) {
        return (-1);
    }
    muste_free(vartype);
...
static long double Testi(int tunnus) { // Laskee eloonj‰‰mifunktioiden eroja
    ...
    muste_free(tapahtumat), muste_free(riskiryh), muste_free(sensuurit), muste_free(paino), muste_free(mV2),
        muste_free(mD), muste_free(mU), muste_free(vector), muste_free(apumat), muste_free(mU2);
    for (row = 0; row < strata; row++) {
        muste_free(mV[row]);
    }
    muste_free(mV);
...
static int intra() { // Tulostaa solmujen Split Statistic -arvot kuvaruudulle
    ...
    print_line(line);
    muste_sprintf(line, "It is recommended to pick up a tree, which "
                        "Split statistic"
                        " -value          ");
    print_line(line);
...
static int Kurvit(tnode *node) { // Luo tulorajaestimaattorin kullekin puun p‰‰tesolmulle,
    ...
    print_line(line);
    muste_sprintf(line, "LINE=3,1,%d YSCALE=0(0.2)1 XSCALE=0(%d)%d YLABEL=Survival XLABEL=Duration", terminal,
                  (int)step, (int)bestval);
    print_line(line);
    muste_sprintf(line, "......................................................................");
    print_line(line);
...
static int uusiT(tnode *node) { // Luo uuden Survo-tiedoston puun kullekin
    ...
    sur_delete(nimi);
    i = fi_create(nimi, Plen, Pm1, Pm, node->datasize + 1, Pf, Pextra, Ptextn, Ptextlen, Ptext, varname, varlen,
                  pvartype);
    if (i < 0) {
        return (-1);
    }
...
void muste_survival(int argc, char *argv[]) {
    ...
    */
    ...
    i = Lue_Data();
    if (i < 0) {
        return; // Luetaan aineisto matriisiin m_taulukko
    }
    ...
    if (puumalli < 1) { // jos kyseess‰ ei ole puumalli
        ...
        k = Pilko();
        if (k < 0) {
            return; // Pilkotaan data kuhunkin luokkaan
        }
        if (strata > 1) {
        ...
        if (i >= 0) {
            ...
            if (nro < 1 || nro > 3) {
                output_open(eout);
                muste_sprintf(line,
                              "Test 1 indicates to Log-Rank, test 2 to Wilcoxon and test 3 to Tarone-Ware testscores");
                print_line(line);
                WAIT;
                output_close(eout);
                return;
