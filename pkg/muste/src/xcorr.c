...
void muste_xcorr(char *argv) {
    ...
    n = 0;
    for (i = 0; i < maxlag; ++i) {
        xx[i] = yy[i] = xy1[i] = xy2[i] = 0.0;
    }
    for (i = 0; i < maxlag; ++i) {
        xs1[i] = xs2[i] = ys1[i] = ys2[i] = 0.0;
    }
...
static int lue_datat() {
    ...
    for (i = 0; i < maxlag; ++i) {
        ...
        if (!autocorr) {
            ...
            xy1[i] = (xy1[i] - ymean * xs1[i] - xmean * ys2[i] + (n - i - 1) * xmean * ymean) / sqrt(xxs * yys);
            xy2[i] = (xy2[i] - ymean * xs2[i] - xmean * ys1[i] + (n - i - 1) * xmean * ymean) / sqrt(xxs * yys);
        }
    }
    if (!autocorr) {
        corr = (corr - n * xmean * ymean) / sqrt(xxs * yys);
    }
...
static int tulostus() {
    ...
    if (autocorr) {
        muste_sprintf(x, "Autocorrelations of %s in data %s:", word[2], word[1]);
    } else {
        muste_sprintf(x, "Auto- and crosscorrelations of %s and %s in data %s:", word[2], word[3], word[1]);
    }
    print_line(x);

    if (autocorr) {
        muste_sprintf(x, " Lag  %.8s", word[2]);
    } else {
        muste_sprintf(x, " Lag %8.8s   %8.8s       Cross+     Cross-", word[2], word[3]);
    }
    print_line(x);
    t = accuracy - 3;
    if (autocorr) {
