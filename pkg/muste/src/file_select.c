...
static int luo_uusi(char *uusi, char *uusinimi) {
    ...
    if (!muste_is_path(uusi)) {
        strcpy(uusinimi, edisk);
        strcat(uusinimi, uusi);
    }
    muste_append_path(uusinimi,
                      ".SVO"); // RS CHA if (strchr(uusinimi+strlen(uusinimi)-4,'.')==NULL) strcat(uusinimi,".SVO");
    ...
    for (j = 0; j < alku; ++j) {
        ...
        if (ferror(d_uusi.d2.survo_data)) {
            ei_tilaa();
            return (-1);
        }
    }
...
void muste_file_select(int argc, char *argv[]) {
    ...
    */
    ...
    i = conditions(&d_vanha);
    if (i < 0) {
        return;
    }

    i = data_open(word[6], &d_sel);
    if (i < 0) {
        s_end(argv[1]);
        return;
    }
    ...
    for (k = 0; k < n_sel; ++k) {
        ...
        if (i < 0) {
            sel_virhe2(osa[k], word[4]);
            return;
        }
        v_var[k] = i;
        type[k] = d_vanha.vartype[i][0];
        i = varfind2(&d_sel, osa[k], 0);
        if (i < 0) {
            sel_virhe2(osa[k], word[6]);
            return;
        }
        s_var[k] = i;

        if (type[k] != d_sel.vartype[i][0] || d_vanha.varlen[v_var[k]] != d_sel.varlen[i]) {
            muste_sprintf(sbuf, "\nType/length of field %s not same in %s and %s!", osa[k], word[4], word[6]);
            sur_print(sbuf);
            WAIT;
            return;
        }
    ...
    if (i >= 0) {
        ...
        if (mark_var < 0) {
            muste_sprintf(sbuf, "MARK variable %s not found in data file %s!", spb[i], word[4]);
            sur_print(sbuf);
            WAIT;
            return;
        }
    ...
    if (i >= 0) {
        ...
        if (nmatch_var < 0) {
            muste_sprintf(sbuf, "NMATCH variable %s not found in data file %s!", spb[i], word[6]);
            sur_print(sbuf);
            WAIT;
            return;
        }
    ...
    ***************************************************************/
    ...
    for (k = 0; k < n_sel; ++k) {
        i_sel[k] = ts_len;
        if (type[k] == 'S') {
            s_len[k] = d_sel.varlen[s_var[k]];
        } else {
            s_len[k] = sizeof(double);
        }
        ts_len += s_len[k];
    }
        ...
        for (l2 = 1; l2 <= n2; ++l2) // RS CHA l2=1L
        {
            for (k = 0; k < n_sel; ++k) {
                if (type[k] == 'S') {
                    ...
                    if (strncmp(v_tila + i_sel[k], s_tila + (l2 - 1) * ts_len + i_sel[k], s_len[k]) != 0) {
                        break;
                    }
                    ...
                    if (*(double *)(v_tila + i_sel[k]) != *(double *)(s_tila + (l2 - 1) * ts_len + i_sel[k])) {
                        break;
                    }
                }
            } /* k */
