...
static int op_compd(void);
static int d_load_samples(void);
static int sort_sample(void);
static int test_normality(void);
static int d_basic_statistics(void);
static int Shapiro_Wilk(void);
static int Dagostino(void);
...
static void op_compare() {
    ...
    strcpy(survoxxx, etmpd);
    strcat(survoxxx, "SURVO.XXX");
    ...
    muste_sprintf(sbuf, "\nUnknown specification TEST=%s", test);
    sur_print(sbuf);
    WAIT;
}
...
static int sort_joint_sample() {
    ...
    while (k < n_total) {
        y = x_space[k];
        h = 1;
        while (k + h < n_total && y == x_space[k + h]) {
            ++h;
        }
        --h;
        a = (float)(k + 1 + h / 2.0);
...
// int test;   /* 0=t 1=all */
{
    ...
    fconv((double)rsum1, "", x);
    set(rivi, x, p1);
    fconv((double)rsum2, "", x);
    set(rivi, x, p2);
    eoutput(rivi);
    ...
    muste_sprintf(rivi, "(P=%s one-sided Mann-Whitney, normal approximation)", spois(strp));
    eoutput(rivi);
    ...
    while (1) {
        ...
        if (d == d1) {
            ...
            s2 = (float)r1 / (float)u;
            muste_sprintf(sbuf, "Critical level %7.5f %.5f  N=%d", (double)s1, s2, u);
            sur_print(sbuf);
...
static int basic_statistics() {
    ...
    muste_sprintf(x, "%d", ns[0]);
    set(rivi, x, p1);
    muste_sprintf(x, "%d", ns[1]);
    set(rivi, x, p2);
    eoutput(rivi);
    ...
    init_rivi(rivi);
    set(rivi, "Mean", 0);
    fnconv(sum1 / ns[0], accuracy + 2, x);
    set(rivi, x, p1);
    fnconv(sum2 / ns[1], accuracy + 2, x);
    set(rivi, x, p2);
    eoutput(rivi);
    init_rivi(rivi);
    ...
    */
    muste_sprintf(rivi, "Student's t=%s df=%d (P=%s one-sided test)", spois(x), ns[0] + ns[1] - 2, spois(strp));
    eoutput(rivi);
    return (1);
}
...
static int ww_test(int n, int n1, int nr) {
    ...
    for (i = 3; i <= nr; ++i) {
        ...
        if (pariton) {
            pr = ((double)(n0 + n1) / (double)k - 2.0) * a;
        } else {
            pr = 2 * a;
        }
        dp_run += pr;
...
static int make_qq_data() {
    ...
    ***********************************/
    if (n1 < n2) {
        ...
        for (i = 0; i < n1; ++i) {
            qp1[i] = ((double)i + 0.625) / ((double)n1 + 0.25);
        }
        for (i = 0; i < n2; ++i) {
            qp2[i] = ((double)i + 0.625) / ((double)n2 + 0.25);
        }
    ...
    output_open(eout);
    eoutput("..........");
    muste_sprintf(rivi, "Data for Q-Q plot %s vs. %s saved in _QQ.SVO", word[1], word[2]);
    eoutput(rivi);
...
static int qq_create_data(char *name, double *x, double *y, int n) {
    ...
    fitextn = 1;
    fitextlen = c2;
    muste_sprintf(tt, "Data for Q-Q plot: %s vs. %s", word[1], word[2]);
    privi[0] = tt;
    fitext = privi;

    fi_create(nimi, 16, 2, 2, (int)n, 64, 8, fitextn, fitextlen, fitext, varname, varlen, vartype);
    data_open(nimi, &d);
    for (j = 1; j <= n; ++j) {
...
static int Smirnov() {
    ...
    */
    ...
    while (j1 < ns[0] && j2 < ns[1]) {
        ...
        k1 = 0;
        while (j1 + k1 < ns[0] && x2[sample[0] + j1 + k1] <= a) {
            ++k1; /* j?rj. vaihd. 17.4.1996 */
        }
        k2 = 0;
        while (j2 + k2 < ns[1] && x2[sample[1] + j2 + k2] <= a) {
            ++k2;
        }
        j1 += k1;
        j2 += k2;
    ...
    muste_sprintf(rivi, "Kolmogorov-Smirnov test D=%7.5f (P=%.5f) D(+)=%7.5f D(-)=%7.5f", ad0, a, ad1, ad2);
    eoutput(rivi);
    ...
    while (1) {
        ...
        if (d == d1) {
            ...
            muste_sprintf(sbuf, "\nStandard error            %7.5f %12.5f %12.5f",
                          muste_sqrt(s * (1.0 - s) / (double)t), muste_sqrt(s1 * (1.0 - s1) / (double)t),
                          muste_sqrt(s2 * (1.0 - s2) / (double)t));
            sur_print(sbuf);
    ...
    eoutput(rivi);
    muste_sprintf(rivi, "Standard error            %7.5f %12.5f %12.5f", muste_sqrt(s * (1.0 - s) / (double)u),
                  muste_sqrt(s1 * (1.0 - s1) / (double)u), muste_sqrt(s2 * (1.0 - s2) / (double)u));
    eoutput(rivi);
    output_close(eout);
    return (1);
}
...
// int test;  /* 0=F 1=all */
{
    ...
    dn = (double)n_total;
    a = dn * (dn + 1.0) * (dn + 1.0) / 4.0;
    s2 = (s2 - a) / (dn - 1.0);
    t = 0.0;
    for (i = 0; i < n_sample; ++i) {
        t += ri[i] * ri[i] / (double)ns[i];
    }
    kw = (t - a) / s2;
    ...
    */
    ...
    while (1) {
        ...
        if (d == d1) {
            d = 0;
            s1 = (double)u1 / (double)u;
            muste_sprintf(sbuf, "Critical level %7.5f  N=%d", s1, u);
            sur_print(sbuf);
...
static int anova() {
    ...
    for (h = 0; h < n_sample; ++h) {
        init_rivi(rivi);
        set(rivi, word[h + 1], 0);
        muste_sprintf(x, "%4d", ns[h]);
        set(rivi, x, 18);
        fnconv(p[h], accuracy + 2, x);
        set(rivi, x, 25);
        fnconv(muste_sqrt(q[h]), accuracy + 2, x);
        set(rivi, x, 40);
        eoutput(rivi);
    }
...
static int Wilcoxon() {
    ...
    */
    muste_sprintf(rivi, "Wilcoxon signed ranks test=%s (P=%s normal approximation)", spois(x), spois(strp));
    eoutput(rivi);
    ...
    for (k = 0; k < n; ++k) {
        ...
        diff[k] = muste_fabs(diff[k]);
        srank[k] = muste_fabs(srank[k]);
    }
    ...
    while (1) {
        ...
        if (d == d1) {
            ...
            s1 = (double)u1 / (double)u;
            s2 = (float)r1 / (float)u;
            muste_sprintf(sbuf, "Critical level %7.5f %11.5f  N=%d", (double)s1, s2, u);
            sur_print(sbuf);
...
static int paired_statistics() {
    ...
    muste_sprintf(rivi, "Samples: N=%d            %-15.14s %-15.14s Difference", n, word[1], word[2]);
    eoutput(rivi);
    ...
    init_rivi(rivi);
    set(rivi, "Mean", 0);
    fnconv(mx, accuracy + 2, xx);
    set(rivi, xx, p1);
    fnconv(my, accuracy + 2, xx);
    set(rivi, xx, p2);
    fnconv(md, accuracy + 2, xx);
    set(rivi, xx, 2 * p2 - p1);
    eoutput(rivi);
    set(rivi, "Standard deviation", 0);
    fnconv(sx, accuracy + 2, xx);
    set(rivi, xx, p1);
    fnconv(sy, accuracy + 2, xx);
    set(rivi, xx, p2);
    fnconv(sd, accuracy + 2, xx);
    set(rivi, xx, 2 * p2 - p1);
    eoutput(rivi);
   ...
   */
    muste_sprintf(rivi, "Paired t=%s (P=%s one-sided t test df=%d)", spois(xx), spois(strp), n - 1);
    eoutput(rivi);
    return (1);
}
...
static int rank_corr() {
    ...
    */
    ...
    /*
            ...
            used += snprintf(rivi + used, LLENGTH - used, " normal approximation)");
    */
    muste_sprintf(rivi, "Kendall's  Tau=%s (Nc=%d Nd=%d P=%s normal approximation)", spois(x), nc, nd, spois(strp));
    eoutput(rivi);
    if (ptau >= 0.0) {
...
static int corr_statistics(double *par, double *psr1) {
    ...
    muste_sprintf(rivi, "Samples: N=%d            %-15.14s %-15.14s", n, word[1], word[2]);
    eoutput(rivi);
    ...
    */
    muste_sprintf(rivi, "Product moment correlation R=%s (P=%s)", spois(xx), spois(strp));
    eoutput(rivi);
...
static int d_basic_statistics() {
    ...
    */
    muste_sprintf(rivi, "Skewness=%s  Kurtosis=%s  (normal values=0)", spois(x), spois(y));
    eoutput(rivi);
...
static int Shapiro_Wilk() {
    ...
    strcpy(tbl_name, survo_path);
    strcat(tbl_name, "SYS/SWCOEFF.TBL"); // RS CHA TBL\\ -> SYS/
    ...
    n2 = (n_total + 1) / 2;
    if (2 * n2 != n_total + 1) {
        seekpos = n_total * n_total / 4;
    } else {
        seekpos = (n_total * n_total - 1) / 4;
    }
    ...
    sw = 0.0;
    for (i = 0; i < n2; ++i) {
        sw += coeff[i] * (x_space[n_total - i - 1] - x_space[i]);
    }
    sw = sw * sw / sd_copy; // sd_copy saatu edellisess? funktiossa

    strcpy(tbl_name, survo_path);
    strcat(tbl_name, "SYS/SWPERC.TBL"); // RS CHA TBL\\ -> SYS/
    taulu = muste_fopen(tbl_name, "rb");
    if (taulu == NULL) {
...
static int Dagostino() // 18.7.2011/SM
{
    ...
    varb2 = 24.0 * dn * (dn - 2.0) * (dn - 3.0) / ((dn + 1.0) * (dn + 1.0) * (dn + 3.0) * (dn + 5.0));
    c = (b2 - meanb2) / muste_sqrt(varb2);
    moment = 6.0 * (dn * dn - 5.0 * dn + 2.0) / ((dn + 7.0) * (dn + 9.0)) *
             muste_sqrt(6.0 * (dn + 3.0) * (dn + 5.0) / (dn * (dn - 2.0) * (dn - 3.0)));
    a = 6.0 + 8.0 / moment * (2 / moment + muste_sqrt(1.0 + 4.0 / (moment * moment)));
...
static int Anderson_Darling() {
    ...
    for (i = 0; i < 22; ++i) {
        p[i] = aa[i] * (1 + (b0[i] + b1[i] / n_total) / n_total);
    }
