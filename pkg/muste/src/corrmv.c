...
static int summat(void);
static int sijoita_keskiarvot(void);
static int momentit(void);
...
static int korjaa_puuttuvat(void);
static int pairwise_corr(void);
static int pair_varaus(void);
static int talleta_frekvenssit(void);
static int pair_tulostus(void);
static int pair_talletus(void);
...
void muste_corrmv(char *argv) {
    ...
    if (i >= 0) {
        strcpy(x2, spb[i]);
        if (strcmp(x2, "PAIRWISE2") == 0 || atoi(x2) == 3) {
            method = 3;
        } else if (muste_strnicmp(x2, "PAIR", 4) == 0 || atoi(x2) == 2) {
            method = 2;
        }
    }
    max_iter = 20;
    ...
    while (iter < max_iter) {
        ...
        ++iter;
        muste_sprintf(sbuf, "\nIteration %d: mean squared difference %g", iter, ero / (double)miss_total);
        if (ero > ero0) {
...
static int tell_miss(char *s) {
    muste_sprintf(s, "%d missing values of %d (%.2g per cent)", miss_total, n * m,
                  100.0 * (double)miss_total / (n * m));
    return (1);
}
...
static int sijoita_keskiarvot() {
    ...
    for (i = 0; i < m; ++i) {
        ...
        if (b < 1e-30) {
            muste_sprintf(sbuf, "\nVariable %.8s is a constant %g", d.varname[d.v[i]], mm1[i]);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
...
static int mv_corrp(double S[], int m,      /* dimensio */
                    char *xname[], int lev, /* kentän leveys */
    ...
    output_open(eout);
    strcpy(xsar, " %");
    strcat(xsar, muste_itoa(sar - 1, ind, 10));
    strcat(xsar, "s");
    strcpy(xriv, " %-");
    strcat(xriv, muste_itoa(sar + 3, ind, 10));
    strcat(xriv, "s");
    strcpy(rriv, "%");
    strcat(rriv, muste_itoa(sar, ind, 10));
    strcat(rriv, ".");
    strcat(rriv, muste_itoa(des, ind, 10));
    strcat(rriv, "f");
    ...
    while (j2 <= m - 1) {
        k = 0;
        for (i = 0; i < sar + 4; ++i) {
            k += muste_sprintf(rivi + k, " ");
        }
        for (j = j1; j <= j2; ++j) {
            strcpy(nimi, xname[j]);
            nimi[sar - 1] = EOS;
            if (sar - 1 > 8) {
                nimi[8] = EOS; // 23.9.2011
            }
...
static int tulostus() {
    ...
    muste_sprintf(rivi, "Means, std.devs and correlations of %s  N=%d", word[1], n);
    eoutput(rivi);
    if (miss_total) {
        ...
        tell_miss(rivi);
        eoutput(rivi);
        muste_sprintf(rivi, "Results after %d iterations, mean squared difference %g", iter, ero / (double)miss_total);
        eoutput(rivi);
    } else {
    ...
    for (i = 0; i < m; ++i) {
        ...
        muste_sprintf(rivi, "%-8.8s %*s   %*s", d.varname[d.v[i]], accuracy + 2, mean, accuracy + 2, stddev);
        eoutput(rivi);
    }
    output_close(eout);
    if (m > 1) {
...
static int iteroi(int iter) {
    ...
    if (i < 1) {
        if (-i < m) {
            muste_sprintf(sbuf, "\nVariable %.8s is linearly dependent on the previous ones!", d.varname[d.v[-i]]);
        } else {
...
static int pairwise_corr() {
    ...
    for (i = 0; i < m; ++i) {
        ...
        for (j = 0; j <= i; ++j) {
            ...
            nn[i + m * j] = 0L;
            if (method == 3) {
                xy[i + m * j] = xy[j + m * i] = xy2[i + m * j] = xy2[j + m * i] = 0.0;
            }
        }
    }
    ...
    for (i = 0; i < m; ++i) {
        sum2[i] = sqrt((sum2[i] - n1[i] * sum[i] * sum[i]) / (n1[i] - 1L));
    }
    ...
    if (method == 2) {
        for (i = 0; i < m; ++i) {
            for (j = 0; j < i; ++j) {
                A[i + m * j] =
                    (A[i + m * j] - nn[i + m * j] * sum[i] * sum[j]) / ((nn[i + m * j] - 1L) * sum2[i] * sum2[j]);
            }
        }
    } else {
        for (i = 0; i < m; ++i) {
            for (j = 0; j < i; ++j) {
                h1 = i + m * j;
                h2 = j + m * i;
                A[h1] = (A[h1] - nn[h1] * xy[h1] * xy[h2]) / ((nn[h1] - 1L) * xy2[h1] * xy2[h2]);
            }
        }
    }
...
static int pair_tulostus() {
    ...
    if (m > 1 && results > 0) {
        muste_sprintf(rivi, "Means, std.devs and pairwise correlations of %s  N=%d", word[1], n);
    } else {
    ...
    for (i = 0; i < m; ++i) {
        ...
        fnconv(sum[i], accuracy + 2, mean);
        fnconv(sum2[i], accuracy + 2, stddev);
        muste_sprintf(rivi, "%-8.8s %*s   %*s %7d", d.varname[d.v[i]], accuracy + 2, mean, accuracy + 2, stddev, n1[i]);
        eoutput(rivi);
    }
    output_close(eout);
    if (m > 1) {
...
static int replacement() {
    ...
    for (i = 0; i < m; ++i) {
        res_dev[i] = sqrt(1.0 / (double)(n - m1) / res_dev[i]);
    }
    ...
    for (l = d.l1; l <= d.l2; ++l) {
        ...
        for (i = 0; i < m; ++i) {
            if (miss[i]) {
                if (replace == 1) {
                    sum[i] += res_dev[i] * normal_dev(); /* 20.6.1996 */
                }
