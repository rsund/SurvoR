...
static int nelder(double *x, double *py, int n, double (*f)(double[]), double *step, double alpha, double beta,
                  double gamma, char **varname, char *fname, double yeps);
...
void muste_forecast(char *argv) {
    ...
    if (i < 0) {
        if (d.l2 + ahead > d.n) {
            ...
            if (d.type == 2) {
                muste_sprintf(sbuf, "\nUse FILE INIT %s,%d  for example!", word[1], d.l2 + ahead - d.n);
                sur_print(sbuf);
            }
            WAIT;
        }
...
static int init_hw() {
    ...
    if (type == 1) {
        ...
        for (t = 1; t <= per; ++t) {
            a = 0.0;
            for (h = 0; h < k; ++h) {
                a += x[h * per + t] / (mean[h] - rr[0] * ((per + 1.0) / 2.0 - t));
            }
            s[t - per] = a / k;
        }
        a = 0.0;
...
static int printout() {
    ...
    muste_sprintf(y, "Holt-Winters' %s Seasonal Forecast: Data %s, Variable %s", laji[type], word[1], word[2]);
    print_line(y);
    *sana = EOS;
    if (autom_period) {
        strcpy(sana, "(judged from data)  ");
    }
    muste_sprintf(y, "Period=%d obs. %sEstimation on observations %d-%d", per, sana, d.l1, d.l2);
    print_line(y);
    ...
    if (noutlier) {
        ...
        if (more_outliers) {
            strcat(y, " (+more to be found)");
        }
        print_line(y);
    }

    muste_sprintf(y, "MSE=%g %s=%.3f %s=%.3f %s=%.3f", mse, parnimi[0], par[0], parnimi[1], par[1], parnimi[2], par[2]);
    print_line(y);
    ...
    if (predvar >= 0) {
        muste_sprintf(y, "Predicted values saved as variable %s (obs. %d-%d)", word[3], d.l1, d.l2 + (int)per);
        print_line(y);
    }
    return (1);
}
...
// double yeps;  /* esim. 0.001 */
{
    ...
    while (1) {
        ...
        if (disp) {
            ...
            sur_print(sbuf);
            for (i = 0; i < n; ++i) {
                muste_sprintf(sbuf, " %s=%g", varname[i], xx[jl][i]);
                sur_print(sbuf);
            }
            muste_sprintf(sbuf, " nf=%d", nf);
            sur_print(sbuf);
        }
        if (nf >= nfi + 200) {
            nfi += 200;
            muste_sprintf(sbuf, "\n nf=%d  To interrupt, press '.'", nf);
            sur_print(sbuf);
        }
        ++nstop;
        if (nstop == stopstep) {
            nstop = 0;
            if ((ylast - y[jl]) / (fabs(ylast) + 1e-30) < yeps) {
                break;
            }
            ylast = y[jl];
        }
...
// int permax;
{
    ...
    if (sur_kbhit()) {
        ...
        sur_print("\n");
        for (i = 0; i < maxlag; ++i) {
            muste_sprintf(sbuf, "MSE%d=%.2g ", i + 1, ms[i] / min);
            sur_print(sbuf);
        }
        WAIT;
    }
