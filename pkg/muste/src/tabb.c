...
static int printout() {
    ...
    for (i = colvar; i < nclassf; ++i) {
        ...
        if (cellvar < 0) {
            ...
            k = print_ftable(name, results_line, eout, dim, nc, cumnc, ctype, f[i], total, missing[i] + misscol,
            ...
            table_name(name, word[1], ++index);
            k = print_stable(name, results_line, eout, dim, nc, cumnc, ctype, f[i], total, missing[i] + misscol,
                             varname, cname, colv, width, y, cellformat, sum[i], sum2[i], celloption, tab_labels);
        }
        if (results_line && k > 1) {
            results_line = k;
        } else {
...
static int compute_frequencies() {
    ...
    for (i = colvar; i < nclassf; ++i) {
        for (u = 0; u < nspace * nc[i]; ++u) {
            f[i][u] = ZERO;
        }
        if (cellvar >= 0) {
            for (u = 0; u < nspace * nc[i]; ++u) {
                sum[i][u] = sum2[i][u] = 0.0;
            }
        }
        missing[i] = ZERO;
    }
