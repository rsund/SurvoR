...
/* i;  akt. muuttuja 0,1,2,...  */
{
    ...
    /*      else
                ...
                PR_EINV;
                muste_sprintf(sbuf,"%s",edsana); sur_print(sbuf);
                }
    */
    LOCATE(rivi[i], varsar[i]);

    if (dat.vartype[v[muuttuja]][2] == 'P' && havainto <= n_alku) {
        suojaus = 1;
    } else {
        suojaus = 0;
    }
...
static void hav_disp(long j) {
    ...
    /*      if (ibm)
                {
    */
    i = muste_sprintf(x, "Record #%6ld  ", j);
    if (dat.vartype[0][0] == 'S') {
        fi_alpha_load(&dat, j, 0, sana);
        i += muste_sprintf(x + i, "%.16s", sana);
    }
    ...
    /*
                ...
                {
                PR_ENRM; LOCATE(2,30); muste_sprintf(sbuf,"Record #%6ld  ",j); sur_print(sbuf);
                if (dat.vartype[0][0]=='S')
                    {
                    fi_alpha_load(&dat,j,0,sana);
                    muste_sprintf(sbuf,"%.16s",sana); sur_print(sbuf);
                    }
                }
    */
    for (i = 0; i < m_act; ++i) {
        ...
        if (ibm) {
            strcpy(x, sana);
            strncat(x, space, varpit[i] - strlen(sana));
            write_string(x, varpit[i], '7', rivi[i], varsar[i]);
        } else {
            LOCATE(rivi[i], varsar[i]);
            PR_EINV;
            muste_sprintf(sbuf, "%s%.*s", sana, (int)(varpit[i] - strlen(sana)), space);
            sur_print(sbuf);
        }
...
static int edit_init() {
    ...
    CLS;
    LOCATE(2, 1);
    muste_sprintf(sbuf, "Data file %s", active_data);
    sur_print(sbuf);
    LOCATE(2, 70);
    muste_sprintf(sbuf, "N=%ld", n);
    sur_print(sbuf);
    ...
    for (i = 0; i < m_act; ++i) {
        ...
        if (mriville > 1) {
            ...
            nimsar[i] = sar + 8 - k;
            LOCATE(riv, nimsar[i]);
            muste_sprintf(sbuf, "%s ", label);
            sur_print(sbuf);
        } else {
            muste_sprintf(sbuf, "%.8s ", dat.varname[v[i]]);
            sur_print(sbuf);
            if (kokonimi) {
                LOCATE(riv, varsar[i] + varpit[i] + 1);
                muste_sprintf(sbuf, "%.*s", c3 + 8 - varsar[i] - varpit[i], dat.varname[v[i]] + 8);
                sur_print(sbuf);
            }
        }
        sar += 9 + varpit[i];
    ...
    n_alku = dat.n;
    if (n_alku == 0L) {
        n_alku = 100000L; /* tyhjÑ tied. ei suojattu */
    }
    return (1);
}
                                                ...
                                                /* n;  new obs.# */
{
    ...
    (*s).n = n;
    LOCATE(2, 70);
    muste_sprintf(sbuf, "N=%6d", n);
    sur_print(sbuf); // RS CHA %6ld -> %6d
}
...
static int talletus() {
            ...
            *p = EOS;
            ...
            if (m) {
                ...
                if (i == m) {
                    ...
                    LOCATE(r3 + 2, 1);
                    PR_EBLD;
                    muste_sprintf(sbuf, "%s is not a permitted value for %.8s", edsana, dat.varname[v[muuttuja]]);
                    sur_print(sbuf);
                    BEEP;
    ...
    if (strncmp(edsana, space, varpit[muuttuja]) == 0) {
        ...
        if (x < min[muuttuja] || x > max[muuttuja]) {
            ...
            PR_EBLD;
            if (x < min[muuttuja]) {
                muste_sprintf(sbuf, "%.8s < %g (smallest permitted value)", dat.varname[v[muuttuja]], min[muuttuja]);
                sur_print(sbuf);
            } else {
                muste_sprintf(sbuf, "%.8s > %g (greatest permitted value)", dat.varname[v[muuttuja]], max[muuttuja]);
                sur_print(sbuf);
            }
            BEEP;
            sur_print("  Press any key!");
...
static void alas() {
    ...
    if (muuttuja < m_act - 1) {
        i = muuttuja;
        while (i < m_act && rivi[i] == rivi[muuttuja]) {
            ++i;
        }
        if (i == m_act) {
            return;
        ...
        seur_rivi = rivi[i];
        while (i < m_act && rivi[i] == seur_rivi && varsar[i] < varsar[muuttuja]) {
            ++i;
        }
        if (i == m_act || rivi[i] != seur_rivi) {
            --i;
        }
        muuttuja = i;
        osoita(muuttuja);
...
static void ylos() {
    ...
    if (muuttuja > 0) {
        i = muuttuja;
        while (i >= 0 && rivi[i] == rivi[muuttuja]) {
            --i;
        }
        if (i < 0) {
            return;
        ...
        edell_rivi = rivi[i];
        while (i >= 0 && rivi[i] == edell_rivi && varsar[i] > varsar[muuttuja]) {
            --i;
        }
        if (i < 0 || rivi[i] != edell_rivi) {
            ++i;
        }
        muuttuja = i;
        osoita(muuttuja);
   ...
   j3;  destination */
{
    ...
    for (j = j1; j <= j2; ++j) {
        fi_rewind(s);
        fi_gets(s, (*s).obs, (*s).len, (long)((*s).data + (j - 1L) * (long)(*s).len));
        for (i = 0; i < (*s).len; ++i) {
    ...
    if (j3 < j1) {
        ...
        for (j = j1 - 1; j >= j3; --j) {
            ...
            fi_gets(s, (*s).obs, (*s).len, (long)((*s).data + (j - 1L) * (long)(*s).len));
            fi_rewind(s);
            fi_puts(s, (*s).obs, (*s).len, (long)((*s).data + (j + j2 - j1 + 1L - 1L) * (long)(*s).len));
        }
    } else {
        k = j3 - j2 - 1;
        for (j = j2 + 1; j <= j3 - 1; ++j) {
    ...
    for (j = j1; j <= j2; ++j) {
        ...
        fi_rewind(s);
        fi_puts(s, (*s).obs, (*s).len, (long)((*s).data + (j + k - 1L) * (long)(*s).len));
    }
    muste_fclose(apu);
    havainto = j1 + k;
}
   ...
   j2;  last obs. to be deleted */
{
    ...
    for (j = j2 + 1L; j <= (*s).n; ++j) {
        ...
        fi_gets(s, (*s).obs, (*s).len, (long)((*s).data + (j - 1L) * (long)(*s).len));
        fi_rewind(s);
        fi_puts(s, (*s).obs, (*s).len, (long)((*s).data + (j - j2 + j1 - 2L) * (long)(*s).len));

        LOCATE(r3 + 2, 70);
        muste_sprintf(sbuf, "%d%% done!", (int)(100 * (j - j2) / ((*s).n - j2))); // RS ADD (int)
        sur_print(sbuf);
    }
...
static void k_copy() {
    ...
    fi_rewind(&dat);
    fi_gets(&dat, dat.obs, dat.len, (long)(dat.data + (hav - 1L) * dat.len));
    fi_rewind(&dat);
    fi_puts(&dat, dat.obs, dat.len, (long)(dat.data + (havainto - 1L) * dat.len));
    putsaa();
    hav_disp(havainto);
}
    ...
    while (1) {
        ...
        if (hav > dat.n) {
            ...
            if (n_haku == 0) {
                ...
                muste_sprintf(sbuf, "%d cases found.", n_haku);
                sur_print(sbuf);
            }
            sur_print(" Press any key!");
            osoita(muuttuja);
...
static void k_help() {
    ...
    PR_EINV;
    muste_sprintf(sbuf, "%s", key_label[CODE_SRCH]);
    sur_print(sbuf);
    PR_ENRM;
    sur_print(")");
...
void muste_file_edit(int argc, char *argv[]) {
    ...
    while (kesken) {
        if (survo_ferror) {
            ...
            PR_EBLD;
            BEEP;
            sur_print("FILE EDIT Error! Cannot save! (Disk full?)  Press any key!");
            WAIT;
            break;
        }
        ch = nextch(tiedotus);
        switch (ch) {
        case 22:
            break; // alt-TAB-ongelma (Win2000) 31.10.01
        ...
        case CODE_INSERT:
            ...
            for (i = pos; i <= pituus; ++i) {
                muste_sprintf(sbuf, "%c", edsana[i - 1]);
                sur_print(sbuf);
            }
            for (i = pos; i <= pituus; ++i) {
                PR_LEFT;
        ...
        case CODE_REF:
            ...
            LOCATE(r3 + 2, 1);
            PR_EINV;
            muste_sprintf(sbuf, "%.*s (Press any key!)", c3 - 10, dat.varname[v[muuttuja]]);
            sur_print(sbuf);
            osoita(muuttuja);
            sur_getch(); // RS CHA nextch("");
            putsaa();
