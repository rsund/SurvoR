...
static char *res(double y, char *x) {
    char *p;

    fnconv(y, accuracy + 2, x);
    if (strchr(x, 'e') != NULL || strchr(x, '.') == NULL) {
        return (x);
    }
    p = x + strlen(x) - 1;
    while (p > x && *p == '0') {
...
static int test_scaletypes() {
    ...
    if (weight_variable >= 0) {
        if (!scale_ok(&d, weight_variable, RATIO_SCALE)) {
            muste_sprintf(sbuf, "\nWeight variable %.8s must have ratio scale!", d.varname[weight_variable]);
            sur_print(sbuf);
            WAIT;
...
static int interval_classify(int i, double x) {
    int class;
    ...
    if (fabs(cwidth[i]) < DEPS) {
        if (debug) {
            debug_print("interval_classify");
        }
        class = ceil((x - cstart[i]) - 1.0);
    } // RS CHA 2.10.2012 if else
    else {
        class = ceil((x - cstart[i]) / cwidth[i] - 1.0);
    }

    if (debug) {
        muste_sprintf(sbuf, "interval_classify: x: %f, class: %d", x, class);
        debug_print(sbuf);
    }

    if (class < 0 || class > maxc - 1) {
        double_width(i, x);
        return (1);
    }
    ++freq[i * maxc + class];
...
static int double_width(int i, double x) {
    int k, k1, k2;
    double askel, start;
    ...
    while (1) {
        ...
        k1 = (cbest[i] - min[i]) / askel + 1;
        k2 = (max[i] - cbest[i]) / askel + 1;
        k = (maxc - k1 - k2) / 2;
    ...
    for (k = 0; k < maxc; ++k) {
        if (fabs(askel) < DEPS) {
            if (debug) {
                debug_print("double_width2");
            }
            k1 = ceil((cstart[i] + (k + 0.5) * cwidth[i] - start));
        } // RS ADD 2.10.2012 if else
        else {
...
static int create_intervals(int i, double x) {
    double paras, askel;
    double minstep;
    int k1, k2, k;
    ...
    if (fabs(askel) < DEPS) {
        if (debug) {
            debug_print("create_intervals1");
        }
        k1 = (paras - (int)min[i]) + 1;
        k2 = ((int)max[i] - paras) + 1;
    } // RS ADD 2.10.2012 if else
    else {
        k1 = (paras - (int)min[i]) / askel + 1;
        k2 = ((int)max[i] - paras) / askel + 1;
    }
    k = (maxc - k1 - k2) / 2;
    ...
    for (k = 0; k < nclass[i]; ++k) {
        if (fabs(askel) < DEPS) {
            if (debug) {
                debug_print("create_intervals2");
            }
            k1 = ceil(class[i * maxc + k] - cstart[i]);
        } // RS ADD 2.10.2012 if else
        else {
...
static int classify(int i, double x) {
    int imc;
    int k, h;
    ...
    k = 0;
    while (k < nclass[i] && x > class[imc + k]) {
        ++k;
    }
    if (k == nclass[i]) {
        if (nclass[i] == maxc) {
            create_intervals(i, x);
            return (1);
        }
        ++nclass[i];
        class[imc + k] = x;
    ...
    for (h = nclass[i]; h > k; --h) {
        class[imc + h] = class[imc + h - 1];
        freq[imc + h] = freq[imc + h - 1];
    }
    ++nclass[i];
    class[imc + k] = x;
    freq[imc + k] = 1;
...
static int str_classify(int i, char *s) {
    int imc;
    int k, h;
    int vert = 0;
    int len;
    ...
    k = 0;
    while (k < n_str_class[i] && (vert = strcmp(s, str_class[imc + k])) > 0) {
        ++k;
    }
    ...
    if (k == n_str_class[i]) {
        if (n_str_class[i] == maxc) {
            n_str_class[i] = -1;
            return (1);
        }
        ++n_str_class[i];
        str_freq[imc + k] = 1;
        str_class[imc + k] = p_str;
        strcpy(p_str, s);
        p_str += len + 1;
        return (1);
    }
...
static int stat_m_rowname(int i, char *name) {
    int k;

    *sbuf = EOS;
    strncat(sbuf, name, 8);

    for (k = 0; k < strlen(sbuf); ++k) {
        rlab[8 * i + k] = sbuf[k];
    }
    for (k = strlen(sbuf); k < 8; ++k) {
        rlab[8 * i + k] = ' ';
    }
    return (1);
}

static int stat_m_save(int i, int col, char *name, double value) {
    int k;
...
static int siev(double x, char *form, char *sana) {
    char *p;
...
/* int is;       0,...,m_str-1 */
{
    ...
    for (h = 0; h < n_str_class[is]; ++h) {
        ...
        if (str_freq[is * maxc + h] > maxf) {
            maxf = str_freq[is * maxc + h];
        }
    }
    lcname = len;
    if (lcname < 8) {
    ...
    maxbar = c3 - k - 1;
    if (maxbar < 1) {
        maxbar = 1; // RS ADD 25.5.2012
    }
    step = 1L;
    while ((double)maxf / (double)step + 1.0 > (double)maxbar) {
        step *= 2;
    }
    if (step > 1L) {
        k += muste_sprintf(line + k, "     %c=%d obs.  ", barchar, step);
    }
    print_line(line);
    ...
    for (h = 0; h < n_str_class[is]; ++h) {
        ...
        lev = fr / step;
        muste_sprintf(line, "%-*.*s %6ld %5.1f %.*s", lcname, lcname, str_class[is * maxc + h], fr,
                      (double)(100.0 * fr / nobs[i]), lev, bar);
        if (fr && !lev) {
            strcat(line, ":");
        }
        print_line(line);
...
static int print_confmean(double confmean, double mean, double stddev, double n) {
    double low, up;
    ...
    low = mean - up;
    up = mean + up;
    muste_sprintf(sbuf, "%g confidence interval for mean: low=%g up=%g", confmean, low, up);
    print_line(sbuf);

    return (1);
}

static double fractile(double pr, int i) {
    int k;
    ...
    return (ed_class + (class[im + k] - ed_class) * (pr_freq - ed_freq) / (sum_freq - ed_freq));
}
...
static int print_fractiles(int i) {
    ...
    for (k = 0; k < nfract; ++k) {
        ...
        h = muste_sprintf(line, "fractile(%s)=%s", fra[k], spois(res(fractile(pr, i), sana)));
        print_line(line);
    }
    return (1);
}
...
static int print_means(int i) {
    ...
    for (k = 0; k < n_means; ++k) {
        ...
        if (a == 0.0) {
            b = exp(b);
            strcpy(nimi, "Geometric mean");
        } else {
            b = pow(b, 1 / a);
            strcpy(nimi, "Power mean");
        }
        if (a == 1.0) {
            strcpy(nimi, "Arithmetic mean");
        } else if (a == 2.0) {
            strcpy(nimi, "Quadratic mean");
        } else if (a == -1.0) {
            strcpy(nimi, "Harmonic mean");
        }
...
static int sum_sums(int i, double x, double weight) {
    int h;
    double y;
...
static int print_auto_corr(int i) {
    ...
    if (nobs[i] < d.l2 - d.l1 + 1) {
        return (1);
    }
    data_load(&d, d.l1, d.v[i], &x1);
    x1 -= x_first[i];
    data_load(&d, d.l2, d.v[i], &x2);
    x2 -= x_first[i];
    ...
    if (ss1 == 0.0 || ss2 == 0.0) {
        ...
        r = (sum_lag[i] - (ntotal - 1) * mean1 * mean2) / sqrt(ss1 * ss2);
    }
    if (r * r < 4.0 / ntotal) {
        return (1);
    }
    if (r > 0.9999999) {
        ...
        if (fabs(x1 + (d.l2 - d.l1) * (x - x1) - x2) < 1e-10) {
            k = muste_sprintf(line, "changing linearly from %s", spoisloppu(spois(res(x1, sana))));
            k += muste_sprintf(line + k, " to %s", spoisloppu(spois(res(x2, sana))));
            k += muste_sprintf(line + k, ", increment=%s", spois(res((x2 - x1) / (ntotal - 1), sana)));
            print_line(line);
...
static int scale_type(char m, char *s) {
    ...
    *s = EOS;
    ...
    case ' ':
        break;
    case '-':
        strcpy(s, "Not a statistical variable");
        break;
    case 'D':
        strcpy(s, "Dichotomous variable");
        break;
    case 'N':
        strcpy(s, "Nominal scale");
        break;
    case 'o':
    case 'O':
        strcat(s, "Ordinal scale");
        break;
    case 's':
    case 'S':
        strcat(s, "Score scale");
        break;
    case 'I':
    case 'i':
        strcat(s, "Interval scale");
        break;
    case 'R':
    case 'r':
        strcat(s, "Ratio scale");
        break;
    case 'F':
        strcpy(s, "Values are frequencies");
        break;
    default:
        muste_sprintf(s, "Scale type %c", m);
        break;
    }
    return (1);
}
...
static int printout() {
    ...
    for (i = 0; i < m; ++i) {
        ...
        if (is >= 0 && ((d.vartype[d.v[i]][3] == 'N' && n_str_class[is] > 1) ||
                        (nclass[i] == 1 && v_str[i] && n_str_class[is] > 1))) {
            strvar = 1;
        } else {
            strvar = 0;
        }
        ...
        /* 8.3.1994 */
        {
            ...
            k = muste_sprintf(line, "entropy=%s (%.1f%%)", spois(res(entropy / log(2.0), sana)),
        ...
        if (scale_ok(&d, d.v[i], ORDINAL_SCALE)) // RS 9.1.2014 REM ;
        {
            ...
            k = muste_sprintf(line, "min=%s in obs.#%ld", spois(res(min[i], sana)), min_obs[i]);
            if (type == 'S') {
                data_alpha_load(&d, min_obs[i], 0, sana);
                k += muste_sprintf(line + k, " (%.32s)", spoisloppu(sana));
            }
            print_line(line);
            k = muste_sprintf(line, "max=%s in obs.#%ld", spois(res(max[i], sana)), max_obs[i]);
            if (type == 'S') {
        ...
        if (w[i] != 0.0 && scale_ok(&d, d.v[i], SCORE_SCALE)) {
            ...
            skewness = ((sum3[i] - 3 * sum2[i] * mean + 3 * sum1[i] * mean * mean) / w[i] - mean * mean * mean) *
                       nobs[i] / (nobs[i] - 1) / (stddev * stddev * stddev);
            k += muste_sprintf(line + k, " skewness=%s", spois(res(skewness, sana)));
            stat_m_save(i, _SKEWNESS, "skewness", skewness);
            kurtosis =
                ((sum4[i] - 4 * sum3[i] * mean + 6 * sum2[i] * mean * mean - 4 * sum1[i] * mean * mean * mean) / w[i] +
                 mean * mean * mean * mean) *
                    nobs[i] / (nobs[i] - 1) / (stddev * stddev * stddev * stddev) -
                3;
            ...
            if (pr_sums) {
                print_sums(pr_sums, i); // 23.7.2001
            }
        ...
        if (cwidth[i] > 0.0) {
            ...
            h1 = 0;
            while (freq[i * maxc + h1] == 0L) {
                ++h1;
            }
            h2 = maxc;
            while (freq[i * maxc + h2 - 1] == 0L) {
                --h2;
            }
        }
        len = 0;
        ...
        for (h = h1; h < h2; ++h) {
            ...
            if (freq[i * maxc + h] > maxf) {
                maxf = freq[i * maxc + h];
            }
        }

        if ((double)maxf < 0.7 * nobs[i]) {
            print_fractiles(i);
        }
        ...
        if (cwidth[i] > 0.0) {
            strcpy(sana, "up.limit");
        } else {
            muste_fieldcopy(sana, d.varname[d.v[i]], 8);
            sana[8] = EOS;
        }
        ...
        if (step > 1L) {
            k += muste_sprintf(line + k, "     %c=%ld obs.  ", barchar, step);
        }
        if (cwidth[i] > 0.0) {
            k += muste_sprintf(line + k, "class width=%s", spois(res(cwidth[i], sana)));
        } else {
            if (!smooth(i)) {
        ...
        for (h = h1; h < h2; ++h) {
            ...
            lev = fr / step;
            muste_sprintf(line, "%*.*s %6ld %5.1f %.*s", lcname, lcname, sana, fr, (double)(100.0 * fr / nobs[i]), lev,
                          bar);
            if (fr && !lev) {
                strcat(line, ":");
            }
...
static int sum_means(int i, double x, double weight) {
    int k;
    double *sum;
    double a;
...
static int statistics() {
    ...
    for (l = d.l1; l <= d.l2; ++l) {
        ...
        for (i = 0; i < m; ++i) {
            ...
            classify(i, x);
            ++nobs[i];
            j = sum_means(i, x, weight);
            if (j < 0) {
                return (-1); /* 28.6.90 */
            }
            j = sum_sums(i, x, weight);
            if (j < 0) {
                return (-1);
            }
            if (x_first[i] == MISSING8) {
                x_first[i] = x;
            }
            x -= x_first[i];
            wx = weight * x;
