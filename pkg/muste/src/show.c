...
void muste_show(int argc, char *argv[]) {
    ...
    space_break = 0;
    jmax = 1000000L;
    if (hae_apu("max_show_lines", sbuf)) {
        jmax = atol(sbuf); // 22.1.2007
    }
...
static int text_show(long rivi1) {
    ...
    *sline = EOS;
    ...
    while (1) {
        ...
        case 'n':
            jseur += (long)ndisp;
            if (jseur + (long)(ndisp - 1) > jmax) {
                jseur = jmax - (long)ndisp + 1L;
            }
            if (jseur < 1L) {
                jseur = 1L;
            }
        ...
        case CODE_DOWN:
            ...
            *******************************************************/
            ...
            ++jseur;
            if (edit != 3 && edit != 2) {
                talleta_alku((long)j, muste_ftell(text));
            }
            break;
        ...
        case 'd':
            ...
            LOCATE(r3 + 2, 1);
            prompt(" Edit: ", y1, len);
            if (strlen(y1) < EDITLEV) {
                strncat(y1, space, EDITLEV - strlen(y1));
            }
            if (strlen(y2) < EDITLEV) {
                strncat(y2, space, EDITLEV - strlen(y2));
            }
            if (strcmp(y1, y2) != 0) {
                muste_fclose(text);
...
static int etsi_rivi(char *s, int j0) {
    ...
    if (edit == 3 && muste_strcmpi(s, "END") == 0) {
        return ((int)jjmax);
    }
...
static int number_of_edit84_lines() {
    ...
    for (i = 0; i < ted2; ++i) {
        lue_rivi(sbuf);
        if (strncmp(sbuf + 1, space, ted1 - 1) != 0) {
            k = i + 1;
        }
    }
    return (k);
...
static int disp_show(long jseur) {
    ...
    for (k = jseur; k < max; ++k) {
        ...
        if (i < c3 + 8) {
            write_string(space, c3 + 8 - i, '4', crivi, i + 1);
        }
        if (k < max - 1) {
            sur_print("\n");
        }
        ...
        if (edit != 2 && edit != 3 && rivit((long)j) < 0L) {
            talleta_alku((long)j, muste_ftell(text));
        }
    }
    CURSOR_ON;
...
static int etsi(long rivi) {
    ...
    if (rivi > j) {
        ...
        while (j < rivi && !feof(text) && i < MAXL) {
            ...
            ++i;
            if (merkki == '\n') {
                if (rivit((long)(j + 1L)) < 0L) {
                    talleta_alku((long)(j + 1L), muste_ftell(text));
                }
                ++j;
                i = 0;
                if (sur_kbhit()) {
...
static int lue_rivi(char *s) {
    ...
    *s = EOS; // 21.7.2006
    ...
    if (edit) {
        if (edit == 3) {
            if (empty32) {
                muste_fieldcopy(line, space, ted1);
                line[ted1] = EOS;
                line[0] = '*';
                return (1);
            }
...
static int tutki(char *s) {
    ...
    n = 0;
    while (*p) {
        if (((unsigned char)*p > 30 && (unsigned char)*p < 167) ||
            (unsigned char)*p == (unsigned char)TAB // RS CHA (int)
            || strchr(win_aeao, *p) != NULL) {
            ++n;
        }
        ++p;
    }
...
static int load(char *s) {
    ...
    if (disp_frame) {
        muste_fieldcopy(line, space, lev);
        line[lev] = EOS;
        line[0] = '*';
        edwrite(line, jj, 0);
        if (zs[jj] == 0) {
...
static int load_codes(char *codefile, char *code, int col) {
    ...
    for (i = 0; i < 256; ++i) {
        code[i] = (char)getc(codes); // RS REM unsigned
    }
    muste_fclose(codes);
    return (1);
}
    ...
    if (edit == 3) {
        ...
        */
        ...
        while (1) {
            k = getc(text);
            if (k == EOF) {
                return (-1); // RS 15.1.2013 k<0 -> k==EOF
            }
            *s = (char)k;
            if (*s == CR) {
                break;
