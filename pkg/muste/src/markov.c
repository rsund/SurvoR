...
static int markov() {
    ...
    muste_fieldcopy(state, mstart, degree);
    state[degree] = EOS;
    ...
    while (1) {
        ...
        if (p >= mend) {
            p = mstart + (int)(tila / 5 * uniform_dev());
        }
        if (sur_kbhit()) {
            sur_getch();
            break;
...
static char *haku(char *state, char *p, int degree) {
    ...
    */
    ...
    if (q == NULL) {
        ...
        if (q == NULL || q + degree - 1 == prev) {
            return (haku(state + 1, p + 1, degree - 1));
        }
    }
    q += degree;
    if (q >= mend) {
...
static int simul_matrix(char *mat) {
    ...
    if (!degree) {
        ...
        muste_sprintf(sbuf, "\n# of columns is %d.", cdim);
        sur_print(sbuf);
        muste_sprintf(sbuf, "\n# of rows must be the same %d", cdim);
        sur_print(sbuf);
    ...
    for (i = 0; i < rdim; ++i) {
        ...
        if (fabs(a - 1.0) > 1e-5) {
            muste_sprintf(sbuf, "\nMatrix %s: row sum %d not =1!", matname, i + i);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
    ...
    if (degree > 1) {
        for (k = 0; k < degree; ++k) {
            start_states[k] = 0; // 24.4.2002
        }
    }

    i = spfind("START");
    if (i >= 0) {
        start_state = atoi(spb[i]) - 1;
    }
...
static int study() {
    ...
    i = spfind("POWER");
    if (i < 0) { /* Transitive closure, Reingold & al. p.338- */
        for (i = 0; i < m2; ++i) {
            if (aa[i] == 0.0) {
                ip4[i] = 0;
            } else {
...
static int study_results(int k, int svd) {
    ...
    muste_sprintf(sbuf, "Structure of Markov chain %s of %d states:", word[2], m);
    print_line(sbuf);
    if (ip2[1] == m) {
    ...
    for (i = 1; i <= k; ++i) {
        ...
        for (e = 0; e < m; ++e) {
            if (ip1[e] == i) {
                *x = EOS;
                strncat(x, rlab + lr * e, lr);
                j += muste_sprintf(sbuf + j, " %s", lyh(x));
                if (j > c2 - 8) {
                    muste_sprintf(sbuf + j, "...");
                    break;
                }
            }
        }
    ...
    if (ip2[0] == 0) {
        print_line("No transient states");
    } else {
        if (ip2[0] == 1) {
    ...
    muste_sprintf(expr, "Class_structure_of_%s_(Transient_states=0)", word[2]);
    if (svd) {
        i = matrix_save("MCLASS.M", vv, rdim, 2, rlab, "Class   Prob    ", lr, 8, -1, expr, 0, 0);
    } else {
...
static int p_label(char *tila, int m, char *lab, int k) {
    ...
    for (i = strlen(x); i < lr; ++i) {
        strcat(x, " ");
    }
    for (i = 0; i < m; ++i) {
        if (strncmp(x, lab + i * k, k) == 0) {
            return (i);
        }
    }
...
static int markov_teach() {
    ...
    ***********************/
    ...
    if (i >= 0) {
        strcpy(sana, spb[i]);
        split(sana, jj, 2);
        jitter_seed = atoi(jj[0]);
        muste_srand(jitter_seed);
        jitter = atoi(jj[1]);
    }
    ...
    if (i >= 0) {
        ...
        for (i = 0; i < suggest; ++i) {
            sug_value[i] = m_coeff * atof(sug[i]);
        }
    }
    ...
    *edellinen_sana = EOS;
    ...
    while (1) {
        ...
        if (show) {
            ...
            if (tn < 1e-6) {
                muste_sprintf(xx, "%e", tn);
            } else {
                muste_sprintf(xx, "%g", tn);
            }
        ...
        if (stop_when_found && strcmp(kohdesana, sana) == 0) {
            break;
        }
        if (show) {
            if (*vast == '.') {
    ...
    if (!show) {
        ++rivi;
        if (nn > nnmax) {
            muste_sprintf(sbuf, "Trials %d No success!. Probability=%e", nn, tn);
        } else {
            muste_sprintf(sbuf, "Success in trial %d. Probability=%e", nn, tn);
        }
        edwrite(space, rivi, 1);
        edwrite(sbuf, rivi, 1);
    }
...
static int arvonta(char *sana) {
    ...
    while (1) {
        ...
        *sana = state[i];
        ...
        if (strcmp(sana, edellinen_sana) != 0 || strcmp(sana, kohdesana) == 0) {
            break;
        }
    }
...
static int mprint2(double *aa, int m, int n) {
    ...
    j = 6;
    for (i = 0; i < m; ++i) {
        j += muste_sprintf(sbuf + j, "%c       ", state[i]);
    }
    sur_print(sbuf);
    for (i = 0; i < m; ++i) {
        sum = 0.0;
        ...
        for (j = 0; j < n; ++j) {
            sum += aa[i + m * j];
            muste_sprintf(sbuf, "%7.5f ", aa[i + m * j]);
            sur_print(sbuf);
        }
        muste_sprintf(sbuf, "*%g  |", sum);
        sur_print(sbuf);
    }
    sur_locate(row, col);
    return (1);
