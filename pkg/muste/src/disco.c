...
static int checkmap(char *initem, int block) {
    ...
    muste_fieldcopy(item, initem, 255);
    item[255] = EOS;
    i = strlen(item);
...
static int searchmap(char *item, int block) {
    ...
    tokapit = strlen(item);
    while (item[tokapit - 1] == ' ' && tokapit > 0) {
        tokapit--;
    }
                    ...
                    if (level == 1) {
                        ...
                        */
                        ...
                        while (k < level && ac == 0) {
                            ...
                            if (apu != candpointers[level - 1][j * (level + 1) + k]) {
                                ac = 1;
                            }
                        ...
                        if (ac == 0) {
                            ...
                            candpointers[level][cc * (level + 2) + k + 1] =
                                candpointers[level - 1][j * (level + 1) + k];
                            /*
...
static int freqitemsets() {
    ...
    while (l > 0 && i < maxvars) {
        ...
        muste_sprintf(sbuf, "\nLevel %i canditems=%i ", i + 1, k);
        sur_print(sbuf);

        if (k > 0) {

            loopcount = 10;
            if (((double)k * (double)obscount / 10000) > 5000) {
                looposa = (int)obscount / 10 + 1;
                looposacount = looposa;
            } else {
                looposacount = obscount + 1;
            }
            ...
            for (j = 0; j <= obscount; j++) {
                if (j > looposacount) {
                    ...
                    loopcount--;
                    muste_sprintf(sbuf, "%i ", loopcount);
                    sur_print(sbuf);
                }
                testcands(i, j, k);
            }
        ...
        l = 0;
        for (j = 0; j < k; j++) {
            if (candpointers[i][j * (i + 2)] >= minfreq) {
                l++;
            }
        }
        freqitems[i] = l;
        i++;
        muste_sprintf(sbuf, "freqitems=%i", l);
        sur_print(sbuf);
    }
    return (1);
}
...
static int opendatafile() {
    ...
    for (i = 0; i < d.m_act; ++i) {
        ...
        if (d.vartype[d.v[i]][0] != 'S') {
            muste_sprintf(sbuf, "\nOnly string variables supported! (%s)", d.varname[d.v[i]]);
            sur_print(sbuf);
            check = +1;
        }
...
static int writeitemfile() {
    ...
    if (i >= 0) {
        muste_sprintf(sbuf, "\nWriting item mapping to the file %s...", spb[i]);
        sur_print(sbuf);
...
static int readitemfile() {
    ...
    if (i < 0) {
        muste_sprintf(sbuf, "\nCan't find MAPFILE specification!");
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
    muste_sprintf(sbuf, "\nReading item mapping from the file %s...", spb[i]);
    sur_print(sbuf);
...
int muste_disco(int argc, char *argv[]) {
    ...
    */
    ...
    if (met < 0) {
        ...
        method = 1;
        if (strcmp(spb[met], "FIND") == 0) {
            method = 1;
        }
        if (strcmp(spb[met], "EXTRACT") == 0) {
            method = 2;
        }
        if (strcmp(spb[met], "MAPFIND") == 0) {
            method = 3;
        }
        if (strcmp(spb[met], "REMAP") == 0) {
            method = 4;
        }
        if (strcmp(spb[met], "TRANSREMAP") == 0) {
            method = 4;
            transremap = 1;
        } // RS 17.1.2013
    }
    ...
    if (blocko < 0) {
        blocklevels = 1;
        sur_print("\nPlease use BLOCKORDER specification");
        WAIT; // RS FIXME Add standard behaviour
        return (-1);
    } else {
        blocklevels = strlen(spb[blocko]);
    }
    ...
    if (method == 4) {
        ...
        if (i < 0) {
            muste_sprintf(sbuf, "\nCan't find REMAPOUTFILE specification!");
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
        ...
        muste_sprintf(sbuf, "\nRemapping data to the file %s...", spb[i]);
        sur_print(sbuf);
        ...
        {
            if (blocklevels > 1) {
                sur_print("\nOnly one block allowed in TRANSREMAP!");
                WAIT;
                return (-1);
            }
            ...
            for (i = 0; i < items; i++) {
                ...
                if (loytyi == 0) {
                    remapc[remapcn] = &remap[i * sizeofcuritem];
                    remapcn++;
                }
            }
            ...
            for (i = 0; i < blocklevels; i++) {
                ...
                for (j = 0; j < ero; j++) {
                    ...
                    if (ch == ' ') {
                        ...
                        // muste_sprintf(sbuf,"BACK:%s - %s",palsti,kamamap[cumblockcount[i]+j+1]); sur_print(sbuf);
                        // WAIT;
                        //      muste_sprintf(sbuf,"%i ",kama[obscount*sizeofkama+j+1]  ); sur_print(sbuf);
                    }
                }
    ...
    if ((method == 1) || (method == 3)) {
        ...
        **********************************************************************************************/
        ...
        muste_sprintf(sbuf, "freqitems=%i", j);
        sur_print(sbuf);
        ...
        if (i >= 0) {
            muste_sprintf(sbuf, "\nWriting results to the file %s...", spb[i]);
            sur_print(sbuf);
            fp = muste_fopen(spb[i], "w");
            if (fp == NULL) {
                sur_print("\nResultfile error!");
            ...
            while (freqitems[i] > 0) {
                ...
                for (j = 0; j < canditems[i]; j++) {
                    ...
                    if (apu >= minfreq) {
                        ...
                        for (k = 0; k < i + 1; k++) {
                            fprintf(fp, "%s ",
                                    &map[((candpointers[i][j * (i + 2) + k + 1] - 1) * sizeofitem + 3) * sizeof(int)]);
                        }
                        fprintf(fp, "\n");
                    }
                }
