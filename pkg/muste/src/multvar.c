...
void muste_multvar(char *argv) {
    ...
    i = spfind("METHOD");
    if (i >= 0) {
        if (strncmp(spb[i], "EXHAUSTIVE", 3) == 0) {
            method = 2;
        } else if (strncmp(spb[i], "STEPWISE", 3) == 0) {
            method = 1;
        } else {
            method = atoi(spb[i]);
        }
...
static int max_var() /* Stepwise_computation_of_Mvar() in pseudocode */
{
    ...
    muste_sprintf(sbuf, "Mvar[%s]=%s (Total variability in a %d*%d matrix)", word[1], sppois(text), m, m);
    print_line(sbuf);
...
static int sum2_Cholesky(double *rr, int m) /* stepwise maximizing sum of residual variances */
{
    ...
    for (k = 1; k < m; ++k) {
        ...
        if (k == 1) {
            var1[p[0]] = p[1];
            tr2[p[0]] = tr;
            if (p[1] < p[0] && var1[p[1]] == p[0] && fabs(tr - tr2[p[1]]) < 1e-7) {
                return (-p[1] - 1);
            }
        }
...
static int max_varp(double *rr, int m) {
    ...
    muste_sprintf(sbuf, "Mvar[%s]=%s minM[%s]=%s dim=%d", word[1], sppois(text), word[1], sppois(text2), m);
    print_line(sbuf);
    print_line("MAT LOAD COVVAR.M,END+2 / Optimally permuted covariance matrix");
    return (1);
}
...
static int cholesky5(double *T, double *X, int m, double eps, int *p) {
    ...
    for (i = 0; i < m; ++i) {
        ...
        for (j = 0; j < i; ++j) {
            if (0.999999 * fabs(T[p[i] + m * p[j]]) > T[p[j] * (m + 1)]) {
                return (-1);
            }
        }
    }
