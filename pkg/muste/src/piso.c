...
static void writefile() {
    ...
    i = spfind("OUTFILE");
    if (i >= 0) {
        muste_sprintf(sbuf, "\nWriting sort-order to the file %s...", spb[i]);
        sur_print(sbuf);
...
static int comparerivi(int rivi1, int rivi2) {
    ...
    kust = 0;
    for (i = 0; i < XKOKO; i++) {
        if (dama[rivi1 * XKOKO + i] != dama[rivi2 * XKOKO + i]) {
            kust++;
        }
...
static void sortdatamin() {
    ...
    for (k = 1; k < maara; k++) {
        ...
        order[apusolmuprev].next = apusolmunext;
        order[apusolmunext].prev = apusolmuprev;
        if (minsolmu1 == orderalku) {
            orderalku = apusolmunext;
        }
        ...
        muste_sprintf(sbuf, "%d:%d ", k, minkust);
        sur_print(sbuf);
    }
}
