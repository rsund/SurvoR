...
char *aname[MAXA];
int na;                  /* model parameters */
static double s_a[MAXA]; // std_errors of a
static double cc[MAXC];
char *cname[MAXC];
int nc; /* model constants */
...
static int hooke(void);
static double rss(double a[]);
static int rss0(void);
static double wsum(void);
...
static double Rsquare(void);
static int save_update1(int k);
static int davidon(int nd);
static char *spois(char *s);
static int dfp(double x[], int m, double (*f)(double x[]), int (*grad)(double fs[], double x[]), double step, int maxnf,
               int *stop);
static int newton(int linind, int type);
static int corr_save(double *R, double *a, double *s, int m, char *names[]);
static int newt(double x[], int m, double (*f)(double x[]), int (*grad)(double fs[], double x[]), double step,
                int maxnf, int linind, int *stop, int type);
static int modified_inv_hess(double *HH, int m);
static int residuals(char *data);
static int op_der(void);
static int grid_search(void);
...
extern int matrix_save0(char *matr, double *A, int m, int n, char *rlab, char *clab, int mrl, int mcl, int type,
                        char *expr, int nrem, int remrivi, char *ptext);
static void estimate_varinit(void);
/*******************
void main(argc,argv)
int argc; char *argv[];
***********************/
void muste_estimate(char *argv) {
    ...
    *p = EOS;
    ...
    if (muste_strcmpi(mod1, "LOGDENSITY") == 0) {
        logd = 1;
    }
    ...
    if (est_disp_all) {
        for (i = 0; i < em; ++i) {
            muste_sprintf(sbuf2, "%s  ", sdata.varname[i]);
            sur_print(sbuf2);
        }
        sur_print("\n");
    }
    ...
    if (est_disp_all) {
        for (i = 0; i < em2; ++i) {
            muste_sprintf(sbuf2, "%d  ", sdata.v[i]);
            sur_print(sbuf2);
        }
        sur_print("\n");
    }
    ...
    ++obs1;
    obs2 = en - obs2; /* nyt ens. ja viim. havainto */
    if (est_disp_all) {
        muste_sprintf(sbuf2, "obs1=%d obs2=%d\n", obs1, obs2);
        sur_print(sbuf2);
        if (!logd) {
            muste_sprintf(sbuf2, "mmod1=%s\n", mmod1);
            sur_print(sbuf2);
        }
    ...
    if (!logd) {
        i = fcmuunna(cmod1, mmod1);
        if (i < 0) {
            return;
        }
    }
    i = fcmuunna(cmod2, mmod2);
    if (i < 0) {
        return;
    }
    i = fcmuunna(cmodw, mmodw);
    if (i < 0) {
        return;
    }
    if (est_disp_all) {
        if (!logd) {
            muste_sprintf(sbuf2, "cmod1=%s\n", cmod1);
            sur_print(sbuf2);
        }
        ...
        for (k = 0; k < saveind; ++k) {
            *sbuf2 = EOS;
            ...
            if (*sbuf2 == 'x') {
                ...
                i = spfind(sbuf2);
                if (i < 0) {
                    muste_sprintf(sbuf2, "\nExpression {%s}=... for UPDATE variable %s not found!", s[k], s[k]);
                    sur_print(sbuf2);
                    WAIT;
                    return;
                }
...
static double sur_st_norm(double x, double y) { return (0.0); }
static double sur_lg_gamma(double x) { return (0.0); }
...
/* nd=1: Vain 1. derivaatat   nd=2: My?s 2. derivaatat */
{
    ...
    for (i = 0; i < na; ++i) {
        strcpy(name, "a");
        strcat(name, muste_itoa(i, index, 10));
        if (est_disp_all) {
    ...
    for (i = 0; i < na; ++i) {
        ...
        for (j = 0; j <= i; ++j) {
            strcpy(name, "a");
            strcat(name, muste_itoa(j, index, 10));
    ...
    if (est_disp_all) {
        for (i = 0; i < lause; ++i) {
            muste_sprintf(sbuf2, "(%c %d %d) ", laji[i], ind[i], lag[i]);
            sur_print(sbuf2);
        }
...
static int varnro(char sana[]) {
    int i;
    for (i = 0; i < em; ++i) {
        if (strcmp(sana, sdata.varname[i]) == 0) {
            return (i);
        }
    }
    return (-1);
    ...
    *sana = EOS;
    for (h = 0; *(s + h) && kesken; ++h) {
        ...
        case '[':
            ...
            if (lag) { /* merk. [+3]=[p3] [-3]=[m3] (+- sotkisivat deriv) */
                jatka(d, '[');
                if (lag < 0) {
                    jatka(d, 'm');
                    lag = -lag;
                ...
                strcat(d, muste_itoa(lag, sana, 10));
                jatka(d, ']');
            }
            *sana = EOS;
            break;
...
static int tutki2(char sana[], char d[]) {
    ...
    for (i = 0; i < em2; ++i) {
        if (strcmp(sana, sdata.varname[sdata.v[i]]) == 0) {
            jatka(d, 'x');
            strcat(d, muste_itoa(i, index, 10));
            *sana = EOS;
            return (1);
        }
    }
...
static int funktio(char sana[]) {
    if (muste_strcmpi(sana, "sqrt") == 0) {
        return (1);
    }
    if (muste_strcmpi(sana, "sqr") == 0) {
        return (1);
    }
    if (muste_strcmpi(sana, "log") == 0) {
        return (2);
    }
    if (muste_strcmpi(sana, "ln") == 0) {
        return (2);
    }
    if (muste_strcmpi(sana, "abs") == 0) {
        return (3);
    }
    if (muste_strcmpi(sana, "int") == 0) {
        return (4);
    }
    if (muste_strcmpi(sana, "exp") == 0) {
        return (5);
    }
    if (muste_strcmpi(sana, "sin") == 0) {
        return (6);
    }
    if (muste_strcmpi(sana, "cos") == 0) {
        return (7);
    }
    if (muste_strcmpi(sana, "tan") == 0) {
        return (8);
    }
    if (muste_strcmpi(sana, "atan") == 0) {
        return (9);
    }
    if (muste_strcmpi(sana, "arctan") == 0) {
        return (9);
    }
    if (muste_strcmpi(sana, "sinh") == 0) {
        return (10);
    }
    if (muste_strcmpi(sana, "cosh") == 0) {
        return (11);
    }
    if (muste_strcmpi(sana, "tanh") == 0) {
        return (12);
    }
    if (muste_strcmpi(sana, "asin") == 0) {
        return (13);
    }
    if (muste_strcmpi(sana, "arcsin") == 0) {
        return (13);
    }
    if (muste_strcmpi(sana, "acos") == 0) {
        return (14);
    }
    if (muste_strcmpi(sana, "arccos") == 0) {
        return (14);
    }
    if (muste_strcmpi(sana, "lgamma") == 0) {
        return (15);
    }
    if (muste_strcmpi(sana, "round") == 0) {
        return (16); /* 8.9.1998 */
    }
    if (strcmp(sana, "PHI") == 0 || strcmp(sana, "Phi") == 0) {
        return (17); /* 19.2.2002 */
    }
    if (strcmp(sana, "phi") == 0) {
        return (18); /* 20.2.2002 */
    }

    if (strcmp(sana, "OVERFLOW") == 0) {
        sur_print("\nNot enough space for (second) derivatives!");
        WAIT;
        s_end(argv1);
        return (-1);
    }
    muste_sprintf(sbuf2, "\nUnknown function '%s'", sana);
    sur_print(sbuf2);
    WAIT;
    s_end(argv1);
    return (-1);
...
static int datain() {
    ...
    for (l = sdata.l1; l <= sdata.l2; ++l) {
        if (unsuitable(&sdata, l)) {
            continue;
        }
        //       if (kbhit()) { getch(); prind=1-prind; }
        if (prind) {
            muste_sprintf(sbuf2, " %ld", l);
            sur_print(sbuf2);
        }
        ++en;
        j0 = j;
...
// char *sanoma;  /* virheilmoituksia varten */
{
    ...
    if (j >= N1) {
        strcpy(sanoma, "Expression is too nested!");
        return (0);
    }
    if (f3 > 0) {
        strcpy(sanoma, ") is missing!");
    ...
    if (i_laus < 0 || i_laus >= nlaus - 1) {
        strcpy(sanoma, "Laus index error!");
        return (0);
    } // RS ADD 16.10.2012
    laji[i_laus] = LOPPU;
    ++i_laus;
...
static int siirto(int k) {
    ...
    */
    if (i_laus < 0 || i_laus >= nlaus - 1) {
        ...
        WAIT;
        s_end(argv1);
        return (-1); // RS ADD return 16.10.2012
    }
    laji[i_laus] = laji2[k];
    ind[i_laus] = ind2[k];
    lag[i_laus] = lag2[k];
...

{
    ...
    p = 0;
    for (i = alku; (laji[i] != LOPPU); ++i) { /* for (h=0; h<=p; ++h)
        ...
        case '^':
            ...
            --p;
            pino[p - 1] = power(pino[p - 1], pino[p]);
            break;
        case 'e':
            --p;
            pino[p - 1] = power(pino[p], pino[p - 1]);
            break;

        case FUNKTIO:
            switch (ind[i]) {
            case 1:
                pino[p - 1] = sqrt(pino[p - 1]);
                break;
            case 2:
                pino[p - 1] = log(pino[p - 1]);
                break;
            case 3: /* abs */
                pino[p - 1] = fabs(pino[p - 1]);
                break;
            case 4: /* int */
                pino[p - 1] = floor(pino[p - 1]);
                break;
            case 5:
                pino[p - 1] = exp(pino[p - 1]);
                break;
            case 6:
                pino[p - 1] = sin(pino[p - 1]);
                break;
            case 7:
                pino[p - 1] = cos(pino[p - 1]);
                break;
            case 8:
                pino[p - 1] = tan(pino[p - 1]);
                break;
            case 9:
                pino[p - 1] = atan(pino[p - 1]);
                break;
            case 10:
                pino[p - 1] = sinh(pino[p - 1]);
                break;
            case 11:
                pino[p - 1] = cosh(pino[p - 1]);
                break;
            case 12:
                pino[p - 1] = tanh(pino[p - 1]);
                break;
            case 13:
                pino[p - 1] = asin(pino[p - 1]);
                break;
            case 14:
                pino[p - 1] = acos(pino[p - 1]);
                break;
            case 15:
                pino[p - 1] = sur_lg_gamma(pino[p - 1]);
                break;
            case 16:
                pino[p - 1] = est_round(pino[p - 1]);
                break;
            case 17:
                pino[p - 1] = sur_st_norm(pino[p - 1], 0.0);
                break; // 19.2.2002 Phi
            case 18:   // 20.2.2002  phi
                       // 1/sqrt(2*3.141592653589793)=0.3989422804014327         ACCURACY=16
                pino[p - 1] = 0.3989422804014327 * exp(-0.5 * pino[p - 1] * pino[p - 1]);
                break;
...
static int hooke() {
    ...
    while (maxstep > minstep && !sur_kbhit()) {
        ...
        sur_print("\nparam: ");
        for (i = 0; i < na; ++i) {
            muste_sprintf(sbuf2, "%10f ", a[i]);
            sur_print(sbuf2);
        }
        sur_print("\n");
        sur_print("steps: ");
        for (i = 0; i < na; ++i) {
            muste_sprintf(sbuf2, "%10f ", step[i]);
            sur_print(sbuf2);
        }
        sur_print("\n");
        ...
        if (criterion == 2) {
            for (j = obs1; j <= obs2; ++j) {
                ...
                if (weightind == 0) {
                    ...
                    sum2 += earvo(j, laus[2], a, cc, xx) * b * b;
                }
            }
            if (weightind == 0) {
                return (sum2);
            } else {
                return (sum2 * n / weightsum);
            }
        } else {
            for (j = obs1; j <= obs2; ++j) {
                b = earvo(j, laus[0], a, cc, xx) - earvo(j, laus[1], a, cc, xx);
                if (weightind == 0) {
                    sum2 += pow(fabs(b), criterion);
                } else {
                    sum2 += earvo(j, laus[2], a, cc, xx) * pow(fabs(b), criterion);
                }
            }
            if (weightind == 0) {
                return (sum2);
...
static int korvaa(char *s, char *x, char *m) {
    ...
    *s = EOS;
    ...
    while (*p != EOS) {
        ...
        if ((q == t) || (strchr("+-*/^(", *(q - 1)) != NULL)) {
            if (strncmp(q, x, len) == 0) {
                q += len;
                if ((*q == EOS) || (strchr("+-*/^)", *q) != NULL)) {
                    strncat(s, p, q - p - len);
                    strcat(s, m);
                    p = q;
                    continue;
...
static char *deriv(char *d, char *s) {
    ...
    if (strcmp(s, MUUTTUJA1) == 0) {
        est_strcpy(d, "1");
        return (d);
    }
    if (strcmp(s, MMUUTTUJA) == 0) {
        est_strcpy(d, "-1");
        return (d);
    }
    p = hajoita(s);
    if (p == -1) {
        est_strcpy(d, "*** ( missing! ***");
        return (d);
    }
    if (p == -2) {
        est_strcpy(d, "*** ) missing! ***");
    ...
    if (p != 0) {
        ...
        *(s1 + p) = EOS;
        ...
        case '^':
            ...
            if (kok(s2)) {
                muste_itoa(atoi(s2) - 1, d, 10);
                potenssi(d1, s1, d);
                tulo(d1, d1, deriv(d, s1));
                est_strcpy(d, s2);
                tulo(d, d, d1);
                return (d);
            }
    ...
    else /* funktiot */
    {
        ...
        *(s2 + strlen(s2) - 1) = EOS;
        ...
        if (!strcmp(s1, "sin")) {
            return (tulo(d, deriv(d, s2), est_strcat(est_strcpy(d1, "cos"), sul(s2, 4))));
        }
        if (!strcmp(s1, "cos")) {
            return (tulo(d, deriv(d, s2), est_strcat(est_strcpy(d1, "-sin"), sul(s2, 4))));
        }
        if (!strcmp(s1, "tan")) {
            est_strcpy(d1, "cos");
            est_strcat(d1, sul(s2, 4));
            potenssi(d1, d1, "-2");
        ...
        if (!strcmp(s1, "PHI") || !strcmp(s1, "Phi")) {
            ...
            //                return(tulo(d,deriv(d,s2),d1)); }
            return (tulo(d, deriv(d, s2), est_strcat(est_strcpy(d1, "phi"), sul(s2, 4))));
        }

        if (!strcmp(s1, "phi")) {
            // Rprintf("\ns=%s s1=%s s2=%s",s,s1,s2); getch();
            est_strcpy(d, "-");
            est_strcat(d, sul(s2, 2));
            tulo(d1, d, s);
            return (tulo(d, deriv(d, s2), d1));
        }
        /* tuntematon funktio */
...
static char *tulo(char *r, char *s1, char *s2) {
    ...
    if (!strcmp(s1, s2)) {
        est_strcpy(r, nelio(s1));
        return (r);
    }

    est_strcpy(r, sul(s1, 1));
    est_strcat(r, "*");
    est_strcat(r, sul(s2, 1));
    return (r);
}
...
static char *potenssi(char *r, char *s1, char *s2) {
    ...
    est_strcpy(r, sul(s1, 2));
    est_strcat(r, "^");
    est_strcat(r, sul(s2, 3));
    return (r);
}
...
static int dfp(double x[], int m, double (*f)(double x[]), int (*grad)(double fs[], double x[]), double step, int maxnf,
    ...
    while (kesken) {
        ...
        step1 = step2 = step;
        ++nit;
        muste_sprintf(sbuf2, "Iteration %d:\n", nit);
        sur_print(sbuf2);
        a1 = 0;
        for (i = 0; i < m; ++i) {
            x1[i] = x[i];
        ...
        while (haku) {
            ...
            a1 = sqrt(a * a - g2 * g3);
            l1 = step1 * (g3 + a1 - a) / (g3 - g2 + 2 * a1);
            muste_sprintf(sbuf2, "L=%f\n", l1);
            sur_print(sbuf2);
            for (i = 0; i < m; ++i) {
                x[i] -= l1 * s2[i];
            }
...
static int corr_save(double *R, double *a, double *s, int m, char *names[]) {
    ...
    for (i = 0; i < m; ++i) {
        ...
        for (k = 0; k < len; ++k) {
            sbuf2[8 * i + k] = names[i][k];
        }
    }
    matrix_save("EST_CORR.M", R, m, m, sbuf2, sbuf2, 8, 8, -1, "R_of_parameter_estimates", 0, 0);
    for (i = 0; i < m; ++i) {
        R[i] = a[i];
        R[m + i] = s[i];
    }
    matrix_save("EST_PAR.M", R, m, 2, sbuf2, "param   std.err.", 8, 8, -1, "Parameters_and_std.errors", 0, 0);
...
// int    type; /* 1=pure 2=modified Newton */
{
    ...
    while (kesken) {
        ...
        sur_print(sbuf2);
        a1 = 0;
        for (i = 0; i < m; ++i) { /* x1[i]=x[i]; s3[i]=fs[i]; */
            a = 0;
            for (j = 0; j < m; ++j) {
                a -= HH[m * i + j] * fs[j];
            }
...
static int residuals(char *data) {
    ...
    if (resvar < 0 && fvar < 0 && gvar < 0 && saveind == 0) {
        return (-1);
    }
    ...
    if (resvar >= 0) {
        muste_sprintf(sbuf2, "\nresiduals as variable %.8s...", sdata.varname[resvar]);
        sur_print(sbuf2);
    }
    if (fvar >= 0) {
        muste_sprintf(sbuf2, "\npredicted values of model as variable %.8s...", sdata.varname[fvar]);
        sur_print(sbuf2);
    }
    if (gvar >= 0) {
        muste_sprintf(sbuf2, "\nlefthandside values of model as variable %.8s...", sdata.varname[gvar]);
        sur_print(sbuf2);
    }
    k = 0;
    for (j = sdata.l1; j <= sdata.l2; ++j) {
        if (unsuitable(&sdata, j)) {
            continue;
        }
...
static int op_der() {
    ...
    if (g == 2) {
        ...
        strcpy(wordi2, word[2]); // RS CHA strcpy(word[2],"x");
    }
    strcpy(s, word[1]);
    deri(der, s, wordi2); // RS CHA deri(der,s,word[2]);

    cline = r1 + r - 1;
    strcpy(x, "Derivative of ");
    strcat(x, word[1]);
    edwrite(space, cline + 1, 1);
    edwrite(x, cline + 1, 2);
    strcpy(x, "with respect to ");
    strcat(x, wordi2);
    strcat(x, " is"); // RS CHA word[2]
    edwrite(space, cline + 2, 1);
    edwrite(x, cline + 2, 2);
    line = cline + 2;
    p = 0;
    while (p < strlen(der)) {
        if (strlen(der + p) > c3 - 4) {
            muste_fieldcopy(x, der + p, c3 - 4);
            *(x + c3 - 4) = EOS;
            p += c3 - 4;
        } else {
            strcpy(x, der + p);
...
static int grid_search() {
    ...
    muste_sprintf(sbuf2, "\nGrid search: %d parameters, %ld combinations", na, nn);
    sur_print(sbuf2);
    ...
    while (1) {
        ...
        if (bb - (int)bb < bb0 - (int)bb0) {
            ...
            muste_sprintf(sbuf2, "  Ready on %s", asctime(endtime));
            sur_print(sbuf2);
            sur_locate(row, col);
        }
        bb0 = bb;
        ...
        if (prind && count == 0) {
            ...
            i = 0;
            for (j = 0; j < na; ++j) {
                i += muste_sprintf(sbuf2 + i, " %f", a_opt[j]);
            }
            sur_print(sbuf2);
...
static int save_matrices(char *aname[], int na, double *a, double *s, int n, double rss, double r2) {
    ...
    *text = EOS;
    ...
    matrix_save0("EST.M", et, N_RG, 1, label, "ESTIMATE", 8, 8, -1, "EST.M", N_LINES, 0, text);
