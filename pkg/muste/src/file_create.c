        			...
        			if (varsrivit<=curvarsrivit) // RS 7.5.2013
        			    ...
        			    if (sbuf[0]=='*' && strncmp(sbuf+1,space,c2)==0 )
                            ...
                            if (varsch=='n' || varsch=='N')
                                ...
                                strcpy(x,"MASK=");
                                for (i=0; i<dat.m; ++i) { x[i+5]=dat.vartype[i][masknro]; 
}x[i+5]=EOS;
                                break;
                                }
        			        }
...
static int file_mask()
        ...
        if (*name=='(')
            ...
            if (*name=='#')
                ...
                {
                for (i=strlen(name); i<8; ++i) { name[i]=' '; 
}name[8]=EOS;
                for (i=0; i<dat.m; ++i)
                    {
        ...
        if (dat.m>m0 && dat.n>0L)
            ...
            for (obs=1L; obs<=dat.n; ++obs)
				...
				if (initval==NULL)
                	...
                	{ for (i=m0; i<dat.m; ++i) { fi_init_save(&dat,obs,i,initval); 
}} // RS 22.12.2012
                }
            }
