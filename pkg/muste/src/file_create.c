...
static int fmake() {
    ...
    if (g > 6) {
        ...
        if (strchr("1248S", stype) == NULL) {
            muste_sprintf(sbuf, "\nInvalid type %c of fields! (Must be 1,2,4,8, or S)", stype);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
    ...
    *base = EOS;
    ...
    for (i = 0; i < m; ++i) {
        ...
        ++i_base;
        if (m == 1) {
            strcpy(x, name_start); // 24.5.2001 pelkkÑ X esim.
        }
        for (j = strlen(x); j < l; ++j) {
            x[j] = ' ';
        }
        x[l] = EOS;
        strcpy(varname[i], x);
        if (*form_limits) {
            strcpy(varname[i] + 9, form_limits);
        }
    }

    fitextn = 1;
    fitextlen = c2;
    muste_sprintf(x, "File with %d fields and %ld cases created by FILE MAKE", m, n);
    privi[0] = x;
    fitext = privi;
    // Rprintf("1"); getch();
    i = fi_create(word[2], filen, m1, m, 0L, l, actsar + 5, fitextn, fitextlen, fitext, varname, varlen, vartype);
    // Rprintf("2");
    if (i < 0 && etu == 0) {
        muste_sprintf(sbuf, "\nFile %s not created!", word[2]);
        sur_print(sbuf);
        WAIT;
        return (1);
    }
    ...
    sur_print("\n");
    muste_sprintf(sbuf, "\nCreating data file %s with %d variables", word[2], m);
    sur_print(sbuf);
    ...
    if (m > 5) {
        ...
        muste_sprintf(sbuf, "\n%s, %s,..., %s", varn[0], varn[1], varn[2]);
        sur_print(sbuf);
    }
    muste_sprintf(sbuf, "\n%ld observations...", n);
    sur_print(sbuf);

    i = fi_open(word[2], &dat);
    if (i < 0) {
        return (-1);
    }
...
static void check_varname(char *s) {
    ...
    muste_fieldcopy(nimi, s, 8);
    nimi[8] = EOS;

    if (strcmp(nimi, "IND     ") == 0 || /* 14.12.1999 */
        strcmp(nimi, "CASES   ") == 0 || strcmp(nimi, "SELECT  ") == 0) {
        sur_print("\nWARNING!");
        sur_print("\nTo avoid confusions,");
...
static void number_error(int j, int k) {
    ...
    sur_print(sbuf);
    muste_sprintf(sbuf, "\nIncorrect # of field (%d expected)", k);
    sur_print(sbuf);
    WAIT;
}
...
static void field_error(int j) {
    ...
    muste_sprintf(sbuf, "\nIncomplete field definition on edit line %d", j);
    sur_print(sbuf);
    edread(x, j);
    len = strlen(x);
    while (x[len - 1] == ' ') {
...
static int check_varlen(int varlen, char type, int j) {
    ...
    if (varlen == 1 || varlen == 2 || varlen == 4 || varlen == 8) {
        return (1);
    }
    muste_sprintf(sbuf, "\nError on edit line %d:", j);
    sur_print(sbuf);
    muste_sprintf(sbuf, "\nIncorrect field length %d for a numeric field!", varlen);
    sur_print(sbuf);
...
static int fields_rivit() {
    ...
    while (j <= ed2) {
        ...
        if (i > 0 && strncmp("FIELDS", muste_strupr(sana[0]), 6) == 0) {
            break;
        }
        ++j;
    }
    ...
    while (j <= ed2) {
        edread(rivi, j);
        i = split(rivi + 1, sana, 1);
        if (i > 0 && strncmp("END", muste_strupr(sana[0]), 3) == 0) {
            break;
        }
        ++j;
    }
...
static int create() {
    ...
    for (i = 0; i < m; ++i) {
        ...
        muste_fieldcopy(vartype[i], space, actsar + 1);
        vartype[i][actsar + 1] = EOS;
        vartype[i][1] = 'A';
        k = 0;
        while (k < actsar + 1 && sana[1][k]) {
            vartype[i][k] = sana[1][k];
            ++k;
        }
        if (vartype[i][0] == 'N') {
            vartype[i][0] = *sana[2];
        ...
        fipituus += varlen[i];
        edread(x, j);
        muste_fieldcopy(varname[i], sana[3], l);
        varname[i][l] = EOS;
    ...
    if (fipituus > filen) {
        if (filen == 0) {
            filen = fipituus + fipituus / 4 + 20;
        } else {
            muste_sprintf(sbuf, "\nRecord length %d less than total length %d of fields already defined!", filen,
                          fipituus);
            sur_print(sbuf);
            WAIT;
            return (-1);
    ...
    if (m1 < m) {
        muste_sprintf(sbuf, "\nMax. # of fields %d less than # of fields %d already defined!", m1, m);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
    ...
    if (actsar > 20) {
        ...
        if (actsar > 64) {
            return (1); // RS CHA exit(1);  /* 25.2.1995 */
        }
    }
    /*
    ...
    */
    i = fi_create(word[2], filen, m1, m, n, l, actsar + 5, textn, c2, fitext, varname, varlen, vartype);
    if (i < 0 && etu == 0) {
        muste_sprintf(sbuf, "\nFile %s not created!", word[2]);
        sur_print(sbuf);
        WAIT;
...
static int conv(unsigned char *sana) {
    ...
    for (i = 0; i < strlen((const char *)sana); ++i) {
        sana[i] = (unsigned char)code[(unsigned char)sana[i]];
    }
    return (1);
}
...
static int convert() {
    ...
    i = load_codes(word[3], code);
    if (i < 0) {
        return (1);
    }
    i = data_open(word[2], &datc);
    if (i < 0) {
        return (1);
    }
    sur_print("\n");
...
static int maskload() {
    ...
    *maskstr = EOS;
    ...
    for (i = 0; i < d.m; ++i) {
        ch = d.d2.vartype[i][1];
        if (*maskstr && strchr(maskstr, ch) == NULL) {
            ch = '-';
        }
    ...
    *vars = EOS;
    i = 1;
    if (g > 4 && muste_strcmpi(word[4], "MASK") == 0) {
        i = 0; // 19.10.2001
    }
    if (*maskstr && i) {
...
static int act_disp() {
    PR_EBLD;
    LOCATE(2, 40);
    muste_sprintf(sbuf, "M(active)=%3d", m_act);
    sur_print(sbuf);
    return (1);
}
...
static int pro_disp() {
    PR_EBLD;
    LOCATE(2, 55);
    muste_sprintf(sbuf, "M(protected)=%3d", m_pro);
    sur_print(sbuf);
    return (1);
}
...
static int col_disp() {
    PR_EBLD;
    LOCATE(2, 30);
    muste_sprintf(sbuf, " Col=%2d", col + 1);
    sur_print(sbuf);
    return (1);
}
...
static int all_disp() {
    ...
    */
    ...
    LOCATE(2, 1);
    PR_EBLD;
    muste_sprintf(sbuf, "Data file %s  M=%3d", active_data, m);
    sur_print(sbuf);
    act_disp();
    pro_disp();
    col_disp();
...
static int nimen_nro(char *s) {
    ...
    for (i = 0; i < dat.m; ++i) {
        if (strncmp(dat.varname[i], t, 8) == 0) {
            return (i);
        }
    }
    return (-1);
...
static int fmask_write() // RS 8.5.2013
{
    ...
    curvarsrivit++;
    for (i = 0; i < dat.m; ++i) {
        if (dat.vartype[i][masknro] == '-' || dat.vartype[i][masknro] == '_' || dat.vartype[i][masknro] == ' ') {
            continue;
        }
...
static int mask_write() {
    ...
    varsr = r;
    if (masknro < 0) {
        masknro = 1; // RS 7.5.2013
    }
    if (tila == 3) {
        masknro = col + 1; // RS 7.5.2013
    }
    if (tila == 99) {
        masknro = col + 1;
        strcpy(x, "VARS=");
        for (i = 0; i < dat.m; ++i) {
            if (dat.vartype[i][masknro] == '-' || dat.vartype[i][masknro] == '_') {
                continue;
            }
            muste_fieldcopy(name, dat.varname[i], 8);
            for (j = 0; j <= 8; j++) {
    ...
    } else // RS 25.11.2012
    {
        ...
        for (i = 0; i < j; i++) {
            ...
            while (k < dat.m) {
                ...
                if (strncmp(dat.varname[k], parm[i], strlen(parm[i])) == 0) {
                    break;
                }
                k++;
            }
...
static int k_copy() {
    ...
    LOCATE(r3 + 2, 1);
    PR_EBLD;
    strcpy(kysymys, "Column ");
    muste_itoa(col + 1, kysymys + 7, 10);
...
static int k_help() {
    ...
    PR_EINV;
    muste_sprintf(sbuf, "\n%s (F8)", key_label[CODE_EXIT]);
    sur_print(sbuf);
    PR_ENRM;
    sur_print(" Return back to SURVO MM EDITOR");
    PR_EINV;
    muste_sprintf(sbuf, "\n%s", key_label[CODE_NEXT]);
    sur_print(sbuf);
    PR_ENRM;
    sur_print(" Next page");
    PR_EINV;
    muste_sprintf(sbuf, "\n%s", key_label[CODE_PREV]);
    sur_print(sbuf);
    PR_ENRM;
    sur_print(" Previous page");
    PR_EINV;
    muste_sprintf(sbuf, "\n%s", key_label[CODE_RETURN]);
    sur_print(sbuf);
    PR_ENRM;
    sur_print(" or ");
    PR_EINV;
    muste_sprintf(sbuf, "%s", key_label[CODE_DOWN]);
    sur_print(sbuf);
    PR_ENRM;
...
static int activate() {
    ...
    tila = 0;
    fmask = 0;
    if (strcmp(info, "KEY_ACTIV") == 0) {
        tila = 1;
    }
    if (strcmp(info, "FMASK") == 0) {
        tila = 1;
        fmask = 1;
    } // RS 8.5.2013
    if (strcmp(info, "MASK") == 0) {
        tila = 2;
    }
    if (strcmp(info, "VARS") == 0) {
        tila = 2; // RS 25.11.2012
    }
    if (tila) {

        *info = EOS;
        if (*active_data != EOS) {
            strcpy(tiedosto, active_data);
        } else {
            LOCATE(r3 + 2, 1);
            PR_EINV;
    ...
    i = fi_open3(tiedosto, &dat, 0, 1, 0, 0);
    if (i < 0) {
        return (-1);
    }
    vain_selailu = 1;
    ...
    if (tila == 2 && m > c2 - 6) {
        muste_sprintf(sbuf, "\n# of fields in file = %d. Edit field width = %d.", m, c2);
        sur_print(sbuf);
    ...
    */
    ...
    if (fmask) // RS 9.5.2013
    {
        ...
        sp_init(r1 + r - 1);
        data_read_open(tiedosto, &apudat);
        if (i < 0) {
            fi_close(&dat);
            return (-1);
        }
        mask(&apudat);
        for (i = 0; i < m; i++) {
            dat.vartype[i][1] = apudat.vartype[i][1];
        }
        data_close(&apudat);
        n_col = 1;
    }
...
static int fc_varfind2(char *s) // RS CHA fc_
{
    ...
    if (*s == '#') {
        ...
        if (var > dat.m - 1) {
            muste_sprintf(sbuf, "\nToo big number #%d (max %d)", var, dat.m);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
...
static int mask_talletus(int i) {
    fi_puts(&dat, dat.vartype[i], dat.extra - 4, (long)(dat.var + 4L + (long)i * ((long)dat.l + (long)dat.extra)));
    return (1);
}
...
static int mask_with_list() // 4.4.2005 (6.4.2005)
{
    ...
    if (g == 3) // RS 8.5.2013
    {
        ...
        while (1) {
            ...
            if (strcmp(s[0], "MASKING") == 0 && strcmp(s[1], mname) == 0) {
                break;
            }
        }
    }
    ...
    for (j = j1; j < j2; ++j) {
        ...
        if (strcmp(s[0], "OTHERS:") == 0) {
            ...
            for (h = 0; h < k - 1; ++h) {
                ...
                if (p != NULL) {
                    *p = EOS;
                    var1 = fc_varfind2(s[h]);
                    if (var1 < 0) {
                        return (0); // RS CHA exit(0);
                    }
                    var2 = fc_varfind2(p + 1);
                    if (var2 < 0) {
                        return (0); // RS CHA exit(0);
                    }
                    if (var1 > var2) {
...
static int file_mask() {
    ...
    if (*name == '(') {
        ...
        if (*name == '#') {
            ...
            name[8] = EOS;
            for (i = 0; i < dat.m; ++i) {
                if (strncmp(name, dat.varname[i], 8) == 0) {
                    break;
                }
            }
            if (i == dat.m) {
...
static int update() {
    ...
    for (i = 0; i < textn; ++i) {
        edread(x, r1 + r + i);
        for (k = strlen(x + 1); k < dat.textlen; ++k) {
            x[k + 1] = ' ';
        }
    ...
    for (i = 0; i < n_fields; ++i) {
        ...
        if (i1 <= edell_i) {
            ...
            fi_reduce_m(&dat, m0);
            fi_close(&dat);
            return (-1); // RS 26.3.2013 fi_reduce+close
        }
        if (i1 > dat.m && i1 != dat.m + 1) {
            muste_sprintf(sbuf, "\nIncorrect field number on edit line %d (%d expected)", j, dat.m + 1);
            sur_print(sbuf);
            WAIT;
            fi_reduce_m(&dat, m0);
            fi_close(&dat);
            return (-1); // RS 26.3.2013 fi_close
        }
        if (i1 > dat.m1) {
            sur_print("\nNot enough space for new fields in current file!");
            muste_sprintf(sbuf, "\nCreate a new one with %d fields at least", i1 + n_fields + fields_rivi - j);
            sur_print(sbuf);
        ...
        if (!uusi && sana[1][0] != dat.vartype[i1 - 1][0]) {
            muste_sprintf(sbuf, "\nError on edit line %d:", j);
            sur_print(sbuf);
            muste_sprintf(sbuf, "\nChange of field type (from %c to %c) not allowed!", dat.vartype[i1 - 1][0],
                          sana[1][0]);
            sur_print(sbuf);
            WAIT;
            fi_reduce_m(&dat, m0);
        ...
        if (uusi) {
            muste_fieldcopy(vartype, space, actsar + 1);
            vartype[actsar + 1] = EOS;
        } else {
            strcpy(vartype, dat.vartype[i1 - 1]);
        }
        vartype[1] = 'A';
        k = 0;
        while (k < actsar + 1 && sana[1][k]) {
        ...
        if (!uusi && k != dat.varlen[i1 - 1]) {
            muste_sprintf(sbuf, "\nError on edit line %d:", j);
            sur_print(sbuf);
            muste_sprintf(sbuf, "\nChange of field length (from %d to %d) not allowed!", dat.varlen[i1 - 1], k);
            sur_print(sbuf);
            WAIT;
            fi_reduce_m(&dat, m0);
            fi_close(&dat);
    ...
    if (dat.m > m0 && dat.n > 0L) {
        ...
        if (i >= 0) {
            ...
            if (strlen(initval) == 0)
                initval = NULL;
            else if (strcmp(initval, "MISSING") == 0)
                initval = NULL;
        }
        ...
        for (obs = 1L; obs <= dat.n; ++obs) {
            ...
            if (initval == NULL) {
                ...
                for (i = m0; i < dat.m; ++i) {
                    fi_init_save(&dat, obs, i, initval);
                }
            } // RS 22.12.2012
        }
    }
    ...
    if (!etu) /* 31.3.91 */
    {
        ...
        for (i = 1; i < dat.m; ++i) {
            for (h = 0; h < i; ++h) {
                if (strncmp(dat.varname[h], dat.varname[i], 8) != 0) {
                    continue;
                }
                if (!k) {
                ...
                muste_sprintf(sbuf, "%.8s ", dat.varname[i]);
                sur_print(sbuf);
            }
        }
        if (k) {
...
static int getvar(int k) // 27.11.2001-  MASK,VARS otettu huomioon!
{
    ...
    if (i == d.m_act) {
        ...
        muste_sprintf(sbuf, "%c %d %s", d.vartype[d.v[i]][0], d.varlen[d.v[i]], d.varname[d.v[i]]);
    }
    j = r1 + r;
    edwrite(space, j, 1);
    edwrite(sbuf, j, 1);
...
static void status() {
    ...
    actsar = 3;
    if (g > 3) {
        actsar = atoi(word[3]) + 1;
    }
    if (actsar > d.d2.extra - 4) {
        actsar = d.d2.extra - 4;
    }
    ...
    for (i = 0; i < d.d2.textn; ++i) {
        k = tulosta(d.d2.fitext[i]);
        if (k < 0) {
            return;
        }
    }
    k = tulosta("FIELDS: (active)");
    if (k < 0) {
        return;
    }
    for (i = 0; i < m; ++i) {
        ...
        while ((p = strchr(type, ' ')) != NULL) {
            *p = '_';
        }
        muste_sprintf(x, "%4d %.*s %3d %s", i + 1, actsar, type, d.varlen[i], d.varname[i]);
        k = tulosta(x);
        if (k < 0) {
            return;
        }
    ...
    // RS CHA N=%ld -> N=%d
    muste_sprintf(x, "Survo data file %s: record=%d bytes, M1=%d L=%d  M=%d N=%d", active_data, d.d2.len, d.d2.m1,
                  d.d2.l, m, d.d2.n);
    tulosta(x);
    data_close(&d);
}
...
static int get_var_name() {
    ...
    *info = EOS;
    ...
    if (!sur_find_svo_file(tiedosto, polku)) {
        return (-1);
    }
    ...
    *info = EOS;
    ...
    for (i = 0; i < dat.m; ++i) {
        ...
        if (strncmp(s, dat.varname[i], len) == 0) {
            break;
        }
    }
    if (i == dat.m) {
