...
static int fit_geom() {
    ...
    df_geom -= 1.0;
    if (df_geom > 0) {
        dp_geom = 1.0 - muste_cdf_chi2(chi2_geom, (double)df_geom, 1e-15);
    }

    dp_geom0 = -1.0;
    --df00;
    if (df00 > 0) {
        dp_geom0 = 1.0 - muste_cdf_chi2(chi00, (double)df00, 1e-15);
    }
    dp_geom1 = -1.0;
    --df01;
    if (df01 > 0) {
        dp_geom1 = 1.0 - muste_cdf_chi2(chi01, (double)df01, 1e-15);
    }
...
static int disp0() {
    ...
    LOCATE(r + 3, 9);
    muste_sprintf(sbuf, "N=%d N0=%d N1=%d p=%g run0=%d run1=%d", n, n0, n1, (double)n1 / (double)n, run01, run11);
    (*g_print)(sbuf);
    ...
    LOCATE(r + 5, 9);
    muste_sprintf(sbuf, "Geometric distribution test: X2=%g df=%d P=%g", chi2_geom, df_geom, dp_geom);
    (*g_print)(sbuf);
    ...
    if (dp_geom0 < 0.0) {
        ...
        muste_sprintf(sbuf, "     for 0-runs separately: X20=%g df=%d P=%g", chi00, df00, dp_geom0);
    }
    (*g_print)(sbuf);
    ...
    if (dp_geom1 < 0.0) {
        ...
        muste_sprintf(sbuf, "     for 1-runs separately: X21=%g df=%d P=%g", chi01, df01, dp_geom1);
    }
    (*g_print)(sbuf);

    LOCATE(r + 8, 9);
    muste_sprintf(sbuf, "Chi_square test for triples: X2=%g df=3 P=%g", x2_pair, px2_pair);
    (*g_print)(sbuf);

    LOCATE(r + 9, 9);
    muste_sprintf(sbuf, "O'Brien-Dyck test: X2=%g df=%g P=%g", x21, df, dp);
    (*g_print)(sbuf);
...
static int runtest_disp() {
    ...
    muste_sprintf(sbuf, "%10d %.8f %.8f lower limit (O'Brien-Dyck test)", u, p1, lower);
    (*g_print)(sbuf);
    LOCATE(r + 11, 9);
