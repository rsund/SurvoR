...
void muste_mnsimul(char *argv) {
    ...
    **************************/
    ...
    if (msn && mX != mY) {
        muste_sprintf("\nUnequal # of variables (rows) in matrices %s and %s", word[1], word[2]);
    }
    m = mX;
    n = atol(word[4]);
...
static int create_file() {
    ...
    for (i = 0; i < m; ++i) {
        ...
        muste_fieldcopy(varname[i], rlabX + 8 * i, 8);
        varname[i][8] = EOS;
    }
    ...
    if (i >= 0) {
        ...
        i = split(x, s, 2);
        if (i > 0) {
            extra_bytes = atoi(s[0]);
        }
        if (i > 1) {
            extra_fields = atoi(s[1]);
        }
    }

    i = fi_create(word[3], filen + extra_bytes, m + extra_fields, m, 0L, namelength, 12, 0, 0, NULL, varname, varlen,
                  vartype);
...
static int etsi_tyypit(char *s) {
    ...
    for (i = 0; i < m; ++i) {
        strcpy(x, varname[i]);
        for (j = 0; j < dat2.m; ++j) {
            if (strncmp(x, dat2.varname[j], 8) == 0) {
                break;
            }
        }
        if (j == dat2.m) {
        ...
        varlen[i] = dat2.d2.varlen[j];
        filen += varlen[i];
        strcpy(x, dat2.d2.vartype[j]);
        j = strlen(x);
        if (j < typelen - 1) {
            strncat(x, space, typelen - 1 - j);
        } else {
            x[typelen] = EOS;
        }
...
static int sampling() {
    ...
    if (dat.type != 2) {
        muste_sprintf(sbuf, "\nData matrix/list with the same name %s already exists!", word[3]);
        sur_print(sbuf);
        sur_print("\nSelect another name!");
        WAIT;
        return (1);
