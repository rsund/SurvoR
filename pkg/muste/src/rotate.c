...
void muste_rotate(char *argv) {
    ...
    for (i = 0; i < m; ++i) {
        sh_ind[i] = 1; // all variables shown!
    }
    weights_on = 0;
    i = spfind("FSHOW");
    if (i < 0) {
    ...
    if (i >= 0) {
        ...
        if (i < 0) {
            muste_sprintf(sbuf, "\nMatrix FSHOW=%s or WEIGHTS=%s not found!", spb[i], spb[i]);
            sur_print(sbuf);
            WAIT;
            return;
        }
    ...
    if (i >= 0) {
        ...
        if (muste_strnicmp(sana[0], "VAR", 3) == 0) {
            ...
            *graph_options = EOS;
            ...
            h2_limit = 0.3;
            if (h > 1) {
                h2_limit = atof(sana[1]);
            }
            i = cos_rot(F, m, k, T, h2_limit);
            if (i < 0) {
                return;
            }
            method = 2;
...
static int print_results() {
    ...
    i = extend_fmatrix(F, m, k, rlab, tclab, &fh, &rlabh, &clabh);
    if (i < 0) {
        return (-1);
    }
    i = matrix_print(fh, m + 1, k + 1, rlabh, clabh, lr, tlc, m + 1, k + 1, NULL, NULL, acc, c3, results_line, eout, x);
    if (results_line) {
        results_line = i;
    }
    if (results <= 70) {
    ...
    i = matrix_print(T, k, k, trlab, tclab, tlr, tlc, k, k, NULL, NULL, acc, c3, results_line, eout, x);
    if (results_line) {
        results_line = i;
    }
    ...
    if (ortho) {
        ...
        i = matrix_print(T, k, k, rlab, clab, lr, lc, k, k, NULL, NULL, acc, c3, results_line, eout, x);
        if (results_line) {
            results_line = i;
        }
        print_line("The factor structure matrix SFACT.M is obtained by the commands:");
        print_line("MAT SFACT.M=AFACT.M*RFACT.M");
        muste_sprintf(x, "MAT LOAD SFACT.M,%s,CUR+1", acc);
        print_line(x);
    }
    print_line("");
    return (1);
...
static int ttype(double *T, int k1, int k2, char *name) {
    ...
    if (k1 != k || k2 != k) {
        muste_sprintf(sbuf, "\nInitial rotation matrix T=%s not a %d*%d matrix!", name, k, k);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int varimax(double *F, int m, int k, double *T, double eps) {
    ...
    sur_print("\nVarimax rotation:");
    muste_sprintf(sbuf, "  m=%d k=%d", m, k);
    sur_print(sbuf);
    while (sur_kbhit()) {
        sur_getch();
    }
...
static int rotate(double *F, int m, int k, int ix, int iy, double angle, double *T) {
    ...
    for (i = 0; i < m; ++i) {
        ax = F[i + m * ix];
        ay = F[i + m * iy];
        F[i + m * ix] = ax * co - ay * si;
        F[i + m * iy] = ax * si + ay * co;
    }
    for (i = 0; i < k; ++i) {
        ax = T[i + k * ix];
        ay = T[i + k * iy];
        T[i + k * ix] = ax * co - ay * si;
        T[i + k * iy] = ax * si + ay * co;
    }
    return (1);
}
...
static int cos_rot(double *F, int m, int k, double *T, double h2_limit) {
    ...
    if (oksum < k) {
        muste_sprintf(sbuf, "\nOnly %d (<%d) of variables exceeding communality level %g . Cannot continue!", oksum, k,
                      h2_limit);
        sur_print(sbuf);
        WAIT;
        return (-1);
    ...
    for (i = 0; i < 32; ++i) {
        sbuf[i] = ' '; /* possible trailing spaces */
    }
    for (i = 0; i < k; ++i) {
        for (h = 0; h < lr; ++h) {
            sbuf[h] = rlab[cos_imax[i] * lr + h];
        }
        for (h = 0; h < tlc; ++h) {
            tclab[i * tlc + h] = sbuf[h];
        }
    }
...
static int oblimin(double *F, int n, int m, double *T, double delta, int max_iter, double eps) {
    ...
    sur_print("\nOblimin rotation:");
    muste_sprintf(sbuf, "  n=%d m=%d", n, m);
    sur_print(sbuf);
    ...
    while (1) {
        for (p = 0; p < m; ++p) {
            for (q = 0; q < m; ++q) {
                ...
                gpq = gg - x[p] - x[q];
                for (i = 0; i < n; ++i) {
                    spq[i] = s[i] - F[i + p * n] * F[i + p * n] - F[i + q * n] * F[i + q * n];
                }
                ypq = 0.0;
                for (i = 0; i < n; ++i) {
                    ypq += F[i + p * n] * F[i + q * n];
                ...
                zpq = 0.0;
                for (i = 0; i < n; ++i) {
                    zpq += F[i + p * n] * F[i + p * n] * F[i + q * n] * F[i + q * n];
                }
                tt = 0.0;
                for (i = 0; i < n; ++i) {
                    tt += F[i + p * n] * F[i + p * n] * spq[i];
                }
                tt -= dn * u[p] * dpq;
                zz = 0.0;
                for (i = 0; i < n; ++i) {
                    zz += F[i + p * n] * F[i + q * n] * spq[i];
                }
                zz -= dn * ypq * dpq;
                pp = 0.0;
                for (i = 0; i < n; ++i) {
                    pp += F[i + p * n] * F[i + p * n] * F[i + p * n] * F[i + q * n];
                }
                pp -= dn * u[p] * ypq;
                rr = zpq - dn * u[p] * u[q];
...
static int save_factcorr(double *cc, int m, char *clab, int lc) {
    matrix_save("RFACT.M", cc, m, m, clab, clab, lc, lc, 0, "RFACT", 0, 0);
    return (1);
}
...
static double root3(double a1, double a2, double a3) {
    ...
    p = a2 - a1 * a1 / 3;
    q = a3 - a1 * a2 / 3 + 2.0 / 27.0 * a1 * a1 * a1;
...
static double ortho_ff(double *A, double *T, int m, int k) // linear right constant CLF
{
    ...
    if (weights_on) {
        for (i = 0; i < m; ++i) {
            aa = FS[i];
            for (j = 0; j < k; ++j) {
                s += aa * linear_right_constant(L[i + m * j]);
            }
        }
    } else {
        for (i = 0; i < m * k; ++i) {
            s += linear_right_constant(L[i]);
        }
    }
    return (s);
}
...
static double ff(double *A, double *T, int m, int k) // linear CLF
{
    ...
    if (weights_on) {
        for (i = 0; i < m; ++i) {
            aa = FS[i];
            for (j = 0; j < k; ++j) {
                s += aa * linear_clf(fabs(L[i + m * j]));
            }
        }
    } else {
        for (i = 0; i < m * k; ++i) {
            s += linear_clf(fabs(L[i]));
        }
    }
    return (s);
}
