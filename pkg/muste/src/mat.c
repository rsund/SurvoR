...
static char *nimet[] = {"TRANSFORM", "MULT",  "SAMPLES",     "INDVAR",    "MERGE",   "MINDIFF", "COLSORT",
                        "CRSORT",    "EIGEN", "CONVOLUTION", "MAXDET",    "U_TO_F",  "INTSCAL", "FRAC_TO_DEC",
                        "SAMPLE",    "SORT",  "MAGIC",       "JACKKNIFE", "#EIGFEW", "#EIGLAN", "AGGRE",
                        "PERMORD",   "FREQ",  "RCSORT",      "PRODDIAG",  "TAB",     "SMOOTH",  "#"};

// RS REM static char **specs=nimet;

static int laske(char *lauseke, double *y); // RS Declaration
static int matload(char *matr, double **A, int *rdim, int *cdim, char **rlab, char **clab, int *lr, int *lc, int *type,
                   char *expr);
...
static void err_mat_parser() {
    muste_sprintf(sbuf, "\nValue of mat_parser=%d (in SURVO.APU) too small!", mat_parser);
    sur_print(sbuf);
    WAIT;
}
...
static int dim_error() {
    ...
    if (etu == 2) {
        ...
        s_end(mat_argv1);
        l_virhe = 1;
        return (-1); // RS CHA exit(1) -> l_virhe=1; return(-1);
    }
...
static int sp_init_mat(int lin) {
    ...
    p = splist + speclist;
    spa = (char **)p;
    p += specmax * sizeof(char *);
    spb = (char **)p;
    p += specmax * sizeof(char *);
    spshad = (char **)p;
    p += specmax * sizeof(char *);
    arvo = (double *)p;
    p += specmax * sizeof(double);
    ...
    if (lin == 0) {
        spn = spread3_mat(info, 0);
        return (spn);
    }

    edread(x, lin);
    spn = spread3_mat(x, lin);
    if (spn < 0) {
        return (spn);
    }
...
static int varaa_tila(double **A,  /* matriisitila (alkuosoite) (malloc) */
                      int m,       /* rivien lkm */
                      int n,       /* sar. lkm   */
...
static int matprint(double *A,              /* tulostettava matriisi */
                    int m, int n,           /* dimensiot */
                    int m1, int m2,         /* tulostettavat rivit */
                    int n1, int n2,         /* tulostettavat sarakkeet */
                    char *rlab, char *clab, /* otsikkovektorit */
                    int mrl, int mcl,       /* otsikkonimien pituudet */
    ...
    output_open(eout);
    if (*muoto == '%') {
        sar = muste_sprintf(x, muoto, fabs(A[0])) + 2;
    } else {
        sar = strlen(muoto) + 1;
    }

    if (*muoto == '*') {
        sar = accuracy + 2; // 13.4.2005
    }
    ...
    output_line(x, eout, eol);
    if (eol) {
        ++eol;
    }

    for (i = m1 - 1; i < m2; ++i) {
        muste_fieldcopy(sana, rlab + i * mrl, mrl);
        sana[mrl] = EOS;
        ...
        for (j = n1 - 1; j < n2; ++j) {
            if (k + sar < c2) {
                if (*muoto == '*') {
                    fnconv2(A[i + m * j], accuracy + 2, sana);
                    h = 1;
                } else {
...
static double funktio(char *s, double x) {
    ...
    if (strcmp(S, "ATN") == 0 || strcmp(S, "ARCTAN") == 0) {
        return (atan(x));
    }
    if (strcmp(S, "ABS") == 0) {
        return (fabs(x));
    ...
    *****************************************/
    ...
    if (i < 0) {
        if (etu == 2) {
            ...
            l_virhe = 1;
            return (-1);
        } // RS CHA exit handling /* 21.4.1997 */
        mat_parser_zero();
        ...
        WAIT;
        l_virhe = 1;
        return (-1); // RS CHA exit handling exit(1);
    }
    if (spb[i] == NULL) {
        *y = arvo[i];
        return (1);
...
double muste_arit_atof(char *lauseke) { return (arit_atof(lauseke)); }

static int arit_atoi(char *lauseke) { return ((int)arit_atof(lauseke)); }

int muste_arit_atoi(char *lauseke) { return ((int)arit_atof(lauseke)); }

static int name(char *matfile, char *matr) {
    int i;

    if ((*matr >= '0' && *matr <= '9') || *matr == '-' || *matr == '+' || *matr == '.' ||
        (*matr >= 'a' && *matr <= 'z') || *matr == '(' || *matr == '.') {
        strcpy(matfile, "*");
        return (0);
    }
    ...
    *matfile = EOS;
    if (!muste_is_path(matr)) {
        strcpy(matfile, edisk); // RS 19.3.2013
    }
    strcat(matfile, matr);
    if (strlen(matr) < 3) {
        strcat(matfile, ".MAT"); // RS 19.3.2013
    } else if (strchr(matr + 2, '.') == NULL && strstr(matr, ".M") == NULL) {
        strcat(matfile, ".MAT"); // RS 1.6.2013 .M
    }
    for (i = 0; i < strlen(matfile); i++) {
        if (*(matfile + i) == '/') {
            *(matfile + i) = '\\'; // RS 19.3.2013
        }
    }
    return (1);
}
...
static int matload2(char *matr,  /* matriisin nimi */
    ...
    if (i == 0) /* skalaari */
    {
        ...
        *lr = *lc = 0;
        *type = 30;
        i = varaa_tila(A, 1, 1, NULL, NULL, 0, 0);
        if (i < 0) {
            return (-1);
        }
    ...
    */
    ...
    for (i = 0; i < ERC; ++i) {
        expr[i] = (char)getc(MAT);
    }
    expr[ERC - 1] = EOS;
    p = strchr(expr, ' ');
    if (p != NULL && p - expr < ERC) {
        *p = EOS;
    }
    if (transp) {
        strcpy(x, expr);
    ...
    i = varaa_tila(A, m, n, rlab, clab, mrl, mcl);
    if (i < 0) {
        return (-1);
    }
    a = *A;
    ...
    ****************************************************/
    ...
    if (transp) {
        for (i = 0; i < m * mrl; ++i) {
            (*rlab)[i] = (char)getc(MAT);
        }
    } else {
        for (i = 0; i < n * mcl; ++i) {
            (*clab)[i] = (char)getc(MAT);
        }
    }
    pl = *rlab;
    if (transp) {
...
static int matload(char *matr,  /* matriisin nimi */
                   double **A,  /* matriisitila (alkuosoite) (malloc) */
                   int *rdim,   /* rivien lkm */
                   int *cdim,   /* sar. lkm   */
                   char **rlab, /* rivien otsikot (malloc) */
...
int muste_polload(char *matr,  /* matriisin nimi */
                  double **A,  /* matriisitila (alkuosoite) (malloc) */
                  int *rdim,   /* rivien lkm */
                  int *cdim,   /* sar. lkm   */
                  char **rlab, /* rivien otsikot (malloc) */
...
static int scalar_write(char *s) {
    ...
    if (sp_read == 0) {
        if (mtx) {
            spn = sp_init_mat(1); // SM 0 -> 1 9.5.2012
        } else {
            spn = sp_init_mat(r1 + r - 1);
        }
        if (spn < 0) {
            return (-1);
        }
...
static int osamat(                     /* return (ensimm.jatkoparametrin indeksi) */
                  char *word[], int j, /* word[j] ensimm. osamatriisinimen osa */
                  int ind[])           /* indeksirajoja varten */
{
    ...
    *p = EOS; /* matriisin nimi nyt word[j] */
    ...
    if (*p == EOS || *p == '*') {
        ...
        q = strchr(p, ':');
        if (q == NULL) {
            ind[0] = ind[1] = lab_atoi(p, rlabX, mX, lrX);
        } else {
            *q = EOS;
            ind[0] = lab_atoi(p, rlabX, mX, lrX);
    ...
    if (*p == ')' || *p == '*') {
        ...
        q = strchr(p, ':');
        if (q == NULL) {
            ind[2] = ind[3] = lab_atoi(p, clabX, nX, lcX);
        } else {
            *q = EOS;
            ind[2] = lab_atoi(p, clabX, nX, lcX);
...
static int etsi_muoto(char *muoto, int minlev, double *A, int m, int n) {
    ...
    if (max) {
        lev = lev2 = (int)(log(max + 1e-7) / log((double)10)) + 1;
    }
    /* +1e-7 7.3.94 */
    if (lev < minlev) {
        lev = minlev;
...
static int mat_save_rem(char *matr,    /* matriisin nimi */
    ...
    if (*matfile == '*') {
        ...
        i = spfind_mat(matr);
        if (i < 0) {
            scalar_write(rivi); /* ei tarvita: aina i>=0 */
        } else {
            spb[i] = NULL;
            arvo[i] = A[0];
        }
    ...
    muste_sprintf(x, "MATRIX84D %d %d %d %d %d %d", m, n, mname, mrl, mcl, type);
    for (i = strlen(x); i < ERC; ++i) {
        x[i] = ' ';
    }
    for (i = 0; i < ERC; ++i) {
        putc((char)x[i], MAT); // RS CHA (int) -> (char)
    }
    ...
    for (i = 0; i < m; ++i) {
        ...
        for (j = j1; j <= j2; ++j) {
            p = (char *)&A[i + m * j];
            for (h = 0; h < sizeof(double); ++h) {
                putc((char)(*(p + h)), MAT); // RS CHA 64-BIT sizeof(long)
            }
        }
    }
...
static int mat_save(char *matr, /* matriisin nimi */
                    double *A,  /* matriisitila */
                    int m,      /* rivien lkm */
                    int n,      /* sar. lkm   */
                    char *rlab, /* rivien otsikot */
    ...
    i = mat_save_rem(matr, A, m, n, rlab, clab, mrl, mcl, type, expr, nrem, remrivi, 0, NULL);
    return (i);
}
...
static int op_add() { return (op_add_sub(1)); }
static int op_sub() { return (op_add_sub(2)); }
...
static int op_mlt() {
    ...
    if (mX == 1 && nX == 1) {
        ...
        strcpy(tnimi, suluin(exprX, 1, expr1));
        strcat(tnimi, "*");
        strcat(tnimi, suluin(exprY, 1, expr1));
        nim(tnimi, exprX);
    ...
    if (typeX == 20) {
        mat_dmlt(T, X, Y, mY, nY);
    } else if (typeY == 20) {
        mat_mltd(T, X, Y, mX, nX);
    } else {
        mat_mlt(T, X, Y, mX, nX, nY);
    }
    strcpy(tnimi, suluin(exprX, 1, expr1));
    strcat(tnimi, "*");
    strcat(tnimi, suluin(exprY, 1, expr1));
...
static int op_transp() {
    ...
    strcpy(tnimi, suluin(exprX, 2, expr1));
    strcat(tnimi, "'");
    nim(tnimi, exprX);
...
static int power_result() {
    ...
    strcpy(tnimi, suluin(exprX, 3, expr1));
    strcat(tnimi, "^");
    strcat(tnimi, exprY);
    nim(tnimi, exprX);
...
static int op_pow() {
    ...
    if ((double)(int)Y[0] != Y[0] || Y[0] < 0.0 || mX == 1 || nX == 1) // 6.8.2006
    {
...
static int op_dim() {
    ...
    *rtext = EOS;
    ...
    muste_sprintf(tnimi, "%s DIM %s /* row%s=%d col%s=%d%s", word[0], word[2], word[2], mX, word[2], nX, rtext);
...
static int op_zer() { return (op_con(0.0)); }
...
static int op_dv() {
    ...
    i = varaa_tila(&T, n, n, NULL, NULL, 0, 0);
    if (i < 0) {
        return (-1);
    }
    for (i = 0; i < n; ++i) {
        ...
        dbl = X[i];
        T[i * (n + 1)] = dbl; /* compiler-virheen takia */
    }
    strcpy(tnimi, "DV(");
    strcat(tnimi, exprX);
    strcat(tnimi, ")");
...
static int op_diag() /* 3.4.1999 */
{
    ...
    i = varaa_tila(&T, k, 1, NULL, NULL, 0, 0);
    if (i < 0) {
        return (-1);
    }
    for (i = 0; i < k; ++i) {
...
static int op_load() {
    ...
    *muoto = EOS;
    ...
    {
        output_line(exprX, eout, eol);
        if (eol) {
            ++eol;
        }
    }
    output_close(eout);
    matprint(X, mX, nX, ind[0], ind[1], ind[2], ind[3], rlabX, clabX, lrX, lcX, muoto, "///", eol, n_row_comments);
    return (1);
}
                                ...
                                /* word[1]=i1:i2 word[2]=j1:j2 matr=A */
{
    ...
    if (*word[1] == EOS || *word[1] == '*') {
        i1 = 1;
        i2 = mX;
    } else {
        i1 = i2 = lab_atoi(word[1], rlabX, mX, lrX);
        if (p != NULL) {
            i2 = lab_atoi(p + 1, rlabX, mX, lrX);
        }
    }
    i = strlen(word[2]) - 1;
    if (word[2][i] == ')') {
    ...
    if (*word[2] == EOS || *word[2] == '*') {
        j1 = 1;
        j2 = nX;
    } else {
        j1 = j2 = lab_atoi(word[2], clabX, nX, lcX);
        if (p != NULL) {
            j2 = lab_atoi(p + 1, clabX, nX, lcX);
        }
    }
    if (l_virhe) {
        return (-1);
...
static int op_copy3() /* MAT C(i,j)=A */
{
    ...
    *p = EOS;
    if (nimisijoitus && *word[2] == '0') {
        j1 = 0;
    } else {
        j1 = lab_atoi(word[2], clabX, nX, lcX);
    }
    if (l_virhe) {
        return (-1);
    }
    if (i1 < 0 || i1 > mX || j1 < 0 || j1 > nX) {
        dim_error();
        return (-1);
    }
...
static int op_mp_inv() {
    ...
    if (g > 1) {
        eps = arit_atof(word[2]);
    } else {
        if (!sp_read) {
            mat_spec_read(r1 + r - 1);
        }
        i = spfind_mat("EPS");
        if (i >= 0) {
            eps = atof(spb[i]);
...
static int op_kronecker() {
    ...
    for (i = 0; i < mX * mY * lrY; ++i) {
        rlabT[i] = 'r';
    }
    for (i = 0; i < nX * nY * lcY; ++i) {
        clabT[i] = 'c';
    }
    for (i = 0; i < mX; ++i) {
        for (j = 0; j < mY; ++j) {
...
static int op_mmt() {
    ...
    strcpy(tnimi, suluin(exprX, 1, expr1));
    strcat(tnimi, "*");
    strcat(tnimi, suluin(exprX, 2, expr1));
    strcat(tnimi, "'");
    nim(tnimi, exprX);
...
static int op_gram_schmidt() {
    ...
    if (j <= 0) {
        ...
        strcpy(tnimi, "Linear_dependence_in_");
        strcat(tnimi, exprX);
        nim(tnimi, exprT);
...
static int op_save() {
    ...
    if (matrivi < 0) {
        if (muste_strcmpi(word[2], "AS") == 0) // RS
        {
            ...
            arivi = r1 + r;
            matnimi = 3;
            matrivi = arivi; // RS 3.11.2013 r -> r1+r
        } else {
            PR_EBLD;
    ...
    while (1) {
        ...
        i = split(x + 1, sana, 1);
        if (i == 0 || muste_strcmpi(sana[0], "END") == 0 || j == r2) {
            break;
        }
    ...
    **************************************************/
    ...
    if (n_row_comments) { // 29.1.2005
        mat_save_rem(word[matnimi], X, mX, nX, rlabX, clabX, lrX, lcX, -1, exprX, arivi - matrivi - 3, matrivi + 1,
                     n_row_comments, row_comments);
    } else {
        mat_save(word[matnimi], X, mX, nX, rlabX, clabX, lrX, lcX, -1, exprX, arivi - matrivi - 3, matrivi + 1);
    }
...
static int op_transform() {
    ...
    for (i = 0; i < mX; ++i) {
        for (j = 0; j < nX; ++j) {
            h = i + mX * j;
            arvo[spn - 2] = (double)(i + 1);
            arvo[spn - 1] = (double)(j + 1);
            if (g == 5) {
                arvo[spn - 3] = X[h];
            } else {
                arvo[spn - 4] = X[h];
...
static void submat_error(char *w2, char *w1) {
    muste_sprintf(sbuf, "\nSelector vector %s not compatible with %s!", w2, w1);
    sur_print(sbuf);
    WAIT;
}
...
static void select_labels(char *lab, int m, int lr, int k, int *sel, char *ulab) {
    int i, j;

    for (i = 0; i < k; ++i) {
        for (j = 0; j < lr; ++j) {
            ulab[i * lr + j] = lab[sel[i] * lr + j];
        }
    }
}
...
static int mat_alloc(double **A, /* matriisitila (alkuosoite) (malloc) */
                     int m,      /* rivien lkm */
                     int n       /* sar. lkm   */
) {
...
static int mat_alloc_lab(double **A,  /* matriisitila (alkuosoite) (malloc) */
                         int m,       /* rivien lkm */
                         int n,       /* sar. lkm   */
                         char **rlab, /* rivien otsikot (malloc) */
                         char **clab  /* sar. otsikot   (malloc) */
...
static int op__permord() {
    ...
    permord_fact[0] = 1;
    for (i = 2; i <= k; ++i) {
        permord_fact[i] = i * permord_fact[i - 1];
    }
...
static int op__proddiag() {
    ...
    n = nX;
    if (mY != n || nY != m) {
        muste_sprintf(sbuf, "Incompatible dimensions in matrices %s,%s", word[3], word[3]);
        sur_print(sbuf);
        WAIT;
        return (1);
    }
...
static int op__tab() {
    ...
    i = data_open(word[3], &dat);
    if (i < 0) {
        return (1);
    }
    ix = varfind(&dat, word[4]);
    if (ix < 0) {
        return (1);
    }
    iy = varfind(&dat, word[5]);
    if (iy < 0) {
        return (1);
    }
...
static int op__smooth() {
    ...
    case 0:
        for (i = 1; i < m - 1; ++i) {
            for (j = 1; j < n - 1; ++j) {
                T[i + m * j] = 0.2 * (X[i + m * j] + X[i - 1 + m * j] + X[i + 1 + m * j] + X[i + m * (j - 1)] +
                                      X[i + m * (j + 1)]);
            }
        }
        break;
        ...
        for (i_iter = 0; i_iter < n_iter; ++i_iter) {
            ...
            for (i = 2; i < m - 2; ++i) {
                for (j = 2; j < n - 2; ++j) {
                    T[i + m * j] =
                        0.1 *
                        (X[i + m * j] + X[i - 1 + m * j] + X[i + 1 + m * j] + X[i + m * (j - 1)] + X[i + m * (j + 1)] +
                         0.750 * (X[i - 1 + m * (j - 1)] + X[i + 1 + m * (j - 1)] + X[i + 1 + m * (j - 1)] +
                                  X[i + 1 + m * (j + 1)]) +
...
static int find_type(char *s, int *pdiag) {
    if (muste_strcmpi(s, "D") == 0 || strcmp(s, "0") == 0) {
        return (DIAGONAL);
    }
    ...
    if (muste_strcmpi(s, "LT") == 0 || strcmp(s, "2") == 0) {
        return (LOWER_TRIANGULAR);
    }
    if (muste_strcmpi(s, "UT") == 0 || strcmp(s, "3") == 0) {
        return (UPPER_TRIANGULAR);
    }
    if (muste_strcmpi(s, "G") == 0 || strcmp(s, "9") == 0) {
        return (GENERAL);
    }
...
static void op__mult() {
    ...
    */
    ...
    typeA = typeB = GENERAL;
    if (g > 5) {
        typeA = find_type(word[5], &diagA);
    }
    if (g > 6) {
        typeB = find_type(word[6], &diagB);
    }
    if (typeA < 0 || typeB < 0) {
        return;
    }
    ...
    if ((typeA < GENERAL && mX != nX) || (typeB < GENERAL && mY != nY) || (nX != mY)) {
        dim_error();
        return;
    }
    ...
    strcpy(expr, suluin(exprX, 1, expr1));
    strcat(expr, "*");
...
static void op__samples() {
    ...
    if ((long)nX > nn) {
        muste_sprintf(sbuf, "\nSample size (%d) must not exceed population size (%ld)", nX, nn);
        sur_print(sbuf);
        WAIT;
        return; // RS CHA exit
    }
    if (muste_strnicmp(word[6], "rand(", 5) == 0) {
        rand_type = 1;
        i = 5;
    } else if (muste_strnicmp(word[6], "urand(", 6) == 0) {
        rand_type = 2;
        i = 6;
    } else {
        rand_type = 1;
...
static void op__indvar() {
    ...
    for (i = 0; i < mX; ++i) {
        ...
        if (v[i] < 0) {
            ...
            if (v[i] < 0) {
                muste_sprintf(sbuf, "\nCannot create variable `%s' in data file `%s'!", x, word[5]);
                sur_print(sbuf);
                WAIT;
                return;
            }
...
static int block_error(char *s, int m, int b) {
    muste_sprintf(sbuf, "\n# of rows (%d) of matrix %s not divisible by size of the block (%d)", m, s, b);
    sur_print(sbuf);
    WAIT;
    return (1);
}
...
static void op__merge() {
    ...
    if (mX % blockX != 0) {
        block_error(word[2], mX, blockX);
        return;
    }
    if (mY % blockY != 0) {
        block_error(word[3], mY, blockY);
        return;
    }

    if (mX / blockX != mY / blockY || nX != nY) {
        muste_sprintf(sbuf, "\nMatrices %s and %s have not compatible dimensions!", word[2], word[3]);
        sur_print(sbuf);
        WAIT;
        return;
    }
...
static void op__mindiff() {
    ...
    if (mX != mY || nX != nY) {
        muste_sprintf(sbuf, "Matrices %s and %s have not same dimensions!", word[2], word[3]);
        sur_print(sbuf);
        WAIT;
        return;
    }
...
static void op__colsort() {
    ...
    method = 1;
    if (g > 6) {
        if (muste_strcmpi(word[6], "MAX") == 0) {
            method = 2;
        } else if (muste_strcmpi(word[6], "ROW") == 0) {
            method = 3;
        } else {
            method = -1;
        }
    ...
    rlabT = rlabX;
    if (method == 3) {
        sort_row = atoi(word[7]) - 1; // 26.2.2013
    }
...
static void eigval_columns() {
    ...
    for (i = 0; i < n; ++i) {
        if (i < n - 1 && fabs(X[i + n * (i + 1)]) > eps) {
            T[i] = X[(n + 1) * i];
            T[i + 1] = X[(n + 1) * (i + 1)];
            T[i + n] = fabs(X[i + n * (i + 1)]);
            T[i + n + 1] = -fabs(X[i + 1 + n * i]);
            ++i;
        } else {
            T[i] = X[(n + 1) * i];
            T[i + n] = 0.0;
    ...
    muste_sprintf(expr, "%s~Eigenvalues(%s)_max_off_diag=%g", word[4], word[3], max);
    nim(expr, exprT);
...
static void op__eigen() {
    ...
    if (muste_strnicmp(word[2], "#VAL", 4) == 0) {
        eigval_columns();
        return;
    }
    ...
    muste_sprintf(expr, "%s~Eigenvalues(%s)", word[3], word[2]);
    nim(expr, exprT);
    ...
    if (pT != NULL) {
        muste_sprintf(expr, "%s~Eigenvectors(%s)", word[4], word[2]);
        nim(expr, exprT);
        i = matrix_save(word[4], T, mX, mX, rlabX, textlab, 8, 8, -1, exprT, 0, 0);
    }
    if (pU != NULL) {
        muste_sprintf(expr, "%s~Left_eigenvectors(%s)", word[5], word[2]);
        nim(expr, exprT);
...
static int is_matrix(char *s) // 21.2.2003
{
    ...
    matrix_name(x, s);
    if (!sur_file_exists(x)) {
        return (0); // RS CHA 2.10.2012 <0 -> !
    }
    return (1);
}
...
static int improve(double *cos_m, int m, int k, int *cos_i, double *pdet, double *cos_t) {
    int i, j, h, ih, i1;
    int muutos, muutos2;
    double det;
...
static int maxdet_rand(double *cos_m, int m, int k, double *s, double *pdet, int type) {
    int i, j;
    double detmax, det;
    long ii, nn;
    char x[LLENGTH];
    ...
    if (i == 0) {
        ...
        strcpy(x, spb[i]);
        if (muste_strnicmp(x, "rand(", 5) == 0) {
            rand_type = 1;
            i = 5;
        } else if (muste_strnicmp(x, "urand(", 6) == 0) {
            rand_type = 2;
            i = 6;
        } else {
            rand_type = 1;
...
static int maxdet_all(double *cos_m, int m, int k, double *s, double *pdet) {
    int i, j;
    double detmax, det;
...
static int maxdet(double *cos_m, int m, int k, double *s, double *pdet, int type, int cancel_same) {
    int i, j, h;
    double da;                     // ,db;
    ...
    for (i = 0; i < m; ++i) {
        if (prind) {
            muste_sprintf(sbuf, "%3d ", i + 1);
            sur_print(sbuf);
        }
        if (sh) {
            fprintf(sh_file, "%s", sbuf);
        }
        cos_i[0] = i;
        det1 = cos_m[i * (m + 1)];
        if (det1 == 0.0) {
        ...
        for (h = 2; h <= k; ++h) {
            ...
            for (j = 0; j < m; ++j) {
                ...
                for (ii = 0; ii < h - 1; ++ii) {
                    ...
                    for (jj = 0; jj < ii; ++jj) {
                        da -= 2 * cos_t[ii + k * jj] * cos_u[ii] * cos_u[jj];
                    }
                }
                if (da > detm) {
                    detm = da;
            ...
            det1 *= detm;
            for (ii = 0; ii < h - 1; ++ii) {
                cos_u[ii] = cos_m[jmax + m * cos_i[ii]];
            }
            for (j = 0; j < h - 1; ++j) {
                da = 0.0;
                for (ii = 0; ii < h - 1; ++ii) {
                    da += cos_t[j + k * ii] * cos_u[ii];
                }
                cos_u2[j] = da;
            }
            for (ii = 0; ii < h - 1; ++ii) {
        ...
        if (prind) {
            muste_sprintf(sbuf, " %g\n", det1);
            sur_print(sbuf);
        }
        if (sh) {
            fprintf(sh_file, "%s", sbuf); // RS ADD ,"%s"
        }
...
static double rtsafe(void (*funcd)(double, double *, double *), double x1, double x2, double xacc) {
    int j;
    ...
    if ((fl > 0.0 && fh > 0.0) || (fl < 0.0 && fh < 0.0)) {
        ...
        sur_print(sbuf);
        WAIT;
        return (0.0); // RS FIXME laskuvirhe! exit(1);
    }
    if (fl == 0.0) {
        return (x1);
    }
    ...
    (*funcd)(rts, &f, &df);
    for (j = 0; j < MAXIT; ++j) {
        if ((((rts - xh) * df - f) * ((rts - xl) * df - f) >= 0.0) || (fabs(2 * f) > fabs(dxold * df))) {
            dxold = dx;
            dx = 0.5 * (xh - xl);
            rts = xl + dx;
            if (xl == rts) {
    ...
    muste_sprintf(sbuf, "\nMax # of iterations (%d) exceeded in rtsafe!", MAXIT);
    // RS REM        exit(1);
...
static void op__maxdet() {
    ...
    if (i) {
        prind = atoi(sbuf); // SM ADD 2.6.2012
    }
    if ((i = spfind("PRIND")) >= 0) {
        prind = atoi(spb[i]);
    }
    ...
    if (g < 6) {
        ...
        switch (i) {
        case 0:
            j = maxdet_all(X, mX, dim, T, &det);
            break;
        case 1:
            j = maxdet(X, mX, dim, T, &det, 1, fast);
            break;
        case 2:
            j = maxdet(X, mX, dim, T, &det, 2, fast);
            break;
        case 3:
            j = maxdet_rand(X, mX, dim, T, &det, 1);
            break;
        case 4:
            j = maxdet_rand(X, mX, dim, T, &det, 2);
            break;
        }
        if (j < 0) {
            return; // RS ADD
    ...
    */
    if (det > 0.99999) {
        mcos = 0.0; // SM ADD 2.6.2012
    } else {
        mcos = rtsafe(*max_cos, 0.0, 1.0, 1e-10);
    }
    muste_sprintf(expr, "maxdet(%s)~%g_orthogonality~%g", word[2], det, acos(mcos) / (3.141592653589793 / 2));
    nim(expr, exprT);
...
#define CONVERGENTS_N 20
    ...
    mT = nkonv + 1;
    i = mat_alloc_lab(&T, mT, 5, &rlabT, &clabT);
    if (i < 0) {
        return (1);
    }
    // Rprintf("\nb=%g",b); getch();
...
static int op__intscal() {
    ...
    k = 0;
    if (strchr(".0123456789", *word[3]) != NULL) {
        k = 1;
        b = atof(word[3]);
    } else {
        i = load_X(word[3]);
...
static int op_srs(int n_samp, int m) {
    ...
    for (i = 0; i < n_samp; ++i) {
        ...
        for (j = 0; j < m; ++j) {
            ...
            for (k = 0; k < lrX; ++k) {
                rlabT[j * lrX + i * m * lrX + k] = rlabX[v[j] * lrX + k];
            }
        }
    }
    if (v != NULL) {
...
static int op__sample() {
    ...
    if (m > mX && muste_strcmpi(word[5 + k], "SRS") == 0) {
        muste_sprintf(sbuf, "\nSample size (%d) must not exceed 'population size' (%d)", m, mX);
        sur_print(sbuf);
        WAIT;
        return (-1); // RS CHA exit
    }
    // TÑssÑ sallitaan vain pelkkÑ luku, koska MAT herjaa "Unknown op.."
    if (muste_strnicmp(word[6 + k], "rand(", 5) == 0) {
        rand_type = 1;
        i = 5;
    } else if (muste_strnicmp(word[6 + k], "urand(", 6) == 0) {
        rand_type = 2;
        i = 6;
    } else {
        rand_type = 1;
...
static int op__sort() {
    ...
    if (k + 1 > nX) {
        muste_sprintf(sbuf, "\nOnly %d columns in matrix %s!", nX, word[3]);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int op__eiglan() {
    ...
    *l_file = EOS;
    ...
    for (h = 0; h < k; ++h) {
        ...
        muste_sprintf(sbuf, "n=%d (%1.1e)", n1, t);
        sur_print(sbuf);
...
static int op__aggre() {
    ...
    for (i = 0; i < mX; ++i) {
        ...
        for (j = 0; j < nX; ++j) {
            u = (int)X[s] - m;
            if (u < 0 || u > k - 1) {
                muste_sprintf(sbuf, "\nAll elements of %s must be integers from %d to %d!", word[3], m, n);
                sur_print(sbuf);
                WAIT;
...
static double funktio_mvarit(char *s, double x) {
    ...
    if (strcmp(S, "RAND") == 0) {
        return (sur_rand0(x, 1));
    } else if (strcmp(S, "URAND") == 0) {
        return (sur_rand0(x, 2));
    } else if (strcmp(S, "SRAND") == 0) {
        return (sur_rand0(x, 3));
    } else if (strcmp(S, "MRAND") == 0) {
        return (sur_rand0(x, 4));
    } else if (strncmp(S, "SQR", 3) == 0) /* 29.9.1996 x<0.0 etc. */
    {
        ...
        return (muste_asin(x)); /* 18.6.92 */
    } else if (strcmp(S, "ARCCOS") == 0) {
        return (muste_acos(x));
    } else if (strcmp(S, "ABS") == 0) {
        return (muste_fabs(x));
    } else if (strcmp(S, "INT") == 0) {
        return (muste_floor(x));
    } else if (strcmp(S, "ROUND") == 0) {
        return (sur_round(x));
    } else if (strcmp(S, "SGN") == 0) {
        return (muste_sign(x)); // RS CHA
    } else if (strcmp(S, "IND") == 0) {
        return (muste_ind(x)); // RS CHA
    } else if (strcmp(S, "PROBIT") == 0) {
        return (probit(x));
    } else if (strcmp(S, "RND") == 0) {
        return (uniform(x));

    } else if (strcmp(S, "LFACT") == 0) {
        return (lfact_var(x)); /* 7.9.2007 */
    }

    // RS ADD START
    if (strcmp(S, "TOTIENT") == 0) {
        return (totient(x)); // 19.4.2009
    }
    if (strcmp(S, "ZETA") == 0) {
        return (zeta(x));
    }
    if (strcmp(S, "LGAMMA") == 0) {
        return (muste_lgamma(x)); // RS
    }
    if (strcmp(S, "GAMMA") == 0) {
        return (muste_gamma(x)); // RS
    }
    if (strcmp(S, "DIGAMMA") == 0) {
        return (muste_digamma(x)); // RS
    }
    if (strcmp(S, "TRIGAMMA") == 0) {
        return (muste_trigamma(x)); // RS
    }
    if (strcmp(S, "TETRAGAMMA") == 0) {
        return (muste_tetragamma(x)); // RS
    }
    if (strcmp(S, "PENTAGAMMA") == 0) {
        return (muste_pentagamma(x)); // RS
    }
...
static double mfunktio_mvarit(char *s, double *x, int n) {
    ...
    */
    ...
    if (strcmp(S, "DNORM") == 0) {
        if (n > 3) {
            return (muste_density_normal(x[0], x[1], x[2], (int)x[3]));
        }
        return (muste_density_normal(x[0], x[1], x[2], (int)0));
    }

    if (strcmp(S, "MAX") == 0) {
        return (muste_max(x, n));
    }
    if (strcmp(S, "MIN") == 0) {
        return (muste_min(x, n));
    }
    if (strcmp(S, "MAXN") == 0) {
        return (muste_maxn(x, n));
    }
    if (strcmp(S, "MINN") == 0) {
        return (muste_minn(x, n));
    }
    if (strcmp(s, "C") == 0) {
        if (n != 2) {
            arg_virhe(s);
    ...
    if (strcmp(S, "BETA") == 0) {
        return (muste_beta(x[0], x[1])); // RS
    }
    if (strcmp(S, "LBETA") == 0) {
        return (muste_lbeta(x[0], x[1])); // RS
    }

    if (strcmp(S, "FIN.PV") == 0) {
        return (muste_fin_pv(x[0], x[1], x[2])); // RS
    }
    if (strcmp(S, "FIN.FV") == 0) {
        return (muste_fin_fv(x[0], x[1], x[2])); // RS
    }
    if (strcmp(S, "FIN.PMT") == 0) {
        return (muste_fin_pmt(x[0], x[1], x[2])); // RS
    }

    if (strcmp(S, "BOXCOX") == 0) {
        return (muste_boxcox(x[0], x[1])); // RS
    }
    if (strcmp(S, "BOXCOX.G") == 0) {
        return (muste_inv_boxcox(x[0], x[1])); // RS
    }

    if (strcmp(S, "DISS") == 0) {
        return (muste_diss(x[0], x[1], (int)0)); // RS
    }
    if (strcmp(S, "DISS.F") == 0) {
        return (muste_diss(x[0], x[1], (int)1)); // RS
    }

    if (strcmp(S, "BESTVAL") == 0) {
        return (muste_bestval(x[0], x[1])); // RS
    }
...
static int varif_mvarit(char *lauseke, double *y) {
    ...
    */
    if (strncmp(a, "str(", 4) == 0) /* 13.3.1991 */
    {
        ...
        sur_print("\nString comparisons not permitted!");
        WAIT;
        return (-1);
        /*    if (tosi<0) return(-1);  */
    }
...
static int f_edit_mvarit(char *s, double *x, int n, double *py) {
    ...
    s[len++] = '(';
    i = 0;
    while (i < spn && (spa[i][strlen(spa[i]) - 1] != ':' || strncmp(s, spa[i], len) != 0)) {
        ++i;
    }
    if (i == spn) {
        s[len - 1] = EOS;
...
static int aseta_earg(double luku, char *sana) {
    ...
    sana[1] = EOS;
    strcat(sana, muste_itoa(n_earg, sana2, 10));
    earg[n_earg++] = luku;
    return (n_earg - 1);
}
...
static void rec_transf() {
    ...
    f[len++] = '(';
    i = 0;
    while (i < spn && (spa[i][strlen(spa[i]) - 1] != ':' || strncmp(f, spa[i], len) != 0)) {
        ++i;
    }

    if (i == spn) {
        muste_sprintf(sbuf, "\nRecurrence relation for %s not defined as a temporary function!", rec_func);
        sur_print(sbuf);
        WAIT;
        return; // RS CHA exit(0);
    }
    ...
    if (i >= 0) {
        ...
        i = split(x, s, 2);
        if (i > 0) {
            start1 = arit_atoi_mvarit(s[0]);
        }
        if (i > 1) {
            start2 = arit_atoi_mvarit(s[1]);
        }
    }
...
static int op__hada() {
    ...
    n = nX;
    if (mY != m || nY != n) {
        muste_sprintf(sbuf, "\nIncompatible dimensions in matrices %s,%s", word[3], word[4]);
        sur_print(sbuf);
        WAIT;
        return (1);
    }
...
static void external_op() {
    ...
    if (mtx) {
        ...
        *info = 'M';
        ...
        for (i = 0; i < sizeof(long); ++i) {
            *p++ = *q++; // RS FIXME 64-BIT ??? sizeof(long) -> sizeof(int)
        }
    ...
    strcpy(opfile, survo_path);
    strcat(opfile, "&MATEXT1.EXE");
    s_end(mat_argv1);
    ...
    if (muste_strcmpi(osa[1], "#") == 0) {
        list_nimet();
        m_end();
        return;
    }

    if (muste_strcmpi(osa[1], "#MULT") == 0) {
        op__mult();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#SAMPLES") == 0) {
        op__samples();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#INDVAR") == 0) {
        op__indvar();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#MERGE") == 0) {
        op__merge();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#MINDIFF") == 0) {
        op__mindiff();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#COLSORT") == 0) {
        op__colsort();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#CRSORT") == 0) {
        op__crsort();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#EIGEN") == 0) {
        op__eigen();
        m_end();
        return;
    } else if (muste_strnicmp(osa[1], "#CONVOL", 7) == 0) {
        op__convol();
        m_end();
        return;
    } else if (muste_strnicmp(osa[1], "#MAXDET", 7) == 0) {
        op__maxdet();
        m_end();
        return;
    } else if (muste_strnicmp(osa[1], "#U_TO_F", 7) == 0) {
        op__u_to_f();
        m_end();
        return;
    } else if (muste_strnicmp(osa[1], "#INTSCAL", 8) == 0) {
        op__intscal();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#FRAC_TO_DEC") == 0) {
        op__frac_to_dec();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#SAMPLE") == 0) {
        op__sample();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#SORT") == 0) {
        op__sort();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#MAGIC") == 0) {
        op__magic();
        m_end();
        return;
    } else if (muste_strnicmp(osa[1], "#JACK", 5) == 0) {
        op__jack();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#EIGFEW") == 0) {
        op__eigfew();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#EIGLAN") == 0) {
        op__eiglan();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#AGGRE") == 0) {
        op__aggre();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#PERMORD") == 0) {
        op__permord();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#FREQ") == 0) {
        op__freq();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#RCSORT") == 0) {
        op__rcsort();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#PRODDIAG") == 0) {
        op__proddiag();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#TAB") == 0) {
        op__tab();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#SMOOTH") == 0) {
        op__smooth();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#RAO_KHATRI") == 0) {
        op__rao_khatri();
        m_end();
        return;
    } else if (muste_strcmpi(osa[1], "#HADAMARD") == 0) {
        op__hada();
        m_end();
        return;
    }
...
static int op1(char op, char *opnd1, char *opnd2, char *tulos) {
    ...
    {
        i = strlen(opnd1) - 1;
        if (opnd1[i] == '\'' && strlen(opnd2) == i && strncmp(opnd1, opnd2, i) == 0) {
            word[1] = opnd2;
            return (op_mtm());
        }
        i = strlen(opnd2) - 1;
        if (opnd2[i] == '\'' && strlen(opnd1) == i && strncmp(opnd1, opnd2, i) == 0) {
            word[1] = opnd1;
            return (op_mmt());
        }
    }
...
static int op2(char *ops, char *opr1) {
    ...
    ********************/
    i = 1;
    while (word[i][strlen(word[i]) - 1] != ')' && i < g) {
        ++i;
    }
    if (i == g) {
        PR_EBLD;
    ...
    g = i;
    word[g][strlen(word[g]) - 1] = EOS;

    // Rprintf("\n"); for (i=1; i<=g; ++i) Rprintf(" %s",word[i]); sur_getch();

    /* Huom! parametrit ovat nyt word[1],...,word[g] */

    if (muste_strcmpi(ops, "CON") == 0) {
        i = op_con(1.0);
        return (i);
    } else if (muste_strcmpi(ops, "ZER") == 0) {
        i = op_zer();
        return (i);
    } else if (muste_strcmpi(ops, "IDN") == 0) {
        i = op_idn_tri(1);
        return (i);
    } else if (muste_strcmpi(ops, "TRI") == 0) {
        i = op_idn_tri(2);
        return (i);
    } else if (muste_strcmpi(ops, "HILBERT") == 0) {
        i = op_idn_tri(3);
        return (i);
    } else if (muste_strcmpi(ops, "INV") == 0) {
        i = op_inv();
        return (i);
    } else if (muste_strcmpi(ops, "DV") == 0) {
        i = op_dv();
        return (i);
    } else if (muste_strcmpi(ops, "VD") == 0) {
        i = op_vd();
        return (i);
    } else if (muste_strcmpi(ops, "MTM") == 0) {
        i = op_mtm();
        return (i);
    } else if (muste_strcmpi(ops, "SUM") == 0) {
        i = op_sum();
        return (i);
    } else if (muste_strcmpi(ops, "MMT") == 0) {
        i = op_mmt();
        return (i);
    } else if (muste_strcmpi(ops, "CENTER") == 0) {
        i = op_center();
        return (i);
    } else if (muste_strcmpi(ops, "NRM") == 0 || muste_strcmpi(ops, "NORM") == 0) {
        i = op_nrm();
        return (i);
    } else if (muste_strcmpi(ops, "CHOL") == 0) {
        i = op_chol();
        return (i);
    } else if (muste_strcmpi(ops, "DINV") == 0) {
        i = op_dinv();
        return (i);
    } else if (muste_strcmpi(ops, "MAX") == 0) {
        i = op_max_min(1);
        return (i);
    } else if (muste_strcmpi(ops, "MIN") == 0) {
        i = op_max_min(2);
        return (i);
    } else if (muste_strcmpi(ops, "VEC") == 0) {
        i = op_vec(0);
        return (i);
    } else if (muste_strcmpi(ops, "NVEC") == 0) {
        i = op_vec(1);
        return (i);
    } else if (muste_strcmpi(ops, "PERM") == 0) {
        i = op_perm(1);
        return (i);
    } else if (muste_strcmpi(ops, "PERM2") == 0) {
        i = op_perm(2);
        return (i);
    } else if (muste_strcmpi(ops, "PERM0") == 0) {
        i = op_perm(0);
        return (i);
    } else if (muste_strcmpi(ops, "P") == 0 && isupper((int)*word[1])) {
        i = op_p();
        return (i);
    } else if (muste_strcmpi(ops, "MMT2") == 0) {
        i = op_mmt2();
        return (i);
    } else if (muste_strcmpi(ops, "MTM2") == 0) {
        i = op_mtm2();
        return (i);
    } else if (muste_strcmpi(ops, "DIAGVEC") == 0) {
        i = op_diagvec();
        return (i);
    } /* 5.3.1995 */
    else if (muste_strnicmp(ops, "KRON", 4) == 0) {
        i = op_kronecker();
        return (i);
    } /* ?.6.1995 */
    else if (muste_strnicmp(ops, "CUM", 3) == 0) {
        i = op_cum(1);
        return (i);
    } /* 8.7.1998 */
    else if (muste_strnicmp(ops, "UNCUM", 3) == 0) {
        i = op_cum(2);
        return (i);
    } /* 8.7.1998 */
    else if (muste_strnicmp(ops, "PROD", 4) == 0) {
        i = op_product();
        return (i);
    } /* 8.7.1998 */
    else if (muste_strcmpi(ops, "SELECT") == 0) {
        i = op_select();
        return (i);
    } /* 4.9.1998 */
    else if (muste_strcmpi(ops, "DIAG") == 0) {
        i = op_diag();
        return (i);
    } /* 3.4.1999 */
    else if (muste_strcmpi(ops, "SUB") == 0) {
        i = op_submat();
        return (i);
    } /* 6.6.1999 */
    else if (muste_strcmpi(ops, "MAX_IJ") == 0) {
        i = op_max_min_ij(1);
        return (i);
    } /* 5.10.1999 */
    else if (muste_strcmpi(ops, "MIN_IJ") == 0) {
        i = op_max_min_ij(2);
        return (i);
    } /* 5.10.1999 */
    else if (muste_strnicmp(ops, "NULL", 4) == 0) {
        i = op_null();
        return (i);
    } /* 10.3.2003 */
    else if (muste_strcmpi(ops, "BASIS") == 0) {
        i = op_basis();
        return (i);
    } /* 10.3.2003 */
    else if (muste_strcmpi(ops, "DET") == 0) {
        i = op_det(1);
        return (i);
    } /* 10.3.2003 */
    else if (muste_strcmpi(ops, "LDET") == 0) {
        i = op_det(2);
        return (i);
    } /* 10.3.2003 */
    else if (muste_strcmpi(ops, "RANK") == 0) {
        i = op_rank();
        return (i);
    } /* 10.3.2003 */
    else if (muste_strcmpi(ops, "TRACE") == 0) {
        i = op_trace1();
        return (i);
    } /* 10.3.2003 */
    else if (muste_strcmpi(ops, "MPINV") == 0) {
        i = op_mp_inv();
        return (i);
    } /* 10.3.2003 */
...
static int op3() {
    ...
    strcpy(p, word[1]);
    muste_strupr(p); /* ennen 4,11,87  strcpy(p,strupr(word[1]); */
    if (strcmp(p, "REM") == 0 || strcmp(p, "/*") == 0 || strcmp(p, "*") == 0) {
        i = op_rem();
        return (1);
    }
    q = strchr(word[2], '=');
    ...
    if (strncmp(p, "SING", 4) == 0 || strcmp(p, "SVD") == 0) {
        i = op_svd();
        return (i);
    }
    if (strcmp(p, "NAME") == 0) {
    ...
    if (strcmp(p, "RLABELS") == 0) {
        i = op_rlab();
        return (i);
    }
    if (strcmp(p, "CLABELS") == 0) {
        i = op_clab();
        return (i);
    }
    if (strcmp(p, "KILL") == 0) {
    ...
    if (strcmp(p, "SOLVE1") == 0) {
        i = op_solve(1);
        return (i);
    } // 13.7.2005
    if (strncmp(p, "SPEC", 4) == 0) {
        i = op_spectral();
        return (i);
    }
    if (strncmp(p, "TRAN", 4) == 0) {
        i = op_transform();
        return (i);
    }
    if (strcmp(p, "DIM") == 0) {
...
static int matrix_op() {
    ...
    if (p == NULL) {
        if (g == 2 && *word[1] != '#') {
            return (op_tell(word[1], word[1]));
        }
        i = op3();
        return (i);
    }

    muste_fieldcopy(tulos, word[1], p - word[1]);
    tulos[p - word[1]] = EOS;
    strcpy(lauseke, p + 1);
    ...
    while (*p) {
        ...
        case '/': /* 11.1.1999 */
            ...
            if (*lauseke == '/' || *lauseke == '\\' || *lauseke == '~' || *lauseke == '<' || *lauseke == '"' ||
                (*lauseke == '.' &&
                 (*(lauseke + 1) == '.' || *(lauseke + 1) == '/' || *(lauseke + 1) == '\\'))) // RS 19.3.2013
            {
                //                    muste_fixme("\nMAT - Conflicting use of /"); // RS FIXME
                for (i = 0; i < strlen(lauseke); i++) {
                    if (*lauseke == '/') {
                        *lauseke = '\\';
                    }
                }
            ...
            muste_fieldcopy(opnd1, lauseke, p - lauseke);
            opnd1[p - lauseke] = EOS;
            op = *p;
            strcpy(opnd2, p + 1);
            /*          Rprintf("\nopnd1=%s",opnd1);
...
int muste_pol_save2(char *matr, struct polynom *pol, char *ots, /* column labels */
...
static int mtx_open(long lpos) {
    ...
    for (nmtx = 0; nmtx < i; ++nmtx) {
        if (strcmp(pmtx[nmtx + 2], "/") == 0) {
            break;
        }
    }
    /*    mtx_prog=pmtx[0]; */
...
static int create_arrays(char *x) {
    ...
    for (i = 0; i < len; ++i) {
        ...
        case '-':
            ...
            break;
        case '*':
            /* A(*,1:5) */ if ((x[i - 1] == '(' && x[i + 1] == ',') || (x[i - 1] == ',' && x[i + 1] == ')')) {
                level[i] = level1;
                break;
            }
            power[i] = 2;
...
*/
#if TESTAUS
        Rprintf("n=%d pos1=%d len2=%d \n", n_pairs[power1], pos1[power1][n_pairs[power1]],
                len2[power1][n_pairs[power1]]);
        sur_getch();
#endif
        if (scalar_test) {
#if TESTAUS
            ...
            sur_getch();
#endif
            test_scalar(x, pos1[power1][n_pairs[power1]], len2[power1][n_pairs[power1]], i);
        }
        ++n_pairs[power1];
    }
}
...
static void replace_scalars(char *x, char *x0) {
    ...
    while (i < len) {
        ...
        muste_sprintf(sbuf, "#%d#", n_scalars);
        ++n_scalars;
        y[j] = EOS;
        strcat(y, sbuf);
        j += strlen(sbuf);
...
static int find_functions(char *x) {
    ...
    for (i = 0; i < len; ++i) {
        switch (x[i]) {
        case '(':
            if (i > 0 && strchr("+-*^,(", x[i - 1]) != NULL) {
                ++level1;
                break;
            }
            j = i - 1;
            while (j >= 0 && strchr("+-*^,(", x[j]) == NULL) {
                --j;
            }
            pos1[0][n] = j + 1;
            func_level[n] = level1;
...
static int set_mat_command(char *x) {
    ...
    if (n_mat == 0 && n_laji == 1 && n_pairs[k] == 1 && strchr(x, '\'') == NULL) {
        return (0);
    }
#if TESTAUS
    Rprintf("min=%.*s|\n", len2[k][imin], x + pos1[k][imin]);
    sur_getch();
#endif

    *expr = EOS;
    strncat(expr, x + pos1[k][imin], len2[k][imin]);
    strcpy(expr2, expr);
...
static int matpar2(char *expr) {
    ...
    while (1) {
        ...
        if (i == 0 && n_mat == 0) {
            return (0); /* common MAT operation */
        }
        if (i == 0) {
            return (1); /* composite MAT command */
        }
    } /* while */
...
int muste_mat(int argc, char *argv[]) {
    ...
    for (i = 0; i < MAXMTX; i++)
        pmtx[i] = NULL; // RS 1.6.2013
    for (i = 0; i < MAXARG + 4; i++)
        str_opnd[i] = NULL; // RS 1.6.2013
    ...
    *********************************************/
    ...
    while (1) {
        ...
        if (g > 2 && strchr(word[1], '(') != NULL && strchr(word[2], '=') != NULL) {
            part_copy = 1;
        }
        ...
        if (!rem_print) {
            if (mtx || mat_parser == 0) {
                ...
                if (i < 0) {
                    mat_error();
                    return (-1);
                } // RS FIXME Close files, free memory
            } else {
        ...
        strcpy(x, "RUN: ");
        strncat(x, comline + 1, c3);
        g = split(comline + 1, word, MAXPARM);
        for (i = 0; i < g; ++i) { /* 6.11.1992 */
            if (word[i][0] == '/' && word[i][1] == EOS) {
                break;
            }
        }
        g = i;
        if (strncmp(muste_strupr(word[0]), "MAT", 3) != 0) {
            break;
        }

        /* "lapsenlapsen vÑlttÑmiseksi 29.2.2000 */
        if (g > 2 && muste_strcmpi(word[1], "SAVE") == 0 && muste_strcmpi(word[2], "DATA") == 0) {
            break;
        }

        LOCATE(r3 + 2, 1);
        PR_EINV;
        muste_sprintf(sbuf, "%s", x);
        sur_print(sbuf);
        PR_ENRM;
        /*          vapauta();    */
    }
