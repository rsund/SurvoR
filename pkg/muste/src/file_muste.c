...
SEXP muste_survodata2r(char *name, int muste_internal) {
    ...
    if (muste_internal) {
        ...
        if ((i = spfind("STRIP_WHITE")) >= 0) {
            strip_white = atoi(spb[i]); // RS 17.11.2013
        }
    }
    ...
    if (othertype == 1) {
        snprintf(buf, LLENGTH, "Survo edit data %s: record=%d bytes, M1=%d L=%d  M=%d N=%d", name, d.d2.len, d.d2.m1,
                 d.d2.l, nvar, nobs);
    } else {
        snprintf(buf, LLENGTH, "Survo data file %s: record=%d bytes, M1=%d L=%d  M=%d N=%d", active_data, d.d2.len,
                 d.d2.m1, d.d2.l, nvar, nobs);
    }
...
SEXP R_LoadSurvoData(SEXP name) { return (muste_survodata2r((char *)CHAR(STRING_ELT(name, 0)), 0)); }
...
SEXP R_LoadSurvoData2(SEXP name) { return (muste_survodata2r((char *)CHAR(STRING_ELT(name, 0)), 1)); }
...
int muste_r2survodata(char *sname, int muste_internal, SEXP df, char *rname) {
    ...
    if (!Rf_inherits(df, "data.frame")) {
        if (muste_internal) {
            sur_print("\nData to be saved must be in a data frame");
            WAIT;
            return (-1);
        } else {
    ...
    for (i = 0; i < nvar; i++) {
        ...
        case INTSXP:
            ...
            for (j = 0; j < nobs; j++) {
                ...
                if (INTEGER(VECTOR_ELT(df, i))[j] == NA_INTEGER) {
                    k = 0;
                } else {
    ...
    fitextn = 1;
    fitextlen = c2;
    strcpy(jakso, " Copied from R data frame ");
    strcat(jakso, rname);
    privi[0] = jakso;
    fitext = privi;
    ...
    i = fi_create(sname, filen, fim1, fim, 0L, fil, fiextra, fitextn, fitextlen, fitext, varname, varlen, pvartype);
    UNPROTECT(1); // names
    if (i < 0) {
        return (-1);
    }
    ...
    for (i = 0; i < nobs; i++) {
        ...
        for (j = 0; j < nvar; j++) {
            ...
            case LGLSXP:
                ...
                if (LOGICAL(VECTOR_ELT(df, j))[i] == NA_LOGICAL) {
                    fi_miss_save(&d2.d2, j2, j);
                } else {
            ...
            case INTSXP:
                ...
                if (INTEGER(VECTOR_ELT(df, j))[i] == NA_INTEGER) {
                    fi_miss_save(&d2.d2, j2, j);
                } else {
            ...
            case REALSXP:
                ...
                if (ISNA(REAL(VECTOR_ELT(df, j))[i])) {
                    fi_miss_save(&d2.d2, j2, j);
                } else {
            ...
            case STRSXP:
                ...
                for (k = strlen(jakso); k < d2.varlen[j]; ++k) {
                    jakso[k] = ' ';
                }
