...
void muste_facta(char *argv) {
    ...
    if (k < 1 || k > p - 1) {
        ...
        if (etu == 2) {
            muste_sprintf(tut_info, "___@6@FACTA@%s@", sbuf);
    ...
    *mess = EOS;
    ...
    i = spfind("METHOD");
    if (i >= 0) {
        if (muste_strcmpi(spb[i], "ULS") == 0) {
            ind = 1;
        }
        if (muste_strcmpi(spb[i], "GLS") == 0) {
            ind = 2;
        }
    }
    ...
    for (i = 0; i < p; ++i) {
        ...
        if (etu == 2) {
            muste_sprintf(tut_info, "___@1@FACTA@%s@", sbuf);
            s_end(argv); // RS CHA argv[1]
            return;
        }
        sur_print("\n");
    ...
    */
    ...
    switch (ind) {
    case 1:
        strcat(x, "Unweighted Least Squares (ULS) solution");
        break;
    case 2:
        strcat(x, "Generalized Least Squares (GLS) solution");
        break;
    case 3:
        strcat(x, "Maximum Likelihood (ML) solution");
        break;
    }
    print_line(x);
    if (*mess) {
    ...
    if (i >= 0) {
        if (ind > 1) {
            ...
            muste_sprintf(x, "factors=%d Chi^2=%g df=%d P=%5.3f reliability=%g", k, chi2k, (int)dk, pr, rho);
            if (rho > 1.0) {
                q = strstr(x, "rel");
                *q = EOS;
            } /* 3.6.1995 */
            ...
            pr = 0.0;
            muste_sprintf(x, "factors=%d Chi^2=%g df=%d P=%5.3f", k, uu, (int)dk, pr);
            print_line(x);
        }
    }
    output_close(eout);
    ...
    acc[accuracy - 1] = EOS;
    matrix_print(S, p, k + 1, rlab, clab, lr, lc, p, k + 1, NULL, NULL, acc, c3, results_line, eout, "Factor matrix");
    s_end(argv);
}
...
static int nwtrap() {
    ...
    if (ind > 1) {
        ...
        if (i != 1 /* || det<1e-40 */) /* -10 aikaisemmin */
        {
            ...
            if (etu == 2) {
                muste_sprintf(tut_info, "___@2@FACTA@%s@", sbuf);
                return (-1);
            }
            sur_print("\n");
            sur_print(sbuf);
    ...
    if (ind > 1) {
        for (i = 0; i < p; ++i) {
            v[i] = log((1.0 - (double)k / (2 * p)) / S[i * (p + 1)]);
        }
    } else {
        bnd = sqrt(eps);
        i = mat_dcholinv(S, p, &det);
        if (i != 1) {
            ...
            mat_cholmove(S, p);
            for (i = 0; i < p; ++i) {
                v[i] = sqrt((1.0 - (double)k / (2 * p)) / S[i * (p + 1)]);
            }
        }
        for (ui = 0; ui < p * p; ++ui) {
            apu = E[ui];
    ...
    while (1) {
        ...
        */
        ...
        if (iter == maxit) {
            ...
            sur_print("\nNo convergence! (Try with FEPS=0.005 or greater.)");
            WAIT;
            return (-1);
        }
        ...
        if (mor == 0) {
            ...
            */
            if (ind > 1) {
                ...
                for (i = 0; i < k; ++i) {
                    d[i] = sqrt(1 / facta_gamma[p - 1 - i] - 1.0);
                }
                for (i = 0; i < p; ++i) {
                    for (j = 0; j < k; ++j) {
...
static int fctgr() {
    ...
    while (1) {
        if (ind > 1) {
            ...
            for (i = 0; i < p; ++i) {
                for (j = 0; j < p; ++j) {
                    E[i + p * j] = om[i + p * j] = psi[i] * psi[j] * S[i + p * j];
                }
            }
            ...
            if (ind == 2) {
                f = 0.0;
                for (i = 0; i < p - k; ++i) {
                    f += (facta_gamma[i] - 1) * (facta_gamma[i] - 1);
                }
                f /= 2;
                for (i = 0; i < p; ++i) {
                    d[i] = facta_gamma[i] * (facta_gamma[i] - 1.0);
                }
            } else /* ind=3 */
            {
                f = 0.0;
                for (i = 0; i < p - k; ++i) {
                    f += log(facta_gamma[i]) + 1 / facta_gamma[i] - 1;
                }
                for (i = 0; i < p; ++i) {
                    d[i] = 1 - 1 / facta_gamma[i];
                }
            }
            ...
            f = 0.0;
            for (i = k; i < p; ++i) {
                f += facta_gamma[i] * facta_gamma[i];
            }
            f /= 2;
            for (i = 0; i < p; ++i) {
                gg[i] = 0.0;
                for (j = k; j < p; ++j) {
                    gg[i] += facta_gamma[j] * om[i + p * j] * om[i + p * j];
                }
                gg[i] *= -2 * psi[i];
            }
        }
            ...
            for (j = 0; j <= i; ++j) {
                ...
                for (m = k; m < p; ++m) {
                    ...
                    for (n = 0; n < k; ++n) {
                        ...
                        db += (facta_gamma[m] + facta_gamma[n]) / ggg * om[i + p * n] * om[j + p * n];
                    }
                    db *= om[i + p * m] * om[j + p * m];
                    da += db;
                    if (i == j) {
                        da2 += (psi[i] * psi[i] - facta_gamma[m] / 2.0) * om[i + p * m] * om[i + p * m];
                    }
                }
                da *= psi[i] * psi[j];
                da = 4 * (da + da2);
            ...
            for (j = 0; j <= i; ++j) {
                ...
                for (m = 0; m < p - k; ++m) {
                    ...
                    for (n = p - k; n < p; ++n) {
                        ...
                        db += facta_gamma[n] * (facta_gamma[m] + facta_gamma[n] - 2) / ggg * om[i + p * n] *
                              om[j + p * n];
                    }
            ...
            for (j = 0; j <= i; ++j) {
                ...
                for (m = 0; m < p - k; ++m) {
                    ...
                    for (n = p - k; n < p; ++n) {
                        ...
                        db += (facta_gamma[m] + facta_gamma[n] - 2) / ggg * om[i + p * n] * om[j + p * n];
                    }
                    if (i == j) {
                        db += 1.0;
                    }
...
static int invalid_R() {
    ...
    if (etu == 2) {
        muste_sprintf(tut_info, "___@5@FACTA@%s@", sbuf);
        return (-1);
    }
    sur_print("\n");
    sur_print(sbuf);
