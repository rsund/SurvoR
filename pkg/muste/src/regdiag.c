...
void muste_regdiag(char *argv) {
    ...
    */
    ...
    if (resvar >= 0 || predvar >= 0 || hatvar >= 0 || studvar >= 0 || cookvar >= 0) {
        ...
        if (i < 0) {
            muste_sprintf(sbuf, "\nCannot write derived variables in data %s", word[1]);
            WAIT;
            return;
        }
        i = save_variables();
    ...
    for (i = 0; i < m; ++i) {
        if (i == 0 && konst) {
            muste_fieldcopy(name, "Constant", 8);
        } else {
            muste_fieldcopy(name, dat.varname[xvar[i - konst]], 8);
        }
        for (h = 0; h < 8; ++h) {
            if (name[h] == EOS) {
                break;
    ...
    strcat(sbuf, word[1]);
    strcat(sbuf, ")");
    matrix_save("REG_CORR.M", invxtx, m, m, r_label, r_label, 8, 8, -1, sbuf, 0,
                0); // RS 2.11.2014 label -> r_label (2nd one)
    save_rg_matrix();
...
static int save_variables() {
    ...
    for (j = dat.l1; j <= dat.l2; ++j) {
        ...
        miss = 0;
        data_load(&dat, j, yvar, &x);
        if (x == MISSING8) {
            miss = 1;
        }
        for (i = 0; i < mxvar; ++i) {
            data_load(&dat, j, xvar[i], &x);
            if (x == MISSING8) {
                miss = 1;
                break;
            }
        ...
        if (cookvar >= 0) {
            if (!miss) {
                ...
                if (x <= 0.0) {
                    ...
                    x = res[jn] * sqrt(hat[jn] / m / (rss / (n - m))) / (1 - hat[jn]);
                }
            }
    ...
    if (resvar >= 0) {
        update_varname(&dat, resvar, "Residual from REGDIAG");
    }
    if (predvar >= 0) {
        update_varname(&dat, predvar, "Predicted value from REGDIAG");
    }
    if (hatvar >= 0) {
        update_varname(&dat, hatvar, "Diagonal of hat matrix from REGDIAG");
    }
    if (studvar >= 0) {
        update_varname(&dat, studvar, "Studentized residual from REGDIAG");
    }
    if (cookvar >= 0) {
        update_varname(&dat, cookvar, "Cook's distance (signed sqrt) from REGDIAG");
    }
    return (1);
}
/*****************
...
static int save_rg_matrix() {
    ...
    *text = EOS;
    ...
    matrix_save0("RG.M", rg, N_RG, 1, label, "REGDIAG ", 8, 8, -1, "RG.M", N_LINES, 0, text);
    return (1);
}
...
static int linreg() {
    ...
    if (konst) {
        muste_sprintf(x + i, " (Constant term included)");
    }
    print_line(x);
    ...
    sur_print("\nSing.values: ");
    for (i = 0; i < m; ++i) {
        muste_sprintf(sbuf, "%g ", D[i]);
        sur_print(sbuf);
    }
    implicit_constant = 0;
    if (m == mxvar) {
        implicit_constant = check_1(X, n, m); /* 15.6.1997 */
    }
    ...
    compute_invxtx();
    for (i = 0; i < m; ++i) {
        se[i] = sqrt(invxtx[i + m * i] * rss / (n - m));
    }
...
static int print_coeff() {
    ...
    for (i = 0; i < m; ++i) {
        if (konst && i == 0) {
            k = muste_sprintf(x, "Constant");
        } else {
            k = muste_sprintf(x, "%8.8s", dat.varname[xvar[i - konst]]);
        }
        ...
        *sana2 = *sana3 = EOS;
        if (!singular) {
            ...
            if (se[i] < 1e-12) {
                strcpy(sana3, "-"); // 16.7.2004
            } else {
                fnconv(b[i] / se[i], ac - 3, sana3);
            }
        }
    ...
    muste_fieldcopy(sana1, dat.varname[yvar], 8);
    sana1[8] = EOS;
    h = 8;
    while (sana1[h - 1] == ' ') {
        sana1[--h] = EOS;
    ...
    print_line(x);
    if (m == mxvar && !implicit_constant) {
        r2_without_c(&a); /* 27.2.1997 */
                          /* 15.6.1997 */
    } else {
        a = 1.0 - rss / ((n - 1) * vy);
    }
    ...
    if (maxlag) {
        ...
        for (i = 1; i <= maxlag; ++i) {
            ...
            muste_sprintf(sbuf, "%3d   %s   %s   %s       %s       %s", i, sana1, sana2, sana3, sana4, sana5);
            print_line(sbuf);
        }
        if (dw_test == 1) {
...
static double dw_probability() {
    ...
    if (dwdata) {
        dw_data = muste_fopen("DWDATA.TXT", "wt");
    }
    spec_rnd();
    ln1 = 0L;
...
static double dw_probability2() {
    ...
    muste_sprintf(sbuf, "\n\nP value for DW by simulation: %d replicates...", nsimul);
    sur_print(sbuf);
