...
static int seek_table(void) {
    ...
    L1 = edline2(wrd[2], 1, 1);
    if (!L1) {
        return -1;
    }
    L2 = edline2(wrd[3], 1, 1);
    if (!L2) {
        return -1;
    }
    return 1;
...
static int show_info(void) {
    ...
    muste_kv_s_disp("\n ");
    muste_kv_s_disp("\n TABLE %s", word[2]);
    muste_kv_s_disp("\n dim=%d cdim=%d rdim=%d cols=%d rows=%d N=%d classes=%d", dim, cdim, rdim, cols, rows, N,
                    classes);
    muste_kv_s_disp("\n ");
    muste_kv_s_disp("\n col names :%s", cnames);
    muste_kv_s_disp("\n col sizes :%s", csizes);
...
static int binary_table(void) {
    ...
    muste_kv_space_split(csizes, wrd, LLENGTH);
    j = 0;
    for (i = 0; i < cdim; i++, j++) {
        crsiz[j] = atoi(wrd[i]);
    }
...
static int read_data(void) {
    ...
    for (i = 0; i < dat.m_act; i++) {
        ...
        nval[i] = 0;
        miss = 0;
        strcpy(nam, dat.varname[dat.v[i]]);
        trim2(nam);
        j = spfind(nam);
        if (j < 0) {
            continue; /* classification not given */
        }
        strcpy(sbuf, spb[j]);
        if (strchr(sbuf, ',') != NULL) { /* <nam>=1,1,3,5 */
            ...
            if (!strncmp(wrd[m - 1], "MISSING", 7)) { /* <nam>=1,3,MISSING */
                miss = 1;
                m--; /* forget missing class for a moment */
            }
            for (k = 0; k < m; k++) {
                values[i * vmax + k] = atof(wrd[k]);
            }
            if (miss) {
                values[i * vmax + k] = MV;
    ...
    for (j = dat.l1; j <= dat.l2; j++) {
        ...
        for (i = 0; i < dat.m_act; i++) {
            ...
            if (x == MISSING8) {
                continue; /* no missings by default */
            }
            for (k = 1; k < vmax; k++) { /* always skip MV */
                if (values[i * vmax + k] == x) {
                    break; /* is already */
                }
...
static int data_to_Z_matrix() {
    ...
    for (i = 0; i < dat.m_act; i++) {
        ...
        qsort((void *)&values[i * vmax], (size_t)n, sizeof(double), compare);
    }
    ...
    for (j = dat.l1, h = 0; j <= dat.l2; j++) {
        ...
        for (i = 0, p = 0; i < dat.m_act; i++) {
            ...
            bit = -1;
            m = n;
            if (values[i * vmax + n] == MV) {
                m--;
            }
            if (x == MISSING8) {
                if (m < n) {
                    bit = n - 1; /* missing class */
                }
            } else if (x >= values[i * vmax] && x <= values[i * vmax + m]) {
                for (k = 1; k <= m; k++) {
                    if (x <= values[i * vmax + k]) {
                        bit = k - 1;
                        break;
                    }
                }
...
static int burt_table(void) {
    ...
    muste_kv_s_disp("\nSaving Burt's table in %s (%d x %d)...", b_name, classes, classes);
    ...
    i = matrix_save(b_name, BURT, classes, classes, lab, lab, lablen, lablen, 10, nam, 0, 0);
    if (i < 0) {
        return -1;
    }
    return 1;
...
static int classifiers(void) {
    ...
    *lab = '\0';
    ...
    if (source == TABLE) {
        strcpy(longlabs, cnames);
        strcat(longlabs, rnames);
    ...
    for (i = 0; i < dim; i++) {
        CLASS[i] = (double)crsiz[i];
    }
    strcpy(cnames, "Classes ");
    if (ccol > 1) {
        for (i = 1 * dim, j = 0; i < 2 * dim; i++, j++) {
            CLASS[i] = (double)cscal[j];
        }
        strcat(cnames, "Scale   ");
        for (i = 2 * dim, j = 0; i < 3 * dim; i++, j++) {
            CLASS[i] = (double)suppl[j];
        }
        strcat(cnames, "Suppl   ");
    }
...
static int Z_matrix(void) {
    ...
    muste_kv_s_disp("\nSaving Z matrix to %s (%d x %d)...", z_name, N, classes);
