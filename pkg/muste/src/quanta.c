...
void muste_quanta(char *argv) {
    ...
    *p = EOS;
    ...
    if (i >= 0) {
        ...
        while (1) {
            ...
            if (y < y_min) {
                ...
                for (i = 0; i < dim; ++i) {
                    qmin[i] = q[i];
                    muste_sprintf(sbuf, "q[%d]=%g ", i + 1, q[i]);
                    sur_print(sbuf);
                }
            }
    ...
    for (jn = d.l1; jn <= d.l2; ++jn) {
        ...
        if (var_quant >= 0) {
            data_save(&d, jn, var_quant, (double)(imin + 1));
        }

        if (var_res >= 0) {
            data_save(&d, jn, var_res, min);
        }

        if (var_coeff >= 0) {
            data_save(&d, jn, var_coeff, (double)kmin);
        }
...
static int kendall() {
    ...
    muste_sprintf(sbuf, "Data: %s Variable: %s  N=%d", word[1], word[2], n);
    print_line(sbuf);
    print_line("GPLOT COSQUANT,quant,score / LINE=1 MODE=SVGA Plot the quantogram!");
