...
static char *methodname[] = {"",
                             ...
                             "Unweighted_centroid",
                             "Weighted_centroid",
                             "Minimum_variance_(Wards_method)"};
static char *distancename[] = {"", "Squared_Euclidian", "Euclidian", "City_Block", "Canberra_metric"};
...
void muste_hcluster(char *argv) {
    ...
    if (spec_init(r1 + r - 1) < 0) {
        return;
    }
    /* spec_init gives also an error message! */
    if (!ismatrix(word[1])) {
        if (data_open(word[1], &d) < 0) {
            return;
        }
        if (mask(&d) < 0) {
            return;
    ...
    if (ismatrix(word[1])) {
        w1mat = 1;
        if (matrix_load(word[1], &aa, &rdim, &cdim, &rlab, &clab, &lr, &lc, &type, expr) < 0) {
        ...
        if ((i = spfind("WEIGHTS")) >= 0) {
            ...
            i = matrix_load(weightmat, &aawei, &rdimwei, &cdimwei, &rlabwei, &clabwei, &lrwei, &lcwei, &typewei,
                            exprwei);
            if (i < 0) {
                muste_sprintf(exprwei, "Error in opening weightmatrix %s", weightmat);
                sur_print(exprwei);
                WAIT;
                return;
            }
            if ((rdimwei == m) && (cdimwei == 1)) {
                weights = 1;
            } else {
                weightwarning = 1;
            }
    ...
    if (savedist) {
        if (distfiletxt) {
            txt_save_distance_matrix();
        } else {
            if ((i = alloc_save_dist()) < 0) {
                ei_tilaa(7, i);
                return;
            }
...
static float dd(int method, int k, int r, int s) {
    ...
    case GROUP_AVERAGE:
        summa = nn[r] + nn[s];
        return ((float)nn[r] / summa * plats(k, r) + (float)nn[s] / summa * plats(k, s));

    case WEIGHTED_AVERAGE:
        return (.5 * plats(k, r) + .5 * plats(k, s));
    ...
    case WEIGHTED_CENTROID:
        return (.5 * plats(k, r) + .5 * plats(k, s) - .25 * plats(r, s));

    case MIN_VARIANCE:
        summa = nn[r] + nn[s] + nn[k];
        return ((float)(nn[r] + nn[k]) / summa * plats(k, r) + (float)(nn[s] + nn[k]) / summa * plats(k, s) -
...
static void do_cluster(void) {
    ...
    for (k = 1; k < n; k++) {
        ...
        for (i = ip + 1; i < n - k + 1; i++) {
            if (aktiva[i] != jmin) {
                (dist[aktiva[i]])[imin] = dd(met, aktiva[i], imin, jmin);
            }
        }
        for (j = 0; j < ip; j++) {
            if (aktiva[j] != jmin) {
                (dist[imin])[aktiva[j]] = dd(met, aktiva[j], imin, jmin);
            }
        }
        nn[imin] += nn[jmin];
...
static int tee_data(void) {
    ...
    for (l = 1, k = 1, gang = 1; gang < n; gang++) {
        while ((il[k] != 0 && !gjord[il[k]]) || (jl[k] != 0 && !gjord[jl[k]])) {
            while (il[k] != 0 && !gjord[il[k]]) {
                k = il[k];
            }
            while (jl[k] != 0 && !gjord[jl[k]]) {
                k = jl[k];
            }
        }
...
static void printout(void) {
    ...
    if (w1mat) {
        muste_sprintf(line,
                      ...
                      "of distance matrix %s N=%d",
                      word[1], n);
    } else {
        muste_sprintf(line,
                      "  Hierarchical cluster analysis "
                      "of observations in %s N=%d",
                      word[1], n);
    }
    print_line(line);
    ...
    if (warning_missing) {
        if (warning_missing == 1) {
            muste_sprintf(line,
                          "Note: %d observation is skipped due "
                          "to missing value.",
                          warning_missing);
            print_line(line);
        } else {
            muste_sprintf(line,
                          "Note: %d observations are skipped due "
                          "to missing values.",
                          warning_missing);
            print_line(line);
        }
    ...
    if (savedist) {
        if (distfiletxt) {
            muste_sprintf(line,
                          "  Distances saved in textfile "
                          "%s   '\376' SHOW %s",
                          distfile, distfile);
        } else {
            muste_sprintf(line,
                          "  Distances saved as "
                          "matrix %s   '\376' MAT LOAD %s,END+2",
                          distfile, distfile);
        }
        print_line(line);
        print_line("");
    ...
    if (results > 10) {
        ...
        for (k = 1; k < n; k++) {
            ...
            if (il[k] == 0) {
                muste_sprintf(line2, "%12.11s", (nimet + ii[k] * NIMIPIT));
            } else {
                muste_sprintf(line2, "   GROUP %-3d", il[k]);
            }
            strcat(line, line2);
            if (jl[k] == 0) {
                muste_sprintf(line2, "%12.11s", (nimet + jj[k] * NIMIPIT));
            } else {
                muste_sprintf(line2, "   GROUP %-3d", jl[k]);
            }
            strcat(line, line2);
    ...
    if (results > 20) {
        ...
        print_line(" | stopping|--------------+----+----+----+----|");
        muste_sprintf(line, " |   rule  | No of groups |%3d |%3d |%3d |%3d |", mojena(1.25), mojena(2.0), mojena(2.75),
                      mojena(3.5));
        print_line(line);
        print_line(" +--------------------------------------------+");
...
static void vika_samatnimet(int j, int k) {
    ...
    nimi2[NIMIPIT] = '\0';
    muste_sprintf(line, "\nWarning: same labels %s and %s", nimi1, nimi2);
    sur_print(line);
    WAIT;
}
...
static int metstring(char *t) {
    ...
    if (samaalku(t, "UNWEIGHTED_AVER")) {
        return 3;
    }
    if (samaalku(t, "WAV")) {
        return 4;
    }
    if (samaalku(t, "WEIGHTED_AVER")) {
        return 4;
    }
    if (samaalku(t, "CEN")) {
        return 5;
    }
    if (samaalku(t, "UNWEIGHTED_CENT")) {
        return 5;
    }
    if (samaalku(t, "WCE")) {
        return 6;
    }
    if (samaalku(t, "WEIGHTED_CENT")) {
        return 6;
    }
    if (samaalku(t, "MIN")) {
        return 7;
...
static void teepskaavio(void) {
    ...
    if (n > NRAJA) {
        font = MINFONT + (350.0 - n) / (350.0 - NRAJA) * (MAXFONT - MINFONT);
    }
    raja_rivi();
    muste_sprintf(line, "  Activate the line below to plot the tree to file %s.PS", treedata);
    print_line(line);
    muste_sprintf(line, " PLOT %s,X,Y", treedata);
    print_line(line);
    print_line("  Change YSCALE and XSCALE below to plot subtrees.");
    muste_sprintf(line,
                  "   YSCALE=1,%d     POINT=Label  FRAME=0 "
                  "XLABEL=<-_Distance",
                  n);
    print_line(line);
    if (landscape) {
        muste_sprintf(line,
                      "   XSCALE=-%-1g/1,0   PEN=[Landscape][Swiss(%d)] "
                      "MODE VGA YLABEL= ",
                      pyoristaylos(maxss), font - 2);
    } else {
        muste_sprintf(line,
                      "   XSCALE=-%-1g/1,0     PEN=[Swiss(%d)] "
                      "MODE VGA YLABEL= ",
                      pyoristaylos(maxss), font);
    }
    ...
    if (!landscape) {
        muste_sprintf(line, "XDIV=100,%d,100 YDIV=100,%d,200", xsize - 200, ysize - 300);
    } else {
        muste_sprintf(line, "XDIV=100,%d,200 YDIV=100,%d,100", xsize - 200, ysize - 300);
    }
    print_line("  Change LINE spec. below to 1 for a 'triangular' tree.");
    muste_sprintf(line2, "   LINE=3   %s", line);
    print_line(line2);
    muste_sprintf(line, "   HEADER=[Swiss(18)],%s;_Input:_%s", methodname[met], word[1]);
    print_line(line);
    muste_sprintf(line, "   DEVICE=PS,%s.PS  SIZE=%d,%d  HOME=%d,%d", treedata, xsize, ysize, xhome, yhome);
    print_line(line);
    raja_rivi();
}
...
static void teegplotkaavio(void) {
    ...
    print_line(line);
    muste_sprintf(line, "   HEADER=[Swiss(10)],%s;_Input:_%s", methodname[met], word[1]);
    print_line(line);
    raja_rivi();
}
