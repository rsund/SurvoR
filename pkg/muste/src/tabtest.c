...
void muste_tabtest(char *argv) {
    ...
    if (ltotal > MAXTOTAL) {
        muste_sprintf(sbuf, "\nTotal frequency %d too great! (max=%d)", ltotal, MAXTOTAL);
        WAIT;
        return;
    }
    total = ltotal;
    ...
    if (i >= 0) {
        ...
        if (strncmp(s, "PR", 2) == 0) {
            test = PROB;
        } else if (strncmp(s, "X2", 2) == 0 || strncmp(s, "X^2", 3) == 0) {
            test = X2;
        } else if (strncmp(s, "G2", 2) == 0 || strncmp(s, "G^2", 3) == 0) {
            test = G2;
        } else {
...
static int cumul_freq() {
    ...
    for (j = 0; j < n; ++j) {
        s += f_c[j];
        cum_c[j] = (double)s / (double)total;
    }
    s = 0;
    for (i = 0; i < m; ++i) {
        s += f_r[i];
        cum_r[i] = (double)s / (double)total;
    }
    return (1);
}
...
static int find_fix() {
    ...
    if (strncmp(x, "FI", 2) == 0) {
        fix = FISHER;
    } else if (strcmp(x, "RC") == 0 || strcmp(x, "CR") == 0) {
        fix = FIX_RC;
    ...
    if (i >= 0) {
        ...
        *q = EOS;
        ...
        if (strcmp(x, "RAND") == 0) {
            ...
            rand_seed1 = sur_rand_seed;
            rand1 = sur_rand;
        } else if (strcmp(x, "URAND") == 0) {
            generator = URAND;
            rand_seed1 = sur_urand_seed;
            rand1 = sur_urand;
        } else {
            rand_error(spb[i]);
            return (-1);
...
static int disp0() {
    ...
    muste_sprintf(sbuf + k, "  X^2=%g P=%6f", chi2 + eps,
                  1 - muste_cdf_chi2(chi2 + eps, (double)((m - 1) * (n - 1)), 1e-15));
    (*g_print)(sbuf);
    LOCATE(r + 3, 9);
    muste_sprintf(sbuf, "%.*s  G^2=%g P=%6f", k, space, g2 + eps,
                  1 - muste_cdf_chi2(g2 + eps, (double)((m - 1) * (n - 1)), 1e-15));
    (*g_print)(sbuf);
    LOCATE(r + 4, 9);
...
static int tab_disp() {
    ...
    lower = p1 - conf_coeff * se;
    if (lower < 0.0) {
        lower = 0.0;
    }
    upper = p1 + conf_coeff * se;
    if (upper > 1.0) {
        upper = 1.0;
    }
    muste_sprintf(sbuf, "%10d %.8f %.8f lower limit", count, p1, lower);
    (*g_print)(sbuf);
    LOCATE(r + 7, 9);
...
static int disp0fit(int m, int n, double chi2) {
    ...
    (*g_print)(sbuf);
    LOCATE(r + 3, 9);
    muste_sprintf(sbuf, "Common Chi-squared test:  X^2=%g P=%6f", chi2 + eps,
                  1 - muste_cdf_chi2(chi2 + eps, (double)(m - 1), 1e-15));
    (*g_print)(sbuf);
