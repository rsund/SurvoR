...
SEXP Muste_EvalRExpr(char *cmd) {
    ...
    apu2 = strchr(cmd, ' ');
    apu3 = strchr(cmd, '<');
    if ((apu2 != NULL && apu3 != NULL && (apu3 - cmd) < (apu2 - cmd)) || (apu2 == NULL)) {
        apu2 = apu3;
    }
    if (strncmp(cmd, ".muste.", 7) == 0 &&
        (apu != NULL && (apu2 == NULL || (apu2 != NULL && (apu - cmd) < (apu2 - cmd))))) {
        muste_sprintf(komento, "if (inherits(try(.muste$ans<-muste:::%s,silent=FALSE), \"try-error\")) FALSE else TRUE",
                      cmd);
    } else {
...
int muste_evalsource_output(char *sfile, char *rout) {
    ...
    if (*rout != EOS) {
        muste_sprintf(cmd, ".muste.runsource(\"%s\",dest=\"%s\")", out, rout);
    } else {
        muste_sprintf(cmd, ".muste.runsource(\"%s\")", out);
    }
...
int muste_system(char *incmd, int odotus) {
    ...
    */
    ...
    if (strncmp(y, "DIR", 3) == 0 || strncmp(y, "dir", 3) == 0 || strncmp(y, "LS", 2) == 0 ||
        strncmp(y, "ls", 2) == 0) {
        if (strchr(y, ' ') == NULL) {
            clip = tyhja;
        } else {
        ...
        if (odotus) {
            snprintf(cmd, LLENGTH, "muste:::.muste.dir('%s',TRUE)", clip);
        } else {
            muste_snprintf(cmd, LLENGTH, "muste:::.muste.dir('%s',FALSE)", clip);
        }
        //		if (wait) muste_sprintf(cmd,"muste:::.muste.dir(\"%s\",TRUE)",clip);
        //		else muste_sprintf(cmd,"muste:::.muste.dir(\"%s\",FALSE)",clip);
    } else {
        if (odotus == 1) {
            muste_snprintf(cmd, LLENGTH, "muste:::.muste.system('%s',TRUE)", y);
        } else if (odotus == 2) {
            strcpy(x, y);
            len = splitq(x, osat, 5);
            muste_snprintf(cmd, LLENGTH, "muste:::.muste.systemopen('%s',FALSE,%d)", y, len); // RS 25.11.2012
        } else {
            muste_snprintf(cmd, LLENGTH, "muste:::.muste.system('%s',FALSE)", y);
        }
...
int muste_requirepackage(char *package) {
    ...
    if (vast == FALSE) {
        ...
        muste_evalr(cmd);
        snprintf(cmd, LLENGTH,
                 "if (!require(%s)) { install.packages(\"%s\",contriburl=\"http://www.survo.fi/muste\") }", package,
                 package);
    }
...
int muste_get_R_string_vec(char *dest, char *sour, int length, int element) {
    ...
    if (muste_is_utf8_string(enc)) {
        muste_iconv(hakubuf, "CP850", "UTF-8");
    } else {
        muste_iconv(hakubuf, "CP850", "");
    }
...
int muste_get_R_string(char *dest, char *sour, int length) { return (muste_get_R_string_vec(dest, sour, length, 0)); }
...
int muste_get_R_int(char *sour) { return (muste_get_R_int_vec(sour, 0)); }
...
double muste_get_R_real(char *sour) { return (muste_get_R_real_vec(sour, 0)); }
...
int muste_theme(int classic) {
    if (classic) {
        muste_evalr(".muste.shadows(\"#FFFEFF\",\"#FFFEFF\")");
    } else {
        muste_evalr(".muste.shadows(\"white\",\"#F8F8F8\")");
    }
    return (1);
}
...
int muste_statusbar(int basic, int shadow) {
    ...
    */
    if (basic) {
        ...
        muste_evalr(str1);
        strcpy(cmd, edisk);
        unsubst_survo_path_in_editor(cmd);
        ...
        if (insert_type && insert_mode) {
            muste_sprintf(
                str1,
                "tkconfigure(.muste$statbarl4,text=\"Ins\",background=.muste$insertcursorcolor,foreground=\"white\")");
        } else {
            muste_sprintf(str1, "tkconfigure(.muste$statbarl4,text=\"Ins\",background=\"white\",foreground=\"white\")");
        }
        ...
        switch (dispm) {
        case 1:
            muste_sprintf(str1, "tkconfigure(.muste$statbarl0,text=\"1\",background=\"white\",foreground=\"red\")");
            break;
        case 2:
            muste_sprintf(str1,
                          "tkconfigure(.muste$statbarl0,text=\"2\",background=\"white\",foreground=\"darkgrey\")");
            break;
        case 3:
            muste_sprintf(str1, "tkconfigure(.muste$statbarl0,text=\"3\",background=\"white\",foreground=\"blue\")");
            break;
        case 4:
            muste_sprintf(str1, "tkconfigure(.muste$statbarl0,text=\"4\",background=\"darkblue\",foreground=\"grey\")");
            break;
        case 5:
            muste_sprintf(str1, "tkconfigure(.muste$statbarl0,text=\"5\",background=\"yellow\",foreground=\"black\")");
            break;
        case 6:
            muste_sprintf(str1,
                          "tkconfigure(.muste$statbarl0,text=\"6\",background=\"white\",foreground=\"forest green\")");
            break;
        case 7:
            muste_sprintf(str1, "tkconfigure(.muste$statbarl0,text=\"7\",background=\"blue\",foreground=\"white\")");
            break;
        case 8:
            muste_sprintf(str1,
                          "tkconfigure(.muste$statbarl0,text=\"8\",background=\"darkblue\",foreground=\"yellow\")");
            break;
        case 9:
            muste_sprintf(str1,
                          "tkconfigure(.muste$statbarl0,text=\"9\",background=\"white\",foreground=\"darkgrey\")");
            break;
        default:
            muste_sprintf(str1, "tkconfigure(.muste$statbarl0,text=\"0\",background=\"white\",foreground=\"black\")");
            break;
        }
        muste_evalr(str1);
    }
...
SEXP Muste_Command(SEXP para) // RS EDT 19.9.2012
{
    ...
    if (strcmp(kojo, "SaveEdt") == 0) {
        ...
        if (i < 0) {
            muste_set_R_int(".muste$saverror", 1);
        }
        return (para);
    }
    ...
    if (strcmp(kojo, "GetSaveName") == 0) {
        ...
        if (g > 1) {
            if (strcmp(parm[0], "SAVE") == 0) {
                muste_set_R_string(".muste$savename", parm[1]);
            }
        }
        return (para);
    }
    ...
    muste_sprintf(cmd,"qq <- q");
        muste_evalr(cmd);
    muste_sprintf(cmd,"quit <- q <- function(save = \"default\", status = 0, runLast = TRUE) { if
    (exists(\"editor\",where=.muste)) muste:::.muste.end(); else qq(save,status,runLast); }"); muste_evalr(cmd);
        return(para);
        }
    */
...
SEXP Muste_Selection(SEXP session) {
    ...
    if (muste_get_R_int(".muste$selection.alt")) {
        mc2 = c2;
        if (mc1 < 1) {
            mc1 = 0;
        } else {
...
SEXP Muste_Edtgoto(SEXP gotoparm) {
    ...
    */
    if (muste_mousewheel == FALSE || muste_no_selection) {
        return (gotoparm);
    }
    ...
    muste_sprintf(eka, "%d", newfirst);
    gprm[1] = eka;
    muste_sprintf(toka, "%d", newcur);
    gprm[2] = toka;

    newfirst = muste_get_R_int(".muste$edtx.newfirst");
    newcur = muste_get_R_int(".muste$edtx.newcur");

    muste_sprintf(kolmas, "%d", newfirst);
    gprm[3] = kolmas;
    muste_sprintf(neljas, "%d", newcur);
    gprm[4] = neljas;
...
You must call it on the main thread and you should be prepared that it may take some time and may interact with the
OS...
...
#define muste_resource_allocation(func, ptr) MUSTE_REAL((muste_func_t *)(func), (ptr))
...
void muste_initstack() {
    ...
    muste_stack[0].all = 0;
    muste_stack_count = 0;
    for (i = 0; i < MUSTESTACKSIZE; i++) {
        muste_stackdepth[i] = 0;
    }
}
...
int muste_restore_stack_count() { return (muste_restore_stack_count_manual(0)); }

int muste_free(void *p) {
    int no;
    for (no = muste_stack[0].all - 1; no >= 0; no--) {
        if (muste_stack[no].ptr == p)
            muste_stack[no].ptr = NULL;
    }
    if (p != NULL) {
...
FILE *muste_fopen(char *path, char *mode) {
    ...
    if (strncmp(path, "http://", 7) == 0 || strncmp(path, "https://", 8) == 0 ||
        strncmp(path, "ftp://", 6) == 0) // RS 15.1.2013
    {
...
size_t muste_fread_impl(void *ptr, size_t size, size_t nmemb, FILE *stream, const char *srcfile, int srcline) {
    size_t n = fread(ptr, size, nmemb, stream);

    if (n != nmemb) {
        if (feof(stream)) {
            Rprintf("%s:%d: short read (EOF): requested=%zu items, read=%zu\n", srcfile, srcline, nmemb, n);
...
char *muste_fgets_impl(char *s, int size, FILE *stream, const char *srcfile, int srcline) {
...
int muste_fscanf_impl(FILE *stream, const char *format, const char *srcfile, int srcline, ...) {
    va_list ap;
    va_start(ap, srcline);
