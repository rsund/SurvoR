...
static void muste_mntest2(char *argv) {
    ...
    m_act = d.m_act;
    if (m_act < 1) {
        sur_print("\nAt least 1 variable must be active!");
        WAIT;
        return;
    }
    ...
    if (i >= 0) {
        strcpy(x, spb[i]);
        i = split(x, osa, 3);
        if (muste_strcmpi(osa[0], "MARDIA") == 0) {
            test = MARDIA;
...
static int talleta_data() {
    ...
    for (j = d.l1; j <= d.l2; ++j) {
        ...
        if (i < m_act) {
            muste_sprintf(sbuf, "Value of variable %s missing in observation #%d!", d.varname[i], j);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
...
static int principal_comp() {
    ...
    for (j = 0L; j < n; ++j) {
        ...
        muste_fseek(temp, posx, SEEK_SET);
        muste_fread(xx, sizeof(double), m_act, temp);
        ...
        if (test == CUBE) {
            ...
            if (ks_stat) {
                fb = b;
                fwrite(&fb, sizeof(float), 1, temp2);
            }
            k = nf * b;
            ++fr1[k];
        ...
        if (test != CUBE) {
            muste_fseek(temp, posy, SEEK_SET);
            fwrite(yy, sizeof(double), dim, temp);
...
static int tunnusluvut() {
    ...
    for (j = 0L; j < n; ++j) {
        if (!ind_zz) {
            muste_fseek(temp, j * (int)dim * (int)sizeof(double), SEEK_SET);
        }
        lue(xx, j);
        if (prind) {
            muste_sprintf(sbuf, "%d ", j + 1);
