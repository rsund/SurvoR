...
static int luo_uusi() {
    ...
    if (expand) // 29.12.2003
    {
        if (expm == -9999) {
            ...
            expm = d1.d2.m - d1.d2.m1; // minimoi
        } else if (expm < d1.d2.m - d1.d2.m1) {
            expm = d1.d2.m - d1.d2.m1;
        }
    ...
    if (d1.type == 2) {
        ...
        // Rprintf("\ndelete=%d|",i); getch();
        i = fi_create(word3, filen, fim1, fim, 0L, fil, fiextra, fitextn, fitextlen, fitext, uvarname, uvarlen,
                      uvartype);
        if (i < 0) {
            return (-1);
        }
        ...
        fitextn = 1;
        fitextlen = c2;
        strcpy(jakso, " Copy of data matrix ");
        strcat(jakso, word[2]);
        privi[0] = jakso;
        fitext = privi;
    ...
    } else /* d1.type==3 */
    {
        ...
        */
        ...
        for (i = 0; i < fim; ++i) {
            uvarlen[i] = d1.varlen[i];
        }
        i = fi_create(word3, filen, fim1, fim, 0L, fil, fiextra, fitextn, fitextlen, fitext, d1.varname, uvarlen,
                      d1.vartype);
        if (i < 0) {
            return (-1);
        }
...
static int tutki_muuttujat() {
    ...
    //		i=conditions(&d1);
    for (i = 0; i < d1.m_act; ++i) {
        muste_fieldcopy(nimi, d1.varname[d1.v[i]], 8);
        nimi[8] = EOS;
...
static int kopioi(long j, long j2, int i) {
    ...
    vi = d1.v[i];
    type1 = d1.vartype[vi][0];
    type2 = d2.vartype[v2[i]][0];
...
static int match_copy2() {
    ...
    for (k = 0; k < n_match; ++k) {
        ...
        muste_fieldcopy(m_name[k], d1.varname[m_var[k]], 8);
        m_name[k][8] = EOS;
    ...
    if (i >= 0) {
        ...
        if (i >= 0) {
            ...
            if (muste_strcmpi(osa[0], "NUL") != 0) {
                ...
                if (odd_var < 0) {
                    muste_sprintf(sbuf, "\nField %s in %s for odd cases not found!", osa[0], word[2]);
                    sur_print(sbuf);
                    WAIT;
                    return (-1);
                }
    ...
    for (j = d1.l1; j <= d1.l2; ++j) {
        ...
        while (1) {
            ...
            if (j2 > d2.n) {
                ...
                {
                    //                        muste_sprintf(sbuf,"\nODD:%s ",m_jakso[k]); sur_print(sbuf); //
                    //                        RS 21.12.2012 jakso -> m_jakso[k]
                    if (prind) // RS 21.12.2012 jakso -> m_jakso[k]
                    {
                        muste_sprintf(sbuf, "\nODD:%s", m_jakso[0]);
                        sur_print(sbuf);
...
void muste_file_copy(int argc, char *argv[]) {
    ...
    *mx1 = 0;
    ...
    if (muste_strcmpi(word[1], "EXPAND") == 0) {
        expand = 1;
        muste_expand = 1;
    }
    ...
    if (expand) {
        ...
        strcpy(sbuf, word[2]);
        h = strlen(sbuf) - 1;
        while (h >= 0 && sbuf[h] != ':' && sbuf[h] != '\\' && sbuf[h] != '/') {
            sbuf[h--] = EOS; // RS FIXME unix path
        }
        ...
        if (!muste_is_path(tempn)) {
            strcpy(tempn, edisk);
            strcat(tempn, "SURVO.TMP");
        } // RS 14.8.2014 sbuf -> "SURVO.TMP"
    ...
    {
        if (muste_strcmpi(word[3], "TO") == 0 && *word[4] == 'R' && *(word[4] + 1) == '>' && !expand) { // RS ADD
            nimi = (word[4] + 2);
        ...
        if (g > 5) {
            if (strcmp(muste_strupr(word[3]), "NEW") == 0) {
                new_file = 1;
            }
            word[3] = word[5];
        }
    ...
    **********************************************/
    ...
    if (new_file) // 23.2.2004
    {
        strcpy(sbuf, word3);
        if (strchr(sbuf, '.') == NULL) {
            strcat(sbuf, ".SVO"); // RS FIXME Not working with relative paths (any paths with dot)
        }
        sur_delete(sbuf);
    }
    ...
    if (i != 3) {
        ...
        if (i < 0) {
            s_end(argv[1]);
            return;
        } // RS CHA Need to be before luo_uusi()
    }
    ...
    for (j = d1.l1; j <= d1.l2; ++j) {
        if (survo_ferror) {
            ...
            s_end(argv[1]);
            return;
        } // RS CHA exit(0) -> s_end return
        if (!expand && unsuitable(&d1, j)) {
            continue;
        }
        if (sur_kbhit()) {
            i = sur_getch();
    ...
    if (expand) {
        ...
        muste_append_path(tempn1,
                          ".SVO"); // RS CHA if (strchr(tempn1+strlen(tempn1)-4,'.')==NULL) strcat(tempn1,".SVO");
