...
static int talletus() {
    ...
    muste_sprintf(sbuf, "\nSaving aggregated data in %s...", word[3]);
    sur_print(sbuf);
    for (j = 1L; j <= aggn; ++j) {
        if (prind) {
            muste_sprintf(sbuf, " %d", (int)j);
            sur_print(sbuf);
        }
        if (sur_kbhit()) {
            sur_getch();
        ...
        if (aggtype == 'S') {
            muste_fieldcopy(label, aspace + (j - 1) * (long)aggrec, agglen);
            label[agglen] = EOS;
    ...
    for (j = d.l1; j <= d.l2; ++j) {
        ...
        if (aggtype) {
            ...
            } else /* uusi tapaus */
            {
                ...
                if (aggtype == 'S') {
                    muste_fieldcopy(hp, aggs, agglen);
                } else {
                    *(double *)hp = aggv;
                }
                if (weight != 1.0) {
...
static int aggvars() {
    ...
    m = 0;
    for (i = 0; i < d.m_act; ++i) {
        if (d.v[i] != aggvar && d.vartype[d.v[i]][1] == 'A') {
            v[m++] = d.v[i];
        }
    }
    /* 11.9.91 */
...
void muste_file_aggre(int argc, char *argv[]) {
    ...
    i = split(x, osa, 2);
    if (i == 0) {
        sur_print("\nError in AGGRE specification!");
        WAIT;
        return;
    }
    aggvar = varfind2(&d, osa[0], 0);
    if (aggvar >= 0) {
        aggtype = d.vartype[aggvar][0];
    } else {
        aggtype = EOS;
    }
    if (aggvar < 0) {
    ...
    aggmode = 0; /* "SUM" */
    if (i > 1) {
        if (muste_strcmpi(osa[1], "MEAN") == 0) {
            aggmode = 1;
        } else if (muste_strcmpi(osa[1], "SUM") == 0) {
            aggmode = 0;
        } else {
