...
static int sijoita() {
    ...
    if (eka) {
        len = lrX;
        if (d.varlen != NULL && len > d.varlen[d.v[0]]) {
            len = d.varlen[d.v[0]];
        }
    } else {
        numlab(rlabX, m, lrX);
    ...
    for (l = d.l1; l <= d.l2; ++l) {
        ...
        if (i > m - 1) {
            break; /* lopussa olevien puutt.hav.sivuutus */
        }
        if (eka) {
...
void muste_matsda(int argc, char *argv[]) {
    ...
    if (d.vartype[d.v[0]][0] == 'S' && !rlabels) {
        eka = 1;
    } else {
        eka = 0;
    }
...
static int not_enough_memory() {
    sur_print("\nNot enough memory (FILE SAVE MAT)!");
    WAIT;
    return (1);
}
    ...
    for (k = first - 1; k < last; ++k) {
        ...
        if (match >= 0) {
            ...
            while (1) {
                ...
                if (strcmp(sana, jakso) == 0) {
                    break;
                }
                ++j;
                if (j > d.l2) {
                    muste_sprintf(sbuf, "\nCase %s not found in data file %s!", sana, word[5]);
                    sur_print(sbuf);
                    WAIT;
                    return (-1);
                }
...
static int luo_uusi() {
    ...
    fitextn = 1;
    fitextlen = c2;
    strcpy(jakso, " Copied from matrix file ");
    strcat(jakso, word[3]);
    privi[0] = jakso;
    fitext = privi;
    i = fi_create(word[5], filen, fim1, fim, 0L, fil, fiextra, fitextn, fitextlen, fitext, varname, varlen, pvartype);
    if (i < 0) {
        return (-1);
    }
