...
int soft_stack_save_load(int k, char *name) {
    ...
    if (k == 1) {
        ...
        if (soft_loadsave_temp_file == NULL) {
            tutstack_error(x, 1);
            return (-1);
        }
        p = soft_info;
        while (*p) {
            putc((int)(*p), soft_loadsave_temp_file);
            ++p;
        }
    ...
    p = soft_info;
    while (!feof(soft_loadsave_temp_file)) {
        *p = (char)getc(soft_loadsave_temp_file);
        ++p;
    }
    *(p - 1) = EOS;
    muste_fclose(soft_loadsave_temp_file);
...
int soft_keys_set(char *s[]) {
    ...
    if (strcmp(s[0], "*") == 0) {
        ...
        strcpy(soft_edit_file, s[0]);
    }

    strcpy(nimi, s[0]);

    if (!muste_is_path(nimi) && !netd(nimi)) {
    ...
    if (*t[r_soft - 1] == '*') // soft_stack_file name
    {
        ...
        if (strchr(soft_stack_file, '.') == NULL) {
            strcat(soft_stack_file, ".STK");
        }
    } else {
        *soft_stack_file = EOS;
    }
    ...
    for (i = 0; i < r_soft; i++) {
        ...
        while (1) {
            ...
            k = split(y, t2, 3);
            if (strncmp(t2[0], "END", 3) == 0) {
                break; // RS CHA strcmp -> strncmp ,3  (unix \r fix)
            }
            ch = *t2[0];
            soft_key_act[h] = '-';
            if (k > 1) {
            ...
            if (k > 2) {
                strcpy(soft_key_command[h], y2 + (t2[2] - y));
            }
...
int show_items_on_header_line(int cc) {
    ...
    for (h = 0; h < n_soft_keys; ++h) {
        ...
        write_string(p, strlen(p), shadow, r3 + 2 + r_soft + 1, 1);
        break;
    }
...
int soft_prompt(char *vastaus, int pituus, int pos) {
    ...
    muste_sprintf(sbuf, "%.*s", pituus, tila);
    PR_EBLK;
    sur_print(sbuf);
    ...
    while (1) {
        ...
        case CODE_RIGHT:
            ...
            break;
        case CODE_DELETE:
            for (i = pos - 1; i < pituus - 1; ++i) {
                tila[i] = tila[i + 1];
            }
            tila[pituus - 1] = ' ';
            sur_cursor_position(&row, &col);
            muste_sprintf(sbuf, "%s", tila + pos - 1);
            PR_EBLK;
            sur_print(sbuf);
            sur_locate(row, col);
            break;
        ...
        default:
            if (insert_mode) {
                if (pos < pituus && tila[pituus - 1] != ' ') {
                    break;
                }
                for (i = pituus - 2; i >= pos - 1; --i) {
                    tila[i + 1] = tila[i];
                }
                tila[pos - 1] = (char)m;
                sur_cursor_position(&row, &col);
...
int soft_key_task(int h, int m_click, int m_dbl) {
    ...
    case 'I':
        if (strcmp(command, "PREV") == 0) {
            ...
            if (*soft_list_name[soft_menu_n - 1] == '-') {
                break;
            }
            k = soft_menu_n - 1;
            while (soft_menu_n > 1) {
    ...
    case 'P':
        ...
        while (1) {
            ...
            strncat(xx, space, k - i);
            row1 = r3 + 2 + soft_key_line[h];
            col1 = soft_key_start[h] - 8;
            cursor(row1, col1);
            write_string(soft_key_text[h], strlen(soft_key_text[h]), ' ', r3 + 2 + r_soft + 1, 1);
            ...
            PR_ENRM;
            sur_print(xx);
            muste_sprintf(sbuf, "%.*s", (int)(k - strlen(xx)), space);
            sur_print(sbuf);
            cursor(r, c);
...
int soft_key_activate(int rr, int cc, int m_click, int m_dbl) {
    ...
    if (rr == 0) {
        show_items_on_header_line(cc);
        return (1);
    }
    if (rr < r3 + 2 || rr > r3 + r_soft + 1) {
        return (1);
    }
    ...
    for (h = 0; h < n_soft_keys; ++h) {
        ...
        if (m_click || m_dbl) {
            return (soft_key_task(h, m_click, m_dbl));
        }
        ...
        p_soft_key_text = soft_key_text[hh];
        soft_current_key_text_shadow = shadow;
        soft_message = 1;
        break;
    }
