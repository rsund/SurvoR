...
static int read_data() {
    ...
    if (maxgap) {
        ...
        if (gaplen > (unsigned int)(maxgap - 1)) {
            gaplen = maxgap - 1;
        }
        ++gap[(int)gaplen];
    }
...
static int freq() {
    ...
    i = (x - fr_a) / fr_d;
    if (i < 0 || i > fr_n - 1) {
        muste_sprintf(sbuf, "\nValue %g not in permitted range (%g,%g) in frequency test!", x, fr_a,
                      fr_a + fr_n * fr_d);
        fr_n = 0;
        WAIT;
        return (-1);
...
static int lags() {
    ...
    for (i = 0; i < maxlag; ++i) {
        ...
        if (lagpos >= i) {
            lagvv[i] += x * lagv[lagpos - i];
        } else {
            lagvv[i] += x * lagv[maxlag - i + lagpos];
        }
    }
    ++lagpos;
    if (lagpos == maxlag) {
...
static int poker() {
    ...
    i = (x - pok_a) / pok_d;
    if (i < 0 || i > n_pok - 1) {
        muste_sprintf(sbuf, "\nValue %g not in permitted range (%g,%g) in Poker test!", x, pok_a,
                      pok_a + n_pok * pok_d);
        poklen = 0;
        WAIT;
        return (-1);
...
static int coupon() {
    ...
    i = (x - coup_a) / coup_d;
    if (i < 0 || i > couplen - 1) {
        muste_sprintf(sbuf, "\nValue %g not in permitted range (%g,%g) in Coupon collector's test!", x, coup_a,
                      coup_a + couplen * coup_d);
        couplen = 0;
        WAIT;
        return (-1);
    }
...
static int rnd_printout() {
    ...
    print_line(sbuf);
    fnconv2(mean, accuracy + 2, s1);
    fnconv2(stddev, accuracy + 2, s2);
    fnconv2(stddev / sqrt((double)n), accuracy + 2, s3);
    muste_sprintf(sbuf, "N=%d mean=%s stddev=%s SE[mean]=%s", n, s1, s2, s3);
    print_line(sbuf);
    if (stddev < 1e-15) {
        muste_sprintf(sbuf, "\nVariable %s is constant=%g", word[2], mean);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
    ...
    for (imax = MAXRUN - 1; imax >= 0; --imax) {
        if (runs_up[imax] + runs_down[imax] > 0) {
            break;
        }
    }
    runs_hald();
    ...
    if (maxlag) {
        ...
        for (i = 0; i < maxlag; ++i) {
            ...
            a1 = sumx2 - sumx * sumx / df;
            a2 = sumy2 - sumy * sumy / df;
            a2 = a / sqrt(a1 * a2);
            if (fabs(a2) > amax) {
                amax = fabs(a2);
            }
            ...
            */
            ...
            if (i == maxlag - 1) {
                fnconv2(amax, accuracy, s1);
                k += muste_sprintf(sbuf + k, "  max.autocorr.=%s", s1);
            }
            print_line(sbuf);
        }
    }
...
static int sub_results() {
    ...
    print_line(" ");
    muste_sprintf(sbuf, "Subsample statistics: samplesize=%u Number of samples =%u", n_sub, n1);
    print_line(sbuf);
    print_line("P values in repeated tests should be uniformly distributed over (0,1)");
    fnconv2(e_sub, accuracy, s1);
    muste_sprintf(sbuf, " P (up.limit)  Mean         Min           Max   (Expected=%s)", s1);
    print_line(sbuf);
    for (i = 0; i < n_subclass; ++i) {
        muste_sprintf(sbuf, "%5.2f    %8u      %8u      %8u", (double)(i + 1) / (double)n_subclass, p_mean[i], p_min[i],
                      p_max[i]);
        print_line(sbuf);
    }
    ...
    muste_sprintf(sbuf, "Tests for mean=0.5: Chi2=%s df=%d P[submean]=%s", s1, n_subclass - 1, s2);
    print_line(sbuf);
    chi2_comp(n_subclass, p_min, &chi2, &pval);
    fnconv2(chi2, accuracy, s1);
    fnconv2(pval, accuracy, s2);
    muste_sprintf(sbuf, "Tests for min.values: Chi2=%s df=%d P[submin]=%s", s1, n_subclass - 1, s2);
    print_line(sbuf);
    chi2_comp(n_subclass, p_max, &chi2, &pval);
    fnconv2(chi2, accuracy, s1);
    fnconv2(pval, accuracy, s2);
    muste_sprintf(sbuf, "Tests for max.values: Chi2=%s df=%d P[submax]=%s", s1, n_subclass - 1, s2);
    print_line(sbuf);
    return (1);
}
...
static int fr_results() {
    ...
    print_line(" ");
    muste_sprintf(sbuf, "Frequency test: N[class]=%d Interval (%g,%g)", fr_n, fr_a, fr_a + fr_n * fr_d);
    print_line(sbuf);
    chi2 = 0.0;
    min = max = fr_f[0];
    e = (double)n / (double)fr_n;
    for (i = 0; i < fr_n; ++i) {
        f = fr_f[i];
        if (f < min) {
            min = f;
    ...
    muste_sprintf(sbuf, "Expected=%s min=%u max=%u chi2=%s df=%d P[freq]=%s", s1, min, max, s2, fr_n - 1, s3);
    print_line(sbuf);
    return (1);
}
...
static int runs_hald() {
    ...
    print_line(" ");
    muste_sprintf(sbuf, "Runs up and down (Hald 1952):");
    print_line(sbuf);
    ...
    muste_sprintf(sbuf, "Total number of runs R=%d E[R]=%g S[R]=%g", rtot, (double)((2 * n - 1) / 3),
                  sqrt((double)((16 * n - 29) / 90)));
    print_line(sbuf);
    return (1);
}

double aa[] = {4529.4,  9044.9,   13568.0,  18091.0, 22615.0, 27892.0,  9044.9,   18097.0,  27139.0,
               36187.0, 45234.0,  55789.0,  13568.0, 27139.0, 40721.0,  54281.0,  67852.0,  83685.0,
               18091.0, 36187.0,  54281.0,  72414.0, 90470.0, 111580.0, 22615.0,  45234.0,  67852.0,
               90470.0, 113262.0, 139476.0, 27892.0, 55789.0, 83685.0,  111580.0, 139476.0, 172860.0};
...
static int runtest2() {
    ...
    muste_sprintf(sbuf, "Runs up and down (Levene and Wolfowitz 1944):");
    print_line(sbuf);
    ...
    for (i = 0; i <= 5; ++i) {
        ...
        a = (double)(n + 1) * (double)ii / ((double)(ii + 1) * fact) - (double)(ii - 1) / fact -
            (double)(n + 1) * (double)(ii + 1) / ((double)(ii + 2) * (double)(ii + 1) * fact) -
            (double)(ii) / fact / (double)(ii + 1);
        muste_sprintf(sbuf, "%6d  %8d %8d          %g", i + 1, up2[i], dn2[i], a);
        print_line(sbuf);
    }
    muste_sprintf(sbuf, "Tests: V[up]=%g P[up]=%g    V[down]=%g P[down]=%g", v_up, 1 - muste_cdf_chi2(v_up, 6.0, 1e-15),
                  v_dn, 1 - muste_cdf_chi2(v_dn, 6.0, 2e-15));
    print_line(sbuf);
    if (n < 4000L) {
...
static int gap_results() {
    ...
    print_line(" ");
    muste_sprintf(sbuf, "Gap test: P(%s,%s)=%s", s1, s2, s3);
    print_line(sbuf);
    ...
    for (i = 0; i <= max; ++i) {
        ...
        schi2 += chi2;
        ++df;
        fnconv(e, accuracy, s1);
        fnconv(chi2, accuracy, s2);
        muste_sprintf(sbuf, "%6d  %8u      %s      %s", i, gap[i], s1, s2);
        if (stop) {
            sbuf[7] = '-';
        }
        print_line(sbuf);
...
static int perm_results() {
    ...
    muste_sprintf(sbuf, "Permutation test: Length=%d Chi2=%s df=%u P[perm]=%s", permlen, s1, n_perm - 1, s2);
    print_line(sbuf);
    return (1);
}
...
static int coup_results() {
    ...
    b = 1.0;
    for (r = 2; r < couplen; ++r) {
        b *= (double)r / (double)couplen;
    }
    nc = 0L;
    for (i = couplen - 1; i < coup_max; ++i) {
        nc += f_coup[i];
    }
