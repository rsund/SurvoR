...
static int lue() {
    ...
    for (l = d.l1; l <= d.l2; ++l) {
        ...
        if (xarvo == MISSING8 || yarvo == MISSING8) {
            continue;
        }
        xx[n1] = xarvo;
        yy[n1] = yarvo;
...
static void f_nearest(int p[], double a[], int l, int *m1, int *m2, int n, int fn) {
    ...
    for (i = 0; i < fn - 1; i++) {
        if (l1 <= 0) {
            ...
            l1 -= 1;
        } else if (*(a + p[l]) - *(a + p[l1 - 1]) >= *(a + p[l2 + 1]) - *(a + p[l])) {
            l2 += 1;
        } else {
            l1 -= 1;
        }
...
static void talletus() {
    ...
    for (l = 0; l < n1; l++, i += 1) {
        kl = l;
        if (prind) {
            muste_sprintf(sbuf, " %d", ihav[kl] + 1);
            sur_print(sbuf);
        }
        if (s_variable >= 0) {
...
static int talleta2() {
    ...
    strcpy(clab, "X       Y       S       ");
    muste_sprintf(mnimi, "DATA %s; n=%d; X-var=%s; Y-var=%s", word[1], n1, d.varname[x_variable],
                  d.varname[y_variable]);
    for (cp = mnimi; *cp; cp++) {
        if (*cp == ' ') {
            *cp = '_';
        }
