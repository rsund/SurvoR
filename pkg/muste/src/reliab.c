...
static int allocate_memory(void) {
    ...
    if (NO_RFACT) { /* create identity matrix */
        ...
        if (chsum != nF) {
            orthogonal = 0; /* not an identity matrix */
        }
    }
    if (orthogonal) {
        for (i = 0; i < mX; i++) {
...
static int reliabilities(void) {
    ...
    for (i = 0; i < mX; i++) {
        for (j = 0; j < mX; j++) {
            if (RCOVd[i] <= 0.0) {
                ...
                CORR[i + mX * j] = RCOV[i + mX * j] / sqrt(RCOVd[i]);
            }
        }
    }
    for (i = 0; i < mX; i++) {
    ...
    if (WEIGHTED) {
        muste_sprintf(mod, " (weighted by %s)", weight);
        strcat(sbuf, mod);
    }
    print_line();
    ...
    for (k = 0; k <= up_lim; k++) {
        if (k < up_lim) {
            if (WEIGHTED) {
                for (i = 0; i < mX; i++) {
                    FC[i] = COEF[(i + 1) + (mX + 1) * k]; /* skip constant! */
                }
                if (WEIGHTED2) {
                    muste_fieldcopy(lab, &clabW2[lc * k], lc);
                } else {
                    muste_fieldcopy(lab, &clabW[lc * k], lc);
                }
                lab[lc] = '\0';
                trim2(lab);
            }
...
static void print_line(void) {
    ...
    if (results < 31) {
        ...
        res_line = results_line;
    }
    if (results >= 0) {
        output_line(sbuf, eout, res_line);
    }
    if (res_line) {
        ++results_line;
    }
