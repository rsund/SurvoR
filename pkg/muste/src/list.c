 ...
 */
char *sk_name[] = {"CONTROL", "SHADOW", "TEXT", "#LINES", "END", "WORD#", "#WORDS", "#SHADOW", "MSHADOW", "START", ""};
...
static char *erik_nimi[] = {"*#chapters",   "*#records", "*#lines",  "*#words",  "*#numbers",           "*#integers",
                            "*#characters", "*#letters", "*#spaces", "*#digits", "*#special characters"};
...
static int list_init_shadows(void);
...
static int hae_valmis_lista(void);
static void help(void);
static void optiot(void);
...
void muste_list(int parmn, char *parm[]) {
    ...
    *txt_file = EOS;
    ...
    if (parmn < 2) {
        sur_print("\nIncomplete LIST operation! See LIST?");
        WAIT;
        return;
    }

    if (muste_strcmpi(parm[1], "SHOW") == 0) {
        op_list_show(arguc, arguv);
    } else if (muste_strcmpi(parm[1], "REPLACE") == 0) {
        op_list_replace(arguc, arguv);
    } else if (muste_strcmpi(parm[1], "MAKE") == 0) {
        op_list_make(arguc, arguv);
    } else if (muste_strcmpi(parm[1], "COUNT") == 0) {
        op_list_count(arguc, arguv);
    } else if (muste_strcmpi(parm[1], "SORT") == 0) {
        op_list_sortsave(arguc, arguv);
    } else if (muste_strcmpi(parm[1], "SAVE") == 0) {
        op_list_sortsave(arguc, arguv);
    } else {
...
static FILE *list_fopen(char *path, char *mode) // RS 21.1.2014
{
    ...
    if (strstr(path, ".EDT") != NULL) {
        ...
        if (strncmp(rivi, "SURVO 98", 8) == 0) // RS 10.1.2014
        {
            ...
            while (1) {
                ...
                *p = EOS;
                ...
                if (rivi_luettu && *x == 'S') {
                    ...
                    i = ed1 * ed2;
                    k = 0;
                    while (k < zshn && listz[i] != '\0') {
                        ++k;
                        i += ed1;
                    }
                    listzs[j] = k + ed2 + 1;
                    // Rprintf("\nlistzs[%d]=%d, p:|%s|",j,listzs[j],p);
                    for (i = 0, h = (listzs[j] - 1) * ed1; i < ed1; ++i, ++h) {
                        listz[h] = ' ';
                    }
                    for (i = 0, h = (listzs[j] - 1) * ed1; i < strlen(p); ++i, ++h) {
                        listz[h] = p[i];
                    }
                    rivi_luettu = 0;
                    continue;
                ...
                for (i = 0, h = (j - 1) * ed1; i < strlen(p); ++i, ++h) {
                    listz[h] = p[i];
                }
                rivi_luettu = 1;
            }
            ...
            strcpy(x, "SURVO84ED ");
            strcat(x, muste_itoa(ed1, number, 10));
            strcat(x, " ");
            strcat(x, muste_itoa(ed2, number, 10));
            strcat(x, " ");
            strcat(x, "                    ");
            x[20] = EOS;
            strcat(x, muste_itoa(ed1, number, 10));
            strcat(x, " S");
            strcat(x, muste_itoa(edshad, number, 10));
            strcat(x, " ");
...
static int etsi_kpl(SURVO_LIST *l, int k, char *chapter) {
    ...
    for (j = 1; j <= ted2; ++j) {
        tedread(x, j);
        i = split(x + 1, osa, 1);
        if (strcmp(osa[0], "DEF") != 0) {
            continue;
        }
        tedread(x, j);
        i = split(x + 1, osa, 4);
        if (strcmp(osa[1], chapter) != 0) {
            continue;
        }
        if (i < 4) {
            tedread(x, j);
        ...
        i = etsi_rivi(osa[2], 1);
        if (i == 0) {
            line_not_found(osa[2]);
            return (-1);
        }
        l->line1[k] = i;
        i = etsi_rivi(osa[3], i);
        if (i == 0) {
            line_not_found(osa[3]);
            return (-1);
        }
        l->line2[k] = i;
        return (1);
...
static int list_find2(int j, SURVO_LIST *l, int kk) /* kk=0: lasketaan vain lukumaara */
{
    ...
    p = strchr(x + 1, ':');
    if (p == NULL) {
        sur_print(": missing in LIST definition!");
        WAIT;
        return (-1);
    }
    ++p;
...
static int list_field_def(SURVO_LIST *l, char *s, int k) {
    ...
    if (*osa[1] == 'L') {
        l->vartype[k] = 'L';
        l->varlen[k] = sizeof(long);
    }

    strcpy(y, osa[3]);
    y[LEN_FIELDNAME - 1] = EOS;
    ...
    if (*osa[n - 1] == '[') /* 25.8.1995 */
    {
        ...
        *q = EOS;
        ...
        if (h >= k) {
            muste_sprintf(sbuf, "\nField %s in [%s] must be defined before %s", p, p, l->varname[k]);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
    ...
    *p = EOS;
    ...
    while (*sk_name[keytype]) {
        ...
        if (muste_strcmpi(osa[0], sk_name[keytype]) == 0) {
            break;
        }
        ++keytype;
    }
    ...
    *p = EOS;
    ...
    while (*sk_name[keytype]) {
        ...
        if (muste_strcmpi(osa[2], sk_name[keytype]) == 0) {
            break;
        }
        ++keytype;
    }
    ...
    switch (keytype) {
    case CONTROL:
        i = read_control(l, k + m, p);
        return (i);
        break;
    case SHADOW:
    case MSHADOW:
        i = read_shadow(l, k + m, p, keytype);
        return (i);
        break;
    case TEXT:
        i = read_text(l, k + m, p);
...
static int read_shadow(SURVO_LIST *l, int k, char *s, int keytype) /* SHADOW || nSHADOW */
{
    ...
    */
    ...
    if (i == 1) {
        ...
        strcpy(l->p_text, osa[0]);
        l->p_text += strlen(osa[0]) + 1;
    } else {
...
static int read_nwords(SURVO_LIST *l, int k, char *s) {
    ...
    if (muste_strnicmp(s, "C:", 2) == 0) {
        s += 2;
        l->sk_char0[k] = 'C';
    } else {
        l->sk_char0[k] = 'U'; /* C=case-sensitive, U=Not */
    }

    if (*s == '#') {
        return (read_nwords_from_file(l, k, s));
    }
    strcpy(x, s);
    if (l->sk_char0[k] == 'U') {
        muste_strupr(x);
...
static int read_nwords_from_file(SURVO_LIST *l, int k, char *s) {
    ...
    *p = EOS;
    ...
    while (1) {
        ...
        if (!code_found) {
            if (feof(sanasto)) {
                muste_sprintf(sbuf, "\nCode word %s not found in file %s", koodisana, sanat);
                sur_print(sbuf);
                WAIT;
                return (-1);
            }
            ...
            if (strncmp(koodisana, p, strlen(koodisana)) != 0) {
                continue;
            }
            p += strlen(koodisana);
            code_found = 1;
        ...
        for (h = 0; h < i; ++h) {
            ...
            strcpy(l->p_text, osa[h]);
            l->p_text += strlen(osa[h]) + 1;
            ++n_sanat;
        }
    }
    return (1);
    ...
    case 3: /* MSHADOW */
        ...
        while (1) {
            ...
            while (i <= loc[2]) {
                ...
                if (i == loc[0] && loc[1] > 1) {
                    spaces_over(y, ys, 1, loc[1] - 1);
                }
                if (i == loc[2] && loc[3] < lwidth - 1) {
                    spaces_over(y, ys, loc[3] + 1, lwidth - 1);
                }
                /*
                printf("\n y=%.50s",y);
                printf("\nys=%.50s",ys); getch();
        ...
        l->start_line[k] = n_lines2v;
        l->start_pos[k] = 1;
        l->end_line[k] = n_lines2 - 1;
        l->end_pos[k] = lwidth - 1;
...
static int read2_nlines(SURVO_LIST *l, int k, char *x) {
    ...
    if (host == -1) {
        ...
        n = l->end_line[host] - l->start_line[host] + 1;
    }
    muste_sprintf(x, "%d", n);
    /* printf("\nnlines=%d",n); getch(); */
...
static int read2_wordn(SURVO_LIST *l, int k, char *x) /* etsii vain 1. rivilta! */
{
    ...
    if (host == -1) {
        ...
        l1 = l->start_line[host];
        pos1 = l->start_pos[host];
    }
    ...
    lev = l->c2;
    muste_fread(s, 1, lev, tempf);
    s[lev] = EOS;
    m = l->sk_int0[k];
    if (m > 20) {
        sur_print("max m in WORD#:m is 20!");
        WAIT;
        return (-1);
    }
...
static int read2_start(SURVO_LIST *l, int k, char *x) {
    ...
    m = l->sk_int0[k];
    *x = EOS;
    strncat(x, s + 1, m); // ei kontrollisaraketta!
    return (1);
}
        ...
        if (type == 0) {
            ...
            for (h = 0; h < type; ++h) {
                ...
                while (1) {
                    ...
                    switch (typ) {
                    case 0: /* text */
                        if ((p == pp || strchr(sanav, ch1) != NULL) && (ch2 == EOS || strchr(sanav, ch2) != NULL)) {
                            ++n;
                        }
                        p += len;
                        break;
                    case 1: /* *text */
                        if (ch2 == EOS || strchr(sanav, ch2) != NULL) {
                            ++n;
                        }
                        p += len;
                        break;
...
static void list_close(SURVO_LIST *l) { list_fclose(text); }
...
static int list_chapter_open(SURVO_LIST *l, int k, char *muoto) {
    ...
    strcpy(chapter, l->chapter[k]);
    i = list_find_chapter(l, k, chapter);
    if (i < 0) {
        return (-1);
    }
    return (1);
...
static int list_txt_find(SURVO_LIST *l, FILE *tempf, int type, /* 0=edit line, 1=shadow line */
                         ...
                         int *loc)                             /* MSHADOW: start and end positions */
{
    ...
    for (i = l1; i <= l2; ++i) {
        ...
        if (p != NULL) {
            ...
            if (loc != NULL) {
                loc[0] = l->start_line[var];
                loc[1] = l->start_pos[var];
            }

            if (end_type == -1 && value != NULL) {
                muste_fieldcopy(value, s + l->start_pos[var], len);
                value[len] = EOS;
            }
            ...
            if (value != NULL) {
                ...
                if (l->end_line[var] == l->start_line[var]) {
                    ...
                    muste_fieldcopy(value, s + l->start_pos[var], h);
                    value[h] = EOS;
                }
            }
            return (1);
...
static int host_start_read(SURVO_LIST *l, int var, int *pl1, int *ppos1, int *pl2, int *ppos2) {
    ...
    /* -23.10.95 { *pl1=0; *ppos1=0; *pl2=2*(l->n_lines-1); *ppos2=l->c2-1; }  */
        ...
        *pl1 = l->start_line[host];
        *ppos1 = l->start_pos[host];
        *pl2 = l->end_line[host];
        *ppos2 = l->end_pos[host];
    }
    return (1);
}

static int list_control_find(SURVO_LIST *l, FILE *tempf, int var, /* current field */
                             int len, char *value)                /* NULL if not needed */
{
    char s[LLENGTH];
    int i, lev;
    int l1, pos1, l2, pos2;
    ...
    for (i = l1; i <= l2; ++i) {
        ...
        while (1) {
            ...
            if (p != NULL) {
                ...
                if (!count) {
                    muste_fieldcopy(x, s2 + h1, h - h1 + 1);
                    x[h - h1 + 1] = EOS;
                    return (1);
                }
                ++n;
...
static int list_field_end_text(SURVO_LIST *l, FILE *tempf, int type, /* 0=edit line, 1=shadow line */
                               int var,                              /* current field */
                               int *loc) {
    char s[LLENGTH];
    char *p;
    ...
    for (i = l1; i <= l2; ++i) {
        ...
        if (p != NULL) {
            l->end_line[var] = i;
            l->end_pos[var] = ps1 + (p - s) - (1 - type);
            if (l->start_pos[var] < 0) {
                --l->end_line[var];
                l->end_pos[var] = l->c2 - 1;
            }

            if (loc != NULL) {
                loc[2] = l->end_line[var];
                loc[3] = l->end_pos[var];
            }
...
static int lst_save(char *comment, char *newlist_path, char *newlist, int n, char case_start, char case_end) {
    ...
    */
    ...
    if (lst_file == NULL) {
        muste_sprintf(sbuf, "\nCannot open file %s for a new list structure!", nimi);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int selaa() {
    ...
    while (1) {
        ...
        case CODE_EXIT:
            break;
        case CODE_NEXT:
            if (il == list.n - 1 && viimrivi == list.line2[il]) {
                break;
            }
            ensrivi = viimrivi + 1;
            ensil = il;
        ...
        case CODE_REF:
            ...
            if (ref_ensil == ensil && ref_ensrivi == ensrivi && ref_sar1 == sar1 && ref_sr == sr && ref_sc == sc) {
                ref_ensrivi = 0;
                break;
            }
            ensil = ref_ensil;
...
static void paikka(int sr, int sc) { LOCATE(sr + rdisp + 1, sc + 8); }
...
static void disp_label(SURVO_LIST *l, int k, int rivi) {
    ...
    i = muste_sprintf(sbuf, "%c%8d   LIST %s: %s in %s", NELIO, sar1, lista, l->chapter[k], l->editfile[k]);
    if (l->line1[k] != 0) {
...
static void nayta_rivi(int riv, int i) {
    ...
    } else /* varjot=0 */
    {
        ...
        if (lev < c3) {
            muste_sprintf(sbuf + k, "%c%.*s", REUNA, c3 - lev - 1, space);
        }
        sur_prin2(sbuf, i, 1);
    }
}
...
static void vaihehaku(int jatkohaku) /* 0=uusi haku 1=jatkoa */
{
    ...
    while (1) {
        ...
        m = nextch("");
        RESTORE_CURSOR;
        if (m == CODE_RETURN || m == CODE_EXIT) { /* 18.2.92 */
            putsaa();
            return;
        }
        if (m == CODE_EXEC) {
            ...
            if (m == CODE_EXEC) {
                ...
                while (1) {
                    ...
                    if (i == 0) {
                        ...
                        nayta();
                        LOCATE(r3disp + 2, hpos);
                        PR_EINV;
                        muste_sprintf(sbuf, "%.62s", space);
                        sur_print(sbuf);
                        LOCATE(r3disp + 2, hpos);
                        PR_EUDL;
                        muste_sprintf(sbuf, "%s", haku);
                    ...
                    PR_EIN2;
                    LOCATE(r3disp + 2, 51);
                    muste_sprintf(sbuf, "N=%ld", n_haku);
                    sur_print(sbuf);
                }
                autom_haku = 0;
                continue;
...
static int etsi_sanaa(char *haku, char *hakuvarjo) {
    ...
    *tyhja = EOS;
    ...
    if (isot_kirjaimet) {
        if (muste_strnicmp(p, haku, len) == 0) {
            return (1);
        }
    } else {
        if (strncmp(p, haku, len) == 0) {
            return (1);
        }
    }
    ...
    while (1) {
        ...
        {
            if (isot_kirjaimet) {
                i = muste_strnicmp(p, haku, len);
            } else {
                i = strncmp(p, haku, len);
            }
            if (i == 0) {
                break;
            }
...
static int etsi_varjosanaa(char *haku, char *hakuvarjo) {
    ...
    while (1) {
        ...
        if (tzs[riv] == 0) {
            *sx = EOS;
            strncat(sx, space, ted1);
        } else {
            tedread(sx, tzs[riv]);
        }
        if (strncmp(p, hakuvarjo, vlen) == 0) {
            if (!autom_haku) {
                nayta();
            }
            break;
...
static void op_list_replace(int argc, char *argv[]) {
    ...
    j1 = edline2(word[3], 1, 1);
    if (j1 == 0) {
        return;
    }
    j2 = edline2(word[4], 1, 1);
    if (j2 == 0) {
        return;
    }
    edread(x, j1);
...
static void korvaa(char *t1, char *st1, char *t2, char *st2) {
    ...
    */
    ...
    for (il = 0; il < list.n; ++il) {
        ...
        muste_sprintf(sbuf, "\nChapter %s in %s", list.chapter[il], list.editfile[il]);
        sur_print(sbuf);
        for (riv = list.line1[il]; riv <= list.line2[il]; ++riv) {
            tedread(x, riv);
            if (tzs[riv] == 0) {
                *x2 = EOS;
                strncat(x2, space, ted1 + 2);
            } else {
                tedread(x2, tzs[riv]);
            ...
            *x = ' ';
            ...
            while (1) {
                ...
                if (*st1) {
                    ...
                    //         Rprintf("\n%.*s",len1,st1);
                    /* -8.4.96 break; */
                    if (strncmp(x2 + (p - x) + 2, st1, len1) != 0) {
                        ++p;
                        continue;
                    }
                }
...
static int talletus_replace(int riv, char *x, char *x2) {
    ...
    if (empty_line(x2 + 3, strlen(x2 + 3))) {
        return (1);
    }

    sur_print("\nNo saving of new shadows!");
    WAIT;
    return (-1);
...
static void op_list_make(int argc, char *argv[]) {
    ...
    strcpy(x, word[2]);
    i = strlen(x) - 1;
    while (i > 0 && x[i] != '\\' && x[i] != '/' && x[i] != ':') {
        --i;
    }
    if (i == 0) {
        strcpy(newlist_path, edisk);
        strcpy(newlist, x);
    } else {
        ++i;
        muste_fieldcopy(newlist_path, x, i);
        newlist_path[i] = EOS;
        strcpy(newlist, x + i);
    }
    ...
    while (1) {
        ...
        if (n_edit_lines + n_lines > lr2) {
            i = close_edit_file_make(n_edit_lines);
            if (i < 0) {
                muste_fclose(tempf);
                return;
            }
        ...
        if (feof(tfile)) {
            ...
            muste_fclose(tempf);
            strcpy(x, newlist_path);
            strcat(x, newlist);
            if (strchr(newlist, '.') == NULL) {
                strcat(x, ".LST");
            }
...
static int find_line_start(char *s, int lin) {
    ...
    for (i = lin; i <= r2; ++i) {
        edread(x, i);
        split(x + 1, osa, 1); // k=split
        if (strncmp(s, osa[0], len) == 0) {
            break;
        }
    }
    if (i > r2) {
...
static int create_edit_file_make(int edtnro) {
    ...
    strcpy(nimi1, newlist);
    muste_sprintf(sbuf, "%d", edtnro);
    strcat(nimi1, sbuf);
    strcpy(nimi2, newlist);
    muste_sprintf(sbuf, "%d", edtnro + 1);
    strcat(nimi2, sbuf);
    strcpy(nimi0, newlist_path);
    strcat(nimi0, nimi1);
    strcat(nimi0, ".EDT");

    muste_sprintf(sbuf, "\nSaving in edit file %s ...", nimi0);
    sur_print(sbuf);
    ...
    if (edt == NULL) {
        muste_sprintf(sbuf, "\nCannot open edit file %s for a sorted list!", nimi0);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int close_edit_file_make(int n_edit_lines) {
    ...
    strcat(header, muste_itoa(lwidth + 1, number, 10));
    strcat(header, " ");
    strcat(header, muste_itoa(lr2, number, 10));
    strcat(header, " ");
    strcat(header, "                    ");
    header[20] = EOS;
    strcat(header, muste_itoa(lwidth + 1, number, 10));
    strcat(header, " S");
    strcat(header, muste_itoa(shad, number, 10));
    strcat(header, " ");
...
static int etsi_phrases(char *s) {
    ...
    while (1) {
        ...
        } else /* omaan tilaan, jotta voidaan rekoodata esim. isoiksi kirjaimiksi */
        {
            ...
            muste_fieldcopy(ph, z + (k - 1) * ed1 + 1, len);
            phr[h] = ph;
            ph += len;
            if (isot) {
                strnuprf(phr[h], len);
...
static int count() {
    ...
    *xs = EOS;
    ...
    while (1) {
        ...
        muste_sprintf(sbuf, "\nChapter %s in edit file %s", list.chapter[list.i], list.editfile[list.i]);
        sur_print(sbuf);
...
static void op_list_sortsave(int argc, char *argv[]) {
    ...
    muste_strupr(word[1]);
    if (strcmp(word[1], "SORT") == 0) {
        komento = SORT;
    } else if (strcmp(word[1], "SAVE") == 0) {
        komento = SAVE;
    } else {
    ...
    */
    ...
    i = spfind("CHECK");
    if (i >= 0) {
        list_check = atoi(spb[i]);
    }
    *codefile = EOS;
    i = spfind("FILTER");
    if (i >= 0) {
        strcpy(codefile, spb[i]);
    }
    i = spfind("NSORT");
    if (i >= 0) {
        samplesize = atol(spb[i]);
    }
    workspace = MAXBLOCK;
    i = spfind("WORKSPACE");
    if (i >= 0) {
    ...
    case_start = list.case_start;
    i = spfind("CASE_START");
    if (i >= 0) {
        case_start = list.case_start = *spb[i];
    }
    case_end = list.case_end;
    i = spfind("CASE_END");
    if (i >= 0) {
        case_end = list.case_end = *spb[i];
    }

    if (*codefile) {
        i = load_codes(codefile, code);
        if (i < 0) {
            return;
        }
    }
    ...
    if (i >= 0) {
        ...
        if (lwidth > 252 || lwidth < 30 || lr2 < 23 || lshad > lr2 ||
            ...
            muste_sprintf(sbuf, "\nIncorrect or spurious LISTDIM=%d,%d,%d setting!", lr2, lwidth, lshad);
            sur_print(sbuf);
    ...
    */
    ...
    if (txtfile != NULL) {
        muste_fclose(txtfile); // RS 26.1.2014
    }
    return;
}
...
static int lue_avaimet(int koko, unsigned int *pnsort) {
    ...
    for (i = 0; i < koko; ++i) {
        ...
        if (list.i != prev_chp) {
            h = prev_chp = list.i;
            muste_sprintf(sbuf, "\nChapter %s in %s:", list.chapter[h], list.editfile[h]);
            sur_print(sbuf);
        }

        ++list.nro;
        if (l_unsuitable(&list)) {
            PR_EINV;
        } else {
            PR_EIN2;
            k = make_key();
        ...
        if (list.case_var == -1) {
            muste_sprintf(sbuf, " %ld", list.nro);
        } else {
            k = list.case_var;
            list_data_load(&list, k);
            h = list.varlen[k];
            strcpy(sbuf, " ");
            strncat(sbuf + 1, list.value[k], h);
            while (h > 1 && sbuf[h] == ' ') {
                sbuf[h--] = EOS;
            }
        }
        ...
        PR_EINV;
        if (l_wait) {
            sur_wait((long)(1000.0 * l_wait), nop, 1);
        }
    }
    *pnsort = n;
    return (1);
...
static int read_case(int filter) {
    ...
    while (1) {
        ...
        if (first_line == 0) {
            if (case_start == *x) {
                first_line = k;
            } else if (case_start == EOS && strncmp((char *)(x + 1), space, c2) != 0) {
                first_line = k;
            }
            if (first_line) {
                chapter = h;
        ...
        */
        if (case_end == *x) {
            return (1);
        } else if (case_end == EOS && strncmp((char *)(x + 1), space, list.c2) == 0) {
            return (1);
        }
    }
    return (1);
...
static int show_case(long n) {
    ...
    for (k = 0; k < n_lines; ++k) {
        ...
        for (i = 0; i < lwidth; ++i) {
            getc(tempf); // xs[i]=(char)getc(tempf);
        }
        muste_sprintf(sbuf, "\n%.*s", c3 + 8, x);
        sur_print(sbuf);
    }
    ...
    for (i = 0; i < n_show; ++i) {
        ...
        muste_sprintf(sbuf, "%s=%s ", list.varname[k], x);
        sur_print(sbuf);
    }
    PR_EINV;
...
static void shell_sort(unsigned int j1, unsigned int j2, int t) {
    ...
    if (iso) {
        ...
        muste_sprintf(sbuf, " %u-%u ", j1 + 1, j2 + 1);
        sur_print(sbuf);
        muste_sprintf(sbuf, " n=%u", n);
        sur_print(sbuf);
    }
    len = sl[t];
    h = n;
    ...
    case '8':
        while (h > 1) {
            ...
            while (1) {
                ind = '1';
                for (i = j1; i <= j2 - h; ++i) {
                    if ((neg[t] && *(double *)(key + ikey[i] + sp[t]) < *(double *)(key + ikey[i + h] + sp[t])) ||
                        (!neg[t] && *(double *)(key + ikey[i] + sp[t]) > *(double *)(key + ikey[i + h] + sp[t]))) {
                        k = ikey[i];
                        ikey[i] = ikey[i + h];
                        ikey[i + h] = k;
                        ind = '0';
    ...
    case '4':
        while (h > 1) {
            ...
            while (1) {
                ind = '1';
                for (i = j1; i <= j2 - h; ++i) {
                    if ((neg[t] && *(float *)(key + ikey[i] + sp[t]) < *(float *)(key + ikey[i + h] + sp[t])) ||
                        (!neg[t] && *(float *)(key + ikey[i] + sp[t]) > *(float *)(key + ikey[i + h] + sp[t]))) {
                        k = ikey[i];
                        ikey[i] = ikey[i + h];
                        ikey[i + h] = k;
                        ind = '0';
    ...
    case '2':
        while (h > 1) {
            ...
            while (1) {
                ind = '1';
                for (i = j1; i <= j2 - h; ++i) {
                    if ((neg[t] && *(int *)(key + ikey[i] + sp[t]) < *(int *)(key + ikey[i + h] + sp[t])) ||
                        (!neg[t] && *(int *)(key + ikey[i] + sp[t]) > *(int *)(key + ikey[i + h] + sp[t]))) {
                        k = ikey[i];
                        ikey[i] = ikey[i + h];
                        ikey[i + h] = k;
                        ind = '0';
    ...
    case 'L':
        while (h > 1) {
            ...
            while (1) {
                ind = '1';
                for (i = j1; i <= j2 - h; ++i) {
                    if ((neg[t] && *(long *)(key + ikey[i] + sp[t]) < *(long *)(key + ikey[i + h] + sp[t])) ||
                        (!neg[t] && *(long *)(key + ikey[i] + sp[t]) > *(long *)(key + ikey[i + h] + sp[t]))) {
                        k = ikey[i];
                        ikey[i] = ikey[i + h];
                        ikey[i + h] = k;
                        ind = '0';
...
/* SORT*0.TMP  *=kierros */
{
    ...
    if (n_osat > 1) {
        ...
        if (sortf == NULL) {
            muste_sprintf(sbuf, "\nCannot read temporary file %s", nimi2);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
    ...
    muste_sprintf(sbuf, "\n\nSaving %ld sorted cases as a list %s ...", nhav, newlist);
    sur_print(sbuf);
    ...
    for (j = 0; j < (long)nhav; ++j) {
        ...
        if (n_edit_lines + n_lines > lr2) {
            i = close_edit_file_sort(n_edit_lines);
            if (i < 0) {
                return (-1);
            }
            edtnro = create_edit_file_sort(edtnro);
...
static int osatalletus(unsigned int nsort, int k) {
    ...
    if (sortf == NULL) {
        muste_sprintf(sbuf, "\nCannot create temporary file %s", nimi);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int lomita(int file1, int file2, int kierros, int nro) {
    ...
    muste_sprintf(sbuf, "\nMerging files SORT%d%d - %d%d.TMP to SORT%d%d.TMP: ", kierros - 1, file1, kierros - 1, file2,
                  kierros, nro);
    sur_print(sbuf);
...
static int vertailu(int k0, int k) {
    ...
    for (t = 0; t < nsk; ++t) {
        ...
        switch (sk_format[t]) {
        case 'S':
            h = strncmp(key + k0 * slen + sp[t], key + k * slen + sp[t], len);
            break;
        case '8':
...
static int create_edit_file_sort(int edtnro) {
    ...
    strcpy(nimi3, etmpd);
    strcat(nimi3, "SURVO2.TMP");
    shadows = muste_fopen(nimi3, "w+b");
    if (shadows == NULL) {
        muste_sprintf(sbuf, "\nCannot open temporary file %s!", nimi3);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
static int tutki_txtfile(int k) {
    ...
    erotin = '|';
    if (i == 2) {
        if (muste_strnicmp(osa[1], "char(", 5) == 0) {
            erotin = atoi(osa[1] + 5);
        } else {
            erotin = *osa[1];
        }
    ...
    if (!muste_is_path(txt_file)) {
        strcpy(txt_file, edisk);
        strcat(txt_file, osa[0]);
    }
...
static int list_find_cond(SURVO_LIST *d, char *nimi, int nro) {
    ...
    {
        sel_type[nro] = '0';
        if (muste_strcmpi(sana[0], "ORDER") == 0) {
            sel_var[nro] = -1;
        } else {
            sel_var[nro] = list_varfind(d, sana[0], 0);
            if (sel_var[nro] < 0) {
                return (-2);
            }
        }
    ...
    else /* CASES-tyyppinen */
    {
        ...
        *p = EOS;
        ...
        if (d->vartype[sel_var[nro]] != 'S') {
            ...
            if (etu == 2) {
                muste_sprintf(tut_info, "˛˛˛@11@CONDITIONS@%s@", sbuf);
                return (-2);
            }
            sur_print("\n");
            sur_print(sbuf);
...
static int l_unsuitable(SURVO_LIST *d) {
    ...
    if (sel_var[0] > -2 && l_unsuit(d, 0)) {
        return (1);
    }
    if (sel_var[1] > -2 && l_unsuit(d, 1)) {
        return (1);
    }
...
static int l_unsuit(SURVO_LIST *d, int nro) {
    ...
    if (sel_type[nro] == '0') {
        if (sel_var[nro] == -1) {
            ...
            list_data_load8(d, sel_var[nro], &x);
        }
        if (x < sel_lower[nro] || x > sel_upper[nro]) {
            return (lt_neg(1, nro));
        }
        return (lt_neg(0, nro));
    }
    ...
    list_alpha_load(d, sel_var[nro], sana + 1);
    *sana = ',';
    len = strlen(sana);
    while (sana[len - 1] == ' ') {
        sana[--len] = EOS;
    }
    if (strcmp(sel_lastcase[nro], sana + 1) == 0) {
        return (lt_neg(0, nro));
    }
    sana[len] = ',';
    ++len;
    sana[len] = EOS;
    if (strncmp(p, sana + 1, len - 1) == 0) {
        return (lt_neg(0, nro));
    }
    if (strstr(p, sana) != NULL) {
        return (lt_neg(0, nro));
    }
    return (lt_neg(1, nro));
}
