...
extern int read_tutword(char s[]);
int tutch(void);
...
int tutch() {
    ...
    if (!feof(tutor)) {
        ...
        if (sur_kbhit()) {
            ...
            default:
                break; // RS CHA FIXME???while(!sur_kbhit()) ; sur_getch(); break;
            }
        }

        if (m == TUT_EFFECTS_OFF) {
            etu2 = etu3 = 0;
            return (255);
        }
        if (m == CODE_PRE && tut_special_code) {
            tut_special();
            return (255);
        }
        /* lis. 4.10.88  */
        ...
        if (etu2 > 0) {
            ...
            if (tut_special_code) {
                return (m); /* 23.10.89 ennen etu2==2 && */
            }
            CURSOR_POS(&rr, &cc);
            CURSOR_OFF;
            LOCATE(r3 + 2, c3 - 10);
            ...
            if (etu2 == 1) {
                if (etu1 > 1) {
                    sur_wait((long)4 * (long)(tut_wait_c * etu1), Wdisp, 0);
                }
            } else {
                press_key(m);
            }
