...
void muste_dcluster(char *argv) {
    ...
    if (m2 != m) {
        muste_sprintf(sbuf, "\n# of rows in %s (%d) not equal to %d!", word[2], m2, m);
        sur_print(sbuf);
        WAIT;
        return;
    }
    ...
    case 1:
        initial_medoids();
        iterate1();
        break; /* short cut technique by S.M. */
    case 2:
        initial_medoids();
        iterate2();
        break;
...
static int init_groups1(int var) {
    ...
    j = d.l1;
    for (i = 0; i < m; ++i) {
        muste_fieldcopy(ots, rlab + lr * i, lr);
        ots[lr] = EOS;
        pots = ots;
        while (*pots == ' ') {
            ++pots;
...
static int print_results() {
    ...
    if (silh_var >= 0) {
        ...
        mean = 0.0;
        for (i = 0; i < ng; ++i) {
            muste_fieldcopy(med_name, rlab + medoid[i] * lr, lr);
            med_name[lr] = EOS;
            muste_sprintf(sbuf, "%2d    %*.*s%6d %3.3g", i + 1, lr, lr, med_name, gsize[i], ssum[i] / gsize[i]);
            eoutput(sbuf);
            mean += ssum[i];
        }
        mean /= m;
        ...
        for (i = 0; i < ng; ++i) {
            muste_fieldcopy(med_name, rlab + medoid[i] * lr, lr);
            med_name[lr] = EOS;
            muste_sprintf(sbuf, "%2d    %*.*s%6d", i + 1, lr, lr, med_name, gsize[i]);
            eoutput(sbuf);
        }
    }
    eoutput(" ");
...
static int print_results3() {
    ...
    if (weight_ind) {
        muste_sprintf(sbuf + i, "(WEIGHT=%g)", weight);
    }
    eoutput(sbuf);
    muste_sprintf(sbuf, "Data %s  N=%d", word[1], m);
    eoutput(sbuf);
