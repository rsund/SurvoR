...
int muste_risksimu(int argc, char *argv[]) {
    ...
    if (i < 0) {
        ...
        nextr = (unsigned long)atoi(spb[i]);
    }
    ...
    if (i >= 0) {
        ...
        if (i >= 0) {
            muste_sprintf(sbuf, "\nWriting raw data from bootstrap to the file %s...", spb[i]);
            sur_print(sbuf);
        ...
        for (munc = 0; munc < kuntia; munc++) {
            muste_sprintf(sbuf, "%i ", kuno[munc]);
            sur_print(sbuf);
