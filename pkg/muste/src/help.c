...
static void kopioi_malli(void);
static int empty_h(char *s);
static int vaihda_sys(void);
static int uusi_sys(void);
static int nayta(void);
static int text_open(char *s);
static int hae(char *s);
static int tdisp(int j);
static int dclear(void);
static int mouse_help_lines(void);
static int tdisp2(int j);
static int show_example(void);
static void kasvata_ikkunaa(void);
static int C_koodi(void);
static int E_koodi(void);
...
static int ota_muistista(void);
static int mouse_help(char *sana);
static void uusi_haku(void);
static void putsaa(void);
static int monivalinta(void);
static int fit(int j);
static void tulosta(char *x);
static int load_lines(void);
...
void muste_help(int argc, char *argv[]) {
    ...
    *info2 = EOS;
    ...
    if (*info == EOS) {
        ...
        strcpy(hakusana, info);
    }
    if (strcmp(hakusana, "???") == 0) {
        edread(x, r1 + r - 1);
        i = c1 + c - 1;
        if (x[i] == ' ' && x[i - 1] == ' ') {
            strcpy(hakusana, "SURVO");
        } else {
            i = nykyinen_sana(x, c1 + c - 1, hakusana);
            if (i < 0) {
                return;
            }
        }
...
static int nykyinen_sana(char *x, int pos, char *hakusana) {
    ...
    *q = EOS;
    ...
    strcpy(hakusana, q + 1);
    muste_strupr(hakusana);
    if ((unsigned int)*hakusana > 127) {
        *hakusana = EOS; // 23.1.2001
    }
...
static int avaa(int isys) {
    ...
    if (keywords == NULL) {
        PR_EBLD;
        muste_sprintf(sbuf, "\nInquiry system missing on path %s%s!", info2, qprefix);
        sur_print(sbuf);
        WAIT;
        PR_ENRM;
        return (-1);
...
static int nayta() {
    ...
    samat = 0;
    if (muste_strcmpi(hakusana, sana[0]) == 0) {
        samat = 1;
    }
    i = text_open(sana[1]);
    samat = 0;
...
static int text_open(char *s) {
    ...
    strcpy(muf, s);
    strcpy(uedq2, info2);
    strcat(uedq2, qprefix);
    strcat(uedq2, s);
    strcat(uedq2, ".EDT");
    if (strcmp(edq2, uedq2) == 0) {
        return (1);
    }
    if (*edq2 != EOS) {
        muste_fclose(text);
...
static int hae(char *s) {
    ...
    while (i <= ted2) {
        tedread(rivi, i);
        if (strncmp(s, rivi + 1, len) == 0) {
            return (i);
        }
        ++i;
    }
    muste_sprintf(sbuf, "\nKeyword %s not found in file %s", s, edq2);
    sur_print(sbuf);
    WAIT;
    return (-1);
}
...
static int mouse_help_lines() {
    ...
    muste_fieldcopy(sbuf + c3 - 15, avainsana, strlen(avainsana));
...
static int tdisp2(int j) {
    ...
    while (1) {
        ...
        if (*rivi == ' ') {
            alkupos = 1 - alkupos; // SM 22.2.2013 0 -> 1-alkupos
        }
        ...
        *koodi = EOS;
        ...
        if (*(rivi + 1) != EOS) /* muutakin kuin @koodi */
        {
            ...
            if (*rivi == 'S') {
                ...
                if (rlen < c3 + 6) {
                    write_string(space, c3 + 6 - rlen, ch, crivi, rlen + 1);
                }
                sur_print("\n");
            }
            ++mdisp;
...
static void kasvata_ikkunaa() {
    ...
    PR_EUDL;
    sur_scroll_up(1, 2, 1, r3 + 2, c3 + 8, (int)shadow_code[sdisp]);
    PR_UP;
...
static void valinnat(int j) {
    ...
    *valinta = EOS;
    ...
    PR_EINV;
    if (v1 != v2) {
        muste_sprintf(sbuf, "Select %c-%c or press ENTER! ", v1, v2);
        sur_print(sbuf);
    } else {
        muste_sprintf(sbuf, "Select %c or press ENTER! ", v1);
        sur_print(sbuf);
    }
    PR_ENRM;
    PR_LEFT;
...
static void putsaa() {
    PR_ENRM;
    LOCATE(r3 + 2, 1);
    muste_sprintf(sbuf, "%.*s", c3 + 6, space);
    sur_print(sbuf);
}
...
static int load_lines() {
    ...
    for (j = 0; j < nrivit; ++j) {
        ...
        *q = EOS;
        ...
        if (!own_window) {
            ...
            if (!empty_h(x + 1)) /* { kesken=1; break; }  */
            {
                if (!riv_lis) {
                    ...
                    muste_sprintf(sbuf, "Not enough empty lines! Insert space for %d new lines (Y/N)?  ", nrivit - j);
                    write_string(sbuf, 62, '7', r3 + 2, 1);
                    LOCATE(r3 + 2, 61);
                    m = nextch("");
        ...
        if (alkupos == 0 && !own_window) {
            *contr = kmerkki;
            edwrite(contr, oline, 0);
        }
    ...
    if (kesken == 1) {
        PR_EBLK;
        LOCATE(r3 + 2, 55);
        sur_print("Not enough empty lines!");
        PR_ENRM;
    } else if (!kesken) {
        PR_EINV;
        LOCATE(r3 + 2, 20);
        sur_print("Lines loaded!");
        PR_ENRM;
    }
    if (own_window) {
        muste_fclose(lines);
...
static void print_lines() {
    ...
    sur_print("(Load them first to the field by '+', please.)!");
    PR_ENRM;
    sur_getch();
    LOCATE(r3 + 2, 1);
    ERASE;
...
static void print_varjorivi(char *rivi, int j, char *varjo) {
    ...
    for (i = 1; i < len; ++i) {
        dispx = varjo[i];
        k = i;
        while (k < len && varjo[k] == dispx) {
            ++k;
        }
        qdisp_mode((int)dispx);
        muste_sprintf(sbuf, "%.*s", k - i, rivi + i);
        sur_print(sbuf);
        i = k - 1;
    }
    PR_EUDL;
    if (len < c3 + 7) {
        muste_sprintf(sbuf, "%.*s", c3 + 7 - len, space);
        sur_print(sbuf);
    }
    sur_print("\n");
}
...
static int kur2(int m, char *sana) {
    ...
    case CODE_EXEC:
        read_string(x + 1, x2 + 1, c3 + 2, row, 1);
        *x = ' ';
