#include "muste.h"
/* &tab.c 29.3.1986/SM (21.10.1992) (22.7.1994) (19.6.1997)
   TAB <data>,L
*/
...
static unsigned int nspace;       /* # of cells */
...
static int print_stable(char *name, int line, char *eout, int dim, int *nc, int *cumnc, int *ctype, FREQ *f, FREQ total,
                        FREQ missing, char **varname, char **cname, int colvar, int minwidth, char *cellvar,
                        char *cellformat, double *sum, double *sum2, int celloption, int tab_labels);
    ...
    for (i = 0; i < dim; ++i) {
        ...
        if (*classlist == '/') {
            ...
            if (d.vartype[classifier[i]][0] != 'S') {
                ...
                muste_sprintf(sbuf, "\n%.8s is not a string variable!", d.varname[classifier[i]]);
                sur_print(sbuf);
                WAIT;
                return (-1);
            }
            ...
            if (n < 2) {
                muste_sprintf(sbuf, "\nInsufficient classification for %.8s", d.varname[classifier[i]]);
                sur_print(sbuf);
                WAIT;
                return (-1);
            }
        ...
        for (k = 0; k < n; ++k) {
            ...
            if (q != NULL) {
                ...
                *q = EOS;
                ++q;
                if (p - clist > MAXSPACE - strlen(q) - 1) {
                    space_error();
                    return (-1);
                }
                while (*q) {
...
static int space_allocation() {
    f = (FREQ *)muste_malloc(nspace * sizeof(FREQ));
    if (f == NULL) {
    ...
    sum = (double *)muste_malloc(nspace * sizeof(double));
    sum2 = (double *)muste_malloc(nspace * sizeof(double));
    if (sum == NULL || sum2 == NULL) {
        sur_print("\nNot enough memory for cell means etc.!");
        WAIT;
        return (-1);
    }
    return (1);
...
static int printout() {
    ...
    if (i >= 0) {
        strcpy(result, spb[i]);
        if (strstr(result, "CSUM") != NULL) {
            ++isum;
        }
        if (strstr(result, "RSUM") != NULL) {
            isum += 2;
        }
    }
    i = spfind("LABELS");
    ...
    if (cellvar < 0) {
        ...
        i = print_ftable(name, results_line, eout, dim, nc, cumnc, ctype, f, total, missing, varname, cname, colvar,
                         width, isum, 0, tab_labels);
        if (results_line && i > 1) {
            results_line = i;
        }
        ...
        if (cellvar == CELL_ONE) {
            ...
            strcpy(y, d.varname[cellvar]);
        }
        table_name(name, word[1], ++index);
        i = print_stable(name, results_line, eout, dim, nc, cumnc, ctype, f, total, missing, varname, cname, colvar,
                         width, y, cellformat, sum, sum2, celloption, tab_labels);
    }
    return (i);
}
                           ...
                           /*                             1      2      */
{
    ...
    *cellformat = EOS;
    if (i > 1) {
        strcpy(cellformat, w[1]);
    }
    celloption = 0;
    if (i > 2) {
        if (muste_strcmpi(w[2], "SUM") == 0) {
            celloption = 1;
        } else if (muste_strcmpi(w[2], "FSUM") == 0) {
            celloption = 2;
        }
    }
    return (1);
                ...
                if (len) {
                    for (k = 0; k < nc[i]; ++k) {
                        ...
                        while ((p = strchr(p, *s)) != NULL) {
                            ...
                            if (strncmp(s, p, len) == 0 && (p[len] == '/' || p[len] == EOS)) {
                                class[i] = k;
                                break;
                            }
                            ++p;
...
// int tab_labels;              /* 0=no TABLE definition, no edit line labels */
{
    ...
    output_open(eout);
    colwidth = minwidth;
    if (colvar && varwidth[colvar - 1] > colwidth) {
        colwidth = varwidth[colvar - 1];
    }
    if (ipros && colwidth < 5) {
        colwidth = 5;
    }
    ...
    if (tab_labels) {
        ...
        if (total) {
            strcat(x, "   N=");
            muste_sprintf(value, "%u", total);
            strcat(x, value);
            if (missing) {
                strcat(x, " N(missing)=");
                muste_sprintf(value, "%u", missing);
                strcat(x, value);
            }
        }
...
// int tab_labels;              /* 0=no TABLE definition, no edit line labels */
{
    ...
    if (starwidth < 4) {
        starwidth = 4; /* space for text 'Mean' */
    }
    freqcol = 0;
    for (i = colvar; i < dim; ++i) {
        freqcol += varwidth[i] + 1;
    ...
    if (tab_labels) {
        ...
        if (celloption == 1 || celloption == 2) {
            strcat(x, " Sums of ");
        } else {
            strcat(x, " Mean and SD of ");
        }
    ...
    while (lin < nlines) {
        ...
        if (celloption == 0) {
            tline_write("Mean", x, meancol);
        } else {
            tline_write("Sum ", x, meancol);
        }
        col = freqcol + colwidth;
        for (i = 0; i < m; ++i) {
            if (f[z] == 0 && celloption == 0) {
                strcpy(value, "-");
            } else {
                if (celloption == 0) {
                    a = sum[z] / (double)f[z];
        ...
        if (celloption == 0) {
            ...
            for (i = 0; i < m; ++i) {
                if (f[z] < 2) {
                    ...
                    a = sqrt((sum2[z] - sum[z] * sum[z] / (double)f[z]) / (double)(f[z] - 1));
                    if (*cellformat) {
                        fconv(a, cellformat, value);
                    } else {
                        fnconv(a, accuracy + 2, value);
...
static int table_to_survo_data_file(char *filename) {
    ...
    **************************/
    ...
    sur_print(sbuf);
    fi_create(name, rec_len, m1, m, (long)size, 64, 8, textn, textlen, text, d_varname, d_varlen, d_vartype);
    ...
    while (1) {
        ...
        if (cellvar >= 0) {
            if (celloption > 0) // RS ADD
            {
                ...
                if (f[n] == 0) {
                    fi_save(&dat, (long)(n + 1), dim + 1, &MISSING8);
                } else {
                    mean = sum[n] / (double)f[n];
                    fi_save(&dat, (long)(n + 1), dim + 1, &mean);
                }
                if (f[n] < 2) {
                    fi_save(&dat, (long)(n + 1), dim + 2, &MISSING8);
                } else {
                    sd = sqrt((sum2[n] - sum[n] * sum[n] / (double)f[n]) / (double)(f[n] - 1));
