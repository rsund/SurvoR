...
int mat_svd_rank(double *X, int mX, int nX, double eps) {
    ...
    svd_eps = 1e-16;
    tol = (1e-300) / eps; // RS 6.9.2013 Switch eps and tol
...
int mat_intval(double *aa, int m, double feps, int nkonv) {
    ...
    for (i = 0; i < m; ++i) {
        ...
        if (aa[i] == 1.0) {
            ...
            k = ds_ratio(fabs(aa[i]), &d[i], &s[i], nkonv, &an);
        }
...
int mat_mmt(double *T, double *X, int m, int n) {
    ...
    i = mat_transp_in_situ(X, m, n);
    if (i < 0) {
        return (-1);
    }
    mat_mtm(T, X, n, m);
    i = mat_transp_in_situ(X, n, m);
    if (i < 0) {
        return (-1);
    }
    return (1);
 ...
 */
int mat_mlt(double *T, double *X, double *Y, int m, int n, int r) {
    int i, j, k;
...
int mat_nonsymm_eigen(double *a, double *t, double *u, int n, int iter, double ep, int n_small) {
    double eps, aii, aij, aji, h, g, hj, aik, aki, aim, ami, tep, tem, d, c, e, akm, amk, cx, sx, cot2x, sig, cotx,
        cos2x, sin2x, te, tee, yh, den, tanhy, chy, shy, c1, c2, s1, s2, tki, tmi, tik, tim;
    ...
    for (it = 0; it < iter; ++it) {
        ...
        for (i = 0; i < nless1; ++i) {
            ...
            for (j = i + 1; j < n; ++j) {
                aij = a[i + n * j];
                aji = a[j + n * i];
                if (fabs(aij + aji) > eps || (fabs(aij - aji) > eps && fabs(aii - a[j * (n + 1)]) > eps)) {
                    jatka = 1;
                    break;
                }
            }
...
int mat_solve(double *x, double *a, double *b, int m, int n, int k, double eps) {
    ...
    if (tyyppi == 20) {
        return (solve_diag(x, a, b, m, k, eps));
    }
    if (tyyppi == 10) {
        return (solve_symm(x, a, b, m, k, eps));
    }
    if (tyyppi == 9) {
        return (solve_upper(x, a, b, m, k, eps));
    }
    if (tyyppi == 8) {
        return (solve_lower(x, a, b, m, k, eps));
    }
