...
static int momentit() {
    ...
    for (i = 0; i < m; ++i) {
        ...
        if (A[i + m * i] == 0.0) {
            muste_sprintf(sbuf, "Variable %s is a constant %g!", d.varname[d.v[i]], sum[i]);
            if (etu == 2) {
    ...
    for (i = 0; i < m; ++i) {
        sum2[i] /= sqrt(nsuhde * weightsum);
    }

    return (1);
}

static int corrp_linreg(double S[], int m,      /* dimensio */
...
static int tulostus() {
    ...
    char rivi[LLENGTH];
    for (i = 0; i < LLENGTH; i++) {
        rivi[i] = 0; // RS ADD 16.10.2012
    }
    i = output_open(eout);
    if (i < 0) {
        return (-1);
    }
    muste_sprintf(rivi, "Means, std.devs and correlations of %s  N=%ld", word[1], n);
    if (painomuuttuja >= 0) {
        strcat(rivi, " Weight=");
...
static int save_corr(double *A, int m) {
    ...
    ***************************/
    ...
    for (i = 0; i < mm; ++i) {
        lab_nimi += 8; // RS ADD
        muste_fieldcopy(lab_nimi, d.varname[d.v[xvariable[i - 1]]], 8);
        lab_nimi[8] = EOS; // RS CHA
...
static int laske_invc() {
    ...
    for (i = 0; i < nxvar; ++i) {
        ...
        for (j = 0; j < nxvar; ++j) {
            jj = xvariable[j];
            invc += sum[ii] * sum[jj] * invR[i + nxvar * j] / sum2[ii] / sum2[jj];
        }
    }
...
static int save_lg_matrix() {
    ...
    *text = EOS;
    ...
    matrix_save0("LG.M", rg, N_RG, 1, label, "LINREG  ", 8, 8, -1, "LG.M", N_LINES, 0, text);
    return (1);
}
...
static int residuals(int k) {
    ...
    sur_print("\nSaving ");
    if (resvar >= 0) {
        muste_sprintf(sbuf, "\nresiduals as variable %.8s...", d.varname[resvar]);
        sur_print(sbuf);
    }
    if (predvar >= 0) {
        muste_sprintf(sbuf, "\npredicted values of model as variable %.8s...", d.varname[predvar]);
        sur_print(sbuf);
    }

    i = data_to_write(aineisto, &d);
    if (i < 0) {
        return (-1);
    }
    ...
    /* 11.10.1996 */
    if (resvar >= 0) {
        update_varname(&d, resvar, "Residual from LINREG");
    }
    if (predvar >= 0) {
        update_varname(&d, predvar, "Predicted value from LINREG");
    }
...
static int regr_talletus() {
    ...
    for (i = 1; i < m2; ++i) {
        muste_fieldcopy(name, d.varname[d.v[xvariable[i - 1]]], 8);
        name[8] = EOS;
        for (h = 0; h < 8; ++h) {
            if (name[h] == EOS) {
                break;
    ...
    for (i = 1; i < m2; ++i) {
        reg[i] = b[i - 1];
        reg[i + m2] = reg_stddev[i];
    }

    sur_print("\nSaving regression coefficients in REG.M");
    strcpy(expr, "regr(");
    strcat(expr, aineisto);
    strcat(expr, ")");
...
static int corr_ind(int i, char *lab, int len, int dim, char *filename) {
    ...
    muste_fieldcopy(nimi, d.varname[d.v[i]], 8);
    nimi[8] = EOS;
    k = 8;
    while (nimi[k - 1] == ' ') {
        nimi[--k] = EOS;
    ...
    for (h = 0; h < dim; ++h) {
        muste_fieldcopy(nimi2, lab + h * len, len);
        nimi2[8] = EOS;
        k = 8;
        while (nimi2[k - 1] == ' ') {
            nimi2[--k] = EOS;
...
void muste_linreg(char *argv) {
    ...
    *****************/
    ...
    if (painomuuttuja >= 0 && !scale_ok(&d, painomuuttuja, RATIO_SCALE)) {
        muste_sprintf(sbuf, "\nWeight variable %.8s has not ratio scale!", d.varname[painomuuttuja]);
        sur_print(sbuf);
        WAIT;
    ...
    for (i = 0; i < d.m; ++i) {
        ...
        if (ch == 'Y') {
            if (nyvar == YMAX) {
                muste_sprintf(sbuf, "\nToo many (more than %d) regressands (Y)", YMAX);
                sur_print(sbuf);
                WAIT;
                return;
            }
    ...
    if ((long)(nxvar + 1) >= n1) {
        ...
        sur_print(sbuf);
        muste_sprintf(sbuf, "%d required at least for the current model.", nxvar + 2);
        sur_print(sbuf);
        WAIT;
        return;
    }
