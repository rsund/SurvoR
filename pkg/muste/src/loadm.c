...
void muste_loadm(int argc, char *argv[]) {
    ...
    s_init(argv[1]);
    if (muste_strcmpi(word[0], "POSDIR") == 0) {
        op_posdir();
        s_end(argv[1]);
        return;
    }
    ...
    if (i >= 0) {
        ...
        i = split(x, osa, 32);
        if (i != nlimit) {
            muste_sprintf(sbuf, "\n%d items in LIMITS, but %d items in SHADOWS!", nlimit, i);
            sur_print(sbuf);
            WAIT;
            return;
        }
    ...
    if (*form == '(') /* (accuracy) */
    {
        ...
        *p = EOS;
        if (form[1] == 'C') {
            ...
            matrix_format(form, atoi(form + 1), A, m, n);
        }
    }
    ...
    */
    ...
    if (nlimit == -1) {
        ...
        lim_or_sha = limit; // 24.6.2007
    }

    matrix_print2(A, m, n, rlab, clab, lr, lc, m2, n2, pmv, pnv, form, k, resline, eout, expr, nlimit, lim_or_sha,
                  shadow, c_form, &one_block);
    output_level = k;
...
static int row_sort(char *spbi, double *A, int m, int n, char *rlab, char *clab, int lr, int lc) {
    ...
    min_loading = 0.0;
    if (i > 1) {
        min_loading = atof(s_word[1]);
    }
    additional_negsort = 0;
    if (*pname == '-') {
        additional_negsort = 1;
        ++pname;
    }
    if (strcmp(pname, "1") != 0) {
...
int matrix_print2(double *A, int m, int n, char *rlab, char *clab, int lr, int lc, int m2,
                  int n2,           /* # of selected rows/cols */
                  int *mv, int *nv, /* lists of selected rows/cols */
                  char *form,       /* format for single element 123.12 or %5.5g  */
                  int width,        /* entire printing width */
                  int editline, char *outfile, char *header, int nlimit, double *limit, char *shadow, char **c_form,
                  int *p_one) {
    int i, j, k, h, ii, jj;
    int len;
    int nblock;
    ...
    if (nlimit == -1) {
        bb = limit; // 24.6.2007 SHADOW_MATRIX
    }
...
int matrix_format(char *muoto, int minlev, double *A, int m, int n) {
    ...
    if (max) {
        lev = lev2 = (int)(log(max) / log((double)10)) + 1;
    }
    if (lev < minlev) {
        lev = minlev;
    }
