...
static void print_line(void) { edwrite(sbuf, results_line++, 1); }
...
static int is_magic(int dim) {
    ...
    if (nodia > 0) {
        nomagic = -100; // Semi-magic (only cols and rows add up to magic sum)
    }
...
static int rnd(int min, int max) { return ((unsigned int)(uniform_dev() * (max - min) + min)); }
