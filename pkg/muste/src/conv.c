...
static char *e10[] = {"ten ",     "eleven ",  "twelve ",    "thirteen ", "fourteen ",
                      "fifteen ", "sixteen ", "seventeen ", "eighteen ", "nineteen "};
static char *ekym[] = {"", "", "twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety"};
static char *epot[] = {"", "x", "hundred "};
static char *enumero[] = {"zero ", "one ", "two ", "three ", "four ", "five ", "six ", "seven ", "eight ", "nine "};
...
static char *snumero[] = {"nolla ", "yksi ",  "kaksi ",   "kolme ",   "nelja ",
                          "viisi ", "kuusi ", "seitsem ", "kahdeks ", "yhdeks "};
...
int muste_checkmp() // RS
{
    ...
    if (!muste_requirepackage("Rmpfr")) {
        return (FALSE);
    }
    if (!muste_requirepackage("gmp")) {
        return (FALSE);
    }
...
int muste_isnumber_dec(const char *s, char dec) // RS 11.3.2013
{
    ...
    q = (char *)sbuf;
    while (*q == ' ') {
        q++; // RS Remove pre-spaces
    }
    p = (char *)(q + strlen(q) - 1);
    while (*p == ' ') {
        *p-- = EOS; // RS Remove post-spaces
    }
    p = q;
    while (*p != EOS) {
        if (*p == dec) {
    ...
    */
    ...
    if (i == 2) // RS i=2 from base_atol above indicates multiple precision
    {
        ...
        for (i = 2; i <= k; i++) {
            ...
            muste_evalr(sbuf);
            muste_get_R_string(factres, ".muste$mpoutstr.comp", LLENGTH);
            if (strcmp(factres, factresold) == 0) {
                j++;
            } else {
                if (j > 1) {
                    muste_sprintf(sbuf, "^%d", j);
                    strcat(res, sbuf);
                    j = 1;
                }
                strcat(res, "*");
...
static int numeric(char *word, char *par1, char *par2, char *res, char laji1, char laji2, char *xpar1, char *xpar2) {
    ...
    *base1 = *base2 = EOS;
    if (laji1 == 'X' && *xpar1 == 'N') {
        strcpy(base1, xpar1 + 1);
    }
    if (laji1 == '0') {
        strcpy(base1, par1);
    }
    if (laji2 == 'X' && *xpar2 == 'N') {
        strcpy(base2, xpar2 + 1);
    }
    if (laji2 == '0') {
        strcpy(base2, par2);
    }
    ...
    if (*base1 == EOS || *base2 == EOS) {
        conversion_impossible(par1, par2);
        return (-1);
    }
...
static void diss(double x, double ear, long *pm, long *pn) {
    ...
    f = 1e10;
    m = k = 0;
    a = pow(10.0, ear); /* k vastaa parametria n */
    while ((double)k < f) {
        ++m;
        k = m * x;
        diss = m * x - k;
...
static int ratio(char *word, char *par1, char *par2, char *res, char laji1, char laji2, char *xpar1, char *xpar2) {
    double x, ear;
    long m, n;
    char y[LLENGTH];
    char sign[2];
    ...
    i = muste_arit_laske(word, &x);
    if (i < 0) {
        return (-1); // RS CHA 18.10.2013
    }
    //        x=atof(word);
    *sign = EOS;
    if (x < 0.0) {
...
static int times(char *word, char *par1, char *par2, char *res, char laji1, char laji2, char *xpar1, char *xpar2,
                 double prefix1, double prefix2) {
    int i;
    char x[LLENGTH];
    char *min, *sec;
    double a;
...
static int x_conversion(char *word, char *par1, char *par2, char *res, char laji1, char laji2, char *xpar1, char *xpar2,
                        double prefix1, double prefix2) {
    char laji;
    /*
    ...
    */
    ...
    switch (laji) {
    case 'T':
        return (temperatures(word, par1, par2, res, xpar1, xpar2));
    case 'C':
        return (currencies(word, par1, par2, res, xpar1, xpar2));
    }
    if (*xpar1 == 't' || *xpar2 == 't') {
...
static void qedread(char *s, int j) {
    ...
    for (i = 0; i < qed1; ++i) {
        s[i] = (unsigned char)getc(measures); // RS ADD unsigned
    }
    s[qed1] = EOS;
}
...
static int etsi(unsigned char *s1) {
    ...
    while (1) {
        ...
        j = (alku + loppu) / 2;
        qedread(avainrivi, j);
        muste_fieldcopy(avain, avainrivi + 1, col2 - 1);
        avain[col2 - 1] = EOS;
...
static int load_codes(unsigned char *code) {
    ...
    strcat(nimi, "SYS/SORTLOW.BIN"); // RS \\ -> /
...
static int tutki_prefix(char *par, double *pprefix) {
    if (strncmp(par, "exa", 3) == 0) {
        *pprefix = 1e18;
        return (3);
    }
    if (strncmp(par, "peta", 4) == 0) {
        *pprefix = 1e15;
        return (4);
    }
    if (strncmp(par, "tera", 4) == 0) {
        *pprefix = 1e12;
        return (4);
    }
    if (strncmp(par, "giga", 4) == 0) {
        *pprefix = 1e9;
        return (4);
    }
    if (strncmp(par, "mega", 4) == 0) {
        *pprefix = 1e6;
        return (4);
    }
    if (strncmp(par, "kilo", 4) == 0) {
        *pprefix = 1e3;
        return (4);
    }
    if (strncmp(par, "hecto", 5) == 0) {
        *pprefix = 1e2;
        return (5);
    }
    if (strncmp(par, "hehto", 5) == 0) {
        *pprefix = 1e2;
        return (5);
    }
    if (strncmp(par, "deca", 4) == 0) {
        *pprefix = 10.0;
        return (4);
    }
    if (strncmp(par, "deka", 4) == 0) {
        *pprefix = 10.0;
        return (4);
    }
    if (strncmp(par, "deci", 4) == 0) {
        *pprefix = 0.1;
        return (4);
    }
    if (strncmp(par, "desi", 4) == 0) {
        *pprefix = 0.1;
        return (4);
    }
    if (strncmp(par, "centi", 5) == 0) {
        *pprefix = 1e-2;
        return (5);
    }
    if (strncmp(par, "sentti", 6) == 0) {
        *pprefix = 1e-2;
        return (6);
    }
    if (strncmp(par, "milli", 5) == 0) {
        *pprefix = 1e-3;
        return (5);
    }
    if (strncmp(par, "micro", 5) == 0) {
        *pprefix = 1e-6;
        return (5);
    }
    if (strncmp(par, "mikro", 5) == 0) {
        *pprefix = 1e-6;
        return (5);
    }
    if (strncmp(par, "nano", 4) == 0) {
        *pprefix = 1e-9;
        return (4);
    }
    if (strncmp(par, "pico", 4) == 0) {
        *pprefix = 1e-12;
        return (4);
    }
    if (strncmp(par, "piko", 4) == 0) {
        *pprefix = 1e-12;
        return (4);
    }
    if (strncmp(par, "femto", 5) == 0) {
        *pprefix = 1e-15;
        return (5);
    }
    if (strncmp(par, "atto", 4) == 0) {
        *pprefix = 1e-18;
        return (4);
    }
    if (*par == 'k') {
...
static int num_conversion(char *word, char *par1, char *par2, char *res) {
    ...
    *res = EOS;
    ...
    if (!i) {
        strcpy(nimi, survo_path);
        strcat(nimi, "SYS/MEASURES.EDT");
    } // RS CHA \\ -> /
    i = avaa(nimi);
    i = mitta(par1, &laji1, kerroin1, &prefix1);
    if (i < 0) {
        return (-1);
    }
    i = mitta(par2, &laji2, kerroin2, &prefix2);
    if (i < 0) {
        return (-1);
    }
    if (laji1 == 'X' || laji2 == 'X') {
...
static int roman_to_int(char *roman, int *pk) {
    ...
    while (*p) {
        ...
        if (q == NULL) {
            muste_sprintf(sbuf, "\n%c not allowed in roman numerals!", *(roman + (p - x)));
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
...
static int integer_conversion(char *word, char *par1, char *par2, char *res) {
    ...
    *res = EOS;
    ...
    if (muste_strnicmp(par2, "ASCII", 5) == 0) {
        if (luku > 255L) {
            sur_print("\nMax. ASCII value is 255");
            return (-1);
        }
        *res = (unsigned char)(int)luku;
        res[1] = EOS;
        return (1);
...
static void syntax_error(char *lauseke, char *ilm) {
    muste_sprintf(sbuf, "\nSyntax error in  %s: %s", lauseke, ilm);
    sur_print(sbuf);
    WAIT;
    return;
}
