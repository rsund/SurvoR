...
void muste_classify(char *argv) {
    ...
    i = spfind("RAW");
    if (i >= 0) {
        muste_rawclass = atoi(spb[i]); // RS 11.3.2013
    }

    sp_char = EOS;
    i = spfind("SPACE"); /* 12.2.1992 */
    if (i >= 0) {
        sp_char = *spb[i];
    }
    wild = EOS;
    ...
    if (outtype != 'S') {
        for (i = 0; i < nclass; ++i) {
            if (*class[i] == EOS) {
                ...
                numclass[i] = atof(class[i]);
            }
        }
    }
    if (others != NULL) {
        if (strcmp(others, "MISSING") == 0) {
            othvalue = MISSING8;
            *others = EOS;
        } else if (strcmp(others, "SAME") == 0) {
            othersame = 1; /* 2.10.1993 */
        } else if (strcmp(others, "NO_CHANGE") == 0 || strcmp(others, "NO CHANGE") == 0) {
            othersame = 2; // 30.9.2000
                           // syy: SPACE=_
        } else {
            othvalue = atof(others);
        }
    }
...
static int tutki_luokitus() {
    ...
    while (1) {
        ...
        if (k == 0 || strcmp(osa[0], "END") == 0 || strncmp(osa[0], "END ", 4) == 0) {
            break; // RS 11.3.2013 strncmp
        }
        if (k < 2) {
            line_error(j);
            return (-1);
        ...
        classtype[nclass] = 0;
        if (muste_rawclass) {
            q = NULL; // RS 11.3.2013 10.5.2013 REM !
        } else {
            q = strchr(osa[0], '-');
        }
...
static int create_outvar() {
    ...
    if (others != NULL && strcmp(others, "SAME") == 0) {
        ...
        for (i = 0; i < nclass; ++i) {
            ...
            if (!muste_isnumber(class[i])) {
                num = 0;
            }
            if (*class[i] == '-') {
                neg = 1;
            }
            if (strchr(class[i], '.') != NULL) {
                des = 1;
            }
            if (strlen(class[i]) > len) {
                len = strlen(class[i]);
            }
            x = fabs(atof(class[i]));
            if (x > max) {
    ...
    if (!sametype) {
        ...
        if (p != NULL) {
            *p = EOS;
            ...
            if (num) {
                if (!neg && !des && max < 255.0) {
                    type = '1';
                    len = 1;
                } else if (!des && max < 32767.0) {
                    type = '2';
                    len = 2;
                } else {
                    type = '4';
        ...
        for (j = d.l1; j <= d.l2; ++j) {
            ...
            if (i == nclass) {
                if (others) {
                    ...
                    {
                        y = x;
                        muste_sprintf(other_str, "%g", x);
                        py = other_str; /* 22.1.1996 */
                    } else {
                        y = othvalue;
                        py = others;
                    }
                } else {
                    muste_sprintf(sbuf, "\nValue %g in observation #%ld cannot be classified!", x, j);
                    sur_print(sbuf);
                    WAIT;
                    return (-1);
                }
            ...
            if (outtype != 'S') {
                ...
                if (d.type == 2) {
                    ...
                    if (k > 0) {
                        strncat(sbuf, space, k);
                    }
                    fi_alpha_save(&d.d2, j, outvar, sbuf);
                } else if (d.type == 1) {
                    ma_save(&d.d1, (int)j, outvar, py);
                } else {
        ...
        for (j = d.l1; j <= d.l2; ++j) {
            ...
            for (i = 0; i < nclass; ++i) {
                if (classtype[i] == 0) {
                    ...
                    if (strncmp(lower[i], jakso, strlen(lower[i])) == 0) {
                        break;
                    }
                    if (wild) {
                        if (w_strncmp(lower[i], jakso, strlen(lower[i]), wild) == 0) {
                            break;
                        }
                    }
                    if (partial) {
                        if (*lower[i] == partial && strstr(jakso, lower[i] + 1) != NULL) {
                            break;
                        }
                    }
                    continue;
                ...
                if (strncmp(jakso, lower[i], strlen(lower[i])) >= 0 &&
                    strncmp(jakso, upper[i], strlen(upper[i])) <= 0) {
                    break;
                } else {
                    continue;
                }
            ...
            if (i == nclass) {
                if (othersame == 2) // NO_CHANGE 30.9.2000
                {
                    ...
                    muste_sprintf(sbuf, "\nValue %s in observation #%ld cannot be classified!", jakso, j);
                    sur_print(sbuf);
                    WAIT;
                    return (-1);
                }
