...
static int space_allocation1() { return (1); }
...
static int residuals() {
    ...
    if (std_residual < 0 && resvar < 0 && predvar < 0) {
        return (1);
    }

    sur_print("\nSaving ");
    if (resvar >= 0) {
        muste_sprintf(sbuf, "\nlms-residuals as variable %.8s...", d.varname[resvar]);
        sur_print(sbuf);
    }
    if (std_residual >= 0) {
        muste_sprintf(sbuf, "\nstandardized lms-residuals as variable %.8s...", d.varname[std_residual]);
        sur_print(sbuf);
    }
    if (predvar >= 0) {
        muste_sprintf(sbuf, "\nlms-predicted values of model as variable %.8s...", d.varname[predvar]);
        sur_print(sbuf);
    }
...
static int printout() {
    ...
    char coefficient[32];
    output_open(eout);
    muste_sprintf(line, "Least Median of Squares Regression: Data %s   N=%d", word[1], n);
    print_line(line);
        ...
        for (j = 0; j < n_total; ++j) {
            ...
            if (xxx > a) {
                ...
                if (xxx > xm1) /* xm1=a:ta lähinnä pienin luku */
                {
                    ...
                    xm2 = xxx; /* xm2=a:ta lähinnä 2. pienin luku */
                }
                if (xxx == a) {
                    ++na;
                }
