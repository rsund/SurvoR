...
static int glm_fit(double (*flink)(double), double (*filink)(double), double (*dfilink)(double), double (*vf)(double),
                   double (*devf)(double, double), double (*fscale)(double, double), double *X, int nx, int mx,
                   double *Y, double *W, double *V, char *lab, double *b, double *sb, double *my, double *pdev,
                   int *pdf);
...
static int lue_temp() {
    ...
    for (j = 0; j < n; ++j) /* n0 aikaisemmin! */
    {
        ...
        Y[j] = temp_load();
        for (i = 0; i < m1; ++i) {
            X[(int)u[i] * (int)n0 + (int)j] = temp_load();
        }
    }
...
static int fit() {
    ...
    *lab = EOS;
    for (i = 0; i < m; ++i) {
        strncat(lab, d.varname[xvar[i]], 8);
    }
    for (j = 0; j < n; ++j) {
        V[j] = Y[j];
    }
    ...
    i = glm_fit(flink, filink, dfilink, vf, devf, fscale, X, n, m, Y, W, V, lab, b, sb, my, &deviance, &df);
    k = output_open(eout);
    if (k < 0) {
        return (-1);
    }
    ...
    muste_sprintf(s, "Data %s: Deviance=%s df=%d", word[1], spois(s1), df);
    print_line(s);
    // i=8; muste_fieldcopy(s1,d.varname[yvar],8); while (s1[i-1]==' '&i>0) --i;
    // 9.8.2011/SM
    i = 8;
    muste_fieldcopy(s1, d.varname[yvar], 8);
    while (i > 0 && s1[i - 1] == ' ') {
        --i;
    }
    ...
    if (results > 0) {
        ...
        for (i = 0; i < m; ++i) {
            if (sb[i] < 0) {
                muste_sprintf(s1, " -      %.*s", accuracy - 4, space);
                muste_sprintf(s2, " aliased%.*s", accuracy - 4, space);
            } else {
                fnconv(b[i], accuracy + 2, s1);
...
static int regr_talletus() {
    ...
    muste_fieldcopy(name, d.varname[yvar], 8);
    name[8] = EOS;
    strcpy(expr, "genreg(");
    strcat(expr, data);
    strcat(expr, ")");
...
static double idn(double x) { return (x); }
static double one(double x) { return (1.0); }
static double log0(double x) {
    if (x < 0.0) {
        math_error = 1;
        sur_print("\nLINK=LOG: Negative values not allowed!");
        WAIT;
        return (0.0);
    }
    if (x == 0.0) {
...
static double twice(double y) { return (2 * y); }
...
static double logit(double y) {
    ...
    if (y > a || y < 0.0) {
        math_error = 1;
        sur_print("\nIllegal value in logit transformation!");
        WAIT;
        return (0.0);
    }
...
static double probitt(double y) {
    ...
    if (y > a || y < 0.0) {
        math_error = 1;
        sur_print("\nIllegal value in probit transformation!");
        WAIT;
        return (0.0);
    }
...
static double complog(double y) {
    ...
    if (y > a || y < 0.0) {
        math_error = 1;
        sur_print("\nIllegal value in complementary log-log transformation!");
        WAIT;
        return (0.0);
    }
...
static double sqroot(double y) {
    if (y < 0.0) {
        math_error = 1;
        sur_print("\nNegative argument in square root!");
        WAIT;
        return (0.0);
    }
    return (sqrt(y));
...
static double iexponent(double y) { return (pow(y, 1.0 / alpha)); }
static double diexponent(double y) { return (pow(y, 1.0 / alpha - 1.0) / alpha); }

static double digamma(double y) { return (-1.0 / (y * y)); }

static double devlog(double y, double my) { return (2 * (y * log(y / my) - (y - my))); }
static double devlin(double y, double my) { return ((y - my) * (y - my)); }
static double devbin(double y, double my) {
    double a, y1, y2;
...
static double devgamma(double y, double my) { return (2 * (-log(y / my) + (y - my) / my)); }

static double scale1(double dev, double df) { return (1.0); }
static double scale_normal(double dev, double df) { return (dev / df); }
...
static int glm_fit_space(int mx, int nx) {
    ...
    **********************/
    ...
    if (lab_fit != NULL) {
        lab_fit = (char *)muste_realloc(lab_fit, 8 * mx + 1);
    } else {
        lab_fit = (char *)muste_malloc(8 * mx + 1);
    }
    if (lab_fit == NULL) {
        not_enough_memory();
        return (-1);
