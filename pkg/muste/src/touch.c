...
static void tutsave(int m) { putc(m, tutor); }
...
static int headline_touch() {
    ...
    muste_sprintf(sbuf, "%c%c%4u%3u SURVO TOUCH MODE   ", pref, dispm2, c1 + c - 1, c1);
    sur_print(sbuf);
    k = c3 - 72;
    muste_sprintf(sbuf, "%s     %.5s   %*.*s%7d%5d ", aika, mode[s], k, k, space, r2, c2);
    sur_print(sbuf);
    //      muste_sprintf(sbuf,"%s     %.5s   %7d%5d ",aika,mode[s],r2,c2); sur_print(sbuf);
    PR_EINV;
    muste_sprintf(sbuf, "%c", key);
    sur_print(sbuf);
    PR_ENRM;
    return (1);
}
...
static int tutch_touch() {
    ...
    if (!feof(tutor)) {
        if (sur_kbhit()) // RS FIXME ei pitäisi tulla tänne
        {
            ...
            default:
                break; // RS CHA FIXME  while(!sur_kbhit()) ; getck(); break;
            }
        }
        special = 0;
        if (etu1 > 1 && etu2 != 2) {
        ...
        if (etu2 > 0) {
            ...
            if (etu2 == 1) {
                if (etu1 > 1) {
                    sur_wait((long)tut_wait_c * etu1, Wdisp, 0);
                }
            } else {
                press_key(m);
            }
...
static int touch() {
    ...
    while (haku) {
        ...
        if ((z[j1] == '+' || z[j1] == '-') && (z[j1 - 1] == 'e' || z[j1 - 1] == 'E'))
        /* 27.8.94 */
        {
            j1 -= 2;
            continue;
    ...
    haku = 1;
    while (haku) {
        if ((z[j2] == 'E' || z[j2] == 'e') && (isdigit((int)z[j2 + 1]) || z[j2 + 1] == '+' || z[j2 + 1] == '-')) {
            j2 += 2;
            continue;
        }
        if ((z[j2] == '+' || z[j2] == '-') && (z[j2 - 1] == 'E' || z[j2 - 1] == 'e')) {
            ++j2;
            continue;
        }
    ...
    muste_fieldcopy(tsana, z + j1, j2 - j1 + 1);
    tsana[j2 - j1 + 1] = EOS;
    if (r > r3 || r < 1 || tdisp == 0) {
        return (1);
    }
    ...
    if ((int)(j1 - jmin - c1 + 2) < 1) {
        return (1); /* miksi (int) tarvitaan??? 24.7.88 */
    }
...
static void print_res(int m) /* m on = tai ! tai ; */
{
    ...
    jmax = jmin + ed1 - 2;
    i = 0;
    j = j0 = jmin + c1 + c - 1 - piste;
    if (jmin + c1 + c - 1 <= piste) {
        return; /* 29.5.89 liikaa kentÑn alussa */
    }
    if (j < jmin) {
        j = jmin;
    }
    ...
    if ((int)(j1 - jmin + 1) < c1) {
        j1 = jmin + c1 - 1; /* (int) 24.7.88 */
    }
    if ((int)(j2 - jmin + 1) > c1 + c3 - 1) {
        j2 = jmin - 1 + c1 + c3 - 1;
    }
    ...
    if (r <= r3 && r > 0 && c <= c3 && c > 0 && tdisp == 1 && j1 <= j2) {
        if (ibm) {
            ...
            muste_fieldcopy(x, z + j1, j2 - j1 + 1);
            x[j2 - j1 + 1] = EOS;
...
static int touch_data_open(char *tsana) {
    ...
    if (!muste_is_path(tsana)) {
        strcpy(tch_data, edisk);
        strcat(tch_data, tsana);
    }
...
static int collect_lines() {
    ...
    PR_EINV;
    muste_sprintf(sbuf, "%.*s", c3 - c + 1, p);
    sur_print(sbuf);
    PR_ENRM;
    if (zs[j] == 0) {
        *x = EOS;
        strncat(x, space, c2 - col);
    } else {
        edread(x, zs[j]);
    }
    fprintf(collect_file, "%s\n", x + col);
    ++n_lines;
    muste_sprintf(sbuf, "Collecting lines: %d   (Print collected lines by `='. Exit by ENTER)", n_lines);
    touch_res(sbuf);
    return (1);
}
...
static int worm_mode(int vaihe, int m) /* int m;  edell. nappi */
{
    ...
    if (vaihe == 0) {
        ...
        case 0:
            ...
            wprevr = wprevc = -1;
            if (i > 0) {
                touch_res("Define a worm by arrow keys! Stop definition by alt-F2!");
            }
            return (i);
        case 1:
            worm = 2;
    ...
    if (m == 'D') {
        display_off = 1 - display_off;
        return (1);
    }
    if (m == 'E') {
        erase_by_worm = 1 - erase_by_worm;
        return (1);
    }
    if (m == 'e') {
        erase_temporarily = 1 - erase_temporarily;
        return (1);
    }
    ...
    case 1:
        if (wn >= nmax) {
            muste_sprintf(sbuf, "\nToo long worm (more than %d characters)!", nmax);
            PR_EINV;
            sur_print(sbuf);
            WAIT;
            worm = 0;
        ...
        if (zs[wrr] == 0) {
            ...
            wsh[wn] = *(z + (zs[wrr] - 1) * ed1 + wcc);
        }
        if (!worm_shad) {
            write_string(&wch[wn], 1, '8', r + 1, c + 8);
        }
        ++wn;
        break;
    case 2:
        ...
        for (i = wn - 1; i >= 0; --i) {
            ...
            if (rr < r3 + 1) {
                write_string(&wch[i], 1, sh, rr + 1, cc + 8);
            }
            --ii;
            if (ii < 0) {
                ii = wn - 1;
        ...
        if (m != 'R') {
            ...
            if (erase_temporarily) {
                ch = ' ';
            }
            if (zs[wpr] == 0) {
                sh = ' ';
            } else {
                sh = *(z + (zs[wpr] - 1) * ed1 + wpc);
            }
            if (rr < r3 + 1) {
                write_string(&ch, 1, sh, rr + 1, cc + 8);
            }
        }
        ++wf;
        if (wf == wn) {
...
static void tfilename(char chainfile[], char tchain[]) {
    *chainfile = EOS;
    if (!muste_is_path(tchain)) {
        strcat(chainfile, edisk);
    }
...
static int compf(char f[]) {
    ...
    if (strncmp(F, "RND", 3) == 0) {
        opnd[0] = uniform(opnd[0]);
        return (1);
    }
    if (strncmp(F, "PROBIT", 3) == 0) {
        opnd[0] = probit(opnd[0]);
        return (1);
    }
    ...
    muste_sprintf(sbuf, "Error in function %s!", f);
    sur_print(sbuf);
    WAIT;
    return (-1);
}
...
static void key_M() {
    ...
    if (s != 2) {
        ...
        if (M_ind == 0) {
            ...
            s = 0;
            eval();
            fconv(opnd[0], "", memsana);
            strcpy(x, "Save result ");
            strcat(x, memsana);
        ...
        strcpy(x, "0: ");
        strcat(x, memsana);
        strcat(x, " saved in memory location ");
        fconv((double)k, "", tsana);
        strcat(x, tsana);
        touch_res(x);
...
static void std_keys(int m) {
    ...
    case 'W':
        ...
        strcpy(wsana, tsana);
        if (tch_open) {
            touch_data_save(wsana);
        }
        take_also_plus = 0;
        break;
    case 'B':
        *wsana = *(z + ed1 * (r1 + r - 2) + c1 + c - 1);
        wsana[1] = EOS;
        PR_EINV;
        muste_sprintf(sbuf, "%c", *wsana);
        sur_print(sbuf);
        PR_ENRM;
        if (tch_open) {
            touch_data_save(wsana);
    ...
    case 'E':
        ...
        PR_EINV;
        muste_sprintf(sbuf, "%.*s", c3 - c + 1, wsana);
        sur_print(sbuf);
        PR_ENRM;
        if (tch_open) {
            touch_data_save(wsana);
...
static void save(char *nimi) {
    ...
    while (kesken) {
        for (h = 0; h < g; ++h) {
            ...
            switch (sana[h][0]) {
            case 'l':
                jatka2(CODE_LEFT, sana[h] + 1, &i);
                break;
            case 'r':
                jatka2(CODE_RIGHT, sana[h] + 1, &i);
                break;
            case 'u':
                jatka2(CODE_UP, sana[h] + 1, &i);
                break;
            case 'd':
                jatka2(CODE_DOWN, sana[h] + 1, &i);
                break;
            case '@':
            case 'F':
            case 'C':
...
static void tsaveload(char *siirtop) {
    ...
    if (muste_strcmpi(word[0], "TCHLOAD") == 0) {
        load(word[1]);
        s_end(siirtop);
    } else if (muste_strcmpi(word[0], "TCHSAVE") == 0) {
        save(word[1]);
    }
}
    ...
    *tch_data = EOS;
    ...
    while (trun) {
        ...
        */
        ...
        key = ' ';
        /* Rprintf("\n%d\n",m); getch(); */
        if (worm && strchr("SCDREe=", (char)m) != NULL) {
            continue;
        }
        if (special) {
            switch (m) {
