...
static int multin_comp(void);
static int multin_comp2(void);
static int multin_comp3(void);
...
void muste_comb(char *argv) {
    ...
    if (!ind_elem && i < 0) /* 1 2 3 4 ... */
    {
        ...
        {
            if (strcmp(symbol[i], symbol[i - 1]) == 0) {
                distinct_symbols = 0;
                break;
            }
        }
    ...
    n_comb = 0L;
    kb_count = 0;
    if (muste_strnicmp(type, "PERM", 4) == 0) {
        permutations();
        tclose();
        s_end(argv);
        return;
    }
    if (muste_strnicmp(type, "SUBS", 4) == 0) {
        subsets();
        tclose();
        s_end(argv);
        return;
    }
    if (muste_strnicmp(type, "PART", 4) == 0) {
        partitions();
        tclose();
        s_end(argv);
        return;
    }
    if (muste_strnicmp(type, "DIST", 4) == 0) {
        distributions();
        tclose();
        s_end(argv);
        return;
    }
    if (muste_strnicmp(type, "INTE", 4) == 0) {
        integers();
        tclose();
        s_end(argv);
        return;
    }
    if (muste_strnicmp(type, "LATT", 4) == 0) {
        lattices();
        tclose();
        s_end(argv);
        return;
    }
    if (muste_strnicmp(type, "R_PERM", 6) == 0) {
        r_permutations();
        tclose();
        s_end(argv);
        return;
    ...
    if (muste_strnicmp(type, "#FIRST", 6) == 0) {
        find_first();
        tclose();
        s_end(argv);
        return;
    }
    if (muste_strnicmp(type, "MULTIN_PROB", 8) == 0) {
        multin_comp();
        tclose();
        s_end(argv);
        return;
...
static int permutations() {
    ...
    if (!restr && !res) {
        ...
        muste_sprintf(sbuf, "Permutations of %d elements: N[%s]=%.15g", n, word[1], dp);
    ...
    edwrite(space, line1, 1);
    muste_sprintf(sbuf, "Permutations of %d elements: N[%s]=%d", n, word[1], n_comb);
    edwrite(sbuf, line1, 1);
    return (1);
}
...
// int *p; /* permutaatio p[0], p[1],..., p[n-1] */
{
    ...
    if (i == 0) {
        return (-1); /* kaikki permutaatiot k?yty l?pi */
    }
    --i;
    j = n - 1;
    pi = p[i];
...
static int permutations2(int n, int m) {
    ...
    // 17.10.2011
    if (!distinct_symbols) {
        permutations_with_repetitions(n, m);
        return (1);
    }
    ...
    if (!res) {
        ...
        edwrite(space, line1, 1);
        muste_sprintf(sbuf, "%d-permutations of %d elements: N[%s]=%.15g", m, n, word[1], dp);
        edwrite(sbuf, line1, 1);
        return (1);
    }
    for (i = 0; i < m; ++i) {
    ...
    edwrite(space, line1, 1);
    muste_sprintf(sbuf, "%d-permutations of %d elements: N[%s]=%d", m, n, word[1], n_comb);
    edwrite(sbuf, line1, 1);
    return (1);
}
...
static int permutations_with_repetitions(int n, int m) {
    ...
    edwrite(space, line1, 1);
    muste_sprintf(sbuf, "%d-permutations of %d elements (with repetitions): N[%s]=%d", m, n, word[1], n_comb);
    edwrite(sbuf, line1, 1);
    return (1);
}
...
static int r_permutations() {
    ...
    edwrite(space, line1, 1);
    muste_sprintf(sbuf, "Permutations of %d elements with restrictions %s: N[%s]=%d", n, matname, word[1], n_comb);
    edwrite(sbuf, line1, 1);
    return (1);
}
...
static int next_restr_perm_from(int k, int n) {
    ...
    for (i = elem1[k] + 1; i < n; ++i) {
        ...
        if (next_restr_perm_from(k + 1, n) == 1) {
            return (1);
        }
    }
    if (i < n) {
...
static int subsets() {
    ...
    if (!res) {
        if (!all) {
            ...
            muste_sprintf(sbuf, "Subsets of size %d of %d elements: N[%s]=%.15g", m, n, word[1], y);
        } else {
            y = pow(2.0, (double)n) - 1;
            muste_sprintf(sbuf, "Non-empty subsets of %d elements: N[%s]=%.15g", n, word[1], y);
        }
        edwrite(sbuf, line1, 1);
        return (1);
    }
    ...
    edwrite(space, line1, 1);
    if (all) {
        muste_sprintf(sbuf, "Non-empty subsets of %d elements: N[%s]=%d", n, word[1], n_comb);
    } else {
        muste_sprintf(sbuf, "Subsets of size %d of %d elements: N[%s]=%d", m0, n, word[1], n_comb);
    }
    edwrite(sbuf, line1, 1);
    return (1);
}
...
static int subsets_with_repetitions(int n, int m) {
    ...
    edwrite(space, line1, 1);
    muste_sprintf(sbuf, "%d-subsets of %d elements (with repetitions): N[%s]=%d", m, n, word[1], n_comb);
    edwrite(sbuf, line1, 1);
    return (1);
}
...
static int partitions() {
    ...
    i = spfind("DISTINCT");
    if (i >= 0) {
        idistinct = atoi(spb[i]);
        rajoitukset += idistinct;
    }
    i = spfind("GREATEST"); // 14.8.2006
    if (i >= 0) {
        greatest_part = atoi(spb[i]);
        if (res == 0) {
            muste_sprintf(sbuf, "\nCompute this by PARTITIONS,%d,%d !", atoi(par[0]), greatest_part);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
    ...
    if (!imax && !imin && !raj2 && res == 0 && all) {
        ...
        edwrite(space, line1, 1);
        muste_sprintf(sbuf, "Partitions of %d: N[%s]=%.15g", n, word[1], da);
        edwrite(sbuf, line1, 1);
        return (1);
    }
    for (m = m1; m <= m2; ++m) {
        ...
        while (1) {
            if (rajoitukset) {
                ok = 1;
                if (imax && elem1[m - 1] > max) {
                    ok = 0;
                }
                if (imin && elem1[0] < min) {
                    ok = 0;
                }
                if (greatest_part && elem1[m - 1] != greatest_part) {
                    ok = 0; // 14.8.2006
                }
    ...
    edwrite(space, line1, 1);
    if (all) {
        muste_sprintf(sbuf, "Partitions of %d: N[%s]=%d", n, word[1], n_comb);
    } else {
        muste_sprintf(sbuf, "Partitions %d of %d: N[%s]=%d", m0, n, word[1], n_comb);
    }
    edwrite(sbuf, line1, 1);
    return (1);
}
...
static double multin_prob1(int n, int m) {
    ...
    k = elem1[0];
    nk = 1;
    lcoeff2 -= comb_lfact[k];
    for (i = 1; i < m; ++i) {
        k2 = elem1[i];
        lcoeff2 -= comb_lfact[k2];
        if (k2 == k) {
            ++nk;
            continue;
        }
        if (nk > 1) {
            lcoeff1 -= comb_lfact[nk];
        }
        k = k2;
        nk = 1;
    }
...
static int tee_comb_lfact(int n) {
    ...
    for (i = 2; i <= n; ++i) {
        comb_lfact[i] = log((double)i) + comb_lfact[i - 1];
    }
    return (1);
}
...
static int min_max_partitions(int n, int m, int min, int max) {
    ...
    if (multin) {
        if (multin == 1) {
            muste_sprintf(sbuf, "Restricted partitions of %d: N[%s]=%.15g P=%.15g", n, word[1], nn, prob);
        } else {
            muste_sprintf(sbuf, "Restricted partitions of %d: N[%s]=%.15g P=%.15g NM=%.15g", n, word[1], nn, prob,
                          sum1);
        }
    } else {
        muste_sprintf(sbuf, "Restricted partitions of %d: N[%s]=%.15g", n, word[1], nn);
    }
    edwrite(sbuf, line1, 1);
    // Rprintf("\nsum1=%.15g",sum1); getch();
    return (1);
    ...
    */
    ...
    if (all) {
        if (res == 0) {
            ...
            muste_sprintf(sbuf, "Partitions of %d: N[%s]=%d", n, word[1], i);
            edwrite(space, line1, 1);
            edwrite(sbuf, line1, 1);
            return (1);
        } else {
...
static int n_dist_part_p(int n, int m) {
    ...
    t = elem2[m - 1];
    elem0[m - 1] = 0;
    s += n_dist_part_p(n, m - 1);
    if (n >= t) {
        elem0[m - 1] = 1;
        s += n_dist_part_p(n - t, m - 1);
    }
    return (s);
}
...
static int distributions() {
    ...
    */
    ...
    edwrite(space, line1, 1);
    if (all) {
        muste_sprintf(sbuf, "Distributions of %d elements: N[%s]=%d", n, word[1], n_comb);
    } else {
        muste_sprintf(sbuf, "Distributions of %d elements into %d cells: N[%s]=%d", n, m0, word[1], n_comb);
    }
    edwrite(sbuf, line1, 1);
    return (1);
}
...
static double laske_multin_prob(int n, int m) // samat p:t ja rajat
{
    ...
    if (m * multin_min > n || m * multin_max < n) {
        return (0.0);
    }
    if (m == 1) {
        return (1.0);
...
static double laske_multin_prob2(int n, int m) // eri p:t, samat rajat
{
    ...
    a = 0.0;
    for (i = multin_min; i <= multin_max; ++i) {
        a += exp(comb_lfact[n] - comb_lfact[i] - comb_lfact[n - i] + i * log(pr1) + (n - i) * log(1.0 - pr1)) *
...
static int multin_comp() {
    ...
    i = spfind("MIN");
    if (i >= 0) {
        multin_min = atoi(spb[i]);
    }
    multin_max = n;
    i = spfind("MAX");
    if (i >= 0) {
        multin_max = atoi(spb[i]);
    }
    ...
    muste_sprintf(sbuf, "Distributions of %d elements into %d cells: P[%s]=%.15g", n, m, word[1], a);
...
static int distributions2(int n, int m) // 13.6.2001
{
    ...
    edwrite(space, line1, 1);
    if (multin) {
        muste_sprintf(sbuf, "Distributions of %d elements into %d cells: N[%s]=%.15g P=%.15g", n, m, word[1], nn, prob);
    } else {
        muste_sprintf(sbuf, "Distributions of %d elements into %d cells: N[%s]=%.15g", n, m, word[1], nn);
    }
...
static int integers() {
    ...
    edwrite(space, line1, 1);
    muste_sprintf(sbuf, "Integers of %d digits in base %d: N[%s]=%d", n, m, word[1], n_comb);
    edwrite(sbuf, line1, 1);
    return (1);
}
...
static int lattices() {
    ...
    edwrite(space, line1, 1);
    muste_sprintf(sbuf, "Lattice points in %d dimensions: N[%s]=%d", n, word[1], n_comb);
    edwrite(sbuf, line1, 1);
    return (1);
}
...
static int print_comb(int *p, int k, int s) {
    ...
    for (i = 0; i < k; ++i) {
        switch (s) {
        case 1:
            h += muste_sprintf(sbuf + h, "%s ", symbol[p[i]]);
            break;
        case 0:
            h += muste_sprintf(sbuf + h, "%d ", p[i]);
            break;
        case 2:
            h += muste_sprintf(sbuf + h, "%s ", symbol2[p[i]]);
            break;
        }
        if (h > LLENGTH - 10) {
            break;
...
static int load_restr_matrix(int n) {
    ...
    if (cdim != rdim || rdim != n) {
        muste_sprintf(sbuf, "\nSize of restriction matrix %s must be %d!", matname, n);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
