...
void fi_gets(SURVO_DATA_FILE *s, char *jakso, muste_int64 pit, muste_int64 paikka) // RS CHA int pit
{
    ...
    // Rprintf("\ngets paikka: %d, max: %d, pit: %d,point: %d, todpoint:
    // %d",(int)paikka,(int)max,(int)pit,(int)(*s).point,muste_ftell((*s).survo_data));
    ...
    if (ero > max) {
        ...
        (*s).point = (muste_int64)ero; // ((long)paikka+(long)pit); // RS CHA (long) -> (int)
    }
...
int fi_find2(char *nimi, SURVO_DATA_FILE *s, char *pathname, int kirjoitus) {
    ...
    muste_append_path(pathname,
                      ".SVO"); // RS CHA if (strchr(pathname+strlen(pathname)-4,'.')==NULL) strcat(pathname,".SVO");

    if (pathname[strlen(pathname) - 1] == '.') {
        pathname[strlen(pathname) - 1] = EOS; // RS ADD
    }
    ...
    */
    ...
    if ((*s).survo_data == NULL) {
        return (-1);
    }
    return (1);
}

int fi_find(char *nimi, SURVO_DATA_FILE *s, char *pathname) { return (fi_find2(nimi, s, pathname, 1)); }
...
int fi_to_write(char *nimi, SURVO_DATA_FILE *s) {
    ...
    if (i < 0) {
        ...
        if (etu == 2) {
            muste_sprintf(tut_info, "___@1@FILE OPEN@%s@", jakso);
            return (-1);
        }
        muste_sprintf(sbuf, "\n%s", jakso);
        sur_print(sbuf);
...
int fi_var_save(SURVO_DATA_FILE *s, int i, char *vartype, int varlen, char *varname) {
    ...
    if (i > (*s).m1 - 1) {
        muste_sprintf(sbuf, "\nMax.# of fields in current file is %d", (*s).m1);
        sur_print(sbuf);
        muste_sprintf(sbuf, "\nNot space for field %.8s", varname);
        sur_print(sbuf);
        muste_sprintf(sbuf, "\nIncrease capacity by FILE EXPAND <data_file>");
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
    if (i > (*s).m - 1) {
        ...
        if ((*s).varpos[i] + varlen > (*s).len) {
            ...
            muste_sprintf(sbuf, " %.8s", varname);
            sur_print(sbuf);
            muste_sprintf(sbuf, "\nIncrease record length %d by FILE EXPAND <data_file>.", (*s).len);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
        strcpy(jakso, varname);
        jakso[8] = EOS;
        k = strlen(jakso);
        for (h = k; h < 8; ++h) {
            jakso[h] = ' ';
    ...
    h = 0; // RS 5.2.2013
    *(int *)(jakso + h) =
        (int)(*s).varpos[i]; // RS FIXME dereferencing type-punned pointer will break strict-aliasing rules
    *(int *)(jakso + 2) = (*s).varlen[i];
    ...
    fi_rewind(s);
    fi_puts(s, jakso, (*s).l + (*s).extra, (long)((*s).var + (long)i * ((long)(*s).l + (long)(*s).extra)));
    return (1);
}
...
int fi_value_to_string(SURVO_DATA_FILE *s, int i, double x, char *sana) {
    ...
    if (fabs(x) > MISSING8 / 1000.0) {
        ...
        if (tarkkuus == 1) {
            ...
            if (tarkkuus > accuracy + 2) {
                tarkkuus = accuracy + 2;
            }
            fnconv(x, tarkkuus, sana);
        }
        if (strlen(sana) > varlen) {
            muste_fieldcopy(sana, space, varlen);
        }
    }
    return (1);
...
void fi_save(SURVO_DATA_FILE *s, long j, /* havainnon nro 1,2,3,... */
    ...
    if (j < 1L) {
        sur_print("Survo saving error!"); /* getch(); */
        return;
    }
    p = (char *)jakso;
    switch ((*s).vartype[i][0]) {
    ...
    case '2':
        if (*(double *)sana < -32768.0 || *(double *)sana > 32767.0) {
            //                        *(short *)jakso=(unsigned short)MISSING2; // RS FIXME dereferencing type-punned
            //                        pointer will break strict-aliasing rules
            *jakso2 = (unsigned short)MISSING2; // RS 29.1.2013
        } else {
            //     //                 *(short *)jakso=(unsigned short)*(double *)sana; // RS FIXME dereferencing
            //     type-punned pointer will break strict-aliasing rules
            *jakso2 = (unsigned short)*(double *)sana; // RS 29.1.2013
        }
        pit = 2;
        p = (char *)jakso2; // RS 29.1.2013 p=
        break;
    case '4': // *(float *)jakso=(float)*(double *)sana; // RS FIXME dereferencing type-punned pointer will break
              // strict-aliasing rules
        *jakso4 = (float)*(double *)sana; // RS 29.1.2013
        pit = 4;
        p = (char *)jakso4; // RS 29.1.2013 p=
        break;
    case '8':
        p = (char *)sana;
        pit = 8;
...
void fi_alpha_load(SURVO_DATA_FILE *s, long j, /* havainnon nro 1,2,3,... */
                   int i,                      /* muuttuja 0,1,2,... */
...
int fi_load(                            /* numeerinen */
            SURVO_DATA_FILE *s, long j, /* havainnon nro 1,2,3,... */
            int i,                      /* muuttuja 0,1,2,... */
    ...
    case '2':
        *px = (double)*(
            (short *)(jakso + h)); // RS FIXME dereferencing type-punned pointer will break strict-aliasing rules
        if (*px == MISSING2) {
            *px = MISSING8;
        }
        return (1);
    case '4':
        if (not_float((unsigned char *)jakso)) {
            *px = MISSING8;
            return (1);
        }
        *px = (double)*(
            (float *)(jakso + h)); // RS FIXME dereferencing type-punned pointer will break strict-aliasing rules
        if (*px > MISSING4 / 10.0) {
            *px = MISSING8;
        }
        return (1);
    case '8':
        if (not_double((unsigned char *)jakso)) {
            *px = MISSING8;
            return (1);
        }
        *px = (double)*(double *)(jakso +
                                  h); // RS FIXME dereferencing type-punned pointer will break strict-aliasing rules
        return (1);
    case 'S':
        if (strncmp(jakso, space, (unsigned int)(*s).varlen[i]) != 0) {
            *px = atof(jakso);
        } else {
            *px = MISSING8;
        }
...
int fi_open3(char *nimi, SURVO_DATA_FILE *s, int laaja, /* 1= m1 muuuttujaa 0= m muuttujaa */
    ...
    if (strncmp(alku, "SURVO 84C DATA", 14) != 0 && strncmp(alku, "MUSTE DATA", 10) != 0) // RS ADD 13.7.2012
    {
        muste_sprintf(jakso, "%s is not a Survo data file!", pathname);
        if (etu == 2) {
            muste_sprintf(tut_info, "___@2@FILE OPEN@%s@", jakso);
            return (-1);
        }
        PR_EINV;
        muste_sprintf(sbuf, "%s", jakso);
    ...
    if (!tekstitieto) {
        ...
        for (i = 0; i < (*s).textn; ++i) {
            ...
            // Rprintf("\nop31 pit: %u,paikka:
            // %u",(long)(*s).textlen,(long)((long)(*s).text+(long)i*(long)(*s).textlen));
    ...
    for (i = 0; i < (*s).m; ++i) {
        ...
        fi_rewind(s);
        // Rprintf("\nop32 npit: %u,paikka:
        // %u",(long)l,(long)((long)(*s).var+(long)i*((long)(*s).l+(long)(*s).extra)+(long)(*s).extra));

        fi_gets(s, jakso, (long)l,
                (long)((long)(*s).var + (long)i * ((long)(*s).l + (long)(*s).extra) + (long)(*s).extra));
        // RS (long)
        // RS 64-bit kokeilu                muste_sprintf(sbuf,"%u",(unsigned
        // int)((*s).var+(long)i*((long)(*s).l+(long)(*s).extra)+(long)(*s).extra));
    ...
    for (i = 0; i < (*s).m; ++i) {
        ...
        // RS (long)         (long)
        // RS 64-bit kokeilu                muste_sprintf(sbuf,"%u",(unsigned
        // int)((*s).var+(long)i*((long)(*s).l+(long)(*s).extra)));
    ...
    if (laaja) {
        for (i = (*s).m; i < (*s).m1; ++i) {
            (*s).vartype[i] = p + (long)i * ((long)(*s).extra - 4L + 1L);
        }
    }
    for (i = 0; i < (*s).m; ++i) {
        (*s).vartype[i] = p;
        ...
        // Rprintf("\nop34 npit: %u,paikka:
        // %u",(long)((*s).extra-4),(long)((long)(*s).var+(long)i*((long)(*s).l+(long)(*s).extra)+4L));

        fi_gets(s, jakso, (long)((*s).extra - 4),
                (long)((long)(*s).var + (long)i * ((long)(*s).l + (long)(*s).extra) + 4L));
        // RS (long)                      (long)

        // RS 64-bit kokeilu        muste_sprintf(sbuf,"%u",(unsigned
        // int)((*s).var+(long)i*((long)(*s).l+(long)(*s).extra)+4L));
        //        fi_gets(s,jakso,(*s).extra-4,atoi(sbuf));

        for (h = 0; h < (*s).extra - 4 - muste_posextra; ++h) {
            *p++ = jakso[h]; // RS ADD 23.5.2012 muste_posextra
        }
        *p++ = EOS;
        //            if (muste_posextra && (*s).varpos[i]>32750) Rprintf("\npos %d: %d",i,*(int *)(jakso+h));
        if (muste_posextra && (*s).varpos[i] > 32750) {
            (*s).varpos[i] = (unsigned int)(*(int *)(jakso + h)); // RS ADD 23.5.2012
        }
    }
...
int fi_open2(char *nimi, SURVO_DATA_FILE *s, int laaja, /* 1= m1 muuuttujaa 0= m muuttujaa */
             int kokonimet,                             /* 1= koko nimi  0=vain kutsumanimet (8 merkkiä) */
             int tekstitieto                            /* 1= tekstiosa luetaan 0=ei tekstiosaa */
) {
...
int fi_open(char *nimi, SURVO_DATA_FILE *s) { return (fi_open3(nimi, s, 0, 0, 0, 1)); }

void fi_alpha_save(SURVO_DATA_FILE *s, long j, /* havainnon nro 1,2,3,... */
                   int i,                      /* muuttuja 0,1,2,... */
                   char *jakso                 /* luettava tieto */
) {

    // Rprintf("\npaikka: %d, pos: %d, jakso:
    // %s",((*s).data+(j-1L)*(long)(*s).len+(long)(*s).varpos[i]),(*s).varpos[i],jakso);
    fi_puts(s, jakso, (*s).varlen[i], (long)((*s).data + (j - 1L) * (long)(*s).len + (long)(*s).varpos[i]));
}
...
int fi_create(char *filename, int filen, int fim1, int fim, long fin, int fil, int fiextra, int fitextn, int fitextlen,
              char *fitext[], char *varname[], int varlen[], char *vartype[]) {
    int i, h;
    ...
    if (sur_find_file(pathname)) // RS CHA survo_data!=NULL)
    {
        if (erun == 0 && etu == 0) {
            ...
            muste_sprintf(sbuf, "\nFile %s already exists!", pathname);
            sur_print(sbuf);
            sur_print("\nOverwrite (Y/N)? ");
            i = 0;
            while (i < 31 || i > 256) {
    ...
    *(int *)(jakso + 46) = filen; // RS ADD 23.5.2012
    for (i = 50; i < 64; ++i) {
        jakso[i] = ' '; // RS CHA 23.5.2012 46->50
    }

    i = talleta(jakso, 64, 0);
    if (i < 0) {
        return (-1);
    }

    for (i = 0; i < fitextn; ++i) {
        if (strlen(fitext[i]) > 249) {
            fitext[i][249] = EOS;
        }
        strcpy(jakso, fitext[i]);
        for (h = strlen(fitext[i]); h < fitextlen; ++h) {
            jakso[h] = ' ';
        }
    ...
    if (h > filen) {
        muste_sprintf(sbuf, "\nRecord length %d too small (%d required, at least)", filen, h);
        sur_print(sbuf);
        WAIT;
        muste_fclose(survo_data);
        cre_del(pathname);
    ...
    for (i = 0; i < fim; ++i) {
        for (h = 0; h < i; ++h) {
            if (strncmp(varname[i], varname[h], 8) == 0) {
                if (strncmp(varname[i], varname0, 8) != 0) {
                    ...
                    sur_print(sbuf);
                    *varname0 = EOS;
                    muste_fieldcopy(varname0, varname[i], 8);
                    varname_error = 1;
                }
            }
        }
        ...
        {
            *(short *)(jakso + h) =
                (short)32751; // RS FIXME dereferencing type-punned pointer will break strict-aliasing rules
        } else {
            *(short *)(jakso + h) =
                (short)pos; // RS FIXME dereferencing type-punned pointer will break strict-aliasing rules
        }
        *(short *)(jakso + 2) = varlen[i];
        ...
        if (muste_posextra) {
            *(int *)(jakso + fiextra - muste_posextra) = pos; // RS ADD 23.5.2012
        }
        pos += varlen[i];
        for (h = 0; h < fil; ++h) {
            jakso[h + fiextra] = ' ';
...
int create_newvar1(SURVO_DATA *d, char *name, char type, int len, char act) // 29.8.2000
{
    ...
    if (d->type != 2) {
        muste_sprintf(sbuf, "\nNew variable %s cannot be created!", name);
        sur_print(sbuf);
        WAIT;
        return (-1);
    }
...
void rem_update(SURVO_DATA *d, char *key, char *text) {
    ...
    if (p == NULL) {
        ...
        for (i = d->d2.textn - 1; i >= 0; --i) {
            ...
            if (p == NULL || p - d->d2.fitext[i] > c3 - 1) {
                continue;
            }
            strcpy(x, key);
            strcat(x, text);
    ...
    for (i = 0; i < d->d2.textn; ++i) {
        fi_rewind(&d->d2);
        fi_puts(&d->d2, d->d2.fitext[i], d->d2.textlen, (long)(d->d2.text + (long)i * (long)(d->d2.textlen)));
    }
}
...
int ma_missing(char *p) {
    ...
    len = strlen(p);
    puuttuu = 0;
    if (strncmp(p, space, (unsigned int)len) == 0) {
        puuttuu = 1;
    } else {
        q = strchr(p, '-');
        if (q != NULL) {
            if (*(q + 1) == ' ' || *(q + 1) == '-' || q - p == len - 1) {
                puuttuu = 1;
            }
        }
    }
...
int ma_load(SURVO_DATA_MATRIX *s, int j, int i, double *px, int alpha) /* 1=alpha 0=num */
{
    int k;
    char *sana[EP4];
    char *p, *r;
    ...
    */
    ...
    if (s->mask != NULL) {
        ...
        if ((s->varpos[i] > 1 && *(r - 1) != ' ') || *(r + s->varlen[i]) != ' ') {
            muste_sprintf(sbuf, "\nIncorrect mask for data on edit line %d!", s->l1 + j - 1);
            if (dsp) {
                return (-1);
            }
            sur_print(sbuf);
...
/* int drivi;    DATA line   0=unknown  */
{
    ...
    if (!drivi) {
        ...
        if (drivi < 0) {
            muste_sprintf(sbuf, "\nDATA %s not found in the edit field", name);
            sur_print(sbuf);
            WAIT;
            return (-1);
        }
    ...
    if (gdat == 2) /* vain DATA <nimi> */
    {
        ...
        while (l2 < r2 + 1) {
            edread(rivi, l2);
            if (strncmp(rivi + 1, space, (unsigned int)c2) == 0) {
                break;
            }
            ++l2;
        }
        ...
        l1 = edline2(dsana[2], 1, 1);
        if (l1 == 0) {
            return (-1);
        }
        l2 = edline2(dsana[3], 1, 1);
        if (l2 == 0) {
            return (-1);
        }
        lx = edline2(dsana[4], 1, 1);
        if (lx == 0) {
            return (-1);
        }
    }
    ...
    */
    labtila = 10 * m;
    if (labtila < 2 * LLENGTH) {
        labtila = 2 * LLENGTH; /* 8.3.1991 */
    }
    ...
    p += m * sizeof(char *);
    for (i = 0; i < m; ++i) {
        s->vartype[i] = tyypit + (TYPELEN + 1) * i;
    }
    s->varlen = (short *)p;
    p += m * sizeof(short);
    s->varpos = (int *)p;
    p += m * sizeof(int); // RS 28.1.2013 varpos short -> int
    masktila = p;
    p += LLENGTH;
    s->mask = (char **)p;
    p += m * sizeof(char *);
    ...
    if (gdat < 6) {
        ...
        if (i < m) {
            muste_sprintf(sbuf, "\nIncorrect mask line for DATA %s", name);
            sur_print(sbuf);
            if (dsp) {
                return (-1);
            }
...
int matr_open(char *name, SURVO_DATA *d) {
    ...
    */
    tila = m * (8 + 1) + m * (TYPELEN + 1) + 2 * m * sizeof(char *) + 2 * m * sizeof(int);
...
int matr_load(SURVO_DATA *d, int j, int i, double *px) {
    ...
    for (h = 0; h < sizeof(double); ++h) {
        *p++ = (char)getc(d->d2.survo_data);
    }
    if (i == 0) {
        muste_fieldcopy(s, (char *)px, 8);
        s[8] = EOS;
        *px = atof(s);
    }
    return (1);
...
int matr_alpha_load(SURVO_DATA *d, int j, int i, char *s) {
    ...
    if (i != 0) {
        sur_print("\nOnly CASE to be used as string variable!");
        WAIT;
        return (-1);
    }
...
int matrix_space(double **A,  /* matriisitila (alkuosoite) (malloc) */
                 int m,       /* rivien lkm */
                 int n,       /* sar. lkm   */
...
int matrix_load(char *matr,  /* matriisin nimi */
    ...
    *type = atoi(osa[6]);
    ...
    mrl = *lr;
    mcl = *lc;
    i = matrix_space(A, m, n, rlab, clab, mrl, mcl);
    if (i < 0) {
        return (-1);
    }
    a = *A;
...
int mat_load(char *matr,  /* matriisin nimi */
             double **A,  /* matriisitila (alkuosoite) (muste_malloc) */
             int *rdim,   /* rivien lkm */
             int *cdim,   /* sar. lkm   */
             char **rlab, /* rivien otsikot (muste_malloc) */
...
char *matrix_label(char *lablist, int l, int len, int i, char *label) {
    ...
    while (lablist[i * l + h] == ' ' && h < l) {
        ++h;
    }

    while (h < l && k < len) {
        label[k] = lablist[i * l + h];
        ++k;
        ++h;
    }
    label[k] = EOS;
...
int matrix_print(double *A, int m, int n, char *rlab, char *clab, int lr, int lc, int m2, int n2, int *mv, int *nv,
                 char *form, int width, int editline, char *outfile, char *header)
...

{
    ...
    output_open(outfile);
    output_line(header, outfile, editline);
    if (editline) {
        ++editline;
    }

    if (*form == '%') {
        len = muste_sprintf(x, form, fabs(A[0])) + 1;
    } else {
        len = strlen(form);
    }
    ...
    while (1) {
        ...
        output_line(x, outfile, editline);
        if (editline) {
            ++editline;
        }
        for (i = i1; i <= i2; ++i) {
            ...
            for (j = j1; j <= j2; ++j) {
                ...
                if (h < 0) {
                    muste_fieldcopy(label, space, len);
                    label[len - 1] = '-';
                }
...
int sample_open(char *name, SURVO_DATA *d, int drivi) {
    ...
    */
    ...
    /*
            ...
            int *varlen; m*sizeof(int);
    */
    tila = m * n * sizeof(double) + m * (8 + 1) + m * (TYPELEN + 1) + 2 * m * sizeof(char *) + 2 * m * sizeof(int);
    d->pspace = muste_malloc(tila);
    if (d->pspace == NULL) {
        tilavirhe();
        return (-1);
...
int fidata_open2(char *name, SURVO_DATA *d, int p1, int p2, int p3, int kirjoitus) {
    ...
    d->type = 2;
    i = fi_open3(name, &(d->d2), p1, p2, p3, kirjoitus);
    if (i < 0) {
        return (-1);
    }
    d->m = d->d2.m;
...
int fidata_open(char *name, SURVO_DATA *d, int p1, int p2, int p3) { return (fidata_open2(name, d, p1, p2, p3, 1)); }
    ...
    if (pch2 == NULL) {
        ...
        while (drivi <= r2) {
            ...
            if (i > 1 && strcmp("DATA", dsana[0]) == 0) {
                if (strncmp(name, dsana[1], strlen(name)) ==
                    0) // RS REM strupr  if (strncmp(name,muste_strupr(dsana[1]),strlen(name))==0)
                {
                    if (strlen(name) == strlen(dsana[1])) {
                        break;
                    }
                    if (dsana[1][strlen(name)] == ':') {
                        i = sample_open(nimi, d, drivi);
                        return (i);
                    }
...
int data_open2(char *nimi, SURVO_DATA *d, int p1, int p2, int p3) { return (data_open3(nimi, d, p1, p2, p3, 1)); }

int data_open(char *name, SURVO_DATA *d) { return (data_open2(name, d, 0, 0, 0)); }

int data_read_open(char *name, SURVO_DATA *d) { return (data_open3(name, d, 0, 0, 0, 0)); }
...
void ma_save(SURVO_DATA_MATRIX *s, int j, int i, char *sana) {
    ...
    if (j > ed2 || j < 1 || s->mask == NULL) {
        return;
    }
    /* 4.9.89 */

    jj = k = (s->l1 + j - 2) * ed1 + s->varpos[i];
    for (ii = 0; ii < s->varlen[i]; ++ii) {
        z[jj++] = ' ';
    }
    jj = k;
    for (ii = 0; ii < s->varlen[i]; ++ii) {
...
int data_save(SURVO_DATA *d, long j, int i, double x) {
    ...
    if (d->type == 2) {
        ...
        if (d->vartype[i][2] == 'P') {
            ...
            if (etu == 2) {
                muste_sprintf(tut_info, "___@4@DATA SAVE@%s@", sbuf);
                return (-1);
            }
            sur_print("\n");
            sur_print(sbuf);
...
int data_load(SURVO_DATA *d, long j, int i, double *px) {
    ...
    if (d->type == 3) {
        sa_load(d, (int)j, i, px);
        return (1);
    }
    if (d->type == 4) {
        matr_load(d, (int)j, i, px);
        return (1);
    }
...
int varfindlong(SURVO_DATA *d, char *nimi, int maxlen) {
    ...
    muste_fieldcopy(nimi2, nimi, len);
    nimi2[len] = EOS; // RS ADD
    for (i = 0; i < d->m; ++i) {
...
int data_alpha_load(SURVO_DATA *d, long j, int i, char *sana) {
    ...
    if (d->type == 3) {
        sur_print("\ndata_alpha_load not in samples!");
        return (-1);
    }
    if (d->type == 4) {
        matr_alpha_load(d, (int)j, i, sana);
        return (1);
    }
...
int bool_mult(char *s, char *pt, char *qt, char *p, char *q) {
    ...
    *x = EOS;
    strncat(x, p, (unsigned int)(q - p) + 1);
    ...
    while (pt < qt) {
        ...
        *y = EOS;
        strncat(y, pt, (unsigned int)(p2 - pt) + 1);
        if (i) {
            strcat(s, "+");
        }
        strcat(s, y);
...
char *bool_neg(char *s, char *pt) {
    ...
    case '(':
        if (*(pt + 1) == '!') { /* poista !! */
            ...
            strcat(s, "!");
            strncat(s, pt + 1, (unsigned int)(q - pt) - 1);
        }
        break;
    ...
    if (*pk != '(') {
        strcat(s, "!(");
        strncat(s, pt + 1, (unsigned int)(pk - pt) - 1);
        strcat(s, ")");
        strcat(s, m);
        strcat(s, "!(");
        strncat(s, pk + 1, (unsigned int)(q - pk) - 1);
        strcat(s, ")");
    }
    return (q + 1);
}
    ...
    for (samecondloop = 0; samecondloop < samecond; samecondloop++) {
        ...
        if (p == NULL) /* IND-tyyppinen */
        {
            ...
            if (strcmp(nimi, "CASES") == 0) {
                muste_sprintf(sbuf,
                              "CASES should not be used with ind-type (,) conditions!\nUse IND or SELECT instead.");
                if (etu == 2) {
                    muste_sprintf(tut_info, "___@11@CONDITIONS@%s@", sbuf);
                    return (-2); // RS CHA exit(1) -> return(-2)
                }
                sur_print("\n");
                sur_print(sbuf);
                return (-2);
            ...
            if (k > 1) {
                if (strcmp(sana[1], "!MISSING") == 0) /* 3.1.2003 */
                {
                    ...
                    if (strcmp(sana[1], "MISSING") == 0) {
                        a = MISSING8; /* 31.12.2002 */
                    } else if (strcmp(sana[1], "MIN") == 0) {
                        a = -1e300;
                    ...
                    if (k > 2) {
                        if (strcmp(sana[2], "MAX") == 0) {
                            sel_upper[nro] = 1e300; // RS ADD
        ...
        else /* CASES-tyyppinen */
        {
            ...
            *p = EOS;
            ...
            if (strcmp(nimi, "IND") == 0) {
                muste_sprintf(sbuf,
                              "IND should not be used with cases-type (:) conditions!\nUse CASES or SELECT instead.");
                if (etu == 2) {
                    muste_sprintf(tut_info, "___@11@CONDITIONS@%s@", sbuf);
                    return (-2); // RS CHA exit(1) -> return(-2)
                }
...
int conditions(SURVO_DATA *d) {
    ...
    for (k = 2; k < n_select; ++k) {
        ...
        if (*q) {
            sel_rel[muste_var_nro + k + 1] = *q; // RS CHA [k+1]
        }
        i = q - p;
        muste_fieldcopy(s, p, (unsigned int)i);
        s[i] = EOS;
        p = q + 1;
        i = find_cond(d, s, k);
        if (i < 0) {
...
int unsuit(SURVO_DATA *d, long l, int nro) {
    ...
    if (i) {
        ...
        for (i = 0; i < len; i++) {
            if (atoi(term[i]) == (int)x) {
                return (t_neg(0, nro));
            }
        }
        return (t_neg(1, nro));
    } else {
        data_alpha_load(d, l, sel_var[nro], sana + 1);
        *sana = ',';
        len = strlen(sana);
        while (sana[len - 1] == ' ') {
            sana[--len] = EOS;
            ...
            if (strchr(p, cases_wild) != NULL || strchr(p, cases_wild2) != NULL) {
                ...
                */
                while (1) {
                    ...
                    if (q != NULL) {
                        *sbuf = EOS;
                        strncat(sbuf, p, (unsigned int)(q - p));
                    } else {
                        strcpy(sbuf, p);
                    }
                    /*           Rprintf("\nsbuf=%s|",sbuf); getch(); */
        ...
        if (strncmp(p, sana + 1, (unsigned int)(len - 1)) == 0) {
            return (t_neg(0, nro));
        }
...
int unsuitable(SURVO_DATA *d, long l) {
    ...
    if (sel_var[0] > -2 && unsuit(d, l, 0)) {
        return (1);
    }
    if (sel_var[1] > -2 && unsuit(d, l, 1)) {
        return (1);
    }
    ...
    *maskset = EOS;
    if (*spb[i] == '#') {
        ...
        if (p != NULL) {
            ...
            muste_fieldcopy(maskset, p + 1, (unsigned int)(q - p - 1));
            maskset[q - p - 1] = EOS;
        }
...
int matrix_save0(char *matr,  /* matriisin nimi */
...
int matrix_save(char *matr, /* matriisin nimi */
                double *A,  /* matriisitila */
                int m,      /* rivien lkm */
                int n,      /* sar. lkm   */
                char *rlab, /* rivien otsikot */
    ...
    i = matrix_save0(matr, A, m, n, rlab, clab, mrl, mcl, type, expr, nrem, remrivi, NULL);
    return (i);
}
