...
static int lue_havainnot(void);
static int havainnot_muistiin(void);
...
static int ortogonalisoi(void);
static int tulosta(void);
static int init_gr(void);
static int vertaa_muihin(void);
static int kopioi_gr(int k);
static int eoutput(char *rivi);
static int init_tilat(void);
static int iteroi(void);
static int init_obs(int gr);
static int sift(int gr, int d);
static int shift(int gr, int d);
static int alustava_luokittelu(void);
...
void muste_cluster(char *argv) {
    ...
    i = spfind("TEMPFILE");
    if (i >= 0) {
        strcpy(tempfile, spb[i]);
    } else {
        strcpy(tempfile, etmpd);
        strcat(tempfile, "SURVO.CLU");
    }
...
static int shift(int gr, int d) {
    ...
    for (i = 0; i < ng; ++i) {
        v1[i] = v2[i] - Q[i + ng2 * gr] + Q[i + ng2 * d]; /* b(d) */
    }
    vsd = vss - v2[gr] + v2[d];
...
static int rand_init() {
    ...
    seed = atol(p + 1);
    if (muste_strnicmp(s, "rnd(", 4) == 0) {
        type = 0;
        return (1);
    }
    if (muste_strnicmp(s, "rand(", 5) == 0) {
        type = 1;
        return (1);
    }
    if (muste_strnicmp(s, "urand(", 6) == 0) {
        type = 2;
        return (1);
    }
...
static double rand_uniform() {
    ...
    case 1:
        if (!next) {
            sur_rand_seed((unsigned int)seed);
            next = 1;
        }
        return (sur_rand());
    case 2:
        if (!next) {
            sur_urand_seed((unsigned int)seed);
            next = 1;
        }
        return (sur_urand());
    }
