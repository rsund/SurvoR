...
write.svo <- function(dataf, svofile) {
  rval <- .Call("do_writeSurvo", as.data.frame(dataf), as.character(svofile),
    as.character(deparse(substitute(dataf))),
    PACKAGE = "muste"
  )
}
  ...
  tryCatch(
    {
      if (.muste$sysname == "Windows") {
        clipb <- try(paste(readClipboard(), collapse = "\n"))
      } else {
  ...
  if (identical(K, "EuroSign")) # Special handling for euro (as in CP858)
    {
      nonascii <- FALSE
      N <- as.integer(213)
    } else if (length(latinchar) == 1) {
    if (latinchar == 163) # Special handling for pound in Mac
      {
        nonascii <- FALSE
        N <- as.integer(156)
      }
  } else if (length(latinchar) == 3) {
...
.muste.command <- function(command = "Exit", force = FALSE) {
  if (.muste$exitok || force) {
    ...
    if (command[[1]] == "Exit") {
      .muste$exitpressed <- as.integer(1)
    } else {
...
.muste.init <- function() {
  ...
  if (.muste$sysname == "Windows") {
    ...
    .muste$Rbin <- paste(file.path(R.home("bin"), "R"))
...
muste <- function(sucro = "<empty>", config = "<empty>") {
  ...
  if (getRversion() >= "2.14.0") {
    ...
    .muste.command(c("Require", "tcltk"), force = TRUE)
  }
