.muste.highlightstyles <- function(x) {
  ...
  styles[desc %in% c(
    "EQ_ASSIGN", "LEFT_ASSIGN",
    "RIGHT_ASSIGN", "COLON_ASSIGN"
  )] <- "assignement"
...
.muste.rbuffonts <- function() {
  ...
  .muste$fontb <- tcltk::tkfont.create(
    f[1], f[2], f[3], as.character(as.numeric(f[4]) - 1),
  ...
  f <- paste(tcltk::tkfont.actual(.muste$font))
  .muste$fonti <- tcltk::tkfont.create(
    f[1], f[2], f[3], as.character(as.numeric(f[4]) - 1),
