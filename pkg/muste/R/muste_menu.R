...
.muste.yscrollbar <- function(visible = TRUE) {
  if (visible) {
    ...
    tcltk::tkgrid.remove(.muste$scry)
  }
}
...
.muste.loadedt <- function() {
  file_name <- tcltk::tclvalue(tcltk::tkgetOpenFile(
    filetypes = "{{Survo edit fields} {.EDT}} {{All files} *}",
...
.muste.savedtname <- function() {
  ...
  saveFile <- tcltk::tclvalue(tcltk::tkgetSaveFile(
    filetypes = "{{Survo edit fields} {.EDT}} {{All files} *}",
...
.muste.close <- function() {
  ...
  response <- tcltk::tclvalue(tcltk::tkmessageBox(
    message = "Exit from Survo?",
