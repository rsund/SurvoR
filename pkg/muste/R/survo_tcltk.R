.muste.mousewheel <- function(t, X, Y, D, s) {
  ...
  #############  mac=1   ############	 windows=9 ##########
  if (as.integer(s) == 1 || as.integer(s) == 9) {
    .muste.xview(.muste$scrx, "scroll", delta, "units")
  } else {
...
.muste.inittkwindow <- function() {
  ...
  if (.muste$sysname == "Darwin") {
    .muste$font <- tcltk::tkfont.create(family = "Courier", size = 14)
  }
  #  RS Menlo font is no longer working 2026-07-01 # if (.muste$sysname=="Darwin") { .muste$font <- tkfont.create(family="Menlo",size=14) }
    ...
    .muste$font <- tcltk::tkfont.create(family = "Courier", size = 12)
  }

  #  .muste$fakefont <- tcltk::tkfont.create(family="Courier",size=4)

  #  .muste.menu()
  .muste$txt <- tcltk::tktext(.muste$ikkuna,
    width = 80, height = 25, foreground = "#000000", background = "snow",
...
.muste.remove.bindings <- function() {
  ...
  for (i in 1:length(bindvec)) {
    tcltk::tkbind(.muste$txt, bindvec[i], "")
  }
}
