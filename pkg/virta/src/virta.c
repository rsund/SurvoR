...
****************************************************************************/
static int test_CreateConnect(sqlite3 *db, void *pAux, int argc, const char *const argv[], sqlite3_vtab **ppVTab,
                              char **pzErr) {
    int err;
...
static int test_Rename(sqlite3_vtab *pVTab, const char *zNew) { return SQLITE_OK; }
...
static int svo_connect(sqlite3 *db, void *pAux, int argc, const char *const argv[], sqlite3_vtab **ppVTab,
                       char **pzErr) {
    (void)pAux; /* Unused */
    int i, k, vi, nvar;
    char ch;
    ...
    if (len > 0) {
        para = xbuf + len - 1;
        while (para >= xbuf && (*para == ' ' || *para == '"'))
            *para-- = EOS;
    }
    para = xbuf;
    while (para < xbuf + len && (*para == ' ' || *para == '"'))
        para++;
    ...
    for (i = 0; i < nvar; i++) {
        ...
        if (ch >= '0' && ch <= '9')
            muste_snprintf(xbuf, svodata->d2.l - 1, "_%s", svodata->varname[vi]);
        else
            muste_snprintf(xbuf, svodata->d2.l - 1, "%s", svodata->varname[vi]);
...
static int svo_readcolumn(sqlite3_vtab_cursor *pCursor, sqlite3_context *context, int N) {
    ...
    case '2':
        ...
        if (a == MISSING8)
            ...
            sqlite3_result_int(context, (int)a);
        break;
    default:
